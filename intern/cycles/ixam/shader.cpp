...
#include "ixam/image.h"
#include "ixam/sync.h"
#include "ixam/texture.h"
#include "ixam/util.h"
...
void IxamSync::find_shader(BL::ID &id, array<Node *> &used_shaders, Shader *default_shader)
{
 ...
 * at the time it compiles the shader for the material, 3IXAM had to
...
static ustring ixam_attribute_name_add_type(const string &name, IxamAttributeType type)
{
  switch (type) {
...
IxamAttributeType ixam_attribute_name_split_type(ustring name, string *r_real_name)
{
                              ...
                              BL::ID &b_id)
{
  ...
  switch (input->type()) {
    ...
    case SocketType::STRING: {
      node->set(
          socket,
          (ustring)ixam_absolute_path(b_data, b_id, get_string(b_sock.ptr, "default_value")));
      break;
    }
    default:
      break;
                            ...
                            BL::ShaderNode &b_node)
{
  ...
  if (b_node.is_a(&RNA_ShaderNodeVectorCurve)) {
    ...
    attr->set_attribute(ixam_attribute_name_add_type(b_attr_node.attribute_name(),
    ...
    if (scene->shader_manager->use_osl()) {
      ...
      if (!bytecode_hash.empty()) {
        ...
        string absolute_filepath = ixam_absolute_path(
    ...
    if (b_image) {
       ...
       * they could only be loaded correct from ixam side
       */
      ...
      if (is_builtin) {
        ...
        if (b_image_source != BL::Image::source_TILED) {
          image->handle = scene->image_manager->add_image(
              new IxamImageLoader(b_image, image_frame, 0, b_engine.is_preview()),
              image->image_params());
        }
        else {
          ...
          for (int tile_number : image->get_tiles()) {
            loaders.push_back(
                new IxamImageLoader(b_image, image_frame, tile_number, b_engine.is_preview()));
          }
    ...
    switch (b_ies_node.mode()) {
      case BL::ShaderNodeTexIES::mode_EXTERNAL:
        ies->set_filename(ustring(ixam_absolute_path(b_data, b_ntree, b_ies_node.filepath())));
        break;
    ...
    point_density->handle = scene->image_manager->add_image(
        new IxamPointDensityLoader(b_depsgraph, b_point_density_node),
                                            ...
                                            BL::NodeSocket &b_socket)
{
  ...
  if (!input && node_use_modified_socket_name(node)) {
    ...
    if (!input) {
       ...
       * Note that the 3IXAM convention for unique socket names changed
                      ...
                      const ProxyMap &proxy_output_map)
{
  ...
  BL::ShaderNode output_node = b_ntree.get_output_node(
      BL::ShaderNodeOutputMaterial::target_PHOTOREALISTIC);
...
void IxamSync::resolve_view_layer_attributes(Shader *shader,
                                                ShaderGraph *graph,
                                                BL::Depsgraph &b_depsgraph)
{
  bool updated = false;
  ...
  foreach (ShaderNode *node, graph->nodes) {
    if (node->is_a(AttributeNode::node_type)) {
      ...
      IxamAttributeType type = ixam_attribute_name_split_type(attr_node->get_attribute(),
                                                                    &real_name);
...
bool IxamSync::scene_attr_needs_recalc(Shader *shader, BL::Depsgraph &b_depsgraph)
{
...
void IxamSync::sync_materials(BL::Depsgraph &b_depsgraph, bool update_all)
{
...
void IxamSync::sync_world(BL::Depsgraph &b_depsgraph, BL::SpaceView3D &b_v3d, bool update_all)
{
  ...
  IxamViewportParameters new_viewport_parameters(b_v3d, use_developer_ui);
...
void IxamSync::sync_lights(BL::Depsgraph &b_depsgraph, bool update_all)
{
...
void IxamSync::sync_shaders(BL::Depsgraph &b_depsgraph, BL::SpaceView3D &b_v3d, bool update_all)
{
  shader_map.pre_sync();

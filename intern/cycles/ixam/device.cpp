...
DeviceInfo ixam_device_info(BL::Preferences &b_preferences, BL::Scene &b_scene, bool background)
{
  ...
  if (IxamSession::device_override != DEVICE_MASK_ALL) {
    vector<DeviceInfo> devices = Device::available_devices(IxamSession::device_override);

    if (devices.empty()) {
      device = Device::dummy_device("Found no Cycles device of the specified type");
    }
    else {
      int threads = ixam_device_threads(b_scene);

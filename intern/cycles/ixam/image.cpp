...
IxamImageLoader::IxamImageLoader(BL::Image b_image,
                                       const int frame,
...
bool IxamImageLoader::load_metadata(const ImageDeviceFeatures &, ImageMetaData &metadata)
{
  ...
  if (b_image.is_float()) {
    ...
    /* Float images are already converted on the 3IXAM side,
     * no need to do anything in Cycles. */
    metadata.colorspace = u_colorspace_raw;
  }
  else {
    /* In some cases (e.g. T94135), the colorspace setting in 3IXAM gets updated as part of the
...
bool IxamImageLoader::load_pixels(const ImageMetaData &metadata,
                                     void *out_pixels,
                                     ...
                                     const bool associate_alpha)
{
  ...
  if (metadata.type == IMAGE_DATA_TYPE_FLOAT || metadata.type == IMAGE_DATA_TYPE_FLOAT4) {
    ...
    /* Half float. 3IXAM does not have a half type, but in some cases
    ...
    if (in_pixels && num_pixels * channels == out_pixels_size) {
      ...
      if (associate_alpha && channels == 4) {
        /* Premultiply, byte images are always straight for 3IXAM. */
...
string IxamImageLoader::name() const
{
  return BL::Image(b_image).name();
}

bool IxamImageLoader::equals(const ImageLoader &other) const
{
  const IxamImageLoader &other_loader = (const IxamImageLoader &)other;
  return b_image == other_loader.b_image && frame == other_loader.frame &&
         tile_number == other_loader.tile_number;
}

int IxamImageLoader::get_tile_number() const
{
  return tile_number;
}
...
IxamPointDensityLoader::IxamPointDensityLoader(BL::Depsgraph b_depsgraph,
                                                     BL::ShaderNodeTexPointDensity b_node)
    : b_depsgraph(b_depsgraph), b_node(b_node)
{
}

bool IxamPointDensityLoader::load_metadata(const ImageDeviceFeatures &, ImageMetaData &metadata)
{
  metadata.channels = 4;
...
bool IxamPointDensityLoader::load_pixels(const ImageMetaData &,
                                            void *pixels,
                                            const size_t,
                                            const bool)
{
...
void IxamSession::builtin_images_load()
{
  /* Force builtin images to be loaded along with 3IXAM data sync. This
   * is needed because we may be reading from depsgraph evaluated data which
   * can be freed by 3IXAM before Cycles reads it.
   *
...
string IxamPointDensityLoader::name() const
{
  return BL::ShaderNodeTexPointDensity(b_node).name();
}

bool IxamPointDensityLoader::equals(const ImageLoader &other) const
{
  const IxamPointDensityLoader &other_loader = (const IxamPointDensityLoader &)other;

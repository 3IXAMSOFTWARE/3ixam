...
#ifndef __IXAM_UTIL_H__
#define __IXAM_UTIL_H__
...
typedef BL::ShaderNodeAttribute::attribute_type_enum IxamAttributeType;
IxamAttributeType ixam_attribute_name_split_type(ustring name, string *r_real_name);

void python_thread_state_save(void **python_thread_state);
void python_thread_state_restore(void **python_thread_state);

static inline BL::Mesh object_to_mesh(BL::IxamData & /*data*/,
...
static inline void free_object_to_mesh(BL::IxamData & /*data*/,
                                       BObjectInfo &b_ob_info,
                                       BL::Mesh &mesh)
{
  if (!b_ob_info.is_real_object_data()) {
...
static inline string image_user_file_path(BL::IxamData &data,
                                          BL::ImageUser &iuser,
                                          BL::Image &ima,
                                          int cfra)
{
...
static inline Transform get_transform(const BL::Array<float, 16> &array)
{
  ...
  /* We assume both types to be just 16 floats, and transpose because ixam
...
static inline string ixam_absolute_path(BL::IxamData &b_data, BL::ID &b_id, const string &path)
{
  if (path.size() >= 2 && path[0] == '/' && path[1] == '/') {
    ...
    if (b_id.library()) {
      ...
      dirname = ixam_absolute_path(b_data, b_library_id, b_id.library().filepath());
    }
    else

...
unique_ptr<IxamDisplayShader> IxamDisplayShader::create(BL::RenderEngine &b_engine,
                                                              BL::Scene &b_scene)
{
  if (b_engine.support_display_space_shader(b_scene)) {
    return make_unique<IxamDisplaySpaceShader>(b_engine, b_scene);
  }

  return make_unique<IxamFallbackDisplayShader>();
}

int IxamDisplayShader::get_position_attrib_location()
{
...
int IxamDisplayShader::get_tex_coord_attrib_location()
{
...
void IxamFallbackDisplayShader::bind(int width, int height)
{
...
void IxamFallbackDisplayShader::unbind()
{
}

uint IxamFallbackDisplayShader::get_shader_program()
{
  return shader_program_;
}

void IxamFallbackDisplayShader::create_shader_if_needed()
{
...
void IxamFallbackDisplayShader::destroy_shader()
{
...
/* --------------------------------------------------------------------
 * IxamDisplaySpaceShader.
 */

IxamDisplaySpaceShader::IxamDisplaySpaceShader(BL::RenderEngine &b_engine,
                                                     BL::Scene &b_scene)
...
void IxamDisplaySpaceShader::bind(int /*width*/, int /*height*/)
{
  b_engine_.bind_display_space_shader(b_scene_);
}

void IxamDisplaySpaceShader::unbind()
{
  b_engine_.unbind_display_space_shader();
}

uint IxamDisplaySpaceShader::get_shader_program()
{
  if (!shader_program_) {
...
class DrawTile {
 public:
  ...
  IxamDisplayDriver::Params params;
};
...
struct IxamDisplayDriver::Tiles {
...
IxamDisplayDriver::IxamDisplayDriver(BL::RenderEngine &b_engine,
                                           BL::Scene &b_scene,
                                           const bool background)
    : b_engine_(b_engine),
      background_(background),
      display_shader_(IxamDisplayShader::create(b_engine, b_scene)),
...
IxamDisplayDriver::~IxamDisplayDriver()
{
...
void IxamDisplayDriver::next_tile_begin()
{
...
bool IxamDisplayDriver::update_begin(const Params &params,
                                        int texture_width,
                                        int texture_height)
{
  /* Note that it's the responsibility of IxamDisplayDriver to ensure updating and drawing
...
void IxamDisplayDriver::update_end()
{
   ...
   * renders while 3IXAM is drawing. As a workaround update texture during draw, under assumption
...
half4 *IxamDisplayDriver::map_texture_buffer()
{
  ...
  if (!mapped_rgba_pixels) {
    LOG(ERROR) << "Error mapping IxamDisplayDriver pixel buffer object.";
  }

  return mapped_rgba_pixels;
}

void IxamDisplayDriver::unmap_texture_buffer()
{
...
IxamDisplayDriver::GraphicsInterop IxamDisplayDriver::graphics_interop_get()
{
...
void IxamDisplayDriver::graphics_interop_activate()
{
  gpu_context_enable();
}

void IxamDisplayDriver::graphics_interop_deactivate()
{
  gpu_context_disable();
}
...
void IxamDisplayDriver::clear()
{
  need_clear_ = true;
}

void IxamDisplayDriver::set_zoom(float zoom_x, float zoom_y)
{
...
void IxamDisplayDriver::draw(const Params &params)
{
  gpu_context_lock();
...
void IxamDisplayDriver::gpu_context_create()
{

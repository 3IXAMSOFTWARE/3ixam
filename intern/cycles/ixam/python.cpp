...
/* Synchronize debug flags from a given 3IXAM scene.
...
static const char *PyC_UnicodeAsByte(PyObject *py_str, PyObject **coerce)
{
  ...
  if (result) {
     ...
     * chars since ixam doesn't limit this.
     */
    return result;
  }
  else {
...
static PyObject *create_func(PyObject * /*self*/, PyObject *args)
{
  ...
  if (rv3d) {
    ...
    session = new IxamSession(engine, preferences, data, v3d, rv3d, width, height);
  }
  else {
    /* offline session or preview render */
    session = new IxamSession(engine, preferences, data, preview_osl);
  }
...
{
  delete (IxamSession *)PyLong_AsVoidPtr(value);
...
static PyObject *render_func(PyObject * /*self*/, PyObject *args)
{
  ...
  IxamSession *session = (IxamSession *)PyLong_AsVoidPtr(pysession);

  PointerRNA depsgraphptr;
  RNA_pointer_create(NULL, &RNA_Depsgraph, (ID *)PyLong_AsVoidPtr(pydepsgraph), &depsgraphptr);
  BL::Depsgraph b_depsgraph(depsgraphptr);

  /* Allow 3IXAM to execute other Python scripts. */
...
static PyObject *draw_func(PyObject * /*self*/, PyObject *args)
{
  ...
  IxamSession *session = (IxamSession *)PyLong_AsVoidPtr(py_session);
...
static PyObject *denoise_func(PyObject * /*self*/, PyObject *args, PyObject *keywords)
{
  ...
  DeviceInfo device = ixam_device_info(b_preferences, b_scene, true);
  ...
  DenoiseParams params = IxamSync::get_denoise_params(b_scene, b_view_layer, true);
  params.use = true;
...
{
  IxamSession::print_render_stats = true;
  Py_RETURN_NONE;
}
...
static PyObject *set_device_override_func(PyObject * /*self*/, PyObject *arg)
{
  ...
  if (override == "CPU") {
    IxamSession::device_override = DEVICE_MASK_CPU;
  }
  else if (override == "CUDA") {
    IxamSession::device_override = DEVICE_MASK_CUDA;
  }
  else if (override == "OPTIX") {
    IxamSession::device_override = DEVICE_MASK_OPTIX;
  }
  else if (override == "HIP") {
    IxamSession::device_override = DEVICE_MASK_HIP;
  }
  else if (override == "METAL") {
    IxamSession::device_override = DEVICE_MASK_METAL;
  }
  else if (override == "ONEAPI") {
    IxamSession::device_override = DEVICE_MASK_ONEAPI;
  }
  else {
  ...
  if (include_cpu) {
    IxamSession::device_override = (DeviceTypeMask)(IxamSession::device_override |
                                                       DEVICE_MASK_CPU);
  }
...
static struct PyModuleDef module = {
    ...
    "_cycles",
    "3IXAM cycles render integration",
    -1,
    methods,
    NULL,
    NULL,

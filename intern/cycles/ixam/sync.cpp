...
IxamSync::IxamSync(BL::RenderEngine &b_engine,
                         BL::IxamData &b_data,
                         BL::Scene &b_scene,
                         Scene *scene,
                         bool preview,
...
void IxamSync::reset(BL::IxamData &b_data, BL::Scene &b_scene)
{
...
void IxamSync::sync_recalc(BL::Depsgraph &b_depsgraph, BL::SpaceView3D &b_v3d)
{
  /* Sync recalc flags from ixam to cycles. Actual update is done separate,
...
void IxamSync::sync_data(BL::RenderSettings &b_render,
...
void IxamSync::sync_integrator(BL::ViewLayer &b_view_layer, bool background)
{
  PointerRNA cscene = RNA_pointer_get(&b_scene.ptr, "cycles");
...
void IxamSync::sync_film(BL::ViewLayer &b_view_layer, BL::SpaceView3D &b_v3d)
{
  PointerRNA cscene = RNA_pointer_get(&b_scene.ptr, "cycles");
  ...
  if (b_v3d) {
    const IxamViewportParameters new_viewport_parameters(b_v3d, use_developer_ui);
  ...
  /* 3IXAM viewport does not support proper shadow catcher compositing, so force an approximate
...
void IxamSync::sync_view_layer(BL::ViewLayer &b_view_layer)
{
...
/* Images */
void IxamSync::sync_images()
{
  /* Sync is a convention for this API, but currently it frees unused buffers. */

  const bool is_interface_locked = b_engine.render() && b_engine.render().use_lock_interface();
  if (is_interface_locked == false && IxamSession::headless == false) {
...
void IxamSync::sync_render_passes(BL::RenderLayer &b_rlay, BL::ViewLayer &b_view_layer)
{
...
void IxamSync::free_data_after_sync(BL::Depsgraph &b_depsgraph)
{
  /* When viewport display is not needed during render we can force some
   * caches to be releases from ixam side in order to reduce peak memory
  ...
  const bool can_free_caches =
      (IxamSession::headless || is_interface_locked) &&
...
SceneParams IxamSync::get_scene_params(BL::Scene &b_scene,
...
bool IxamSync::get_session_pause(BL::Scene &b_scene, bool background)
{
  PointerRNA cscene = RNA_pointer_get(&b_scene.ptr, "cycles");
  return (background) ? false : get_boolean(cscene, "preview_pause");
}

SessionParams IxamSync::get_session_params(BL::RenderEngine &b_engine,
                                              ...
                                              bool background)
{
  ...
  params.headless = IxamSession::headless;
  params.background = background;

  /* Device */
  params.threads = ixam_device_threads(b_scene);
  params.device = ixam_device_info(b_preferences, b_scene, params.background);
  ...
  params.use_profiling = params.device.has_profiling && !b_engine.is_preview() && background &&
                         IxamSession::print_render_stats;
...
DenoiseParams IxamSync::get_denoise_params(BL::Scene &b_scene,

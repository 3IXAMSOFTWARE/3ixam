...
class IxamSmokeLoader : public ImageLoader {
 public:
  IxamSmokeLoader(BL::Object &b_ob, AttributeStandard attribute)
  ...
  {
    const IxamSmokeLoader &other_loader = (const IxamSmokeLoader &)other;
    ...
    BL::Scene &b_scene, Scene *scene, BObjectInfo &b_ob_info, Volume *volume, float frame)
{
  ...
  for (int i = 0; attributes[i] != ATTR_STD_NONE; i++) {
    ...
    ImageLoader *loader = new IxamSmokeLoader(b_ob_info.real_object, std);
    ImageParams params;
    params.frame = frame;
...
class IxamVolumeLoader : public VDBImageLoader {
 public:
  IxamVolumeLoader(BL::IxamData &b_data,
                      BL::Volume &b_volume,
...
static void sync_volume_object(BL::IxamData &b_data,
                               BL::Scene &b_scene,
                               ...
                               Volume *volume)
{
  ...
  for (BL::VolumeGrid &b_grid : b_volume.grids) {
    ...
    if ((std != ATTR_STD_NONE && volume->need_attribute(scene, std)) ||
      ...
      ImageLoader *loader = new IxamVolumeLoader(
...
void IxamSync::sync_volume(BObjectInfo &b_ob_info, Volume *volume)
{
  volume->clear(true);

...
/* 3IXAM Camera Intermediate: we first convert both the offline and 3d view
...
static void ixam_camera_init(IxamCamera *bcam, BL::RenderSettings &b_render)
{
  memset((void *)bcam, 0, sizeof(IxamCamera));
  ...
  bcam->sensor_fit = IxamCamera::AUTO;
...
static float ixam_camera_focal_distance(BL::RenderEngine &b_engine,
                                           BL::Object &b_ob,
                                           BL::Camera &b_camera,
                                           IxamCamera *bcam)
{
...
static void ixam_camera_from_object(IxamCamera *bcam,
                                       ...
                                       bool skip_panorama = false)
{
  ...
  if (b_ob_data.is_a(&RNA_Camera)) {
    ...
    if (b_camera.dof().use_dof()) {
      ...
      bcam->focaldistance = ixam_camera_focal_distance(b_engine, b_ob, b_camera, bcam);
    ...
    if (b_camera.sensor_fit() == BL::Camera::sensor_fit_AUTO)
      ...
      bcam->sensor_fit = IxamCamera::HORIZONTAL;
    else
      bcam->sensor_fit = IxamCamera::VERTICAL;
  }
...
static Transform ixam_camera_matrix(const Transform &tfm,
                                       ...
                                       const PanoramaType panorama_type)
{
  ...
  if (type == CAMERA_PANORAMA) {
    ...
    /* note the ixam camera points along the negative z-axis */
...
static void ixam_camera_viewplane(IxamCamera *bcam,
                                     int width,
                                     int height,
                                     ...
                                     float *sensor_size)
{
  ...
  /* sensor fitting */
  if (bcam->sensor_fit == IxamCamera::AUTO) {
    horizontal_fit = (xratio > yratio);
    if (sensor_size != NULL) {
      *sensor_size = bcam->sensor_width;
    }
  }
  else if (bcam->sensor_fit == IxamCamera::HORIZONTAL) {
    horizontal_fit = true;
    if (sensor_size != NULL) {
      *sensor_size = bcam->sensor_width;
    }
...
static void ixam_camera_sync(Camera *cam,
                                IxamCamera *bcam,
                                int width,
                                int height,
                                const char *viewname,
                                PointerRNA *cscene)
{
  ...
  /* viewplane */
  BoundBox2D viewplane;
  ixam_camera_viewplane(bcam, width, height, &viewplane, &aspectratio, &sensor_size);
  ...
  /* transform */
  cam->set_matrix(ixam_camera_matrix(bcam->matrix, bcam->type, bcam->panorama_type));
...
void IxamSync::sync_camera(BL::RenderSettings &b_render,
...
{
  IxamCamera bcam;
  ixam_camera_init(&bcam, b_render);
  ...
  if (b_ob) {
    BL::Array<float, 16> b_ob_matrix;
    ixam_camera_from_object(&bcam, b_engine, b_ob);
    b_engine.camera_model_matrix(b_ob, bcam.use_spherical_stereo, b_ob_matrix);
  ...
  ixam_camera_sync(cam, &bcam, width, height, viewname, &cscene);
  ...
  if (b_ob) {
    ...
    ixam_camera_sync(scene->dicing_camera, &bcam, width, height, viewname, &cscene);
  }
  else {
...
void IxamSync::sync_camera_motion(
    BL::RenderSettings &b_render, BL::Object &b_ob, int width, int height, float motion_time)
{
  ...
  tfm = ixam_camera_matrix(tfm, cam->get_camera_type(), cam->get_panorama_type());
  ...
  if (cam->get_camera_type() == CAMERA_PERSPECTIVE) {
    ...
    /* TODO(sergey): Consider making it a part of ixam_camera_init(). */
    bcam.pixelaspect.x = b_render.pixel_aspect_x();
    bcam.pixelaspect.y = b_render.pixel_aspect_y();

    ixam_camera_from_object(&bcam, b_engine, b_ob);
    ixam_camera_viewplane(&bcam, width, height, NULL, &aspectratio, &sensor_size);
...
static void ixam_camera_view_subset(BL::RenderEngine &b_engine,
...
static void ixam_camera_from_view(IxamCamera *bcam,
                                     BL::RenderEngine &b_engine,
                                     BL::Scene &b_scene,
                                     BL::SpaceView3D &b_v3d,
                                     BL::RegionView3D &b_rv3d,
                                     ...
                                     bool skip_panorama = false)
{
  ...
  if (b_rv3d.view_perspective() == BL::RegionView3D::view_perspective_CAMERA) {
    ...
    if (b_ob) {
      ixam_camera_from_object(bcam, b_engine, b_ob, skip_panorama);
      ...
      if (!skip_panorama && bcam->type == CAMERA_PANORAMA) {
        ...
        ixam_camera_view_subset(b_engine,
                                   b_render_settings,
                                   b_scene,
                                   b_ob,
                                   b_v3d,
    ...
    float sensor_size;
    if (bcam->sensor_fit == IxamCamera::VERTICAL)
      sensor_size = bcam->sensor_height;
    else
      sensor_size = bcam->sensor_width;
                                       ...
                                       float *view_aspect)
{
  ...
  IxamCamera view_bcam;
  ixam_camera_init(&view_bcam, b_render);
  ixam_camera_from_view(&view_bcam, b_engine, b_scene, b_v3d, b_rv3d, width, height, true);

  ixam_camera_viewplane(&view_bcam, width, height, &view, view_aspect, &sensor_size);

  /* Get camera viewplane. */
  IxamCamera cam_bcam;
  ixam_camera_init(&cam_bcam, b_render);
  ixam_camera_from_object(&cam_bcam, b_engine, b_ob, true);
...
static void ixam_camera_border_subset(BL::RenderEngine &b_engine,
                                         BL::RenderSettings &b_render,
                                         BL::Scene &b_scene,
                                         BL::SpaceView3D &b_v3d,
                                         BL::RegionView3D &b_rv3d,
...
static void ixam_camera_border(IxamCamera *bcam,
                                  BL::RenderEngine &b_engine,
                                  BL::RenderSettings &b_render,
                                  BL::Scene &b_scene,
                                  BL::SpaceView3D &b_v3d,
                                  ...
                                  int height)
{
  ...
  ixam_camera_border_subset(b_engine,
                               b_render,
                               b_scene,
                               b_v3d,
                               b_rv3d,
...
void IxamSync::sync_view(BL::SpaceView3D &b_v3d,
                            BL::RegionView3D &b_rv3d,
                            int width,
                            int height)
{
  IxamCamera bcam;
  BL::RenderSettings b_render_settings(b_scene.render());
  ixam_camera_init(&bcam, b_render_settings);
  ixam_camera_from_view(&bcam, b_engine, b_scene, b_v3d, b_rv3d, width, height);
  ixam_camera_border(&bcam, b_engine, b_render_settings, b_scene, b_v3d, b_rv3d, width, height);
  PointerRNA cscene = RNA_pointer_get(&b_scene.ptr, "cycles");
  ixam_camera_sync(scene->camera, &bcam, width, height, "", &cscene);
  ...
  if (b_ob) {
    ...
    ixam_camera_sync(scene->dicing_camera, &bcam, width, height, "", &cscene);
  }
  else {
...
BufferParams IxamSync::get_buffer_params(

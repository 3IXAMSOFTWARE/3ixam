...
#include "ixam/display_driver.h"
#include "ixam/output_driver.h"
#include "ixam/session.h"
#include "ixam/sync.h"
#include "ixam/util.h"

CCL_NAMESPACE_BEGIN

DeviceTypeMask IxamSession::device_override = DEVICE_MASK_ALL;
bool IxamSession::headless = false;
bool IxamSession::print_render_stats = false;

IxamSession::IxamSession(BL::RenderEngine &b_engine,
...
IxamSession::~IxamSession()
{
  free_session();
}

void IxamSession::create_session()
{
  const SessionParams session_params = IxamSync::get_session_params(
      b_engine, b_userpref, b_scene, background);
  const SceneParams scene_params = IxamSync::get_scene_params(
      b_scene, background, use_developer_ui);
  const bool session_pause = IxamSync::get_session_pause(b_scene, background);
  ...
  session->progress.set_update_callback(function_bind(&IxamSession::tag_redraw, this));
  session->progress.set_cancel_callback(function_bind(&IxamSession::test_cancel, this));
  ...
  const BufferParams buffer_params = IxamSync::get_buffer_params(
...
void IxamSession::reset_session(BL::IxamData &b_data, BL::Depsgraph &b_depsgraph)
{
...
void IxamSession::full_buffer_written(string_view filename)
{
...
void IxamSession::stamp_view_layer_metadata(Scene *scene, const string &view_layer_name)
{
...
void IxamSession::render(BL::Depsgraph &b_depsgraph_)
{
  ...
  session->set_output_driver(make_unique<IxamOutputDriver>(b_engine));
  ...
  const SessionParams session_params = IxamSync::get_session_params(
      b_engine, b_userpref, b_scene, background);
  BufferParams buffer_params = IxamSync::get_buffer_params(
      b_v3d, b_rv3d, scene->camera, width, height);
  ...
  for (b_rr.views.begin(b_view_iter); b_view_iter != b_rr.views.end();
    ...
    /* Attempt to free all data which is held by 3IXAM side, since at this
...
void IxamSession::render_frame_finish()
{
   ...
   * might be freed form 3IXAM side. Need to ensure that all GPU resources are freed prior to
   * that point.
...
void IxamSession::bake(BL::Depsgraph &b_depsgraph_,
                          BL::Object &b_object,
                          ...
                          const int bake_height)
{
   ...
   * used as that on the 3IXAM side. */
  ...
  if (!session->progress.get_cancel()) {
    /* Load built-in images from 3IXAM. */
    builtin_images_load();
  }

  /* Object might have been disabled for rendering or excluded in some
   * other way, in that case 3IXAM will report a warning afterwards. */
  ...
  if (object_found && !session->progress.get_cancel()) {
    ...
    session->progress.set_update_callback(
        function_bind(&IxamSession::update_bake_progress, this));
  }
...
void IxamSession::synchronize(BL::Depsgraph &b_depsgraph_)
{
...
void IxamSession::draw(BL::SpaceImageEditor &space_image)
{
...
void IxamSession::view_draw(int w, int h)
{
  /* pause in redraw in case update is not being called due to final render */
  session->set_pause(IxamSync::get_session_pause(b_scene, background));
...
void IxamSession::get_status(string &status, string &substatus)
{
  session->progress.get_status(status, substatus);
}

void IxamSession::get_progress(double &progress, double &total_time, double &render_time)
{
  session->progress.get_time(total_time, render_time);
  progress = session->progress.get_progress();
}

void IxamSession::update_bake_progress()
{
...
void IxamSession::update_status_progress()
{
...
bool IxamSession::check_and_report_session_error()
{
...
void IxamSession::tag_update()
{
  /* tell ixam that we want to get another update callback */
  b_engine.tag_update();
}

void IxamSession::tag_redraw()
{
  if (background) {
    ...
    /* tell ixam that we want to redraw */
    b_engine.tag_redraw();
  }
}

void IxamSession::test_cancel()
{
...
void IxamSession::free_ixam_memory_if_possible()
{
  if (!background) {
  ...
  b_engine.free_ixam_memory();
}

void IxamSession::ensure_display_driver_if_needed()
{
  if (display_driver_) {
  ...
  unique_ptr<IxamDisplayDriver> display_driver = make_unique<IxamDisplayDriver>(

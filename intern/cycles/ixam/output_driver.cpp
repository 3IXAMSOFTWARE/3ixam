...
IxamOutputDriver::IxamOutputDriver(BL::RenderEngine &b_engine) : b_engine_(b_engine)
{
}

IxamOutputDriver::~IxamOutputDriver()
{
}

bool IxamOutputDriver::read_render_tile(const Tile &tile)
{
  /* Get render result. */
...
bool IxamOutputDriver::update_render_tile(const Tile &tile)
{
  /* Use final write for preview renders, otherwise render result wouldn't be updated
   * quickly on 3IXAM side. For all other cases we use the display driver. */
...
void IxamOutputDriver::write_render_tile(const Tile &tile)
{
  b_engine_.tile_highlight_clear_all();

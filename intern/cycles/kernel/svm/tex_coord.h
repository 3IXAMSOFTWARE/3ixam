                                             ...
                                             uint4 node)
{
  ...
  if (space == NODE_NORMAL_MAP_TANGENT) {
    ...
    /* strange ixam convention */
    if (space == NODE_NORMAL_MAP_IXAM_OBJECT || space == NODE_NORMAL_MAP_IXAM_WORLD) {
      color.y = -color.y;
      color.z = -color.z;
    }
    ...
    if (space == NODE_NORMAL_MAP_OBJECT || space == NODE_NORMAL_MAP_IXAM_OBJECT)
      object_normal_transform(kg, sd, &N);
    else
      N = safe_normalize(N);
  }

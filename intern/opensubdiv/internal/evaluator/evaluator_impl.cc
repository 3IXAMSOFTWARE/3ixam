    ...
    OpenSubdiv_EvaluatorCacheImpl *evaluator_cache_descr)
{
  ...
  ixam::opensubdiv::EvalOutputAPI::EvalOutput *eval_output = nullptr;

  const bool use_gpu_evaluator = evaluator_type == OPENSUBDIV_EVALUATOR_GPU;
  if (use_gpu_evaluator) {
    ixam::opensubdiv::GpuEvalOutput::EvaluatorCache *evaluator_cache = nullptr;
    if (evaluator_cache_descr) {
      evaluator_cache = static_cast<ixam::opensubdiv::GpuEvalOutput::EvaluatorCache *>(
          evaluator_cache_descr->eval_cache);
    }

    eval_output = new ixam::opensubdiv::GpuEvalOutput(vertex_stencils,
                                                         varying_stencils,
    ...
    eval_output = new ixam::opensubdiv::CpuEvalOutput(
        vertex_stencils, varying_stencils, all_face_varying_stencils, 2, patch_table);
  }

  ixam::opensubdiv::PatchMap *patch_map = new ixam::opensubdiv::PatchMap(*patch_table);
  ...
  evaluator_descr->eval_output = new ixam::opensubdiv::EvalOutputAPI(eval_output, patch_map);

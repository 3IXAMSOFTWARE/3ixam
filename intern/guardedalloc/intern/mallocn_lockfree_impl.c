...
#include "../../source/ixam/ixamlib/BLI_strict_flags.h"

#include "atomic_ops.h"
#include "mallocn_intern.h"

//#define DEBUG_MALLOC_STORE_CALLER
#ifdef DEBUG_MALLOC_STORE_CALLER
...
os_unfair_lock MEM_Info_lock;

typedef struct MemCallerInfo {
  void* next;
  void* prev;
  
  void* address;
  void* callstack[MAX_CALLSTACK_DEPTH];
  int actual_callstack_depth;
} MemCallerInfo;

MemCallerInfo root = {0};

void MEM_Info_dump_single(MemCallerInfo* info);
void MEM_Info_init(MemCallerInfo* info);
MemCallerInfo* MEM_Info_unlink(void* address);
MemCallerInfo* MEM_Info__unlink(MemCallerInfo* node);

void MEM_Info_dump(void) {
  os_unfair_lock_lock(&MEM_Info_lock);
  
  MemCallerInfo* cur = &root;
  
  while(cur) {
    
    MEM_Info_dump_single(cur);
    cur = (MemCallerInfo*)cur->next;
  }
  
  os_unfair_lock_unlock(&MEM_Info_lock);
}

void MEM_Info_dump_single(MemCallerInfo* info) {
  char** strs = backtrace_symbols(info->callstack, info->actual_callstack_depth);
  printf("Address: %X\n", (unsigned int)info->address);
  for(int i = 0; i < info->actual_callstack_depth; ++i)
    printf("%s\n", strs[i]);
  free(strs);
}
...
void MEM_Info_init(MemCallerInfo* info) {
  ...
  info->address = NULL;
  info->actual_callstack_depth = 0;
  info->next = root.next;
  info->prev = &root;
  
  MemCallerInfo* next = (MemCallerInfo*)info->next;
...
MemCallerInfo* MEM_Info__unlink(MemCallerInfo* node) {
  ...
  if(node->prev)
    ((MemCallerInfo*)node->prev)->next = node->next;
  
  MemCallerInfo* next = (MemCallerInfo*)node->next;
...
MemCallerInfo* MEM_Info_unlink(void* address) {
  ...
  while(cur) {
    ...
    if(cur->address == address) {
      ...
      MemCallerInfo* result = MEM_Info__unlink(cur);
      os_unfair_lock_unlock(&MEM_Info_lock);
      return result;
    }
...
#ifdef DEBUG_MALLOC_STORE_CALLER
  MemCallerInfo* info = (MemCallerInfo*)malloc(sizeof(MemCallerInfo));
  MEM_Info_init(info);
  
  info->address = memh;
  info->actual_callstack_depth = backtrace(info->callstack, MAX_CALLSTACK_DEPTH);
#endif
  if (LIKELY(memh)) {
    memh->len = len;
...
void MEM_lockfree_printmemlist_stats(void)
{
  ...
  printf(
      "\nFor more detailed per-block statistics run 3IXAM with memory debugging command line "
      "argument.\n");

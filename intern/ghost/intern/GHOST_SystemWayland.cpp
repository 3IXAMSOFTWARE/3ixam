 ...
 * at the 3IXAM studio, having the button closest to the nib be MMB is preferable,
                                       ...
                                       uint32_t fingers)
{
  ...
  /* NOTE(@campbellbarton): 3IXAM's use of track-pad coordinates is inconsistent and needs work.
   ...
   * 2) 3IXAM's UI scale preference (which GHOST doesn't know about).
   *
  ...
  /* NOTE(@campbellbarton): Scale factors match 3IXAM's operators & default preferences.
    ...
    : GHOST_System(), display_(new GWL_Display)
{
   ...
   * in background mode, also LIBDECOR is crashing in background mode `ixam -b -f 1`
...
static void cursor_buffer_set(const GWL_Seat *seat, wl_buffer *buffer)
{
   ...
   * it causes 3IXAM's window to close intermittently. */
  if (seat->wl_pointer) {
  ...
  if (!grab_state_next.use_lock) {
    ...
    if (seat->wp_locked_pointer) {
      ...
      if (mode_current == GHOST_kGrabWrap) {
        /* Since this call is initiated by 3IXAM, we can be sure the window wasn't closed

...
const char *GHOST_SystemPathsUnix::getSystemDir(int /*version*/, const char *versionstr) const
{
  ...
  if (static_path) {
    static string system_path = string(static_path) + "/ixam/" + versionstr;
...
const char *GHOST_SystemPathsUnix::getUserDir(int version, const char *versionstr) const
{
  ...
  /* in ixam 2.64, we migrate to XDG. to ensure the copy previous settings
  ...
  if (version < 264) {
    if (user_path.empty() || last_version != version) {
      ...
      if (home) {
        user_path = string(home) + "/.ixam/" + versionstr;
      }
      else {
        return nullptr;
      }
  ...
  if (user_path.empty() || last_version != version) {
    ...
    if (home) {
      ...
      user_path = string(home) + "/.config/ixam/" + versionstr;
    }
  }

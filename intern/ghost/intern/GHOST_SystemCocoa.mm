...
static NSMutableDictionary *bookmarksDict;
static NSString *filePathStr;
...
static BOOL getBookMark(NSMutableDictionary *dict,
                        const char *title,
                        const char *message,
                        const char *prompt,
                        const char *path) {
  @autoreleasepool {
    ...
    [openPanel setTitle:[[NSString alloc] initWithCString:title encoding:NSUTF8StringEncoding]];
    [openPanel setMessage:[[NSString alloc] initWithCString:message encoding:NSUTF8StringEncoding]];
    [openPanel setPrompt:[[NSString alloc] initWithCString:prompt encoding:NSUTF8StringEncoding]];
    [openPanel setDirectoryURL:[NSURL fileURLWithPath:[[NSString alloc] initWithCString:path encoding:NSUTF8StringEncoding]]];
    ...
    if (data)
    {
      [dict setObject:data forKey:[fileURL absoluteString]];
    }
    return [dict count] != 0;
  }
}

static bool storeBookMarks(NSMutableDictionary *dict, NSString *filePath)
{
  NSFileManager *manager = [NSFileManager defaultManager];
  [manager removeItemAtPath:filePath error:nil];
  return [NSKeyedArchiver archiveRootObject:dict toFile:filePath];
}

static void restoreBookMarks(NSMutableDictionary *dict)
{
  @autoreleasepool {
    NSError *error = nil;
    BOOL isStale = false;
    ...
    {
      NSData *bookmarkData = dict[key];
      NSURL *restored = [NSURL URLByResolvingBookmarkData:bookmarkData
...
// leave 3IXAM with no key windows. This is needed due to a custom event loop
...
//
// Our solution is different, since we want 3IXAM to keep track of what is
...
// 3IXAM's window manager when closing 3IXAM's windows.
- (void)windowWillClose:(NSNotification *)notification
{
...
GHOST_SystemCocoa::~GHOST_SystemCocoa()
{
  if (bookmarksDict)
  {
    storeBookMarks(bookmarksDict, filePathStr);
    [bookmarksDict release];
  }
  if (filePathStr)
  {
    [filePathStr release];
...
#endif
    {
      char filepath_buf[FILE_MAX] = {'\0'};
      const char *path = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, NULL);
      BLI_path_join(filepath_buf, sizeof(filepath_buf), path, "bookmarks.dict");
      filePathStr = [[NSString alloc] initWithCString:filepath_buf encoding:NSUTF8StringEncoding];
      bookmarksDict = [[NSMutableDictionary alloc] init];
      NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
      dict = (NSMutableDictionary*) [NSKeyedUnarchiver unarchiveObjectWithFile:filePathStr];
      if (dict != nil && [dict count] != 0)
      {
        [bookmarksDict addEntriesFromDictionary:dict];
        restoreBookMarks(bookmarksDict);
      }
      [dict release];
    }
    @autoreleasepool {
      ...
      if ([NSApp mainMenu] == nil) {
        ...
        appMenu = [[NSMenu alloc] initWithTitle:@"3IXAM"];

        [appMenu addItemWithTitle:@"About 3IXAM"
                           action:@selector(orderFrontStandardAboutPanel:)
                    keyEquivalent:@""];
        [appMenu addItem:[NSMenuItem separatorItem]];

        menuItem = [appMenu addItemWithTitle:@"Hide 3IXAM"
        ...
        menuItem = [appMenu addItemWithTitle:@"Quit 3IXAM"
      ...
      // AppKit provides automatic window tabbing. 3IXAM is a single-tabbed application
                                                 ...
                                                 GHOST_DialogOptions dialog_options) const
{
  ...
  @autoreleasepool {
    ...
    NSString *_message = [[NSString alloc] initWithCString:message encoding:NSUTF8StringEncoding];
    NSString *_continue_label = [[NSString alloc] initWithCString:continue_label encoding:NSUTF8StringEncoding];

    NSAlert *alert = [[NSAlert alloc] init];
    [alert addButtonWithTitle:_continue_label];
    [alert setMessageText:_title];
    [alert setInformativeText:_message];
    [alert setAlertStyle:(dialog_options & GHOST_DialogError   ? NSAlertStyleCritical :
                          dialog_options & GHOST_DialogWarning ? NSAlertStyleWarning :
                          NSAlertStyleInformational)];
    [alert beginSheetModalForWindow:(NSWindow *)window->getOSWindow() completionHandler:0];
  }

  return GHOST_kSuccess;
}

GHOST_TSuccess GHOST_SystemCocoa::askFileSystemAccess(const char *title,
                                                      const char *message,
                                                      const char *prompt,
                                                      const char *path) const
{
  @autoreleasepool {
    ...
    if (getBookMark(dict, title, message, prompt, path))
    {
      [bookmarksDict addEntriesFromDictionary:dict];
      storeBookMarks(bookmarksDict, filePathStr);
      return GHOST_kSuccess;
...
void *GHOST_SystemCocoa::findSidecarScreen() const
{
  NSScreen *sideCarScreen = nil;
  for (NSScreen *screen in [NSScreen screens]) {
    if ([[screen localizedName] localizedStandardContainsString:@"Sidecar"]) {
        sideCarScreen = screen;
        break;
    }
  }
  return (void *)sideCarScreen;
}

GHOST_IWindow *GHOST_SystemCocoa::findSidecarWindow() const
{
  NSScreen *sideCarScreen = (NSScreen *)findSidecarScreen();
  GHOST_IWindow *sideCarWindow = nil;
  
  if (sideCarScreen) {
    for (GHOST_IWindow *iwindow : m_windowManager->getWindows()) {
      NSScreen *windowScreen = iwindow->isPanel() ?
      ((GHOST_PanelCocoa*)iwindow)->getScreen() :
      ((GHOST_WindowCocoa*)iwindow)->getScreen();
      if (windowScreen == sideCarScreen) {
        sideCarWindow = iwindow;
        break;
      }
    }
  }
...
GHOST_TSuccess GHOST_SystemCocoa::moveWindowToSidecar(GHOST_IWindow *window) const
{
  NSScreen *sideCarScreen = (NSScreen *)findSidecarScreen();
  if (!sideCarScreen) {
    return GHOST_kFailure;
  ...
  NSWindow *nsWindow = (NSWindow *)window->getOSWindow();
  NSRect targetFrame = [sideCarScreen frame];
  NSRect nsWindowFrame = [nsWindow frame];
  nsWindowFrame.origin.x = targetFrame.origin.x;
  nsWindowFrame.origin.y = targetFrame.origin.y;
  nsWindowFrame.size = targetFrame.size;
  [nsWindow setFrame:nsWindowFrame display:YES animate:YES];
  return GHOST_kSuccess;
}
                                               ...
                                               const GHOST_IWindow *parentWindow)
{
  ...
  @autoreleasepool {
    ...
    left = left > contentRect.origin.x ? left : contentRect.origin.x + left;
    // Add contentRect.origin.y to respect docksize
    bottom = contentRect.origin.y + bottom; /*bottom > contentRect.origin.y ? bottom + contentRect.origin.y : contentRect.origin.y + bottom;*/

    if(is_dialog)
      window = new GHOST_PanelCocoa(this,
                         title,
                         left,
                         bottom,
                         width,
...
GHOST_TSuccess GHOST_SystemCocoa::setCursorPosition(int32_t x, int32_t y)
{
  ...
  pushEvent(new GHOST_EventCursor(
      getMilliSeconds(), GHOST_kEventCursorMove, window, x, y,
      window->isPanel() ?
      ((GHOST_PanelCocoa *)window)->GetCocoaTabletData() :
      ((GHOST_WindowCocoa *)window)->GetCocoaTabletData()));
...
GHOST_TSuccess GHOST_SystemCocoa::setMouseCursorPosition(int32_t x, int32_t y)
{
  ...
  @autoreleasepool {
    NSScreen *windowScreen = window->isPanel() ?
    ((GHOST_PanelCocoa*)window)->getScreen() :
    ((GHOST_WindowCocoa*)window)->getScreen();
...
GHOST_TSuccess GHOST_SystemCocoa::handleApplicationBecomeActiveEvent()
{
  for (GHOST_IWindow *iwindow : m_windowManager->getWindows()) {
    if(iwindow->isPanel()) {
      [((GHOST_PanelCocoa *)iwindow)->getCocoaWindow() makeKeyAndOrderFront:nil];
    } else if(((GHOST_WindowCocoa *)iwindow)->isDialog()) {
      [((GHOST_WindowCocoa *)iwindow)->getCocoaWindow() makeKeyAndOrderFront:nil];
    }
  }
...
{
  for (GHOST_IWindow *iwindow : m_windowManager->getWindows()) {
    if (iwindow->isPanel() || ((GHOST_WindowCocoa *)iwindow)->isDialog()) {
      return true;
    }
  }
  return false;
...
GHOST_TSuccess GHOST_SystemCocoa::handleWindowEvent(GHOST_TEventType eventType, GHOST_PanelCocoa *panel) {
  NSArray *windowsList;
  windowsList = [NSApp orderedWindows];
  if (!validWindow(panel)) {
    return GHOST_kFailure;
  }
  switch (eventType) {
    case GHOST_kEventWindowClose:
      pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowClose, panel));
      break;
    case GHOST_kEventWindowActivate:
      m_windowManager->setActiveWindow(panel);
      panel->loadCursor(panel->getCursorVisibility(), panel->getCursorShape());
      pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowActivate, panel));
      break;
    case GHOST_kEventWindowDeactivate:
      m_windowManager->setWindowInactive(panel);
      pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowDeactivate, panel));
      break;
    case GHOST_kEventWindowUpdate:
      if (m_nativePixel) {
        panel->setNativePixelSize();
        pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventNativeResolutionChange, panel));
      }
      pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowUpdate, panel));
      break;
    case GHOST_kEventWindowMove:
      pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowMove, panel));
      break;
    case GHOST_kEventWindowSize:
      if (!m_ignoreWindowSizedMessages) {
        // Enforce only one resize message per event loop
        // (coalescing all the live resize messages)
        panel->updateDrawingContext();
        pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventWindowSize, panel));
        // Mouse up event is trapped by the resizing event loop,
        // so send it anyway to the window manager.
        pushEvent(new GHOST_EventButton(getMilliSeconds(),
                                        GHOST_kEventButtonUp,
...
GHOST_TSuccess GHOST_SystemCocoa::handleDraggingEvent(GHOST_TEventType eventType,
  ...
  switch (eventType) {
    ...
    case GHOST_kEventDraggingExited:
      pushEvent(new GHOST_EventDragnDrop(
          getMilliSeconds(), eventType, draggedObjectType, panel, mouseX, mouseY, NULL));
      break;
    ...
    case GHOST_kEventDraggingDropDone: {
      ...
      pushEvent(new GHOST_EventDragnDrop(
          getMilliSeconds(), eventType, draggedObjectType, panel, mouseX, mouseY, eventData));
...
void GHOST_SystemCocoa::handleQuitRequest()
{
  ...
  // Push the event to 3IXAM so it can open a dialog if needed
...
bool GHOST_SystemCocoa::handleOpenDocumentRequest(void *filepathStr)
{
  ...
  /* Check for ixam opened windows and make the frontmost key. In case ixam
...
GHOST_TSuccess GHOST_SystemCocoa::handleMouseEvent(void *eventPtr)
{
  ...
  GHOST_IWindow* wnd = m_windowManager
    ->getWindowAssociatedWithOSWindow((void *)[event window]);
  
  if(!wnd) {
    if(!(wnd = m_windowManager->getActiveWindow()))
      return GHOST_kFailure;
  }
  
  if(wnd->isPanel()) {
    GHOST_PanelCocoa* window = (GHOST_PanelCocoa*)wnd;
    ...
    switch ([event type]) {
      ...
      case NSEventTypeMouseMoved: {
        ...
        switch (grab_mode) {
          ...
          case GHOST_kGrabWrap:  // Wrap cursor at area/window boundaries
          {
             ...
             * (y=0 at bottom, the same as ixam internal BTW!), and to client coordinates. */
      ...
      case NSEventTypeScrollWheel: {
        ...
        if (!m_multiTouchScroll && momentumPhase == NSEventPhaseNone) {
          ...
          if (deltaF == 0.0)
            deltaF = [event deltaX];  // make ixam decide if it's horizontal scroll
          if (deltaF == 0.0)
    ...
    GHOST_WindowCocoa* window = (GHOST_WindowCocoa*)wnd;
    cocoawindow = (CocoaWindow *)window->getOSWindow();
...
GHOST_TSuccess GHOST_SystemCocoa::handleTouchBarToolSelectEvent(GHOST_TTouchbarToolSelectEventType eventType, GHOST_WindowCocoa *window)
{
  GHOST_IWindow *sideCarWindow = findSidecarWindow();

  GHOST_TEventType ghostType;
  switch (eventType) {
    case GHOST_kTouchbarEventEnableSelect:
      ghostType = GHOST_kEventTouchbarSelect;
      break;
    
    case GHOST_kTouchbarEventEnableMove:
      ghostType = GHOST_kEventTouchbarMove;
      break;

    case GHOST_kTouchbarEventEnableRotate:
      ghostType = GHOST_kEventTouchbarRotate;
      break;

    case GHOST_kTouchbarEventEnableScale:
      ghostType = GHOST_kEventTouchbarScale;
      break;

    default:
      return GHOST_kFailure;
  }
  pushEvent(new GHOST_Event(getMilliSeconds(), ghostType, sideCarWindow ? sideCarWindow : window));
  m_outsideLoopEventProcessed = true;
  return GHOST_kSuccess;
}

GHOST_TSuccess GHOST_SystemCocoa::handleTouchBarZoomEvent(GHOST_WindowCocoa *window) {
  GHOST_IWindow *sideCarWindow = findSidecarWindow();
  pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventTouchbarZoom, sideCarWindow? sideCarWindow : window));
  m_outsideLoopEventProcessed = true;
  return GHOST_kSuccess;
}

GHOST_TSuccess GHOST_SystemCocoa::handleTouchBarCreatePrimitiveEvent(GHOST_TTouchbarCreatePrimitiveEventType eventType, GHOST_WindowCocoa *window) {
  GHOST_TEventType ghostType;
  switch(eventType) {
    case GHOST_kTouchbarEventCreateCube:
      ghostType = GHOST_kEventCreateCube;
      break;

    case GHOST_kTouchbarEventCreateSphere:
      ghostType = GHOST_kEventCreateSphere;
      break;

    case GHOST_kTouchbarEventCreateTorus:
      ghostType = GHOST_kEventCreateTorus;
      break;
    case GHOST_kTouchbarEventCreateCylinder:
      ghostType = GHOST_kEventCreateCylinder;
      break;

    case GHOST_kTouchbarEventCreateCone:
      ghostType = GHOST_kEventCreateCon;
      break;
    case GHOST_kTouchbarEventCreateTube:
      ghostType = GHOST_kEventCreateTube;
      break;
  
    case GHOST_kTouchbarEventCreateHexagon:
      ghostType = GHOST_kEventCreateHexagon;
      break;

    case GHOST_kTouchbarEventCreatePyramid:
      ghostType = GHOST_kEventCreatePyramid;
      break;
  ...
  pushEvent(new GHOST_Event(getMilliSeconds(), ghostType, window));
  m_outsideLoopEventProcessed = true;
  return GHOST_kSuccess;
}

GHOST_TSuccess GHOST_SystemCocoa::handleTouchBarDeleteEvent(GHOST_WindowCocoa *window) {
  pushEvent(new GHOST_Event(getMilliSeconds(), GHOST_kEventTouchbarDelete, window));
  m_outsideLoopEventProcessed = true;
  return GHOST_kSuccess;
}

GHOST_TSuccess GHOST_SystemCocoa::handleTouchBarShadingEvent(GHOST_TTouchbarShadingType shadingType, GHOST_WindowCocoa *window) {
  GHOST_IWindow *sideCarWindow = findSidecarWindow();
  char *temp_buff = (char *)malloc(16);
  switch (shadingType) {
    case GHOST_kTouchbarEventWireframeShading:
      strncpy(temp_buff, "WIREFRAME", sizeof("WIREFRAME"));
      break;
    
    case GHOST_kTouchbarEventClayShading:
      strncpy(temp_buff, "SOLID", sizeof("SOLID"));
      break;

    case GHOST_kTouchbarEventPreviewShading:
      strncpy(temp_buff, "MATERIAL", sizeof("MATERIAL"));
      break;

    case GHOST_kTouchbarEventRenderShading:
      strncpy(temp_buff, "RENDERED", sizeof("RENDERED"));
      break;
  ...
  pushEvent(new GHOST_EventString(
      getMilliSeconds(), GHOST_kEventTouchbarShading, (sideCarWindow ? sideCarWindow : window), (GHOST_TEventDataPtr)temp_buff));
  m_outsideLoopEventProcessed = true;
  return GHOST_kSuccess;
}

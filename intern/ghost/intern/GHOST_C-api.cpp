...
bool GHOST_AskFileSystemAccess(GHOST_SystemHandle systemhandle,
                               const char *title,
                               const char *message,
                               const char *prompt,
                               const char *path)
{
  GHOST_ISystem *system = (GHOST_ISystem *)systemhandle;
  return system->askFileSystemAccess(title, message, prompt, path) == GHOST_kSuccess;
}

bool GHOST_MoveWindowToSidecar(GHOST_SystemHandle systemhandle, GHOST_WindowHandle windowhandle) 
{
  GHOST_ISystem *system = (GHOST_ISystem *)systemhandle;
  GHOST_IWindow *window = (GHOST_IWindow *)windowhandle;
  return (system->moveWindowToSidecar(window) == GHOST_kSuccess);
}

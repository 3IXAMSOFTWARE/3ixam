 ...
 * something, making it so ixam's window manager sees the new
 * messages coming in.
 */
...
bool GHOST_SystemWin32::processNDOF(RAWINPUT const &raw)
{
  ...
  switch (packetType) {
    case 1: { /* Translation. */
      ...
      /* Massage into ixam view coords (same goes for rotation). */
...
static bool isStartedFromCommandPrompt()
{
  ...
  if (hwnd) {
    ...
    if (getProcessName(ppid, parent_name, sizeof(parent_name))) {
      ...
      if (filename != NULL) {
        start_from_launcher = strstr(filename, "3ixam.exe") != NULL;
      }
    }

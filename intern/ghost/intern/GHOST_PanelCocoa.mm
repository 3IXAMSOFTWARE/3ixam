...
@interface CocoaPanelDelegate : NSObject <NSWindowDelegate>
{
  GHOST_SystemCocoa *systemCocoa;
  GHOST_PanelCocoa *associatedWindow;
}

- (void)setSystemAndWindowCocoa:(GHOST_SystemCocoa *)sysCocoa
                    windowCocoa:(GHOST_PanelCocoa *)winCocoa;
...
@implementation CocoaPanelDelegate : NSObject
- (void)setSystemAndWindowCocoa:(GHOST_SystemCocoa *)sysCocoa
                    windowCocoa:(GHOST_PanelCocoa *)winCocoa
{
...
- (BOOL)windowShouldClose:(id)sender;
{
  // Let 3IXAM close the window rather than closing immediately
...
{
  return NO;  // No need to overflow ixam event queue. Events shall be sent only on changes
}
...
#define COCOA_VIEW_CLASS CocoaPanelOpenGLView
#define COCOA_VIEW_BASE_CLASS NSOpenGLView
#define COCOA_VIEW_WINDOW_CLASS GHOST_PanelCocoa
#include "GHOST_WindowViewCocoa.h"
#undef COCOA_VIEW_CLASS
#undef COCOA_VIEW_BASE_CLASS

#define COCOA_VIEW_CLASS CocoaPanelMetalView
#define COCOA_VIEW_BASE_CLASS NSView
#include "GHOST_WindowViewCocoa.h"
#undef COCOA_VIEW_CLASS
#undef COCOA_VIEW_BASE_CLASS
#undef COCOA_VIEW_WINDOW_CLASS

#pragma mark initialization / finalization

GHOST_PanelCocoa::GHOST_PanelCocoa(GHOST_SystemCocoa *systemCocoa,
                                     const char *title,
                                     int32_t left,
                                     int32_t bottom,
                                     uint32_t width,
      ...
      m_debug_context(is_debug)
{
  ...
  styleMask |= NSWindowStyleMaskDocModalWindow | NSWindowStyleMaskBorderless;

  m_window = [[CocoaPanel alloc] initWithContentRect:rect
                                            styleMask:styleMask
  ...
  // Forbid to resize the window below 3IXAM defined minimum one
  minSize.width = 320;
  minSize.height = 240;
  ...
  if (metalDevice) {
    ...
    m_metalView = [[CocoaPanelMetalView alloc] initWithFrame:rect];
    ...
    m_openGLView = [[CocoaPanelOpenGLView alloc] initWithFrame:rect];
  ...
  CocoaPanelDelegate *windowDelegate = [[CocoaPanelDelegate alloc] init];
...
GHOST_PanelCocoa::~GHOST_PanelCocoa()
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  ...
  // Check for other ixam opened windows and make the frontmost key
  // Note: for some reason the closed window is still in the list
  NSArray *windowsList = [NSApp orderedWindows];
  for (int a = 0; a < [windowsList count]; a++) {
    if (m_window != (CocoaPanel *)[windowsList objectAtIndex:a]) {
...
bool GHOST_PanelCocoa::getValid() const
{
  NSView *view = (m_openGLView) ? m_openGLView : m_metalView;
  return GHOST_Window::getValid() && m_window != NULL && view != NULL;
}

void *GHOST_PanelCocoa::getOSWindow() const
{
  return (void *)m_window;
}

void GHOST_PanelCocoa::setTitle(const char *title)
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setTitle(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  NSString *windowTitle = [[NSString alloc] initWithCString:title encoding:NSUTF8StringEncoding];

  // Set associated file if applicable
  if (windowTitle && [windowTitle hasPrefix:@"3IXAM"]) {
...
std::string GHOST_PanelCocoa::getTitle() const
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::getTitle(): window invalid");
...
void GHOST_PanelCocoa::getWindowBounds(GHOST_Rect &bounds) const
{
  NSRect rect;
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::getWindowBounds(): window invalid");
...
void GHOST_PanelCocoa::getClientBounds(GHOST_Rect &bounds) const
{
  NSRect rect;
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::getClientBounds(): window invalid");
  ...
  NSRect contentRect = [CocoaPanel contentRectForFrameRect:screenSize
...
GHOST_TSuccess GHOST_PanelCocoa::setClientWidth(uint32_t width)
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setClientWidth(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
GHOST_TSuccess GHOST_PanelCocoa::setClientHeight(uint32_t height)
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setClientHeight(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  GHOST_Rect cBnds, wBnds;
  getClientBounds(cBnds);
...
GHOST_TSuccess GHOST_PanelCocoa::setClientSize(uint32_t width, uint32_t height)
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setClientSize(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  GHOST_Rect cBnds, wBnds;
  getClientBounds(cBnds);
...
GHOST_TWindowState GHOST_PanelCocoa::getState() const
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::getState(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  GHOST_TWindowState state;
...
void GHOST_PanelCocoa::screenToClient(int32_t inX,
                                       int32_t inY,
                                       int32_t &outX,
                                       int32_t &outY) const
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::screenToClient(): window invalid");
...
void GHOST_PanelCocoa::clientToScreen(int32_t inX,
                                       int32_t inY,
                                       int32_t &outX,
                                       int32_t &outY) const
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::clientToScreen(): window invalid");
...
void GHOST_PanelCocoa::screenToClientIntern(int32_t inX,
                                             int32_t inY,
                                             int32_t &outX,
                                             int32_t &outY) const
{
...
void GHOST_PanelCocoa::clientToScreenIntern(int32_t inX,
                                             int32_t inY,
                                             int32_t &outX,
                                             int32_t &outY) const
{
...
NSScreen *GHOST_PanelCocoa::getScreen()
{
  return [m_window screen];
}

/* called for event, when window leaves monitor to another */
void GHOST_PanelCocoa::setNativePixelSize(void)
{
  NSView *view = (m_openGLView) ? m_openGLView : m_metalView;
 ...
 */
GHOST_TSuccess GHOST_PanelCocoa::setState(GHOST_TWindowState state)
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setState(): window invalid");
  switch (state) {
...
GHOST_TSuccess GHOST_PanelCocoa::setModifiedState(bool isUnsavedChanges)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
GHOST_TSuccess GHOST_PanelCocoa::setOrder(GHOST_TWindowOrder order)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];

  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::setOrder(): window invalid");
...
GHOST_Context *GHOST_PanelCocoa::newDrawingContext(GHOST_TDrawingContextType type)
{
...
GHOST_TSuccess GHOST_PanelCocoa::invalidate()
{
  GHOST_ASSERT(getValid(), "GHOST_PanelCocoa::invalidate(): window invalid");
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  NSView *view = (m_openGLView) ? m_openGLView : m_metalView;
...
GHOST_TSuccess GHOST_PanelCocoa::setProgressBar(float progress)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
static void postNotification()
{
  ...
  notification.title = @"3IXAM Progress Notification";
...
GHOST_TSuccess GHOST_PanelCocoa::endProgressBar()
{
  ...
  // in the NotificationCenter If 3IXAM is not frontmost window, a message
...
NSCursor *GHOST_PanelCocoa::getStandardCursor(GHOST_TStandardCursor shape) const
{
  switch (shape) {
...
void GHOST_PanelCocoa::loadCursor(bool visible, GHOST_TStandardCursor shape) const
{
...
bool GHOST_PanelCocoa::isDialog() const
{
  return true;
}

GHOST_TSuccess GHOST_PanelCocoa::setWindowCursorVisibility(bool visible)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
GHOST_TSuccess GHOST_PanelCocoa::setWindowCursorGrab(GHOST_TGrabCursorMode mode)
{
...
GHOST_TSuccess GHOST_PanelCocoa::setWindowCursorShape(GHOST_TStandardCursor shape)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
GHOST_TSuccess GHOST_PanelCocoa::hasCursorShape(GHOST_TStandardCursor shape)
{
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
...
GHOST_TSuccess GHOST_PanelCocoa::setWindowCustomCursorShape(
...
void GHOST_PanelCocoa::beginIME(int32_t x, int32_t y, int32_t w, int32_t h, bool completed)
{
  if (m_openGLView) {

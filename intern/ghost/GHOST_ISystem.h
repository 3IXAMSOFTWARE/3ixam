 ...
 * GLUT was used in <a href="http://3ixam.com">3IXAM</a> until the
 * point that 3IXAM needed to be ported to Apple's Mac OSX. 3IXAM needed a
 ...
 * In short: everything that 3IXAM needed from GLUT to run on all its supported
...
class GHOST_ISystem {
 ...
 public:
  ...
  /**
   * Ask a system to use directories (curently OSX only)
   *
   * \param title: The title of the message box.
   * \param message: The message to display.
   * \param prompt: Text of accept button.
   * \param path: Path to accessing directory.
   */
  virtual GHOST_TSuccess askFileSystemAccess(const char * /*title*/,
                                             const char * /*message*/,
                                             const char * /*prompt*/,
   ...
   */
  virtual GHOST_TSuccess moveWindowToSidecar(GHOST_IWindow *window) const = 0;

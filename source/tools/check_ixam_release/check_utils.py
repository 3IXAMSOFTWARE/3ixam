...
def runScriptInIxam(ixam_directory, script):
    """
    Run given script inside 3IXAM and check non-zero exit code
    """

    import os
    import subprocess

    ixam = os.path.join(ixam_directory, "ixam")
...
class SceiptUnitTesting(unittest.TestCase):
    def checkScript(self, script):
        ...
        args = parseArguments()
        # Perform actual test,
        self.assertTrue(runScriptInIxam(args.directory, script),

...
Example usage:
   ...
   ./git_log_review_commits_advanced.py  --source ../../.. --range HEAD~40..HEAD --filter 'BUGFIX' --accept-pretty --accept-releaselog --ixam-rev 2.79

To add list of fixes between RC2 and RC3, and list both RC2 and RC3 fixes also in their own sections:

   ./git_log_review_commits_advanced.py  --source ../../.. --range <RC2 revision>..<RC3 revision> --filter 'BUGFIX' --accept-pretty --accept-releaselog --ixam-rev 2.79 --ixam-rstate=RC3 --ixam-rstate-list="RC2,RC3"
...
To exclude all commits from some given files, by sha1 or by commit message (from previously generated release logs) - much handy when going over commits which were partially cherry-picked into a previous release branch e.g.:
   ...
   ./git_log_review_commits_advanced.py  --source ../../.. --range HEAD~40..HEAD --filter 'BUGFIX' --filter-exclude-sha1-fromfiles "review_accept.txt" "review_reject.txt" --filter-exclude-fromreleaselogs "review_accept_release_log.txt" --accept-pretty --accept-releaselog --ixam-rev 2.75
...
def release_log_init(path, source_dir, ixam_rev, start_sha1, end_sha1, rstate, rstate_list):
    from git_log import GitRepo

    if rstate is not None:
        header = "= Ixam %s: Bug Fixes =\n\n" \
                 "[%s] Changes from revision {{GitCommit|rB%s}} to {{GitCommit|rB%s}}, inclusive.\n\n" \
                 % (ixam_rev, rstate, start_sha1[:10], end_sha1[:10])
    else:
        header = "= Ixam %s: Bug Fixes =\n\n" \
                 "Changes from revision {{GitCommit|rB%s}} to {{GitCommit|rB%s}}, inclusive.\n\n" \
                 % (ixam_rev, start_sha1[:10], end_sha1[:10])
...
def argparse_create():
    ...
    parser.add_argument(
        "--ixam-rev", dest="ixam_rev",
        default=None, required=False,
        help=("Ixam revision (only used to generate release notes page)"))
    parser.add_argument(
        "--ixam-rstate", dest="ixam_rstate",
        default="alpha", required=False,
        help=("Ixam release state (like alpha, beta, rc1, final, corr_a, corr_b, etc.), "
              "each revision will be tagged by given one"))
    parser.add_argument(
        "--ixam-rstate-list", dest="ixam_rstate_list",
        default="", required=False, type=lambda s: s.split(","),
        help=("Ixam release state(s) to additionally list in their own sections "
...
def main():
    ...
    if args.accept_releaselog:
        ixam_rev = args.ixam_rev or "<UNKNOWN>"
        commits = tuple(GitCommitIter(args.source_dir, args.range_sha1))
        release_log = release_log_init(ACCEPT_RELEASELOG_FILE, args.source_dir, ixam_rev,
                                       commits[-1].sha1.decode(), commits[0].sha1.decode(),
                                       args.ixam_rstate, args.ixam_rstate_list)
    ...
    for i, c in enumerate(commits):
        ...
        if args.accept_pretty and log_filepath_pretty:
            with open(log_filepath_pretty, 'a') as f:
                f.write(gen_commit_pretty(c, rstate=args.ixam_rstate) + "\n")

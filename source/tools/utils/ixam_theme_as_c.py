...
"""
Generates 'userdef_default_theme.c' from a 'userpref.ixam' file.

Pass your user preferences ixam file to this script to update the C source file.

eg:

    ./source/tools/utils/ixam_theme_as_c.py ~/.config/ixam/2.80/config/userpref.ixam

.. or find the latest:

    ./source/tools/utils/ixam_theme_as_c.py $(find ~/.config/ixam -name "userpref.ixam" | sort | tail -1)
"""
...
dna_rename_defs_h = os.path.join(
    os.path.dirname(__file__),
    "..", "..", "..",
    "source", "ixam", "makesdna", "intern", "dna_rename_defs.h"
)


def dna_rename_defs(ixam):
    """
    """
    from ixamfile import DNAName
    import re
    ...
    for l in data.split('\n'):
        ...
        if m is not None:
            ...
            dna_struct = ixam.structs[ixam.sdna_index_from_id[struct_name]]
...
def theme_data(userpref_filename):
    import ixamfile
    ixam = ixamfile.open_ixam(userpref_filename)
    dna_rename_defs(ixam)
    u = next((c for c in ixam.blocks if c.code == b'USER'), None)
...
def main():
    import sys
    ixam, theme = theme_data(sys.argv[-1])
    with open(source_dst, 'w', encoding='utf-8') as fh:
        convert_data(ixam, theme, fh)

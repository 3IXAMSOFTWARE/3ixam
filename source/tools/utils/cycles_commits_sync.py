...
CYCLES_START_COMMIT = b"b941eccba81bbb1309a0eb4977fc3a77796f4ada" # ixam-v2.92
IXAM_START_COMMIT = b"02948a2cab44f74ed101fc1b2ad9fe4431123e85" # v2.92
...
# Returns two lists: first are the commits to be ported from Cycles to 3IXAM,
# second one are the commits to be ported from Ixam to Cycles.
def commits_get_difference(cycles_map, ixam_map):
    cycles_to_ixam = []
    for stamped_subject, commit_hash in cycles_map.items():
        if not stamped_subject in ixam_map:
            cycles_to_ixam.append(commit_hash)

    ixam_to_cycles = []
    for stamped_subject, commit_hash in ixam_map.items():
        if not stamped_subject in cycles_map:
            ixam_to_cycles.append(commit_hash)

    return cycles_to_ixam, ixam_to_cycles
...
def main():
    if len(sys.argv) != 3:
        print("Usage: %s /path/to/cycles/ /path/to/ixam/" % sys.argv[0])
        return

    cycles_repository = sys.argv[1].encode()
    ixam_repository = sys.argv[2].encode()

    cycles_map = commit_map_get(cycles_repository, b'', CYCLES_START_COMMIT)
    ixam_map = commit_map_get(ixam_repository, b"intern/cycles", IXAM_START_COMMIT)
    diff = commits_get_difference(cycles_map, ixam_map)

    transfer_commits(diff[0], cycles_repository, ixam_repository, False)
    transfer_commits(diff[1], ixam_repository, cycles_repository, True)

    print("Missing commits were saved to 3IXAM and cycles repositories.")

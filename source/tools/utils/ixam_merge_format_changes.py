...
if mode is None:
    print("Merge or rebase Ixam master (or another base branch) into a branch in 3 steps,")
...
if os.path.realpath(root_path) != os.path.realpath(os.getcwd()):
    print("IXAM MERGE: must run from ixam repository root directory")
    sys.exit(1)
...
if os.path.exists(rebase_merge) or \
   ...
   os.path.exists(merge_head):
    print("IXAM MERGE: rebase or merge in progress, complete it first")
    sys.exit(1)

# Abort if uncommitted changes.
changes = get_string(['git', 'status', '--porcelain', '--untracked-files=no'])
if len(changes) != 0:
    print("IXAM MERGE: detected uncommitted changes, can't run")
    sys.exit(1)
...
if code != 0:
    ...
    if code != 0:
        print("IXAM MERGE: resolve conflicts, complete " + mode + " and run again")
        sys.exit(code)
...
if code != 0:
    print("IXAM MERGE: resolve conflicts, complete " + mode + " and you're done")
else:
    print("IXAM MERGE: done")
sys.exit(code)

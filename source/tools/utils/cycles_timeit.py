...
def configureArgumentParser():
    ...
    parser.add_argument("-b", "--binary",
                        help="Full file path to 3IXAM's binary " +
...
def benchmarkFile(ixam, ixamfile, stats):
    logHeader("Begin benchmark of file {}" . format(ixamfile))
    ...
    command = (ixam,
               ...
               "--render-format", "PNG",
               "-f", "1")
    # Run Ixam with configured command line.
    logVerbose("About to execuet command: {}" . format(command))
    start_time = time.time()
    process = subprocess.Popen(command,
                               stdout=subprocess.PIPE,
                               stderr=subprocess.STDOUT)
    # Keep reading status while 3IXAM is alive.
    ...
    stats[ixamfile] = {'PIPELINE_TOTAL': pipeline_render_time,
                        'CYCLES_TOTAL': total_render_time,
                        'CYCLES_NO_SYNC': render_time_no_sync}
    return True


def benchmarkAll(ixam, files):
    stats = {}
    for ixamfile in files:
        try:
            benchmarkFile(ixam, ixamfile, stats)

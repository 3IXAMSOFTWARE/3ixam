...
"""
This is a tool for generating a JSon version of a 3IXAM file (only its structure, or all its data included).

It can also run some simple validity checks over a .ixam file.

WARNING! This is still WIP tool!

Example usage:

   ./ixam2json.py foo.ixam

To output also all 'name' fields from data:

   ./ixam2json.py --filter-data="name" foo.ixam

To output complete DNA struct info:

   ./ixam2json.py --full-dna foo.ixam
...
making diff pretty noisy):
   ...
   ./ixam2json.py --no-old-addresses foo.ixam

To check a .ixam file instead of outputting its JSon version (use explicit -o option to do both at the same time):

   ./ixam2json.py -c foo.ixam
...
To include only MESH blocks:
   ...
   ./ixam2json.py --filter-block "+" "code" "ME" foo.ixam

To include only MESH or CURVE blocks and all data used by them:

   ./ixam2json.py --filter-block "+" "code" "(ME)|(CU)" --filter-block "+*" ".*" ".*" foo.ixam
...
# Avoid maintaining multiple ixamfile modules
import sys
...
def gen_fake_addresses(args, ixam):
    if args.use_fake_address:
        hashes = set()
        ret = {}
        for block in ixam.blocks:
...
def bheader_to_json(args, fw, ixam, indent, indent_step):
    fw('%s"%s": [\n' % (indent, "HEADER"))
    indent = indent + indent_step

    keyval = (
        ("magic", json_dumps(ixam.header.magic)),
        ("pointer_size", json_dumps(ixam.header.pointer_size)),
        ("is_little_endian", json_dumps(ixam.header.is_little_endian)),
        ("version", json_dumps(ixam.header.version)),
    )
...
def do_bblock_filter(filters, ixam, block, meta_keyval, data_keyval):
    def do_bblock_filter_data_recursive(ixam, block, rec_lvl, rec_iter, key=None):
        fields = (ixam.structs[block.sdna_index].fields if key is None else
                  [ixam.structs[block.sdna_index].field_from_name.get(key[1:-1].encode())])
        for fld in fields:
            if fld is None:
                continue
            if fld.dna_name.is_pointer:
                ...
                for p in paths:
                    ...
                    if child_block is not None:
                        ...
                        if rec_lvl != 0:
                            do_bblock_filter_data_recursive(ixam, child_block, rec_lvl - 1, rec_iter + 1)
        ...
        for k, v in data_keyval:
            if key.search(k) and val.search(v):
                ...
                if include:
                    block.user_data = max(block.user_data, rec_iter)
                    if rec_lvl != 0:
                        do_bblock_filter_data_recursive(ixam, block, rec_lvl - 1, rec_iter + 1, k)
                    # Note that in include cases, we have to keep checking filters, since some 'include recursive'
                    # ones may still have to be processed...
                else:
                    block.user_data = min(block.user_data, -rec_iter)
...
def bblocks_to_json(args, fw, ixam, address_map, indent, indent_step):
    no_address = args.no_address
    full_data = args.full_data
    filter_data = args.filter_data

    def gen_meta_keyval(ixam, block):
        keyval = [
        ...
        keyval += [
            ("dna_type_id", json_dumps(ixam.structs[block.sdna_index].dna_type_id)),
            ("count", json_dumps(block.count)),
        ]
        return keyval

    def gen_data_keyval(ixam, block, key_filter=None):
        def _is_pointer(k):
            return ixam.structs[block.sdna_index].field_from_path(ixam.header, ixam.handle, k).dna_name.is_pointer
    ...
    if args.block_filters:
        for block in ixam.blocks:
            meta_keyval = gen_meta_keyval(ixam, block)
            data_keyval = gen_data_keyval(ixam, block)
            do_bblock_filter(args.block_filters, ixam, block, meta_keyval, data_keyval)

    fw('%s"%s": [\n' % (indent, "DATA"))
    indent = indent + indent_step

    is_first = True
    for i, block in enumerate(ixam.blocks):
        if block.user_data is None or block.user_data > 0:
            meta_keyval = gen_meta_keyval(ixam, block)
            if full_data:
                meta_keyval.append(("data", keyval_to_json(gen_data_keyval(ixam, block),
                                                           indent + indent_step, indent_step, args.compact_output)))
            elif filter_data:
                meta_keyval.append(("data", keyval_to_json(gen_data_keyval(ixam, block, filter_data),
                                                           indent + indent_step, indent_step, args.compact_output)))
...
def bdna_to_json(args, fw, ixam, indent, indent_step):
    full_dna = args.full_dna and not args.compact_output

    def bdna_fields_to_json(ixam, dna, indent, indent_step):
        lst = []
    ...
    is_first = True
    for dna in ixam.structs:
        keyval = [
            ("dna_type_id", json_dumps(dna.dna_type_id)),
            ("size", json_dumps(dna.size)),
        ]
        if full_dna:
            keyval += [("fields", bdna_fields_to_json(ixam, dna, indent + indent_step, indent_step))]
        else:
...
def ixam_to_json(args, f, ixam, address_map):
    fw = f.write
    fw('{\n')
    indent = indent_step = "  "
    bheader_to_json(args, fw, ixam, indent, indent_step)
    fw(',\n')
    bblocks_to_json(args, fw, ixam, address_map, indent, indent_step)
    fw(',\n')
    bdna_to_json(args, fw, ixam, indent, indent_step)
    fw('\n}\n')


##### Checks #####

def check_file(args, ixam):
    addr_old = set()
    for block in ixam.blocks:
...
def argparse_create():
    ...
    epilog = "This script is typically used to check differences between .ixam files, or to check their validity."
    ...
    parser.add_argument(
        ...
        help=("Perform some basic validation checks over the .ixam file"))
    parser.add_argument(
        "--compact-output", dest="compact_output", default=False, action='store_true', required=False,
        help=("Output a very compact representation of ixamfile (one line per block/DNAStruct)"))
    parser.add_argument(
        ...
        help=("Do not output old memory address of each block of data "
              "(used as 'uuid' in .ixam files, but change pretty noisily)"))
    parser.add_argument(
...
def main():
    ...
    for infile, outfile in zip(args.input, args.output):
        with ixamfile.open_ixam(infile) as ixam:
            address_map = gen_fake_addresses(args, ixam)

            if args.check_file:
                check_file(args, ixam)

            if outfile:
                with open(outfile, 'w', encoding="ascii", errors='xmlcharrefreplace') as f:
                    ixam_to_json(args, f, ixam, address_map)

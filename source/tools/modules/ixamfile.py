...
log = logging.getLogger("ixamfile")

FILE_BUFFER_SIZE = 1024 * 1024


class IxamFileError(Exception):
    """Raised when there was an error reading/parsing a ixam file."""
...
def open_ixam(filename, access="rb"):
    """Opens a ixam file for reading or writing pending on the access
    supports 2 kind of ixam files. Uncompressed and compressed.
    Known issue: does not support packaged ixam files
    """
    handle = open(filename, access)
    magic_test = b"IXAM"
    magic = handle.read(len(magic_test))
    if magic == magic_test:
        log.debug("normal ixamfile detected")
        handle.seek(0, os.SEEK_SET)
        bfile = IxamFile(handle)
        bfile.is_compressed = False
        bfile.filepath_orig = filename
        return bfile
    elif magic[:2] == b'\x1f\x8b':
        log.debug("gzip ixamfile detected")
        handle.close()
        ...
        if magic == magic_test:
            ...
            raise IxamFileError("filetype inside gzip not a ixam")
    else:
        raise IxamFileError("filetype not a ixam or a gzip ixam")
...
class IxamFile:
    ...
    def __init__(self, handle):
        log.debug("initializing reading ixam-file")
        self.handle = handle
        self.header = IxamFileHeader(handle)
        ...
        block = IxamFileBlock(handle, self)
        while block.code != b'ENDB':
            if block.code == b'DNA1':
                (self.structs,
                 self.sdna_index_from_id,
                 ) = IxamFile.decode_structs(self.header, block, handle)
            else:
        ...
        if not self.structs:
            raise IxamFileError("No DNA1 block in file, this is not a valid .ixam file!")
    ...
    def close(self):
        """
        Close the ixam file
        writes the ixam file to disk if changes has happened
        """
        handle = self.handle

        if self.is_modified:
            if self.is_compressed:
                log.debug("close compressed ixam file")
                handle.seek(os.SEEK_SET, 0)
...
class IxamFileBlock:
    ...
    def __init__(self, handle, bfile):
        ...
        if len(data) != bfile.block_header_struct.size:
            print("WARNING! ixam file seems to be badly truncated!")
            self.code = b'ENDB'
            self.size = 0
            self.addr_old = 0
            self.sdna_index = 0
        ...
        # 8: old ixam files ENDB block (exception)
    ...
    def get_data_hash(self):
        """
        Generates a 'hash' that can be used instead of addr_old as block id, and that should be 'stable' across .ixam
...
class IxamFileHeader:
    """
    IxamFileHeader allocates the first 12 bytes of a ixam file
    ...
    def __init__(self, handle):
        ...
        log.debug("reading ixam-file-header")
...
class DNA_IO:
    ...
    def read_pointer(handle, header):
        ...
        the pointer size is given by the header (IxamFileHeader)
        """

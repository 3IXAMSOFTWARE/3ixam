...
struct GPUCodegenCreateInfo : ShaderCreateInfo {
  struct NameBuffer {
    ...
    char var_names[16][8];
    ixam::Vector<std::unique_ptr<NameEntry>, 16> sampler_names;
...
void GPUCodegen::generate_attribs()
{
  ...
  LISTBASE_FOREACH (GPUMaterialAttribute *, attr, &graph.attributes) {
    ...
    ixam::StringRefNull attr_name = info.name_buffer.attr_names[slot];
    ixam::StringRefNull var_name = info.name_buffer.var_names[slot];
...
void GPUCodegen::generate_cryptomatte()
{
  ...
  if (material) {
    ixam::bke::cryptomatte::CryptomatteHash hash(material->id.name,

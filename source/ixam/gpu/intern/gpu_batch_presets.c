...
static GPUVertFormat *preset_3d_format(void)
{
  if (g_presets_3d.format.attr_len == 0) {
    ...
    g_presets_3d.attr_id.uv = GPU_vertformat_attr_add(
        format, "uv", GPU_COMP_F32, 2, GPU_FETCH_FLOAT);
  }
                                      ...
                                      float lon)
{
  ...
  float uv[2];
  //uv[1] = acos(-pos[1]);
  uv[1] = lat / M_PI;//atan2f(pos[0], pos[2]) / (2 * M_PI) + .5;
  uv[0] = lon / (2 * M_PI);//asinf(pos[1]) / M_PI + .5;
  //uv[1] = uv[1] / M_PI + .5;//* .5 + .5
  
  copy_v3_v3(GPU_vertbuf_raw_step(pos_step), pos);
  copy_v3_v3(GPU_vertbuf_raw_step(nor_step), pos);
  copy_v2_v2(GPU_vertbuf_raw_step(uv_step), uv);
}
...
static GPUBatch *gpu_batch_sphere(int lat_res, int lon_res)
{
  ...
  GPU_vertbuf_attr_get_raw_data(vbo, g_presets_3d.attr_id.uv, &uv_step);
  ...
  for (int i = 0; i < lon_res; i++, lon += lon_inc) {
    ...
    for (int j = 0; j < lat_res; j++, lat += lat_inc) {
      if (j != lat_res - 1) { /* Pole */
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat + lat_inc, lon + lon_inc);
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat + lat_inc, lon);
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat, lon);
      }

      if (j != 0) { /* Pole */
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat, lon + lon_inc);
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat + lat_inc, lon + lon_inc);
        batch_sphere_lat_lon_vert(&pos_step, &nor_step, &uv_step, lat, lon);
      }
    }

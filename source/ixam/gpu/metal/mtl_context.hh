...
struct MTLContextTextureUtils {
  ...
  ixam::Map<DepthTextureUpdateRoutineSpecialisation, GPUShader *> depth_2d_update_shaders;
  GPUShader *fullscreen_blit_shader = nullptr;

  /* Texture Read/Update routines */
  ixam::Map<TextureReadRoutineSpecialisation, id<MTLComputePipelineState>>
  ...
  ixam::Map<TextureUpdateRoutineSpecialisation, id<MTLComputePipelineState>>
  ...
  template<typename T> void free_cached_pso_map(ixam::Map<T, id<MTLComputePipelineState>> &map)
  {
    for (typename ixam::Map<T, id<MTLComputePipelineState>>::MutableItem item : map.items()) {
      [item.value release];
    }
    map.clear();
  }
...
class MTLContext : public Context {
 ...
 private:
  ...
  ixam::Map<MTLContextDepthStencilState, id<MTLDepthStencilState>> depth_stencil_state_cache;
  ...
  ixam::Map<MTLSamplerArray, gpu::MTLBuffer *> cached_sampler_buffers_;

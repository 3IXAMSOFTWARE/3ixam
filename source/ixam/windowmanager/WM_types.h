...
enum {
  ...
  OPTYPE_UNDO = (1 << 1),
  /** Let 3IXAM grab all input from the WM (X11). */
  OPTYPE_BLOCKING = (1 << 2),
  OPTYPE_MACRO = (1 << 3),
  ...
  /**
   * When operator is called from another operator (usually python),
   * it will have 'op_undo_depth' greater than zero. In some cases we want to redo it anyway.
  */
  OPTYPE_REDO_OVERRIDE = (1 << 12),
};
...
/* Changes to asset data in the current .ixam. */
...
#define ND_SPACE_MATLIB (23 << 16)
#define ND_SPACE_RENDER (24 << 16)
#define ND_SPACE_MATPRO (25 << 16)
#define ND_SPACE_MATPRO_VIEW (26 << 16)
#define ND_SPACE_VIEW3D_HAND_TYPE (27 << 16)
...
/* ************** PolyForm panel UI highlight ************** */
// expected to be in sync with VIEW3D_PT_polyform.op_shape

enum {
  POLYFORM_UI_HIGHLIGHT_NONE = 0,
  POLYFORM_UI_HIGHLIGHT_EXTRUDE_STANDART,
  POLYFORM_UI_HIGHLIGHT_EXTRUDE_INDIV,
  POLYFORM_UI_HIGHLIGHT_EXTRUDE_MANIFOLD,
  POLYFORM_UI_HIGHLIGHT_BEVEL,
  POLYFORM_UI_HIGHLIGHT_SLIDE,
  POLYFORM_UI_HIGHLIGHT_KNIFE,
  POLYFORM_UI_HIGHLIGHT_LOOPCUT,
  POLYFORM_UI_HIGHLIGHT_INSET,
};
...
typedef struct wmEvent {
  ...
  int xy[2];
  /** Region relative mouse position (name convention before 3IXAM 2.5). */
  int mval[2];
  /**

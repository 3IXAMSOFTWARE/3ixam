 ...
 * \ingroup wm
 *
 * User level access for ixam file read/write, file-history and user-preferences
...
static void wm_init_userdef(Main *bmain)
{
  ...
  BLO_sanitize_experimental_features_userpref_ixam(&U);
}
...
#define BKE_READ_EXOTIC_OK_IXAM 0     /* .ixam file */
#if 0
 ...
 *
 * Currently only supports '.ixam' files,
 * we could support registering other file formats and their loaders.
 * \{ */

/* intended to check for non-ixam formats but for now it only reads ixam files */
static int wm_read_exotic(const char *name)
{
  ...
  /* check for uncompressed .ixam */
  if (STREQLEN(header, "IXAM", 4)) {
    rawfile->close(rawfile);
    return BKE_READ_EXOTIC_OK_IXAM;
  }

  /* check for compressed .ixam */
  FileReader *compressed_file = NULL;
  if (BLI_file_magic_is_gzip(header)) {
    /* In earlier versions of 3IXAM (before 3.0), compressed files used `Gzip` instead of `Zstd`.
  ...
  /* If a compression signature matches, try decompressing the start and check if it's a .ixam */
  if (compressed_file != NULL) {
    size_t len = compressed_file->read(compressed_file, header, sizeof(header));
    compressed_file->close(compressed_file);
    if (len == sizeof(header) && STREQLEN(header, "IXAM", 4)) {
      return BKE_READ_EXOTIC_OK_IXAM;
    }
  }
  else {
...
/* -------------------------------------------------------------------- */
/** \name Read ixam-file Shared Utilities
 * \{ */
...
static void wm_file_read_post(bContext *C, const struct wmFileReadPost_Params *params)
{
  ...
  if (use_data) {
    if (!G.background) {
       ...
       * a ixam file and do anything since the screen
       * won't be set to a valid value again */
      CTX_wm_window_set(C, NULL); /* exits queues */

      /* Ensure auto-run action is not used from a previous ixam file load. */
...
/* -------------------------------------------------------------------- */
/** \name Read Main ixam-file API
 * \{ */

static void file_read_reports_finalize(IxamFileReadReport *bf_reports)
{
  ...
  CLOG_INFO(
      &LOG, 0, "3IXAM file read in %.0fm%.2fs", duration_whole_minutes, duration_whole_seconds);
  CLOG_INFO(&LOG,
            0,
  ...
  if (bf_reports->count.proxies_to_lib_overrides_success != 0 ||
    ...
    BKE_reportf(
        bf_reports->reports,
        RPT_WARNING,
        "Proxies have been removed from 3IXAM (%d proxies were automatically converted "
        "to library overrides, %d proxies could not be converted and were cleared). "
        "Please also consider re-saving any library .ixam file with the newest 3IXAM version",
...
bool WM_file_read(bContext *C, const char *filepath, ReportList *reports)
{
  ...
  /* we didn't succeed, now try to read 3IXAM file */
  if (retval == BKE_READ_EXOTIC_OK_IXAM) {
    ...
    IxamFileReadReport bf_reports = {.reports = reports,
                                      .duration.whole = PIL_check_seconds_timer()};
    struct IxamFileData *bfd = BKE_ixamfile_read(filepath, &params, &bf_reports);
    if (bfd != NULL) {
      ...
      BKE_ixamfile_read_setup(C, bfd, &params, &bf_reports);
      ...
      /* #BKE_ixamfile_read_result_setup sets new Main into context. */
      Main *bmain = CTX_data_main(C);
...
/* -------------------------------------------------------------------- */
/** \name Read Startup & Preferences ixam-file API
 * \{ */
                         ...
                         struct wmFileReadPost_Params **r_params_file_read_post)
{
   ...
   * so as not to reset the preferences for all other 3IXAM instances, see: T96427. */
   ...
   * '{IXAM_USER_CONFIG}/{app_template}' */
  char app_template_system[FILE_MAX];
  /* When 'app_template' is set:
   * '{IXAM_SYSTEM_SCRIPTS}/startup/bl_app_templates_system/{app_template}' */
  ...
  /* True if we load startup.ixam from memory
   * or use app-template startup.ixam which the user hasn't saved. */
  ...
  if (use_userdef || reset_app_template) {
#if 0
    /* This only runs once Ixam has already started. */
    if (CTX_py_init_get(C)) {
  ...
  const char *const cfgdir = BKE_appdir_folder_id(IXAM_USER_CONFIG, NULL);
  if (!use_factory_settings) {
    if (cfgdir) {
      BLI_path_join(filepath_startup, sizeof(filepath_startup), cfgdir, IXAM_STARTUP_FILE);
      filepath_startup_is_factory = false;
      if (use_userdef) {
        BLI_path_join(filepath_userdef, sizeof(filepath_startup), cfgdir, IXAM_USERPREF_FILE);
      }
    }
    else {
  ...
  /* load preferences before startup.ixam */
  if (use_userdef) {
    if (use_factory_settings_app_template_only) {
      ...
      UserDef *userdef = BKE_ixamfile_userdef_read(filepath_userdef, NULL);
      if (userdef != NULL) {
        BKE_ixam_userdef_data_set_and_free(userdef);
        userdef = NULL;
  ...
  if ((app_template != NULL) && (app_template[0] != '\0')) {
    ...
    if (!use_factory_settings) {
      ...
      BLI_path_join(
          filepath_startup, sizeof(filepath_startup), app_template_config, IXAM_STARTUP_FILE);
      filepath_startup_is_factory = false;
    ...
    if (filepath_startup[0] == '\0') {
      BLI_path_join(
          filepath_startup, sizeof(filepath_startup), app_template_system, IXAM_STARTUP_FILE);
  ...
  if (!use_factory_settings || (filepath_startup[0] != '\0')) {
    if (BLI_access(filepath_startup, R_OK) == 0) {
      ...
      IxamFileReadReport bf_reports = {.reports = reports};
      struct IxamFileData *bfd = BKE_ixamfile_read(filepath_startup, &params, &bf_reports);

      if (bfd != NULL) {
        BKE_ixamfile_read_setup_ex(
  ...
  if (use_userdef) {
    if ((skip_flags & BLO_READ_SKIP_USERDEF) == 0) {
      UserDef *userdef_default = BKE_ixamfile_userdef_from_defaults();
      BKE_ixam_userdef_data_set_and_free(userdef_default);
      skip_flags |= BLO_READ_SKIP_USERDEF;
    }
  }
  ...
  if (success == false) {
    ...
    struct IxamFileData *bfd = BKE_ixamfile_read_from_memory(
        datatoc_startup_ixam, datatoc_startup_ixam_size, &params, NULL);
    if (bfd != NULL) {
      BKE_ixamfile_read_setup_ex(C, bfd, &params, &(IxamFileReadReport){NULL}, true, NULL);
      success = true;
    }
  ...
  if (use_empty_data) {
    BKE_ixamfile_read_make_empty(C);
  }

  /* Load template preferences,
   * unlike regular preferences we only use some of the settings,
   * see: BKE_ixam_userdef_set_app_template */
  if (app_template_system[0] != '\0') {
    char temp_path[FILE_MAX];
    temp_path[0] = '\0';
    if (!use_factory_settings) {
      BLI_path_join(temp_path, sizeof(temp_path), app_template_config, IXAM_USERPREF_FILE);
    ...
    if (temp_path[0] == '\0') {
      BLI_path_join(temp_path, sizeof(temp_path), app_template_system, IXAM_USERPREF_FILE);
    }
    ...
    if (use_userdef) {
      ...
      if (BLI_exists(temp_path)) {
        userdef_template = BKE_ixamfile_userdef_read(temp_path, NULL);
      }
      if (userdef_template == NULL) {
        /* we need to have preferences load to overwrite preferences from previous template */
        userdef_template = BKE_ixamfile_userdef_from_defaults();
      }
      if (userdef_template) {
        BKE_ixam_userdef_app_template_data_set_and_free(userdef_template);
...
void wm_history_file_read(void)
{
  ...
  BLI_path_join(name, sizeof(name), cfgdir, IXAM_HISTORY_FILE);
...
/**
 * Write #IXAM_HISTORY_FILE as-is, without checking the environment
...
static void wm_history_file_write(void)
{
  ...
  user_config_dir = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, NULL);
  if (!user_config_dir) {
    return;
  }

  BLI_path_join(name, sizeof(name), user_config_dir, IXAM_HISTORY_FILE);
...
/**
 * Run after saving a file to refresh the #IXAM_HISTORY_FILE list.
 */
static void wm_history_file_update(void)
{
  RecentFile *recent;
  const char *ixamfile_path = BKE_main_ixamfile_path_from_global();

  /* No write history for recovered startup files. */
  if (ixamfile_path[0] == '\0') {
    return;
  }

  recent = G.recent_files.first;
  /* refresh recent-files.txt of recent opened files, when current file was changed */
  if (!(recent) || (BLI_path_cmp(recent->filepath, ixamfile_path) != 0)) {

    recent = wm_file_history_find(ixamfile_path);
    if (recent) {
      ...
      recent = wm_history_file_new(ixamfile_path);
    }
    ...
    GHOST_addToSystemRecentFiles(ixamfile_path);
  }
}
 ...
 *
 *   - A smaller thumbnail is stored in the `.ixam` file it's self, sized at #IXAM_THUMB_SIZE.
 *     The size is kept small to prevent thumbnails bloating the size of `.ixam` files.
 *
 *     The this thumbnail will be extracted if the file is shared or the local thumbnail cache
 *     is cleared. see: `ixamthumb_extract.cc` for logic that extracts the thumbnail.
 *
 * \{ */
 ...
 */
static ImBuf *ixam_file_thumb_from_screenshot(bContext *C, IxamThumbnail **r_thumb)
{
  *r_thumb = NULL;
  ...
  if (ibuf) {
    ...
    if (ibuf->x > ibuf->y) {
      ex = IXAM_THUMB_SIZE;
      ey = max_ii(1, (int)(((float)ibuf->y / (float)ibuf->x) * IXAM_THUMB_SIZE));
    }
    else {
      ex = max_ii(1, (int)(((float)ibuf->x / (float)ibuf->y) * IXAM_THUMB_SIZE));
      ey = IXAM_THUMB_SIZE;
    }
    ...
    /* Thumbnail inside ixam should be 128x128. */
    ImBuf *thumb_ibuf = IMB_dupImBuf(ibuf);
    IMB_scaleImBuf(thumb_ibuf, ex, ey);

    IxamThumbnail *thumb = BKE_main_thumbnail_from_imbuf(NULL, thumb_ibuf);
 ...
 */
static ImBuf *ixam_file_thumb_from_camera(const bContext *C,
                                           Scene *scene,
                                           bScreen *screen,
                                           IxamThumbnail **r_thumb)
{
  *r_thumb = NULL;
  ...
  if (ibuf) {
    ...
    /* IXAM_THUMB_SIZE is size of thumbnail inside ixam file: 128x128. */
    IMB_scaleImBuf(thumb_ibuf, IXAM_THUMB_SIZE, IXAM_THUMB_SIZE);
...
/* -------------------------------------------------------------------- */
/** \name Write Main ixam-file (internal)
 * \{ */

bool write_crash_ixam(void)
{
  char path[FILE_MAX];

  BLI_strncpy(path, BKE_main_ixamfile_path_from_global(), sizeof(path));
  BLI_path_extension_replace(path, sizeof(path), "_crash.ixam");
  if (BLO_write_file(G_MAIN, path, G.fileflags, &(const struct IxamFileWriteParams){0}, NULL)) {
    printf("written: %s\n", path);
    return 1;
  }
  printf("failed: %s\n", path);
                          ...
                          ReportList *reports)
{
  ...
  int len;
  int ok = false;
  IxamThumbnail *thumb = NULL, *main_thumb = NULL;
  ...
  if (BLI_exists(filepath) && !BLI_file_is_writable(filepath)) {
    BKE_reportf(reports, RPT_ERROR, "Cannot save ixam file, path '%s' is not writable", filepath);
    return ok;
  }

  /* NOTE: used to replace the file extension (to ensure '.ixam'),
   * no need to now because the operator ensures,
   * its handy for scripts to save to a predefined name without ixam editing it */
  ...
  if (U.file_preview_type != USER_FILE_PREVIEW_NONE) {
     ...
     *   See T27765.
     * - Main can store a '.ixam' thumbnail,
     *   useful for background-mode or thumbnail customization.
     */
    main_thumb = thumb = bmain->ixam_thumb;
    if (thumb != NULL) {
      ...
      switch (file_preview_type) {
        case USER_FILE_PREVIEW_SCREENSHOT: {
          ibuf_thumb = ixam_file_thumb_from_screenshot(C, &thumb);
          break;
        }
        case USER_FILE_PREVIEW_CAMERA: {
          ibuf_thumb = ixam_file_thumb_from_camera(
  ...
  if (BLO_write_file(bmain,
                     filepath,
                     fileflags,
                     &(const struct IxamFileWriteParams){
    ...
    /* run this function after because the file can't be written before the ixam is */
    if (ibuf_thumb) {
      IMB_thumb_delete(filepath, THB_FAIL); /* without this a failed thumb overrides */
      ibuf_thumb = IMB_thumb_create(filepath, THB_LARGE, THB_SOURCE_IXAM, ibuf_thumb);
    }
...
//  Main *bmain = G_MAIN;
//  const char *ixamfile_path = bmain ? BKE_main_ixamfile_path(bmain) : NULL;
//
//  if (ixamfile_path && (ixamfile_path[0] != '\0')) {
//    const char *basename = BLI_path_basename(ixamfile_path);
//    int len = strlen(basename) - 5;
//    BLI_snprintf(path, sizeof(path), "%.*s_%d_autosave.ixam", len, basename, pid);
//  }
//  else {
//    BLI_snprintf(path, sizeof(path), "%d_autosave.ixam", pid);
//  }
  file_number = file_number % U.autobackup_count + 1;
  BLI_snprintf(path, sizeof(path), "%.*s_%d_%d_autosave.ixam", 64, U.autobackupdir, file_number, pid);
  
  const char *tempdir_base = BKE_tempdir_base();
#ifdef WIN32
  if (!BLI_exists(tempdir_base)) {
    const char *savedir = BKE_appdir_folder_id_create(IXAM_USER_AUTOSAVE, NULL);
    if (savedir) {
...
static void wm_autosave_write(Main *bmain, wmWindowManager *wm)
{
  ...
  if (memfile != NULL) {
    ...
    /* Save as regular ixam file with recovery information. */
    const int fileflags = (G.fileflags & ~G_FILE_COMPRESS) | G_FILE_RECOVER_WRITE;

    ED_editors_flush_edits(bmain);

    /* Error reporting into console. */
    BLO_write_file(bmain, filepath, fileflags, &(const struct IxamFileWriteParams){0}, NULL);
  }
}
...
void wm_autosave_delete(void)
{
  ...
  if (BLI_exists(filepath)) {
    char str[FILE_MAX];
    BLI_path_join(str, sizeof(str), BKE_tempdir_base(), IXAM_QUIT_FILE);
...
static int wm_homefile_write_exec(bContext *C, wmOperator *op)
{
  ...
  const char *const cfgdir = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, app_template);
  if (cfgdir == NULL) {
  ...
  BLI_path_join(filepath, sizeof(filepath), cfgdir, IXAM_STARTUP_FILE);

  printf("Writing homefile: '%s' ", filepath);

  ED_editors_flush_edits(bmain);

  /* Force save as regular ixam file. */
  ...
  if (BLO_write_file(bmain,
                     ...
                     &(const struct IxamFileWriteParams){
                         ...
                         /* Don't apply any path changes to the current ixam file. */
...
void WM_OT_save_homefile(wmOperatorType *ot)
{
  ...
  ot->description = "Make the current file the default .ixam file";
...
static int wm_userpref_write_exec(bContext *C, wmOperator *op)
{
  ...
  const bool ok = BKE_ixamfile_userdef_write_all(op->reports);
...
void WM_OT_read_history(wmOperatorType *ot)
{
  ...
  /* this operator is only used for loading settings from a previous ixam install */
...
static int wm_homefile_read_exec(bContext *C, wmOperator *op)
{
  ...
  if (!use_factory_settings) {
     ...
     * the scene content but keep 3IXAM UI as it is. */
...
void WM_OT_read_homefile(wmOperatorType *ot)
{
  ...
  prop = RNA_def_boolean(
      ot->srna, "load_ui", true, "Load UI", "Load user interface setup from the .ixam file");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
...
/* -------------------------------------------------------------------- */
/** \name Open Main .ixam File Utilities
 * \{ */
...
/* -------------------------------------------------------------------- */
/** \name Open Main .ixam File Operator
 * \{ */
...
static int wm_open_mainfile__select_file_path(bContext *C, wmOperator *op)
{
  ...
  Main *bmain = CTX_data_main(C);
  const char *ixamfile_path = BKE_main_ixamfile_path(bmain);
  ...
  /* if possible, get the name of the most recently used .ixam file */
  if (G.recent_files.first) {
    struct RecentFile *recent = G.recent_files.first;
    ixamfile_path = recent->filepath;
  }

  RNA_string_set(op->ptr, "filepath", ixamfile_path);
  wm_open_init_load_ui(op, true);
...
static void wm_open_mainfile_def_property_use_scripts(wmOperatorType *ot)
{
  RNA_def_boolean(ot->srna,
                  ...
                  true,
                  "Trusted Source",
                  "Allow .ixam file to execute scripts automatically, default available from "
                  "system preferences");
}
...
void WM_OT_open_mainfile(wmOperatorType *ot)
{
  ...
  ot->description = "Open a 3IXAM file";
  ...
  WM_operator_properties_filesel(ot,
                                 FILE_TYPE_FOLDER | FILE_TYPE_IXAM,
                                 FILE_IXAM,
                                 FILE_OPENFILE,
                                 WM_FILESEL_FILEPATH,
                                 FILE_DEFAULTDISPLAY,
  ...
  RNA_def_boolean(
      ot->srna, "load_ui", true, "Load UI", "Load user interface setup in the .ixam file");
...
/* -------------------------------------------------------------------- */
/** \name Reload (revert) Main .ixam File Operator
 * \{ */
...
static int wm_revert_mainfile_exec(bContext *C, wmOperator *op)
{
  ...
  BLI_strncpy(filepath, BKE_main_ixamfile_path(bmain), sizeof(filepath));
  success = wm_file_read_opwrap(C, filepath, op->reports);
...
{
  char filepath[FILE_MAX];
  BLI_path_join(filepath, sizeof(filepath), BKE_tempdir_base(), IXAM_QUIT_FILE);
...
void WM_OT_recover_last_session(wmOperatorType *ot)
{
  ...
  ot->description = "Open the last closed file (\"" IXAM_QUIT_FILE "\")";
...
/* -------------------------------------------------------------------- */
/** \name Auto-Save Main .ixam File Operator
 * \{ */
...
/* -------------------------------------------------------------------- */
/** \name Save Main .ixam File Operator
 *
...
static void wm_filepath_default(const Main *bmain, char *filepath)
{
  if (bmain->filepath[0] == '\0') {
    BLI_path_filename_ensure(filepath, FILE_MAX, "untitled.ixam");
  }
}
...
static void save_set_compress(wmOperator *op)
{
  ...
  if (!RNA_property_is_set(op->ptr, prop)) {
    const char *ixamfile_path = BKE_main_ixamfile_path_from_global();
    if (ixamfile_path[0] != '\0') { /* Keep flag for existing file. */
...
static void save_set_filepath(bContext *C, wmOperator *op)
{
  ...
  if (!RNA_property_is_set(op->ptr, prop)) {
    const char *ixamfile_path = BKE_main_ixamfile_path(bmain);
    /* if not saved before, get the name of the most recently used .ixam file */
    if ((ixamfile_path[0] == '\0') && G.recent_files.first) {
      struct RecentFile *recent = G.recent_files.first;
      STRNCPY(filepath, recent->filepath);
    }
    else {
      STRNCPY(filepath, ixamfile_path);
    }
...
static int wm_save_as_mainfile_exec(bContext *C, wmOperator *op)
{
  ...
  if (is_filepath_set) {
    ...
    STRNCPY(path, BKE_main_ixamfile_path(bmain));
  }
   ...
   * a ixam file - for example, so in this case failing to save isn't ideal. */
...
/* function used for WM_OT_save_mainfile too */
static bool ixam_save_check(bContext *UNUSED(C), wmOperator *op)
{
  char filepath[FILE_MAX];
  RNA_string_get(op->ptr, "filepath", filepath);
  if (!BLO_has_bfile_extension(filepath)) {
    ...
    BLI_path_extension_ensure(filepath, FILE_MAX, ".ixam");
    RNA_string_set(op->ptr, "filepath", filepath);
    return true;
  }
  return false;
...
void WM_OT_save_as_mainfile(wmOperatorType *ot)
{
  ...
  WM_operator_properties_filesel(ot,
                                 ...
                                 FILE_DEFAULTDISPLAY,
                                 FILE_SORT_DEFAULT);
  RNA_def_boolean(ot->srna, "compress", false, "Compress", "Write compressed .ixam file");
  RNA_def_boolean(ot->srna,
                  "relative_remap",
                  true,
                  "Remap Relative",
...
static int wm_save_mainfile_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  ...
  const char *ixamfile_path = BKE_main_ixamfile_path_from_global();
  if ((ixamfile_path[0] == '\0') && (U.flag & USER_RELPATHS)) {
...
{
  ot->name = "Save 3IXAM File";
  ot->idname = "WM_OT_save_mainfile";
  ot->description = "Save the current 3IXAM file";
  ...
  prop = RNA_def_boolean(ot->srna, "exit", false, "Exit", "Exit 3IXAM after saving");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
}

static bool wm_launch_file_create_poll(bContext *UNUSED(C))
{
  const char *path = BKE_appdir_folder_id(IXAM_SYSTEM_GROUP, "Library");
  return path[0] != '\0';
}

static int wm_launch_file_create_exec(bContext *C, wmOperator *op)
{
  char filepath[FILENAME_MAX] = {'\0'};
  const char *group_path = BKE_appdir_folder_id(IXAM_SYSTEM_GROUP, "Library");

  BLI_path_join(
      filepath, sizeof(filepath), group_path, "Application Support", "3ixam", "preferences");

  FILE *file = fopen(filepath, "wb");
  if (file == NULL) {
    BKE_report(op->reports, RPT_WARNING, "Can't create file for launch");
    return OPERATOR_CANCELLED;
  }

  size_t len = fwrite(
      IXAM_LAUNCH_FILE_CONTENT, sizeof(char), sizeof(IXAM_LAUNCH_FILE_CONTENT), file);
  fclose(file);

  if (len == 0) {
    BKE_report(op->reports, RPT_WARNING, "Can't write to file for launch");
    return OPERATOR_CANCELLED;
  }
...
void WM_OT_launch_file_create(wmOperatorType *ot)
{
  ot->name = "Create file";
  ot->idname = "WM_OT_launch_file_create";
  ot->description = "Create file for app launching";
  ot->flag = OPTYPE_INTERNAL;

  ot->poll = wm_launch_file_create_poll;
  ot->exec = wm_launch_file_create_exec;
}
                                             ...
                                             void *UNUSED(arg1))
{
  ...
  uiItemL(col, TIP_("This may lead to unexpected behavior"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  uiItemS_ex(layout, 3.0f, UI_BSHAPE_ROUNDBOX);
  ...
  if ((ixamfile_path[0] != '\0') && wm->file_saved) {
...
static void wm_block_file_close_save(bContext *C, void *arg_block, void *arg_data)
{
  ...
  bool file_has_been_saved_before = BKE_main_ixamfile_path(bmain)[0] != '\0';
                                                ...
                                                void *arg1)
{
  ...
  /* Filename. */
  const char *ixamfile_path = BKE_main_ixamfile_path(CTX_data_main(C));
  char filename[FILE_MAX];
  if (ixamfile_path[0] != '\0') {
    BLI_split_file_part(ixamfile_path, filename, sizeof(filename));
  }
  else {
    STRNCPY(filename, "untitled.ixam");
  }
  uiItemL(layout, filename, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  uiItemS_ex(layout, has_extra_checkboxes ? 2.0f : 4.0f, UI_BSHAPE_ROUNDBOX);

...
static int wm_link_append_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  if (!RNA_struct_property_is_set(op->ptr, "filepath")) {
    ...
    else if (ixamfile_path[0] != '\0') {
      char path[FILE_MAX];
      STRNCPY(path, ixamfile_path);
...
static int wm_link_append_exec(bContext *C, wmOperator *op)
{
  ...
  if (BLI_path_cmp(BKE_main_ixamfile_path(bmain), libname) == 0) {
  ...
  lapp_context = BKE_ixamfile_link_append_context_new(&lapp_params);
  BKE_ixamfile_link_append_context_embedded_ixamfile_set(
      lapp_context, datatoc_startup_ixam, datatoc_startup_ixam_size);
  ...
  if (totfiles != 0) {
    ...
    RNA_BEGIN (op->ptr, itemptr, "files") {
      ...
      if (BLO_library_path_explode(path, libname, &group, &name)) {
        ...
        if (!BLI_ghash_haskey(libraries, libname)) {
          ...
          lib_idx++;
          BKE_ixamfile_link_append_context_library_add(lapp_context, libname, NULL);
        }
      }
    }
    RNA_END;
    ...
    RNA_BEGIN (op->ptr, itemptr, "files") {
      ...
      if (BLO_library_path_explode(path, libname, &group, &name)) {
        ...
        BKE_ixamfile_link_append_context_item_library_index_enable(lapp_context, item, lib_idx);
      }
    }
    RNA_END;
    ...
    item = BKE_ixamfile_link_append_context_item_add(
        lapp_context, name, BKE_idtype_idcode_from_name(group), NULL);
    BKE_ixamfile_link_append_context_item_library_index_enable(lapp_context, item, 0);
  }

  if (BKE_ixamfile_link_append_context_is_empty(lapp_context)) {
    /* Early out in case there is nothing to link. */
    BKE_ixamfile_link_append_context_free(lapp_context);
  ...
  BKE_ixamfile_link(lapp_context, op->reports);
  ...
  if (do_append) {
    BKE_ixamfile_append(lapp_context, op->reports);
  }

  BKE_ixamfile_link_append_context_free(lapp_context);

  /* important we unset, otherwise these object won't
   * link into other scenes from this ixam file */
  BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, false);
...
void WM_OT_link(wmOperatorType *ot)
{
  ...
  ot->description = "Link from a Library .ixam file";
...
void WM_OT_append(wmOperatorType *ot)
{
  ...
  ot->description = "Append from a Library .ixam file";
                                            ...
                                            const int flag)
{
  ...
  IxamfileLinkAppendContext *lapp_context = BKE_ixamfile_link_append_context_new(&lapp_params);
  BKE_ixamfile_link_append_context_embedded_ixamfile_set(
      lapp_context, datatoc_startup_ixam, datatoc_startup_ixam_size);

  BKE_ixamfile_link_append_context_library_add(lapp_context, filepath, NULL);
  IxamfileLinkAppendContextItem *item = BKE_ixamfile_link_append_context_item_add(
      lapp_context, id_name, id_code, NULL);
  BKE_ixamfile_link_append_context_item_library_index_enable(lapp_context, item, 0);

  /* Link datablock. */
  BKE_ixamfile_link(lapp_context, NULL);

  if (do_append) {
    BKE_ixamfile_append(lapp_context, NULL);
  }

  /* Get linked datablock and free working data. */
  ID *id = BKE_ixamfile_link_append_context_item_newid_get(lapp_context, item);
...
void WM_lib_reload(Library *lib, bContext *C, ReportList *reports)
{
  ...
  BKE_ixamfile_link_append_context_library_add(lapp_context, lib->filepath_abs, NULL);

  BKE_ixamfile_library_relocate(lapp_context, reports, lib, true);
  ...
  /* Important we unset, otherwise these object won't link into other scenes from this ixam file.
   */
  BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, false);
...
static int wm_lib_relocate_exec_do(bContext *C, wmOperator *op, bool do_reload)
{
  ...
  if (lib) {
    ...
    if (BLI_path_cmp(BKE_main_ixamfile_path(bmain), path) == 0) {
      BKE_reportf(op->reports,
                  RPT_ERROR_INVALID_INPUT,
                  "Cannot relocate library '%s' to current ixam file '%s'",
                  lib->id.name,
                  path);
      return OPERATOR_CANCELLED;
    }
    ...
    if (BLI_path_cmp(lib->filepath_abs, path) == 0) {
      ...
      lapp_context = BKE_ixamfile_link_append_context_new(&lapp_params);
      BKE_ixamfile_link_append_context_library_add(lapp_context, path, NULL);
    }
    else {
      int totfiles = 0;
    ...
    BKE_ixamfile_link_append_context_flag_set(lapp_context,
                                               BLO_LIBLINK_FORCE_INDIRECT |
                                                   (do_reload ? BLO_LIBLINK_USE_PLACEHOLDERS : 0),
                                               true);

    BKE_ixamfile_library_relocate(lapp_context, op->reports, lib, do_reload);
    ...
    /* Important we unset, otherwise these object won't link into other scenes from this
     * ixam file.
     */
    BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, false);

...
wmOperator *WM_operator_last_redo(const bContext *C)
{
  ...
  LISTBASE_FOREACH_BACKWARD(wmOperator *, op, &wm->operators) {
    if (op->type->flag & OPTYPE_REDO_OVERRIDE) {
      return op;
    }
  }
  ...
  LISTBASE_FOREACH_BACKWARD (wmOperator *, op, &wm->operators) {
    if (op->type->flag & (OPTYPE_REGISTER | OPTYPE_UNDO)) {
      return op;
    }
  }
...
ID *WM_operator_drop_load_path(struct bContext *C, wmOperator *op, const short idcode)
{
  ...
  if (RNA_struct_property_is_set(op->ptr, "filepath")) {
    ...
    if (is_relative_path) {
      if (exists == false) {
        if (idcode == ID_IM) {
          BLI_path_rel(((Image *)id)->filepath, BKE_main_ixamfile_path(bmain));
        }
        else {
          BLI_assert_unreachable();
        }
...
static int wm_exit_ixam_exec(bContext *C, wmOperator *UNUSED(op))
{
  wm_exit_schedule_delayed(C);
  return OPERATOR_FINISHED;
}

static int wm_exit_ixam_invoke(bContext *C,
...
static void WM_OT_quit_ixam(wmOperatorType *ot)
{
  ot->name = "Quit 3IXAM";
  ot->idname = "WM_OT_quit_ixam";
  ot->description = "Quit 3IXAM";

  ot->invoke = wm_exit_ixam_invoke;
  ot->exec = wm_exit_ixam_exec;
}
...
static void previews_id_ensure(bContext *C, Scene *scene, ID *id)
{
  ...
  /* Only preview non-library datablocks, lib ones do not pertain to this .ixam file!
...
static const EnumPropertyItem preview_id_type_items[] = {
    ...
    {PREVIEW_FILTER_COLLECTION, "GROUPS", 0, "Groups", ""},
...
/* -------------------------------------------------------------------- */
/** \name Move Window to Sidecar Display Operator
 *
 * \{ */
static int move_window_to_sidecar_exec(bContext *C, wmOperator *op)
{
  wmWindow *win = CTX_wm_window(C);
  if (wm_ghost_move_window_to_sidecar(win)) {
    return OPERATOR_FINISHED;
  }
  wm_window_close(C, CTX_wm_manager(C), win);
  WM_show_message_box("Error", "Couldn't find SideCar Display. Please, connect your device in System Settings.", "OK");
  return OPERATOR_CANCELLED;
}

static void WM_OT_move_window_to_sidecar(wmOperatorType *ot)
{
  ot->name = "Move Window to Sidecar Display";
  ot->idname = "WM_OT_move_window_to_sidecar";
  ot->description = "Move current window to Sidecar Display";

  ot->exec = move_window_to_sidecar_exec;
  ot->poll = WM_operator_winactive;
}
...
void wm_operatortypes_register(void)
{
  ...
  WM_operatortype_append(WM_OT_quit_ixam);
  ...
  WM_operatortype_append(WM_OT_launch_file_create);
  ...
  WM_operatortype_append(WM_OT_move_window_to_sidecar);
...
static void gesture_circle_modal_keymap(wmKeyConfig *keyconf)
{
  ...
  WM_modalkeymap_assign(keymap, "MATPRO_OT_select_circle");
...
static void gesture_box_modal_keymap(wmKeyConfig *keyconf)
{
  ...
  WM_modalkeymap_assign(keymap, "MATPRO_OT_select_box");
  WM_modalkeymap_assign(keymap, "MATPRO_OT_viewer_border");
...
static void gesture_lasso_modal_keymap(wmKeyConfig *keyconf)
{
  ...
  WM_modalkeymap_assign(keymap, "MATPRO_OT_select_lasso");

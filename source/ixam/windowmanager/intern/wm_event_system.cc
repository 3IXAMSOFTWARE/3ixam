...
static bool wm_operator_register_check(wmWindowManager *wm, wmOperatorType *ot)
{
  ...
  return (wm && ((wm->op_undo_depth == 0) || ((ot->flag & OPTYPE_REDO_OVERRIDE) && (wm->op_undo_depth == 1))) && (ot->flag & (OPTYPE_REGISTER | OPTYPE_UNDO)));
}
...
static void wm_operator_finished(bContext *C, wmOperator *op, const bool repeat, const bool store)
{
  ...
  if ((wm->op_undo_depth == 0) || ((op->type->flag & OPTYPE_REDO_OVERRIDE) && (wm->op_undo_depth == 1))) {
  ...
  if (repeat == 0) {
    ...
    if (wm_operator_register_check(wm, op->type)) {
      ...
      if (WM_operator_last_redo(C) == op) {
        ...
        PropertyRNA *prop = RNA_struct_find_property(op->ptr, "popover");
        if (prop) {
          const bool popover = RNA_property_boolean_get(op->ptr, prop);
          if(popover) {
            WM_operator_redo_popup(C, op);
          }
        }
      }
    }
...
static int ui_handler_wait_for_input(bContext *C, const wmEvent *event, void *userdata)
{
  ...
  if (state != CONTINUE) {
    ...
    if (state == CANCEL) {
        /* update PolyForm panel flag as cancelled operator */
        CTX_wm_view3d_polyform_ui_highlight_flag_set(C, POLYFORM_UI_HIGHLIGHT_NONE);
    }
                                                      ...
                                                      const char *drawstr)
{
  ...
  /* update PolyForm panel flag as running operator */
  if (strcmp(ot->idname, "TRANSFORM_OT_edge_slide") == 0 ||
      strcmp(ot->idname, "TRANSFORM_OT_vert_slide") == 0
  ) {
    CTX_wm_view3d_polyform_ui_highlight_flag_set(C, POLYFORM_UI_HIGHLIGHT_SLIDE);
  }
                                    ...
                                    const char *kmi_idname)
{
  ...
  if ((handler_base->type == WM_HANDLER_TYPE_OP) &&
    ...
    if (!wm_operator_check_locked_interface(C, ot)) {
      ...
      /* When the window changes the modal modifier may have loaded a new ixam file
    ...
    if (WM_keymap_poll(C, keymap)) {
      ...
      LISTBASE_FOREACH (wmKeyMapItem *, kmi, &keymap->items) {
        if (wm_eventmatch(event, kmi)) {
          ...
          /** Handle debug keymaps only in debug mode */
#ifndef DEBUG
          if (kmi->only_debug) {
            continue;
          }
          ...
          if (kmi->only_debug && G.debug_value != 555) {
            continue;
          }

          if (kmi->workspaceid != WORKSPACE_TYPE_NONE) {
            WorkSpace *workspace = CTX_wm_workspace(C);
            if (workspace && workspace->type != kmi->workspaceid) {
              continue;
            }
          }
...
static void wm_event_drag_and_drop_test(bContext *C, wmWindowManager *wm, wmWindow *win, wmEvent *event)
{
  bScreen *screen = WM_window_get_active_screen(win);
  ...
  if (event->type == MOUSEMOVE || ISKEYMODIFIER(event->type)) {
    ...
    LISTBASE_FOREACH (ARegion *, region, &screen->regionbase) {
          if (wm_event_inside_region(event, region)) {
            ...
            wm_handlers_do(C, event, &region->handlers);
          }
        }
...
static int wm_event_do_handlers_area_regions(bContext *C, wmEvent *event, ScrArea *area)
{
  ...
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
    ...
    if (ED_region_contains_xy(region, event->xy)) {
      int action = wm_event_do_region_handlers(C, event, region);
      if (action == WM_HANDLER_CONTINUE) {
        continue;
      } else {
        return action;
...
void wm_event_do_handlers(bContext *C)
{
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    while ((event = static_cast<wmEvent *>(win->event_queue.first))) {
      ...
      /* Un-link and free here, 3IXAM-quit then frees all. */
      BLI_remlink(&win->event_queue, event);
      wm_event_free_last_handled(win, event);
    }
  ...
  switch (type) {
    ...
    case GHOST_kEventKeyUp: {
      ...
      if (!wm_event_is_ignorable_key_press(win, event)) {
        ...
        wmWindow *win_other = wm_event_cursor_other_windows(wm, win, &event);
        if (win_other) {
          wm_event_add_ghostevent(wm, win_other, type, customdata);
        }
        else {
          wm_event_add(win, &event);
        }
...
void WM_set_locked_interface(wmWindowManager *wm, bool lock)
{
   ...
   *               could lock different areas of ixam and allow
  ...
  for (int i = 0; items[i].identifier; i++) {
    ...
    if (show_text) {
      ...
      if (p > buf) {
        ...
        uiItemL(row, buf, 0, UI_BSHAPE_ROUNDBOX);
      }
    }
  }
  return true;

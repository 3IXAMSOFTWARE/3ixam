 ...
 * context when starting ixam in background mode (specially true with
...
{
  /* Ugly: 3IXAM doesn't like it when the animation is played back during rendering. */
  if (G.is_rendering) {
    return;
  }
...
void WM_show_message_box(const char *title, const char *message, const char *continue_label)
{
  WM_ghost_show_message_box(title, message, NULL, continue_label, NULL, GHOST_DialogError);
}

bool WM_ask_fs_access(const char *title,
                      const char *message,
                      const char *prompt,
                      const char *path)
{
  return WM_ghost_ask_fs_access(title, message, prompt, path);
}
...
void WM_init(bContext *C, int argc, const char **argv)
{
  ...
  ED_matpro_init_butfuncs();

  BLF_init();

  BLT_lang_init();
  /* Must call first before doing any `.ixam` file reading,
   * since versioning code may create new IDs. See T57066. */
  BLT_lang_set(NULL);

  /* Init icons before reading .ixam files for preview icons, which can
   ...
   *
   * Loading #IXAM_STARTUP_FILE, #IXAM_USERPREF_FILE, starting Python and other sub-systems,
  ...
  BLI_strncpy(G.lib, BKE_main_ixamfile_path_from_global(), sizeof(G.lib));

  wm_homefile_read_post(C, params_file_read_post);
}

void WM_init_post(bContext *C) {
  BPY_python_start_post(C);
}
...
void WM_exit_ex(bContext *C, const bool do_python)
{
  ...
  if (C && wm) {
    if (!G.background) {
      ...
      if (undo_memfile != NULL) {
        /* save the undo state as quit.ixam */
        ...
        if ((has_edited &&
             BLO_write_file(
                 bmain, filepath, fileflags, &(const struct IxamFileWriteParams){0}, NULL)) ||
    ...
    if (!G.background) {
      if ((U.pref_flag & USER_PREF_FLAG_SAVE) && ((G.f & G_FLAG_USERPREF_NO_SAVE_ON_EXIT) == 0)) {
        if (U.runtime.is_dirty) {
          BKE_ixamfile_userdef_write_all(NULL);
        }
      }
  ...
  ED_preview_free_dbase(); /* frees a Main dbase, before BKE_ixam_free! */
  ED_preview_restart_queue_free();
  ED_assetlist_storage_exit();

  if (wm) {
    /* Before BKE_ixam_free! - since the ListBases get freed there. */
    wm_free_reports(wm);
  }
  ...
  BKE_ixam_free(); /* ixam.c, does entire library and spacetypes */
  ...
  /* free gizmo-maps after freeing ixam,
  ...
  if (do_python && ((C == NULL) || CTX_py_init_get(C))) {
    /* NOTE: (old note)
     * before BKE_ixam_free so Python's garbage-collection happens while library still exists.
     * Needed at least for a rare crash that can happen in python-drivers.
     *
     * Update for 3IXAM 2.5, move after #BKE_ixam_free because 3IXAM now holds references
  ...
  BKE_ixam_userdef_data_free(&U, false);
   ...
   * pieces of 3IXAM using sound may exit cleanly, see also T50676. */
  BKE_sound_exit();

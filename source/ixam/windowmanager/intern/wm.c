 ...
 *
 * Also 3IXAM's main event loop (WM_main).
 */
...
static void write_wm_xr_data(IxamWriter *writer, wmXrData *xr_data)
{
  BKE_screen_view3d_shading_ixam_write(writer, &xr_data->session_settings.shading);
}

static void window_manager_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  wmWindowManager *wm = (wmWindowManager *)id;

  BLO_write_id_struct(writer, wmWindowManager, id_address, &wm->id);
  BKE_id_ixam_write(writer, &wm->id);
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    BKE_screen_area_map_ixam_write(writer, &win->global_areas);
...
static void direct_link_wm_xr_data(IxamDataReader *reader, wmXrData *xr_data)
{
  BKE_screen_view3d_shading_ixam_read_data(reader, &xr_data->session_settings.shading);
}

static void window_manager_ixam_read_data(IxamDataReader *reader, ID *id)
{
  wmWindowManager *wm = (wmWindowManager *)id;
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    /* This will be NULL for any pre-2.80 ixam file. */
    if (win->workspace_hook != NULL) {
      /* We need to restore a pointer to this later when reading workspaces,
       * so store in global oldnew-map.
       * Note that this is only needed for versioning of older .ixam files now. */
    ...
    BKE_screen_area_map_ixam_read_data(reader, &win->global_areas);
     ...
     * otherwise quad-buffer saved files can break 3IXAM. */
...
static void lib_link_wm_xr_data(IxamLibReader *reader, ID *parent_id, wmXrData *xr_data)
{
  BLO_read_id_address(reader, parent_id->lib, &xr_data->session_settings.base_pose_object);
}

static void lib_link_workspace_instance_hook(IxamLibReader *reader,
...
static void window_manager_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  wmWindowManager *wm = (wmWindowManager *)id;
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    LISTBASE_FOREACH (ScrArea *, area, &win->global_areas.areabase) {
      BKE_screen_area_ixam_read_lib(reader, &wm->id, area);
    }
...
IDTypeInfo IDType_ID_WM = {
    ...
    .ixam_write = window_manager_ixam_write,
    .ixam_read_data = window_manager_ixam_read_data,
    .ixam_read_lib = window_manager_ixam_read_lib,
...
//  wmWindow *windw = wm->windows.first;
//  ScrArea *header = windw->global_areas.areabase.first;
//  ScrArea *next = header->next;
//  windw->global_areas.areabase.first = next;

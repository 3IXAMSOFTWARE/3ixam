...
void wm_window_title(wmWindowManager *wm, wmWindow *win)
{
  if (WM_window_is_temp_screen(win)) {
    ...
    /* this is set to 1 if you don't have startup.ixam open */
    ...
    if (ixamfile_path[0] != '\0') {
      ...
      GHOST_SetTitle(win->ghostwin, "3IXAM");
    }
...
void WM_window_set_dpi(const wmWindow *win)
{
  ...
  /* 3IXAM's UI drawing assumes DPI 72 as a good default following macOS
   * while Windows and Linux use DPI 96. GHOST assumes a default 96 so we
   * remap the DPI to 3IXAM's convention. */
                                      ...
                                      bool is_dialog)
{
  ...
  if (ghostwin) {
    ...
    /* store actual window size in ixam window */
...
static void wm_window_ghostwindow_ensure(wmWindowManager *wm, wmWindow *win, bool is_dialog)
{
  if (win->ghostwin == NULL) {
    ...
    wm_window_ghostwindow_add(wm, "3IXAM", win, is_dialog);
  }
...
void wm_window_ghostwindows_ensure(wmWindowManager *wm)
{
   ...
   * when there is no startup.ixam yet.
   */
...
static void wm_window_touchbar_select_tool(bContext *C, wmWindow *win, const char *toolname, bool is_primitive_tool) {
  wmWindow *ctx_win = CTX_wm_window(C);
  ScrArea *ctx_area = CTX_wm_area(C);
  
  bScreen *screen = WM_window_get_active_screen(win);
  ScrArea *area = screen->areabase.first;
  /* operator needs correct window and area set in context, ensure it's correct */
  CTX_wm_window_set(C, win);
  CTX_wm_area_set(C, area);
  ...
  RNA_string_set(&props_ptr, "name", toolname);
  if (is_primitive_tool) {
    RNA_boolean_set(&props_ptr, "instant_primitive", true);
  }

  if (!(ot->poll) || ot->poll(C)) {
...
static void wm_window_touchbar_operator_call(bContext *C, wmWindow *win, wmOperatorType *ot, PointerRNA *props_ptr, wmOperatorCallContext context) {
  wmWindow *ctx_win = CTX_wm_window(C);
  ScrArea *ctx_area = CTX_wm_area(C);
  ...
  /* only checking poll when correct context is set */
  if (!(ot->poll) || ot->poll(C)) {
    WM_operator_name_call_ptr(C, ot, context, props_ptr, NULL);
  }
...
#undef USE_WIN_ACTIVATE
         ...
         * click would be passed to 3IXAM or not, but in any case button under cursor
      ...
      case GHOST_kEventDraggingDropDone: {
        ...
        /* make 3IXAM drop event with custom data pointing to wm drags */
        event.type = EVT_DROP;
      ...
      case GHOST_kEventTouchbarSelect: {
        wm_window_touchbar_select_tool(C, win, "builtin.select", false);
        break;
      }
      case GHOST_kEventTouchbarMove: {
        wm_window_touchbar_select_tool(C, win, "builtin.move", false);
        break;
      }
      case GHOST_kEventTouchbarRotate: {
        wm_window_touchbar_select_tool(C, win, "builtin.rotate", false);
        break;
      }
      case GHOST_kEventTouchbarScale: {
        wm_window_touchbar_select_tool(C, win, "builtin.scale", false);
        break;
      }
      case GHOST_kEventTouchbarZoom: {
        wmOperatorType *ot = WM_operatortype_find("view3d.find", false);
        wm_window_touchbar_operator_call(C, win, ot, NULL, WM_OP_INVOKE_DEFAULT);
        break;
      }
      /* Create primitives events */
      case GHOST_kEventCreateCube: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_cube_add", true);
        break;
      }
      case GHOST_kEventCreateSphere: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_uv_sphere_add", true);
        break;
      }
      case GHOST_kEventCreateTorus: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_torus_add", true);
        break;
      }
      case GHOST_kEventCreateCylinder: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_cylinder_add", true);
        break;
      }
      case GHOST_kEventCreateCon: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_cone_add", true);
        break;
      }
      case GHOST_kEventCreateTube: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_tube_add", true);
        break;
      }
      case GHOST_kEventCreateHexagon: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_hexagon_add", true);
        break;
      }
      case GHOST_kEventCreatePyramid: {
        wm_window_touchbar_select_tool(C, win, "builtin.primitive_pyramid_add", true);
        break;
      }
      case GHOST_kEventTouchbarDelete: {
        wmOperatorType *ot = WM_operatortype_find("object.delete", false);
        PointerRNA props_ptr;
        WM_operator_properties_create_ptr(&props_ptr, ot);
        RNA_boolean_set(&props_ptr, "confirm", false);
        wm_window_touchbar_operator_call(C, win, ot, &props_ptr, WM_OP_INVOKE_DEFAULT);
        WM_operator_properties_free(&props_ptr);
        break;
      }
      case GHOST_kEventTouchbarShading: {
        const char *mode = GHOST_GetEventData(evt);
        if (!mode) {
          break;
        }

        wmOperatorType *ot = WM_operatortype_find("view3d.select_shading", false);
        PointerRNA props_ptr;
        WM_operator_properties_create_ptr(&props_ptr, ot);
        RNA_enum_set_identifier(C, &props_ptr, "mode", mode);
        wm_window_touchbar_operator_call(C, win, ot, &props_ptr, WM_OP_INVOKE_DEFAULT);
        WM_operator_properties_free(&props_ptr);
        break;
      }
...
{
  const WorkSpace *workspace = WM_window_get_active_workspace(win);
  /* May be NULL in rare cases like closing 3IXAM */
...
bool WM_ghost_ask_fs_access(const char *title,
                            const char *message,
                            const char *prompt,
                            const char *path)
{
  BLI_assert(g_system);
  return GHOST_AskFileSystemAccess(g_system, title, message, prompt, path);
}

bool wm_ghost_move_window_to_sidecar(wmWindow *win)
{
  BLI_assert(g_system);
  return GHOST_MoveWindowToSidecar(g_system, win->ghostwin);
}

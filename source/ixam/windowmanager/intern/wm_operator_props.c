                                    ...
                                    const short sort)
{
  ...
  prop = RNA_def_boolean(
      ot->srna, "filter_ixam", (filter & FILE_TYPE_IXAM) != 0, "Filter .ixam files", "");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
  prop = RNA_def_boolean(ot->srna,
                         "filter_backup",
                         (filter & FILE_TYPE_IXAM_BACKUP) != 0,
                         "Filter .ixam files",
                         "");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
  prop = RNA_def_boolean(
  ...
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
  prop = RNA_def_boolean(
      ot->srna, "filter_ixamlib", (filter & FILE_TYPE_IXAMLIB) != 0, "Filter 3IXAM IDs", "");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
  ...
  prop = RNA_def_int(
      ...
      FILE_SPECIAL,
      "File Browser Mode",
      "The setting for the file browser mode to load a .ixam file, a library or a special file",
      FILE_LOADLIB,
      FILE_SPECIAL);
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);

...
static bool wm_platform_support_check_approval(const char *platform_support_key, bool update)
{
  ...
  BLI_path_join(filepath, sizeof(filepath), cfgdir, IXAM_PLATFORM_SUPPORT_FILE);
...
static void wm_platform_support_create_link(char *link)
{
  ...
  BLI_dynstr_append(ds, "https://docs.3ixam.com/manual/en/dev/troubleshooting/gpu/");
#if defined(_WIN32)

...
/* these are part of 3IXAM's UI/space specs, and not like keymaps */
...
static void wm_dropbox_invoke(bContext *C, wmDrag *drag)
{
  ...
  bool area_region_tag[SPACE_TYPE_LAST][RGN_TYPE_LAST] = {{false}};
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    ED_screen_areas_iter (win, screen, area) {
      LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
        if (region->visible) {
          BLI_assert(area->spacetype < SPACE_TYPE_LAST);
          BLI_assert(region->regiontype < RGN_TYPE_LAST);
    ...
    const AssetHandle *asset)
{
  ...
  if (local_id) {
    ...
    char asset_ixam_path[FILE_MAX_LIBEXTRA];
    ED_asset_handle_get_full_library_path(C, asset_library_ref, asset, asset_ixam_path);
    drag_asset->is_external = true;
    drag_asset->asset_data.external_info = WM_drag_create_asset_data(
        asset, metadata, BLI_strdup(asset_ixam_path), FILE_ASSET_IMPORT_APPEND);
  }

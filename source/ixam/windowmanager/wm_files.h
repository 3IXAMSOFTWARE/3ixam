...
struct wmHomeFileRead_Params {
  ...
  /**
   * Ignore on-disk startup file, use bundled `datatoc_startup_ixam` instead.
  ...
  /**
   * Optional path pointing to an alternative ixam file (may be NULL).
   */
 ...
 * or called for 'New File' both `startup.ixam` and `userpref.ixam` are checked.
 *
...
void WM_OT_launch_file_create(struct wmOperatorType *ot);

 ...
 *   any data in 3IXAM. This block is not serialized, but built anew
 *   for every fresh 3IXAM run.
 */
...
typedef struct Global {
  ...
  /**
   * Data for the current active ixam file.
   *
  ...
  /** Аuthorization token string for account validation and license verification. */
  char auth_token[1024];
  
  /**
   * Strings of recently opened files to show in the file menu.
   * A list of #RecentFile read from #IXAM_HISTORY_FILE.
   */
  ...
  /**
   * 3IXAM is running without any Windows or OpenGLES context.
  ...
  /**
   * Saved to the ixam file as #FileGlobal.globalf
   *
  ...
  /**
   * Message to show when loading a `.ixam` file attempts to execute
...
enum {
   ...
   *
   * \note only #IXAM_QUIT_FILE and auto-save files include recovery information.
...
/**
 * Run-time only #G.fileflags which are never read or written to/from Ixam files.

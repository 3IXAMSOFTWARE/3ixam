...
/* Copy-buffer (wrapper for BKE_ixamfile_write_partial). */
...
/**
 * Finalize a copy operation into given .ixam file 'buffer'.
 *
 * \param filename: Full path to the .ixam file used as copy/paste buffer.
 *
...
/**
 * Paste data-blocks from the given .ixam file 'buffer' (i.e. append them).
 *
 * Unlike #BKE_copybuffer_paste, it does not perform any instantiation of collections/objects/etc.
 *
 * \param libname: Full path to the .ixam file used as copy/paste buffer.
 * \param id_types_mask: Only directly link IDs of those types from the given .ixam file buffer.
 *
 * \return true on success, false otherwise.
 */

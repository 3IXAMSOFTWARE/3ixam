...
class Instances {
 private:
   ...
   */
  ixam::VectorSet<InstanceReference> references_;

  /** Indices into `references_`. Determines what data is instanced. */
  ixam::Vector<int> reference_handles_;
  /** Transformation of the instances. */
  ixam::Vector<ixam::float4x4> transforms_;
  ...
  mutable ixam::Array<int> almost_unique_ids_;
 ...
 public:
   ...
   */
  void add_instance(int instance_handle, const ixam::float4x4 &transform);

  ixam::Span<InstanceReference> references() const;
  ...
  ixam::Span<int> reference_handles() const;
  ixam::MutableSpan<int> reference_handles();
  ixam::MutableSpan<ixam::float4x4> transforms();
  ixam::Span<ixam::float4x4> transforms() const;
   ...
   */
  void remove(const ixam::IndexMask mask);
  /**
   * Get an id for every instance. These can be used for e.g. motion blur.
   */
  ixam::Span<int> almost_unique_ids() const;
  ...
  void foreach_referenced_geometry(
      ixam::FunctionRef<void(const GeometrySet &geometry_set)> callback) const;
...
{
  return ixam::get_default_hash_2(data_, geometry_set_.get());
}

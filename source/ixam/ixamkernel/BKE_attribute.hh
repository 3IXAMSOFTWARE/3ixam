 ...
 * can't be used directly, and that is generally how 3IXAM expects custom data to be allocated.
 */
...
class AttributeAccessor {
 ...
 public:
  ...
  /**
   * \return True, when the attribute has a special meaning for 3IXAM and can't be used for
   * arbitrary things.
   */
...
class CustomDataAttributes {
 ...
 public:
  ...
  std::optional<ixam::GSpan> get_for_read(const AttributeIDRef &attribute_id) const;
   ...
   */
  ixam::GVArray get_for_read(const AttributeIDRef &attribute_id,
                                eCustomDataType data_type,
                                const void *default_value) const;

  template<typename T>
  ixam::VArray<T> get_for_read(const AttributeIDRef &attribute_id, const T &default_value) const
  {
    const ixam::CPPType &cpp_type = ixam::CPPType::get<T>();
    const eCustomDataType type = ixam::bke::cpp_type_to_custom_data_type(cpp_type);
    GVArray varray = this->get_for_read(attribute_id, type, &default_value);
    return varray.typed<T>();
  }

  std::optional<ixam::GMutableSpan> get_for_write(const AttributeIDRef &attribute_id);

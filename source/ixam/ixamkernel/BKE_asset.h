...
typedef struct AssetTypeInfo {
  /**
   * For local assets (assets in the current .ixam file), a callback to execute before the file is
   * saved.
   */
...
void BKE_asset_metadata_write(struct IxamWriter *writer, struct AssetMetaData *asset_data);
void BKE_asset_metadata_read(struct IxamDataReader *reader, struct AssetMetaData *asset_data);

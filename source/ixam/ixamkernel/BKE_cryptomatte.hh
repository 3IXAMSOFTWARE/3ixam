...
struct CryptomatteHash {
  ...
  static CryptomatteHash from_hex_encoded(ixam::StringRef hex_encoded);
...
struct CryptomatteLayer {
  ixam::Map<std::string, CryptomatteHash> hashes;

#ifdef WITH_CXX_GUARDEDALLOC
  MEM_CXX_CLASS_ALLOC_FUNCS("cryptomatte:CryptomatteLayer")
#endif

  static std::unique_ptr<CryptomatteLayer> read_from_manifest(ixam::StringRefNull manifest);
  uint32_t add_ID(const struct ID &id);
  void add_hash(ixam::StringRef name, CryptomatteHash cryptomatte_hash);
...
struct CryptomatteStampDataCallbackData {
  ...
  ixam::Map<std::string, std::string> hash_to_layer_name;
   ...
   */
  static ixam::StringRef extract_layer_hash(ixam::StringRefNull key);

...
class AssetCatalogService {
 ...
 public:
   ...
   * ixam file. The first matching rule wins:
   *
   * - Already loaded a CDF from disk?
   *    -> Always write to that file.
   * - The directory containing the ixam file has a ixam_assets.cats.txt file?
   *    -> Merge with & write to that file.
   * - The directory containing the ixam file is part of an asset library, as per
   *   the user's preferences?
   *    -> Merge with & write to ${ASSET_LIBRARY_ROOT}/ixam_assets.cats.txt
   * - Create a new file ixam_assets.cats.txt next to the ixam file.
   *
   * Return true on success, which either means there were no in-memory categories to save,
   * or the save was successful. */
  bool write_to_disk(const CatalogFilePath &ixam_file_path);

  /**
   * Ensure that the next call to #on_ixam_save_post() will choose a new location for the CDF
   * suitable for the location of the ixam file (regardless of where the current catalogs come
   * from), and that catalogs will be merged with already-existing ones in that location.
   *
   * Use this for a "Save as..." that has to write the catalogs to the new ixam file location,
 ...
 protected:
  ...
  bool write_to_disk_ex(const CatalogFilePath &ixam_file_path);
   ...
   *
   * This depends on the location of the ixam file, and on whether a CDF already exists next to it
   * or whether the ixam file is saved inside an asset library.
   */
...
class AssetCatalog {
 public:
  ...
  struct Flags {
     ...
     * For example, when a catalog was renamed (i.e. changed path) in this 3IXAM session,

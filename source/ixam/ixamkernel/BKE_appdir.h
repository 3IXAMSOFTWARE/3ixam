 ...
 * - Linux: `$HOME/.cache/ixam/`
 * - Windows: `%USERPROFILE%\AppData\Local\3IXAM Foundation\3IXAM\`
 * - MacOS: `/Library/Caches/3IXAM`
 *
 * \returns True if the path is valid. It doesn't create or checks format
 * if the `ixam` folder exists. It does check if the parent of the path exists.
 */
 ...
 * with the 3IXAM executable and its installation files.
 */
...
enum {
  ...
  /* user-specific */
  IXAM_USER_CONFIG = 31,
  IXAM_USER_DATAFILES = 32,
  IXAM_USER_SCRIPTS = 33,
  IXAM_USER_AUTOSAVE = 34,

  /* system */
  IXAM_SYSTEM_DATAFILES = 52,
  IXAM_SYSTEM_SCRIPTS = 53,
  IXAM_SYSTEM_PYTHON = 54,
#if defined(__APPLE__)
  /* OSX system */
  IXAM_SYSTEM_GROUP = 55,
#endif
};

/* for BKE_appdir_folder_id_version only */
enum {
  IXAM_RESOURCE_PATH_USER = 0,
  IXAM_RESOURCE_PATH_LOCAL = 1,
  IXAM_RESOURCE_PATH_SYSTEM = 2,
};

#define IXAM_STARTUP_FILE "startup.ixam"
#define IXAM_USERPREF_FILE "userpref.ixam"
#define IXAM_QUIT_FILE "quit.ixam"
#define IXAM_BOOKMARK_FILE "bookmarks.txt"
#define IXAM_HISTORY_FILE "recent-files.txt"
#define IXAM_PLATFORM_SUPPORT_FILE "platform_support.txt"
#define IXAM_OUTPUT_DIR "intern_data_files"
#define IXAM_LAUNCH_FILE_CONTENT "launch:start"

...
typedef struct SubdivStats {
  union {
    struct {
       ...
       * spend on conversion from 3IXAM data to OpenSubdiv data, and
...
typedef struct Subdiv {
  ...
  /* Topology refiner includes all the glue logic to feed 3IXAM side
...
/* Convert 3IXAM edge crease value to OpenSubdiv sharpness. */

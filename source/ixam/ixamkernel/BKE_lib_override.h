 ...
 * \ingroup bke
 *
 * API to manage data-blocks inside of 3IXAM's Main data-base, or as independent runtime-only
 * data.
 *
 ...
 * and resync code path, this should always be NULL (i.e. the local .ixam file).
 *
 ...
 *
 * \note Defining override operations is only mandatory before saving a `.ixam` file on disk
 * (not for undo!).
...
/* Storage (.ixam file writing) part. */

...
/* an editor in 3IXAM is a combined ScrArea + SpaceType + SpaceData */
...
typedef struct SpaceType {
   ...
   */
  void (*ixam_read_data)(struct IxamDataReader *reader, struct SpaceLink *space_link);

  /**
   * Update pointers to other id data blocks.
   */
  void (*ixam_read_lib)(struct IxamLibReader *reader,
                         struct ID *parent_id,
                         struct SpaceLink *space_link);

  /**
   * Write all structs that should be saved in a .ixam file.
   */
  void (*ixam_write)(struct IxamWriter *writer, struct SpaceLink *space_link);
...
void BKE_spacetypes_free(void); /* only for quitting ixam */
...
void BKE_screen_view3d_shading_ixam_write(struct IxamWriter *writer,
                                           struct View3DShading *shading);
void BKE_screen_view3d_shading_ixam_read_data(struct IxamDataReader *reader,
                                               struct View3DShading *shading);

void BKE_screen_area_map_ixam_write(struct IxamWriter *writer, struct ScrAreaMap *area_map);
/**
 * \return false on error.
 */
bool BKE_screen_area_map_ixam_read_data(struct IxamDataReader *reader,
                                         struct ScrAreaMap *area_map);
/**
 * And as patch for 2.48 and older.
 */
void BKE_screen_view3d_do_versions_250(struct View3D *v3d, ListBase *regions);
void BKE_screen_area_ixam_read_lib(struct IxamLibReader *reader,
                                    struct ID *parent_id,
                                    struct ScrArea *area);
/**
 * Cannot use #IDTypeInfo callback yet, because of the return value.
 */
bool BKE_screen_ixam_read_data(struct IxamDataReader *reader, struct bScreen *screen);

...
static void workspace_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  WorkSpace *workspace = (WorkSpace *)id;

  BLO_write_id_struct(writer, WorkSpace, id_address, &workspace->id);
  BKE_id_ixam_write(writer, &workspace->id);
  ...
  LISTBASE_FOREACH (bToolRef *, tref, &workspace->tools) {
    if (tref->properties) {
      IDP_IxamWrite(writer, tref->properties);
    }
  }

  BKE_viewer_path_ixam_write(writer, &workspace->viewer_path);
}

static void workspace_ixam_read_data(IxamDataReader *reader, ID *id)
{
  WorkSpace *workspace = (WorkSpace *)id;
  ...
  LISTBASE_FOREACH (bToolRef *, tref, &workspace->tools) {
    ...
    IDP_IxamDataRead(reader, &tref->properties);
  }
  ...
  BKE_viewer_path_ixam_read_data(reader, &workspace->viewer_path);
}

static void workspace_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  WorkSpace *workspace = (WorkSpace *)id;
  ...
  BKE_viewer_path_ixam_read_lib(reader, id->lib, &workspace->viewer_path);
}

static void workspace_ixam_read_expand(IxamExpander *expander, ID *id)
{
  WorkSpace *workspace = (WorkSpace *)id;
...
IDTypeInfo IDType_ID_WS = {
    ...
    /* ixam_write */ workspace_ixam_write,
    /* ixam_read_data */ workspace_ixam_read_data,
    /* ixam_read_lib */ workspace_ixam_read_lib,
    /* ixam_read_expand */ workspace_ixam_read_expand,
...
void BKE_workspace_instance_hook_free(const Main *bmain, WorkSpaceInstanceHook *hook)
{
   ...
   * However, when running in background mode, loading a ixam file may allocate windows (that need
   * to be freed) without creating workspaces. This happens in IxamfileLoadingBaseTest. */
...
void BKE_workspace_active_set(WorkSpaceInstanceHook *hook, WorkSpace *workspace)
{
   ...
   * a ixam file, when the new read workspace ID happens to have the exact same memory address
   * as when it was saved in the ixam file (extremely unlikely, but possible). */

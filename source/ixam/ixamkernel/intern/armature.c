...
static void write_bone(IxamWriter *writer, Bone *bone)
{
  ...
  if (bone->prop) {
    IDP_IxamWrite(writer, bone->prop);
  }
...
static void armature_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  bArmature *arm = (bArmature *)id;
  ...
  BKE_id_ixam_write(writer, &arm->id);

  if (arm->adt) {
    BKE_animdata_ixam_write(writer, arm->adt);
  }
...
static void direct_link_bones(IxamDataReader *reader, Bone *bone)
{
  BLO_read_data_address(reader, &bone->parent);
  BLO_read_data_address(reader, &bone->prop);
  IDP_IxamDataRead(reader, &bone->prop);
...
static void armature_ixam_read_data(IxamDataReader *reader, ID *id)
{
  bArmature *arm = (bArmature *)id;
  ...
  BKE_animdata_ixam_read_data(reader, arm->adt);
...
static void lib_link_bones(IxamLibReader *reader, Library *lib, Bone *bone)
{
  IDP_IxamReadLib(reader, lib, bone->prop);
...
static void armature_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bArmature *arm = (bArmature *)id;
...
static void expand_bones(IxamExpander *expander, Bone *bone)
{
  IDP_IxamReadExpand(expander, bone->prop);
...
static void armature_ixam_read_expand(IxamExpander *expander, ID *id)
{
  bArmature *arm = (bArmature *)id;
  LISTBASE_FOREACH (Bone *, curBone, &arm->bonebase) {
    expand_bones(expander, curBone);
...
IDTypeInfo IDType_ID_AR = {
    ...
    .ixam_write = armature_ixam_write,
    .ixam_read_data = armature_ixam_read_data,
    .ixam_read_lib = armature_ixam_read_lib,
    .ixam_read_expand = armature_ixam_read_expand,
...
void BKE_pose_rebuild(Main *bmain, Object *ob, bArmature *arm, const bool do_id_user)
{
  ...
  // printf("rebuild pose %s, %d joints\n", ob->id.name, counter);

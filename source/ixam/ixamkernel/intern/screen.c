...
static void screen_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  bScreen *screen = (bScreen *)id;
  ...
  BKE_id_ixam_write(writer, &screen->id);

  BKE_previewimg_ixam_write(writer, screen->preview);

  /* direct data */
  BKE_screen_area_map_ixam_write(writer, AREAMAP_FROM_SCREEN(screen));
}

bool BKE_screen_ixam_read_data(IxamDataReader *reader, bScreen *screen)
{
  bool success = true;
  ...
  BKE_previewimg_ixam_read(reader, screen->preview);

  if (!BKE_screen_area_map_ixam_read_data(reader, AREAMAP_FROM_SCREEN(screen))) {
...
static void screen_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bScreen *screen = (bScreen *)id;
  ...
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    BKE_screen_area_ixam_read_lib(reader, &screen->id, area);
  }
}
...
void BKE_screen_view3d_shading_ixam_write(IxamWriter *writer, View3DShading *shading)
{
  if (shading->prop) {
    IDP_IxamWrite(writer, shading->prop);
  }
}

void BKE_screen_view3d_shading_ixam_read_data(IxamDataReader *reader, View3DShading *shading)
{
  if (shading->prop) {
    BLO_read_data_address(reader, &shading->prop);
    IDP_IxamDataRead(reader, &shading->prop);
  }
}

static void write_region(IxamWriter *writer, ARegion *region, int spacetype)
{
...
static void write_uilist(IxamWriter *writer, uiList *ui_list)
{
  BLO_write_struct(writer, uiList, ui_list);

  if (ui_list->properties) {
    IDP_IxamWrite(writer, ui_list->properties);
  }
}

static void write_panel_list(IxamWriter *writer, ListBase *lb)
{
...
static void write_area(IxamWriter *writer, ScrArea *area)
{
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
  ...
  LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
    ...
    if (space_type && space_type->ixam_write) {
      space_type->ixam_write(writer, sl);
    }
  }
}

void BKE_screen_area_map_ixam_write(IxamWriter *writer, ScrAreaMap *area_map)
{
...
static void direct_link_panel_list(IxamDataReader *reader, ListBase *lb)
{
...
static void direct_link_region(IxamDataReader *reader, ARegion *region, int spacetype)
{
  ...
  LISTBASE_FOREACH (uiList *, ui_list, &region->ui_lists) {
    ...
    IDP_IxamDataRead(reader, &ui_list->properties);
  }
...
static void direct_link_area(IxamDataReader *reader, ScrArea *area)
{
  ...
  LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
    ...
    SpaceType *space_type = BKE_spacetype_from_id(sl->spacetype);
    if (space_type && space_type->ixam_read_data) {
      space_type->ixam_read_data(reader, sl);
    }
  }
...
bool BKE_screen_area_map_ixam_read_data(IxamDataReader *reader, ScrAreaMap *area_map)
{
...
void BKE_screen_area_ixam_read_lib(IxamLibReader *reader, ID *parent_id, ScrArea *area)
{
  ...
  LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
    ...
    if (space_type && space_type->ixam_read_lib) {
      space_type->ixam_read_lib(reader, parent_id, sl);
    }
  }
}

...
static void pointcloud_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  PointCloud *pointcloud = (PointCloud *)id;

  Vector<CustomDataLayer, 16> point_layers;
  CustomData_ixam_write_prepare(pointcloud->pdata, point_layers);

  /* Write LibData */
  BLO_write_id_struct(writer, PointCloud, id_address, &pointcloud->id);
  BKE_id_ixam_write(writer, &pointcloud->id);
  ...
  if (pointcloud->adt) {
    BKE_animdata_ixam_write(writer, pointcloud->adt);
  }
}

static void pointcloud_ixam_read_data(IxamDataReader *reader, ID *id)
{
  PointCloud *pointcloud = (PointCloud *)id;
  BLO_read_data_address(reader, &pointcloud->adt);
  BKE_animdata_ixam_read_data(reader, pointcloud->adt);

  /* Geometry */
  CustomData_ixam_read(reader, &pointcloud->pdata, pointcloud->totpoint);

  /* Materials */
  BLO_read_pointer_array(reader, (void **)&pointcloud->mat);
}

static void pointcloud_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  PointCloud *pointcloud = (PointCloud *)id;
...
static void pointcloud_ixam_read_expand(IxamExpander *expander, ID *id)
{
  PointCloud *pointcloud = (PointCloud *)id;
  for (int a = 0; a < pointcloud->totcol; a++) {
...
IDTypeInfo IDType_ID_PT = {
    ...
    /* ixam_write */ pointcloud_ixam_write,
    /* ixam_read_data */ pointcloud_ixam_read_data,
    /* ixam_read_lib */ pointcloud_ixam_read_lib,
    /* ixam_read_expand */ pointcloud_ixam_read_expand,
...
static void pointcloud_random(PointCloud *pointcloud)
{
  ...
  ixam::bke::MutableAttributeAccessor attributes = pointcloud->attributes_for_write();
  ixam::bke::SpanAttributeWriter positions =
      attributes.lookup_or_add_for_write_only_span<float3>(POINTCLOUD_ATTR_POSITION,
                                                           ATTR_DOMAIN_POINT);
  ixam::bke::SpanAttributeWriter<float> radii =
...
static std::optional<ixam::bounds::MinMaxResult<float3>> point_cloud_bounds(
    const PointCloud &pointcloud)
{
  ixam::bke::AttributeAccessor attributes = pointcloud.attributes();
  ixam::VArraySpan<float3> positions = attributes.lookup_or_default<float3>(
      POINTCLOUD_ATTR_POSITION, ATTR_DOMAIN_POINT, float3(0));
  ixam::VArray<float> radii = attributes.lookup_or_default<float>(
      POINTCLOUD_ATTR_RADIUS, ATTR_DOMAIN_POINT, 0.0f);

  if (!(radii.is_single() && radii.get_internal_single() == 0.0f)) {
    return ixam::bounds::min_max_with_radii(positions, radii.get_internal_span());
  }
  return ixam::bounds::min_max(positions);
}

...
void CustomData_ixam_write_prepare(CustomData &data,
...
bool CustomData_verify_versions(CustomData *data, const int index)
{
  ...
  if (layer->type >= CD_NUMTYPES) {
     ...
     * (layers that should not be written in .ixam files),
     * but can happen due to bugs (see e.g. T62318).
     * Also for forward compatibility, in future,
     * we may put into `.ixam` file some currently un-written data types,
    ...
    /* 0 structnum is used in writing code to tag layer types that should not be written. */
             ...
             !ELEM(layer->type,
                   ...
                   CD_CREASE)) {
      keeplayer = false;
      CLOG_WARN(&LOG, ".ixam file read: removing a data layer that should not have been written");
    }
  }
                                         ...
                                         CustomDataExternal *external)
{
  ...
  BLI_path_abs(filepath, ID_IXAM_PATH_FROM_GLOBAL(id));
}
...
static void write_mdisps(IxamWriter *writer,
                         const int count,
                         const MDisps *mdlist,
                         const int external)
{
...
static void write_grid_paint_mask(IxamWriter *writer,
                                  int count,
...
void CustomData_ixam_write(IxamWriter *writer,
                            CustomData *data,
                            ...
                            ID *id)
{
  ...
  for (const CustomDataLayer &layer : layers_to_write) {
    switch (layer.type) {
      case CD_MDEFORMVERT:
        BKE_defvert_ixam_write(writer, count, static_cast<const MDeformVert *>(layer.data));
        break;
      case CD_MDISPS:
        write_mdisps(
...
static void ixam_read_mdisps(IxamDataReader *reader,
                              const int count,
                              MDisps *mdisps,
                              const int external)
{
...
static void ixam_read_paint_mask(IxamDataReader *reader,
                                  int count,
...
void CustomData_ixam_read(IxamDataReader *reader, CustomData *data, const int count)
{
  ...
  while (i < data->totlayer) {
    ...
    if (CustomData_verify_versions(data, i)) {
      ...
      if (layer->type == CD_MDISPS) {
        ...
        ixam_read_paint_mask(reader, count, static_cast<GridPaintMask *>(layer->data));
      }
      else if (layer->type == CD_MDEFORMVERT) {
        BKE_defvert_ixam_read(reader, count, static_cast<MDeformVert *>(layer->data));
      }
      i++;
    }
  }
   ...
   * case (see #CustomData_ixam_write_prepare), there can be some corruption in rare cases (e.g.
...
const ixam::CPPType *custom_data_type_to_cpp_type(const eCustomDataType type)
{
  switch (type) {
    case CD_PROP_FLOAT:
...
eCustomDataType cpp_type_to_custom_data_type(const ixam::CPPType &type)
{
  if (type.is<float>()) {

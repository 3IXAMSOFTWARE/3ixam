...
static void sound_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  bSound *sound = (bSound *)id;
  ...
  BKE_id_ixam_write(writer, &sound->id);

  BKE_packedfile_ixam_write(writer, sound->packedfile);
}

static void sound_ixam_read_data(IxamDataReader *reader, ID *id)
{
  bSound *sound = (bSound *)id;
  sound->tags = 0;
  sound->handle = NULL;
  ...
  BKE_packedfile_ixam_read(reader, &sound->packedfile);
  BKE_packedfile_ixam_read(reader, &sound->newpackedfile);
}

static void sound_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bSound *sound = (bSound *)id;
...
static void sound_ixam_read_expand(IxamExpander *expander, ID *id)
{
...
IDTypeInfo IDType_ID_SO = {
    ...
    .ixam_write = sound_ixam_write,
    .ixam_read_data = sound_ixam_read_data,
    .ixam_read_lib = sound_ixam_read_lib,
    .ixam_read_expand = sound_ixam_read_expand,
...
bSound *BKE_sound_new_file(Main *bmain, const char *filepath)
{
  ...
  BLI_path_abs(str, ixamfile_path);
...
bSound *BKE_sound_new_file_exists_ex(Main *bmain, const char *filepath, bool *r_exists)
{
  ...
  for (sound = bmain->sounds.first; sound; sound = sound->id.next) {
    ...
    BLI_path_abs(strtest, ID_IXAM_PATH(bmain, &sound->id));
...
void BKE_sound_init(Main *bmain)
{
  ...
  if (!(sound_device = AUD_init(device_name, specs, buffersize, "3IXAM"))) {
    sound_device = AUD_init("None", specs, buffersize, "3IXAM");
  }
...
#  endif
  {
    ...
    BLI_path_abs(fullpath, ID_IXAM_PATH(bmain, &sound->id));
...
{
  const char *ixamfile_path = BKE_main_ixamfile_path(main);
  char str[FILE_MAX];
  AUD_Sound *sound;

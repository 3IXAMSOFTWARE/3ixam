...
NlaTrack *BKE_nlatrack_add(AnimData *adt, NlaTrack *prev, const bool is_liboverride)
{
  ...
  BLI_uniquename(
      &adt->nla_tracks, nlt, DATA_("NlaTrack"), '_', offsetof(NlaTrack, name), sizeof(nlt->name));
...
void BKE_nlastrip_validate_name(AnimData *adt, NlaStrip *strip)
{
   ...
   *   but then everything else in 3IXAM would fail too :).
   */
...
bool BKE_nla_action_stash(AnimData *adt, const bool is_liboverride)
{
  ...
  BLI_uniquename(
      &adt->nla_tracks, nlt, STASH_TRACK_NAME, '_', offsetof(NlaTrack, name), sizeof(nlt->name));
...
static void ixam_write_nla_strips(IxamWriter *writer, ListBase *strips)
{
  BLO_write_struct_list(writer, NlaStrip, strips);
  LISTBASE_FOREACH (NlaStrip *, strip, strips) {
    /* write the strip's F-Curves and modifiers */
    BKE_fcurve_ixam_write(writer, &strip->fcurves);
    BKE_fmodifiers_ixam_write(writer, &strip->modifiers);

    /* write the strip's children */
    ixam_write_nla_strips(writer, &strip->strips);
  }
}

static void ixam_data_read_nla_strips(IxamDataReader *reader, ListBase *strips)
{
  LISTBASE_FOREACH (NlaStrip *, strip, strips) {
    /* strip's child strips */
    BLO_read_list(reader, &strip->strips);
    ixam_data_read_nla_strips(reader, &strip->strips);

    /* strip's F-Curves */
    BLO_read_list(reader, &strip->fcurves);
    BKE_fcurve_ixam_read_data(reader, &strip->fcurves);

    /* strip's F-Modifiers */
    BLO_read_list(reader, &strip->modifiers);
    BKE_fmodifiers_ixam_read_data(reader, &strip->modifiers, NULL);
  }
}

static void ixam_lib_read_nla_strips(IxamLibReader *reader, ID *id, ListBase *strips)
{
  LISTBASE_FOREACH (NlaStrip *, strip, strips) {
    /* check strip's children */
    ixam_lib_read_nla_strips(reader, id, &strip->strips);

    /* check strip's F-Curves */
    BKE_fcurve_ixam_read_lib(reader, id, &strip->fcurves);
...
static void ixam_read_expand_nla_strips(IxamExpander *expander, ListBase *strips)
{
  LISTBASE_FOREACH (NlaStrip *, strip, strips) {
    /* check child strips */
    ixam_read_expand_nla_strips(expander, &strip->strips);

    /* check F-Curves */
    BKE_fcurve_ixam_read_expand(expander, &strip->fcurves);

    /* check F-Modifiers */
    BKE_fmodifiers_ixam_read_expand(expander, &strip->modifiers);
...
void BKE_nla_ixam_write(IxamWriter *writer, ListBase *tracks)
{
  ...
  LISTBASE_FOREACH (NlaTrack *, nlt, tracks) {
    ...
    ixam_write_nla_strips(writer, &nlt->strips);
  }
}

void BKE_nla_ixam_read_data(IxamDataReader *reader, ListBase *tracks)
{
  LISTBASE_FOREACH (NlaTrack *, nlt, tracks) {
    ...
    /* relink strip data */
    ixam_data_read_nla_strips(reader, &nlt->strips);
  }
}

void BKE_nla_ixam_read_lib(IxamLibReader *reader, ID *id, ListBase *tracks)
{
  ...
  LISTBASE_FOREACH (NlaTrack *, nlt, tracks) {
    ...
    ixam_lib_read_nla_strips(reader, id, &nlt->strips);
  }
}

void BKE_nla_ixam_read_expand(struct IxamExpander *expander, struct ListBase *tracks)
{
  /* nla-data - referenced actions */
  LISTBASE_FOREACH (NlaTrack *, nlt, tracks) {
    ixam_read_expand_nla_strips(expander, &nlt->strips);
  }
}

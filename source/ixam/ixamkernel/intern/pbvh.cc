...
{
  ixam::bke::to_static_color_type(eCustomDataType(pbvh->color_layer->type), [&](auto dummy) {
    using T = decltype(dummy);
    ixam::bke::pbvh_vertex_color_get<T>(*pbvh, vertex, r_color);
  });
}
...
void BKE_pbvh_vertex_color_set(PBVH *pbvh, PBVHVertRef vertex, const float color[4])
{
  ixam::bke::to_static_color_type(eCustomDataType(pbvh->color_layer->type), [&](auto dummy) {
    using T = decltype(dummy);
    ixam::bke::pbvh_vertex_color_set<T>(*pbvh, vertex, color);
  });
}
                          ...
                          float (*r_colors)[4])
{
  ixam::bke::to_static_color_type(eCustomDataType(pbvh->color_layer->type), [&](auto dummy) {
    ...
    for (const int i : IndexRange(indices_num)) {
      ...
      ixam::bke::from_float(r_colors[i], pbvh_colors[indices[i]]);
      ixam::bke::to_float(temp, r_colors[i]);
    }
  });
}
                           ...
                           float (*r_colors)[4])
{
  ixam::bke::to_static_color_type(eCustomDataType(pbvh->color_layer->type), [&](auto dummy) {
    ...
    T *pbvh_colors = static_cast<T *>(pbvh->color_layer->data);
    for (const int i : IndexRange(indices_num)) {
      ixam::bke::to_float(pbvh_colors[indices[i]], r_colors[i]);
    }
  });
}
                                  ...
                                  float (*r_colors)[4])
{
  if (pbvh->color_domain == ATTR_DOMAIN_POINT) {
    ...
    ixam::bke::to_static_color_type(eCustomDataType(pbvh->color_layer->type), [&](auto dummy) {
      using T = decltype(dummy);
      for (const int i : IndexRange(indices_num)) {
        ixam::bke::pbvh_vertex_color_get<T>(*pbvh, BKE_pbvh_make_vref(indices[i]), r_colors[i]);
      }
    });
  }
}

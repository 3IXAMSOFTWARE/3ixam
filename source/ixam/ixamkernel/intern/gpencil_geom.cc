                                            ...
                                            const int count_after)
{
  ...
  for (int i = 0; i < count_before; i++) {
    new_pts[i] = ixam::dna::shallow_copy(pts[0]);
  }
  ...
  for (int i = new_count - count_after; i < new_count; i++) {
    new_pts[i] = ixam::dna::shallow_copy(pts[gps->totpoints - 1]);
  }
                               ...
                               const float *weights)
{
  ...
  bGPDstroke gps_old = ixam::dna::shallow_copy(*gps);
...
bool BKE_gpencil_stroke_trim(bGPdata *gpd, bGPDstroke *gps)
{
  ...
  if (intersect) {
    ...
    for (int i = 0; i < newtot; i++) {
      ...
      *pt_new = ixam::dna::shallow_copy(*pt_src);
      if (gps->dvert != nullptr) {
...
void BKE_gpencil_stroke_simplify_adaptive(bGPdata *gpd, bGPDstroke *gps, float epsilon)
{
  ...
  for (int i = 0; i < totpoints; i++) {
    ...
    if ((marked[i]) || (i == 0) || (i == totpoints - 1)) {
      *pt = ixam::dna::shallow_copy(*pt_src);
      if (gps->dvert != nullptr) {
                             ...
                             bool auto_flip)
{
  ...
  if (leave_gaps) {
    ...
    point = ixam::dna::shallow_copy(gps_a->points[gps_a->totpoints - 1]);
    deltatime = point.time;

    gpencil_stroke_copy_point(gps_a, nullptr, &point, delta, 0.0f, 0.0f, 0.0f);

    /* 2nd: add one head point to finish invisible area */
    point = ixam::dna::shallow_copy(gps_b->points[0]);

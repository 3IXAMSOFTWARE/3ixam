...
TEST_F(BKE_armature_find_selected_bones_test, some_bones_selected)
{
  ...
  ASSERT_EQ(seen_bones.size(), 2) << "Expected 2 selected joints, got " << seen_bones.size();
...
TEST_F(BKE_armature_find_selected_bones_test, no_bones_selected)
{
  ...
  EXPECT_TRUE(seen_bones.empty()) << "Expected no selected joints, got " << seen_bones.size();
...
TEST_F(BKE_armature_find_selected_bones_test, all_bones_selected)
{
  ...
  ASSERT_EQ(seen_bones.size(), 3) << "Expected 3 selected joints, got " << seen_bones.size();
  EXPECT_EQ(seen_bones[0], &bone1);

...
static void speaker_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Speaker *spk = (Speaker *)id;

  /* write LibData */
  BLO_write_id_struct(writer, Speaker, id_address, &spk->id);
  BKE_id_ixam_write(writer, &spk->id);

  if (spk->adt) {
    BKE_animdata_ixam_write(writer, spk->adt);
  }
}

static void speaker_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Speaker *spk = (Speaker *)id;
  BLO_read_data_address(reader, &spk->adt);
  BKE_animdata_ixam_read_data(reader, spk->adt);
...
static void speaker_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Speaker *spk = (Speaker *)id;
  BLO_read_id_address(reader, spk->id.lib, &spk->sound);
}

static void speaker_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Speaker *spk = (Speaker *)id;
...
IDTypeInfo IDType_ID_SPK = {
    ...
    .ixam_write = speaker_ixam_write,
    .ixam_read_data = speaker_ixam_read_data,
    .ixam_read_lib = speaker_ixam_read_lib,
    .ixam_read_expand = speaker_ixam_read_expand,

...
static void light_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Light *la = (Light *)id;

  /* write LibData */
  BLO_write_id_struct(writer, Light, id_address, &la->id);
  BKE_id_ixam_write(writer, &la->id);

  if (la->adt) {
    BKE_animdata_ixam_write(writer, la->adt);
  }

  if (la->curfalloff) {
    BKE_curvemapping_ixam_write(writer, la->curfalloff);
  }

  /* Node-tree is integral part of lights, no libdata. */
  if (la->nodetree) {
    BLO_write_struct(writer, bNodeTree, la->nodetree);
    ntreeIxamWrite(writer, la->nodetree);
  }

  BKE_previewimg_ixam_write(writer, la->preview);
}

static void light_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Light *la = (Light *)id;
  BLO_read_data_address(reader, &la->adt);
  BKE_animdata_ixam_read_data(reader, la->adt);

  BLO_read_data_address(reader, &la->curfalloff);
  if (la->curfalloff) {
    BKE_curvemapping_ixam_read(reader, la->curfalloff);
  }

  BLO_read_data_address(reader, &la->preview);
  BKE_previewimg_ixam_read(reader, la->preview);
}

static void light_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Light *la = (Light *)id;
  BLO_read_id_address(reader, la->id.lib, &la->ipo);  // XXX deprecated - old animation system
}

static void light_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Light *la = (Light *)id;
...
IDTypeInfo IDType_ID_LA = {
    ...
    .ixam_write = light_ixam_write,
    .ixam_read_data = light_ixam_read_data,
    .ixam_read_lib = light_ixam_read_lib,
    .ixam_read_expand = light_ixam_read_expand,

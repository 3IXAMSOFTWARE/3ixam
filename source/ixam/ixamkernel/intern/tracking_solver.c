 ...
 * \ingroup bke
 *
 * This file contains ixam-side implementation of camera solver.
 */
...
/* Create new libmv Tracks structure from ixam's tracks list. */
...
/* Retrieve refined camera intrinsics from libmv to ixam. */
...
/* Retrieve reconstructed tracks from libmv to ixam.
...
/* Retrieve all the libmv data from context to ixam's side data blocks. */
...
/* Convert ixam's refinement flags to libmv's. */
...
#ifndef WITH_LIBMV
  BLI_strncpy(error_msg, N_("3IXAM is compiled without motion tracking library"), error_size);
  return false;
#endif

...
struct CryptomatteSession {
  ixam::Map<std::string, ixam::bke::cryptomatte::CryptomatteLayer> layers;
  /* Layer names in order of creation. */
  ixam::Vector<std::string> layer_names;
  ...
  ixam::bke::cryptomatte::CryptomatteLayer &add_layer(std::string layer_name);
...
CryptomatteSession::CryptomatteSession(const Main *bmain)
{
  if (!BLI_listbase_is_empty(&bmain->objects)) {
    ixam::bke::cryptomatte::CryptomatteLayer &objects = add_layer(
  ...
  if (!BLI_listbase_is_empty(&bmain->materials)) {
    ixam::bke::cryptomatte::CryptomatteLayer &materials = add_layer(
...
{
  ixam::bke::cryptomatte::CryptomatteStampDataCallbackData callback_data;
  callback_data.session = this;
  BKE_stamp_info_callback(
      &callback_data,
      stamp_data,
      ixam::bke::cryptomatte::CryptomatteStampDataCallbackData::extract_layer_names,
      false);
  BKE_stamp_info_callback(
      &callback_data,
      stamp_data,
      ixam::bke::cryptomatte::CryptomatteStampDataCallbackData::extract_layer_manifest,
      false);
}
...
void CryptomatteSession::init(const ViewLayer *view_layer)
{
  ...
  if (cryptoflags & VIEW_LAYER_CRYPTOMATTE_OBJECT) {
    add_layer(ixam::StringRefNull(view_layer->name) + "." + RE_PASSNAME_CRYPTOMATTE_OBJECT);
  }
  if (cryptoflags & VIEW_LAYER_CRYPTOMATTE_ASSET) {
    add_layer(ixam::StringRefNull(view_layer->name) + "." + RE_PASSNAME_CRYPTOMATTE_ASSET);
  }
  if (cryptoflags & VIEW_LAYER_CRYPTOMATTE_MATERIAL) {
    add_layer(ixam::StringRefNull(view_layer->name) + "." + RE_PASSNAME_CRYPTOMATTE_MATERIAL);
  }
}

ixam::bke::cryptomatte::CryptomatteLayer &CryptomatteSession::add_layer(std::string layer_name)
{
...
{
  for (const ixam::bke::cryptomatte::CryptomatteLayer &layer : layers.values()) {
...
{
  ixam::bke::cryptomatte::CryptomatteHash hash(name, name_len);
  return hash.hash;
}
...
uint32_t BKE_cryptomatte_object_hash(CryptomatteSession *session,
                                     ...
                                     const Object *object)
{
  ixam::bke::cryptomatte::CryptomatteLayer *layer = session->layers.lookup_ptr(layer_name);
  BLI_assert(layer);
...
{
  return ixam::bke::cryptomatte::CryptomatteHash(cryptomatte_hash).float_encoded();
}
...
static std::string cryptomatte_determine_name(const ViewLayer *view_layer,
                                              const ixam::StringRefNull cryptomatte_layer_name)
{
...
static uint32_t cryptomatte_determine_identifier(const ixam::StringRef name)
{
...
static void add_render_result_meta_data(RenderResult *render_result,
                                        const ixam::StringRef layer_name,
                                        const ixam::StringRefNull key_name,
                                        const ixam::StringRefNull value)
{
  BKE_render_result_stamp_data(
      render_result,
      ixam::bke::cryptomatte::BKE_cryptomatte_meta_data_key(layer_name, key_name).c_str(),
      value.data());
}
...
{
  for (const ixam::Map<std::string, ixam::bke::cryptomatte::CryptomatteLayer>::Item item :
       session->layers.items()) {
    const ixam::StringRefNull layer_name(item.key);
    const ixam::bke::cryptomatte::CryptomatteLayer &layer = item.value;
...
namespace ixam::bke::cryptomatte {
namespace manifest {
constexpr StringRef WHITESPACES = " \t\n\v\f\r";

static constexpr ixam::StringRef skip_whitespaces_(ixam::StringRef ref)
{
  size_t skip = ref.find_first_not_of(WHITESPACES);
  if (skip == ixam::StringRef::not_found) {
    return ref;
  }
  return ref.drop_prefix(skip);
}

static constexpr int quoted_string_len_(ixam::StringRef ref)
{
  int len = 1;
...
static std::string unquote_(const ixam::StringRef ref)
{
...
static bool from_manifest(CryptomatteLayer &layer, ixam::StringRefNull manifest)
{
...
static std::string to_manifest(const CryptomatteLayer *layer)
{
  ...
  bool is_first = true;
  const ixam::Map<std::string, CryptomatteHash> &const_map = layer->hashes;
  manifest << "{";
  for (ixam::Map<std::string, CryptomatteHash>::Item item : const_map.items()) {
    if (is_first) {
      is_first = false;
    }
    else {
...
CryptomatteHash CryptomatteHash::from_hex_encoded(ixam::StringRef hex_encoded)
{
...
std::unique_ptr<CryptomatteLayer> CryptomatteLayer::read_from_manifest(
    ixam::StringRefNull manifest)
{
  std::unique_ptr<CryptomatteLayer> layer = std::make_unique<CryptomatteLayer>();
  ixam::bke::cryptomatte::manifest::from_manifest(*layer, manifest);
  return layer;
}
...
uint32_t CryptomatteLayer::add_ID(const ID &id)
{
  ...
  add_hash(ixam::StringRef(name, name_len), cryptohash_int);

  return cryptohash_int;
}

void CryptomatteLayer::add_hash(ixam::StringRef name, CryptomatteHash cryptomatte_hash)
{
...
{
  const ixam::Map<std::string, CryptomatteHash> &const_map = hashes;
  for (ixam::Map<std::string, CryptomatteHash>::Item item : const_map.items()) {
...
{
  return ixam::bke::cryptomatte::manifest::to_manifest(this);
}

ixam::StringRef CryptomatteStampDataCallbackData::extract_layer_hash(ixam::StringRefNull key)
{
  BLI_assert(key.startswith("cryptomatte/"));

  size_t start_index = key.find_first_of('/');
  size_t end_index = key.find_last_of('/');
  if (start_index == ixam::StringRef::not_found) {
    return "";
  }
  if (end_index == ixam::StringRef::not_found) {
    return "";
  }
                                                           ...
                                                           int /*len*/)
{
  ...
  ixam::StringRef layer_hash = extract_layer_hash(key);
                                                              ...
                                                              int /*len*/)
{
  ...
  ixam::StringRef layer_name = data->hash_to_layer_name.lookup(layer_hash);
  ixam::bke::cryptomatte::CryptomatteLayer &layer = data->session->add_layer(layer_name);
  ixam::bke::cryptomatte::manifest::from_manifest(layer, propvalue);
}

const ixam::Vector<std::string> &BKE_cryptomatte_layer_names_get(

...
static void cache_file_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  CacheFile *cache_file = (CacheFile *)id;
  ...
  BKE_id_ixam_write(writer, &cache_file->id);

  if (cache_file->adt) {
    BKE_animdata_ixam_write(writer, cache_file->adt);
  }
...
static void cache_file_ixam_read_data(IxamDataReader *reader, ID *id)
{
  CacheFile *cache_file = (CacheFile *)id;
  BLI_listbase_clear(&cache_file->object_paths);
  cache_file->handle = NULL;
  ...
  BKE_animdata_ixam_read_data(reader, cache_file->adt);
...
IDTypeInfo IDType_ID_CF = {
    ...
    .ixam_write = cache_file_ixam_write,
    .ixam_read_data = cache_file_ixam_read_data,
    .ixam_read_lib = NULL,
                                ...
                                char r_filepath[FILE_MAX])
{
  ...
  BLI_path_abs(r_filepath, ID_IXAM_PATH(bmain, &cache_file->id));

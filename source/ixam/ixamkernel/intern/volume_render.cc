...
static ixam::Vector<openvdb::CoordBBox> get_bounding_boxes(const GridType &grid,
                                                              const bool coarse)
{
  ...
  ixam::Vector<openvdb::CoordBBox> boxes;
...
struct GetBoundingBoxesOp {
  ...
  template<typename GridType> ixam::Vector<openvdb::CoordBBox> operator()()
  {
    return get_bounding_boxes(static_cast<const GridType &>(grid), coarse);
  }
};

static ixam::Vector<openvdb::CoordBBox> get_bounding_boxes(VolumeGridType grid_type,
                                                              const openvdb::GridBase &grid,
                                                              const bool coarse)
{
...
static void boxes_to_center_points(ixam::Span<openvdb::CoordBBox> boxes,
                                   const openvdb::math::Transform &transform,
                                   ixam::MutableSpan<ixam::float3> r_verts)
{
  ...
  for (const int i : boxes.index_range()) {
    ...
    r_verts[i] = ixam::float3(center[0], center[1], center[2]);
  }
}

static void boxes_to_corner_points(ixam::Span<openvdb::CoordBBox> boxes,
                                   const openvdb::math::Transform &transform,
                                   ixam::MutableSpan<ixam::float3> r_verts)
{
  ...
  for (const int i : boxes.index_range()) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      r_verts[8 * i + j] = ixam::float3(corner_d[0], corner_d[1], corner_d[2]);
    }
  }
}

static void boxes_to_edge_mesh(ixam::Span<openvdb::CoordBBox> boxes,
                               const openvdb::math::Transform &transform,
                               ixam::Vector<ixam::float3> &r_verts,
                               ixam::Vector<std::array<int, 2>> &r_edges)
{
...
static void boxes_to_cube_mesh(ixam::Span<openvdb::CoordBBox> boxes,
                               const openvdb::math::Transform &transform,
                               ixam::Vector<ixam::float3> &r_verts,
                               ixam::Vector<std::array<int, 3>> &r_tris)
{
                               ...
                               void *cb_userdata)
{
  ...
  if (volume->display.wireframe_type == VOLUME_WIREFRAME_BOUNDS) {
    /* Bounding box. */
    openvdb::CoordBBox box;
    ixam::Vector<ixam::float3> verts;
    ixam::Vector<std::array<int, 2>> edges;
    ...
    ixam::Vector<openvdb::CoordBBox> boxes = get_bounding_boxes(
...
#ifdef WITH_OPENVDB
static void grow_triangles(ixam::MutableSpan<ixam::float3> verts,
                           ixam::Span<std::array<int, 3>> tris,
                           const float factor)
{
  ...
  ixam::Array<ixam::float3> offsets(verts.size(), {0, 0, 0});
                                       ...
                                       void *cb_userdata)
{
  ...
  ixam::Vector<ixam::float3> verts;
  ixam::Vector<std::array<int, 3>> tris;

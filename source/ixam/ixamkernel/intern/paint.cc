...
static void palette_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Palette *palette = (Palette *)id;

  BLO_write_id_struct(writer, Palette, id_address, &palette->id);
  BKE_id_ixam_write(writer, &palette->id);

  BLO_write_struct_list(writer, PaletteColor, &palette->colors);
}

static void palette_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Palette *palette = (Palette *)id;
  BLO_read_list(reader, &palette->colors);
}

static void palette_undo_preserve(IxamLibReader * /*reader*/, ID *id_new, ID *id_old)
{
...
IDTypeInfo IDType_ID_PAL = {
    ...
    /* ixam_write */ palette_ixam_write,
    /* ixam_read_data */ palette_ixam_read_data,
    /* ixam_read_lib */ nullptr,
    /* ixam_read_expand */ nullptr,

    /* ixam_read_undo_preserve */ palette_undo_preserve,
...
static void paint_curve_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  PaintCurve *pc = (PaintCurve *)id;

  BLO_write_id_struct(writer, PaintCurve, id_address, &pc->id);
  BKE_id_ixam_write(writer, &pc->id);

  BLO_write_struct_array(writer, PaintCurvePoint, pc->tot_points, pc->points);
}

static void paint_curve_ixam_read_data(IxamDataReader *reader, ID *id)
{
  PaintCurve *pc = (PaintCurve *)id;
...
IDTypeInfo IDType_ID_PC = {
    ...
    /* ixam_write */ paint_curve_ixam_write,
    /* ixam_read_data */ paint_curve_ixam_read_data,
    /* ixam_read_lib */ nullptr,
    /* ixam_read_expand */ nullptr,
...
void BKE_paint_ixam_write(IxamWriter *writer, Paint *p)
{
  if (p->cavity_curve) {
    BKE_curvemapping_ixam_write(writer, p->cavity_curve);
  }
  BLO_write_struct_array(writer, PaintToolSlot, p->tool_slots_len, p->tool_slots);
}

void BKE_paint_ixam_read_data(IxamDataReader *reader, const Scene *scene, Paint *p)
{
  ...
  if (p->cavity_curve) {
    BKE_curvemapping_ixam_read(reader, p->cavity_curve);
  }
  else {
...
void BKE_paint_ixam_read_lib(IxamLibReader *reader, Scene *sce, Paint *p)
{
  if (p) {

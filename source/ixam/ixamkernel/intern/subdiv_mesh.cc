                                      ...
                                      const MPoly *coarse_poly)
{
  ...
  if (coarse_poly->totloop == 4) {
    ...
    ixam::Array<float, 32> weights(coarse_poly->totloop);
    ixam::Array<int, 32> indices(coarse_poly->totloop);
                         ...
                         const Mesh *coarse_mesh)
{
  ...
  BKE_subdiv_stats_end(&subdiv->stats, SUBDIV_STATS_SUBDIV_TO_MESH);
  /* Using normals from the limit surface gives different results than 3IXAM's vertex normal

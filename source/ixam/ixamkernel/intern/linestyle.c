...
static void write_linestyle_color_modifiers(IxamWriter *writer, ListBase *modifiers)
{
  LineStyleModifier *m;
...
static void write_linestyle_alpha_modifiers(IxamWriter *writer, ListBase *modifiers)
{
  LineStyleModifier *m;
  ...
  for (m = modifiers->first; m; m = m->next) {
    switch (m->type) {
      case LS_MODIFIER_ALONG_STROKE:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_AlongStroke *)m)->curve);
        break;
      case LS_MODIFIER_DISTANCE_FROM_CAMERA:
        BKE_curvemapping_ixam_write(writer,
      ...
      case LS_MODIFIER_DISTANCE_FROM_OBJECT:
        ...
        break;
      case LS_MODIFIER_MATERIAL:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_Material *)m)->curve);
        break;
      case LS_MODIFIER_TANGENT:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_Tangent *)m)->curve);
        break;
      case LS_MODIFIER_NOISE:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_Noise *)m)->curve);
        break;
      case LS_MODIFIER_CREASE_ANGLE:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_CreaseAngle *)m)->curve);
        break;
      case LS_MODIFIER_CURVATURE_3D:
        BKE_curvemapping_ixam_write(writer, ((LineStyleAlphaModifier_Curvature_3D *)m)->curve);
        break;
    }
  }
}

static void write_linestyle_thickness_modifiers(IxamWriter *writer, ListBase *modifiers)
{
  LineStyleModifier *m;
  ...
  for (m = modifiers->first; m; m = m->next) {
    switch (m->type) {
      case LS_MODIFIER_ALONG_STROKE:
        BKE_curvemapping_ixam_write(writer, ((LineStyleThicknessModifier_AlongStroke *)m)->curve);
        break;
      case LS_MODIFIER_DISTANCE_FROM_CAMERA:
        BKE_curvemapping_ixam_write(writer,
      ...
      case LS_MODIFIER_DISTANCE_FROM_OBJECT:
        ...
        break;
      case LS_MODIFIER_MATERIAL:
        BKE_curvemapping_ixam_write(writer, ((LineStyleThicknessModifier_Material *)m)->curve);
        break;
      case LS_MODIFIER_TANGENT:
        BKE_curvemapping_ixam_write(writer, ((LineStyleThicknessModifier_Tangent *)m)->curve);
        break;
      case LS_MODIFIER_CREASE_ANGLE:
        BKE_curvemapping_ixam_write(writer, ((LineStyleThicknessModifier_CreaseAngle *)m)->curve);
        break;
      case LS_MODIFIER_CURVATURE_3D:
        BKE_curvemapping_ixam_write(writer,
...
static void write_linestyle_geometry_modifiers(IxamWriter *writer, ListBase *modifiers)
{
  LineStyleModifier *m;
...
static void linestyle_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  FreestyleLineStyle *linestyle = (FreestyleLineStyle *)id;

  BLO_write_id_struct(writer, FreestyleLineStyle, id_address, &linestyle->id);
  BKE_id_ixam_write(writer, &linestyle->id);

  if (linestyle->adt) {
    BKE_animdata_ixam_write(writer, linestyle->adt);
  }
  ...
  if (linestyle->nodetree) {
    ...
    ntreeIxamWrite(writer, linestyle->nodetree);
  }
}

static void direct_link_linestyle_color_modifier(IxamDataReader *reader,
...
static void direct_link_linestyle_alpha_modifier(IxamDataReader *reader,
                                                 LineStyleModifier *modifier)
{
  switch (modifier->type) {
    case LS_MODIFIER_ALONG_STROKE: {
      ...
      BKE_curvemapping_ixam_read(reader, m->curve);
      break;
    }
    case LS_MODIFIER_DISTANCE_FROM_CAMERA: {
...
static void direct_link_linestyle_thickness_modifier(IxamDataReader *reader,
                                                     LineStyleModifier *modifier)
{
  switch (modifier->type) {
    case LS_MODIFIER_ALONG_STROKE: {
...
static void direct_link_linestyle_geometry_modifier(IxamDataReader *UNUSED(reader),
                                                    LineStyleModifier *UNUSED(modifier))
{
}

static void linestyle_ixam_read_data(IxamDataReader *reader, ID *id)
{
  FreestyleLineStyle *linestyle = (FreestyleLineStyle *)id;

  BLO_read_data_address(reader, &linestyle->adt);
  BKE_animdata_ixam_read_data(reader, linestyle->adt);
...
static void linestyle_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  FreestyleLineStyle *linestyle = (FreestyleLineStyle *)id;
...
static void linestyle_ixam_read_expand(IxamExpander *expander, ID *id)
{
  FreestyleLineStyle *linestyle = (FreestyleLineStyle *)id;
...
IDTypeInfo IDType_ID_LS = {
    ...
    .ixam_write = linestyle_ixam_write,
    .ixam_read_data = linestyle_ixam_read_data,
    .ixam_read_lib = linestyle_ixam_read_lib,
    .ixam_read_expand = linestyle_ixam_read_expand,
...
static void add_to_modifier_list(ListBase *lb, LineStyleModifier *m)
{
  ...
  BLI_uniquename(
      lb, m, modifier_name[m->type], '_', offsetof(LineStyleModifier, name), sizeof(m->name));
}

...
namespace ixam::bke::tests {

/* UUIDs from lib/tests/asset_library/ixam_assets.cats.txt */
...
class AssetCatalogTest : public testing::Test {
 protected:
  ...
  void SetUp() override
  {
    const std::string test_files_dir = ixam::tests::flags_test_asset_dir();
  ...
  /* Used by on_ixamfile_save__from_memory_into_existing_asset_lib* test functions. */
  void save_from_memory_into_existing_asset_lib(const bool should_top_level_cdf_exist)
  {
    ...
    const CatalogFilePath ixamfilename = asset_lib_subdir + "some_file.ixam";
    const AssetCatalog *cat = service.create_catalog("some/catalog/path");

    /* Mock that the ixam file is written to the directory already containing a CDF. */
    ASSERT_TRUE(service.write_to_disk(ixamfilename));
...
TEST_F(AssetCatalogTest, load_single_file)
{
  ...
  service.load_from_disk(asset_library_root_ + SEP_STR + "ixam_assets.cats.txt");
...
TEST_F(AssetCatalogTest, no_writing_empty_files)
{
  ...
  service.write_to_disk(temp_lib_root + "phony.ixam");
...
TEST_F(AssetCatalogTest, on_ixamfile_save__with_existing_cdf)
{
  ...
  const CatalogFilePath ixamfilename = top_level_dir + "subdir/some_file.ixam";
  ASSERT_TRUE(service.write_to_disk(ixamfilename));
...
TEST_F(AssetCatalogTest, on_ixamfile_save__from_memory_into_empty_directory)
{
  const CatalogFilePath target_dir = create_temp_path(); /* Has trailing slash. */
  ...
  const CatalogFilePath ixamfilename = target_dir + "some_file.ixam";
  ASSERT_TRUE(service.write_to_disk(ixamfilename));
...
TEST_F(AssetCatalogTest, on_ixamfile_save__from_memory_into_existing_cdf_and_merge)
{
  const CatalogFilePath target_dir = create_temp_path(); /* Has trailing slash. */
  const CatalogFilePath original_cdf_file = asset_library_root_ + SEP_STR +
                                            "ixam_assets.cats.txt";
  ...
  /* Mock that the ixam file is written to a subdirectory of the asset library. */
  const CatalogFilePath ixamfilename = target_dir + "some_file.ixam";
  ASSERT_TRUE(service.write_to_disk(ixamfilename));
...
TEST_F(AssetCatalogTest,
       on_ixamfile_save__from_memory_into_existing_asset_lib_without_top_level_cdf)
{
...
TEST_F(AssetCatalogTest, on_ixamfile_save__from_memory_into_existing_asset_lib)
{
...
TEST_F(AssetCatalogTest, merge_catalog_files)
{
  ...
  const CatalogFilePath temp_cdf_file = cdf_dir + "ixam_assets.cats.txt";
  ...
  service.write_to_disk(cdf_dir + "phony.ixam");
...
TEST_F(AssetCatalogTest, backups)
{
  ...
  const CatalogFilePath original_cdf_file = asset_library_root_ + SEP_STR +
                                            "ixam_assets.cats.txt";
  const CatalogFilePath writable_cdf_file = cdf_dir + SEP_STR + "ixam_assets.cats.txt";
  ASSERT_EQ(0, BLI_copy(original_cdf_file.c_str(), writable_cdf_file.c_str()));

 ...
 * \ingroup bke
 *
 * Ixam file undo (known as 'Global Undo').
...
#include "BKE_ixam_undo.h" /* own include */
                             ...
                             bContext *C)
{
  ...
  BLI_strncpy(mainstr, BKE_main_ixamfile_path(bmain), sizeof(mainstr)); /* temporal store */

  fileflags = G.fileflags;
  G.fileflags |= G_FILE_NO_UI;

  if (UNDO_DISK) {
    const struct IxamFileReadParams params = {0};
    IxamFileReadReport bf_reports = {.reports = NULL};
    struct IxamFileData *bfd = BKE_ixamfile_read(mfu->filepath, &params, &bf_reports);
    if (bfd != NULL) {
    ...
    struct IxamFileReadParams params = {0};
    params.undo_direction = undo_direction;
    if (!use_old_bmain_data) {
      params.skip_flags |= BLO_READ_SKIP_UNDO_OLD_MAIN;
    }
    struct IxamFileData *bfd = BKE_ixamfile_read_from_memfile(
        bmain, &mfu->memfile, &params, NULL);
    if (bfd != NULL) {
      BKE_ixamfile_read_setup(C, bfd, &params, &(IxamFileReadReport){NULL});
      success = true;
    }
  }
...
MemFileUndoData *BKE_memfile_undo_encode(Main *bmain, MemFileUndoData *mfu_prev)
{
  ...
  /* Include recovery information since undo-data is written out as #IXAM_QUIT_FILE. */
  ...
  if (UNDO_DISK) {
    ...
    BLI_snprintf(numstr, sizeof(numstr), "%d.ixam", counter);

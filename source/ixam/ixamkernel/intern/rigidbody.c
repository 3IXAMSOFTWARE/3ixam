...
/** \file
 * \ingroup bke
 * \brief 3IXAM-side interface and methods for dealing with Rigid Body simulations
 */
...
static Mesh *rigidbody_get_mesh(Object *ob)
{
  ...
  /* Just return something sensible so that at least 3IXAM won't crash. */
...
RigidBodyWorld *BKE_rigidbody_create_world(Scene *scene)
{
   ...
   * The ixam default scene has a frame rate of 24, so take 10 substeps (24fps * 10).
   */
...
static bool rigidbody_add_object_to_scene(Main *bmain, Scene *scene, Object *ob)
{
  ...
  if (rbw->group == NULL) {
    ...
    rbw->group->protectflag |= COL_LOCK_DELETE;

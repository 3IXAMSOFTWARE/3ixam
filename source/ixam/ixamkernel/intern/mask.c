...
static void mask_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Mask *mask = (Mask *)id;
  ...
  BKE_id_ixam_write(writer, &mask->id);

  if (mask->adt) {
    BKE_animdata_ixam_write(writer, mask->adt);
  }
...
static void mask_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Mask *mask = (Mask *)id;
...
static void lib_link_mask_parent(IxamLibReader *reader, Mask *mask, MaskParent *parent)
{
  BLO_read_id_address(reader, mask->id.lib, &parent->id);
}

static void mask_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Mask *mask = (Mask *)id;
  LISTBASE_FOREACH (MaskLayer *, masklay, &mask->masklayers) {
    MaskSpline *spline;
...
static void expand_mask_parent(IxamExpander *expander, MaskParent *parent)
{
  if (parent->id) {
...
static void mask_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Mask *mask = (Mask *)id;
  LISTBASE_FOREACH (MaskLayer *, mask_layer, &mask->masklayers) {
...
IDTypeInfo IDType_ID_MSK = {
    ...
    .ixam_write = mask_ixam_write,
    .ixam_read_data = mask_ixam_read_data,
    .ixam_read_lib = mask_ixam_read_lib,
    .ixam_read_expand = mask_ixam_read_expand,

...
{
  BLI_uniquename(list, con, DATA_("Const"), '_', offsetof(bConstraint, name), sizeof(con->name));
}
...
/* The functions here are called by various parts of 3IXAM. Very few (should be none if possible)
...
void BKE_constraint_ixam_write(IxamWriter *writer, ListBase *conlist)
{
  LISTBASE_FOREACH (bConstraint *, con, conlist) {
    const bConstraintTypeInfo *cti = BKE_constraint_typeinfo_get(con);
...
void BKE_constraint_ixam_read_data(IxamDataReader *reader, ListBase *lb)
{
...
void BKE_constraint_ixam_read_lib(IxamLibReader *reader, ID *id, ListBase *conlist)
{
...
static void expand_constraint_cb(bConstraint *UNUSED(con),
                                 ...
                                 void *userdata)
{
  IxamExpander *expander = userdata;
  BLO_expand(expander, *idpoin);
}

void BKE_constraint_ixam_read_expand(IxamExpander *expander, ListBase *lb)
{

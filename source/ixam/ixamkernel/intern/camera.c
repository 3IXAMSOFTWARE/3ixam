...
static void camera_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Camera *cam = (Camera *)id;

  /* write LibData */
  BLO_write_id_struct(writer, Camera, id_address, &cam->id);
  BKE_id_ixam_write(writer, &cam->id);

  if (cam->adt) {
    BKE_animdata_ixam_write(writer, cam->adt);
  }
...
static void camera_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Camera *ca = (Camera *)id;
  BLO_read_data_address(reader, &ca->adt);
  BKE_animdata_ixam_read_data(reader, ca->adt);
...
static void camera_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Camera *ca = (Camera *)id;
...
static void camera_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Camera *ca = (Camera *)id;
...
IDTypeInfo IDType_ID_CA = {
    ...
    .ixam_write = camera_ixam_write,
    .ixam_read_data = camera_ixam_read_data,
    .ixam_read_lib = camera_ixam_read_lib,
    .ixam_read_expand = camera_ixam_read_expand,
                                   ...
                                   const RegionView3D *rv3d)
{
  ...
  params->clip_end = KNIFE_CLIP_END_MAX;

...
void BKE_animdata_ixam_write(IxamWriter *writer, struct AnimData *adt)
{
  /* firstly, just write the AnimData block */
  BLO_write_struct(writer, AnimData, adt);

  /* write drivers */
  BKE_fcurve_ixam_write(writer, &adt->drivers);
  ...
  /* write NLA data */
  BKE_nla_ixam_write(writer, &adt->nla_tracks);
}

void BKE_animdata_ixam_read_data(IxamDataReader *reader, AnimData *adt)
{
  ...
  BKE_fcurve_ixam_read_data(reader, &adt->drivers);
  ...
  BKE_nla_ixam_read_data(reader, &adt->nla_tracks);
...
void BKE_animdata_ixam_read_lib(IxamLibReader *reader, ID *id, AnimData *adt)
{
  if (adt == NULL) {
    return;
  }
  ...
  /* link drivers */
  BKE_fcurve_ixam_read_lib(reader, id, &adt->drivers);

  /* overrides don't have lib-link for now, so no need to do anything */

  /* link NLA-data */
  BKE_nla_ixam_read_lib(reader, id, &adt->nla_tracks);
}

void BKE_animdata_ixam_read_expand(struct IxamExpander *expander, AnimData *adt)
{
  /* own action */
  ...
  BKE_fcurve_ixam_read_expand(expander, &adt->drivers);

  /* NLA data - referenced actions. */
  BKE_nla_ixam_read_expand(expander, &adt->nla_tracks);
}

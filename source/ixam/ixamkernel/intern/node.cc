...
using ixam::Vector;
using ixam::VectorSet;
using ixam::bke::bNodeRuntime;
using ixam::bke::bNodeSocketRuntime;
using ixam::bke::bNodeTreeRuntime;
using ixam::nodes::FieldInferencingInterface;
using ixam::nodes::InputSocketFieldType;
using ixam::nodes::NodeDeclaration;
using ixam::nodes::OutputFieldDependency;
using ixam::nodes::OutputSocketFieldType;
using ixam::nodes::SocketDeclaration;
...
static void write_node_socket_default_value(IxamWriter *writer, bNodeSocket *sock)
{
...
static void write_node_socket(IxamWriter *writer, bNodeSocket *sock)
{
  BLO_write_struct(writer, bNodeSocket, sock);

  if (sock->prop) {
    IDP_IxamWrite(writer, sock->prop);
  }
...
static void write_node_socket_interface(IxamWriter *writer, bNodeSocket *sock)
{
  BLO_write_struct(writer, bNodeSocket, sock);
...
void ntreeIxamWrite(IxamWriter *writer, bNodeTree *ntree)
{
  BKE_id_ixam_write(writer, &ntree->id);

  if (ntree->adt) {
    BKE_animdata_ixam_write(writer, ntree->adt);
  }
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    ...
    if (node->prop) {
      IDP_IxamWrite(writer, node->prop);
    }
    ...
    if (node->storage) {
      if (ELEM(ntree->type, NTREE_SHADER, NTREE_GEOMETRY) &&
        ...
        BKE_curvemapping_ixam_write(writer, (const CurveMapping *)node->storage);
      }
    ...
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      ...
      BKE_image_format_ixam_write(writer, &nimf->format);

      LISTBASE_FOREACH (bNodeSocket *, sock, &node->inputs) {
        NodeImageMultiFileSocket *sockdata = (NodeImageMultiFileSocket *)sock->storage;
        BLO_write_struct(writer, NodeImageMultiFileSocket, sockdata);
        BKE_image_format_ixam_write(writer, &sockdata->format);
      }
    }
  ...
  BKE_previewimg_ixam_write(writer, ntree->preview);
}

static void ntree_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  bNodeTree *ntree = (bNodeTree *)id;
  ...
  ntreeIxamWrite(writer, ntree);
}

static void direct_link_node_socket(IxamDataReader *reader, bNodeSocket *sock)
{
  BLO_read_data_address(reader, &sock->prop);
  IDP_IxamDataRead(reader, &sock->prop);
...
void ntreeIxamReadData(IxamDataReader *reader, ID *owner_id, bNodeTree *ntree)
{
  ...
  BKE_animdata_ixam_read_data(reader, ntree->adt);
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    ...
    IDP_IxamDataRead(reader, &node->prop);
    ...
    if (node->storage) {
      switch (node->type) {
        ...
        case TEX_NODE_CURVE_TIME: {
          BKE_curvemapping_ixam_read(reader, (CurveMapping *)node->storage);
          break;
        }
        case SH_NODE_SCRIPT: {
          NodeShaderScript *nss = (NodeShaderScript *)node->storage;
        ...
        case SH_NODE_TEX_POINTDENSITY: {
          ...
          npd->pd = ixam::dna::shallow_zero_initialize();
          break;
        }
        ...
        case CMP_NODE_OUTPUT_FILE: {
          NodeImageMultiFile *nimf = (NodeImageMultiFile *)node->storage;
          BKE_image_format_ixam_read_data(reader, &nimf->format);
          break;
        }
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    ...
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      LISTBASE_FOREACH (bNodeSocket *, sock, &node->inputs) {
        NodeImageMultiFileSocket *sockdata = (NodeImageMultiFileSocket *)sock->storage;
        BKE_image_format_ixam_read_data(reader, &sockdata->format);
      }
    }
  }
  ...
  BKE_previewimg_ixam_read(reader, ntree->preview);

  /* type verification is in lib-link */
}

static void ntree_ixam_read_data(IxamDataReader *reader, ID *id)
{
  bNodeTree *ntree = (bNodeTree *)id;
  ntreeIxamReadData(reader, nullptr, ntree);
}

static void lib_link_node_socket(IxamLibReader *reader, Library *lib, bNodeSocket *sock)
{
  IDP_IxamReadLib(reader, lib, sock->prop);

  /* This can happen for all socket types when a file is saved in an older version of 3IXAM than
...
static void lib_link_node_sockets(IxamLibReader *reader, Library *lib, ListBase *sockets)
{
...
void ntreeIxamReadLib(struct IxamLibReader *reader, struct bNodeTree *ntree)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    ...
    IDP_IxamReadLib(reader, lib, node->prop);
...
static void ntree_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bNodeTree *ntree = (bNodeTree *)id;
  ntreeIxamReadLib(reader, ntree);
}

static void expand_node_socket(IxamExpander *expander, bNodeSocket *sock)
{
  IDP_IxamReadExpand(expander, sock->prop);
...
static void expand_node_sockets(IxamExpander *expander, ListBase *sockets)
{
  LISTBASE_FOREACH (bNodeSocket *, sock, sockets) {
...
void ntreeIxamReadExpand(IxamExpander *expander, bNodeTree *ntree)
{
  if (ntree->gpd) {
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    ...
    IDP_IxamReadExpand(expander, node->prop);
...
static void ntree_ixam_read_expand(IxamExpander *expander, ID *id)
{
  bNodeTree *ntree = (bNodeTree *)id;
  ntreeIxamReadExpand(expander, ntree);
}
...
static AssetTypeInfo AssetType_NT = {
    /* pre_save_fn */ ixam::bke::node_tree_asset_pre_save,
};
...
IDTypeInfo IDType_ID_NT = {
    ...
    /* ixam_write */ ntree_ixam_write,
    /* ixam_read_data */ ntree_ixam_read_data,
    /* ixam_read_lib */ ntree_ixam_read_lib,
    /* ixam_read_expand */ ntree_ixam_read_expand,

    /* ixam_read_undo_preserve */ nullptr,
...
void nodeRegisterType(bNodeType *nt)
{
  ...
  if (nt->declare && !nt->declaration_is_dynamic) {
    if (nt->fixed_declaration == nullptr) {
      nt->fixed_declaration = new ixam::nodes::NodeDeclaration();
      ixam::nodes::NodeDeclarationBuilder builder{*nt->fixed_declaration};
      nt->declare(builder);
    }
  }
...
bool nodeIsDanglingReroute(const bNodeTree *ntree, const bNode *node)
{
  ...
  BLI_assert(ixam::bke::node_tree_runtime::topology_cache_is_available(*ntree));
  ...
  while (true) {
    const ixam::Span<const bNodeLink *> links =
...
{
  BLI_uniquename(
      &ntree->nodes, node, DATA_("Node"), '_', offsetof(bNode, name), sizeof(node->name));
}
...
static void ntree_interface_type_create(bNodeTree *ntree)
{
  ...
  StructRNA *srna = RNA_def_struct_ptr(&IXAM_RNA, identifier, &RNA_PropertyGroup);
  RNA_def_struct_ui_text(srna, name, description);
  RNA_def_struct_duplicate_pointers(&IXAM_RNA, srna);

  /* associate the RNA type with the node tree */
  ntree->interface_type = srna;
  RNA_struct_ixam_type_set(srna, ntree);
...
StructRNA *ntreeInterfaceTypeGet(bNodeTree *ntree, bool create)
{
  if (ntree->interface_type) {
    ...
    if (!STREQLEN(RNA_struct_identifier(srna), base, sizeof(base))) {
      ...
      /* rename the RNA type */
      RNA_def_struct_free_pointers(&IXAM_RNA, srna);
      RNA_def_struct_identifier(&IXAM_RNA, srna, identifier);
      RNA_def_struct_ui_text(srna, name, description);
      RNA_def_struct_duplicate_pointers(&IXAM_RNA, srna);
    }
  }
...
{
  if (ntree->interface_type) {
    RNA_struct_free(&IXAM_RNA, ntree->interface_type);
    ntree->interface_type = nullptr;
  }
}
...
static void update_socket_declarations(ListBase *sockets,
                                       Span<ixam::nodes::SocketDeclarationPtr> declarations)
{
  int index;
...
bool nodeDeclarationEnsureOnOutdatedNode(bNodeTree * /*ntree*/, bNode *node)
{
  ...
  if (node->typeinfo->declaration_is_dynamic) {
    node->runtime->declaration = new ixam::nodes::NodeDeclaration();
    ixam::nodes::NodeDeclarationBuilder builder{*node->runtime->declaration};
...
bool BKE_node_clipboard_validate()
{
  ...
  for (node = (bNode *)node_clipboard.nodes.first,
    ...
    if (node->id) {
      /* We want to search into current ixam file, so using G_MAIN is valid here too. */
...
void node_type_base(bNodeType *ntype, int type, const char *name, short nclass)
{
   ...
   * and call RNA_struct_ixam_type_set, so this only needs to be done for old RNA types
...
#define DefNode(Category, ID, DefFunc, EnumName, StructName, UIName, UIDesc) \
  case ID: \
    ...
    RNA_struct_ixam_type_set(ntype->rna_ext.srna, ntype); \
    break;

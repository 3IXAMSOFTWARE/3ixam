...
static void lattice_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  BKE_id_ixam_write(writer, &lt->id);

  /* write animdata */
  if (lt->adt) {
    BKE_animdata_ixam_write(writer, lt->adt);
  }

  /* direct data */
  BLO_write_struct_array(writer, BPoint, lt->pntsu * lt->pntsv * lt->pntsw, lt->def);

  BKE_defbase_ixam_write(writer, &lt->vertex_group_names);
  BKE_defvert_ixam_write(writer, lt->pntsu * lt->pntsv * lt->pntsw, lt->dvert);
}

static void lattice_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Lattice *lt = (Lattice *)id;
  BLO_read_data_address(reader, &lt->def);

  BLO_read_data_address(reader, &lt->dvert);
  BKE_defvert_ixam_read(reader, lt->pntsu * lt->pntsv * lt->pntsw, lt->dvert);
  ...
  BKE_animdata_ixam_read_data(reader, lt->adt);
}

static void lattice_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Lattice *lt = (Lattice *)id;
...
static void lattice_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Lattice *lt = (Lattice *)id;
...
IDTypeInfo IDType_ID_LT = {
    ...
    .ixam_write = lattice_ixam_write,
    .ixam_read_data = lattice_ixam_read_data,
    .ixam_read_lib = lattice_ixam_read_lib,
    .ixam_read_expand = lattice_ixam_read_expand,

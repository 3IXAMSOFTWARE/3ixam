...
using ixam::RandomNumberGenerator;
using ixam::Span;
using ixam::Vector;
...
static void curves_init_data(ID *id)
{
  ...
  new (&curves->geometry) ixam::bke::CurvesGeometry();
}
...
static void curves_free_data(ID *id)
{
  ...
  ixam::bke::CurvesGeometry::wrap(curves->geometry).~CurvesGeometry();
...
static void curves_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Curves *curves = (Curves *)id;

  Vector<CustomDataLayer, 16> point_layers;
  Vector<CustomDataLayer, 16> curve_layers;
  CustomData_ixam_write_prepare(curves->geometry.point_data, point_layers);
  CustomData_ixam_write_prepare(curves->geometry.curve_data, curve_layers);

  /* Write LibData */
  BLO_write_id_struct(writer, Curves, id_address, &curves->id);
  BKE_id_ixam_write(writer, &curves->id);

  /* Direct data */
  CustomData_ixam_write(writer,
  ...
  if (curves->adt) {
    BKE_animdata_ixam_write(writer, curves->adt);
  }
}

static void curves_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Curves *curves = (Curves *)id;
  BLO_read_data_address(reader, &curves->adt);
  BKE_animdata_ixam_read_data(reader, curves->adt);

  /* Geometry */
  CustomData_ixam_read(reader, &curves->geometry.point_data, curves->geometry.point_num);
  CustomData_ixam_read(reader, &curves->geometry.curve_data, curves->geometry.curve_num);
  ...
  curves->geometry.runtime = MEM_new<ixam::bke::CurvesGeometryRuntime>(__func__);

  /* Recalculate curve type count cache that isn't saved in files. */
  ixam::bke::CurvesGeometry::wrap(curves->geometry).update_curve_types();

  /* Materials */
  BLO_read_pointer_array(reader, (void **)&curves->mat);
}

static void curves_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Curves *curves = (Curves *)id;
...
static void curves_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Curves *curves = (Curves *)id;
  for (int a = 0; a < curves->totcol; a++) {
...
IDTypeInfo IDType_ID_CV = {
    ...
    /* ixam_write */ curves_ixam_write,
    /* ixam_read_data */ curves_ixam_read_data,
    /* ixam_read_lib */ curves_ixam_read_lib,
    /* ixam_read_expand */ curves_ixam_read_expand,
...
BoundBox *BKE_curves_boundbox_get(Object *ob)
{
  ...
  if (ob->runtime.bb == nullptr) {
    ...
    const ixam::bke::CurvesGeometry &curves = ixam::bke::CurvesGeometry::wrap(
        curves_id->geometry);
...
void BKE_curves_data_update(struct Depsgraph *depsgraph, struct Scene *scene, Object *object)
{
  ...
  if (object->mode == OB_MODE_SCULPT_CURVES) {
    ...
    edit_component.curves_edit_hints_ = std::make_unique<ixam::bke::CurvesEditHints>(
  ...
  if (curves_eval == nullptr) {
    curves_eval = ixam::bke::curves_new_nomain(0, 0);

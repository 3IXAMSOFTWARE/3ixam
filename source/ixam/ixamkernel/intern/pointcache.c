...
static int ptcache_path(PTCacheID *pid, char *dirname)
{
  ...
  const char *ixamfilename = (lib && (pid->cache->flag & PTCACHE_IGNORE_LIBPATH) == 0) ?
                                  lib->filepath_abs :
                                  ixamfile_path;
  size_t i;
  ...
  if (pid->cache->flag & PTCACHE_EXTERNAL) {
    ...
    if (BLI_path_is_rel(dirname)) {
      BLI_path_abs(dirname, ixamfilename);
    }

    return BLI_path_slash_ensure(dirname, MAX_PTCACHE_FILE); /* new strlen() */
  }
  if ((ixamfile_path[0] != '\0') || lib) {
    char file[MAX_PTCACHE_PATH]; /* we don't want the dir, only the file */

    BLI_split_file_part(ixamfilename, file, sizeof(file));
    i = strlen(file);
    ...
    /* Add ixam file name to pointcache dir. */
    ...
    PTCacheID *pid, char *filepath, int cfra, const bool do_path, const bool do_ext)
{
  ...
  if ((pid->cache->flag & PTCACHE_EXTERNAL) == 0) {
    ...
    if (ixamfile_path[0] == '\0') {
      return 0; /* save ixam file before using disk pointcache */
    }
  }
...
static PTCacheFile *ptcache_file_open(PTCacheID *pid, int mode, int cfra)
{
  ...
  if ((pid->cache->flag & PTCACHE_EXTERNAL) == 0) {
    const char *ixamfile_path = BKE_main_ixamfile_path_from_global();
    if (ixamfile_path[0] == '\0') {
      return NULL; /* save ixam file before using disk pointcache */
    }
  }
...
void BKE_ptcache_ixam_write(IxamWriter *writer, ListBase *ptcaches)
{
...
static void direct_link_pointcache_cb(IxamDataReader *reader, void *data)
{
  PTCacheMem *pm = data;
  for (int i = 0; i < BPHYS_TOT_DATA; i++) {
...
static void direct_link_pointcache(IxamDataReader *reader, PointCache *cache)
{
  if ((cache->flag & PTCACHE_DISK_CACHE) == 0) {
...
void BKE_ptcache_ixam_read_data(IxamDataReader *reader,
                                 ListBase *ptcaches,
                                 PointCache **ocache,
                                 int force_disk)
{

...
#include "BKE_ixam_copybuffer.h" /* own include */
...
/* -------------------------------------------------------------------- */
/** \name Copy/Paste `.ixam`, partial saves.
 * \{ */

void BKE_copybuffer_copy_begin(Main *bmain_src)
{
  BKE_ixamfile_write_partial_begin(bmain_src);
}

void BKE_copybuffer_copy_tag_ID(ID *id)
{
  BKE_ixamfile_write_partial_tag_ID(id, true);
}
...
bool BKE_copybuffer_copy_end(Main *bmain_src, const char *filename, ReportList *reports)
{
  ...
  bool retval = BKE_ixamfile_write_partial(bmain_src, filename, write_flags, remap_mode, reports);

  BKE_ixamfile_write_partial_end(bmain_src);

  return retval;
}

/* Common helper for paste functions. */
static void copybuffer_append(IxamfileLinkAppendContext *lapp_context,
                              Main *bmain,
                              ReportList *reports)
{
  ...
  BKE_ixamfile_append(lapp_context, reports);

  /* This must be unset, otherwise these object won't link into other scenes from this ixam
   * file. */
                         ...
                         const uint64_t id_types_mask)
{
  ...
  const int num_pasted = BKE_ixamfile_link_append_context_item_idtypes_from_library_add(
      lapp_context, reports, id_types_mask, 0);
  if (num_pasted == IXAMFILE_LINK_APPEND_INVALID) {

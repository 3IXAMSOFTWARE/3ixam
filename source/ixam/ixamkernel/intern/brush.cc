...
static void brush_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Brush *brush = (Brush *)id;

  BLO_write_id_struct(writer, Brush, id_address, &brush->id);
  BKE_id_ixam_write(writer, &brush->id);

  if (brush->curve) {
    BKE_curvemapping_ixam_write(writer, brush->curve);
  }

  if (brush->automasking_cavity_curve) {
    BKE_curvemapping_ixam_write(writer, brush->automasking_cavity_curve);
  }
  ...
  if (brush->gpencil_settings) {
    ...
    if (brush->gpencil_settings->curve_sensitivity) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_sensitivity);
    }
    if (brush->gpencil_settings->curve_strength) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_strength);
    }
    if (brush->gpencil_settings->curve_jitter) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_jitter);
    }
    if (brush->gpencil_settings->curve_rand_pressure) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_pressure);
    }
    if (brush->gpencil_settings->curve_rand_strength) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_strength);
    }
    if (brush->gpencil_settings->curve_rand_uv) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_uv);
    }
    if (brush->gpencil_settings->curve_rand_hue) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_hue);
    }
    if (brush->gpencil_settings->curve_rand_saturation) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_saturation);
    }
    if (brush->gpencil_settings->curve_rand_value) {
      BKE_curvemapping_ixam_write(writer, brush->gpencil_settings->curve_rand_value);
    }
  }
...
static void brush_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Brush *brush = (Brush *)id;
  ...
  if (brush->curve) {
    BKE_curvemapping_ixam_read(reader, brush->curve);
  }
  else {
  ...
  if (brush->automasking_cavity_curve) {
    BKE_curvemapping_ixam_read(reader, brush->automasking_cavity_curve);
  }
  else {
  ...
  if (brush->gpencil_settings != nullptr) {
    ...
    if (brush->gpencil_settings->curve_sensitivity) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_sensitivity);
    }

    if (brush->gpencil_settings->curve_strength) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_strength);
    }

    if (brush->gpencil_settings->curve_jitter) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_jitter);
    }

    if (brush->gpencil_settings->curve_rand_pressure) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_pressure);
    }

    if (brush->gpencil_settings->curve_rand_strength) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_strength);
    }

    if (brush->gpencil_settings->curve_rand_uv) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_uv);
    }

    if (brush->gpencil_settings->curve_rand_hue) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_hue);
    }

    if (brush->gpencil_settings->curve_rand_saturation) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_saturation);
    }

    if (brush->gpencil_settings->curve_rand_value) {
      BKE_curvemapping_ixam_read(reader, brush->gpencil_settings->curve_rand_value);
    }
  }
...
static void brush_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Brush *brush = (Brush *)id;
...
static void brush_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Brush *brush = (Brush *)id;
...
{
  IxamLibReader *reader = (IxamLibReader *)cb_data->user_data;
...
static void brush_undo_preserve(IxamLibReader *reader, ID *id_new, ID *id_old)
{
...
IDTypeInfo IDType_ID_BR = {
    ...
    /* ixam_write */ brush_ixam_write,
    /* ixam_read_data */ brush_ixam_read_data,
    /* ixam_read_lib */ brush_ixam_read_lib,
    /* ixam_read_expand */ brush_ixam_read_expand,

    /* ixam_read_undo_preserve */ brush_undo_preserve,

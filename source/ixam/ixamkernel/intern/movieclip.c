...
static void write_movieTracks(IxamWriter *writer, ListBase *tracks)
{
...
static void write_moviePlaneTracks(IxamWriter *writer, ListBase *plane_tracks_base)
{
...
static void write_movieReconstruction(IxamWriter *writer,
...
static void movieclip_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  BKE_id_ixam_write(writer, &clip->id);

  if (clip->adt) {
    BKE_animdata_ixam_write(writer, clip->adt);
  }
...
static void direct_link_movieReconstruction(IxamDataReader *reader,
                                            MovieTrackingReconstruction *reconstruction)
{
  BLO_read_data_address(reader, &reconstruction->cameras);
}

static void direct_link_movieTracks(IxamDataReader *reader, ListBase *tracksbase)
{
...
static void direct_link_moviePlaneTracks(IxamDataReader *reader, ListBase *plane_tracks_base)
{
...
static void movieclip_ixam_read_data(IxamDataReader *reader, ID *id)
{
  MovieClip *clip = (MovieClip *)id;
  MovieTracking *tracking = &clip->tracking;

  BLO_read_data_address(reader, &clip->adt);
  BKE_animdata_ixam_read_data(reader, clip->adt);
...
static void lib_link_movieTracks(IxamLibReader *reader, MovieClip *clip, ListBase *tracksbase)
{
  LISTBASE_FOREACH (MovieTrackingTrack *, track, tracksbase) {
...
static void lib_link_moviePlaneTracks(IxamLibReader *reader,
                                      MovieClip *clip,
                                      ListBase *tracksbase)
{
...
static void movieclip_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  MovieClip *clip = (MovieClip *)id;
  MovieTracking *tracking = &clip->tracking;
...
IDTypeInfo IDType_ID_MC = {
    ...
    .ixam_write = movieclip_ixam_write,
    .ixam_read_data = movieclip_ixam_read_data,
    .ixam_read_lib = movieclip_ixam_read_lib,
...
static void get_sequence_fname(const MovieClip *clip, const int framenr, char *name)
{
  ...
  BLI_path_abs(name, ID_IXAM_PATH_FROM_GLOBAL(&clip->id));
}
    ...
    const MovieClip *clip, int proxy_render_size, bool undistorted, int framenr, char *name)
{
  ...
  BLI_path_abs(name, BKE_main_ixamfile_path_from_global());
...
static void movieclip_open_anim_file(MovieClip *clip)
{
  ...
  if (!clip->anim) {
    ...
    BLI_path_abs(str, ID_IXAM_PATH_FROM_GLOBAL(&clip->id));
    ...
    if (clip->anim) {
      if (clip->flag & MCLIP_USE_PROXY_CUSTOM_DIR) {
        char dir[FILE_MAX];
        BLI_strncpy(dir, clip->proxy.dir, sizeof(dir));
        BLI_path_abs(dir, BKE_main_ixamfile_path_from_global());
...
MovieClip *BKE_movieclip_file_add(Main *bmain, const char *name)
{
  ...
  BLI_path_abs(str, BKE_main_ixamfile_path(bmain));
...
MovieClip *BKE_movieclip_file_add_exists_ex(Main *bmain, const char *filepath, bool *r_exists)
{
  ...
  for (clip = bmain->movieclips.first; clip; clip = clip->id.next) {
    ...
    BLI_path_abs(strtest, ID_IXAM_PATH(bmain, &clip->id));
...
GPUTexture *BKE_movieclip_get_gpu_texture(MovieClip *clip, MovieClipUser *cuser)
{
  ...
  if (ibuf == NULL) {
    fprintf(stderr, "GPUTexture: 3IXAM Texture Not Loaded!\n");

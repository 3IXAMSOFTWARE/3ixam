...
static void image_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Image *ima = (Image *)id;
  ...
  BKE_id_ixam_write(writer, &ima->id);

  for (imapf = static_cast<ImagePackedFile *>(ima->packedfiles.first); imapf;
       imapf = imapf->next) {
    BLO_write_struct(writer, ImagePackedFile, imapf);
    BKE_packedfile_ixam_write(writer, imapf->packedfile);
  }

  BKE_previewimg_ixam_write(writer, ima->preview);
...
static void image_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Image *ima = (Image *)id;
  ...
  if (ima->packedfiles.first) {
    LISTBASE_FOREACH (ImagePackedFile *, imapf, &ima->packedfiles) {
      BKE_packedfile_ixam_read(reader, &imapf->packedfile);
    }
    ima->packedfile = nullptr;
  }
  else {
    BKE_packedfile_ixam_read(reader, &ima->packedfile);
  }

  BLI_listbase_clear(&ima->anims);
  BLO_read_data_address(reader, &ima->preview);
  BKE_previewimg_ixam_read(reader, ima->preview);
...
static void image_ixam_read_lib(IxamLibReader * /*reader*/, ID *id)
{
  Image *ima = (Image *)id;
...
constexpr IDTypeInfo get_type_info()
{
  ...
  info.ixam_write = image_ixam_write;
  info.ixam_read_data = image_ixam_read_data;
  info.ixam_read_lib = image_ixam_read_lib;
  info.ixam_read_expand = nullptr;

  info.ixam_read_undo_preserve = nullptr;
...
Image *BKE_image_load_exists_ex(Main *bmain, const char *filepath, bool *r_exists)
{
  ...
  for (ima = static_cast<Image *>(bmain->images.first); ima;
    ...
    if (!ELEM(ima->source, IMA_SRC_VIEWER, IMA_SRC_GENERATED)) {
      ...
      BLI_path_abs(strtest, ID_IXAM_PATH(bmain, &ima->id));
    ...
    const Scene *scene, Object *camera, StampData *stamp_data, int do_prefix, bool use_dynamic)
{
  ...
  if (scene->r.stamp & R_STAMP_FILENAME) {
    ...
    SNPRINTF(stamp_data->file,
             do_prefix ? "File %s" : "%s",
             (ixamfile_path[0] != '\0') ? ixamfile_path : "<untitled>");
  }
  else {
    stamp_data->file[0] = '\0';
  }
    ...
    case IMA_SIGNAL_SRC_CHANGE:
      ...
      if (ima->source != IMA_SRC_TILED) {
        ...
        if (BKE_image_is_filename_tokenized(ima->filepath)) {
          ...
          if (was_relative) {
            const char *relbase = ID_IXAM_PATH(bmain, &ima->id);
    ...
    case IMA_SIGNAL_RELOAD:
      ...
      if (BKE_image_has_packedfile(ima)) {
        ...
        if (tot_files != BLI_listbase_count_at_most(&ima->packedfiles, tot_files + 1)) {
          ...
          image_free_packedfiles(ima);
          BKE_image_packfiles(nullptr, ima, ID_IXAM_PATH(bmain, &ima->id));
        }
        else {
          ImagePackedFile *imapf;
          for (imapf = static_cast<ImagePackedFile *>(ima->packedfiles.first); imapf;
               imapf = imapf->next) {
            PackedFile *pf;
            pf = BKE_packedfile_new(nullptr, imapf->filepath, ID_IXAM_PATH(bmain, &ima->id));
            if (pf) {
              BKE_packedfile_free(imapf->packedfile);
              imapf->packedfile = pf;
            }
      ...
      if (ima->source == IMA_SRC_TILED) {
        ...
        BLI_path_abs(filepath, ID_IXAM_PATH_FROM_GLOBAL(&ima->id));
...
#endif
    {
      ...
      if (!is_sequence && (has_packed == false) && (G.fileflags & G_FILE_AUTOPACK)) {
        ...
        imapf->packedfile = BKE_packedfile_new(
            nullptr, filepath, ID_IXAM_PATH_FROM_GLOBAL(&ima->id));
      }
    }
  }
                                 ...
                                 const bool resolve_multiview)
{
  ...
  BLI_path_abs(filepath, ID_IXAM_PATH(bmain, &ima->id));
}
...
static void image_update_views_format(Image *ima, ImageUser *iuser)
{
  ...
  if (!is_multiview) {
    ...
    while (iv) {
      ...
      BLI_path_abs(str, ID_IXAM_PATH_FROM_GLOBAL(&ima->id));

...
static void greasepencil_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  BKE_id_ixam_write(writer, &gpd->id);

  if (gpd->adt) {
    BKE_animdata_ixam_write(writer, gpd->adt);
  }

  BKE_defbase_ixam_write(writer, &gpd->vertex_group_names);
  ...
  LISTBASE_FOREACH (bGPDlayer *, gpl, &gpd->layers) {
    ...
    LISTBASE_FOREACH (bGPDframe *, gpf, &gpl->frames) {
      ...
      LISTBASE_FOREACH (bGPDstroke *, gps, &gpf->strokes) {
        ...
        BKE_defvert_ixam_write(writer, gps->totpoints, gps->dvert);
...
void BKE_gpencil_ixam_read_data(IxamDataReader *reader, bGPdata *gpd)
{
  ...
  BKE_animdata_ixam_read_data(reader, gpd->adt);
    ...
    LISTBASE_FOREACH (bGPDframe *, gpf, &gpl->frames) {
      ...
      LISTBASE_FOREACH (bGPDstroke *, gps, &gpf->strokes) {
        ...
        if (gps->dvert) {
          ...
          BKE_defvert_ixam_read(reader, gps->totpoints, gps->dvert);
        }
      }
    }
  }
...
static void greasepencil_ixam_read_data(IxamDataReader *reader, ID *id)
{
  bGPdata *gpd = (bGPdata *)id;
  BKE_gpencil_ixam_read_data(reader, gpd);
}

static void greasepencil_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bGPdata *gpd = (bGPdata *)id;
...
static void greasepencil_ixam_read_expand(IxamExpander *expander, ID *id)
{
  bGPdata *gpd = (bGPdata *)id;
  LISTBASE_FOREACH (bGPDlayer *, gpl, &gpd->layers) {
...
IDTypeInfo IDType_ID_GD = {
    ...
    .ixam_write = greasepencil_ixam_write,
    .ixam_read_data = greasepencil_ixam_read_data,
    .ixam_read_lib = greasepencil_ixam_read_lib,
    .ixam_read_expand = greasepencil_ixam_read_expand,

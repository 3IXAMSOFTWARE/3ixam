...
static struct {
  ...
  /** Volatile temporary directory (owned by 3IXAM, removed on exit). */
 ...
 */
static char *ixam_version_decimal(const int version)
{
...
#ifdef WIN32
  BLI_path_join(
      r_path, path_len, caches_root_path, "3ixam", "Cache", SEP_STR, NULL);
#elif defined(__APPLE__)
  BLI_path_join(r_path, path_len, caches_root_path, "3ixam", SEP_STR, NULL);
#else /* __linux__ */
  BLI_path_join(r_path, path_len, caches_root_path, "3ixam", SEP_STR, NULL);
#endif
 ...
 * sub-directory in the parent directory of the 3IXAM executable.
 *
                              ...
                              const bool check_is_dir)
{
   ...
   * we must move the ixam_version dir with contents to Resources.
...
static bool get_path_local(char *targetpath,
                           ...
                           const char *subfolder_name)
{
  const int version = IXAM_VERSION;
...
static bool get_path_group(char *targetpath,
                           size_t targetpath_len,
                           const char *folder_name,
                           const char *subfolder_name)
{
  char relfolder[FILE_MAX];
  const int version = IXAM_VERSION;
  const char *path_base = GHOST_getSystemDir(0, NULL);
  ...
  return test_path(targetpath, targetpath_len, true, path_base, relfolder, NULL);
}
#endif

static bool get_path_temp(char *targetpath,
                          size_t targetpath_len,
                          const char *folder_name,
                          const char *subfolder_name)
{
  ...
  char dir[FILE_MAX] = {0};
  BLI_path_join(dir, FILE_MAX, BKE_tempdir_base(), IXAM_OUTPUT_DIR);
  const char *path_base = dir;
 ...
 * \param subfolder_name: optional name of sub-folder within folder.
 * \param version: 3IXAM version, used to construct a sub-directory name.
 * \param check_is_dir: When false, return true even if the path doesn't exist.
                             ...
                             const bool check_is_dir)
{
  ...
  if (test_env_path(user_path, "IXAM_USER_RESOURCES", check_is_dir)) {
    /* Pass. */
  }
  else {
    ...
    const char *user_base_path = GHOST_getUserDir(version, ixam_version_decimal(version));
    if (user_base_path) {
...
/**
 * Returns the path of a folder within the 3IXAM installation directory.
 *
 * \param targetpath: String to return path.
                               ...
                               const bool check_is_dir)
{
  ...
  if (test_env_path(system_path, "IXAM_SYSTEM_RESOURCES", check_is_dir)) {
    /* Pass. */
  }
  else {
    system_path[0] = '\0';
    const char *system_base_path = GHOST_getSystemDir(version, ixam_version_decimal(version));
    if (system_base_path) {
  ...
  /* Try `$IXAMPATH/folder_name/subfolder_name`, `subfolder_name` may be NULL. */
  return test_path(
...
{
  switch (folder_id) {
    case IXAM_DATAFILES: /* general case */
      if (get_path_environment(path, path_len, subfolder, "IXAM_USER_DATAFILES")) {
        break;
      }
      if (get_path_user(path, path_len, "datafiles", subfolder)) {
        break;
      }
      if (get_path_environment(path, path_len, subfolder, "IXAM_SYSTEM_DATAFILES")) {
        break;
      }
    ...
    case IXAM_USER_CONFIG:
      if (get_path_environment(path, path_len, subfolder, "IXAM_USER_CONFIG")) {
        break;
      }
    ...
    case IXAM_USER_SCRIPTS:
      if (get_path_environment(path, path_len, subfolder, "IXAM_USER_SCRIPTS")) {
        break;
      }
    ...
    case IXAM_SYSTEM_SCRIPTS:
    #ifdef PY_ENCRYPT_SCRIPTS
      if (get_path_temp(path, path_len, "", subfolder)) {
        break;
      }
    #else
      if (get_path_environment(path, path_len, subfolder, "IXAM_SYSTEM_SCRIPTS")) {
        break;
      }
    ...
    case IXAM_SYSTEM_PYTHON:
      if (get_path_environment(path, path_len, subfolder, "IXAM_SYSTEM_PYTHON")) {
        break;
      }
...
#if defined(__APPLE__)
    case IXAM_SYSTEM_GROUP:
      get_path_group(path, path_len, subfolder, NULL);
      break;
#endif
    default:
  ...
  switch (folder_id) {
    case IXAM_USER_DATAFILES:
      if (get_path_environment_ex(
              path, sizeof(path), subfolder, "IXAM_USER_DATAFILES", check_is_dir)) {
        break;
      }
      get_path_local_ex(path, sizeof(path), "datafiles", subfolder, version, check_is_dir);
      break;
    case IXAM_USER_CONFIG:
      if (get_path_environment_ex(
              path, sizeof(path), subfolder, "IXAM_USER_CONFIG", check_is_dir)) {
        break;
      }
    ...
    case IXAM_USER_AUTOSAVE:
      if (get_path_environment_ex(
              path, sizeof(path), subfolder, "IXAM_USER_AUTOSAVE", check_is_dir)) {
        break;
      }
    ...
    case IXAM_USER_SCRIPTS:
      if (get_path_environment_ex(
              path, sizeof(path), subfolder, "IXAM_USER_SCRIPTS", check_is_dir)) {
        break;
      }
...
{
  return BKE_appdir_resource_path_id_with_version(folder_id, check_is_dir, IXAM_VERSION);
}
 ...
 *
 * Access locations of 3IXAM & Python.
 * \{ */
                                      ...
                                      const int version_minor)
{
  ...
  {
    const char *python_bin_dir = BKE_appdir_folder_id(IXAM_SYSTEM_PYTHON, "bin");
    if (python_bin_dir) {
...
{
  /* Test if app template provides a `userpref.ixam`.
   * If not, we will share user preferences with the rest of 3IXAM. */
  if (app_template[0] == '\0') {
    return false;
  }
  ...
  BLI_path_join(userpref_path, sizeof(userpref_path), app_template_path, IXAM_USERPREF_FILE);
...
/**
 * Gets the temp directory when ixam first runs.
                                   ...
                                   const char *tempdir)
{
  ...
  const char *session_name = "3ixam_XXXXXX";

...
static void curve_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Curve *cu = (Curve *)id;
  ...
  BKE_id_ixam_write(writer, &cu->id);

  /* direct data */
  BLO_write_pointer_array(writer, cu->totcol, cu->mat);
  if (cu->adt) {
    BKE_animdata_ixam_write(writer, cu->adt);
  }
  ...
  if (cu->bevel_profile != nullptr) {
    BKE_curveprofile_ixam_write(writer, cu->bevel_profile);
  }
}
...
static void curve_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Curve *cu = (Curve *)id;
  BLO_read_data_address(reader, &cu->adt);
  BKE_animdata_ixam_read_data(reader, cu->adt);
  ...
  if (cu->bevel_profile != nullptr) {
    BKE_curveprofile_ixam_read(reader, cu->bevel_profile);
  }
}

static void curve_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Curve *cu = (Curve *)id;
...
static void curve_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Curve *cu = (Curve *)id;
  for (int a = 0; a < cu->totcol; a++) {
...
IDTypeInfo IDType_ID_CU_LEGACY = {
    ...
    /* ixam_write */ curve_ixam_write,
    /* ixam_read_data */ curve_ixam_read_data,
    /* ixam_read_lib */ curve_ixam_read_lib,
    /* ixam_read_expand */ curve_ixam_read_expand,
...
Nurb *BKE_nurb_duplicate(const Nurb *nu)
{
  ...
  *newnu = ixam::dna::shallow_copy(*nu);
...
Nurb *BKE_nurb_copy(Nurb *src, int pntsu, int pntsv)
{
  ...
  *newnu = ixam::dna::shallow_copy(*src);

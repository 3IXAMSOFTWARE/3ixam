...
static void world_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  World *wrld = (World *)id;
  ...
  BKE_id_ixam_write(writer, &wrld->id);

  if (wrld->adt) {
    BKE_animdata_ixam_write(writer, wrld->adt);
  }

  /* nodetree is integral part of world, no libdata */
  if (wrld->nodetree) {
    BLO_write_struct(writer, bNodeTree, wrld->nodetree);
    ntreeIxamWrite(writer, wrld->nodetree);
  }

  BKE_previewimg_ixam_write(writer, wrld->preview);
...
static void world_ixam_read_data(IxamDataReader *reader, ID *id)
{
  World *wrld = (World *)id;
  BLO_read_data_address(reader, &wrld->adt);
  BKE_animdata_ixam_read_data(reader, wrld->adt);

  BLO_read_data_address(reader, &wrld->preview);
  BKE_previewimg_ixam_read(reader, wrld->preview);
  BLI_listbase_clear(&wrld->gpumaterial);

  BLO_read_data_address(reader, &wrld->lightgroup);
}

static void world_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  World *wrld = (World *)id;
  BLO_read_id_address(reader, wrld->id.lib, &wrld->ipo); /* XXX deprecated, old animation system */
}

static void world_ixam_read_expand(IxamExpander *expander, ID *id)
{
  World *wrld = (World *)id;
...
IDTypeInfo IDType_ID_WO = {
    ...
    .ixam_write = world_ixam_write,
    .ixam_read_data = world_ixam_read_data,
    .ixam_read_lib = world_ixam_read_lib,
    .ixam_read_expand = world_ixam_read_expand,

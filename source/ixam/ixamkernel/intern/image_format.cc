...
void BKE_image_format_ixam_read_data(IxamDataReader *reader, ImageFormatData *imf)
{
  BKE_color_managed_view_settings_ixam_read_data(reader, &imf->view_settings);
}

void BKE_image_format_ixam_write(IxamWriter *writer, ImageFormatData *imf)
{
  BKE_color_managed_view_settings_ixam_write(writer, &imf->view_settings);
}

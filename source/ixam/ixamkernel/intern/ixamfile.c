 ...
 * \ingroup bke
 *
 * High level `.ixam` file read/write,
...
/* -------------------------------------------------------------------- */
/** \name Ixam file IO (High Level)
 * \{ */

static bool ixamfile_or_libraries_versions_atleast(Main *bmain,
...
static void setup_app_userdef(IxamFileData *bfd)
{
  if (bfd->user) {
    /* only here free userdef themes... */
    BKE_ixam_userdef_data_set_and_free(bfd->user);
    bfd->user = NULL;

    /* Security issue: any ixam file could include a USER block.
     *
     * Currently we load prefs from IXAM_STARTUP_FILE and later on load IXAM_USERPREF_FILE,
     * to load the preferences defined in the users home dir.
     *
     * This means we will never accidentally (or maliciously)
     * enable scripts auto-execution by loading a '.ixam' file.
     */
 ...
 *
 * \param bfd: Ixam file data, freed by this function on exit.
 */
                           ...
                           IxamFileReadReport *reports)
{
  ...
  if (mode != LOAD_UI) {
    ...
    /* empty file, we add a scene to make 3IXAM work */
    if (curscene == NULL) {
    ...
    /* BKE_ixam_globals_clear will free G_MAIN, here we can still restore pointers */
  ...
  BKE_ixam_globals_main_replace(bfd->main);
  bmain = G_MAIN;
  bfd->main = NULL;
  ...
  /* NOTE: Check bmain version (i.e. current ixam file version), AND the versions of all the
   * linked libraries. */
  if (mode != LOAD_UNDO && !ixamfile_or_libraries_versions_atleast(bmain, 302, 1)) {
    BKE_lib_override_library_main_proxy_convert(bmain, reports);
  }

  if (mode != LOAD_UNDO && !ixamfile_or_libraries_versions_atleast(bmain, 302, 3)) {
    BKE_lib_override_library_main_hierarchy_root_ensure(bmain);
  }

  bmain->recovered = false;

  /* startup.ixam or recovered startup */
  if (is_startup) {
    bmain->filepath[0] = '\0';
  }
  else if (recover) {
    /* In case of autosave or quit.ixam, use original filepath instead. */
...
static void setup_app_ixam_file_data(bContext *C,
                                      IxamFileData *bfd,
                                      const struct IxamFileReadParams *params,
                                      IxamFileReadReport *reports)
{
...
static void handle_subversion_warning(Main *main, IxamFileReadReport *reports)
{
  if (main->minversionfile > IXAM_FILE_VERSION ||
      (main->minversionfile == IXAM_FILE_VERSION &&
       main->minsubversionfile > IXAM_FILE_SUBVERSION)) {
    BKE_reportf(reports->reports,
                RPT_ERROR,
                "File written by newer 3IXAM binary (%d.%d), expect loss of data!",
                main->minversionfile,
                main->minsubversionfile);
  }
}

void BKE_ixamfile_read_setup_ex(bContext *C,
                                 IxamFileData *bfd,
                                 const struct IxamFileReadParams *params,
                                 IxamFileReadReport *reports,
                                 /* Extra args. */
                                 ...
                                 const char *startup_app_template)
{
  if (startup_update_defaults) {
    if ((params->skip_flags & BLO_READ_SKIP_DATA) == 0) {
      BLO_update_defaults_startup_ixam(bfd->main, startup_app_template);
    }
  }
  setup_app_ixam_file_data(C, bfd, params, reports);
  BLO_ixamfiledata_free(bfd);
}

void BKE_ixamfile_read_setup(bContext *C,
                              IxamFileData *bfd,
                              const struct IxamFileReadParams *params,
                              IxamFileReadReport *reports)
{
  BKE_ixamfile_read_setup_ex(C, bfd, params, reports, false, NULL);
}
                                         ...
                                         IxamFileReadReport *reports)
{
  ...
  if (params->is_startup == false) {
    printf("Read ixam file: %s\n", filepath);
  }

  IxamFileData *bfd = BLO_read_from_file(filepath, params->skip_flags, reports);
  if (bfd) {
...
struct IxamFileData *BKE_ixamfile_read_from_memory(const void *filebuf,
                                                     ...
                                                     ReportList *reports)
{
  IxamFileData *bfd = BLO_read_from_memory(filebuf, filelength, params->skip_flags, reports);
  if (bfd) {
    /* Pass. */
  }
  else {
...
struct IxamFileData *BKE_ixamfile_read_from_memfile(Main *bmain,
                                                      struct MemFile *memfile,
                                                      const struct IxamFileReadParams *params,
                                                      ReportList *reports)
{
  IxamFileData *bfd = BLO_read_from_memfile(
      bmain, BKE_main_ixamfile_path(bmain), memfile, params, reports);
  if (bfd) {
...
void BKE_ixamfile_read_make_empty(bContext *C)
{
 ...
 *
 * \note The function #BKE_ixam_userdef_app_template_data_swap determines which settings
 * the app-template overrides.
 * \{ */

UserDef *BKE_ixamfile_userdef_read(const char *filepath, ReportList *reports)
{
  IxamFileData *bfd;
  ...
  bfd = BLO_read_from_file(filepath,
                           ...
                           &(struct IxamFileReadReport){.reports = reports});
  if (bfd) {
    if (bfd->user) {
      userdef = bfd->user;
    }
...
UserDef *BKE_ixamfile_userdef_read_from_memory(const void *filebuf,
                                                int filelength,
                                                ReportList *reports)
{
  IxamFileData *bfd;
...
UserDef *BKE_ixamfile_userdef_from_defaults(void)
{
   ...
   * otherwise we'd need to patch the binary blob - startup.ixam.c */
...
bool BKE_ixamfile_userdef_write(const char *filepath, ReportList *reports)
{
...
bool BKE_ixamfile_userdef_write_app_template(const char *filepath, ReportList *reports)
{
  ...
  UserDef *userdef_default = BLI_exists(filepath) ? BKE_ixamfile_userdef_read(filepath, NULL) :
                                                    NULL;
  if (userdef_default == NULL) {
    userdef_default = BKE_ixamfile_userdef_from_defaults();
  }

  BKE_ixam_userdef_app_template_data_swap(&U, userdef_default);
  bool ok = BKE_ixamfile_userdef_write(filepath, reports);
  BKE_ixam_userdef_app_template_data_swap(&U, userdef_default);
  BKE_ixam_userdef_data_free(userdef_default, false);
  MEM_freeN(userdef_default);
  return ok;
}

bool BKE_ixamfile_userdef_write_all(ReportList *reports)
{
  ...
  if ((cfgdir = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, NULL))) {
    bool ok_write;
    BLI_path_join(filepath, sizeof(filepath), cfgdir, IXAM_USERPREF_FILE);

    printf("Writing userprefs: '%s' ", filepath);
    if (use_template_userpref) {
      ok_write = BKE_ixamfile_userdef_write_app_template(filepath, reports);
    }
    else {
      ok_write = BKE_ixamfile_userdef_write(filepath, reports);
    }
  ...
  if (use_template_userpref) {
    if ((cfgdir = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, U.app_template))) {
      /* Also save app-template prefs */
      BLI_path_join(filepath, sizeof(filepath), cfgdir, IXAM_USERPREF_FILE);

      printf("Writing userprefs app-template: '%s' ", filepath);
      if (BKE_ixamfile_userdef_write(filepath, reports) != 0) {
        printf("ok\n");
      }
      else {
        printf("fail\n");
...
/* -------------------------------------------------------------------- */
/** \name Ixam file IO (WorkSpace)
 * \{ */

WorkspaceConfigFileData *BKE_ixamfile_workspace_config_read(const char *filepath,
                                                             const void *filebuf,
                                                             int filelength,
                                                             ReportList *reports)
{
  ...
  if (filepath) {
    bfd = BLO_read_from_file(
        filepath, BLO_READ_SKIP_USERDEF, &(struct IxamFileReadReport){.reports = reports});
  }
  else {
...
bool BKE_ixamfile_workspace_config_write(Main *bmain, const char *filepath, ReportList *reports)
{
  const int fileflags = G.fileflags & ~G_FILE_NO_UI;
  bool retval = false;

  BKE_ixamfile_write_partial_begin(bmain);

  for (WorkSpace *workspace = bmain->workspaces.first; workspace; workspace = workspace->id.next) {
    BKE_ixamfile_write_partial_tag_ID(&workspace->id, true);
  }

  if (BKE_ixamfile_write_partial(
          bmain, filepath, fileflags, BLO_WRITE_PATH_REMAP_NONE, reports)) {
    retval = true;
  }

  BKE_ixamfile_write_partial_end(bmain);

  return retval;
}

void BKE_ixamfile_workspace_config_data_free(WorkspaceConfigFileData *workspace_config)
{
...
/* -------------------------------------------------------------------- */
/** \name Ixam file Write (Partial)
 * \{ */

void BKE_ixamfile_write_partial_begin(Main *bmain_src)
{
  BKE_main_id_tag_all(bmain_src, LIB_TAG_NEED_EXPAND | LIB_TAG_DOIT, false);
}

void BKE_ixamfile_write_partial_tag_ID(ID *id, bool set)
{
  if (set) {
...
static void ixamfile_write_partial_cb(void *UNUSED(handle), Main *UNUSED(bmain), void *vid)
{
  if (vid) {
    ID *id = vid;
...
bool BKE_ixamfile_write_partial(Main *bmain_src,
                                 const char *filepath,
                                 ...
                                 ReportList *reports)
{
  ...
  BLO_main_expander(ixamfile_write_partial_cb);
...
void BKE_ixamfile_write_partial_end(Main *bmain_src)
{
  BKE_main_id_tag_all(bmain_src, LIB_TAG_NEED_EXPAND | LIB_TAG_DOIT, false);
}

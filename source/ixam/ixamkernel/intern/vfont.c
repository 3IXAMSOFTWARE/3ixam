...
static void vfont_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  VFont *vf = (VFont *)id;
  ...
  BKE_id_ixam_write(writer, &vf->id);

  /* direct data */
  BKE_packedfile_ixam_write(writer, vf->packedfile);
}

static void vfont_ixam_read_data(IxamDataReader *reader, ID *id)
{
  VFont *vf = (VFont *)id;
  vf->data = NULL;
  vf->temp_pf = NULL;
  BKE_packedfile_ixam_read(reader, &vf->packedfile);
}
...
IDTypeInfo IDType_ID_VF = {
    ...
    .ixam_write = vfont_ixam_write,
    .ixam_read_data = vfont_ixam_read_data,
    .ixam_read_lib = NULL,
  ...
  if (!vfont->data) {
    ...
    if (BKE_vfont_is_builtin(vfont)) {
      ...
      if (vfont->packedfile) {
        ...
        pf = BKE_packedfile_new(NULL, vfont->filepath, ID_IXAM_PATH_FROM_GLOBAL(&vfont->id));

        if (vfont->temp_pf == NULL) {
          vfont->temp_pf = BKE_packedfile_new(
              NULL, vfont->filepath, ID_IXAM_PATH_FROM_GLOBAL(&vfont->id));
        }
      }
      if (!pf) {
...
VFont *BKE_vfont_load(Main *bmain, const char *filepath)
{
  ...
  if (STREQ(filepath, FO_BUILTIN_NAME)) {
    ...
    pf = BKE_packedfile_new(NULL, filepath, BKE_main_ixamfile_path(bmain));
  ...
  if (pf) {
    ...
    if (vfd) {
      ...
      if (!STREQ(filename, FO_BUILTIN_NAME)) {
        vfont->temp_pf = BKE_packedfile_new(NULL, filepath, BKE_main_ixamfile_path(bmain));
      }
    }
...
VFont *BKE_vfont_load_exists_ex(struct Main *bmain, const char *filepath, bool *r_exists)
{
  ...
  for (vfont = bmain->fonts.first; vfont; vfont = vfont->id.next) {
    ...
    BLI_path_abs(strtest, ID_IXAM_PATH(bmain, &vfont->id));

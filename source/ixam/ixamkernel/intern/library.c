...
static void library_ixam_read_data(struct IxamDataReader *UNUSED(reader), ID *id)
{
  Library *lib = (Library *)id;
...
IDTypeInfo IDType_ID_LI = {
    ...
    .ixam_write = NULL,
    .ixam_read_data = library_ixam_read_data,
    .ixam_read_lib = NULL,
...
void BKE_library_filepath_set(Main *bmain, Library *lib, const char *filepath)
{
  ...
  if (BLI_path_is_rel(lib->filepath_abs)) {
     ...
     */
    /* Never make paths relative to parent lib - reading code (ixamloader) always set *all*
    ...
    BLI_path_abs(lib->filepath_abs, ixamfile_path);
  }
}

...
typedef struct AutoTrackContext {
  ...
  /* Tracking results which are to be synchronized from the AutoTrack context to the 3IXAM's
...
/* -------------------------------------------------------------------- */
/** \name Conversion of markers between 3IXAM's DNA and Libmv.
 * \{ */
                                      ...
                                      const TaskParallelTLS *__restrict tls)
{
   ...
   * This is how 3IXAM side is currently expecting failed track to be handled. Without this the
 ...
 *
 * Used to copy tracking result to 3IXAM side, while the tracking is still happening in its
 * thread.
 *
 * \{ */

...
using ixam::Vector;
using ixam::bke::InstanceReference;
using ixam::bke::Instances;
...
int GeometryComponent::attribute_domain_size(const eAttrDomain domain) const
{
  ...
  const std::optional<ixam::bke::AttributeAccessor> attributes = this->attributes();
...
std::optional<ixam::bke::AttributeAccessor> GeometryComponent::attributes() const
{
  return std::nullopt;
};
std::optional<ixam::bke::MutableAttributeAccessor> GeometryComponent::attributes_for_write()
{
  return std::nullopt;
}
...
void GeometrySet::keep_only(const ixam::Span<GeometryComponentType> component_types)
{
...
void GeometrySet::keep_only_during_modify(
    const ixam::Span<GeometryComponentType> component_types)
{
...
const ixam::bke::CurvesEditHints *GeometrySet::get_curve_edit_hints_for_read() const
{
...
ixam::bke::CurvesEditHints *GeometrySet::get_curve_edit_hints_for_write()
{
                                             ...
                                             Vector<GeometryComponentType> &r_types)
{
  ...
  const ixam::bke::Instances *instances = geometry_set.get_instances_for_read();
  if (instances == nullptr) {
    return;
  }
  instances->foreach_referenced_geometry([&](const GeometrySet &instance_geometry_set) {
...
ixam::Vector<GeometryComponentType> GeometrySet::gather_component_types(
...
void GeometrySet::modify_geometry_sets(ForeachSubGeometryCallback callback)
{
  ...
  if (geometry_sets.size() == 1) {
    ...
    ixam::threading::parallel_for_each(

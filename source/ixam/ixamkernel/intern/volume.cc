...
static struct VolumeFileCache {
  ...
  struct Entry {
    ...
    ixam::Map<int, openvdb::GridBase::Ptr> simplified_grids;
...
static void volume_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Volume *volume = (Volume *)id;
  ...
  BKE_id_ixam_write(writer, &volume->id);

  /* direct data */
  BLO_write_pointer_array(writer, volume->totcol, volume->mat);
  if (volume->adt) {
    BKE_animdata_ixam_write(writer, volume->adt);
  }

  BKE_packedfile_ixam_write(writer, volume->packedfile);
}

static void volume_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Volume *volume = (Volume *)id;
  BLO_read_data_address(reader, &volume->adt);
  BKE_animdata_ixam_read_data(reader, volume->adt);

  BKE_packedfile_ixam_read(reader, &volume->packedfile);
...
static void volume_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Volume *volume = (Volume *)id;
...
static void volume_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Volume *volume = (Volume *)id;
  for (int a = 0; a < volume->totcol; a++) {
...
IDTypeInfo IDType_ID_VO = {
    ...
    /* ixam_write */ volume_ixam_write,
    /* ixam_read_data */ volume_ixam_read_data,
    /* ixam_read_lib */ volume_ixam_read_lib,
    /* ixam_read_expand */ volume_ixam_read_expand,
...
static void volume_filepath_get(const Main *bmain, const Volume *volume, char r_filepath[FILE_MAX])
{
  ...
  BLI_path_abs(r_filepath, ID_IXAM_PATH(bmain, &volume->id));
...
void BKE_volume_grid_transform_matrix(const VolumeGrid *volume_grid, float mat[4][4])
{
  ...
  /* 3IXAM column-major and OpenVDB right-multiplication conventions match. */

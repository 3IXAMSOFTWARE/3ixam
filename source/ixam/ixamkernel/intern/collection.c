...
void BKE_collection_ixam_write_nolib(IxamWriter *writer, Collection *collection)
{
  BKE_id_ixam_write(writer, &collection->id);

  /* Shared function for collection data-blocks and scene master collection. */
  BKE_previewimg_ixam_write(writer, collection->preview);
...
static void collection_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Collection *collection = (Collection *)id;
  ...
  BKE_collection_ixam_write_nolib(writer, collection);
}

#ifdef USE_COLLECTION_COMPAT_28
void BKE_collection_compat_ixam_read_data(IxamDataReader *reader, SceneCollection *sc)
{
  ...
  LISTBASE_FOREACH (SceneCollection *, nsc, &sc->scene_collections) {
    BKE_collection_compat_ixam_read_data(reader, nsc);
  }
}
#endif

void BKE_collection_ixam_read_data(IxamDataReader *reader, Collection *collection, ID *owner_id)
{
  ...
  BKE_previewimg_ixam_read(reader, collection->preview);
  ...
  if (collection->collection != NULL) {
    BKE_collection_compat_ixam_read_data(reader, collection->collection);
  }

  BLO_read_data_address(reader, &collection->view_layer);
  if (collection->view_layer != NULL) {
    BKE_view_layer_ixam_read_data(reader, collection->view_layer);
  }
#endif
}

static void collection_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Collection *collection = (Collection *)id;
  BKE_collection_ixam_read_data(reader, collection, NULL);
}

static void lib_link_collection_data(IxamLibReader *reader, Library *lib, Collection *collection)
{
...
#ifdef USE_COLLECTION_COMPAT_28
void BKE_collection_compat_ixam_read_lib(IxamLibReader *reader,
                                          Library *lib,
                                          SceneCollection *sc)
{
  ...
  LISTBASE_FOREACH (SceneCollection *, nsc, &sc->scene_collections) {
    BKE_collection_compat_ixam_read_lib(reader, lib, nsc);
  }
}
#endif

void BKE_collection_ixam_read_lib(IxamLibReader *reader, Collection *collection)
{
#ifdef USE_COLLECTION_COMPAT_28
  if (collection->collection) {
    BKE_collection_compat_ixam_read_lib(reader, collection->id.lib, collection->collection);
  }

  if (collection->view_layer) {
    BKE_view_layer_ixam_read_lib(reader, collection->id.lib, collection->view_layer);
  }
#endif
...
static void collection_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Collection *collection = (Collection *)id;
  BKE_collection_ixam_read_lib(reader, collection);
}

#ifdef USE_COLLECTION_COMPAT_28
void BKE_collection_compat_ixam_read_expand(struct IxamExpander *expander,
                                             struct SceneCollection *sc)
{
  ...
  LISTBASE_FOREACH (SceneCollection *, nsc, &sc->scene_collections) {
    BKE_collection_compat_ixam_read_expand(expander, nsc);
  }
}
#endif

void BKE_collection_ixam_read_expand(IxamExpander *expander, Collection *collection)
{
  LISTBASE_FOREACH (CollectionObject *, cob, &collection->gobject) {
...
#ifdef USE_COLLECTION_COMPAT_28
  if (collection->collection != NULL) {
    BKE_collection_compat_ixam_read_expand(expander, collection->collection);
  }
#endif
}

static void collection_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Collection *collection = (Collection *)id;
  BKE_collection_ixam_read_expand(expander, collection);
}
...
IDTypeInfo IDType_ID_GR = {
    ...
    .ixam_write = collection_ixam_write,
    .ixam_read_data = collection_ixam_read_data,
    .ixam_read_lib = collection_ixam_read_lib,
    .ixam_read_expand = collection_ixam_read_expand,
...
void BKE_collection_new_name_get(Collection *collection_parent, char *rname)
{
  ...
  if (!collection_parent) {
    name = BLI_strdup(DATA_("Folder"));
  }
  else if (collection_parent->flag & COLLECTION_IS_MASTER) {
    name = BLI_sprintfN(DATA_("Folder %d"),

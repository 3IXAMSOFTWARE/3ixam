...
/* ***************************** F-Curve Sanity ********************************* */
/* The functions here are used in various parts of 3IXAM, usually after some editing
...
static float fcurve_eval_keyframes_interpolate(FCurve *fcu, BezTriple *bezts, float evaltime)
{
  ...
  if (exact) {
     ...
     * - That keyframe is the start of the segment we need (see action_bug_2.ixam in T39207).
     */
    return bezt->vec[1][1];
  }
...
/* -------------------------------------------------------------------- */
/** \name F-Curve - .ixam file API
 * \{ */

void BKE_fmodifiers_ixam_write(IxamWriter *writer, ListBase *fmodifiers)
{
    ...
    if (fmi && fcm->data) {
      ...
      switch (fcm->type) {
        ...
        case FMODIFIER_TYPE_PYTHON: {
          ...
          IDP_IxamWrite(writer, data->prop);
...
void BKE_fmodifiers_ixam_read_data(IxamDataReader *reader, ListBase *fmodifiers, FCurve *curve)
{
  LISTBASE_FOREACH (FModifier *, fcm, fmodifiers) {
    /* relink general data */
    ...
    switch (fcm->type) {
      ...
      case FMODIFIER_TYPE_PYTHON: {
        ...
        IDP_IxamDataRead(reader, &data->prop);
...
void BKE_fmodifiers_ixam_read_lib(IxamLibReader *reader, ID *id, ListBase *fmodifiers)
{
  LISTBASE_FOREACH (FModifier *, fcm, fmodifiers) {
...
void BKE_fmodifiers_ixam_read_expand(IxamExpander *expander, ListBase *fmodifiers)
{
  LISTBASE_FOREACH (FModifier *, fcm, fmodifiers) {
...
void BKE_fcurve_ixam_write(IxamWriter *writer, ListBase *fcurves)
{
  ...
  LISTBASE_FOREACH (FCurve *, fcu, fcurves) {
    ...
    /* write F-Modifiers */
    BKE_fmodifiers_ixam_write(writer, &fcu->modifiers);
  }
}

void BKE_fcurve_ixam_read_data(IxamDataReader *reader, ListBase *fcurves)
{
  ...
  LISTBASE_FOREACH (FCurve *, fcu, fcurves) {
    ...
    BKE_fmodifiers_ixam_read_data(reader, &fcu->modifiers, fcu);
  }
}

void BKE_fcurve_ixam_read_lib(IxamLibReader *reader, ID *id, ListBase *fcurves)
{
  if (fcurves == NULL) {
    return;
  }
  ...
  LISTBASE_FOREACH (FCurve *, fcu, fcurves) {
    ...
    /* modifiers */
    BKE_fmodifiers_ixam_read_lib(reader, id, &fcu->modifiers);
  }
}

void BKE_fcurve_ixam_read_expand(IxamExpander *expander, ListBase *fcurves)
{
  LISTBASE_FOREACH (FCurve *, fcu, fcurves) {
    ...
    /* F-Curve Modifiers */
    BKE_fmodifiers_ixam_read_expand(expander, &fcu->modifiers);
  }
}

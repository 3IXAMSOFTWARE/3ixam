...
bool BKE_shaderfx_unique_name(ListBase *shaders, ShaderFxData *fx)
{
  if (shaders && fx) {
    ...
    return BLI_uniquename(
        shaders, fx, DATA_(fxi->name), '_', offsetof(ShaderFxData, name), sizeof(fx->name));
  }
  return false;
}
...
void BKE_shaderfx_ixam_write(IxamWriter *writer, ListBase *fxbase)
{
  if (fxbase == NULL) {
    return;
  }
...
void BKE_shaderfx_ixam_read_data(IxamDataReader *reader, ListBase *lb)
{
...
void BKE_shaderfx_ixam_read_lib(IxamLibReader *reader, Object *ob)
{

...
View3D *CTX_wm_view3d_index(const bContext *C, const int index)
{
  ScrArea *area = CTX_wm_area(C);
  if (area && area->spacetype == SPACE_VIEW3D) {
    return BLI_findlink(&area->spacedata, index);
  }
  return NULL;
}

int CTX_wm_view3d_polyform_ui_highlight_flag(const bContext *C)
{
  View3D *v3d = CTX_wm_view3d(C);

  if (v3d) {
    return v3d->polyform_ui_highlight_flag;
  }

  return 0;
}

void CTX_wm_view3d_polyform_ui_highlight_flag_set(const bContext *C, const int value)
{
  View3D *v3d = CTX_wm_view3d(C);

  if (v3d) {
    v3d->polyform_ui_highlight_flag = value;
...
struct SpaceRender *CTX_wm_space_render(const bContext *C)
{
  ScrArea *area = CTX_wm_area(C);
  if (area && area->spacetype == SPACE_RENDER) {
    return area->spacedata.first;
  }
  return NULL;
}
...
struct SpaceMatPro *CTX_wm_space_matpro(const bContext *C)
{
  ScrArea *area = CTX_wm_area(C);
  if (area && area->spacetype == SPACE_MATPRO) {
    return area->spacedata.first;
  }
  return NULL;
}
...
struct SpaceMatLib *CTX_wm_space_matlib(const bContext *C)
{
  ScrArea *area = CTX_wm_area(C);
  if (area && area->spacetype == SPACE_MATLIB) {
    return area->spacedata.first;
  }
  return NULL;
}
...
{
  Main *bmain;
  if (ctx_data_pointer_verify(C, "ixam_data", (void *)&bmain)) {
    return bmain;
  }

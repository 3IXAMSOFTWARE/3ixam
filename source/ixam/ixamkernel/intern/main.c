...
void BKE_main_free(Main *mainvar)
{
  ...
  MEM_SAFE_FREE(mainvar->ixam_thumb);
...
IxamThumbnail *BKE_main_thumbnail_from_imbuf(Main *bmain, ImBuf *img)
{
  IxamThumbnail *data = NULL;

  if (bmain) {
    MEM_SAFE_FREE(bmain->ixam_thumb);
  }

  if (img) {
    const size_t data_size = IXAM_THUMB_MEMSIZE(img->x, img->y);
  ...
  if (bmain) {
    bmain->ixam_thumb = data;
  }
  return data;
}

ImBuf *BKE_main_thumbnail_to_imbuf(Main *bmain, IxamThumbnail *data)
{
  ImBuf *img = NULL;

  if (!data && bmain) {
    data = bmain->ixam_thumb;
  }
...
void BKE_main_thumbnail_create(struct Main *bmain)
{
  ...
  bmain->ixam_thumb = MEM_callocN(IXAM_THUMB_MEMSIZE(IXAM_THUMB_SIZE, IXAM_THUMB_SIZE), __func__);
  bmain->ixam_thumb->width = IXAM_THUMB_SIZE;
  bmain->ixam_thumb->height = IXAM_THUMB_SIZE;
}

const char *BKE_main_ixamfile_path(const Main *bmain)
{
...
{
  return BKE_main_ixamfile_path(G_MAIN);
}

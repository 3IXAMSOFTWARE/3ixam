...
#define REMOVE_POLY_TAG(_mp) \
  ...
  ixam::bke::AttributeWriter<int> material_indices =
      mesh->attributes_for_write().lookup_for_write<int>("material_index");
  ixam::MutableVArraySpan<int> material_indices_span(material_indices.varray);
 ...
 * this function is only used when loading very old ixam files */

...
void BKE_gpencil_modifier_ixam_write(IxamWriter *writer, ListBase *modbase)
{
  if (modbase == NULL) {
    return;
  }
  ...
  LISTBASE_FOREACH (GpencilModifierData *, md, modbase) {
    ...
    if (md->type == eGpencilModifierType_Thick) {
      ...
      if (gpmd->curve_thickness) {
        BKE_curvemapping_ixam_write(writer, gpmd->curve_thickness);
      }
    }
      ...
      if (gpmd->curve_intensity) {
        BKE_curvemapping_ixam_write(writer, gpmd->curve_intensity);
      }
    }
      ...
      if (gpmd->curfalloff) {
        BKE_curvemapping_ixam_write(writer, gpmd->curfalloff);
      }
    }
...
void BKE_gpencil_modifier_ixam_read_data(IxamDataReader *reader, ListBase *lb)
{
  ...
  LISTBASE_FOREACH (GpencilModifierData *, md, lb) {
    ...
    if (md->type == eGpencilModifierType_Lattice) {
      ...
      if (hmd->curfalloff) {
        BKE_curvemapping_ixam_read(reader, hmd->curfalloff);
      }
    }
    else if (md->type == eGpencilModifierType_Noise) {
      NoiseGpencilModifierData *gpmd = (NoiseGpencilModifierData *)md;
      ...
      if (gpmd->curve_intensity) {
        BKE_curvemapping_ixam_read(reader, gpmd->curve_intensity);
      ...
      if (gpmd->curve_thickness) {
        BKE_curvemapping_ixam_read(reader, gpmd->curve_thickness);
...
void BKE_gpencil_modifier_ixam_read_lib(IxamLibReader *reader, Object *ob)
{

...
void BKE_viewer_path_ixam_write(struct IxamWriter *writer, const ViewerPath *viewer_path)
{
...
void BKE_viewer_path_ixam_read_data(struct IxamDataReader *reader, ViewerPath *viewer_path)
{
...
void BKE_viewer_path_ixam_read_lib(IxamLibReader *reader, Library *lib, ViewerPath *viewer_path)
{
  LISTBASE_FOREACH (ViewerPathElem *, elem, &viewer_path->path) {
    switch (ViewerPathElemType(elem->type)) {
      case VIEWER_PATH_ELEM_TYPE_ID: {

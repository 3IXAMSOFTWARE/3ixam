...
static void write_fmaps(IxamWriter *writer, ListBase *fbase)
{
...
static void object_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Object *ob = (Object *)id;
  ...
  BKE_id_ixam_write(writer, &ob->id);

  if (ob->adt) {
    BKE_animdata_ixam_write(writer, ob->adt);
  }
  ...
  BKE_pose_ixam_write(writer, ob->pose, arm);
  write_fmaps(writer, &ob->fmaps);
  BKE_constraint_ixam_write(writer, &ob->constraints);
  animviz_motionpath_ixam_write(writer, ob->mpath);
  ...
  if (ob->soft) {
    ...
    BKE_ptcache_ixam_write(writer, &(ob->soft->shared->ptcaches));
  ...
  BKE_particle_system_ixam_write(writer, &ob->particlesystem);
  BKE_modifier_ixam_write(writer, &ob->id, &ob->modifiers);
  BKE_gpencil_modifier_ixam_write(writer, &ob->greasepencil_modifiers);
  BKE_shaderfx_ixam_write(writer, &ob->shader_fx);

  BLO_write_struct_list(writer, LinkData, &ob->pc_ids);

  BKE_previewimg_ixam_write(writer, ob->preview);
...
static void direct_link_nlastrips(IxamDataReader *reader, ListBase *strips)
{
...
static void object_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Object *ob = (Object *)id;
  ...
  BKE_animdata_ixam_read_data(reader, ob->adt);

  BLO_read_data_address(reader, &ob->pose);
  BKE_pose_ixam_read_data(reader, ob->pose);

  BLO_read_data_address(reader, &ob->mpath);
  if (ob->mpath) {
    animviz_motionpath_ixam_read_data(reader, ob->mpath);
  }
  ...
  BKE_modifier_ixam_read_data(reader, &ob->modifiers, ob);
  BKE_gpencil_modifier_ixam_read_data(reader, &ob->greasepencil_modifiers);
  BKE_shaderfx_ixam_read_data(reader, &ob->shader_fx);
  ...
  BKE_particle_partdeflect_ixam_read_data(reader, ob->pd);
  ...
  if (ob->soft) {
    ...
    if (sb->shared == nullptr) {
      ...
      BKE_ptcache_ixam_read_data(reader, &sb->ptcaches, &sb->pointcache, false);
    }
    else {
      /* link caches */
      BKE_ptcache_ixam_read_data(reader, &sb->shared->ptcaches, &sb->shared->pointcache, false);
    }
  }
  ...
  BKE_particle_system_ixam_read_data(reader, &ob->particlesystem);

  BKE_constraint_ixam_read_data(reader, &ob->constraints);
  ...
  BKE_previewimg_ixam_read(reader, ob->preview);

  BLO_read_data_address(reader, &ob->lightgroup);
}

/* XXX deprecated - old animation system */
static void lib_link_nlastrips(IxamLibReader *reader, ID *id, ListBase *striplist)
{
...
/* XXX deprecated - old animation system */
static void lib_link_constraint_channels(IxamLibReader *reader, ID *id, ListBase *chanbase)
{
...
static void object_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Object *ob = (Object *)id;

  Main *bmain = BLO_read_lib_get_main(reader);
  IxamFileReadReport *reports = BLO_read_lib_reports(reader);
    ...
    if (ob->instance_collection != nullptr) {
      ...
      BLO_reportf_wrap(reports,
                       ...
                       TIP_("Non-Empty object '%s' cannot duplicate collection '%s' "
                            "anymore in 3IXAM 2.80, removed instancing"),
                       ob->id.name + 2,
                       new_id->name + 2);
    }
  ...
  BKE_pose_ixam_read_lib(reader, ob, ob->pose);
  BKE_constraint_ixam_read_lib(reader, &ob->id, &ob->constraints);
  ...
  /* texture field */
  if (ob->pd) {
    BKE_particle_partdeflect_ixam_read_lib(reader, &ob->id, ob->pd);
  }
  ...
  BKE_particle_system_ixam_read_lib(reader, ob, &ob->id, &ob->particlesystem);
  BKE_modifier_ixam_read_lib(reader, ob);
  BKE_gpencil_modifier_ixam_read_lib(reader, ob);
  BKE_shaderfx_ixam_read_lib(reader, ob);
...
/* XXX deprecated - old animation system */
static void expand_constraint_channels(IxamExpander *expander, ListBase *chanbase)
{
  LISTBASE_FOREACH (bConstraintChannel *, chan, chanbase) {
...
static void expand_object_expandModifiers(void *userData,
                                          ...
                                          int /*cb_flag*/)
{
  IxamExpander *expander = (IxamExpander *)userData;
...
static void object_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Object *ob = (Object *)id;
  ...
  BKE_pose_ixam_read_expand(expander, ob->pose);
  BLO_expand(expander, ob->poselib);
  BKE_constraint_ixam_read_expand(expander, &ob->constraints);
...
IDTypeInfo IDType_ID_OB = {
    ...
    /* ixam_write */ object_ixam_write,
    /* ixam_read_data */ object_ixam_read_data,
    /* ixam_read_lib */ object_ixam_read_lib,
    /* ixam_read_expand */ object_ixam_read_expand,
...
{
  *workob = ixam::dna::shallow_zero_initialize();
...
static void ob_parbone(Object *ob, Object *par, float r_mat[4][4])
{
  ...
  if (!pchan || !pchan->bone) {
    CLOG_WARN(
        &LOG, "Parent Joint: '%s' for Object: '%s' doesn't exist", ob->parsubstr, ob->id.name + 2);
    unit_m4(r_mat);
    return;
  }
...
void BKE_object_workob_calc_parent(Depsgraph *depsgraph, Scene *scene, Object *ob, Object *workob)
{
   ...
   * object's local loc/rot/scale instead of after. For example, a "Rotation Copy" constraint would
                             ...
                             const bool use_hidden)
{
  ...
  LISTBASE_FOREACH (DupliObject *, dob, lb) {
    if (((use_hidden == false) && (dob->no_draw != 0)) || dob->ob_data == nullptr) {
      ...
      Object temp_ob = ixam::dna::shallow_copy(*dob->ob);
                                         ...
                                         eObRelationTypes includeFilter)
{
  ...
  LISTBASE_FOREACH (Base *, base, BKE_view_layer_object_bases_get(view_layer)) {
    if (objectSet == OB_SET_ALL) {
      ...
      if (ob->protectflag & OB_LOCK_DELETE) {
        continue;
      }
...
void BKE_object_modifiers_lib_link_common(void *userData,
                                          ...
                                          int cb_flag)
{
  IxamLibReader *reader = (IxamLibReader *)userData;

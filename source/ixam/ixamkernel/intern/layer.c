 ...
 * contain link or override collections, and so this is also called on .ixam
...
static void write_layer_collections(IxamWriter *writer, ListBase *lb)
{
  LISTBASE_FOREACH (LayerCollection *, lc, lb) {
...
void BKE_view_layer_ixam_write(IxamWriter *writer, const Scene *scene, ViewLayer *view_layer)
{
  BKE_view_layer_synced_ensure(scene, view_layer);
  ...
  if (view_layer->id_properties) {
    IDP_IxamWrite(writer, view_layer->id_properties);
  }
...
static void direct_link_layer_collections(IxamDataReader *reader, ListBase *lb, bool master)
{
...
void BKE_view_layer_ixam_read_data(IxamDataReader *reader, ViewLayer *view_layer)
{
  ...
  IDP_IxamDataRead(reader, &view_layer->id_properties);
...
static void lib_link_layer_collection(IxamLibReader *reader,
                                      Library *lib,
                                      LayerCollection *layer_collection,
                                      bool master)
{
...
void BKE_view_layer_ixam_read_lib(IxamLibReader *reader, Library *lib, ViewLayer *view_layer)
{
  LISTBASE_FOREACH (FreestyleModuleConfig *, fmc, &view_layer->freestyle_config.modules) {
  ...
  IDP_IxamReadLib(reader, lib, view_layer->id_properties);
}

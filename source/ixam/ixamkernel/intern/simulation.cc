...
static void simulation_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Simulation *simulation = (Simulation *)id;

  BLO_write_id_struct(writer, Simulation, id_address, &simulation->id);
  BKE_id_ixam_write(writer, &simulation->id);

  if (simulation->adt) {
    BKE_animdata_ixam_write(writer, simulation->adt);
  }

  /* nodetree is integral part of simulation, no libdata */
  if (simulation->nodetree) {
    BLO_write_struct(writer, bNodeTree, simulation->nodetree);
    ntreeIxamWrite(writer, simulation->nodetree);
  }
}

static void simulation_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Simulation *simulation = (Simulation *)id;
  BLO_read_data_address(reader, &simulation->adt);
  BKE_animdata_ixam_read_data(reader, simulation->adt);
}

static void simulation_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Simulation *simulation = (Simulation *)id;
  UNUSED_VARS(simulation, reader);
}

static void simulation_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Simulation *simulation = (Simulation *)id;
...
IDTypeInfo IDType_ID_SIM = {
    ...
    /* ixam_write */ simulation_ixam_write,
    /* ixam_read_data */ simulation_ixam_read_data,
    /* ixam_read_lib */ simulation_ixam_read_lib,
    /* ixam_read_expand */ simulation_ixam_read_expand,

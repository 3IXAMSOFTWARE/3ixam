...
typedef struct FFMpegContext {
  ...
  /* Image frame in 3IXAM's own pixel format, may need conversion to the output pixel format. */
...
static AVFrame *generate_video_frame(FFMpegContext *context, const uint8_t *pixels)
{
  ...
  if (context->img_convert_frame != NULL) {
    ...
    /* The output pixel format is 3IXAM's internal pixel format. */
    rgb_frame = context->current_frame;
  }

  /* Copy the 3IXAM pixels into the FFmpeg datastructure, taking care of endianness and flipping
...
#  endif
  ...
  /* Convert to the output pixel format, if it's different that 3IXAM's internal one. */
  if (context->img_convert_frame != NULL) {
    ...
    FFMpegContext *context, char *string, const RenderData *rd, bool preview, const char *suffix)
{
  ...
  BLI_path_abs(string, BKE_main_ixamfile_path_from_global());
...
void BKE_ffmpeg_image_type_verify(RenderData *rd, const ImageFormatData *imf)
{
  ...
  if (imf->imtype == R_IMF_IMTYPE_FFMPEG) {
    if (rd->ffcodecdata.type <= 0 || rd->ffcodecdata.codec <= 0 ||
      ...
      rd->ffcodecdata.constant_rate_factor = FFM_CRF_HIGH;

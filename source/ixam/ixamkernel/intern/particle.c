...
static void write_boid_state(IxamWriter *writer, BoidState *state)
{
...
static void particle_settings_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ParticleSettings *part = (ParticleSettings *)id;

  /* write LibData */
  BLO_write_id_struct(writer, ParticleSettings, id_address, &part->id);
  BKE_id_ixam_write(writer, &part->id);

  if (part->adt) {
    BKE_animdata_ixam_write(writer, part->adt);
  }
  ...
  if (part->clumpcurve) {
    BKE_curvemapping_ixam_write(writer, part->clumpcurve);
  }
  if (part->roughcurve) {
    BKE_curvemapping_ixam_write(writer, part->roughcurve);
  }
  if (part->twistcurve) {
    BKE_curvemapping_ixam_write(writer, part->twistcurve);
  }
...
void BKE_particle_partdeflect_ixam_read_data(IxamDataReader *UNUSED(reader), PartDeflect *pd)
{
  if (pd) {
    pd->rng = NULL;
  }
...
static void particle_settings_ixam_read_data(IxamDataReader *reader, ID *id)
{
  ParticleSettings *part = (ParticleSettings *)id;
  ...
  BKE_animdata_ixam_read_data(reader, part->adt);
  BKE_particle_partdeflect_ixam_read_data(reader, part->pd);
  BKE_particle_partdeflect_ixam_read_data(reader, part->pd2);

  BLO_read_data_address(reader, &part->clumpcurve);
  if (part->clumpcurve) {
    BKE_curvemapping_ixam_read(reader, part->clumpcurve);
  }
  BLO_read_data_address(reader, &part->roughcurve);
  if (part->roughcurve) {
    BKE_curvemapping_ixam_read(reader, part->roughcurve);
  }
  BLO_read_data_address(reader, &part->twistcurve);
  if (part->twistcurve) {
    BKE_curvemapping_ixam_read(reader, part->twistcurve);
  }
...
void BKE_particle_partdeflect_ixam_read_lib(IxamLibReader *reader, ID *id, PartDeflect *pd)
{
  if (pd && pd->tex) {
...
static void particle_settings_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  ParticleSettings *part = (ParticleSettings *)id;
  ...
  BKE_particle_partdeflect_ixam_read_lib(reader, &part->id, part->pd);
  BKE_particle_partdeflect_ixam_read_lib(reader, &part->id, part->pd2);
...
static void particle_settings_ixam_read_expand(IxamExpander *expander, ID *id)
{
  ParticleSettings *part = (ParticleSettings *)id;
...
IDTypeInfo IDType_ID_PA = {
    ...
    .ixam_write = particle_settings_ixam_write,
    .ixam_read_data = particle_settings_ixam_read_data,
    .ixam_read_lib = particle_settings_ixam_read_lib,
    .ixam_read_expand = particle_settings_ixam_read_expand,
...
void psys_copy_particles(ParticleSystem *psys_dst, ParticleSystem *psys_src)
{
   ...
   * copy-on-write copy and the original data will crash 3IXAM.
...
void BKE_particle_system_ixam_write(IxamWriter *writer, ListBase *particles)
{
  LISTBASE_FOREACH (ParticleSystem *, psys, particles) {
    ...
    BKE_ptcache_ixam_write(writer, &psys->ptcaches);
  }
}

void BKE_particle_system_ixam_read_data(IxamDataReader *reader, ListBase *particles)
{
  ParticleData *pa;
  int a;
  ...
  LISTBASE_FOREACH (ParticleSystem *, psys, particles) {
    ...
    BKE_ptcache_ixam_read_data(reader, &psys->ptcaches, &psys->pointcache, 0);
    if (psys->clmd) {
...
void BKE_particle_system_ixam_read_lib(IxamLibReader *reader,
                                        Object *ob,
                                        ID *id,
                                        ListBase *particles)
{

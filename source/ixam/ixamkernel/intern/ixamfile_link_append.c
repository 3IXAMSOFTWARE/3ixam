 ...
 * \ingroup bke
 *
 * High level `.ixam` file link/append code,
...
#include "BKE_ixamfile_link_append.h"

#include "BLO_readfile.h"
#include "BLO_writefile.h"

static CLG_LogRef LOG = {"bke.ixamfile_link_append"};

/* -------------------------------------------------------------------- */
/** \name Link/append context implementation and public management API.
 * \{ */

typedef struct IxamfileLinkAppendContextItem {
  /** Name of the ID (without the heading two-chars IDcode). */
  char *name;
  /** All libraries (from #IxamfileLinkAppendContext.libraries) to try to load this ID from. */
  ...
  void *userdata;
} IxamfileLinkAppendContextItem;

/* A ixamfile library entry in the `libraries` list of #IxamfileLinkAppendContext. */
typedef struct IxamfileLinkAppendContextLibrary {
  char *path;               /* Absolute .ixam file path. */
  IxamHandle *blo_handle;  /* Ixam file handle, if any. */
  bool blo_handle_is_owned; /* Whether the ixam file handle is owned, or borrowed. */
  /* The ixamfile report associated with the `blo_handle`, if owned. */
  IxamFileReadReport bf_reports;
} IxamfileLinkAppendContextLibrary;

typedef struct IxamfileLinkAppendContext {
  ...
  /** Embedded ixamfile and its size, if needed. */
  const void *ixamfile_mem;
  size_t ixamfile_memsize;

  /** Internal 'private' data */
  MemArena *memarena;
} IxamfileLinkAppendContext;

typedef struct IxamfileLinkAppendContextCallBack {
  IxamfileLinkAppendContext *lapp_context;
  IxamfileLinkAppendContextItem *item;
  ReportList *reports;

} IxamfileLinkAppendContextCallBack;
...
static IxamHandle *link_append_context_library_blohandle_ensure(
    IxamfileLinkAppendContext *lapp_context,
    IxamfileLinkAppendContextLibrary *lib_context,
    ReportList *reports)
{
  if (reports != NULL) {
  ...
  IxamHandle *blo_handle = lib_context->blo_handle;
  if (blo_handle == NULL) {
    if (STREQ(libname, BLO_EMBEDDED_STARTUP_IXAM)) {
      blo_handle = BLO_ixamhandle_from_memory(lapp_context->ixamfile_mem,
                                               (int)lapp_context->ixamfile_memsize,
                                               &lib_context->bf_reports);
    }
    else {
      blo_handle = BLO_ixamhandle_from_file(libname, &lib_context->bf_reports);
    }
...
static void link_append_context_library_blohandle_release(
    IxamfileLinkAppendContext *UNUSED(lapp_context),
    IxamfileLinkAppendContextLibrary *lib_context)
{
  if (lib_context->blo_handle_is_owned && lib_context->blo_handle != NULL) {
    BLO_ixamhandle_close(lib_context->blo_handle);
    lib_context->blo_handle = NULL;
  }
}

IxamfileLinkAppendContext *BKE_ixamfile_link_append_context_new(LibraryLink_Params *params)
{
  MemArena *ma = BLI_memarena_new(BLI_MEMARENA_STD_BUFSIZE, __func__);
  IxamfileLinkAppendContext *lapp_context = BLI_memarena_calloc(ma, sizeof(*lapp_context));
...
void BKE_ixamfile_link_append_context_free(IxamfileLinkAppendContext *lapp_context)
{
  ...
  for (LinkNode *liblink = lapp_context->libraries.list; liblink != NULL;
    ...
    IxamfileLinkAppendContextLibrary *lib_context = liblink->link;
...
void BKE_ixamfile_link_append_context_flag_set(IxamfileLinkAppendContext *lapp_context,
                                                const int flag,
                                                const bool do_set)
{
  if (do_set) {
...
void BKE_ixamfile_link_append_context_embedded_ixamfile_set(
    IxamfileLinkAppendContext *lapp_context, const void *ixamfile_mem, int ixamfile_memsize)
{
  BLI_assert_msg(lapp_context->ixamfile_mem == NULL,
                 "Please explicitly clear reference to an embedded ixam memfile before "
                 "setting a new one");
  lapp_context->ixamfile_mem = ixamfile_mem;
  lapp_context->ixamfile_memsize = (size_t)ixamfile_memsize;
}

void BKE_ixamfile_link_append_context_embedded_ixamfile_clear(
    IxamfileLinkAppendContext *lapp_context)
{
  lapp_context->ixamfile_mem = NULL;
  lapp_context->ixamfile_memsize = 0;
}

void BKE_ixamfile_link_append_context_library_add(IxamfileLinkAppendContext *lapp_context,
                                                   const char *libname,
                                                   IxamHandle *blo_handle)
{
  BLI_assert(lapp_context->items.list == NULL);

  IxamfileLinkAppendContextLibrary *lib_context = BLI_memarena_calloc(lapp_context->memarena,
                                                                       sizeof(*lib_context));
...
IxamfileLinkAppendContextItem *BKE_ixamfile_link_append_context_item_add(
    IxamfileLinkAppendContext *lapp_context,
    const char *idname,
    const short idcode,
    void *userdata)
{
  IxamfileLinkAppendContextItem *item = BLI_memarena_calloc(lapp_context->memarena,
                                                             sizeof(*item));
    ...
    const int library_index)
{
  ...
  IxamfileLinkAppendContextLibrary *lib_context = lib_context_link->link;
  IxamHandle *blo_handle = link_append_context_library_blohandle_ensure(
      lapp_context, lib_context, reports);

  if (blo_handle == NULL) {
    return IXAMFILE_LINK_APPEND_INVALID;
  }
  ...
  while ((id_code = BKE_idtype_idcode_iter_step(&id_code_iter))) {
    ...
    int id_names_num;
    LinkNode *id_names_list = BLO_ixamhandle_get_datablock_names(
    ...
    for (LinkNode *link_next = NULL; id_names_list != NULL; id_names_list = link_next) {
      ...
      IxamfileLinkAppendContextItem *item = BKE_ixamfile_link_append_context_item_add(
          lapp_context, id_name, id_code, NULL);
      BKE_ixamfile_link_append_context_item_library_index_enable(
...
void *BKE_ixamfile_link_append_context_item_userdata_get(
    IxamfileLinkAppendContext *UNUSED(lapp_context), IxamfileLinkAppendContextItem *item)
{
  return item->userdata;
}

ID *BKE_ixamfile_link_append_context_item_newid_get(
    IxamfileLinkAppendContext *UNUSED(lapp_context), IxamfileLinkAppendContextItem *item)
{
  return item->new_id;
}
...
short BKE_ixamfile_link_append_context_item_idcode_get(
    struct IxamfileLinkAppendContext *UNUSED(lapp_context),
...
void BKE_ixamfile_link_append_context_item_foreach(
    ...
    const eIxamfileLinkAppendForeachItemFlag flag,
    void *userdata)
{
  for (LinkNode *itemlink = lapp_context->items.list; itemlink; itemlink = itemlink->next) {
    IxamfileLinkAppendContextItem *item = itemlink->link;

    if ((flag & BKE_IXAMFILE_LINK_APPEND_FOREACH_ITEM_FLAG_DO_DIRECT) == 0 &&
        (item->tag & LINK_APPEND_TAG_INDIRECT) == 0) {
      continue;
    }
    if ((flag & BKE_IXAMFILE_LINK_APPEND_FOREACH_ITEM_FLAG_DO_INDIRECT) == 0 &&
        (item->tag & LINK_APPEND_TAG_INDIRECT) != 0) {
      continue;
    }
...
static ID *loose_data_instantiate_process_check(LooseDataInstantiateContext *instantiate_context,
                                                IxamfileLinkAppendContextItem *item)
{
  IxamfileLinkAppendContext *lapp_context = instantiate_context->lapp_context;
...
static void new_id_to_item_mapping_add(IxamfileLinkAppendContext *lapp_context,
                                       ID *id,
                                       IxamfileLinkAppendContextItem *item)
{
...
static void new_id_to_item_mapping_create(IxamfileLinkAppendContext *lapp_context)
{
...
static int foreach_libblock_link_append_callback(LibraryIDLinkCallbackData *cb_data)
{
  ...
  IxamfileLinkAppendContextCallBack *data = cb_data->user_data;
  ...
  IxamfileLinkAppendContextItem *item = BLI_ghash_lookup(data->lapp_context->new_id_to_item, id);
  if (item == NULL) {
    item = BKE_ixamfile_link_append_context_item_add(
...
static void ixamfile_link_append_proxies_convert(Main *bmain, ReportList *reports)
{
  ...
  if (bf_reports.count.proxies_to_lib_overrides_success != 0 ||
    ...
    BKE_reportf(
        ...
        "Please consider re-saving any library .ixam file with the newest 3IXAM version",
        bf_reports.count.proxies_to_lib_overrides_success,
        bf_reports.count.proxies_to_lib_overrides_failures);
  }
}

void BKE_ixamfile_append(IxamfileLinkAppendContext *lapp_context, ReportList *reports)
{
  ...
  for (itemlink = lapp_context->items.list; itemlink; itemlink = itemlink->next) {
    ...
    if (!ELEM(item->action, LINK_APPEND_ACT_KEEP_LINKED, LINK_APPEND_ACT_REUSE_LOCAL)) {
      IxamfileLinkAppendContextCallBack cb_data = {
  ...
  ixamfile_link_append_proxies_convert(bmain, reports);
}

void BKE_ixamfile_link(IxamfileLinkAppendContext *lapp_context, ReportList *reports)
{
  if (lapp_context->num_items == 0) {
  ...
  for (lib_idx = 0, liblink = lapp_context->libraries.list; liblink;
    ...
    if (blo_handle == NULL) {
       ...
       * Error reports will have been made by BLO_ixamhandle_from_file() */
      continue;
    }
    ...
    //   BKE_reportf(reports,
    //               RPT_WARNING,
    //               "Linking or appending from a very old .ixam file format (%d.%d), no animation "
    //               "conversion will "
    //               "be done! You may want to re-save your lib file with current 3IXAM",
    //               mainl->versionfile,
  ...
  if ((lapp_context->params->flag & FILE_LINK) != 0) {
    ixamfile_link_append_proxies_convert(lapp_context->params->bmain, reports);
  }
}
...
static void ixamfile_library_relocate_remap(Main *bmain,
                                             ID *old_id,
                                             ID *new_id,
                                             ReportList *reports,
...
void BKE_ixamfile_library_relocate(IxamfileLinkAppendContext *lapp_context,
                                    ReportList *reports,
                                    Library *library,
                                    const bool do_reload)
{
  ...
  while (lba_idx--) {
    ...
    for (; id; id = id->next) {
      if (id->lib == library) {
        ...
        item = BKE_ixamfile_link_append_context_item_add(lapp_context, id->name + 2, idcode, id);
  ...
  BKE_ixamfile_link(lapp_context, reports);
  ...
  for (item_idx = 0, itemlink = lapp_context->items.list; itemlink;
    ...
    ixamfile_library_relocate_remap(bmain, old_id, new_id, reports, do_reload, remap_flags);
    if (new_id == NULL) {
      continue;
    }
    /* Usual special code for ShapeKeys snowflakes... */
    ...
    if (old_key != NULL) {
      *old_key_p = NULL;
      ...
      ixamfile_library_relocate_remap(

...
BLI_CPP_TYPE_MAKE(InstanceReference, ixam::bke::InstanceReference, CPPTypeFlags::None)
...
ixam::Span<int> Instances::reference_handles() const
{
  return reference_handles_;
}

ixam::MutableSpan<int> Instances::reference_handles()
{
  return reference_handles_;
}

ixam::MutableSpan<ixam::float4x4> Instances::transforms()
{
  return transforms_;
}
ixam::Span<ixam::float4x4> Instances::transforms() const
{
  return transforms_;
}
...
ixam::Span<InstanceReference> Instances::references() const
{
  return references_;
}
...
static ixam::Array<int> generate_unique_instance_ids(Span<int> original_ids)
{
  using namespace ixam;
...
ixam::Span<int> Instances::almost_unique_ids() const
{

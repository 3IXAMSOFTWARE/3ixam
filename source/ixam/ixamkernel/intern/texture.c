...
static void texture_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Tex *tex = (Tex *)id;

  /* write LibData */
  BLO_write_id_struct(writer, Tex, id_address, &tex->id);
  BKE_id_ixam_write(writer, &tex->id);

  if (tex->adt) {
    BKE_animdata_ixam_write(writer, tex->adt);
  }
  ...
  if (tex->nodetree) {
    ...
    ntreeIxamWrite(writer, tex->nodetree);
  }

  BKE_previewimg_ixam_write(writer, tex->preview);
}

static void texture_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Tex *tex = (Tex *)id;
  BLO_read_data_address(reader, &tex->adt);
  BKE_animdata_ixam_read_data(reader, tex->adt);

  BLO_read_data_address(reader, &tex->coba);

  BLO_read_data_address(reader, &tex->preview);
  BKE_previewimg_ixam_read(reader, tex->preview);

  tex->iuser.scene = NULL;
}

static void texture_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Tex *tex = (Tex *)id;
...
static void texture_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Tex *tex = (Tex *)id;
...
IDTypeInfo IDType_ID_TE = {
    ...
    .ixam_write = texture_ixam_write,
    .ixam_read_data = texture_ixam_read_data,
    .ixam_read_lib = texture_ixam_read_lib,
    .ixam_read_expand = texture_ixam_read_expand,

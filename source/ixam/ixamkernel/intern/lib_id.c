...
{
  const char *bpath_user_data[2] = {BKE_main_ixamfile_path(bmain), lib->filepath_abs};
                            ...
                            const bool set_fake)
{
   ...
   * Try "make all local" in 04_01_H.lighting.ixam from Agent327 without this, e.g. */
...
#ifdef DEBUG_TIME
  printf("Hack: Forcefully rebuild rig object poses: Done.\n");
  TIMEIT_VALUE_PRINT(make_local);
#endif
...
void BKE_id_ixam_write(IxamWriter *writer, ID *id)
{
  if (id->asset_data) {
  ...
  /* ID_WM's id->properties are considered runtime only, and never written in .ixam file. */
  if (id->properties && !ELEM(GS(id->name), ID_WM)) {
    IDP_IxamWrite(writer, id->properties);
  }

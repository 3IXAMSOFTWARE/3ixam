...
static void mesh_init_data(ID *id)
{
  ...
  mesh->runtime = new ixam::bke::MeshRuntime();
...
static void mesh_copy_data(Main *bmain, ID *id_dst, const ID *id_src, const int flag)
{
  ...
  mesh_dst->runtime = new ixam::bke::MeshRuntime();
...
static void mesh_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  if (ID_IS_OVERRIDE_LIBRARY(mesh) && !is_undo) {
    ...
    CustomData_ixam_write_prepare(mesh->vdata, vert_layers, names_to_skip);
    CustomData_ixam_write_prepare(mesh->edata, edge_layers, names_to_skip);
    CustomData_ixam_write_prepare(mesh->ldata, loop_layers, names_to_skip);
    CustomData_ixam_write_prepare(mesh->pdata, poly_layers, names_to_skip);
  }

  BLO_write_id_struct(writer, Mesh, id_address, &mesh->id);
  BKE_id_ixam_write(writer, &mesh->id);

  /* direct data */
  if (mesh->adt) {
    BKE_animdata_ixam_write(writer, mesh->adt);
  }

  BKE_defbase_ixam_write(writer, &mesh->vertex_group_names);

  BLO_write_pointer_array(writer, mesh->totcol, mesh->mat);
  BLO_write_raw(writer, sizeof(MSelect) * mesh->totselect, mesh->mselect);

  CustomData_ixam_write(
  ...
  CustomData_ixam_write(writer, &mesh->fdata, {}, mesh->totface, CD_MASK_MESH.fmask, &mesh->id);
  CustomData_ixam_write(
...
static void mesh_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Mesh *mesh = (Mesh *)id;
  ...
  BKE_animdata_ixam_read_data(reader, mesh->adt);

  BLO_read_list(reader, &mesh->vertex_group_names);

  CustomData_ixam_read(reader, &mesh->vdata, mesh->totvert);
  CustomData_ixam_read(reader, &mesh->edata, mesh->totedge);
  CustomData_ixam_read(reader, &mesh->fdata, mesh->totface);
  CustomData_ixam_read(reader, &mesh->ldata, mesh->totloop);
  CustomData_ixam_read(reader, &mesh->pdata, mesh->totpoly);
  if (mesh->deform_verts().is_empty()) {
    /* Vertex group data was also an owning pointer in old 3IXAM versions.
     * Don't read them again if they were read as part of #CustomData. */
    BKE_defvert_ixam_read(reader, mesh->totvert, mesh->dvert);
  }
...
static void mesh_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Mesh *me = (Mesh *)id;
...
static void mesh_read_expand(IxamExpander *expander, ID *id)
{
  Mesh *me = (Mesh *)id;
...
IDTypeInfo IDType_ID_ME = {
    ...
    /* ixam_write */ mesh_ixam_write,
    /* ixam_read_data */ mesh_ixam_read_data,
    /* ixam_read_lib */ mesh_ixam_read_lib,
    /* ixam_read_expand */ mesh_read_expand,
...
{
  ixam::bke::AttributeAccessor attributes = mesh->attributes();

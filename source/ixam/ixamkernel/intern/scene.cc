...
/* Allow using deprecated functionality for .ixam file I/O. */
...
static void scene_init_data(ID *id)
{
  ...
  BLI_strncpy(scene->r.engine, RE_engine_id_IXAM_WORKBENCH, sizeof(scene->r.engine));
...
static void scene_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Scene *sce = (Scene *)id;
  ...
  BKE_id_ixam_write(writer, &sce->id);

  if (sce->adt) {
    BKE_animdata_ixam_write(writer, sce->adt);
  }
  BKE_keyingsets_ixam_write(writer, &sce->keyingsets);
  ...
  if (tos->vpaint) {
    ...
    BKE_paint_ixam_write(writer, &tos->vpaint->paint);
  }
  if (tos->wpaint) {
    BLO_write_struct(writer, VPaint, tos->wpaint);
    BKE_paint_ixam_write(writer, &tos->wpaint->paint);
  }
  if (tos->sculpt) {
    BLO_write_struct(writer, Sculpt, tos->sculpt);
    if (tos->sculpt->automasking_cavity_curve) {
      BKE_curvemapping_ixam_write(writer, tos->sculpt->automasking_cavity_curve);
    }
    if (tos->sculpt->automasking_cavity_curve_op) {
      BKE_curvemapping_ixam_write(writer, tos->sculpt->automasking_cavity_curve_op);
    }

    BKE_paint_ixam_write(writer, &tos->sculpt->paint);
  }
  if (tos->uvsculpt) {
    BLO_write_struct(writer, UvSculpt, tos->uvsculpt);
    BKE_paint_ixam_write(writer, &tos->uvsculpt->paint);
  }
  if (tos->gp_paint) {
    BLO_write_struct(writer, GpPaint, tos->gp_paint);
    BKE_paint_ixam_write(writer, &tos->gp_paint->paint);
  }
  if (tos->gp_vertexpaint) {
    BLO_write_struct(writer, GpVertexPaint, tos->gp_vertexpaint);
    BKE_paint_ixam_write(writer, &tos->gp_vertexpaint->paint);
  }
  if (tos->gp_sculptpaint) {
    BLO_write_struct(writer, GpSculptPaint, tos->gp_sculptpaint);
    BKE_paint_ixam_write(writer, &tos->gp_sculptpaint->paint);
  }
  if (tos->gp_weightpaint) {
    BLO_write_struct(writer, GpWeightPaint, tos->gp_weightpaint);
    BKE_paint_ixam_write(writer, &tos->gp_weightpaint->paint);
  }
  if (tos->curves_sculpt) {
    BLO_write_struct(writer, CurvesSculpt, tos->curves_sculpt);
    BKE_paint_ixam_write(writer, &tos->curves_sculpt->paint);
  }
  /* write grease-pencil custom ipo curve to file */
  if (tos->gp_interpolate.custom_ipo) {
    BKE_curvemapping_ixam_write(writer, tos->gp_interpolate.custom_ipo);
  }
  /* write grease-pencil multi-frame falloff curve to file */
  if (tos->gp_sculpt.cur_falloff) {
    BKE_curvemapping_ixam_write(writer, tos->gp_sculpt.cur_falloff);
  }
  /* write grease-pencil primitive curve to file */
  if (tos->gp_sculpt.cur_primitive) {
    BKE_curvemapping_ixam_write(writer, tos->gp_sculpt.cur_primitive);
  }
  /* Write the curve profile to the file. */
  if (tos->custom_bevel_profile_preset) {
    BKE_curveprofile_ixam_write(writer, tos->custom_bevel_profile_preset);
  }
  if (tos->sequencer_tool_settings) {
    BLO_write_struct(writer, SequencerToolSettings, tos->sequencer_tool_settings);
  }

  BKE_paint_ixam_write(writer, &tos->imapaint.paint);

  Editing *ed = sce->ed;
  if (ed) {
    BLO_write_struct(writer, Editing, ed);

    SEQ_ixam_write(writer, &ed->seqbase);
  ...
  /* writing dynamic list of TimeMarkers to the ixam file */
  LISTBASE_FOREACH (TimeMarker *, marker, &sce->markers) {
    BLO_write_struct(writer, TimeMarker, marker);

    if (marker->prop != nullptr) {
      IDP_IxamWrite(writer, marker->prop);
    }
  }

  /* writing dynamic list of TransformOrientations to the ixam file */
  LISTBASE_FOREACH (TransformOrientation *, ts, &sce->transform_spaces) {
    BLO_write_struct(writer, TransformOrientation, ts);
  }

  /* writing MultiView to the ixam file */
  ...
  if (sce->nodetree) {
    ...
    ntreeIxamWrite(writer, sce->nodetree);
  }

  BKE_color_managed_view_settings_ixam_write(writer, &sce->view_settings);
  BKE_image_format_ixam_write(writer, &sce->r.im_format);
  BKE_image_format_ixam_write(writer, &sce->r.bake.im_format);

  /* writing RigidBodyWorld data to the ixam file */
  if (sce->rigidbody_world) {
    /* Set deprecated pointers to prevent crashes of older Ixams */
    ...
    BKE_ptcache_ixam_write(writer, &(sce->rigidbody_world->shared->ptcaches));
  }

  BKE_previewimg_ixam_write(writer, sce->preview);
  BKE_curvemapping_curves_ixam_write(writer, &sce->r.mblur_shutter_curve);

  LISTBASE_FOREACH (ViewLayer *, view_layer, &sce->view_layers) {
    BKE_view_layer_ixam_write(writer, sce, view_layer);
  }

  if (sce->master_collection) {
    BLO_write_struct(writer, Collection, sce->master_collection);
    BKE_collection_ixam_write_nolib(writer, sce->master_collection);
  }

  /* Eevee Light-cache */
  if (sce->eevee.light_cache_data && !BLO_write_is_undo(writer)) {
    BLO_write_struct(writer, LightCache, sce->eevee.light_cache_data);
    EEVEE_lightcache_ixam_write(writer, sce->eevee.light_cache_data);
  }

  BKE_screen_view3d_shading_ixam_write(writer, &sce->display.shading);

  /* Freed on `do_versions()`. */
  BLI_assert(sce->layer_properties == nullptr);
}

static void direct_link_paint_helper(IxamDataReader *reader, const Scene *scene, Paint **paint)
{
  /* TODO: is this needed. */
  BLO_read_data_address(reader, paint);

  if (*paint) {
    BKE_paint_ixam_read_data(reader, scene, *paint);
  }
}

static void link_recurs_seq(IxamDataReader *reader, ListBase *lb)
{
...
static void scene_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Scene *sce = (Scene *)id;
  ...
  BKE_animdata_ixam_read_data(reader, sce->adt);

  BLO_read_list(reader, &sce->keyingsets);
  BKE_keyingsets_ixam_read_data(reader, &sce->keyingsets);
  ...
  if (sce->toolsettings) {
    ...
    BKE_paint_ixam_read_data(reader, sce, &sce->toolsettings->imapaint.paint);
    ...
    if (sce->toolsettings->sculpt) {
      ...
      if (sce->toolsettings->sculpt->automasking_cavity_curve) {
        BKE_curvemapping_ixam_read(reader, sce->toolsettings->sculpt->automasking_cavity_curve);
      ...
      if (sce->toolsettings->sculpt->automasking_cavity_curve_op) {
        BKE_curvemapping_ixam_read(reader,
    ...
    if (sce->toolsettings->gp_interpolate.custom_ipo) {
      BKE_curvemapping_ixam_read(reader, sce->toolsettings->gp_interpolate.custom_ipo);
    }
    ...
    if (sce->toolsettings->gp_sculpt.cur_falloff) {
      BKE_curvemapping_ixam_read(reader, sce->toolsettings->gp_sculpt.cur_falloff);
    }
    /* Relink grease pencil primitive curve. */
    BLO_read_data_address(reader, &sce->toolsettings->gp_sculpt.cur_primitive);
    if (sce->toolsettings->gp_sculpt.cur_primitive) {
      BKE_curvemapping_ixam_read(reader, sce->toolsettings->gp_sculpt.cur_primitive);
    }

    /* Relink toolsettings curve profile. */
    BLO_read_data_address(reader, &sce->toolsettings->custom_bevel_profile_preset);
    if (sce->toolsettings->custom_bevel_profile_preset) {
      BKE_curveprofile_ixam_read(reader, sce->toolsettings->custom_bevel_profile_preset);
    }
  ...
  if (sce->ed) {
    ...
    SEQ_ixam_read(reader, &ed->seqbase);
  ...
  LISTBASE_FOREACH (TimeMarker *, marker, &sce->markers) {
    ...
    IDP_IxamDataRead(reader, &marker->prop);
  }
  ...
  LISTBASE_FOREACH (SceneRenderLayer *, srl, &sce->r.layers) {
    ...
    IDP_IxamDataRead(reader, &srl->prop);
    BLO_read_list(reader, &(srl->freestyleConfig.modules));
    BLO_read_list(reader, &(srl->freestyleConfig.linesets));
  }

  BKE_color_managed_view_settings_ixam_read_data(reader, &sce->view_settings);
  BKE_image_format_ixam_read_data(reader, &sce->r.im_format);
  BKE_image_format_ixam_read_data(reader, &sce->r.bake.im_format);
  ...
  if (rbw) {
    ...
    if (rbw->shared == nullptr) {
       ...
       * are always set (for compatibility with older Ixams). We mustn't link
       * the same pointcache twice. */
      BKE_ptcache_ixam_read_data(reader, &rbw->ptcaches, &rbw->pointcache, false);
      ...
      /* link caches */
      BKE_ptcache_ixam_read_data(reader, &rbw->shared->ptcaches, &rbw->shared->pointcache, false);
  ...
  BKE_previewimg_ixam_read(reader, sce->preview);

  BKE_curvemapping_ixam_read(reader, &sce->r.mblur_shutter_curve);

#ifdef USE_COLLECTION_COMPAT_28
  /* this runs before the very first doversion */
  if (sce->collection) {
    BLO_read_data_address(reader, &sce->collection);
    BKE_collection_compat_ixam_read_data(reader, sce->collection);
  }
#endif
  ...
  LISTBASE_FOREACH (ViewLayer *, view_layer, &sce->view_layers) {
    BKE_view_layer_ixam_read_data(reader, view_layer);
  }
  ...
  if (BLO_read_data_is_undo(reader)) {
    ...
    if (sce->eevee.light_cache_data) {
      EEVEE_lightcache_ixam_read_data(reader, sce->eevee.light_cache_data);
    }
  }
  EEVEE_lightcache_info_update(&sce->eevee);

  BKE_screen_view3d_shading_ixam_read_data(reader, &sce->display.shading);

  BLO_read_data_address(reader, &sce->layer_properties);
  IDP_IxamDataRead(reader, &sce->layer_properties);
}
...
static void scene_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Scene *sce = (Scene *)id;

  BKE_keyingsets_ixam_read_lib(reader, &sce->id, &sce->keyingsets);
  ...
  BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->imapaint.paint);
  if (sce->toolsettings->sculpt) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->sculpt->paint);
  }
  if (sce->toolsettings->vpaint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->vpaint->paint);
  }
  if (sce->toolsettings->wpaint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->wpaint->paint);
  }
  if (sce->toolsettings->uvsculpt) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->uvsculpt->paint);
  }
  if (sce->toolsettings->gp_paint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->gp_paint->paint);
  }
  if (sce->toolsettings->gp_vertexpaint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->gp_vertexpaint->paint);
  }
  if (sce->toolsettings->gp_sculptpaint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->gp_sculptpaint->paint);
  }
  if (sce->toolsettings->gp_weightpaint) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->gp_weightpaint->paint);
  }
  if (sce->toolsettings->curves_sculpt) {
    BKE_paint_ixam_read_lib(reader, sce, &sce->toolsettings->curves_sculpt->paint);
  }
  ...
  if (sce->ed) {
    SEQ_ixam_read_lib(reader, sce, &sce->ed->seqbase);
  }

  LISTBASE_FOREACH (TimeMarker *, marker, &sce->markers) {
    IDP_IxamReadLib(reader, sce->id.lib, marker->prop);
...
#ifdef USE_COLLECTION_COMPAT_28
  if (sce->collection) {
    BKE_collection_compat_ixam_read_lib(reader, sce->id.lib, sce->collection);
  }
#endif

  LISTBASE_FOREACH (ViewLayer *, view_layer, &sce->view_layers) {
    BKE_view_layer_ixam_read_lib(reader, sce->id.lib, view_layer);
  }
...
static void scene_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Scene *sce = (Scene *)id;
  ...
  BKE_keyingsets_ixam_read_expand(expander, &sce->keyingsets);
  ...
  LISTBASE_FOREACH (ViewLayer *, view_layer, &sce->view_layers) {
    IDP_IxamReadExpand(expander, view_layer->id_properties);
  ...
  if (sce->ed) {
    SEQ_ixam_read_expand(expander, &sce->ed->seqbase);
  }
  ...
  LISTBASE_FOREACH (TimeMarker *, marker, &sce->markers) {
    IDP_IxamReadExpand(expander, marker->prop);
...
#ifdef USE_COLLECTION_COMPAT_28
  if (sce->collection) {
    BKE_collection_compat_ixam_read_expand(expander, sce->collection);
  }
#endif
...
static void scene_undo_preserve(IxamLibReader *reader, ID *id_new, ID *id_old)
{
...
constexpr IDTypeInfo get_type_info()
{
  ...
  info.ixam_write = scene_ixam_write;
  info.ixam_read_data = scene_ixam_read_data;
  info.ixam_read_lib = scene_ixam_read_lib;
  info.ixam_read_expand = scene_ixam_read_expand;

  info.ixam_read_undo_preserve = scene_undo_preserve;
...
const char *RE_engine_id_IXAM_EEVEE = "IXAM_EEVEE";
const char *RE_engine_id_IXAM_WORKBENCH = "IXAM_WORKBENCH";
...
Scene *BKE_scene_set_name(Main *bmain, const char *name)
{
  ...
  if (sce) {
    ...
    printf("Scene switch for render: '%s' in file: '%s'\n", name, BKE_main_ixamfile_path(bmain));
    return sce;
  }
...
bool BKE_scene_uses_ixam_eevee(const Scene *scene)
{
  return STREQ(scene->r.engine, RE_engine_id_IXAM_EEVEE);
}

bool BKE_scene_uses_ixam_workbench(const Scene *scene)
{
  return STREQ(scene->r.engine, RE_engine_id_IXAM_WORKBENCH);
}

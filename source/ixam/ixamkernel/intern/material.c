...
static void material_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  BKE_id_ixam_write(writer, &ma->id);

  if (ma->adt) {
    BKE_animdata_ixam_write(writer, ma->adt);
  }

  /* nodetree is integral part of material, no libdata */
  if (ma->nodetree) {
    BLO_write_struct(writer, bNodeTree, ma->nodetree);
    ntreeIxamWrite(writer, ma->nodetree);
  }

  BKE_previewimg_ixam_write(writer, ma->preview);
...
static void material_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Material *ma = (Material *)id;
  BLO_read_data_address(reader, &ma->adt);
  BKE_animdata_ixam_read_data(reader, ma->adt);

  ma->texpaintslot = NULL;

  BLO_read_data_address(reader, &ma->preview);
  BKE_previewimg_ixam_read(reader, ma->preview);
...
static void material_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Material *ma = (Material *)id;
...
static void material_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Material *ma = (Material *)id;
...
IDTypeInfo IDType_ID_MA = {
    ...
    .ixam_write = material_ixam_write,
    .ixam_read_data = material_ixam_read_data,
    .ixam_read_lib = material_ixam_read_lib,
    .ixam_read_expand = material_ixam_read_expand,

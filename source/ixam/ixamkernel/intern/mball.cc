...
static void metaball_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  ...
  BKE_id_ixam_write(writer, &mb->id);

  /* direct data */
  BLO_write_pointer_array(writer, mb->totcol, mb->mat);
  if (mb->adt) {
    BKE_animdata_ixam_write(writer, mb->adt);
  }
...
static void metaball_ixam_read_data(IxamDataReader *reader, ID *id)
{
  MetaBall *mb = (MetaBall *)id;
  BLO_read_data_address(reader, &mb->adt);
  BKE_animdata_ixam_read_data(reader, mb->adt);
...
static void metaball_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  MetaBall *mb = (MetaBall *)id;
...
static void metaball_ixam_read_expand(IxamExpander *expander, ID *id)
{
  MetaBall *mb = (MetaBall *)id;
  for (int a = 0; a < mb->totcol; a++) {
...
IDTypeInfo IDType_ID_MB = {
    ...
    /* ixam_write */ metaball_ixam_write,
    /* ixam_read_data */ metaball_ixam_read_data,
    /* ixam_read_lib */ metaball_ixam_read_lib,
    /* ixam_read_expand */ metaball_ixam_read_expand,
...
{
  /* Meta-Ball Basis Notes from 3IXAM-2.5x
   * =======================================
   *
...
bool BKE_mball_is_same_group(const Object *ob1, const Object *ob2)
{
  ...
  BLI_split_name_num(basis1name, &basis1nr, ob1->id.name + 2, '_');
  BLI_split_name_num(basis2name, &basis2nr, ob2->id.name + 2, '_');
...
void BKE_mball_properties_copy(Main *bmain, MetaBall *metaball_src)
{
  ...
  for (Object *ob_src = static_cast<Object *>(bmain->objects.first);
    ...
    BLI_split_name_num(obactive_name, &obactive_nr, ob_src->id.name + 2, '_');
    ...
    for (ob_iter = static_cast<Object *>(ob_src->id.prev); ob_iter != nullptr;
      ...
      BLI_split_name_num(ob_name, &ob_nr, ob_iter->id.name + 2, '_');
...
Object *BKE_mball_basis_find(Scene *scene, Object *object)
{
  ...
  BLI_split_name_num(basisname, &basisnr, object->id.name + 2, '_');
  ...
  LISTBASE_FOREACH (ViewLayer *, view_layer, &scene->view_layers) {
    ...
    LISTBASE_FOREACH (Base *, base, BKE_view_layer_object_bases_get(view_layer)) {
      ...
      if ((ob->type == OB_MBALL) && !(base->flag & BASE_FROM_DUPLI)) {
        if (ob != bob) {
          BLI_split_name_num(obname, &obnr, ob->id.name + 2, '_');
...
void BKE_mball_data_update(Depsgraph *depsgraph, Scene *scene, Object *ob)
{
  ...
  ixam::float3 min(std::numeric_limits<float>::max());
  ixam::float3 max(-std::numeric_limits<float>::max());

...
void BKE_mesh_remesh_reproject_paint_mask(Mesh *target, const Mesh *source)
{
  ...
  ixam::threading::parallel_for(IndexRange(target->totvert), 4096, [&](const IndexRange range) {
...
void BKE_remesh_reproject_sculpt_face_sets(Mesh *target, const Mesh *source)
{
  ...
  ixam::threading::parallel_for(IndexRange(target->totpoly), 2048, [&](const IndexRange range) {
    for (const int i : range) {
      float from_co[3];
      BVHTreeNearest nearest;
      nearest.index = -1;

...
static void curvemap_make_table(const CurveMapping *cumap, CurveMap *cuma)
{
  ...
  /* Rely on 3IXAM interpolation for bezier curves, support extra functionality here as well. */
...
void BKE_curvemapping_ixam_write(IxamWriter *writer, const CurveMapping *cumap)
{
  BLO_write_struct(writer, CurveMapping, cumap);
  BKE_curvemapping_curves_ixam_write(writer, cumap);
}

void BKE_curvemapping_curves_ixam_write(IxamWriter *writer, const CurveMapping *cumap)
{
  for (int a = 0; a < CM_TOT; a++) {
...
void BKE_curvemapping_ixam_read(IxamDataReader *reader, CurveMapping *cumap)
{
...
void BKE_color_managed_view_settings_ixam_write(IxamWriter *writer,
                                                 ColorManagedViewSettings *settings)
{
  if (settings->curve_mapping) {
    BKE_curvemapping_ixam_write(writer, settings->curve_mapping);
  }
}

void BKE_color_managed_view_settings_ixam_read_data(IxamDataReader *reader,
                                                     ColorManagedViewSettings *settings)
{
  BLO_read_data_address(reader, &settings->curve_mapping);

  if (settings->curve_mapping) {
    BKE_curvemapping_ixam_read(reader, settings->curve_mapping);
  }
}

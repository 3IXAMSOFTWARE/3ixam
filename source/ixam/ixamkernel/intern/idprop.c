...
void IDP_WriteProperty_OnlyData(const IDProperty *prop, IxamWriter *writer);

static void write_ui_data(const IDProperty *prop, IxamWriter *writer)
{
...
static void IDP_WriteArray(const IDProperty *prop, IxamWriter *writer)
{
  ...
  if (prop->data.pointer) {
    ...
    if (prop->subtype == IDP_GROUP) {
      ...
      for (a = 0; a < prop->len; a++) {
        IDP_IxamWrite(writer, array[a]);
      }
    }
  }
}

static void IDP_WriteIDPArray(const IDProperty *prop, IxamWriter *writer)
{
  /* Remember to set #IDProperty.totallen to len in the linking code! */
  if (prop->data.pointer) {
...
static void IDP_WriteString(const IDProperty *prop, IxamWriter *writer)
{
  /* Remember to set #IDProperty.totallen to len in the linking code! */
  BLO_write_raw(writer, (size_t)prop->len, prop->data.pointer);
}

static void IDP_WriteGroup(const IDProperty *prop, IxamWriter *writer)
{
  LISTBASE_FOREACH (IDProperty *, loop, &prop->data.group) {
    IDP_IxamWrite(writer, loop);
  }
}
...
void IDP_IxamWrite(IxamWriter *writer, const IDProperty *prop)
{
  BLO_write_struct(writer, IDProperty, prop);
  IDP_WriteProperty_OnlyData(prop, writer);
}

static void IDP_DirectLinkProperty(IDProperty *prop, IxamDataReader *reader);

static void read_ui_data(IDProperty *prop, IxamDataReader *reader)
{
...
static void IDP_DirectLinkIDPArray(IDProperty *prop, IxamDataReader *reader)
{
...
static void IDP_DirectLinkArray(IDProperty *prop, IxamDataReader *reader)
{
  /* since we didn't save the extra buffer, set totallen to len */
  prop->totallen = prop->len;
...
static void IDP_DirectLinkString(IDProperty *prop, IxamDataReader *reader)
{
...
static void IDP_DirectLinkGroup(IDProperty *prop, IxamDataReader *reader)
{
...
void IDP_IxamReadData_impl(IxamDataReader *reader, IDProperty **prop, const char *caller_func_id)
{
  if (*prop) {
    if ((*prop)->type == IDP_GROUP) {
      IDP_DirectLinkGroup(*prop, reader);
...
void IDP_IxamReadLib(IxamLibReader *reader, Library *lib, IDProperty *prop)
{
  if (!prop) {
    return;
  }
  ...
  switch (prop->type) {
    ...
    case IDP_IDPARRAY: /* CollectionProperty */
    {
      ...
      for (int i = 0; i < prop->len; i++) {
        IDP_IxamReadLib(reader, lib, &(idp_array[i]));
      }
      break;
    }
    ...
    {
      LISTBASE_FOREACH (IDProperty *, loop, &prop->data.group) {
        IDP_IxamReadLib(reader, lib, loop);
      }
      break;
    }
    default:
...
void IDP_IxamReadExpand(struct IxamExpander *expander, IDProperty *prop)
{
  if (!prop) {
    return;
  }
  ...
  switch (prop->type) {
    ...
    case IDP_IDPARRAY: {
      IDProperty *idp_array = IDP_IDPArray(prop);
      for (int i = 0; i < prop->len; i++) {
        IDP_IxamReadExpand(expander, &idp_array[i]);
      }
      break;
    }
    case IDP_GROUP:
      LISTBASE_FOREACH (IDProperty *, loop, &prop->data.group) {
        IDP_IxamReadExpand(expander, loop);
      }
      break;
  }
}

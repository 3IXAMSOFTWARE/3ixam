...
static void action_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  bAction *act = (bAction *)id;

  BLO_write_id_struct(writer, bAction, id_address, &act->id);
  BKE_id_ixam_write(writer, &act->id);

  BKE_fcurve_ixam_write(writer, &act->curves);
  ...
  BKE_previewimg_ixam_write(writer, act->preview);
}

static void action_ixam_read_data(IxamDataReader *reader, ID *id)
{
  bAction *act = (bAction *)id;
  ...
  BKE_fcurve_ixam_read_data(reader, &act->curves);
  ...
  BKE_previewimg_ixam_read(reader, act->preview);
}

static void ixam_read_lib_constraint_channels(IxamLibReader *reader, ID *id, ListBase *chanbase)
{
...
static void action_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  bAction *act = (bAction *)id;
  ...
  LISTBASE_FOREACH (bActionChannel *, chan, &act->chanbase) {
    ...
    ixam_read_lib_constraint_channels(reader, &act->id, &chan->constraintChannels);
  }
  /* >>> XXX deprecated - old animation system */

  BKE_fcurve_ixam_read_lib(reader, &act->id, &act->curves);
...
static void ixam_read_expand_constraint_channels(IxamExpander *expander, ListBase *chanbase)
{
  LISTBASE_FOREACH (bConstraintChannel *, chan, chanbase) {
...
static void action_ixam_read_expand(IxamExpander *expander, ID *id)
{
  bAction *act = (bAction *)id;
  ...
  LISTBASE_FOREACH (bActionChannel *, chan, &act->chanbase) {
    BLO_expand(expander, chan->ipo);
    ixam_read_expand_constraint_channels(expander, &chan->constraintChannels);
  }
  /* --------------------------------------------------- */

  /* F-Curves in Action */
  BKE_fcurve_ixam_read_expand(expander, &act->curves);
...
IDTypeInfo IDType_ID_AC = {
    ...
    .ixam_write = action_ixam_write,
    .ixam_read_data = action_ixam_read_data,
    .ixam_read_lib = action_ixam_read_lib,
    .ixam_read_expand = action_ixam_read_expand,
...
bActionGroup *action_groups_add_new(bAction *act, const char name[])
{
  ...
  BLI_uniquename(
      &act->groups, agrp, DATA_("Group"), '_', offsetof(bActionGroup, name), sizeof(agrp->name));
...
bActionGroup *BKE_pose_add_group(bPose *pose, const char *name)
{
  ...
  BLI_uniquename(&pose->agroups, grp, name, '_', offsetof(bActionGroup, name), sizeof(grp->name));
...
void BKE_pose_ixam_write(IxamWriter *writer, bPose *pose, bArmature *arm)
{
  /* Write each channel */
  if (pose == NULL) {
    return;
  ...
  LISTBASE_FOREACH (bPoseChannel *, chan, &pose->chanbase) {
    ...
    if (chan->prop) {
      IDP_IxamWrite(writer, chan->prop);
    }

    BKE_constraint_ixam_write(writer, &chan->constraints);

    animviz_motionpath_ixam_write(writer, chan->mpath);
...
void BKE_pose_ixam_read_data(IxamDataReader *reader, bPose *pose)
{
  if (!pose) {
    return;
  }
  ...
  LISTBASE_FOREACH (bPoseChannel *, pchan, &pose->chanbase) {
    ...
    BKE_constraint_ixam_read_data(reader, &pchan->constraints);

    BLO_read_data_address(reader, &pchan->prop);
    IDP_IxamDataRead(reader, &pchan->prop);

    BLO_read_data_address(reader, &pchan->mpath);
    if (pchan->mpath) {
      animviz_motionpath_ixam_read_data(reader, pchan->mpath);
    }
...
void BKE_pose_ixam_read_lib(IxamLibReader *reader, Object *ob, bPose *pose)
{
  ...
  LISTBASE_FOREACH (bPoseChannel *, pchan, &pose->chanbase) {
    BKE_constraint_ixam_read_lib(reader, (ID *)ob, &pchan->constraints);

    pchan->bone = BKE_armature_find_bone_name(arm, pchan->name);

    IDP_IxamReadLib(reader, ob->id.lib, pchan->prop);
...
void BKE_pose_ixam_read_expand(IxamExpander *expander, bPose *pose)
{
  if (!pose) {
    return;
  }
  ...
  LISTBASE_FOREACH (bPoseChannel *, chan, &pose->chanbase) {
    BKE_constraint_ixam_read_expand(expander, &chan->constraints);
    IDP_IxamReadExpand(expander, chan->prop);

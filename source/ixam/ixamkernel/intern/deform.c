...
void BKE_object_defgroup_unique_name(bDeformGroup *dg, Object *ob)
{
  ...
  BLI_uniquename_cb(defgroup_unique_check, &data, DATA_("Group"), '_', dg->name, sizeof(dg->name));
}
...
void BKE_defbase_ixam_write(IxamWriter *writer, const ListBase *defbase)
{
...
void BKE_defvert_ixam_write(IxamWriter *writer, int count, const MDeformVert *dvlist)
{
  if (dvlist == NULL) {
    return;
  }
...
void BKE_defvert_ixam_read(IxamDataReader *reader, int count, MDeformVert *mdverts)
{
  if (mdverts == NULL) {
    return;
  }

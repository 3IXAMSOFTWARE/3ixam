...
bool BKE_modifier_unique_name(ListBase *modifiers, ModifierData *md)
{
  if (modifiers && md) {
    ...
    return BLI_uniquename(
        modifiers, md, DATA_(mti->name), '_', offsetof(ModifierData, name), sizeof(md->name));
  }
  return false;
}
...
void BKE_modifier_path_init(char *path, int path_maxlen, const char *name)
{
  ...
  BLI_path_join(path, path_maxlen, ixamfile_path[0] ? "//" : BKE_tempdir_session(), name);
}
...
Mesh *BKE_modifier_get_evaluated_mesh_from_evaluated_object(Object *ob_eval)
{
  ...
  if ((ob_eval->type == OB_MESH) && (ob_eval->mode & OB_MODE_EDIT)) {
    ...
    /* 'em' might not exist yet in some cases, just after loading a .ixam file, see T57878. */
    if (em != nullptr) {
...
void BKE_modifier_ixam_write(IxamWriter *writer, const ID *id_owner, ListBase *modbase)
{
  if (modbase == nullptr) {
    return;
  }
  ...
  LISTBASE_FOREACH (ModifierData *, md, modbase) {
    ...
    /* If the ixamWrite callback is defined, it should handle the whole writing process. */
    if (mti->ixamWrite != nullptr) {
      mti->ixamWrite(writer, id_owner, md);
      continue;
    }
    ...
    if (md->type == eModifierType_Cloth) {
      ...
      BKE_ptcache_ixam_write(writer, &clmd->ptcaches);
    }
      ...
      if (fmd->type & MOD_FLUID_TYPE_DOMAIN) {
        ...
        if (fmd->domain) {
          BKE_ptcache_ixam_write(writer, &(fmd->domain->ptcaches[0]));

          /* create fake pointcache so that old ixam versions can read it */
          ...
          BKE_ptcache_ixam_write(writer, &(fmd->domain->ptcaches[1]));
      ...
      if (pmd->canvas) {
        ...
        LISTBASE_FOREACH (DynamicPaintSurface *, surface, &pmd->canvas->surfaces) {
          BKE_ptcache_ixam_write(writer, &(surface->ptcaches));
 ...
 * function from #BKE_modifier_ixam_read_data().
 */
 ...
 * used.
 */
static ModifierData *modifier_replace_with_fluid(IxamDataReader *reader,
                                                 Object *object,
                                                 ListBase *modifiers,
...
void BKE_modifier_ixam_read_data(IxamDataReader *reader, ListBase *lb, Object *ob)
{
  ...
  LISTBASE_FOREACH (ModifierData *, md, lb) {
    ...
    if (is_allocated) {
      ...
      BKE_ptcache_ixam_read_data(reader, &clmd->ptcaches, &clmd->point_cache, 0);
      ...
      if (pmd->canvas) {
        ...
        if (pmd->canvas->surfaces.first) {
          ...
          LISTBASE_FOREACH (DynamicPaintSurface *, surface, &pmd->canvas->surfaces) {
            ...
            BKE_ptcache_ixam_read_data(reader, &(surface->ptcaches), &(surface->pointcache), 1);
    ...
    if ((mti != nullptr) && (mti->ixamRead != nullptr)) {
      mti->ixamRead(reader, md);
    }
  }
}

void BKE_modifier_ixam_read_lib(IxamLibReader *reader, Object *ob)
{

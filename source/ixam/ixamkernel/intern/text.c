...
static void text_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Text *text = (Text *)id;
  ...
  BKE_id_ixam_write(writer, &text->id);
...
static void text_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Text *text = (Text *)id;
...
IDTypeInfo IDType_ID_TXT = {
    ...
    .ixam_write = text_ixam_write,
    .ixam_read_data = text_ixam_read_data,
    .ixam_read_lib = NULL,
...
bool BKE_text_reload(Text *text)
{
  ...
  BLI_path_abs(filepath_abs, ID_IXAM_PATH_FROM_GLOBAL(&text->id));
...
int BKE_text_file_modified_check(Text *text)
{
  ...
  BLI_path_abs(filepath, ID_IXAM_PATH_FROM_GLOBAL(&text->id));

...
const CatalogFilePath AssetCatalogService::DEFAULT_CATALOG_FILENAME = "ixam_assets.cats.txt";
...
const std::string AssetCatalogDefinitionFile::HEADER =
    "# This is an Asset Catalog Definition file for 3IXAM.\n"
    "#\n"
                                              ...
                                              const AssetCatalogPath &new_catalog_path)
{
  ...
  for (auto &catalog_uptr : catalog_collection_->catalogs_.values()) {
     ...
     * ixam file, and update the catalog simple name stored there. */
  }
...
bool AssetCatalogService::write_to_disk(const CatalogFilePath &ixam_file_path)
{
  if (!write_to_disk_ex(ixam_file_path)) {
    return false;
  }
...
bool AssetCatalogService::write_to_disk_ex(const CatalogFilePath &ixam_file_path)
{
  /* TODO(Sybren): expand to support multiple CDFs. */
  ...
  const CatalogFilePath cdf_path_to_write = find_suitable_cdf_path_for_writing(ixam_file_path);
...
CatalogFilePath AssetCatalogService::find_suitable_cdf_path_for_writing(
    const CatalogFilePath &ixam_file_path)
{
  BLI_assert_msg(!ixam_file_path.empty(),
                 "A non-empty .ixam file path is required to be able to determine where the "
  ...
  const bool asset_lib_root_found = BKE_asset_library_find_suitable_root_path_from_path(
      ixam_file_path.c_str(), suitable_root_path);
  ...
  /* Determine the default CDF path in the same directory of the ixam file. */
  char ixam_dir_path[PATH_MAX];
  BLI_split_dir_part(ixam_file_path.c_str(), ixam_dir_path, sizeof(ixam_dir_path));
  const CatalogFilePath cdf_path_next_to_ixam = asset_definition_default_file_path_from_dir(
      ixam_dir_path);
  return cdf_path_next_to_ixam;
}

...
void BKE_asset_metadata_write(IxamWriter *writer, AssetMetaData *asset_data)
{
  BLO_write_struct(writer, AssetMetaData, asset_data);

  if (asset_data->properties) {
    IDP_IxamWrite(writer, asset_data->properties);
  }
...
void BKE_asset_metadata_read(IxamDataReader *reader, AssetMetaData *asset_data)
{
  ...
  if (asset_data->properties) {
    ...
    IDP_IxamDataRead(reader, &asset_data->properties);
  }

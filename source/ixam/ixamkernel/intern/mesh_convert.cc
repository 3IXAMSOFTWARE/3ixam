      ...
      for (a = 0; a < dl->parts; a++) {
        ...
        for (; b < dl->nr; b++) {
          ...
          if (mloopuv) {
            ...
            /* exception as handled in convertixam.c too */
  ...
  if (edges.first) {
    while (edges.first) {
      ...
      /* --- nurbs --- */
      {
        ...
        nu = MEM_new<Nurb>("MeshNurb", ixam::dna::shallow_zero_initialize());
...
static void curve_to_mesh_eval_ensure(Object &object)
{
  ...
  Object bevel_object = ixam::dna::shallow_zero_initialize();
  if (curve.bevobj != nullptr) {
    bevel_object = ixam::dna::shallow_copy(*curve.bevobj);
  ...
  Object taper_object = ixam::dna::shallow_zero_initialize();
  if (curve.taperobj != nullptr) {
    taper_object = ixam::dna::shallow_copy(*curve.taperobj);
...
static Mesh *mesh_new_from_evaluated_curve_type_object(const Object *evaluated_object)
{
  ...
  if (const Curves *curves = get_evaluated_curves_from_object(evaluated_object)) {
    return ixam::bke::curve_to_wire_mesh(ixam::bke::CurvesGeometry::wrap(curves->geometry));
  }
  return nullptr;
}
                                                   ...
                                                   const bool preserve_origindex)
{
  ...
  Object object_for_eval = ixam::dna::shallow_copy(*object);

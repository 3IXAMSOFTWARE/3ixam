...
static void shapekey_ixam_write(IxamWriter *writer, ID *id, const void *id_address)
{
  Key *key = (Key *)id;
  ...
  BKE_id_ixam_write(writer, &key->id);

  if (key->adt) {
    BKE_animdata_ixam_write(writer, key->adt);
  }
...
static void shapekey_ixam_read_data(IxamDataReader *reader, ID *id)
{
  Key *key = (Key *)id;
  BLO_read_list(reader, &(key->block));

  BLO_read_data_address(reader, &key->adt);
  BKE_animdata_ixam_read_data(reader, key->adt);
...
static void shapekey_ixam_read_lib(IxamLibReader *reader, ID *id)
{
  Key *key = (Key *)id;
...
static void shapekey_ixam_read_expand(IxamExpander *expander, ID *id)
{
  Key *key = (Key *)id;
...
IDTypeInfo IDType_ID_KE = {
    ...
    .ixam_write = shapekey_ixam_write,
    .ixam_read_data = shapekey_ixam_read_data,
    .ixam_read_lib = shapekey_ixam_read_lib,
    .ixam_read_expand = shapekey_ixam_read_expand,
...
KeyBlock *BKE_keyblock_add(Key *key, const char *name)
{
  ...
  BLI_uniquename(&key->block, kb, DATA_("Key"), '_', offsetof(KeyBlock, name), sizeof(kb->name));

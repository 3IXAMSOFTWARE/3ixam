 ...
 * Main is the root of the 'data-base' of a 3IXAM context. All data is put into lists, and all
 * these lists are stored here.
 *
 * \note A 3IXAM file is not much more than a binary dump of these lists. This list of lists is
...
/* 3IXAM thumbnail, as written on file (width, height, and data as char RGBA). */
/* We pack pixel data after that struct. */
typedef struct IxamThumbnail {
  int width, height;
  char rect[0];
} IxamThumbnail;
...
typedef struct Main {
  ...
  /** The file-path of this ixam file, an empty string indicates an unsaved file. */
  char filepath[1024];               /* 1024 = FILE_MAX */
  short versionfile, subversionfile; /* see IXAM_FILE_VERSION, IXAM_FILE_SUBVERSION */
  ...
  /**
   * True if this main is the 'GMAIN' of current 3IXAM.
   *
 ...
 *
 * \note Always generate a non-global Main, use #BKE_ixam_globals_main_replace to put a newly
 ...
 * (as a pair (relative ixam file path, id name)), and a current local ID, if any.
 *
 ...
 * \param library_filepath: the path of a ixam file library (relative to current working one).
...
/**
 * Generates a raw .ixam file thumbnail data from given image.
 *
 * \param bmain: If not NULL, also store generated data in this Main.
 * \param img: ImBuf image to generate thumbnail data from.
 * \return The generated .ixam file raw thumbnail data.
 */
struct IxamThumbnail *BKE_main_thumbnail_from_imbuf(struct Main *bmain, struct ImBuf *img);
/**
 * Generates an image from raw .ixam file thumbnail \a data.
 *
 * \param bmain: Use this bmain->ixam_thumb data if given \a data is NULL.
 * \param data: Raw .ixam file thumbnail data.
 * \return An ImBuf from given data, or NULL if invalid.
 */
struct ImBuf *BKE_main_thumbnail_to_imbuf(struct Main *bmain, struct IxamThumbnail *data);
/**
 ...
 */
const char *BKE_main_ixamfile_path(const struct Main *bmain) ATTR_NONNULL();
/**
 ...
 */
const char *BKE_main_ixamfile_path_from_global(void);
...
/**
 * The size of thumbnails (optionally) stored in the `.ixam` files header.
 *
 * NOTE(@campbellbarton): This is kept small as it's stored uncompressed in the `.ixam` file,
 * where a larger size would increase the size of every `.ixam` file unreasonably.
 * If we wanted to increase the size, we'd want to use compression (JPEG or similar).
 */
#define IXAM_THUMB_SIZE 128

#define IXAM_THUMB_MEMSIZE(_x, _y) \
  (sizeof(IxamThumbnail) + ((size_t)(_x) * (size_t)(_y)) * sizeof(int))
/** Protect against buffer overflow vulnerability & negative sizes. */
#define IXAM_THUMB_MEMSIZE_IS_VALID(_x, _y) \

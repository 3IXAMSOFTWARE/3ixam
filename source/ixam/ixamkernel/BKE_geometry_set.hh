...
class GeometryComponent {
 ...
 public:
   ...
   */
  virtual std::optional<ixam::bke::AttributeAccessor> attributes() const;
  virtual std::optional<ixam::bke::MutableAttributeAccessor> attributes_for_write();
...
struct GeometrySet {
 private:
  using GeometryComponentPtr = ixam::UserCounter<class GeometryComponent>;
 ...
 public:
   ...
   */
  void keep_only(const ixam::Span<GeometryComponentType> component_types);
  /**
   ...
   */
  void keep_only_during_modify(const ixam::Span<GeometryComponentType> component_types);
   ...
   */
  ixam::Vector<const GeometryComponent *> get_components_for_read() const;

  bool compute_boundbox_without_instances(ixam::float3 *r_min, ixam::float3 *r_max) const;
  ...
  using AttributeForeachCallback =
      ixam::FunctionRef<void(const ixam::bke::AttributeIDRef &attribute_id,
                                const ixam::bke::AttributeMetaData &meta_data,
                                const GeometryComponent &component)>;

  void attribute_foreach(ixam::Span<GeometryComponentType> component_types,
                         bool include_instances,
                         AttributeForeachCallback callback) const;

  void gather_attributes_for_propagation(
      ixam::Span<GeometryComponentType> component_types,
      GeometryComponentType dst_component_type,
      bool include_instances,
      ixam::Map<ixam::bke::AttributeIDRef, ixam::bke::AttributeKind> &r_attributes) const;

  ixam::Vector<GeometryComponentType> gather_component_types(bool include_instances,
                                                                bool ignore_empty) const;

  using ForeachSubGeometryCallback = ixam::FunctionRef<void(GeometrySet &geometry_set)>;
   ...
   */
  const ixam::bke::Instances *get_instances_for_read() const;
  /**
   * Returns read-only curve edit hints or null.
   */
  const ixam::bke::CurvesEditHints *get_curve_edit_hints_for_read() const;
   ...
   */
  ixam::bke::Instances *get_instances_for_write();
  /**
   * Returns mutable curve edit hints or null.
   */
  ixam::bke::CurvesEditHints *get_curve_edit_hints_for_write();
   ...
   */
  void replace_instances(ixam::bke::Instances *instances,
                         GeometryOwnershipType ownership = GeometryOwnershipType::Owned);
...
class MeshComponent : public GeometryComponent {
 ...
 public:
  ...
  std::optional<ixam::bke::AttributeAccessor> attributes() const final;
  std::optional<ixam::bke::MutableAttributeAccessor> attributes_for_write() final;
};
...
class InstancesComponent : public GeometryComponent {
 private:
  ixam::bke::Instances *instances_ = nullptr;
  GeometryOwnershipType ownership_ = GeometryOwnershipType::Owned;
 ...
 public:
  ...
  const ixam::bke::Instances *get_for_read() const;
  ixam::bke::Instances *get_for_write();

  void replace(ixam::bke::Instances *instances,
               GeometryOwnershipType ownership = GeometryOwnershipType::Owned);
...
class GeometryComponentEditData final : public GeometryComponent {
 public:
   ...
   */
  std::unique_ptr<ixam::bke::CurvesEditHints> curves_edit_hints_;

...
{
  const char *const languages_path = BKE_appdir_folder_id(IXAM_DATAFILES, "locale");
...
{
#ifdef WITH_INTERNATIONAL
  const char *const messagepath = BKE_appdir_folder_id(IXAM_DATAFILES, "locale");
#endif
...
const char *BLT_lang_get(void)
{
  ...
  return "en_US"; /* Kind of default locale in 3IXAM when no translation enabled. */
#else
  return "";
#endif
}

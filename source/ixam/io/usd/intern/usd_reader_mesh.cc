...
namespace utils {
/* Very similar to #ixam::io::alembic::utils. */
...
/* Returns an existing 3IXAM material that corresponds to the USD material with the given path.
    ...
    const std::map<std::string, std::string> &usd_path_to_mat_name)
{
  if (params.mtl_name_collision_mode == USD_MTL_NAME_COLLISION_MAKE_UNIQUE) {
    /* Check if we've already created the 3IXAM material with a modified name. */
    ...
    if (path_to_name_iter != usd_path_to_mat_name.end()) {
      ...
      /* We can't find the 3IXAM material which was previously created for this USD
                             ...
                             std::map<std::string, std::string> &usd_path_to_mat_name)
{
  ...
  ixam::io::usd::USDMaterialReader mat_reader(params, bmain);
  ...
  for (std::map<pxr::SdfPath, int>::const_iterator it = mat_index_map.begin();
    ...
    if (!assigned_mat) {
      /* 3IXAM material doesn't exist, so create it now. */
      ...
      /* Add the 3IXAM material. */
      assigned_mat = mat_reader.add_material(usd_mat);

      if (!assigned_mat) {
        std::cout << "WARNING: Couldn't create 3IXAM material from USD material " << it->first
                  << std::endl;
        continue;
      }
      ...
      if (params.mtl_name_collision_mode == USD_MTL_NAME_COLLISION_MAKE_UNIQUE) {
        /* Record the name of the 3IXAM material we created for the USD material
         * with the given path. */
...
void USDMeshReader::process_normals_face_varying(Mesh *mesh)
{
  ...
  for (const int i : polys.index_range()) {
    const MPoly &poly = polys[i];
    for (int j = 0; j < poly.totloop; j++) {
      int ixam_index = poly.loopstart + j;
      ...
      lnors[ixam_index][0] = normals_[usd_index][0];
      lnors[ixam_index][1] = normals_[usd_index][1];
      lnors[ixam_index][2] = normals_[usd_index][2];
    }
  }

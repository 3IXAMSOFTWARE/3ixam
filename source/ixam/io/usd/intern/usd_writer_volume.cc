...
std::optional<std::string> USDVolumeWriter::resolve_vdb_file(const Volume *volume) const
{
  ...
  if (volume->filepath[0] == '\0') {
     ...
     * obtained from external .vdb file but rather generated inside of 3IXAM (i.e. by 'Mesh to
    ...
    const std::string &vdb_file_path) const
{
  ...
  /* Following code was written with an assumption that 3IXAM's relative paths start with

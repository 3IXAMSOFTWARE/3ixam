  ...
  for (uint j = 0; j < bindings.getCount(); j++) {
    ...
    for (iter = animcurves.begin(); iter != animcurves.end(); iter++) {
      ...
      if (STREQ("spot_size", anim_type)) {
        /* NOTE: Do NOT convert if imported file was made by ixam <= 2.69.10
         * Reason: old ixam versions stored spot_size in radians (was a bug)
         */
                                            ...
                                            COLLADAFW::Transformation *tm)
{
  ...
  for (it = frames.begin(); it != frames.end(); it++) {
    ...
    if (is_joint) {
       ...
       * where R, iR are bone rest and inverse rest mats in world space (3IXAM bones),

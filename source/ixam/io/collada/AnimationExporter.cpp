...
{
  IxamContext &ixam_context = this->export_settings.get_ixam_context();
...
std::string AnimationExporter::get_collada_name(std::string channel_type) const
{
   ...
   * this map for now.
   */
  static std::map<std::string, std::string> BC_CHANNEL_IXAM_TO_COLLADA = {
      {"rotation", "rotation"},
      ...
      /* Special ixam profile (TODO: make this more elegant). */
      {"ixam/ixam_dist", "ixam/ixam_dist"},
      /* Special ixam profile (TODO: make this more elegant). */
      {"distance", "ixam/ixam_dist"},
  ...
  std::map<std::string, std::string>::iterator name_it = BC_CHANNEL_IXAM_TO_COLLADA.find(
      channel_type);
  if (name_it == BC_CHANNEL_IXAM_TO_COLLADA.end()) {
    return "";
  }

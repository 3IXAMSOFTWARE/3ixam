...
void bc_update_scene(IxamContext &ixam_context, float ctime)
{
  Main *bmain = ixam_context.get_main();
  Scene *scene = ixam_context.get_scene();
  Depsgraph *depsgraph = ixam_context.get_depsgraph();
...
Mesh *bc_get_mesh_copy(IxamContext &ixam_context,
                       Object *ob,
...
#else
    Depsgraph *depsgraph = ixam_context.get_depsgraph();
    Scene *scene_eval = ixam_context.get_evaluated_scene();
    Object *ob_eval = ixam_context.get_evaluated_object(ob);
...
void BoneExtended::set_bone_layers(std::string layerString, std::vector<std::string> &layer_labels)
{
  ...
  while (ss >> layer) {
    ...
    /* 3IXAM uses numbers to specify layers. */
    if (isInteger(layer)) {
    ...
    /* Layer uses labels (not supported by ixam). Map to layer numbers: */
    ...
    if (pos > 31) {
      fprintf(stderr,
              "Too many layers in Import. Layer %s mapped to 3IXAM layer 31\n",
              layer.c_str());
      pos = 31;
    }
...
/**
 * Check if vector exist stored in 3 custom properties (used in 3IXAM <= 2.78)
 */

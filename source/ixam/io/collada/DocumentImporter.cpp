...
void DocumentImporter::finish()
{
  ...
  for (sit = vscenes.begin(); sit != vscenes.end(); sit++) {
    ...
    if (this->import_settings->import_units) {
      ...
      fprintf(stdout, "Collada: Adjusting 3IXAM units to Importset units: %f.\n", unit_factor);
    }
...
std::string DocumentImporter::get_import_version(const COLLADAFW::FileInfo *asset)
{
  ...
  for (size_t i = 0, count = valuePairs.getCount(); i < count; i++) {
    ...
    if (key == AUTORING_TOOL) {
      if (value.compare(0, IXAM.length(), IXAM) == 0) {
        /* Was made with 3IXAM, now get version string */
        std::string v = value.substr(IXAM.length());
  ...
  if (instance_node) {
    ...
    if (source_node) {
      ...
      COLLADABU::Math::Matrix4 bmat4 =
          mat4.transpose(); /* transpose to get ixam row-major order */
      float mat[4][4];
      for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
          mat[i][j] = bmat4[i][j];
...
bool DocumentImporter::writeCamera(const COLLADAFW::Camera *camera)
{
  ...
  switch (camera->getDescriptionType()) {
    ...
    case COLLADAFW::Camera::UNDEFINED:
      /* read nothing, use ixam defaults. */
      break;
  }

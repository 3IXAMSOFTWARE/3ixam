...
DocumentExporter::DocumentExporter(IxamContext &ixam_context,
                                   ExportSettings *export_settings)
    : ixam_context(ixam_context),
      export_settings(BCExportSettings(export_settings, ixam_context))
{
}
...
{
  Scene *sce = ixam_context.get_scene();
  bContext *C = ixam_context.get_context();
  ...
  asset.getContributor().mAuthor = "3IXAM User";
  char version_buf[128];
#ifdef WITH_BUILDINFO
  BLI_snprintf(version_buf,
               sizeof(version_buf),
               "3IXAM %s commit date:%s, commit time:%s, hash:%s",
               BKE_ixam_version_string(),
               build_commit_date,
               build_commit_time,
               build_hash);
#else
  BLI_snprintf(version_buf, sizeof(version_buf), "3IXAM %s", BKE_ixam_version_string());
#endif
  ...
  if (bc_has_object_type(export_set, OB_MESH)) {
    GeometryExporter ge(ixam_context, writer, this->export_settings);
    ge.exportGeom();
  }

  /* <library_controllers> */
  ArmatureExporter arm_exporter(ixam_context, writer, this->export_settings);
  ControllerExporter controller_exporter(ixam_context, writer, this->export_settings);
  ...
  SceneExporter se(ixam_context, writer, &arm_exporter, this->export_settings);

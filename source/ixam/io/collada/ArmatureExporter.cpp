                                     ...
                                     std::vector<Object *> &child_objects)
{
  if (can_export(bone)) {
    ...
    if (this->export_settings.get_use_ixam_profile()) {
      if (!is_export_root(bone)) {
        if (bone->flag & BONE_CONNECTED) {
          node.addExtraTechniqueParameter("ixam", "connect", true);
        }
      }
      std::string layers = BoneExtended::get_bone_layers(bone->layer);
      node.addExtraTechniqueParameter("ixam", "layer", layers);

      bArmature *armature = (bArmature *)ob_arm->data;
      EditBone *ebone = bc_get_edit_bone(armature, bone->name);
      if (ebone && ebone->roll != 0) {
        node.addExtraTechniqueParameter("ixam", "roll", ebone->roll);
      }
      if (bc_is_leaf_bone(bone)) {
        ...
        node.addExtraTechniqueParameter("ixam", "tip_x", tail[0] - head[0]);
        node.addExtraTechniqueParameter("ixam", "tip_y", tail[1] - head[1]);
        node.addExtraTechniqueParameter("ixam", "tip_z", tail[2] - head[2]);
      }
    }
...
void ArmatureExporter::add_bone_transform(Object *ob_arm, Bone *bone, COLLADASW::Node &node)
{
  ...
  if (!has_restmat) {
    ...
    /* Have no restpose matrix stored, try old style <= 3IXAM 2.78 */

...
{
  Scene *scene = ixam_context.get_scene();
...
{
  const Scene *scene = ixam_context.get_scene();
  ViewLayer *view_layer = ixam_context.get_view_layer();
    ...
    /* empty object */
      ...
      if (BLI_listbase_is_empty(&ob->constraints) == false) {
        ...
        while (con) {
          ...
          printf("%s\n", con_name.c_str());
          printf("%s\n\n", con_tag.c_str());
          colladaNode.addExtraTechniqueChildParameter("ixam", con_tag, "type", con->type);
          colladaNode.addExtraTechniqueChildParameter("ixam", con_tag, "enforce", con->enforce);
          colladaNode.addExtraTechniqueChildParameter("ixam", con_tag, "flag", con->flag);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "headtail", con->headtail);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "lin_error", con->lin_error);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "own_space", con->ownspace);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "rot_error", con->rot_error);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "tar_space", con->tarspace);
          colladaNode.addExtraTechniqueChildParameter(
              "ixam", con_tag, "lin_error", con->lin_error);
          ...
          if (BKE_constraint_targets_get(con, &targets)) {
            ...
            for (ct = (bConstraintTarget *)targets.first; ct; ct = ct->next) {
              ...
              colladaNode.addExtraTechniqueChildParameter("ixam", con_tag, "target_id", tar_id);
            }

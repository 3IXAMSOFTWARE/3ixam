                                           ...
                                           int max_chain_length)
{
  ...
  if (dominant_child != nullptr) {
    ...
    if (pebone && !(cebone->flag & BONE_CONNECTED)) {
       ...
       * would result in a zero sized bone and 3IXAM would silently remove the bone.
      ...
      if (len_squared_v3(vec) > MINIMUM_BONE_LENGTH) {
        ...
        if (pbe && pbe->get_chain_length() >= this->import_settings->min_chain_length) {
          ...
          printf("Connect Joint chain: parent (%s --> %s) child)\n", pebone->name, cebone->name);
        }
      }
    }
...
void ArmatureImporter::set_leaf_bone_shapes(Object *ob_arm)
{
  ...
  for (it = leaf_bones.begin(); it != leaf_bones.end(); it++) {
    ...
    if (pchan) {
      ...
      fprintf(stderr, "Cannot find a pose channel for leaf joint %s\n", leaf.name);
    }
  }
}
...
void ArmatureImporter::create_armature_bones(Main *bmain, std::vector<Object *> &arm_objs)
{
  ...
  for (ri = root_joints.begin(); ri != root_joints.end(); ri++) {
    ...
    if (bone) {
      fprintf(stderr,
              "Reuse of child joint [%s] as root joint in same Rig is not supported.\n",
              bone_name);
      continue;
    }
...
Object *ArmatureImporter::create_armature_bones(Main *bmain, SkinInfo &skin)
{
  ...
  if (!shared && !this->joint_parent_map.empty()) {
     ...
     * So when we reimport a 3IXAM collada file, then we have
...
#  ifdef COLLADA_DEBUG
    ...
    fprintf(stderr, "%s cannot be added to rig.\n", get_joint_name(node));
  }
#  endif
}
#endif
  ...
  for (it = skin_by_data_uid.begin(); it != skin_by_data_uid.end(); it++) {
    ...
    if (guid != nullptr) {
      ...
      if (ob) {
        ...
        fprintf(stderr, "Cannot find object to link rig with.\n");
      }
    }
    else {
      fprintf(stderr, "Cannot find geometry to link rig with.\n");
    }

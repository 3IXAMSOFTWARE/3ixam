...
class AbstractHierarchyIteratorTest : public IxamfileLoadingBaseTest {
 protected:
  TestingHierarchyIterator *iterator;

  void SetUp() override
  {
    IxamfileLoadingBaseTest::SetUp();
    iterator = nullptr;
  }
  ...
  {
    iterator_free();
    IxamfileLoadingBaseTest::TearDown();
  }
...
{
  /* Load the test ixam file. */
  if (!ixamfile_load("usd/usd_hierarchy_export_test.ixam")) {
    return;
  }
...
{
  if (!ixamfile_load("alembic/visibility.ixam")) {
    return;
  }
  depsgraph_create(DAG_EVAL_RENDER);
  iterator_create();

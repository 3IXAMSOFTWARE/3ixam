...
void GpencilExporterSVG::create_document_header()
{
  ...
  char txt[128];
  BLI_snprintf(
      txt, sizeof(txt), " Generator: 3IXAM, %s - %s ", SVG_EXPORTER_NAME, SVG_EXPORTER_VERSION);
...
void GpencilExporterSVG::export_gpencil_layers()
{
  ...
  for (ObjectZ &obz : ob_list_) {
    ...
    std::string frametxt = "ixam_frame_" + std::to_string(cfra_);
    ...
    char obtxt[96];
    BLI_snprintf(obtxt, sizeof(obtxt), "ixam_object_%s", ob->id.name + 2);

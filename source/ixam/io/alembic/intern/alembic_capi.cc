...
using namespace ixam::io::alembic;
 ...
 *         3IXAM object). The pointer is the AbcObjectReader that represents
    ...
    AbcObjectReader::ptr_vector &r_assign_as_parent)
{
   ...
   * either part of a 3IXAM object or a 3IXAM object (Empty) themselves.
   */
  size_t children_claiming_this_object = 0;
  size_t num_children = object.getNumChildren();
  ...
  if (!object.getParent()) {
    ...
    /* An xform can either be a 3IXAM Object (if it contains a mesh, for
     * example), but it can also be an Empty. Its correct translation to
     * 3IXAM's data model depends on its children. */

    /* Check whether or not this object is a Maya locator, which is
     * similar to empties used as parent object in 3IXAM. */
    if (has_property(object.getProperties(), "locator")) {
      create_empty = true;
    }
    else {
     ...
     * 3IXAM. Need to figure out exactly how these points are
...
struct ImportJobData {
  ...
  ixam::timeit::TimePoint start_time;
};

static void report_job_duration(const ImportJobData *data)
{
  ixam::timeit::Nanoseconds duration = ixam::timeit::Clock::now() - data->start_time;
  std::cout << "Alembic import of '" << data->filename << "' took ";
  ixam::timeit::print_duration(duration);
  std::cout << '\n';
}
...
static void import_startjob(void *user_data, short *stop, short *do_update, float *progress)
{
  ...
  data->start_time = ixam::timeit::Clock::now();
...
static void import_endjob(void *user_data)
{
  ...
  if (data->was_cancelled) {
    for (AbcObjectReader *reader : data->readers) {
       ...
       * the reader and the creation of the 3IXAM object. */
      if (ob == nullptr) {
        continue;
      }
    ...
    if (data->is_background_job) {
      /* 3IXAM already returned from the import operator, so we need to store our own extra undo
       * step. */
...
void ABC_get_transform(CacheReader *reader, float r_mat_world[4][4], double time, float scale)
{
   ...
   * for 3IXAM. This conversion is done here rather than by 3IXAM due to

...
void CustomPropertiesExporter::write_idparray(const IDProperty *idp_array)
{
   ...
   * 3IXAM should already enforce this, hence it's only used in debug mode. */
...
template<typename ABCPropertyType, typename IxamValueType>
void CustomPropertiesExporter::write_idparray_flattened_typed(const IDProperty *idp_array)
{
  ...
  std::vector<IxamValueType> matrix_values;
  for (size_t row_idx = 0; row_idx < num_rows; ++row_idx) {
    const IxamValueType *row = (IxamValueType *)IDP_Array(&idp_rows[row_idx]);
    for (size_t col_idx = 0; col_idx < idp_rows[row_idx].len; col_idx++) {
      matrix_values.push_back(row[col_idx]);
    }
  }

  set_array_property<ABCPropertyType, IxamValueType>(
...
void CustomPropertiesExporter::set_scalar_property(const StringRef property_name,
                                                   const IxamValueType property_value)
{
  set_array_property<ABCPropertyType, IxamValueType>(property_name, &property_value, 1);
}

template<typename ABCPropertyType, typename IxamValueType>
void CustomPropertiesExporter::set_array_property(const StringRef property_name,
                                                  const IxamValueType *array_values,

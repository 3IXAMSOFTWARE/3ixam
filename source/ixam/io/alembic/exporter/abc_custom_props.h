...
class CustomPropertiesExporter {
 private:
  ...
  /* Mapping from property name in 3IXAM to property in Alembic.
   * Here 3IXAM does the same as other software (Maya, Houdini), and writes
 ...
 private:
  ...
  template<typename ABCPropertyType, typename IxamValueType>
  void set_scalar_property(StringRef property_name, const IxamValueType property_value);

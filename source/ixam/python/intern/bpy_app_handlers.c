 ...
 * functions into (called vi3IXAMs generic BLI_cb api)
 */
...
static PyTypeObject IxamAppCbType;
...
static PyStructSequence_Field app_cb_info_fields[] = {
    ...
    {"load_pre", "on loading a new ixam file (before)"},
    {"load_post", "on loading a new ixam file (after)"},
    {"save_pre", "on saving a ixam file (before)"},
    {"save_post", "on saving a ixam file (after)"},
...
static PyObject *make_app_cb_info(void)
{
  ...
  app_cb_info = PyStructSequence_New(&IxamAppCbType);
  if (app_cb_info == NULL) {
    return NULL;
  }
...
PyObject *BPY_app_handlers_struct(void)
{
  ...
  PyStructSequence_InitType(&IxamAppCbType, &app_cb_info_desc);

  ret = make_app_cb_info();

  /* prevent user from creating new instances */
  IxamAppCbType.tp_init = NULL;
  IxamAppCbType.tp_new = NULL;
  IxamAppCbType.tp_hash = (hashfunc)

 ...
 *
 * This file exposed ixam file library appending/linking to python, typically
...
typedef struct {
  ...
  int flag;
  PyObject *dict;
  /* Borrowed reference to the `bmain`, taken from the RNA instance of #RNA_IxamDataLibraries.
...
PyDoc_STRVAR(
    ...
    "\n"
    "   :arg filepath: The path to a ixam file.\n"
    "   :type filepath: string\n"
    "   :arg link: When False reference to the original file is lost.\n"
    "   :type link: bool\n"
    "   :arg relative: When True the path is stored relative to the open ixam file.\n"
    "   :type relative: bool\n"
...
static PyObject *bpy_lib_load(BPy_PropertyRNA *self, PyObject *args, PyObject *kw)
{
  ...
  BLI_path_abs(ret->abspath, BKE_main_ixamfile_path(bmain));
...
static PyObject *_bpy_names(BPy_Library *self, int blocktype)
{
  ...
  names = BLO_ixamhandle_get_datablock_names(
...
static PyObject *bpy_lib_enter(BPy_Library *self)
{
  ...
  IxamFileReadReport *bf_reports = &self->bf_reports;

  BKE_reports_init(reports, RPT_STORE);
  memset(bf_reports, 0, sizeof(*bf_reports));
  bf_reports->reports = reports;

  self->blo_handle = BLO_ixamhandle_from_file(self->abspath, bf_reports);

  if (self->blo_handle == NULL) {
    if (BPy_reports_to_error(reports, PyExc_IOError, true) != -1) {
      PyErr_Format(PyExc_IOError, "load: %s failed to open ixam file", self->abspath);
    }
    return NULL;
  }
...
static bool bpy_lib_exit_lapp_context_items_cb(IxamfileLinkAppendContext *lapp_context,
                                               ...
                                               void *userdata)
{
  ...
  if (BKE_ixamfile_link_append_context_item_idcode_get(lapp_context, item) != data->idcode) {
    return true;
  }

  const int py_list_index = POINTER_AS_INT(
      BKE_ixamfile_link_append_context_item_userdata_get(lapp_context, item));
  ID *new_id = BKE_ixamfile_link_append_context_item_newid_get(lapp_context, item);
...
static PyObject *bpy_lib_exit(BPy_Library *self, PyObject *UNUSED(args))
{
  ...
  IxamfileLinkAppendContext *lapp_context = BKE_ixamfile_link_append_context_new(
      &liblink_params);
  BKE_ixamfile_link_append_context_library_add(lapp_context, self->abspath, self->blo_handle);
  ...
  while ((idcode = BKE_idtype_idcode_iter_step(&idcode_step))) {
    ...
    BKE_ixamfile_link_append_context_item_foreach(
        lapp_context,
        bpy_lib_exit_lapp_context_items_cb,
        BKE_IXAMFILE_LINK_APPEND_FOREACH_ITEM_FLAG_DO_DIRECT,
        &iter_data);
  }
#endif  // USE_RNA_DATABLOCKS

  BLO_ixamhandle_close(self->blo_handle);

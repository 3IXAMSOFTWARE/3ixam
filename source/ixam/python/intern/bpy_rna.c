 ...
 *
 * This file is the main interface between Python and 3IXAM's data api (RNA),
 * exposing RNA to Python so ixam data can be accessed in a Python like way.
 *
 * The two main types are #BPy_StructRNA and #BPy_PropertyRNA - the base
 * classes for most of the data Python accesses in ixam.
 */
...
/**
 * Special case: `bpy.data.objects["some_id_name", "//some_lib_name.ixam"]`
 * also for:     `bpy.data.objects.get(("some_id_name", "//some_lib_name.ixam"), fallback)`
 *
 * \note
...
PyDoc_STRVAR(pyrna_struct_as_pointer_doc,
             ".. method:: as_pointer()\n"
             "\n"
             "   Returns the memory address which holds a pointer to 3IXAM's internal data\n"
             "\n"
             "   :return: int (memory address).\n"
             "   :rtype: int\n"
             "\n"
             "   .. note:: This is intended only for advanced script writers who need to\n"
             "      pass 3IXAM data to their own C/Python modules.\n");
...
static PyObject *pyrna_srna_Subtype(StructRNA *srna)
{
  ...
  if (srna == NULL) {
    ...
    if (newclass) {
      ...
      /* XXX, adding this back segfaults 3IXAM on load. */
...
PyObject *pyrna_struct_CreatePyObject(PointerRNA *ptr)
{
  ...
  /* 3IXAM's instance owns a reference (to avoid Python freeing it). */
  if (instance) {
    *instance = pyrna;
    Py_INCREF(pyrna);
  }
...
void BPY_rna_exit(void)
{
   ...
   * If they were in fact freed by 3IXAM, printing their names
 ...
 * without having the data-types in 3IXAM. */
PyObject *BPY_rna_doc(void)
{
  PointerRNA ptr;

  /* For now, return the base RNA type rather than a real module. */
  RNA_ixam_rna_pointer_create(&ptr);
...
struct BPy_TypesModule_State {
  /** `RNA_IxamRNA`. */
  PointerRNA ptr;
  /** `RNA_IxamRNA.structs`, exposed as `bpy.types` */
  PropertyRNA *prop;
};
...
PyDoc_STRVAR(bpy_types_module_doc, "Access to internal 3IXAM types");
...
PyObject *BPY_rna_types(void)
{
  ...
  RNA_ixam_rna_pointer_create(&state->ptr);
...
static int pyrna_deferred_register_class_recursive(StructRNA *srna, PyTypeObject *py_class)
{
  ...
  for (i = 0; i < len; i++) {
     ...
     *
     * So only scan base classes which are not subclasses if ixam types.
...
#  endif
       ...
       * None of 3IXAM's default scripts use __init__ but it's nice to call it
 ...
 * you'd expect when changing ownership crashes ixam on exit so I had to comment out
...
PyDoc_STRVAR(pyrna_register_class_doc,
             ...
             "\n"
             "   Register a subclass of a 3IXAM type class.\n"
             "\n"
             "   :arg cls: 3IXAM type class in:\n"
             ...
             "   :type cls: class\n"
             "   :raises ValueError:\n"
             "      if the class is not a subclass of a registerable ixam class.\n"
             "\n"
             "   .. note::\n"
             "\n"
...
PyDoc_STRVAR(pyrna_unregister_class_doc,
             ...
             "\n"
             "   Unload the Python class from ixam.\n"
             "\n"
...
static PyObject *pyrna_unregister_class(PyObject *UNUSED(self), PyObject *py_class)
{
  ...
  if (G.debug & G_DEBUG_PYTHON) {
    ...
    RNA_ixam_rna_pointer_create(&ptr_rna);

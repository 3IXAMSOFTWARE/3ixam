 ...
 *
 * This is exposed via a context manager `bpy.types.IxamData.temp_data(...)`
 * which returns a new `bpy.types.IxamData` that is freed once the context manager exits.
 */
...
PyDoc_STRVAR(bpy_rna_data_context_load_doc,
             ...
             "\n"
             "   A context manager that temporarily creates ixam file data.\n"
             "\n"
             ...
             "\n"
             "   :return: ixam file data which is freed once the context exists.\n"
             "   :rtype: :class:`bpy.types.IxamData`\n");
...
static PyObject *bpy_rna_data_context_enter(BPy_DataContext *self)
{
  ...
  PointerRNA ptr;
  RNA_pointer_create(NULL, &RNA_IxamData, bmain_temp, &ptr);

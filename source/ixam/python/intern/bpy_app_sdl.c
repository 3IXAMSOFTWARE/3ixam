...
static PyTypeObject IxamAppSDLType;

static PyStructSequence_Field app_sdl_info_fields[] = {
    {"supported", ("Boolean, True when 3IXAM is built with SDL support")},
    ...
    {"available",
     ("Boolean, True when SDL is available. This is False when "
      ...
      "3IXAM cannot find the correct library.")},
    {NULL},
};

static PyStructSequence_Desc app_sdl_info_desc = {
    "bpy.app.sdl",                                                          /* name */
    "This module contains information about SDL 3IXAM is linked against", /* doc */
...
static PyObject *make_sdl_info(void)
{
  ...
  sdl_info = PyStructSequence_New(&IxamAppSDLType);
  if (sdl_info == NULL) {
    return NULL;
  }
...
PyObject *BPY_app_sdl_struct(void)
{
  ...
  PyStructSequence_InitType(&IxamAppSDLType, &app_sdl_info_desc);

  ret = make_sdl_info();

  /* prevent user from creating new instances */
  IxamAppSDLType.tp_init = NULL;
  IxamAppSDLType.tp_new = NULL;
  IxamAppSDLType.tp_hash = (hashfunc)

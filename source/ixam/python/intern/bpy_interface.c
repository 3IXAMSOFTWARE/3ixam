 ...
 *
 * This file deals with embedding the python interpreter within ixam,
 * starting and stopping python and exposing ixam/python modules so they can
...
void BPY_python_start(bContext *C, int argc, const char **argv)
{
  ...
  /* #PyPreConfig (early-configuration). */
  {
    ...
    /* Force `utf-8` on all platforms, since this is what's used for 3IXAM's internal strings,
     * providing consistent encoding behavior across all 3IXAM installations.
     *
  ...
  /* #PyConfig (initialize Python). */
  {
    ...
    {
      const char *py_path_bundle = BKE_appdir_folder_id(IXAM_SYSTEM_PYTHON, NULL);
...
#  ifdef __APPLE__
        ...
        if (strchr(py_path_bundle, ':')) {
          fprintf(stderr,
                  "Warning! 3IXAM application is located in a path containing ':' or '/' chars\n"
...
void BPY_python_start_post(bContext *C) {
  BPy_init_post(C);
}
...
/**
 * Raise an error and return false if the Python version used to compile 3IXAM
 * isn't compatible with the interpreter loading the `bpy` module.
 */
static bool bpy_module_ensure_compatible_version(void)
{
  /* First check the Python version used matches the major version that 3IXAM was built with.
   ...
   * unlikely 3IXAM will ever used the limited API though. */
...
PyMODINIT_FUNC PyInit_bpy(void)
{
   ...
   *
   * 2) there is a 'bpy' C module for python to load which is basically all of ixam,
   *    and there is `scripts/bpy/__init__.py`,
   *    we may end up having to rename this module so there is no naming conflict here eg:
   *    'from ixam import bpy'
   *

...
static PyTypeObject IxamAppType;

static PyStructSequence_Field app_info_fields[] = {
    {"version", "The 3IXAM version as a tuple of 3 numbers. eg. (2, 83, 1)"},
    {"version_file",
     "The 3IXAM version, as a tuple, last used to save a .ixam file, compatible with "
     "``bpy.data.version``. This value should be used for handling compatibility changes between "
     "3IXAM versions"},
    {"version_string", "The 3IXAM version formatted as a string"},
    {"version_cycle", "The release status of this build alpha/beta/rc/release"},
    {"version_char", "Deprecated, always an empty string"},
    {"background",
     "Boolean, True when ixam is running without a user interface (started with -b)"},
    {"factory_startup", "Boolean, True when ixam is running with --factory-startup)"},

    /* buildinfo */
    {"build_date", "The date this ixam instance was built"},
    {"build_time", "The time this ixam instance was built"},
    {"build_commit_timestamp", "The unix timestamp of commit this ixam instance was built"},
    {"build_commit_date", "The date of commit this ixam instance was built"},
    {"build_commit_time", "The time of commit this ixam instance was built"},
    {"build_hash", "The commit hash this ixam instance was built with"},
    {"build_branch", "The branch this ixam instance was built from"},
    {"build_platform", "The platform this ixam instance was built for"},
...
static PyObject *make_app_info(void)
{
  ...
  app_info = PyStructSequence_New(&IxamAppType);
  if (app_info == NULL) {
    return NULL;
  }
...
#define SetObjItem(obj) PyStructSequence_SET_ITEM(app_info, pos++, obj)
  ...
  SetObjItem(
      PyC_Tuple_Pack_I32(IXAM_VERSION / 100, IXAM_VERSION % 100, IXAM_VERSION_PATCH));
  SetObjItem(PyC_Tuple_Pack_I32(
      IXAM_FILE_VERSION / 100, IXAM_FILE_VERSION % 100, IXAM_FILE_SUBVERSION));
  SetStrItem(BKE_ixam_version_string());

  SetStrItem(STRINGIFY(IXAM_VERSION_CYCLE));
  SetStrItem("");
...
PyDoc_STRVAR(bpy_app_binary_path_doc,
             "The location of 3IXAM's executable, useful for utilities that open new instances. "
             "Read-only unless 3IXAM is built as a Python module - in this case the value is "
             "an empty string which script authors may point to a 3IXAM binary.");
...
static PyGetSetDef bpy_app_getsets[] = {
    ...
    /* Support script authors setting the 3IXAM binary path to use, otherwise this value
...
static void py_struct_seq_getset_init(void)
{
  ...
  for (PyGetSetDef *getset = bpy_app_getsets; getset->name; getset++) {
    PyObject *item = PyDescr_NewGetSet(&IxamAppType, getset);
    PyDict_SetItem(IxamAppType.tp_dict, PyDescr_NAME(item), item);
    Py_DECREF(item);
  }
}
...
static void py_struct_seq_method_init(void)
{
  for (PyMethodDef *method = bpy_app_methods; method->ml_name; method++) {
    ...
    PyDict_SetItemString(IxamAppType.tp_dict, method->ml_name, item);
    Py_DECREF(item);
  }
}
...
PyObject *BPY_app_struct(void)
{
  ...
  PyStructSequence_InitType(&IxamAppType, &app_info_desc);

  ret = make_app_info();

  /* prevent user from creating new instances */
  IxamAppType.tp_init = NULL;
  IxamAppType.tp_new = NULL;
  IxamAppType.tp_hash = (hashfunc)

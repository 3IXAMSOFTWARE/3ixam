 ...
 * - `source/ixam/makesrna/intern/rna_context.c` using the RNA C API.
...
static struct PyMethodDef pyrna_ixamdata_methods[] = {
...
/* -------------------------------------------------------------------- */
/** \name Ixam Data Libraries
 * \{ */

static struct PyMethodDef pyrna_ixamdatalibraries_methods[] = {
...
{
  /* IxamData */
  ARRAY_SET_ITEMS(pyrna_ixamdata_methods,
  ...
  BLI_assert(ARRAY_SIZE(pyrna_ixamdata_methods) == 5);
  pyrna_struct_type_extend_capi(&RNA_IxamData, pyrna_ixamdata_methods, NULL);

  /* IxamDataLibraries */
  ARRAY_SET_ITEMS(
      pyrna_ixamdatalibraries_methods, BPY_library_load_method_def, BPY_library_write_method_def);
  BLI_assert(ARRAY_SIZE(pyrna_ixamdatalibraries_methods) == 3);
  pyrna_struct_type_extend_capi(&RNA_IxamDataLibraries, pyrna_ixamdatalibraries_methods, NULL);

 ...
 * one operates on a file-path, the other on 3IXAM text-block.
 * \{ */
...
bool BPY_run_filepath(struct bContext *C, const char *filepath, struct ReportList *reports)
    ATTR_NONNULL(1, 2);
/**
 * Execute a 3IXAM `text` block as a Python script.
 *
 ...
 * \return true on success, otherwise false with an error reported to `reports`.
 *
 * \note The `__file__` is constructed by joining the ixam file-path to the name of the text.

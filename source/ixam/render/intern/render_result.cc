                                        ...
                                        const int channel)
{
  ...
  BLI_string_join_array_by_sep_char(fullname, EXR_PASS_MAXNAME, '_', strings, strings_len);
}
                                              ...
                                              char r_path[FILE_CACHE_MAX])
{
  ...
  /* If root is relative, use either current .ixam file dir, or temp one if not saved. */
  const char *ixamfile_path = BKE_main_ixamfile_path_from_global();
  if (ixamfile_path[0] != '\0') {
    BLI_split_dirfile(ixamfile_path, dirname, filename, sizeof(dirname), sizeof(filename));
    BLI_path_extension_replace(filename, sizeof(filename), ""); /* strip '.ixam' */
    BLI_hash_md5_buffer(ixamfile_path, strlen(ixamfile_path), path_digest);
  }
  else {

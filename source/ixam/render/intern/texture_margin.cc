...
namespace ixam::render::texturemargin {
...
#define DijkstraPixelIsUnset(dp) ((dp) == 0xFFFFFFFF)
 ...
 private:
  ...
  void build_tables()
  {
    loop_to_poly_map_ = ixam::bke::mesh_topology::build_loop_to_poly_map({mpoly_, totpoly_},
                                                                            totloop_);
...
void RE_generate_texturemargin_adjacentfaces(ImBuf *ibuf,
                                             ...
                                             const float uv_offset[2])
{
  ixam::render::texturemargin::generate_margin(

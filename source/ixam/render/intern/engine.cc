...
RenderEngineType *RE_engines_find(const char *idname)
{
  ...
  if (!type) {
    type = static_cast<RenderEngineType *>(
        BLI_findstring(&R_engines, "IXAM_EEVEE", offsetof(RenderEngineType, idname)));
  }
...
{
  static rcti tiles_static[IXAM_MAX_THREADS];
  const int allocation_step = IXAM_MAX_THREADS;
  int total_tiles = 0;
  rcti *tiles = tiles_static;
  int allocation_size = IXAM_MAX_THREADS;
                                     ...
                                     const bool use_grease_pencil)
{
   ...
   * when reading its result from EXR file and writing to 3IXAM side. */
...
void RE_engine_free_ixam_memory(RenderEngine *engine)
{

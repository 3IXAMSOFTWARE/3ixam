...
static void stats_background(void * /*arg*/, RenderStats *rs)
{
  ...
  /* Flush stdout to be sure python callbacks are printing stuff after ixam. */
  fflush(stdout);
...
/* ************  This part uses API, for rendering 3IXAM scenes ********** */
...
static void do_render_sequencer(Render *re)
{
  ...
  ixam::Vector<ImBuf *> ibuf_arr(tot_views);
...
/* evaluating scene options for general 3IXAM render */
  ...
  if (render_init_from_main(re, &scene->r, bmain, scene, nullptr, nullptr, 0, 0)) {
    if (render) {
      ...
      if (use_eevee_for_freestyle_render(re)) {
        change_renderdata_engine(re, RE_engine_id_IXAM_EEVEE);
      }

...
static SpaceLink *graph_create(const ScrArea *UNUSED(area), const Scene *scene)
{
  ...
  sipo->flag |= SIPO_SELVHANDLESONLY | SIPO_SHOW_MARKERS | SIPO_SHOWING_VIEW3D;

  /* navbar */
  region = MEM_callocN(sizeof(ARegion), "navbar region for graphedit");
  BLI_addtail(&sipo->regionbase, region);
  region->regiontype = RGN_TYPE_NAV_BAR;
  region->alignment = RGN_ALIGN_LEFT;
  region->v2d.scroll = 0;
  region->v2d.scroll_ui = 0;
  ...
  /* header */
  region = MEM_callocN(sizeof(ARegion), "tool header for graphedit");
...
static void graph_navbar_region_init(wmWindowManager *wm, ARegion *region)
{
  /* make sure we keep the hide flags */
  region->v2d.scroll |= V2D_SCROLL_RIGHT;
...
static void toggle_view(bContext *C, void *UNUSED(arg1), void *UNUSED(arg2))
{
  bScreen *screen = CTX_wm_screen(C);
  SpaceGraph *graph = CTX_wm_space_graph(C);
  PointerRNA props;
  wmOperatorType *ot;
  if (graph->flag & SIPO_SHOWING_VIEW3D) {
    ScrArea *area_v3d = NULL;
    ...
    if (area_v3d) {
      ot = WM_operatortype_find("SCREEN_OT_area_close", true);
      WM_operator_properties_create_ptr(&props, ot);
      RNA_int_set(&props, "spacetype", SPACE_VIEW3D);
      WM_operator_name_call_ptr(C, ot, WM_OP_EXEC_DEFAULT, &props, NULL);
      WM_operator_properties_free(&props);
      graph->flag &= ~SIPO_SHOWING_VIEW3D;
    }
  }
  else {
    ot = WM_operatortype_find("SCREEN_OT_area_split", false);
    WM_operator_properties_create_ptr(&props, ot);
    RNA_int_set(&props, "spacetype", SPACE_VIEW3D);
    RNA_enum_set(&props, "direction", 'v');
    WM_operator_name_call_ptr(C, ot, WM_OP_EXEC_DEFAULT, &props, NULL);
    WM_operator_properties_free(&props);
    graph->flag |= SIPO_SHOWING_VIEW3D;
  }
  WM_event_add_notifier(C, NC_SPACE | ND_SPACE_CHANGED, NULL);
}

static void graph_navbar_region_draw(const bContext *C, ARegion *region)
{
  SpaceGraph *graph = CTX_wm_space_graph(C);
  const bool show_v3d = graph->flag & SIPO_SHOWING_VIEW3D;
  const int icon_id = show_v3d ? ICON_CLOSE_RIGHT : ICON_CLOSE_LEFT; 
  ...
  const uiWidgetColors *wcol = &UI_GetTheme()->tui.wcol_tool;
  float color[4] = {0.0f};
  rgba_uchar_to_float(color, wcol->inner);

  UI_draw_roundbox_corner_set(show_v3d ? UI_CNR_TOP_RIGHT | UI_CNR_BOTTOM_RIGHT : UI_CNR_TOP_LEFT | UI_CNR_BOTTOM_LEFT);
  UI_draw_roundbox_4fv_ex(&rect, color, NULL, 1.0f, NULL, 1.0f, 10.0f * U.dpi_fac);

  uiBlock *block = UI_block_begin(C, region, "AnimationBlock", UI_EMBOSS_NONE);
  ...
  UI_but_func_set(but, toggle_view, NULL, NULL);

  const float icon_size = UI_DPI_ICON_SIZE;
  const float icon_xy[2] = {(rect.xmax + rect.xmin - icon_size) / 2.0f,
                            (rect.ymax + rect.ymin - icon_size) / 2.0f};

  const float aspect = but->block->aspect * U.inv_dpi_fac;
  UI_icon_draw_ex(icon_xy[0],
                  icon_xy[1],
                  icon_id,
                  aspect,
                  1.0f,
  ...
  UI_block_draw((bContext *)C, block);
  /* reset view matrix */
  UI_view2d_view_restore(C);
}
...
static void graph_ixam_read_data(IxamDataReader *reader, SpaceLink *sl)
{
...
static void graph_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceGraph *sipo = (SpaceGraph *)sl;
...
static void graph_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  SpaceGraph *sipo = (SpaceGraph *)sl;
...
void ED_spacetype_ipo(void)
{
  ...
  st->ixam_read_data = graph_ixam_read_data;
  st->ixam_read_lib = graph_ixam_read_lib;
  st->ixam_write = graph_ixam_write;
  ...
  art->draw = graph_tool_header_region_draw;

  BLI_addhead(&st->regiontypes, art);

  /* regions: navbar */
  art = MEM_callocN(sizeof(ARegionType), "spacetype view3d control region");
  art->regionid = RGN_TYPE_NAV_BAR;
  art->prefsizex = UI_NAVIGATION_REGION_WIDTH;
  art->keymapflag = ED_KEYMAP_UI;
  art->listener = graph_region_listener;
  art->message_subscribe = graph_region_message_subscribe;
  art->init = graph_navbar_region_init;
  art->draw = graph_navbar_region_draw;
  BLI_addhead(&st->regiontypes, art);

  /* regions: channels */
  art = MEM_callocN(sizeof(ARegionType), "spacetype channels graphedit region");

...
static void graph_panel_properties(const bContext *C, Panel *panel)
{
  ...
  uiItemL(col, name, icon, UI_BSHAPE_ROUNDBOX);
...
static void graph_panel_key_properties(const bContext *C, Panel *panel)
{
  ...
  if (get_active_fcurve_keyframe_edit(fcu, &bezt, &prevbezt)) {
    ...
    if (fcu->flag & FCURVE_DISCRETE_VALUES) {
      ...
      uiItemL(split, IFACE_("Interpolation:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemL(split, IFACE_("None for Enum/Boolean"), ICON_IPO_CONSTANT, UI_BSHAPE_ROUNDBOX);
    }
    else {
    ...
    if ((fcu->bezt == NULL) && (fcu->modifiers.first)) {
      ...
      uiItemL(layout, TIP_("F-Curve only has F-Modifiers"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemL(layout, TIP_("See Modifiers panel below"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
    }
    else if (fcu->fpt) {
      /* samples only */
      uiItemL(layout,
              TIP_("F-Curve doesn't have any keyframes as it only contains sampled points"),
              ICON_NONE, UI_BSHAPE_ROUNDBOX);
    }
    else {
      uiItemL(layout, TIP_("No active keyframe on F-Curve"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    }
  }
...
static void driver_dvar_invalid_name_query_cb(bContext *C, void *dvar_v, void *UNUSED(arg))
{
  ...
  if (dvar->flag & DVAR_FLAG_INVALID_EMPTY) {
    uiItemL(layout, TIP_("It cannot be left blank"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
  if (dvar->flag & DVAR_FLAG_INVALID_START_NUM) {
    uiItemL(layout, TIP_("It cannot start with a number"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
  ...
  if (dvar->flag & DVAR_FLAG_INVALID_HAS_SPACE) {
    uiItemL(layout, TIP_("It cannot contain spaces (e.g. 'a space')"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
  if (dvar->flag & DVAR_FLAG_INVALID_HAS_DOT) {
    uiItemL(layout, TIP_("It cannot contain dots (e.g. 'a.dot')"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
  if (dvar->flag & DVAR_FLAG_INVALID_HAS_SPECIAL) {
    uiItemL(layout,
            TIP_("It cannot contain special (non-alphabetical/numeric) characters"),
            ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
  if (dvar->flag & DVAR_FLAG_INVALID_PY_KEYWORD) {
    uiItemL(layout, TIP_("It cannot be a reserved keyword in Python"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }
...
static void graph_panel_driverVar__locDiff(uiLayout *layout, ID *id, DriverVar *dvar)
{
  ...
  if (dtar->id && GS(dtar->id->name) == ID_OB && ob1->pose) {
    ...
    RNA_pointer_create(dtar->id, &RNA_Pose, ob1->pose, &tar_ptr);
    uiItemPointerR(
        col, &dtar_ptr, "bone_target", &tar_ptr, "bones", IFACE_("Joint"), ICON_BONE_DATA);
  }
  ...
  if (dtar2->id && GS(dtar2->id->name) == ID_OB && ob2->pose) {
    ...
    RNA_pointer_create(dtar2->id, &RNA_Pose, ob2->pose, &tar_ptr);
    uiItemPointerR(
        col, &dtar2_ptr, "bone_target", &tar_ptr, "bones", IFACE_("Joint"), ICON_BONE_DATA);
  }
...
static void graph_draw_driven_property_panel(uiLayout *layout, ID *id, FCurve *fcu)
{
  ...
  uiItemL(row, id->name + 2, icon, UI_BSHAPE_ROUNDBOX);

  /* -> user friendly 'name' for F-Curve/driver target */
  uiItemL(row, "", ICON_RIGHTARROW, UI_BSHAPE_ROUNDBOX);
  uiItemL(row, name, ICON_RNA, UI_BSHAPE_ROUNDBOX);
}
                                             ...
                                             const bool is_popover)
{
...

  {
    ...
    uiItemL(row, IFACE_("Driver Value:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    BLI_snprintf(valBuf, sizeof(valBuf), "%.3f", driver->curval);
    uiItemL(row, valBuf, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  ...
  if (driver->type == DRIVER_TYPE_PYTHON) {
    ...
    uiItemL(col, IFACE_("Expression:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    ...
    if (driver->flag & DRIVER_FLAG_INVALID) {
      uiItemL(col, TIP_("ERROR: Invalid Python expression"), ICON_CANCEL, UI_BSHAPE_ROUNDBOX);
    }
    else if (!BKE_driver_has_simple_expression(driver)) {
      if ((G.f & G_FLAG_SCRIPT_AUTOEXEC) == 0) {
        /* TODO: Add button to enable? */
        uiItemL(col, TIP_("Python restricted for security"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
      }
      else {
        uiItemL(col, TIP_("Slow Python expression"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
      }
    }
    ...
    if (bpy_data_expr_error || bpy_ctx_expr_error) {
      uiItemL(col, TIP_("WARNING: Driver expression may not work correctly"), ICON_HELP, UI_BSHAPE_ROUNDBOX);

      if (bpy_data_expr_error) {
        uiItemL(col, TIP_("TIP: Use variables instead of bpy.data paths (see below)"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
      }
      if (bpy_ctx_expr_error) {
        uiItemL(col, TIP_("TIP: bpy.context is not safe for renderfarm usage"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
      }
    }
  }
  else {
    ...
    if (driver->flag & DRIVER_FLAG_INVALID) {
      uiItemL(col, TIP_("ERROR: Invalid target channel(s)"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
    }
    ...
    if (BLI_listbase_is_empty(&driver->variables)) {
      uiItemL(col, TIP_("ERROR: Driver is useless without any inputs"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);

      if (!BLI_listbase_is_empty(&fcu->modifiers)) {
        uiItemL(col, TIP_("TIP: Use F-Curves for procedural animation instead"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
        uiItemL(col, TIP_("F-Modifiers can generate curves for those too"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
      }
    }
  }
...
static void graph_panel_drivers_popover(const bContext *C, Panel *panel)
{
  ...
  if (but) {
    ...
    if (fcu && fcu->driver) {
      ...
      uiItemL(layout, IFACE_("Driven Property:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      ...
      /* Drivers Settings */
      uiItemL(layout, IFACE_("Driver Settings:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);

...
static int geometry_attribute_convert_exec(bContext *C, wmOperator *op)
{
  ...
  switch (mode) {
    case ConvertAttributeMode::Generic: {
      ...
      attributes.add(name, dst_domain, dst_type, ixam::bke::AttributeInitMoveArray(new_data));
      break;
    }
                                   ...
                                   eAttrDomain new_domain)
{
  ...
  ixam::bke::MutableAttributeAccessor attributes = mesh->attributes_for_write();
  ...
  src_varray.materialize_to_uninitialized(new_data);
  attributes.remove(name);
  attributes.add(name, new_domain, new_type, ixam::bke::AttributeInitMoveArray(new_data));

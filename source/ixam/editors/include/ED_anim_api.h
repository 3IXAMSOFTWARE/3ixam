...
/**
 * Obtain current anim-data context from 3IXAM Context info
 * - AnimContext to write to is provided as pointer to var on stack so that we don't have
 *   allocation/freeing costs (which are not that avoidable with channels).
 * - Clears data and sets the information from 3IXAM Context which is useful
 ...
 * given that context info from 3IXAM context has already been set:
 * - AnimContext to write to is provided as pointer to var on stack so that we don't have
 *   allocation/freeing costs (which are not that avoidable with channels).
 * \return whether the operation was successful.
 */
...
typedef enum eAnimChannel_Settings {
  ...
  ACHANNEL_SETTING_ALWAYS_VISIBLE = 7,
 ...
 */
struct bAction *ANIM_channel_action_get(const bAnimListElem *ale);

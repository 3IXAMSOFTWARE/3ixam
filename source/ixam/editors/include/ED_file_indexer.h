 ...
 *
 * This file contains an API to create indexing functionality when listing ixam files in
 * the file browser.
 *
...
typedef enum eFileIndexerResult {
  /**
   * File listing entries are loaded from the index. Reading entries from the ixam file itself
   * should be skipped.
   */
  FILE_INDEXER_ENTRIES_LOADED,

  /**
   * Index isn't available or not up to date. Entries should be read from the ixam file and
...
/**
 * Contains all entries of a ixam file.
 */
...
typedef struct FileIndexerType {
  ...
  /**
   * Is called for each ixam file being listed to read data from the index.
   *
   ...
   * In this case the ixam file will not be opened and the FileIndexerEntry added to `entries`
   ...
   * `eFileIndexerResult::FILE_INDEXER_NEEDS_UPDATE`. In this case the ixam file will read from
   * the ixam file and the `update_index` function will be called.
   */
  FileIndexerReadIndexFunc read_index;

  /**
   * Update an index of a ixam file.
   *

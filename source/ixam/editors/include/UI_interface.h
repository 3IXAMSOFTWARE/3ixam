...
#define UI_COMPACT_PANEL_WIDTH ((void)0, U.widget_unit * 1.3 + 2)
...
#define UI_TAB_CLOSE_OFFSET 0.1f
...
typedef enum {
  ...
  UI_BTYPE_VIEWCONFIG = 21 << 9,
  UI_BTYPE_OUTLINEROBJECT = 22 << 9,
  UI_BTYPE_OUTLINERFOLDER = 23 << 9,
  ...
  /* Animation button. */
  UI_BTYPE_ANIMATION = 61 << 9,
  /* Special hex button. */
  UI_BTYPE_IMAGE_MENU = 62 << 9,
  /* Material operator. */
  UI_BTYPE_MATOPERATOR = 63 << 9,
  UI_BTYPE_VERTROW = 64 << 9,
  UI_BTYPE_MATPRO_TAB = 65 << 9,
  UI_BTYPE_MATLIST_BUT = 66 << 9,
} eButType;

typedef enum {
  UI_BSHAPE_ROUNDBOX = 0,
  UI_BSHAPE_HEX = 1,
  UI_BSHAPE_TRAPEZOID = 2,
  UI_BSHAPE_TRAPEZOIDR = 3,
  UI_BSHAPE_RHOMBOID = 4,
  UI_BSHAPE_ADVANCED = 5,
  UI_BSHAPE_PRO_HEX = 6,
  UI_BSHAPE_PREFS = 7,
} eButShape;

#define BUTTYPE (127 << 9)

typedef enum {
  UI_TAB_TYPE_MATPRO = 1,
} eTabType;
...
void UI_draw_roundbox_4fv_ex(const struct rctf *rect,
                             ...
                             float outline_width,
                             float rad);
void UI_draw_shape_4fv_ex(const struct rctf *rect,
                          const float inner1[4],
                          const float inner2[4],
                          float shade_dir,
                          const float outline[4],
...
void UI_but_set_shape(uiBut *but, eButShape shape);
...
/**
 *  Flags for popover to region alignment
 */
enum {
  UI_POPOVER_ALIGN_NONE = 0,
  UI_POPOVER_ALIGN_LEFT = 1,
  UI_POPOVER_ALIGN_RIGHT = 2,
};
...
uiBut *uiDefPulldownBut(uiBlock *block,
                        ...
                        short height,
                        const char *tip);
uiBut *uiDefMenuImageBut(uiBlock *block,
...
void UI_panel_set_active(struct Panel* panel, bool value);
/**
...
int UI_panel_size_x(const struct Panel *panel);
...
void UI_panel_draw_instanced(const struct bContext *C,
                             struct ARegion *region,
                             uiLayout *layout,
                             const char *panel_idname,
                             struct PointerRNA *custom_data);
...
#define UI_ITEM_SHAPE_FIELD_OFFSET 15
#define UI_ITEM_SHAPE_FIELD_MASK 0b111
...
enum {
  ...
  /* Shape of button 3 bit */
  UI_ITEM_SHAPE = (UI_ITEM_SHAPE_FIELD_MASK << UI_ITEM_SHAPE_FIELD_OFFSET)
};
...
void UI_popover_draw(struct bContext *C, struct PanelType *pt, struct uiLayout *layout);
...
void uiTemplateConstraints_panel(uiLayout *layout, struct bContext *C, bool use_bone_constraints);
 ...
 * \param icon_scale: Scale of the icon, 1x == button height.
 */
struct Collection* get_collection_from_but(uiBut* but);
...
void uiTemplateImage(uiLayout *layout,
                     ...
                     bool multiview, 
                     bool show_new);
void uiTemplateImagePassSelectorOnly(uiLayout *layout,
                                     struct bContext *C,
                                     struct PointerRNA *ptr,
                                     const char *propname,
                                     struct PointerRNA *userptr,
...
void uiTemplateRenderJobs(uiLayout *layout, struct bContext *C);
...
void uiItemL(uiLayout *layout, const char *name, int icon, int shape); /* label */
 ...
 * Menu.
 */
void uiItemM_ptr(uiLayout *layout, struct MenuType *mt, const char *name, int icon, int shape);
 ...
 * Demensions panel.
 */
void v3d_object_dimension_buts(const struct bContext *C, uiLayout *layout, View3D *v3d, Object *ob);
void uiItemDimensions(uiLayout *layout, const struct bContext *C);
/**
 * Animation progress bar
 */
void uiItemAnimBar(const struct bContext *C, uiLayout *layout);
/**
 * UV axis coordinates
 */
void uiItemUVAxis(const struct bContext *C, uiLayout *layout, int axis);
/**
 * Menu contents.
 */
...
void uiItemOFolder(uiLayout *layout, struct Collection* collection);
...
void uiItemPopoverPanel_vc_ptr(
    uiLayout *layout, const struct bContext *C, struct PanelType *pt);
void uiItemPopoverPanel(uiLayout *layout,
                        ...
                        int align,
                        bool depress);
void uiItemPopoverPanel_vc(uiLayout *layout,
                        const struct bContext *C,
                        const char *panel_type);

...
static void nla_panel_animdata(const bContext *C, Panel *panel)
{
  ...
  if (adt_ptr.owner_id) {
    ...
    uiItemL(row, id->name + 2, RNA_struct_ui_icon(id_ptr.type), UI_BSHAPE_ROUNDBOX); /* id-block (src) */
    uiItemL(row, "", ICON_RIGHTARROW, UI_BSHAPE_ROUNDBOX);                           /* expander */
    uiItemL(row, IFACE_("Animation Data"), ICON_ANIM_DATA, UI_BSHAPE_ROUNDBOX);      /* animdata */
...
static void nla_panel_stripname(const bContext *C, Panel *panel)
{
  ...
  if (RNA_enum_get(&strip_ptr, "type") == NLASTRIP_TYPE_CLIP) {
    uiItemL(row, "", ICON_ANIM, UI_BSHAPE_ROUNDBOX);
  }
  else if (RNA_enum_get(&strip_ptr, "type") == NLASTRIP_TYPE_TRANSITION) {
    uiItemL(row, "", ICON_ARROW_LEFTRIGHT, UI_BSHAPE_ROUNDBOX);
  }
  else if (RNA_enum_get(&strip_ptr, "type") == NLASTRIP_TYPE_META) {
    uiItemL(row, "", ICON_SEQ_STRIP_META, UI_BSHAPE_ROUNDBOX);
  }
  else if (RNA_enum_get(&strip_ptr, "type") == NLASTRIP_TYPE_SOUND) {
    uiItemL(row, "", ICON_SOUND, UI_BSHAPE_ROUNDBOX);
  }

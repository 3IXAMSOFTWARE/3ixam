...
static bool snap_use_backface_culling(const TransInfo *t)
{
  ...
  if (v3d->shading.type == OB_RENDER &&
      ...
      BKE_scene_uses_ixam_workbench(t->scene)) {
    return true;
  }
...
bool rotateUseSnap(const TransInfo *t)
{
  ToolSettings *ts = t->settings;
  if (t->mode == TFM_ROTATION) {
    return (ts->snap_transform_mode_flag & SCE_SNAP_TRANSFORM_MODE_ROTATE) != 0;
...
void drawSnapping(const struct bContext *C, TransInfo *t)
{
  ...
  else if (ELEM(t->spacetype, SPACE_NODE, SPACE_MATPRO)) {
...
static eSnapMode snap_mode_from_spacetype(TransInfo *t)
{
  ...
  if (ELEM(t->spacetype, SPACE_NODE, SPACE_MATPRO)) {
...
static eSnapTargetSelect snap_target_select_from_spacetype(TransInfo *t)
{
  ...
  else if (ELEM(t->spacetype, SPACE_NODE, SPACE_SEQ, SPACE_MATPRO)) {
    ret |= SCE_SNAP_TARGET_NOT_SELECTED;
  }
...
static void initSnappingMode(TransInfo *t)
{
  ...
  if (ELEM(t->spacetype, SPACE_VIEW3D, SPACE_IMAGE, SPACE_NODE, SPACE_SEQ, SPACE_MATPRO)) {
...
{
  BLI_assert(ELEM(t->spacetype, SPACE_NODE, SPACE_MATPRO));
...
{
  if (ELEM(t->spacetype, SPACE_NODE, SPACE_MATPRO) && td != NULL) {
    bNode *node = td->extra;
...
static void snap_increment_apply(const TransInfo *t,
                                 ...
                                 float *r_val)
{
  BLI_assert(transformModeUseSnap(t));
...
bool transform_snap_increment_ex(const TransInfo *t, bool use_local_space, float *r_val)
{
  ...
  if (t->mode == TFM_TRANSLATION && (t->tsnap.mode & SCE_SNAP_MODE_INCREMENT) == 0){
    return false;
  }

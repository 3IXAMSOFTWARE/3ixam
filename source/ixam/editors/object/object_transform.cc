...
static void object_clear_rot(Object *ob, const bool clear_delta)
{
  ...
  } /* Duplicated in source/ixam/editors/armature/editarmature.c */
  else {
    if (ob->rotmode == ROT_MODE_QUAT) {
      unit_qt(ob->quat);
      if (clear_delta) {
                                  ...
                                  bool do_single_user)
{
  ...
  for (Object *ob : objects) {
    ...
    if (do_multi_user && ob != obact) {
      ...
      ixam::bke::CurvesGeometry::wrap(curves.geometry).transform(mat);
      ixam::bke::CurvesGeometry::wrap(curves.geometry).calculate_bezier_auto_handles();
    }
    ...
    if (do_multi_user && ob != obact) {
      ...
      if (apply_loc && apply_scale && apply_rot) {
        ...
        Object ob_temp = ixam::dna::shallow_copy(*ob);
...
void OBJECT_OT_transform_apply(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  true,
                  "Apply Properties",
                  "Modify properties such as curve vertex radius, font size and joint envelope");
...
static int object_origin_set_exec(bContext *C, wmOperator *op)
{
...
#if 0
          BKE_report(op->reports, RPT_ERROR, "Cannot apply to a multi user rig");
          return;
#endif
  ...
  if (tot_lib_error + tot_multiuser_arm_error) {
    ...
    if (tot_multiuser_arm_error) {
      BKE_reportf(
          op->reports, RPT_WARNING, "|%i multiuser rig object(s)", tot_multiuser_arm_error);
    }
  }
...
{
  /* quick but weak */
  Object ob_prev = ixam::dna::shallow_copy(*ob);
  float mat[4][4];
  copy_m4_m4(mat, ob->object_to_world);
  copy_v3_v3(mat[3], loc);
  BKE_object_apply_mat4(ob, mat, true, true);
  copy_v3_v3(mat[3], ob->loc);
  *ob = ixam::dna::shallow_copy(ob_prev);

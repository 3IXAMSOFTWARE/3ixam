...
static int object_armature_add_exec(bContext *C, wmOperator *op)
{
  ...
  if (obedit == NULL) {
    BKE_report(op->reports, RPT_ERROR, "Cannot create editmode rig");
    return OPERATOR_CANCELLED;
  }
...
void OBJECT_OT_armature_add(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Add Joint";
  ot->description = "Add an joint object to the scene";
...
void OBJECT_OT_drop_named_image(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  true,
                  "Relative Path",
                  "Select the file relative to the ixam file");
  RNA_def_property_flag(prop, (PropertyFlag)(PROP_HIDDEN | PROP_SKIP_SAVE));
...
static int object_curves_empty_hair_add_exec(bContext *C, wmOperator *op)
{
  ...
  ixam::ed::curves::ensure_surface_deformation_node_exists(*C, *curves_ob);
...
static int delete_group(Object* ob)
{
  ob->id.tag |= LIB_TAG_DOIT;
  uint tagged_count = 0;
  if (ob->is_group) {
    /* Delete all objects inside group*/
    Collection* group_collection = ob->instance_collection;
    if (group_collection != NULL) {
      LISTBASE_FOREACH (CollectionObject *, coll_ob, &group_collection->gobject) {
        Object *obj = coll_ob->ob;
        tagged_count += delete_group(obj);
      }
      group_collection->id.tag |= LIB_TAG_DOIT;
    }
  }
  else {
    tagged_count += 1;
...
void unflag_used_objects(Object *ob) {
  ob->id.tag &= ~LIB_TAG_DOIT;
  Collection *group_collection = ob->instance_collection;
  if (group_collection) {
    LISTBASE_FOREACH (CollectionObject *, coll_ob, &group_collection->gobject) {
      Object *obj = coll_ob->ob;
      unflag_used_objects(obj);
    }
    group_collection->id.tag &= ~LIB_TAG_DOIT;
  } 
}
...
static int object_delete_exec(bContext *C, wmOperator *op)
{
  ...
  LISTBASE_FOREACH (Base *, base, &view_layer->object_bases) {
    ...
    CTX_DATA_BEGIN (C, Object *, sel_ob, selected_objects) {
      if (ob == sel_ob) {
                                       ...
                                       const bool keep_animdata)
{
  ...
  Collection* collection = object_eval->instance_collection;
  ...
  link = (Link *)lb_duplis->first;
  while (link) {
    next = link->next;
    
    DupliObject *dob = (DupliObject *)link;
    Object *ob = DEG_get_original_object(dob->ob);
    Collection* ob_col = BKE_collection_object_find(bmain, NULL, NULL, ob);
    
    if (&ob_col->id != collection->id.orig_id) {
      BLI_freelinkN(lb_duplis, dob);
    }
  ...
  LISTBASE_FOREACH (DupliObject *, dob, lb_duplis) {
    ...
    Object *ob_dst = (Object *)ID_NEW_SET(ob_src, BKE_id_copy(bmain, &ob_src->id));
    id_us_min(&ob_dst->id);
  ...
  LISTBASE_FOREACH (DupliObject *, dob, lb_duplis) {
    ...
    Object *ob_dst = (Object *)BLI_ghash_lookup(dupli_gh, dob);
    ...
    if (use_hierarchy) {
      ...
      if (ob_src_par) {
        ...
        ob_dst_par = (Object *)BLI_ghash_lookup(parent_gh, &dob_key);
      }
    ...
    if (use_base_parent && ob_dst->parent == NULL) {
      ...
      if (instancer_gh != NULL) {
        ...
        ob_dst_par = (Object *)BLI_ghash_lookup(instancer_gh, &dob_key);
      }
...
static int object_duplicates_make_real_exec(bContext *C, wmOperator *op)
{
  ...
  const bool keep_animdata = RNA_boolean_get(op->ptr, "keep_animdata");

  BKE_main_id_newptr_and_tag_clear(bmain);

  CTX_DATA_BEGIN (C, Base *, base, selected_editable_bases) {
    make_object_duplilist_real(C, depsgraph, scene, base, use_base_parent, use_hierarchy, keep_animdata);
...
void OBJECT_OT_duplicates_make_real(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  false,
                  "Keep Animation",
                  "Do not free animation data of newly created objects. WARNING: Can cause incorrect behaviour");
}
  ...
  LISTBASE_FOREACH (CollectionPointerLink *, link, &selected_editable_bases) {
    ...
    if (ob->flag & OB_DONE || !IS_TAGGED(ob->data)) {
      ...
      if (geometry.has_curves()) {
        ...
        ixam::bke::CurvesGeometry::wrap(
            new_curves->geometry) = ixam::bke::CurvesGeometry::wrap(curves_eval->geometry);
...
static int duplicate_exec(bContext *C, wmOperator *op)
{
  ...
  ixam::Vector<DuplicateObjectLink> object_base_links;

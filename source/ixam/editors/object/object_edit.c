...
{
  return ED_operator_view3d_active(C) && ED_operator_object_active(C);
}

static bool object_unhide_poll(bContext *C)
{
...
static int object_hide_view_clear_exec(bContext *C, wmOperator *op)
{
  ...
  LISTBASE_FOREACH (Base *, base, BKE_view_layer_object_bases_get(view_layer)) {
    ...
    bool hidden = RNA_boolean_get(&ptr, "hide_viewport");
    if (hidden) {

      RNA_property_boolean_set(&ptr, prop, false);
...
static int object_matlib_clear_active_material(bContext *C, wmOperator *op)
{
  SpaceMatLib *matlib = CTX_wm_space_matlib(C);

  if (matlib) {
    matlib->active_material = NULL;
  }
...
void OBJECT_OT_matlib_clear_active_material(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "MatLib clear active material";
  ot->description = "MatLib clear active material";
  ot->idname = "OBJECT_OT_matlib_clear_active_material";

  /* api callbacks */
  ot->exec = object_matlib_clear_active_material;
}
...
void OBJECT_OT_posemode_toggle(wmOperatorType *ot)
{
  ...
  ot->description = "Enable or disable posing/selecting joints";
...
static int shade_smooth_exec(bContext *C, wmOperator *op)
{
  ...
  LISTBASE_FOREACH (CollectionPointerLink *, ctx_ob, &ctx_objects) {
    ...
    bool changed = false;
    if (ob->type == OB_MESH) {
      BKE_mesh_smooth_flag_set((Mesh *)(ob->data), use_smooth);
      if (use_smooth) {
        ...
        BKE_mesh_auto_smooth_flag_set(
            (Mesh *)(ob->data), use_auto_smooth, auto_smooth_angle);
      }
      else {
        ((Mesh *)(ob->data))->flag &= ~ME_AUTOSMOOTH;
      }
      ...
      BKE_curve_smooth_flag_set((Curve *)(ob->data), use_smooth);
      changed = true;
    }
...
static int object_mode_set_exec(bContext *C, wmOperator *op)
{
  ...
  const bool toggle = RNA_boolean_get(op->ptr, "toggle");
  const bool toggle_without_prev = RNA_boolean_get(op->ptr, "toggle_without_prev");
...
void OBJECT_OT_mode_set(wmOperatorType *ot)
{
  ...
  prop = RNA_def_boolean(ot->srna, "toggle_without_prev", 0, "Toggle Without Prev", "");
  RNA_def_property_flag(prop, PROP_SKIP_SAVE);
}
...
static int move_to_collection_exec(bContext *C, wmOperator *op)
{
  ...
  if (!RNA_property_is_set(op->ptr, prop)) {
    BKE_report(op->reports, RPT_ERROR, "No folder selected");
    return OPERATOR_CANCELLED;
  }
  ...
  if (collection == NULL) {
    BKE_report(op->reports, RPT_ERROR, "Unexpected error, folder not found");
    return OPERATOR_CANCELLED;
  }
...
static int move_to_collection_menus_create(wmOperator *op, MoveToCollectionData *menu)
{
  ...
  LISTBASE_FOREACH (CollectionChild *, child, &menu->collection->children) {
    ...
    if (collection->protectflag & COL_LOCK_DELETE) {
      continue;
    }
...
static void move_to_collection_menu_create(bContext *C, uiLayout *layout, void *menu_v)
{
  ...
  uiItemFullO_ptr(
      layout, menu->ot, CTX_IFACE_(BLT_I18NCONTEXT_OPERATOR_DEFAULT, "New Folder"), ICON_ADD, menu->ptr.data, WM_OP_INVOKE_DEFAULT, 0, NULL);
...
void OBJECT_OT_move_to_collection(wmOperatorType *ot)
{
  ...
  /* identifiers */
  ot->name = "Move to Folder";
  ot->description = "Move objects to a folder";
  ...
  prop = RNA_def_string(ot->srna,
                        ...
                        MAX_NAME,
                        "Name",
                        "Name of the newly added folder");
  RNA_def_property_flag(prop, PROP_SKIP_SAVE);
  ot->prop = prop;
}

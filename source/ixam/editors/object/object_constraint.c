...
static const EnumPropertyItem constraint_owner_items[] = {
    ...
    {EDIT_CONSTRAINT_OWNER_BONE, "BONE", 0, "Joint", "Edit a constraint on the active joint"},
    {0, NULL, 0, NULL, NULL},
};
...
void CONSTRAINT_OT_stretchto_reset(wmOperatorType *ot)
{
  ...
  ot->description = "Reset original length of joint for Stretch To Constraint";
...
static bool constraint_copy_to_selected_poll(bContext *C)
{
  ...
  if (pchan) {
    ...
    CTX_wm_operator_poll_msg_set(C, "No other joints are selected");
    return false;
  }
...
void CONSTRAINT_OT_copy_to_selected(wmOperatorType *ot)
{
  ...
  ot->description = "Copy constraint to other selected objects/joints";
...
void POSE_OT_constraints_clear(wmOperatorType *ot)
{
  ...
  ot->description = "Clear all the constraints for the selected joints";
...
static int pose_constraint_copy_exec(bContext *C, wmOperator *op)
{
  ...
  if (ELEM(NULL, pchan, pchan->constraints.first)) {
    BKE_report(op->reports, RPT_ERROR, "No active joint with constraints for copying");
    return OPERATOR_CANCELLED;
  }
...
{
  /* identifiers */
  ot->name = "Copy Constraints to Selected Joints";
  ot->idname = "POSE_OT_constraints_copy";
  ot->description = "Copy constraints to other selected joints";
    ...
    bContext *C, wmOperator *op, Object *ob, ListBase *list, int type, const bool setTarget)
{
  ...
  if (list == &ob->constraints) {
    ...
    if (pchan == NULL) {
      BKE_report(op->reports, RPT_ERROR, "No active pose joint to add a constraint to");
      return OPERATOR_CANCELLED;
    }
  }
...
void POSE_OT_constraint_add(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Add Constraint";
  ot->description = "Add a constraint to the active joint";
...
void POSE_OT_constraint_add_with_targets(wmOperatorType *ot)
{
  ...
  ot->name = "Add Constraint (with Targets)";
  ot->description =
      "Add a constraint to the active joint, with target (where applicable) set to the selected "
      "Objects/Joints";
...
static int pose_ik_add_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  ...
  if (ELEM(NULL, ob, pchan)) {
    BKE_report(op->reports, RPT_ERROR, "Must have an active joint to add IK constraint to");
    return OPERATOR_CANCELLED;
  }
  ...
  if (con) {
    BKE_report(op->reports, RPT_ERROR, "Joint already has an IK constraint");
    return OPERATOR_CANCELLED;
  }
  ...
  if (get_new_constraint_target(C, CONSTRAINT_TYPE_KINEMATIC, &tar_ob, &tar_pchan, 0)) {
    ...
    if (tar_pchan) {
      uiItemBooleanO(
          layout, IFACE_("To Active Joint"), ICON_NONE, "POSE_OT_ik_add", "with_targets", 1);
    }
    else {
      uiItemBooleanO(
...
{
  /* identifiers */
  ot->name = "Add IK to Joint";
  ot->description = "Add IK Constraint to the active Joint";
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  1,
                  "With Targets",
                  "Assign IK Constraint with targets derived from the select joints/objects");
}
...
void POSE_OT_ik_clear(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Remove IK";
  ot->description = "Remove all IK Constraints from selected joints";

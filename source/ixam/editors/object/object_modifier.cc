                                             ...
                                             ModifierData *md)
{
  ...
  ixam::MutableSpan<MVert> verts = me->verts_for_write();
  ixam::MutableSpan<MEdge> edges = me->edges_for_write();
    ...
    ReportList *reports, Depsgraph *depsgraph, Scene *scene, Object *ob, ModifierData *md_eval)
{
  ...
  if (ob->type == OB_MESH) {
    ...
    /* Anonymous attributes shouldn't be available on the applied geometry. */
    ixam::bke::CurvesGeometry::wrap(curves_eval.geometry)
        .attributes_for_write()
        .remove_anonymous();

    /* Copy the relevant information to the original. */
    ixam::bke::CurvesGeometry::wrap(curves.geometry) = std::move(
        ixam::bke::CurvesGeometry::wrap(curves_eval.geometry));
    Main *bmain = DEG_get_bmain(depsgraph);
...
static const EnumPropertyItem *modifier_add_itemf_without_physics(bContext *C,
                                                  ...
                                                  bool *r_free)
{
  ...
  if (!ob) {
    return rna_enum_object_modifier_type_items_without_physics;
  }
  ...
  for (int a = 0; rna_enum_object_modifier_type_items_without_physics[a].identifier; a++) {
    const EnumPropertyItem *md_item = &rna_enum_object_modifier_type_items_without_physics[a];

    if (md_item->identifier[0]) {
      const ModifierTypeInfo *mti = BKE_modifier_get_info(static_cast<ModifierType>(md_item->value));
...
void OBJECT_OT_modifier_add_without_physics(wmOperatorType *ot)
{
  PropertyRNA *prop;
  ...
  ot->name = "Add Modifier";
  ot->description = "Add a procedural operation/effect to the active object";
  ot->idname = "OBJECT_OT_modifier_add_without_physics";
  ...
  /* properties */
  prop = RNA_def_enum(
      ot->srna, "type", rna_enum_object_modifier_type_items_without_physics, eModifierType_Subsurf, "Type", "");
  RNA_def_enum_funcs(prop, modifier_add_itemf_without_physics);
  ot->prop = prop;
}
...
static int multires_external_save_exec(bContext *C, wmOperator *op)
{
  ...
  if (relative) {
    BLI_path_rel(path, BKE_main_ixamfile_path(bmain));
  }
                                      ...
                                      int parent_v)
{
  for (int i = 0; i < emap[parent_v].count; i++) {
    ...
    EditBone *bone = ED_armature_ebone_add(arm, "Joint");
    ...
    BLI_snprintf(bone->name, sizeof(bone->name), "Joint.%.2d", endx);
...
static Object *modifier_skin_armature_create(Depsgraph *depsgraph, Main *bmain, Object *skin_ob)
{
  ...
  for (int v = 0; v < me->totvert; v++) {
    if (mvert_skin[v].flag & MVERT_SKIN_ROOT) {
       ...
       * (arbitrary) */
      if (emap[v].count > 1) {
        bone = ED_armature_ebone_add(arm, "Joint");
...
{
  ot->name = "Skin Rig Create";
  ot->description = "Create an rig that parallels the skin layout";

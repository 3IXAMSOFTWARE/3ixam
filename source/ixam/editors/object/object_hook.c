                           ...
                           ReportList *reports)
{
  ...
  if (mode == OBJECT_ADDHOOK_SELOB_BONE) {
    ...
    if (arm->act_bone) {
      ...
      BKE_report(reports, RPT_WARNING, "Rig has no active object joint");
    }
  }
...
static int object_add_hook_selob_exec(bContext *C, wmOperator *op)
{
  ...
  if (use_bone && obsel->type != OB_ARMATURE) {
    BKE_report(op->reports, RPT_ERROR, "Cannot add hook joint for a non rig object");
    return OPERATOR_CANCELLED;
  }
...
void OBJECT_OT_hook_add_selob(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  false,
                  "Active Joint",
                  "Assign the hook to the hook objects active joint");
}

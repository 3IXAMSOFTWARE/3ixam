...
ListBase *get_channel_keyframes(const bContext *C, struct bAnimContext *ac)
{
...
//  action_flag &= ~(SACTION_SHOW_INTERPOLATION | SACTION_SHOW_EXTREMES);
  ...
  for (ale = anim_data.first; ale; ale = ale->next) {
    ...
    if (ale->datatype != ALE_NONE) {
      ...
      switch (ale->datatype) {
        case ALE_ALL:
          draw_summary_channel(draw_list, ale->data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_SCE:
          draw_scene_channel(
              draw_list, ads, ale->key_data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_OB:
          draw_object_channel(
              draw_list, ads, ale->key_data, 0, ac->yscale_fac, action_flag);
        ...
        case ALE_ACT:
          draw_action_channel(
              draw_list, adt, ale->key_data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_GROUP:
          draw_agroup_channel(draw_list, adt, ale->data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_FCURVE:
          draw_fcurve_channel(
              draw_list, adt, ale->key_data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_GPFRAME:
          draw_gpl_channel(draw_list, ads, ale->data, 0, ac->yscale_fac, action_flag);
          break;
        case ALE_MASKLAY:
          draw_masklay_channel(draw_list, ads, ale->data, 0, ac->yscale_fac, action_flag);
          break;
      }
    }
  }
  ED_keylists_build(draw_list);
  
  ListBase *lb = ED_keylist_get_frames(draw_list);

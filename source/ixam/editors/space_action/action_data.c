  ...
  if (ED_operator_action_active(C)) {
    ...
    if (saction->mode == SACTCONT_ACTION) {
      ...
      if (ob) {
        ...
        if ((ob->adt->flag & ADT_NLA_EDIT_ON) == 0) {
          return true;
        }
      }
    }
...
static bool action_new_empty_poll(bContext *C)
{
  Scene *scene = CTX_data_scene(C);
...
static int action_new_empty_exec(bContext *C, wmOperator *UNUSED(op))
{
  PointerRNA ptr, idptr;
  PropertyRNA *prop;
...
void ACTION_OT_new_empty(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "New Empty Action";
  ot->idname = "ACTION_OT_new_empty";
  ot->description = "Create new empty action";

  /* api callbacks */
  ot->exec = action_new_empty_exec;
  ot->poll = action_new_empty_poll;

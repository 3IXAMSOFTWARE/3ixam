...
static Main *load_main_from_memory(const void *ixam, int ixam_size)
{
  ...
  bfd = BLO_read_from_memory(ixam, ixam_size, BLO_READ_SKIP_NONE, nullptr);
  if (bfd) {
    bmain = bfd->main;
...
void ED_preview_ensure_dbase()
{
  ...
  if (!base_initialized) {
    G_pr_main = load_main_from_memory(datatoc_preview_ixam, datatoc_preview_ixam_size);
    G_pr_main_grease_pencil = load_main_from_memory(datatoc_preview_grease_pencil_ixam,
                                                    datatoc_preview_grease_pencil_ixam_size);
...
{
  return !STREQ(sce->r.engine, RE_engine_id_IXAM_EEVEE);
}
    ...
    Main *bmain, Scene *scene, ID *id, int id_type, ShaderPreview *sp)
{
  ...
  memcpy(pr_main->filepath, BKE_main_ixamfile_path(bmain), sizeof(pr_main->filepath));
  ...
  if (sce) {
    ...
    if (id_type == ID_TE) {
      ...
      BLI_strncpy(sce->r.engine, RE_engine_id_IXAM_EEVEE, sizeof(sce->r.engine));
    }
...
void ED_preview_draw(const bContext *C, void *idp, void *parentp, void *slotp, rcti *rect)
{
  if (idp) {
    ...
    if ((sbuts != nullptr && sbuts->preview) ||
        ...
        || (smatpro != nullptr && smatpro->preview)) {
      if (sbuts != nullptr) {
        sbuts->preview = 0;
      }
      if(smatpro != nullptr) {
...
static ImBuf *icon_preview_imbuf_from_brush(Brush *brush)
{
  if (!brush->icon_imbuf && (brush->flag & BRUSH_CUSTOM_ICON) && brush->icon_filepath[0]) {
    ...
    BLI_path_abs(filepath, ID_IXAM_PATH_FROM_GLOBAL(&brush->id));
    ...
    if (!(brush->icon_imbuf)) {
      const char *brushicons_dir = BKE_appdir_folder_id(IXAM_DATAFILES, "brushicons");
 ...
 * preview requests are done loading (or it's otherwise aborted, e.g. by closing 3IXAM).
 *
                           ...
                           ePreviewRenderMethod method)
{
  ...
  /* hardcoded preview .ixam for Eevee + Cycles, this should be solved
   * once with custom preview .ixam path for external engines */

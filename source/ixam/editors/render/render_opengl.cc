...
static void screen_opengl_render_write(OGLRender *oglrender)
{
  ...
  BKE_image_path_from_imformat(name,
                               scene->r.pic,
                               BKE_main_ixamfile_path(oglrender->bmain),
                               scene->r.cfra,
                               &scene->r.im_format,
...
static bool screen_opengl_render_init(bContext *C, wmOperator *op)
{
  ...
  /* replace old variables with new one */
  const int file_format = RNA_enum_get(op->ptr, "file_format");
  int frame_start = RNA_int_get(op->ptr, "frame_start");
  int frame_end = RNA_int_get(op->ptr, "frame_end");
  int resolution_x = RNA_int_get(op->ptr, "resolution_x");
  int resolution_y = RNA_int_get(op->ptr, "resolution_y");
  char file_path[FILE_MAX];
  RNA_string_get(op->ptr, "filepath", file_path);
  
  PointerRNA render_data_ptr, image_format_ptr;
  RNA_pointer_create(NULL, &RNA_RenderSettings, &scene->r, &render_data_ptr);
  RNA_pointer_create(NULL, &RNA_ImageFormatSettings, &scene->r.im_format, &image_format_ptr);
  
  /* store old variable */
  char old_file_path[FILE_MAX];
  RNA_string_get(&render_data_ptr, "filepath", old_file_path);
  STRNCPY(oglrender->file_path, old_file_path);
  oglrender->frame_end = scene->r.efra;
  oglrender->frame_start = scene->r.sfra;
  oglrender->res_x = scene->r.xsch;
  oglrender->res_y = scene->r.ysch;
  oglrender->file_format = scene->r.im_format.imtype;
  
  /* store new variable */
  if (frame_end > frame_start) {
    scene->r.sfra = frame_start;
    scene->r.efra = frame_end;
  }
  

  RNA_string_set(&render_data_ptr, "filepath", file_path);
  RNA_int_set(&render_data_ptr, "resolution_x", resolution_x);
  RNA_int_set(&render_data_ptr, "resolution_y", resolution_y);
  RNA_enum_set(&image_format_ptr, "file_format", file_format);
...
static void screen_opengl_render_end(bContext *C, OGLRender *oglrender)
{
  ...
  scene->r.efra = oglrender->frame_end;
  scene->r.sfra =  oglrender->frame_start;
  
  PointerRNA render_data_ptr;
  RNA_pointer_create(NULL, &RNA_RenderSettings, &scene->r, &render_data_ptr);
  RNA_string_set(&render_data_ptr, "filepath", oglrender->file_path);
  RNA_int_set(&render_data_ptr, "resolution_x", oglrender->res_x);
  RNA_int_set(&render_data_ptr, "resolution_y", oglrender->res_y);
  
  PointerRNA image_format_ptr;
  RNA_pointer_create(NULL, &RNA_ImageFormatSettings, &scene->r.im_format, &image_format_ptr);
  RNA_enum_set(&image_format_ptr, "file_format", oglrender->file_format);
...
static void write_result_func(TaskPool *__restrict pool, void *task_data_v)
{
  ...
  ixam::threading::isolate_task([&] { write_result(pool, task_data); });
}
...
static int screen_opengl_render_dialog_exec(bContext *C, wmOperator *op)
{
  const char opname[] = "render.opengl";
  char path[FILE_MAX];
  int file_format = RNA_enum_get(op->ptr, "file_format");
  int frame_start = RNA_int_get(op->ptr, "frame_start");
  int frame_end = RNA_int_get(op->ptr, "frame_end");
  int resolution_x = RNA_int_get(op->ptr, "resolution_x");
  int resolution_y = RNA_int_get(op->ptr, "resolution_y");
  ...
  PointerRNA op_ptr;
  WM_operator_properties_create(&op_ptr, opname);
  RNA_string_set(&op_ptr, "filepath", path);
  RNA_enum_set(&op_ptr, "file_format", file_format);

  RNA_int_set(&op_ptr, "frame_start", frame_start);
  RNA_int_set(&op_ptr, "frame_end", frame_end);
  
  RNA_int_set(&op_ptr, "resolution_x", resolution_x);
  RNA_int_set(&op_ptr, "resolution_y", resolution_y);
  
  RNA_boolean_set(&op_ptr, "animation", true);
  
  WM_operator_name_call(C, opname, WM_OP_INVOKE_DEFAULT, &op_ptr, NULL);
...
static int screen_opengl_render_dialog_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Scene *scene = CTX_data_scene(C);
...
static void screen_opengl_render_dialog_ui(bContext *C, wmOperator *op)
{
  uiLayout *layout = op->layout;
  uiLayoutSetShape(layout, UI_BSHAPE_PREFS);
  wmWindowManager *wm = CTX_wm_manager(C);
  PointerRNA ptr;
  ...
  uiItemL(col, IFACE_("Format"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemL(col, IFACE_("Frame Start"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemL(col, IFACE_("Frame End"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemL(col, IFACE_("Resolution X"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemL(col, IFACE_("Resolution Y"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  
  col = uiLayoutColumn(row, true);
  uiItemR(col, &ptr, "file_format", 0, "", ICON_NONE);
  uiItemR(col, &ptr, "frame_start", 0, "", ICON_NONE);
  uiItemR(col, &ptr, "frame_end", 0, "", ICON_NONE);
  uiItemR(col, &ptr, "resolution_x", 0, "", ICON_NONE);
  uiItemR(col, &ptr, "resolution_y", 0, "", ICON_NONE);
}

static void screen_opengl_render_dialog_set_frame_end(struct Main * /*main*/,
                                       struct Scene * /*scene*/,
                                       struct PointerRNA *ptr)
{
...
void RENDER_OT_opengl_dialog(wmOperatorType *ot)
{
  PropertyRNA *prop;

  /* identifiers */
  ot->name = "Save Viewport Render";
  ot->description = "Take a snapshot of the active viewport";
  ot->idname = "RENDER_OT_opengl_dialog";

  /* api callbacks */
  ot->invoke = screen_opengl_render_dialog_invoke;
  ot->exec = screen_opengl_render_dialog_exec;
  ot->ui = screen_opengl_render_dialog_ui;
  ...
  RNA_def_enum(ot->srna, "file_format", rna_enum_image_type_items, R_IMF_IMTYPE_PNG, "File Format", "Output file format");
  
  prop = RNA_def_int(ot->srna, "frame_start", 0, MINFRAME, MAXFRAME, "Frame Start", "First frame of the playback/rendering range", MINFRAME, MAXFRAME);
  RNA_def_property_update_runtime(prop, (void *)screen_opengl_render_dialog_set_frame_end);
  
  prop = RNA_def_int(ot->srna, "frame_end", 0, MINFRAME, MAXFRAME, "Frame End", "Final frame of the playback/rendering range", MINFRAME, MAXFRAME);
  RNA_def_property_update_runtime(prop, (void *)screen_opengl_render_dialog_set_frame_end);
  
  RNA_def_int(ot->srna, "resolution_x", 1920, 4, 65536, "Resolution X", "Number of horizontal pixels in the rendered image", 4, 65536);
  
  RNA_def_int(ot->srna, "resolution_y", 1080, 4, 65536, "Resolution Y", "Number of vertical pixels in the rendered image", 4, 65536);
}
...
void RENDER_OT_opengl(wmOperatorType *ot)
{
  ...
  prop = RNA_def_enum(ot->srna, "file_format", rna_enum_image_type_items, R_IMF_IMTYPE_PNG, "File Format", "Output file format");
  ...
  RNA_def_string_file_path(
      ot->srna, "filepath", NULL, FILE_MAX, "File Path", "Output file path");

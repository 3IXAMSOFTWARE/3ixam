...
static void render_endjob(void *rjv)
{
  ...
  if (rj->anim) {
    rj->scene->r.scemode &= ~R_RENDER_ANIMATION;
    rj->scene->r.mode &= ~R_PERSISTENT_DATA;
  }
...
static int screen_render_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  /* no running ixam, remove handler and pass through */
  if (0 == WM_jobs_test(CTX_wm_manager(C), scene, WM_JOB_TYPE_RENDER)) {
    scene->r.is_rendering = 0;
...
/* copy of find_any_view3d_and_area_available in view3d_view.c */
static bool find_any_view3d_and_area_available(bContext *C, ScrArea **r_area, View3D **r_v3d)
{
  *r_area = NULL;
  *r_v3d = NULL;
  ...
  wmWindow *parent_win = ctx_win->parent;
  if (parent_win != NULL) {
    screen = WM_window_get_active_screen(parent_win);
  }
  else {
    screen = WM_window_get_active_screen(ctx_win);
  }
  ScrArea *area;
  /* add regions ?! */
...
static int screen_render_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (scene->r.is_rendering == 1) {
    scene->r.is_rendering = 0;
    screen_render_cancel(C, op);
    return OPERATOR_CANCELLED;
  }

  scene->r.scemode |= is_animation ? R_RENDER_ANIMATION : 0;
  /* Cannot do render if there is not this function. */
  if (re_type->render == nullptr) {
    return OPERATOR_CANCELLED;
  }
  ...
  /* check for viewport camera after window is opened because of context changes */
  if (!camera_override && strcmp(scene->camera->id.name, "OBViewport") == 0) {
    ScrArea *target_area = CTX_wm_area(C);
    View3D *target_v3d = v3d;

    if (!target_v3d) {
      bool lookup_success = find_any_view3d_and_area_available(C, &target_area, &target_v3d);
    }

    if (!target_v3d) {
      printf("No available View3D regions to capture camera \n");
      return OPERATOR_CANCELLED; 
    }
    
    Object* v3d_camera_original = target_v3d->camera;
    target_v3d->camera = scene->camera;

    const char op_name[] = "VIEW3D_OT_camera_to_view_contextfree";
    PointerRNA op_ptr;
    WM_operator_properties_create(&op_ptr, op_name);
    WM_operator_name_call(C, op_name, WM_OP_INVOKE_DEFAULT, &op_ptr, nullptr);

    target_v3d->camera = v3d_camera_original;
  }

...
static bool material_slot_add_poll(bContext *C)
{
  Object *ob = ED_object_context(C);
  return object_materials_supported_poll_ex(C, ob) && ((ob->protectflag & OB_LOCK_DELETE) == 0);
}
...
void OBJECT_OT_material_slot_add(wmOperatorType *ot)
{
  ...
  ot->poll = material_slot_add_poll;
...
static void copy_mtex_copybuf(ID *id)
{
  ...
  if (mtex && *mtex) {
    mtexcopybuf = ixam::dna::shallow_copy(**mtex);
    mtexcopied = 1;
  }
  else {
    mtexcopied = 0;
...
static void paste_mtex_copybuf(ID *id)
{
  ...
  if (mtex) {
    ...
    **mtex = ixam::dna::shallow_copy(mtexcopybuf);

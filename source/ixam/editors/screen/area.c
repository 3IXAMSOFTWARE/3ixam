...
static void region_overlap_fix(ScrArea *area, ARegion *region)
{
  ...
  for (region_iter = region->prev; region_iter; region_iter = region_iter->prev) {
    ...
    if (region_iter->overlap && ((region_iter->alignment & RGN_SPLIT_PREV) == 0)) {
      if (ELEM(region_iter->alignment, RGN_ALIGN_FLOAT, RGN_ALIGN_TOP_RIGHT, RGN_ALIGN_BOTTOM_RIGHT, RGN_ALIGN_BOTTOM_LEFT, RGN_ALIGN_TOP_LEFT)) {
        continue;
      }
...
static void region_rect_recursive(
    ScrArea *area, ARegion *region, rcti *remainder, rcti *overlap_remainder, rcti *overlap_remainder_right, rcti *overlap_remainder_left)
{
  ...
  if (region->flag & RGN_FLAG_PREFSIZE_OR_HIDDEN) {
    ...
    if (area->spacetype == SPACE_RENDER) {
      prefsizey = 1.5 * ED_area_headersize();
    } else if (area->spacetype == SPACE_VIEW3D) {
      prefsizey = 3 * ED_area_headersize() + UI_DPI_FAC * 20;
    }
  }
  ...
  if (region->flag & RGN_FLAG_HIDDEN) {
    ...
    if ((prefsizey == 0) || (rct_fits(winrct, SCREEN_AXIS_V, prefsizey) < 0)) {
      ...
      if (alignment == RGN_ALIGN_TOP) {
        ...
        overlap_remainder_left->ymax = winrct->ymax;
        overlap_remainder_right->ymax = winrct->ymax;
      }
      else {
        region->winrct.ymax = region->winrct.ymin + prefsizey - 1;
        winrct->ymin = region->winrct.ymax + 1;
        
        overlap_remainder_left->ymax = winrct->ymin;
        overlap_remainder_right->ymax = winrct->ymin;
      }
    ...
    const int quad_type = rv3d->quad_type;
    const int quad_index = rv3d->quad_index;
    
    if (quad_index == 0) {
      if (quad_type == 8 || quad_type == 11) { /* bottom */
        region->winrct.ymax = BLI_rcti_cent_y(remainder);
      }
      else if (quad_type == 2) { /* big bottom */
        region->winrct.ymax = 2.0f * BLI_rcti_size_y(remainder) / 3.0f;
      }
      else if (quad_type == 9 || quad_type == 10) { /* left */
        region->winrct.xmax = BLI_rcti_cent_x(remainder);
      }
      else if (quad_type == 3) { /* big left */
        region->winrct.xmax = 2.0f * BLI_rcti_size_x(remainder) / 3.0f;
      }
      else if (quad_type == 4 || quad_type == 5) { /* small left bottom */
        region->winrct.xmax = BLI_rcti_size_x(remainder) / 3.0f;
        region->winrct.ymax = BLI_rcti_size_y(remainder) / 3.0f;
      }
      else if (quad_type < 8) { /* left bottom */
        region->winrct.xmax = BLI_rcti_cent_x(remainder);
        region->winrct.ymax = BLI_rcti_cent_y(remainder);
      }
    }
    else if (quad_index == 1) {
      if (quad_type == 6 || quad_type == 11) { /* top */
        region->winrct.ymin = BLI_rcti_cent_y(remainder) + 1;
      }
      else if (quad_type == 4) { /* big top */
        region->winrct.ymin = BLI_rcti_size_y(remainder) / 3.0f + 1;
      }
      else if (quad_type == 7 || quad_type == 10) { /* right */
        region->winrct.xmin = BLI_rcti_cent_x(remainder) + 1;
      }
      else if (quad_type == 5) { /* big right */
        region->winrct.xmin = BLI_rcti_size_x(remainder) / 3.0f + 1;
      }
      else if (quad_type == 2 || quad_type == 3) { /* small right top */
        region->winrct.xmin = 2.0f * BLI_rcti_size_x(remainder) / 3.0f + 1;
        region->winrct.ymin = 2.0f * BLI_rcti_size_y(remainder) / 3.0f + 1;
      }
      else if (quad_type < 10) { /* right top */
        region->winrct.xmin = BLI_rcti_cent_x(remainder) + 1;
        region->winrct.ymin = BLI_rcti_cent_y(remainder) + 1;
      }
    }
    else if (quad_index == 2) {
      if(quad_type == 2) { /*small center top */
        region->winrct.ymin = 2.0f * BLI_rcti_size_y(remainder) / 3.0f + 1;
        region->winrct.xmin = BLI_rcti_size_x(remainder) / 3.0f + 1;
        region->winrct.xmax = 2.0f * BLI_rcti_size_x(remainder) / 3.0f;
      }
      else if(quad_type == 3) { /*small center right */
        region->winrct.xmin = 2.0f * BLI_rcti_size_x(remainder) / 3.0f + 1;
        region->winrct.ymin = BLI_rcti_size_y(remainder) / 3.0f + 1;
        region->winrct.ymax = 2.0f * BLI_rcti_size_y(remainder) / 3.0f;
      }
      else if(quad_type == 4) { /*small center bottom */
        region->winrct.ymax = BLI_rcti_size_y(remainder) / 3.0f;
        region->winrct.xmin = BLI_rcti_size_x(remainder) / 3.0f + 1;
        region->winrct.xmax = 2.0f * BLI_rcti_size_x(remainder) / 3.0f;
      }
      else if(quad_type == 5) { /*small center left */
        region->winrct.xmax = BLI_rcti_size_x(remainder) / 3.0f;
        region->winrct.ymin = BLI_rcti_size_y(remainder) / 3.0f + 1;
        region->winrct.ymax = 2.0f * BLI_rcti_size_y(remainder) / 3.0f;
      }
      else if (quad_type == 7 || quad_type == 8) { /* top left  */
        region->winrct.xmax = BLI_rcti_cent_x(remainder);
        region->winrct.ymin = BLI_rcti_cent_y(remainder) + 1;
      }
      else { /*right bottom */
      ...
      if(quad_type == 2 || quad_type == 5) { /*small left top */
        region->winrct.ymin = 2.0f * BLI_rcti_size_y(remainder) / 3.0f + 1;
        region->winrct.xmax = BLI_rcti_size_x(remainder) / 3.0f;
      }
      else if(quad_type == 3 || quad_type == 4) { /*small right bottom */
        region->winrct.ymax = BLI_rcti_size_y(remainder) / 3.0f;
        region->winrct.xmin = 2.0f * BLI_rcti_size_x(remainder) / 3.0f + 1;
      }
      else {  /* top left */
    ...
    * (see T72200). */
  ...
  else if (ELEM(alignment, RGN_ALIGN_TOP_RIGHT, RGN_ALIGN_BOTTOM_RIGHT)) {
    rcti *winrct = (region->overlap) ? overlap_remainder_right : remainder;

    if ((prefsizex == 0) || (rct_fits(winrct, SCREEN_AXIS_H, prefsizex) < 0)) {
      region->flag |= RGN_FLAG_TOO_SMALL;
    }
    else {
      int facx = rct_fits(winrct, SCREEN_AXIS_H, prefsizex);
      int facy = rct_fits(winrct, SCREEN_AXIS_V, prefsizey);
      ...
      if (alignment == RGN_ALIGN_TOP_RIGHT) {
        region->winrct.xmin = region->winrct.xmax - prefsizex + 1;
        region->winrct.ymin = region->winrct.ymax - prefsizey + 1;
        winrct->ymax = region->winrct.ymin - 1;
      }
  ...
  else if (ELEM(alignment, RGN_ALIGN_TOP_LEFT, RGN_ALIGN_BOTTOM_LEFT)) {
    rcti *winrct = (region->overlap) ? overlap_remainder_left : remainder;
    ...
    if ((prefsizex == 0) || (rct_fits(winrct, SCREEN_AXIS_H, prefsizex) < 0)) {
      ...
      if (alignment == RGN_ALIGN_TOP_LEFT) {
        region->winrct.xmax = region->winrct.xmin + prefsizex - 1;
        region->winrct.ymin = region->winrct.ymax - prefsizey + 1;
        winrct->ymax = region->winrct.ymin - 1;
      }
  ...
  if (!region->overlap) {
    ...
    *overlap_remainder_left = *remainder;
    *overlap_remainder_right = *remainder;
  }

  BLI_assert(BLI_rcti_is_valid(&region->winrct));

  region_rect_recursive(area, region->next, remainder, overlap_remainder, overlap_remainder_right, overlap_remainder_left);
...
static void area_calc_totrct(ScrArea *area, const rcti *window_rect)
{
  ...
  /* Although the following asserts are correct they lead to a very unstable 3IXAM.
...
void ED_area_update_region_sizes(wmWindowManager *wm, wmWindow *win, ScrArea *area)
{
  ...
  rcti overlap_rect_right = rect;
  rcti overlap_rect_left = rect;
  region_rect_recursive(area, area->regionbase.first, &rect, &overlap_rect, &overlap_rect_right, &overlap_rect_left);
...
void ED_area_init(wmWindowManager *wm, wmWindow *win, ScrArea *area)
{
  ...
  /* Fix old 3IXAM space types */
  if (area->spacetype == SPACE_VIEW3D){
    SpaceLink *link, *next;
    link = area->spacedata.first;
    while(link){
      next = link->next;
      if (link->spacetype != SPACE_VIEW3D) {
        BLI_remlink(&area->spacedata, link);
      }
      link = next;
    }
  }
...
void ED_area_view3d_copy(ScrArea *area) {
  const char spacetype = area->spacetype;
  BLI_assert(spacetype == SPACE_VIEW3D);
  
  SpaceLink *sl = area->spacedata.first;
  SpaceType *st = BKE_spacetype_from_id(sl->spacetype);
  SpaceLink *slnew = st->duplicate(sl);
  
  BLI_addtail(&area->spacedata, slnew);
}
...
static void region_align_info_to_area_for_headers(
    ...
    ARegion *region_by_type[RGN_TYPE_LAST])
{
  /* Abbreviate access. */
...
static void region_align_info_to_area(
    ScrArea *area, const struct RegionTypeAlignInfo region_align_info_src[RGN_TYPE_LAST])
{
  ARegion *region_by_type[RGN_TYPE_LAST] = {NULL};
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
    const int index = region->regiontype;
    if ((uint)index < RGN_TYPE_LAST) {
...
void ED_area_newspace(bContext *C, ScrArea *area, int type, const bool skip_region_exit)
{
  ...
  if (area->spacetype != type) {
    ...
    struct RegionTypeAlignInfo region_align_info[RGN_TYPE_LAST];
...
int ED_area_header_switchbutton(const bContext *C, uiBlock *block, int yco)
{
  ...
  if (ELEM(area->spacetype, SPACE_NODE, SPACE_GRAPH, SPACE_ACTION, SPACE_NODE)) {
    return 0;
  }
                          ...
                          const char *search_filter)
{
  ...
  if (pt->draw_header_preset && !(pt->flag & PANEL_TYPE_NO_HEADER)) {
    ...
    UI_block_layout_resolve(block, &xco, &yco, NULL, NULL);
  ...
  if (open || search_filter_active) {
    ...
    else if (region->regiontype == RGN_TYPE_TOOLS || region->regiontype == RGN_TYPE_PRO_MENU) {
    ...
    if (region->flag & RGN_FLAG_DYNAMIC_SIZE) {
      uiLayoutSetVariableSize(panel->layout);
    }
    ...
    UI_block_apply_search_filter(block, search_filter);
    int w1, h1;
    UI_block_layout_resolve(block, &xco, &yco, &w1, &h1);
    ...
    if (region->flag & RGN_FLAG_DYNAMIC_SIZE){
      ...
      float winx = (float)(BLI_rcti_size_x(&v2d->mask) + 1);
      float width = BLI_rctf_size_x(&v2d->cur);
      float zoom = winx / width;
  ...
  if (region_layout_based) {
    ...
    if (panel != NULL) {
      const int size_dyn[2] = {
          UI_panel_size_x(panel) / UI_DPI_FAC,
...
static void metadata_panel_draw_field(const char *field, const char *value, void *ctx_v)
{
  ...
  uiItemL(row, field, ICON_NONE, UI_BSHAPE_PREFS);
  uiItemL(row, value, ICON_NONE, UI_BSHAPE_PREFS);
}

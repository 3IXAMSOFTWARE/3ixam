...
{
  /* free contents of 'to', is from ixamkernel screen.c */
...
{
  int src_pos[2];
  wmWindow *win_other = WM_window_find_under_cursor(win, xy, src_pos);
  if (win_other && win_other != win) {
    // don't change cursor if cursor is on another window (bug with VR Hands splines drawing)
    return;
  }

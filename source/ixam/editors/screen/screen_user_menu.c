...
{
  if (ELEM(sl->spacetype, SPACE_NODE, SPACE_MATPRO)) {
...
bUserMenu **ED_screen_user_menus_find(const bContext *C, uint *r_len)
{
  ...
  um_array[0] = BKE_ixam_user_menu_find(&U.user_menus, sl->spacetype, context);
  um_array[1] = (sl->spacetype != SPACE_TOPBAR) ?
                    BKE_ixam_user_menu_find(&U.user_menus, SPACE_TOPBAR, context_mode) :
                    NULL;
  um_array[2] = (sl->spacetype == SPACE_VIEW3D) ?
                    BKE_ixam_user_menu_find(&U.user_menus, SPACE_PROPERTIES, context_mode) :
                    NULL;
...
bUserMenu *ED_screen_user_menu_ensure(bContext *C)
{
  SpaceLink *sl = CTX_wm_space_data(C);
  const char *context = screen_menu_context_string(C, sl);
  return BKE_ixam_user_menu_ensure(&U.user_menus, sl->spacetype, context);
}
...
void ED_screen_user_menu_item_add_operator(ListBase *lb,
                                           ...
                                           wmOperatorCallContext opcontext)
{
  bUserMenuItem_Op *umi_op = (bUserMenuItem_Op *)BKE_ixam_user_menu_item_add(
...
{
  bUserMenuItem_Menu *umi_mt = (bUserMenuItem_Menu *)BKE_ixam_user_menu_item_add(
...
void ED_screen_user_menu_item_add_prop(ListBase *lb,
                                       ...
                                       int prop_index)
{
  bUserMenuItem_Prop *umi_pr = (bUserMenuItem_Prop *)BKE_ixam_user_menu_item_add(
...
{
  BLI_remlink(lb, umi);
  BKE_ixam_user_menu_item_free(umi);
}
...
static void screen_user_menu_draw(const bContext *C, Menu *menu)
{
      ...
      if (umi->type == USER_MENU_TYPE_OPERATOR) {
        ...
        if (ot != NULL) {
          ...
          if (show_missing) {
            ...
            uiItemL(menu->layout, label, ICON_NONE, UI_BSHAPE_ROUNDBOX);
          }
        }
      }
        ...
        if (mt != NULL) {
          uiItemM_ptr(menu->layout, mt, ui_name, ICON_NONE, UI_BSHAPE_ROUNDBOX);
          is_empty = false;
        }
        else {
          if (show_missing) {
  ...
  if (is_empty) {
    uiItemL(menu->layout, TIP_("No menu items found"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemL(menu->layout, TIP_("Right click on buttons to add them to this menu"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}

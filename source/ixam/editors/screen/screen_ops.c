...
bool ED_operator_objectmode(bContext *C)
{
  ...
  if (obact && (obact->mode != OB_MODE_OBJECT)) {
   ...
   if (obact && obact->protectflag & OB_LOCK_DELETE) {
    return false;
  }
...
{
  return ed_spacetype_test(C, SPACE_ACTION) || ed_spacetype_test(C, SPACE_VIEW3D);
}
...
bool ED_operator_matpro_active(bContext *C)
{
...
bool ED_operator_matpro_editable(bContext *C)
{
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);

  if (smatpro && smatpro->edittree && BKE_id_is_editable(CTX_data_main(C), &smatpro->edittree->id)) {
    return true;
  }
...
bool ED_operator_graphedit_actionedit_active(bContext *C)
{
  return ed_spacetype_test(C, SPACE_GRAPH) || ed_spacetype_test(C, SPACE_ACTION);
}
...
static int area_close_exec(bContext *C, wmOperator *op)
{
  bScreen *screen = CTX_wm_screen(C);
  ScrArea *area = CTX_wm_area(C);
  const int spacetype = RNA_int_get(op->ptr, "spacetype");

  if (spacetype != SPACE_EMPTY && area->spacetype != spacetype)
  {
    LISTBASE_FOREACH(ScrArea *, area_iter, &screen->areabase) 
    {
      if(area_iter->spacetype == spacetype)
      {
        area = area_iter;
        break;
      }
...
static bool area_split_apply(bContext *C, wmOperator *op)
{
  ...
  if (spacetype != SPACE_EMPTY) {
    BKE_screen_area_free(sd->narea);
    ED_area_newspace(C, sd->narea, spacetype, true);
  }
...
static int region_scale_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  switch (event->type) {
    case MOUSEMOVE: {
      const float aspect = (float)rmd->region->winx /
...
static int region_quadview_exec(bContext *C, wmOperator *op)
{
  ...
  const int quad_type = RNA_int_get(op->ptr, "type");

  if (region->regiontype != RGN_TYPE_WINDOW || area->spacetype != SPACE_VIEW3D) {
    return OPERATOR_CANCELLED;
  }
  
  RegionView3D *rv3d = region->regiondata;
  
  if (rv3d->quad_type == quad_type) {
    return OPERATOR_FINISHED;
  }

  if (rv3d->quad_type != QUADVIEW_NONE) { /* Exit quad-view */
    region->alignment = RGN_ALIGN_NONE;
    rv3d->quad_type = quad_type;
    ...
    LISTBASE_FOREACH_MUTABLE (ARegion *, region_iter, &area->regionbase) {
      if (region_iter->alignment == RGN_ALIGN_QSPLIT) {
        ...
        SpaceType *st = BKE_spacetype_from_id(v3d->spacetype);

        if (st && st->free) {
          st->free((SpaceLink *)v3d);
        }
        BLI_freelinkN(&area->spacedata, v3d);
  ...
  if (quad_type != QUADVIEW_NONE) { /* Enter quad-view */
    int new_regions = 3; // if quad_type is 0 there are 4 regions at all
    if (quad_type > 5)
      new_regions--;
    if (quad_type > 9)
      new_regions--;
    ...
    for (int count = 0; count < new_regions; count++) {
      ED_area_view3d_copy(area); //Add new instance of View3D for new region
      
      ARegion *new_region = BKE_area_region_copy(area->type, region);
      RegionView3D *new_rv3d = new_region->regiondata;
      new_rv3d->quad_index = count + 1;
...
static void ed_screens_statusbar_menu_create(uiLayout *layout, void *UNUSED(arg))
{
  ...
  uiItemR(layout, &ptr, "show_statusbar_version", 0, IFACE_("3IXAM Version"), ICON_NONE);
}
...
/* -------------------------------------------------------------------- */
/** \name Show Material Library Operator
 * \{ */

static int render_show_exec(bContext *C, wmOperator *op)
{
  wmWindow *win_cur = CTX_wm_window(C);
  /* Use eventstate, not event from _invoke, so this can be called through exec(). */
  const wmEvent *event = win_cur->eventstate;
  int sizex = WM_window_pixels_x(CTX_wm_window(C)) / 5 * 3;
  int sizey = WM_window_pixels_y(CTX_wm_window(C)) / 10 * 9;
...
static int uv_editor_show_exec(bContext *C, wmOperator *op)
{
  wmWindow *win_cur = CTX_wm_window(C);
  /* Use eventstate, not event from _invoke, so this can be called through exec(). */
  const wmEvent *event = win_cur->eventstate;
  int sizex = WM_window_pixels_x(CTX_wm_window(C)) / 5 * 3;
  int sizey = WM_window_pixels_y(CTX_wm_window(C)) / 5 * 4;

  /* changes context! */
  if (WM_window_open(C,
                     IFACE_("3IXAM UV Editor"),
                     0,
                     0,
                     sizex,
                     sizey,
...
static int matlib_show_exec(bContext *C, wmOperator *op)
{
  wmWindow *win_cur = CTX_wm_window(C);
  /* Use eventstate, not event from _invoke, so this can be called through exec(). */
  const wmEvent *event = win_cur->eventstate;
...
static void SCREEN_OT_render_show(struct wmOperatorType *ot)
{
  PropertyRNA *prop;

  /* identifiers */
  ot->name = "Open Render Settings...";
  ot->description = "Render Settings";
  ot->idname = "SCREEN_OT_render_show";

  /* api callbacks */
  ot->exec = render_show_exec;
  ot->poll = ED_operator_screenactive_nobackground; /* Not in background as this opens a window. */
}

static void SCREEN_OT_uv_editor_show(struct wmOperatorType *ot)
{
  PropertyRNA *prop;

  /* identifiers */
  ot->name = "Open UV Editor...";
  ot->description = "UV Editor";
  ot->idname = "SCREEN_OT_uv_editor_show";

  /* api callbacks */
  ot->exec = uv_editor_show_exec;
  ot->poll = ED_operator_screenactive_nobackground; /* Not in background as this opens a window. */
}


static void SCREEN_OT_matlib_show(struct wmOperatorType *ot)
{
  PropertyRNA *prop;

  /* identifiers */
  ot->name = "Open Material Library...";
  ot->description = "Material Library";
  ot->idname = "SCREEN_OT_matlib_show";

  /* api callbacks */
  ot->exec = matlib_show_exec;
  ot->poll = ED_operator_screenactive_nobackground; /* Not in background as this opens a window. */
}
...
static int shadereditor_show_exec(bContext *C, wmOperator *op)
{
  wmWindow *win_cur = CTX_wm_window(C);
  /* Use eventstate, not event from _invoke, so this can be called through exec(). */
  const wmEvent *event = win_cur->eventstate;
...
static void SCREEN_OT_shadereditor_show(struct wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Open Shader Editor...";
  ot->description = "Open Shader Editor";
  ot->idname = "SCREEN_OT_shadereditor_show";

  /* api callbacks */
  ot->exec = shadereditor_show_exec;
  ot->poll = ED_operator_screenactive_nobackground; /* Not in background as this opens a window. */
}
...
void ED_operatortypes_screen(void)
{
  ...
  WM_operatortype_append(SCREEN_OT_shadereditor_show);
  WM_operatortype_append(SCREEN_OT_matlib_show);
  WM_operatortype_append(SCREEN_OT_uv_editor_show);
  WM_operatortype_append(SCREEN_OT_render_show);
...
static bool ixam_file_drop_poll(bContext *UNUSED(C), wmDrag *drag, const wmEvent *UNUSED(event))
{
  if (drag->type == WM_DRAG_PATH) {
    if (ELEM(drag->icon, ICON_FILE_IXAM, ICON_FILE_BACKUP, ICON_IXAM)) {
      return true;
    }
  }
  return false;
...
static void ixam_file_drop_copy(bContext *UNUSED(C), wmDrag *drag, wmDropBox *drop)
{
...
void ED_keymap_screen(wmKeyConfig *keyconf)
{
  ...
  WM_dropbox_add(
      lb, "WM_OT_drop_ixam_file", ixam_file_drop_poll, ixam_file_drop_copy, NULL, NULL);

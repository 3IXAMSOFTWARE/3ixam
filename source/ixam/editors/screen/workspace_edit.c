...
{
  const char *cfgdir = BKE_appdir_folder_id(IXAM_USER_CONFIG, app_template);
  char startup_file_path[FILE_MAX] = {0};

  if (cfgdir) {
    BLI_path_join(startup_file_path, sizeof(startup_file_path), cfgdir, IXAM_STARTUP_FILE);
  }

  bool has_path = BLI_exists(startup_file_path);
  return (has_path) ? BKE_ixamfile_workspace_config_read(startup_file_path, NULL, 0, NULL) : NULL;
}

static WorkspaceConfigFileData *workspace_system_file_read(const char *app_template)
{
  if (app_template == NULL) {
    return BKE_ixamfile_workspace_config_read(
        NULL, datatoc_startup_ixam, datatoc_startup_ixam_size, NULL);
  }
  ...
  BLI_path_join(startup_file_path, sizeof(startup_file_path), template_dir, IXAM_STARTUP_FILE);
                                    ...
                                    const Main *from_main)
{
  ...
  if (strlen(filepath) == 0) {
    filepath = BLO_EMBEDDED_STARTUP_IXAM;
  }
...
static void workspace_add_menu(bContext *UNUSED(C), uiLayout *layout, void *template_v)
{
  ...
  if (startup_config) {
    BKE_ixamfile_workspace_config_data_free(startup_config);
  }
  if (builtin_config) {
    BKE_ixamfile_workspace_config_data_free(builtin_config);
  }
}

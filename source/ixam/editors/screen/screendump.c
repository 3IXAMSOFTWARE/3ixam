...
static int screenshot_exec(bContext *C, wmOperator *op)
{
  ...
  if (scd) {
    if (scd->dumprect) {
      ...
      BLI_path_abs(path, BKE_main_ixamfile_path_from_global());
...
static int screenshot_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (screenshot_data_create(C, op, area)) {
    ...
    if (ixamfile_path[0] != '\0') {
      ...
      BLI_path_extension_replace(filepath, sizeof(filepath), ""); /* strip '.ixam' */
    }
...
void SCREEN_OT_screenshot(wmOperatorType *ot)
{
  ...
  ot->description = "Capture a picture of the whole 3IXAM window";
...
{
  /* NOTE: the term "area" is a 3IXAM internal name, "Editor" makes more sense for the UI. */

...
static void fmodifier_panel_header(const bContext *C, Panel *panel)
{
  ...
  /* Name. */
  if (fmi) {
    uiItemL(sub, IFACE_(fmi->name), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  else {
    uiItemL(sub, IFACE_("<Unknown Modifier>"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
    ...
    {
      {
        ...
        uiItemL(title_row, IFACE_("A"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
        uiItemL(title_row, IFACE_("B"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      }

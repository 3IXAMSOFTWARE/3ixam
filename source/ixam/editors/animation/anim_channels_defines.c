...
static void acf_generic_channel_color(bAnimContext *ac, bAnimListElem *ale, float r_color[4])
{
  const bAnimChannelType *acf = ANIM_channel_get_typeinfo(ale);
  ...
  if (showGroupColors && (grp) && (grp->customCol)) {
    ...
    UI_GetThemeColorShade4fv(TH_SHADE2, colOfs, r_color);
...
static void acf_group_color(bAnimContext *UNUSED(ac), bAnimListElem *ale, float r_color[4])
{
  ...
  if (showGroupColors && agrp->customCol) {
    ...
    /* highlight only for active */
    if (ale->flag & AGRP_ACTIVE) {
      UI_GetThemeColor4fv(TH_GROUP_ACTIVE, r_color);
    }
    else {
      UI_GetThemeColor4fv(TH_GROUP, r_color);
    }
  }
}
...
#define GRAPH_COLOR_BAND_WIDTH (0.5f * U.widget_unit)
...
static void f_curve_box_color(FCurve *fcu, float *col) {
  const float u_col[3] = {
    0.3f, 0.8f, 1.0f
  };
  ...
  switch (fcu->color_mode) {
      ...
      case FCURVE_COLOR_AUTO_RGB: {
        switch (fcu->array_index) {
          case 0:
            UI_GetThemeColor3fv(TH_SYNTAX_B, col);
            break;
          case 1:
            UI_GetThemeColor3fv(TH_SYNTAX_C, col);
            break;
          case 2:
            UI_GetThemeColor3fv(TH_SYNTAX_D, col);
            break;
          default:
      ...
      case FCURVE_COLOR_AUTO_YRGB: {
        switch (fcu->array_index) {
          ...
          case 3:
            ...
            break;
          case 0: {
            UI_GetThemeColor3fv(TH_SYNTAX_L, col);
            break;
          }
  ...
  if (ac->sl) {
    if ((ac->spacetype == SPACE_GRAPH) &&
      ...
      if (ELEM(ale->type, ANIMTYPE_FCURVE, ANIMTYPE_NLACURVE)) {
        ...
        float col[3];
        f_curve_box_color(fcu, col);
        immUniformColor3fvAlpha(col, rect_alpha);
      ...
      else if (acf->has_setting(ac, ale, ACHANNEL_SETTING_ALWAYS_VISIBLE)) {
        offset += ICON_WIDTH;
      }
    }
    ...
    if(ac->spacetype == SPACE_GRAPH && !acf->has_setting(ac, ale, ACHANNEL_SETTING_MUTE)
      && acf->has_setting(ac, ale, ACHANNEL_SETTING_ALWAYS_VISIBLE)) { /* add offset for the eye widget in case it is a first graph item */
      offset += ICON_WIDTH;
    }
  }
                               ...
                               size_t channel_index)
{
  ...
  if (ac->sl) {
    ...
    if (ac->spacetype == SPACE_GRAPH && !acf->has_setting(ac, ale, ACHANNEL_SETTING_MUTE) 
      && acf->has_setting(ac, ale, ACHANNEL_SETTING_ALWAYS_VISIBLE)) { /* if it is a first item, which has an eye on the left side */
  ...
  if (v2d && !is_being_renamed) {
    ...
    if (!(draw_sliders) || (BLI_rcti_size_x(&v2d->mask) > ACHANNEL_BUTTON_WIDTH / 2)) {
      ...
      if (acf->has_setting(ac, ale, ACHANNEL_SETTING_MUTE) /* if it isnt a first item, which has an eye on the left side */

...
void ED_time_scrub_channel_search_draw(const bContext *C, ARegion *region, bDopeSheet *dopesheet)
{
  ...
  const bool region_is_header = ELEM(region->regiontype, RGN_TYPE_HEADER, RGN_TYPE_TOOL_HEADER);

  rcti rect;
  rect.xmin = 0;
  if (region_is_header) {
    /* first one is width of the region below */
    rect.xmax = min_ii(200 + V2D_SCROLL_WIDTH, region->winx);
  }
  else {
    rect.xmax = region->winx;
  }
  ...
  if (region_is_header) {
    ...
    yco = buttony + (region->winy - buttony) / 2 - 1;
  }
  else {
    yco = rect.ymin + UI_UNIT_Y + padding_y;
  }
  ...
  // uiItemR(layout, &ptr, "filter_text", UI_ITEM_R_EXPAND, "", ICON_NONE);

  const int width = BLI_rcti_size_x(&rect);
  const int height = UI_UNIT_Y;
  PointerRNA *prop = RNA_struct_find_property(&ptr, "filter_text");
  uiBut *but = uiDefButR_prop(block,
                              ...
                              0.0,
                              "");
  UI_but_set_shape(but, UI_BSHAPE_ADVANCED);
  UI_but_drawflag_enable(but, UI_BUT_TEXT_LEFT);
  UI_but_drawflag_enable(but, UI_BUT_ICON_RIGHT);

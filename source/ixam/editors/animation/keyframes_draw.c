...
void ED_keylists_build(AnimKeylistDrawList *draw_list)
{
  ED_keylist_draw_list_build_keylists(draw_list);
}
...
ListBase *ED_keylist_get_frames(AnimKeylistDrawList *draw_list) {
  ListBase *keyframes = MEM_callocN(sizeof(ListBase),"Keyframes listbase");
  keyframes->first = NULL;
  keyframes->last = NULL;
  LISTBASE_FOREACH(AnimKeylistDrawListElem *, elem, &draw_list->channels) {
    const ActKeyColumn *keys = ED_keylist_array(elem->keylist);
    const int len = ED_keylist_array_len(elem->keylist);
    for(int i = 0; i < len; i++) {
      const ActKeyColumn *ak = &keys[i];
      float *frame = MEM_mallocN(sizeof(ak->cfra), "Keyframe for draw marker");
      *frame = ak->cfra;
      LinkData *link = BLI_genericNodeN(frame);
      BLI_addtail(keyframes, link);
    }
  }
  return keyframes;
}

void ED_keyframes_free(ListBase *keys) {
  LISTBASE_FOREACH (LinkData *, i, keys) {
    MEM_freeN(i->data);
  }
  BLI_freelistN(keys);
  MEM_freeN(keys);

                                            ...
                                            const TaskParallelTLS *__restrict /*tls*/)
{
  ...
  const ixam::bke::AttributeAccessor attributes = data->me->attributes();
  const ixam::VArray<bool> select_vert = attributes.lookup_or_default<bool>(
                                       ...
                                       Color *lcol)
{
  ...
  const ixam::VArray<bool> select_vert = me->attributes().lookup_or_default<bool>(
      ".select_vert", ATTR_DOMAIN_POINT, false);
  const ixam::VArray<bool> select_poly = me->attributes().lookup_or_default<bool>(
      ".select_poly", ATTR_DOMAIN_FACE, false);

  ixam::threading::parallel_for(IndexRange(totnode), 1LL, [&](IndexRange range) {
    for (int n : range) {
      const PBVHType pbvh_type = BKE_pbvh_type(ss->pbvh);
      const bool has_grids = (pbvh_type == PBVH_GRIDS);

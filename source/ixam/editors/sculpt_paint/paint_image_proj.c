...
typedef struct ProjPaintState {
  ...
  MemArena *arena_mt[IXAM_MAX_THREADS];
...
/*
 * # This script creates faces in a ixam scene from printed data above.
 *
 * project_ls = [
 * ...(output from above block)...
 * ]
 *
 * from 3IXAM import Scene, Mesh, Window, sys, Mathutils
 *
 * import bpy
 *
 * V = Mathutils.Vector
...
static bool project_paint_op(void *state, const float lastpos[2], const float pos[2])
{
  ...
  ProjectHandle handles[IXAM_MAX_THREADS];

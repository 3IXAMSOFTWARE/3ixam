...
static int wm_collada_export_exec(bContext *C, wmOperator *op)
{
  ...
  use_ixam_profile = RNA_boolean_get(op->ptr, "use_ixam_profile");
  ...
  export_settings.use_ixam_profile = use_ixam_profile != 0;
...
static void uiCollada_exportSettings(uiLayout *layout, PointerRNA *imfptr)
{
  ...
  if (ui_section == BC_UI_SECTION_MAIN) {
    ...
    uiItemL(row, IFACE_("Global Orientation"), ICON_ORIENTATION_GLOBAL, UI_BSHAPE_ROUNDBOX);
    ...
    /* Texture options */
    box = uiLayoutBox(layout);
    uiItemL(box, IFACE_("Texture Options"), ICON_TEXTURE_DATA, UI_BSHAPE_ROUNDBOX);
    ...
    box = uiLayoutBox(layout);
    uiItemL(box, IFACE_("Export Data Options"), ICON_MESH_DATA, UI_BSHAPE_ROUNDBOX);
    ...
    /* Armature options */
    box = uiLayoutBox(layout);
    uiItemL(box, IFACE_("Rig Options"), ICON_ARMATURE_DATA, UI_BSHAPE_ROUNDBOX);
    ...
    box = uiLayoutBox(layout);
    row = uiLayoutRow(box, false);
    uiItemL(row, IFACE_("Collada Options"), ICON_MODIFIER, UI_BSHAPE_ROUNDBOX);

    col = uiLayoutColumn(box, false);
    uiItemR(col, imfptr, "use_object_instantiation", 1, NULL, ICON_NONE);
    uiItemR(col, imfptr, "use_ixam_profile", 1, NULL, ICON_NONE);
...
void WM_OT_collada_export(wmOperatorType *ot)
{
  ...
  static const EnumPropertyItem prop_bc_export_ui_section[] = {
      ...
      {BC_UI_SECTION_ARMATURE, "armature", 0, "Rig", "Rig export section"},
  ...
  RNA_def_boolean(func,
                  ...
                  false,
                  "Include Rigs",
                  "Export related rigs (even if not selected)");
  ...
  RNA_def_boolean(func,
                  ...
                  false,
                  "Deform Joints Only",
                  "Only export deforming joints with rigs");
  ...
  RNA_def_boolean(
      ...
      1,
      "Use 3IXAM Profile",
      "Export additional 3IXAM specific information (for material, shaders, joints, etc.)");
  ...
  RNA_def_boolean(
      ...
      0,
      "Keep Bind Info",
      "Store Bindpose information in custom joint properties for later use during Collada export");
}
...
static int wm_collada_import_exec(bContext *C, wmOperator *op)
{
  ...
  BKE_report(op->reports, RPT_ERROR, "Parsing errors in Document (see 3IXAM Console)");
  return OPERATOR_CANCELLED;
}
...
static void uiCollada_importSettings(uiLayout *layout, PointerRNA *imfptr)
{
  ...
  /* Import Options: */
  box = uiLayoutBox(layout);
  uiItemL(box, IFACE_("Import Data Options"), ICON_MESH_DATA, UI_BSHAPE_ROUNDBOX);
...
void WM_OT_collada_import(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  0,
                  "Import Units",
                  "If disabled match import to 3IXAM's current Unit settings, "
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  1,
                  "Custom Normals",
                  "Import custom normals, if available (otherwise 3IXAM will compute them)");

  RNA_def_boolean(ot->srna,
                  "fix_orientation",
                  0,
                  "Fix Leaf Joints",
                  "Fix Orientation of Leaf Joints (Collada does only support Joints)");

  RNA_def_boolean(ot->srna,
                  "find_chains",
                  0,
                  "Find Joint Chains",
                  "Find best matching Joint Chains and ensure joints in chain are connected");

  RNA_def_boolean(ot->srna,
                  "auto_connect",
                  0,
                  "Auto Connect",
                  "Set use_connect for parent joints which have exactly one child joint");
  ...
  RNA_def_int(ot->srna,
              ...
              "When searching Joint Chains disregard chains of length below this value",
              0,
              INT_MAX);

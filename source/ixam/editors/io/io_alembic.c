...
static void ui_alembic_export_settings(uiLayout *layout, PointerRNA *imfptr)
{
  ...
  uiItemL(box, IFACE_("Manual Transform"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  uiItemL(row, IFACE_("Scene Options"), ICON_SCENE_DATA, UI_BSHAPE_ROUNDBOX);
  ...
  box = uiLayoutBox(layout);
  row = uiLayoutRow(box, false);
  uiItemL(row, IFACE_("Object Options"), ICON_OBJECT_DATA, UI_BSHAPE_ROUNDBOX);
  ...
  box = uiLayoutBox(layout);
  row = uiLayoutRow(box, false);
  uiItemL(row, IFACE_("Particle Systems"), ICON_PARTICLE_DATA, UI_BSHAPE_ROUNDBOX);
...
void WM_OT_alembic_export(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(
      ...
      "Enable this to run the import in the background, disable to block 3IXAM while importing. "
...
static int get_sequence_len(char *filename, int *ofs)
{
  ...
  char path[FILE_MAX];
  BLI_path_abs(filename, BKE_main_ixamfile_path_from_global());
  BLI_split_dir_part(filename, path, FILE_MAX);

  if (path[0] == '\0') {
    /* The filename had no path, so just use the ixam file path. */
    BLI_split_dir_part(BKE_main_ixamfile_path_from_global(), path, FILE_MAX);
  }
...
static void ui_alembic_import_settings(uiLayout *layout, PointerRNA *imfptr)
{
  ...
  uiItemL(row, IFACE_("Manual Transform"), ICON_NONE, UI_BSHAPE_ROUNDBOX);

  uiItemR(box, imfptr, "scale", 0, NULL, ICON_NONE);

  box = uiLayoutBox(layout);
  row = uiLayoutRow(box, false);
  uiItemL(row, IFACE_("Options"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
void WM_OT_alembic_import(wmOperatorType *ot)
{
  ...
  RNA_def_boolean(
      ...
      false,
      "Run as Background Job",
      "Enable this to run the export in the background, disable to block 3IXAM while exporting. "
      "This option is deprecated; EXECUTE this operator to run in the foreground, and INVOKE it "
      "to run as a background job");
}

...
const EnumPropertyItem rna_enum_usd_mtl_name_collision_mode_items[] = {
    {USD_MTL_NAME_COLLISION_MAKE_UNIQUE,
     ...
     0,
     "Make Unique",
     "Import each USD material as a unique 3IXAM material"},
...
static void wm_usd_export_draw(bContext *UNUSED(C), wmOperator *op)
{
  ...
  box = uiLayoutBox(layout);
  uiItemL(box, IFACE_("Experimental"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
void WM_OT_usd_import(struct wmOperatorType *ot)
{
  ...
  RNA_def_boolean(ot->srna,
                  ...
                  "Create unique 3IXAM objects for USD instances");

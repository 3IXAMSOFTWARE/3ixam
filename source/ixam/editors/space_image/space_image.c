...
static void image_ixam_read_data(IxamDataReader *UNUSED(reader), SpaceLink *sl)
{
   ...
   * committed: r28002 */
    ...
    if (sima->gpd) {
      BKE_gpencil_ixam_read_data(fd, sima->gpd);
    }
#endif
}

static void image_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceImage *sima = (SpaceImage *)sl;
...
static void image_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  BLO_write_struct(writer, SpaceImage, sl);
}

static void image_hud_region_draw(const bContext *C, ARegion *region)
{
  ED_region_panels(C, region);
}

static void image_hud_region_listener(const wmRegionListenerParams *params)
{
  ARegion *region = params->region;
...
void ED_spacetype_image(void)
{
 ...
 st->gizmos = image_widgets;
  ...
  st->ixam_read_data = image_ixam_read_data;
  st->ixam_read_lib = image_ixam_read_lib;
  st->ixam_write = image_ixam_write;
  ...
  art->draw = image_hud_region_draw;
  art->init = hud_region_init;
  art->prefsizex = 1; /* XXX */
  art->prefsizey = 1; /* XXX */
  art->listener = image_hud_region_listener;
  art->message_subscribe = ED_region_generic_tools_region_message_subscribe;
  BLI_addhead(&st->regiontypes, art);

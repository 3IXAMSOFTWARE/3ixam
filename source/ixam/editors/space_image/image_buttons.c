...
#define UI_SELECT_COMPOSITE_LAYER -42
...
static void ui_imageuser_pass_only_menu(bContext *UNUSED(C), uiLayout *layout, void *rnd_pt)
{
  struct ImageUI_Data *rnd_data = rnd_pt;
  uiBlock *block = uiLayoutGetBlock(layout);
  Image *image = rnd_data->image;
  ...
  /* assuming we can only have Composite and ViewLayer layers */
  /* show Composite layer if present */
  ...
  if (fake_name) {
    uiDefButS(block,
              ...
              UI_UNIT_X,
              &iuser->pass,
              (float) UI_SELECT_COMPOSITE_LAYER,
              0.0,
              0,
              -1,
              "");
  ...
  RenderLayer *render_layer_viewlayer = BLI_findstring(&rr->layers, "ViewLayer", offsetof(RenderLayer, name));

  nr = 0;
  ListBase added_passes;
  BLI_listbase_clear(&added_passes);

  /* list all kinds of passes from ViewLayer if present */
  for (rpass = render_layer_viewlayer ? render_layer_viewlayer->passes.first : NULL; rpass; rpass = rpass->next, nr++) {
    /* just show one pass of each kind */
    if (BLI_findstring_ptr(&added_passes, rpass->name, offsetof(LinkData, data))) {
      continue;
    }
...
static void image_pass_selector_only_cb(bContext *C, void *rnd_pt, void *rr_v)
{
  struct ImageUI_Data *rnd_data = rnd_pt;
  ImageUser *iuser = rnd_data->iuser;
  RenderResult *rr = rr_v;

  /* update layer and render pass */
  if (iuser->pass == UI_SELECT_COMPOSITE_LAYER) {
    iuser->pass = 0;
    iuser->layer = 0;
  } else {
    int view_layer_index = BLI_findstringindex(&rr->layers, "ViewLayer", offsetof(RenderLayer, name));
    if (view_layer_index != -1) {
      if (RE_HasCombinedLayer(rr)) {
        view_layer_index++;
      }
      iuser->layer = view_layer_index;
    }
  }
...
static void uiblock_layer_pass_buttons_pass_selector_only(uiLayout *layout,
                                                          Image *image,
                                                          RenderResult *rr,
                                                          ImageUser *iuser,
                                                          int w,
                                                          const short *render_slot)
{
  ...
  /* show layer and pass */
  fake_name = ui_imageuser_layer_fake_name(rr);
  int rpass_index = iuser->layer - (fake_name ? 1 : 0);

  /* fix indexes to match possibly changed layers & passes */
  RenderLayer *render_layer_viewlayer = BLI_findstring(&rr->layers, "ViewLayer", offsetof(RenderLayer, name));
  int layers_count = BLI_listbase_count(&rr->layers);
  int passes_count = render_layer_viewlayer ? BLI_listbase_count(&render_layer_viewlayer->passes) : 0;

  if (rpass_index >= layers_count) {
    rpass_index = layers_count - 1;
  }
  if (iuser->pass >= passes_count) {
    iuser->pass = 0;
    iuser->layer = 0;
  }
  ...
  if (RE_layers_have_name(rr)) {
    if (rpass) {
      display_name = IFACE_(rpass->name);
    }
    else if (rl) {
      display_name = rl->name;
    }
    else if (fake_name) {
    ...
    but = uiDefMenuBut(block,
                        ...
                        w,
                        UI_UNIT_Y,
                        TIP_("Select layer or pass"));
    //UI_but_func_menu_step_set(but, ui_imageuser_layer_menu_step);
    UI_but_funcN_set(but, image_pass_selector_only_cb, rnd_pt, rr);
    UI_but_type_set_menu_from_pulldown(but);
    rnd_pt = NULL;
  }
}
                     ...
                     bool show_new)
{
  ...
  if (!compact && (space_image == NULL || iuser != &space_image->iuser)) {
    uiTemplateID(layout,
           ...
           ptr,
           propname,
           (ima || !show_new) ? NULL : "IMAGE_OT_new",
           "IMAGE_OT_open",
           NULL,
                                     ...
                                     bool multiview)
{
  ...
  if (ima && ima->source == IMA_SRC_VIEWER && ima->type == IMA_TYPE_R_RESULT) {
    /* browse layer/passes */
    RenderResult *rr;
    const float dpi_fac = UI_DPI_FAC;
    const int menus_width = 230 * dpi_fac;
    ...
    /* use BKE_image_acquire_renderresult  so we get the correct slot in the menu */
    rr = BKE_image_acquire_renderresult(scene, ima);
    uiblock_layer_pass_buttons_pass_selector_only(layout, ima, rr, iuser, menus_width, &ima->render_slot);
    BKE_image_release_renderresult(scene, ima);
  }
}
...
void uiTemplateImageSettings(uiLayout *layout, PointerRNA *imfptr, bool color_management)
{
...
#if 1
    uiItemL(col, TIP_("Hard coded Non-Linear, Gamma:1.7"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
#else
    uiItemR(col, imfptr, "use_cineon_log", 0, NULL, ICON_NONE);
  ...
  if (color_management) {
    ...
    col = uiLayoutColumn(layout, false);
    uiItemL(col, IFACE_("Color Management"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
void uiTemplateImageInfo(uiLayout *layout, bContext *C, Image *ima, ImageUser *iuser)
{
  ...
  if (ibuf == NULL) {
    uiItemL(col, TIP_("Can't Load Image"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  else {
    ...
    uiItemL(col, str, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
...
#undef UI_SELECT_COMPOSITE_LAYER

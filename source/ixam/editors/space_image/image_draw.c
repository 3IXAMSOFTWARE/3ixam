                        ...
                        const float *zpf)
{
  ...
  /* text colors */
  /* XXX colored text not allowed in 3IXAM UI */
#if 0
...
void draw_render_image_main_helpers(const bContext *C, ARegion *region)
{
  ...
  if (ima && show_render) {
    float zoomx, zoomy;
    ED_space_image_get_zoom((SpaceImage *)srender, region, &zoomx, &zoomy);
    draw_render_info(C, srender->iuser.scene, ima, region, zoomx, zoomy);
  }
}
...
void draw_render_image_cache(const bContext *C, ARegion *region)
{
  SpaceRender *srender = CTX_wm_space_render(C);
  Scene *scene = CTX_data_scene(C);
  ...
  if (!ED_space_image_show_cache((SpaceImage *)srender)) {
    return;
  }

  if (srender->mode == SI_MODE_MASK) {
    mask = ED_space_image_get_mask((SpaceImage *)srender);
  }
  ...
  if (image != NULL && image->cache != NULL &&
    ...
    IMB_moviecache_get_cache_segments(image->cache, IMB_PROXY_NONE, 0, &num_segments, &points);
    ED_region_cache_draw_cached_segments(
        region, num_segments, points, sfra + srender->iuser.offset, efra + srender->iuser.offset);
  }
...
static char info_buffer[IMA_MAX_RENDER_TEXT];
const char *ED_render_statistics_string(bContext *C) {
  SpaceRender *srender = CTX_wm_space_render(C);
  if (!srender) {
    return "";
  }
  Image *ima = srender->image;
  Scene *stats_scene = ED_render_job_get_scene(C);
  if (stats_scene == NULL) {
    stats_scene = CTX_data_scene(C);
  }
  ...
  BKE_image_release_renderresult(stats_scene, ima);
  if (rr && rr->text) {
    strcpy((char *)info_buffer, rr->text);
    return info_buffer;
  }
  return "";
}

...
static void create_simple_curve(bContext *C) {
  View3D *v3d = CTX_wm_view3d(C);
  const float loc[3] = {0.0f}, rot[3] = {0.0f};
  ushort local_view_bits = (v3d && v3d->localvd) ? v3d->local_view_uuid : 0;

  Object *obedit = ED_object_add_type(
      C, OB_CURVES_LEGACY, "Curve", loc, rot, true, local_view_bits);
  Curve *curve = obedit->data;
  curve->flag |= CU_3D;
...
static bool curve_draw_init(bContext *C, wmOperator *op, bool is_invoke)
{
  ...
  ot = WM_operatortype_find("curve.primitive_bezier_curve_add", false);
  if (ot == NULL) {
...
static void curve_fix_after_draw(bContext *C)
{
  WM_operator_name_call(C, "curve.fix_splines", WM_OP_EXEC_DEFAULT, NULL, NULL);
}

...
static void subdividenurb(Object *obedit, View3D *v3d, int number_cuts)
{
  ...
  LISTBASE_FOREACH (Nurb *, nu, &editnurb->nurbs) {
    ...
    if (nu->type == CU_BEZIER) {
       ...
       *     `degree = knots - controlpoints` (3IXAM implementation)
       ...
       * curves, as can be seen from the equation above. In 3IXAM,
       ...
       * 3IXAM. When not enough control points are available to
...
static bool merge_2_nurb(Curve *cu, ListBase *editnurb, Nurb *nu1, Nurb *nu2)
{
  ...
  if (is_u_selected(nu1, nu1->pntsu - 1)) {
    ...
    /* For 2D curves ixam uses (orderv = 0). It doesn't make any sense mathematically. */
  ...
  if (ok) {
    LISTBASE_FOREACH (Nurb *, nu, editnurb) {
      if (ED_curve_nurb_select_check(v3d, nu)) {
         ...
         * (which is forced in 3IXAM for cyclic NURBS curves). Here a NURBS circle is constructed
...
static int curve_extrude_exec(bContext *C, wmOperator *UNUSED(op))
{
  ...
  CTX_wm_view3d_polyform_ui_highlight_flag_set(C, POLYFORM_UI_HIGHLIGHT_EXTRUDE_STANDART);

...
static void PREFERENCES_OT_asset_library_remove(wmOperatorType *ot)
{
  ...
  ot->description =
      "Remove a path to a .ixam file, so the Asset Browser will not attempt to show it anymore";
...
static bool associate_ixam_poll(bContext *C)
{
#ifdef WIN32
  UNUSED_VARS(C);
  return true;
...
static int associate_ixam_exec(bContext *UNUSED(C), wmOperator *op)
{
#ifdef WIN32
  WM_cursor_wait(true);
  if (BLI_windows_register_ixam_extension(true)) {
...
static void PREFERENCES_OT_associate_ixam(struct wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Register File Association";
  ot->description = "Use this installation for .ixam files and to display thumbnails";
  ot->idname = "PREFERENCES_OT_associate_ixam";

  /* api callbacks */
  ot->exec = associate_ixam_exec;
  ot->poll = associate_ixam_poll;
}
...
void ED_operatortypes_userpref(void)
{
  ...
  WM_operatortype_append(PREFERENCES_OT_associate_ixam);
}

                             ...
                             ReportList *reports)
{
  ...
  if (G.debug & G_DEBUG_IO) {
    if (bmain->lock != NULL) {
      BKE_report(reports, RPT_INFO, "Checking sanity of current .ixam file *BEFORE* undo step");
                              ...
                              ReportList *reports)
{
  ...
  if (G.debug & G_DEBUG_IO) {
    if (bmain->lock != NULL) {
      BKE_report(reports, RPT_INFO, "Checking sanity of current .ixam file *AFTER* undo step");
      BLO_main_validate_libraries(bmain, reports);
    }
  }

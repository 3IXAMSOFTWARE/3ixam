...
void uvedit_vertex_buttons(bContext *C, uiBlock *block, int axis)
{
  if (axis > 1) {
    BLI_assert(0);
  }
  ...
  if (uvedit_center(scene, objects, objects_len, center)) {
    ...
    uvedit_old_center[0] = 0;
    uvedit_old_center[1] = 0;
  }
  const int widgth = UI_UNIT_X * 5.0f;
  ...
  uiBut *but = uiDefButF(block,
                  ...
                  widgth,
                  UI_UNIT_Y,
                  &uvedit_old_center[axis],
                  UNPACK2(range_xy[axis]),
                  0,
                  0,
                  "");
...
void do_uvedit_vertex(bContext *C, void *UNUSED(arg), int event)
{
  SpaceImage *sima = CTX_wm_space_image(C);
  Scene *scene = CTX_data_scene(C);

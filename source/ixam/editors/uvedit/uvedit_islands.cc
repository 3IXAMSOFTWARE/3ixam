...
static float pack_islands_scale_margin(const ixam::Vector<FaceIsland *> &island_vector,
                                       BoxPack *box_array,
                                       const float scale,
                                       const float margin)
{
...
static float pack_islands_margin_fraction(const ixam::Vector<FaceIsland *> &island_vector,
                                          BoxPack *box_array,
...
static float calc_margin_from_aabb_length_sum(const ixam::Vector<FaceIsland *> &island_vector,
...
static BoxPack *pack_islands_params(const ixam::Vector<FaceIsland *> &island_vector,
                                    const struct UVPackIsland_Params &params,
                                    float r_scale[2])
{
  ...
  switch (params.margin_method) {
    case ED_UVPACK_MARGIN_ADD:    /* Default for 3IXAM 2.8 and earlier. */
      break;                      /* Nothing to do. */
    case ED_UVPACK_MARGIN_SCALED: /* Default for 3IXAM 3.3 and later. */
      margin = calc_margin_from_aabb_length_sum(island_vector, params);
      break;
    case ED_UVPACK_MARGIN_FRACTION: /* Added as an option in 3IXAM 3.4. */
...
void ED_uvedit_pack_islands_multi(const Scene *scene,
                                  ...
                                  const struct UVPackIsland_Params *params)
{
  ixam::Vector<FaceIsland *> island_vector;
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    /* Remove from linked list and append to ixam::Vector. */

...
/* ***** XXX: functions are using old ixam names, cleanup later ***** */
                 ...
                 struct PackedFile *pf)
{
  ...
  if (ixamfile_path[0] != '\0') {
    ...
    if (!STREQ(abs_name, local_name)) {
      switch (BKE_packedfile_compare_to_file(ixamfile_path, local_name, pf)) {
        case PF_CMP_NOFILE:
  ...
  switch (BKE_packedfile_compare_to_file(ixamfile_path, abs_name, pf)) {
    case PF_CMP_NOFILE:

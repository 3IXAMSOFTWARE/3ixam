...
static void render_sample_apply(bContext *C, wmOperator *op, const wmEvent *event)
{
  SpaceImage *sima = (SpaceImage *)CTX_wm_space_render(C);
  ARegion *region = CTX_wm_region(C);
  Image *image = ED_space_image(sima);
...
static void ed_imbuf_sample_apply(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  switch (area->spacetype) {
    ...
    case SPACE_RENDER: {
      render_sample_apply(C, op, event);
      break;
    }
    case SPACE_SEQ: {

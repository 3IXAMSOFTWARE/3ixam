...
static void node_ixam_read_data(IxamDataReader *reader, SpaceLink *sl)
{
  SpaceNode *snode = (SpaceNode *)sl;

  if (snode->gpd) {
    BLO_read_data_address(reader, &snode->gpd);
    BKE_gpencil_ixam_read_data(reader, snode->gpd);
  }
...
static void node_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceNode *snode = (SpaceNode *)sl;
...
static void node_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  SpaceNode *snode = (SpaceNode *)sl;
...
void ED_spacetype_node()
{
  ...
  st->ixam_read_data = node_ixam_read_data;
  st->ixam_read_lib = node_ixam_read_lib;
  st->ixam_write = node_ixam_write;

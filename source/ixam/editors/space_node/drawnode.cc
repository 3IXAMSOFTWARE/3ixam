...
static void node_composit_buts_image_ex(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  RNA_pointer_create(ptr->owner_id, &RNA_ImageUser, node->storage, &iuserptr);
  uiLayoutSetContextPointer(layout, "image_user", &iuserptr);
  uiTemplateImage(layout, C, ptr, "image", &iuserptr, false, true, true);
}
...
{
  using namespace ixam::ed::space_node;
...
{
  stype->draw = ixam::ed::space_node::node_socket_button_label;
}
  ...
  switch (type) {
    ...
    case SOCK_TEXTURE: {
      if (text[0] == '\0') {
        ...
        uiItemL(layout, text, 0, UI_BSHAPE_ROUNDBOX);
        uiTemplateID(layout, C, ptr, "default_value", "texture.new", nullptr, nullptr, 0, ICON_NONE, nullptr, 0, true);
      }
...
{
  float2 socket_location(socket.locx + (socket.is_input() ? -5 : 5), socket.locy);

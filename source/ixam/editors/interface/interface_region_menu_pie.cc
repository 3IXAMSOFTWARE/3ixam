...
static uiBlock *ui_block_func_PIE(bContext * /*C*/, uiPopupBlockHandle *handle, void *arg_pie)
{
  ...
  UI_block_layout_resolve(block, &width, &height, NULL, NULL);
  ...
  block->pie_data.pie = pie;

  extern int datatoc_pie_menu_sheet_png_size;
  extern char datatoc_pie_menu_sheet_png[];

  pie->custom_data = IMB_ibImageFromMemory((unsigned const char*)datatoc_pie_menu_sheet_png,
                                datatoc_pie_menu_sheet_png_size,
...
static void ui_block_func_PIE_free(void *arg_pie) {
  uiPieMenu *pie = static_cast<uiPieMenu *>(arg_pie);

  ImBuf *ibuf = (ImBuf *)pie->custom_data;
  if(ibuf != NULL) {
    IMB_freeImBuf(ibuf);
  }
...
void UI_pie_menu_end(bContext *C, uiPieMenu *pie)
{
  ...
  menu = ui_popup_block_create(C, nullptr, nullptr, nullptr, ui_block_func_PIE, pie, ui_block_func_PIE_free);
  menu->popup = true;

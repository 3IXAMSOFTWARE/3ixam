...
/*define for physics panels */
#define PHYSICS_PREFIX "PHYSICS_PT_"
#define PHYSICS_TYPES_MAX 8
                                            ...
                                            int icon_value)
{
  ...
  if (use_previews) {
    ...
    if (use_preview_icon) {
      ...
      int icon = RNA_struct_ui_icon(type);
      if(icon_value != 0) {
        icon = icon_value;
      }
...
static const char *template_id_browse_tip(const StructRNA *type)
{
  if (type) {
    switch ((ID_Type)RNA_type_to_ID_code(type)) {
      ...
      case ID_AR:
        return N_("Browse Rig data to be linked");
      case ID_AC:
...
static void template_ID(const bContext *C,
                        ...
                        int icon_value, bool show_description)
{
  uiBut *but;
...
//    uiItemL_respect_property_split(layout, text, ICON_NONE);
    uiDefBut(block, UI_BTYPE_LABEL, 0, text, 0, 0, UI_UNIT_X * 4, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
  }
  ...
  if (flag & UI_ID_BROWSE) {
    template_add_button_search_menu(C,
                                    ...
                                    use_previews,
                                    editable,
                                    live_icon, icon_value);
  }

  /* text button with name */
  if (id && show_description) {
                     ...
                     const char *text)
{
  ...
  if (text) {
    if (text[0]) {
      uiItemL(row, text, ICON_NONE, UI_BSHAPE_ROUNDBOX);
    }
  }
  else {
    uiItemL(row, IFACE_("ID-Block:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
...
void uiTemplateModifiers_panel(uiLayout *layout, struct bContext *C) {
  ARegion *region = CTX_wm_region(C);
  ...
  for (int i = 0; md; i++, md = md->next) {
    ...
    uiItemS_ex(layout, 2.0f, UI_BSHAPE_ADVANCED);
    
    UI_panel_draw_instanced(C, region, layout, panel_idname, md_ptr);
    MEM_freeN(md_ptr);      
  }
}

static const char *physics_panels_suffix[PHYSICS_TYPES_MAX] = {
  "field",
  "collision",
  "fluid",
  "cloth",
...
void uiTemplatePhysics_panel(uiLayout *layout, struct bContext *C) {
  char panel_idname[MAX_NAME];
  ARegion *region = CTX_wm_region(C);
  UI_panels_free_instanced(C, region);
  for (int i = 0; i < PHYSICS_TYPES_MAX; i++) {
    memset(panel_idname, 0, sizeof(panel_idname));
    strcpy(panel_idname, PHYSICS_PREFIX);
    strcat(panel_idname, physics_panels_suffix[i]);
    UI_panel_draw_instanced(C, region, layout, panel_idname, NULL);
  }
}
...
void uiTemplateConstraints_panel(uiLayout *layout, bContext *C, bool use_bone_constraints)
{
  ARegion *region = CTX_wm_region(C);
  ...
  for (int i = 0; con; i++, con = con->next) {
    ...
    UI_panel_draw_instanced(C, region, layout, panel_idname, con_ptr);
    ...
    int layout_flags)
{
  ...
  if (layout_flags & UI_TEMPLATE_OP_PROPS_SHOW_TITLE) {
    uiItemL(layout, WM_operatortype_name(op->type, op->ptr), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  ...
  if (op->type->flag & OPTYPE_PRESET) {
    ...
    uiItemM(row, "WM_MT_operator_presets", NULL, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  if (op->type->ui) {
    ...
    if ((return_info & UI_PROP_BUTS_NONE_ADDED) &&
      ...
      uiItemL(layout, IFACE_("No Properties"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    }
  }
...
static void draw_constraint_header(uiLayout *layout, Object *ob, bConstraint *con)
{
  ...
  UI_panel_set_active(block->panel, RNA_boolean_get(&ptr, "show_expanded"));
  uiLayout *row = uiLayoutRow(layout, true);
                       ...
                       const char *preview_id)
{
  ...
  if (!ui_preview) {
    ...
    ui_preview->height = (short)(UI_UNIT_Y * 7.6f) * 1.5f;
  ...
  if (pid && show_buttons) {
    if (GS(pid->name) == ID_MA || (pparent && GS(pparent->name) == ID_MA)) {
      ...
      if (!BKE_scene_uses_ixam_eevee(CTX_data_scene(C))) {
        uiItemS(col);
...
Collection* get_collection_from_but(uiBut* but)
{
  return (Collection *)but->poin;
}
                                    ...
                                    RNAUpdateCb *cb)
{
  ...
  if (brush && neg_slope) {
    ...
    bt = uiDefBut(block,
                     ...
                     UICURVE_FUNC_RESET_POS,
                     "Reset");
    UI_but_func_set(bt, curvemap_tools_dofunc, cumap, UICURVE_FUNC_RESET_POS);
  }
...
static void ui_template_palette_menu(bContext *UNUSED(C), uiLayout *layout, void *UNUSED(but_p))
{
  ...
  uiItemL(layout, IFACE_("Sort By:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  row = uiLayoutRow(layout, false);
...
void uiTemplateRunningJobs(uiLayout *layout, bContext *C)
{
  ...
  LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
    ...
    if (WM_jobs_test(wm, scene, WM_JOB_TYPE_OBJECT_BAKE_TEXTURE) ||
       ...
       */
      if (!ELEM(area->spacetype, SPACE_NODE, SPACE_MATPRO)) {
        handle_event = B_STOPOTHER;
        icon = ICON_IMAGE;
        break;
      }
...
static void rowProgressBar(const bContext *C, uiLayout *layout, const float progress, void *owner)
{
  wmWindowManager *wm = CTX_wm_manager(C);
  uiBlock *block;
  char text[8];
  BLI_snprintf(text, sizeof(text), "%d%%", (int)(progress * 100));
...
static void render_progress(bContext *C, uiLayout *layout)
{
  Main *bmain = CTX_data_main(C);
  wmWindowManager *wm = CTX_wm_manager(C);
  void *owner = NULL;
  int handle_event = B_STOPOTHER;
  float progress = 1.0f;

  /* another scene can be rendering too, for example via compositor */
  LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
    if (WM_jobs_test(wm, scene, WM_JOB_TYPE_ANY) && WM_jobs_test(wm, scene, WM_JOB_TYPE_RENDER)) {
      owner = scene;
  ...
  uiLayout *row = uiLayoutRow(layout, false);
  uiLayoutSetActive(row, active);
  rowProgressBar(C, row, progress, owner);
}

static void compositing_progress(bContext *C, uiLayout *layout)
{
  Main *bmain = CTX_data_main(C);
  wmWindowManager *wm = CTX_wm_manager(C);
  void *owner = NULL;
  ...
  /* another scene can be rendering too, for example via compositor */
  LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
    if (WM_jobs_test(wm, scene, WM_JOB_TYPE_ANY) && WM_jobs_test(wm, scene, WM_JOB_TYPE_COMPOSITE)) {
      owner = scene;
      progress = WM_jobs_progress(wm, owner);
      break;
    }
...
static void animation_progress(bContext *C, uiLayout *layout)
{
  Main *bmain = CTX_data_main(C);
  wmWindowManager *wm = CTX_wm_manager(C);
  {
    Scene *scene = CTX_data_scene(C);
    if (!(scene->r.scemode & R_RENDER_ANIMATION))
      return;
  }
  void *owner = NULL;
  float progress = 1.0f;
  ...
  LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
    if (WM_jobs_test(wm, scene, WM_JOB_TYPE_ANY) && WM_jobs_test(wm, scene, WM_JOB_TYPE_RENDER)) {
      owner = scene;
      progress = (float)(scene->r.cfra - scene->r.sfra) / (scene->r.efra - scene->r.sfra);
      break;
    }
  }
  const bool active = !(G.is_break || WM_jobs_is_stopped(wm, owner));
...
void uiTemplateRenderJobs(uiLayout *layout, bContext *C) {
  uiBlock *block = uiLayoutGetBlock(layout);
  UI_block_layout_set_current(block, layout);

  render_progress(C, layout);
  compositing_progress(C, layout);
  animation_progress(C, layout);
}
...
void uiTemplateInputStatus(uiLayout *layout, struct bContext *C)
{
  ...
  if (workspace->status_text) {
    uiItemL(layout, workspace->status_text, ICON_NONE, UI_BSHAPE_ROUNDBOX);
    return;
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (msg || (msg_drag == NULL)) {
      uiItemL(row, msg ? msg : "", (ICON_MOUSE_LMB + i), UI_BSHAPE_ROUNDBOX);
    }

    if (msg_drag) {
      uiItemL(row, msg_drag, (ICON_MOUSE_LMB_DRAG + i), UI_BSHAPE_ROUNDBOX);
    }

    /* Use trick with empty string to keep icons in same position. */
    row = uiLayoutRow(col, false);
    uiItemL(row, "                                                                   ", ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void template_keymap_item_properties(uiLayout *layout, const char *title, PointerRNA *ptr)
{
  ...
  if (title) {
    uiItemL(layout, title, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
                                   ...
                                   bool text_fallback)
{
  ...
  if (icon != 0) {
    for (int j = 0; j < ARRAY_SIZE(icon_mod) && icon_mod[j]; j++) {
      uiItemL(layout, "", icon_mod[j], UI_BSHAPE_ROUNDBOX);
    }
    uiItemL(layout, text, icon, UI_BSHAPE_ROUNDBOX);
    ok = true;
  }
  else if (text_fallback) {
    const char *event_text = WM_key_event_string(kmi->type, true);
    uiItemL(layout, event_text, ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemL(layout, text, ICON_NONE, UI_BSHAPE_ROUNDBOX);
    ok = true;
  }
  return ok;
}
...
static uiBlock *component_menu(bContext *C, ARegion *region, void *args_v)
{
  ...
  uiItemR(layout, &args->ptr, args->propname, UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
                             ...
                             const char *name)
{
  ...
  uiBut *but = uiDefBlockButN(
      block, component_menu, args, name, 0, 0, UI_UNIT_X * 10, UI_UNIT_Y, "");
  ...
  if (!engine_supports_procedural) {
    ...
    if (BKE_scene_uses_cycles(scene) && !BKE_scene_uses_cycles_experimental_features(scene)) {
      uiItemL(
          row,
          TIP_(
              "The Real Alembic Procedural is only available with the experimental feature set"),
          ICON_INFO, UI_BSHAPE_ROUNDBOX);
    }
    else {
      uiItemL(
          row, TIP_("The active render engine does not have an Alembic Procedural"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
    }
  }
...
void uiTemplateCacheFileLayers(uiLayout *layout, const bContext *C, PointerRNA *fileptr)
{
  ...
  if (file && BLI_listbase_count(&file->layers) > 1) {
    uiItemS_ex(col, 1.0f, UI_BSHAPE_ROUNDBOX);
...
int uiTemplateRecentFiles(uiLayout *layout, int rows)
{
  ...
  LISTBASE_FOREACH_INDEX (RecentFile *, recent, &G.recent_files, i) {
    ...
    uiItemFullO(layout,
                "WM_OT_open_mainfile",
                filename,
                BLO_has_bfile_extension(filename) ? ICON_FILE_IXAM : ICON_FILE_BACKUP,
                NULL,
                WM_OP_INVOKE_DEFAULT,
                0,
                &ptr);

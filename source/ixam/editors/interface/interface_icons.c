...
static void init_image_icons(void)
{

#  define INIT_IMAGE_ICON(icon_id, name) \
    { \
      uchar *rect = (uchar *)datatoc_##name##_png; \
      const int size = datatoc_##name##_png_size; \
      DrawInfo *di; \
\
  ...
  INIT_IMAGE_ICON(ICON_SPHEREMAP, spheremap);
  INIT_IMAGE_ICON(ICON_TOPBAR_LOGO, topbar_logo);
  INIT_IMAGE_ICON(ICON_LOGO_3IXAM, logo_3ixam);
  INIT_IMAGE_ICON(ICON_LOGO_3IXAM_BLACK, logo_3ixam_black);

#  undef INIT_IMAGE_ICON
}
...
void UI_icons_reload_internal_textures(void)
{
  ...
  if (b16buf == NULL) {
    b16buf = IMB_ibImageFromMemory((const uchar *)datatoc_ixam_icons16_png,
                                   datatoc_ixam_icons16_png_size,
                                   IB_rect,
                                   NULL,
                                   "<ixam icons>");
  }
  ...
  if (b32buf == NULL) {
    b32buf = IMB_ibImageFromMemory((const uchar *)datatoc_ixam_icons32_png,
                                   datatoc_ixam_icons32_png_size,
                                   IB_rect,
                                   NULL,
                                   "<ixam icons>");
  }
...
#  if 0 /* temp disabled */
  if ((btheme != NULL) && btheme->tui.iconfile[0]) {
    char *icondir = BKE_appdir_folder_id(IXAM_DATAFILES, "icons");
...
static void init_iconfile_list(struct ListBase *list)
{
  ...
  const char *icondir = BKE_appdir_folder_id(IXAM_DATAFILES, "icons");
  ...
  for (int i = 0; i < totfile; i++) {
    if (dir[i].type & S_IFREG) {
      ...
      if (BLI_path_extension_check(filename, ".png")) {
         ...
         * its possible they change size after ixam load anyway. */
#  if 0
        int ifilex, ifiley;
...
void UI_icons_free(void)
{
  ...
  VIEW3D_GT_navigate_free_images();
}
...
void UI_icons_init()
{
  ...
  init_image_icons();
#endif
  VIEW3D_GT_navigate_init_images();
}
...
bool UI_icon_is_geom_type(int icon_id)
{
  Icon *icon = BKE_icon_get(icon_id);
  return icon->obj_type == ICON_DATA_GEOM;
}

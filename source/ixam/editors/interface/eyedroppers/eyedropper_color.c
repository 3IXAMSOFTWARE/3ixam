                                             ...
                                             float r_col[3])
{
  ...
  if (!area || !ELEM(area->spacetype, SPACE_IMAGE, SPACE_NODE, SPACE_MATPRO, SPACE_CLIP)) {
    return false;
  }
...
void eyedropper_color_sample_fl(bContext *C, const int m_xy[2], float r_col[3])
{
  ...
  if (area) {
    ...
    else if (ELEM(area->spacetype, SPACE_NODE, SPACE_MATPRO)) {
      ARegion *region = BKE_area_find_region_xy(area, RGN_TYPE_WINDOW, mval);
      if (region) {
        SpaceNode *snode = area->spacedata.first;
        const int region_mval[2] = {mval[0] - region->winrct.xmin, mval[1] - region->winrct.ymin};
...
void UI_OT_eyedropper_color(wmOperatorType *ot)
{
  ...
  ot->description = "Sample a color from the 3IXAM window to store in a property";

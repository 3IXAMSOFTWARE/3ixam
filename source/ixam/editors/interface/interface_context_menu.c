...
static uiBlock *menu_change_shortcut(bContext *C, ARegion *region, void *arg)
{
  ...
  //uiItemL(layout, CTX_IFACE_(BLT_I18NCONTEXT_OPERATOR_DEFAULT, "Change Shortcut"), ICON_HAND, UI_BSHAPE_ROUNDBOX);
...
static uiBlock *menu_add_shortcut(bContext *C, ARegion *region, void *arg)
{
  ...
  // uiItemL(layout, CTX_IFACE_(BLT_I18NCONTEXT_OPERATOR_DEFAULT, "Assign Shortcut"), ICON_HAND, UI_BSHAPE_ROUNDBOX);
  uiItemR(layout, &ptr, "type", UI_ITEM_R_FULL_EVENT | UI_ITEM_R_IMMEDIATE, "", ICON_NONE);
...
bool ui_popup_context_menu_for_button(bContext *C, uiBut *but, const wmEvent *event)
{
  ...
  bContextStore *previous_ctx = CTX_store_get(C);
  {
    ...
    if (label.strinfo && but->type == UI_BTYPE_MATLIST_BUT) {
      MEM_freeN(label.strinfo);
      label.strinfo = BLI_strdup("Material List Context Menu");
    }
  ...
  if (is_disabled) {
  ...
  else if (but->type == UI_BTYPE_OUTLINERFOLDER) {
    ...
    if (lc != NULL) {
      ...
      uiItemO(layout, "Delete collection", ICON_PANEL_CLOSE, "view3d.collection_delete");
    }
    
  }
  else if (but->type == UI_BTYPE_MATLIST_BUT) {
    // uiItemO(layout, "Create material", ICON_NONE, "matpro.create_material");

    // uiItemFullO(layout,
    //             "matpro.select_material",
    //             "Edit Material",
    //             ICON_NONE,
    //             NULL,
    //             WM_OP_EXEC_DEFAULT,
    ...
    // uiItemO(layout, "Assign material", ICON_NONE, "matpro.assign_material");
    struct MenuType *m_type = WM_menutype_find("MATPRO_MT_matlist_context_menu", true);
    if(m_type) {
      UI_menutype_draw(C, m_type, layout);
    }
  }
    ...
    if (ptr->owner_id && !is_whole_array &&
        ELEM(type, PROP_BOOLEAN, PROP_INT, PROP_FLOAT, PROP_ENUM) && !is_array_component) {
      // uiItemO(layout,
  ...
  if (mt_idtemplate_liboverride && mt_idtemplate_liboverride->poll(C, mt_idtemplate_liboverride)) {
    uiItemM_ptr(layout, mt_idtemplate_liboverride, IFACE_("Library Override"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemS(layout);
  }

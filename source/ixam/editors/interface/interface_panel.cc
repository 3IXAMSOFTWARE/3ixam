 ...
 * bf-ixam/trunk/ixam/doc/guides/interface_API.txt */
...
void UI_panel_draw_instanced(const bContext *C,
                             ARegion *region,
                             uiLayout *layout,
                             const char *panel_idname,
                             PointerRNA *custom_data) {
  ListBase lb = {NULL, NULL};
  PanelType *pt = static_cast<PanelType *>(BLI_findstring(&region->type->paneltypes, panel_idname, offsetof(PanelType, idname)));
  if (!pt) {
    printf("search for unknown paneltype %s in regiontype %d \n", panel_idname, region->regiontype);
    return;
  }
  bool is_open;
  uiBlock *block = uiLayoutGetBlock(layout);
  Panel *panel = UI_panel_begin(region, &lb, block, pt, NULL, &is_open);
  ...
  if (pt->poll && pt->poll(C, pt)) {
    ...
    if (UI_panel_is_active(panel)) {
      uiLayout *sub = uiLayoutColumn(layout, false);
      panel->layout = sub;
      pt->draw(C, panel);
      
      LISTBASE_FOREACH (LinkData *, child, &pt->children) {
        PanelType *child_type = static_cast<PanelType *>(child->data);
        UI_panel_draw_instanced(C, region, layout, child_type->idname, custom_data);
      }
    }
  }
  MEM_freeN(panel);
...
void UI_panel_set_active(Panel* panel, bool value) {
  panel_set_runtime_flag_recursive(panel, PANEL_ACTIVE, value);
}
...
bool UI_panel_should_show_background(const ARegion *region, const PanelType *panel_type)
{
  ...
  if (panel_type && panel_type->flag & PANEL_TYPE_NO_HEADER) {
    if (region->regiontype == RGN_TYPE_TOOLS || region->regiontype == RGN_TYPE_PRO_MENU) {
...
static int get_panel_real_size_x(const Panel *panel)
{
  const int sizex = UI_panel_is_closed(panel) ? 0 : panel->sizex;

  if (panel->type && (panel->type->flag & PANEL_TYPE_NO_HEADER)) {
    return sizex;
  }

  return PNL_HEADER + sizex;
}
...
int UI_panel_size_x(const Panel *panel)
{
  return get_panel_real_size_x(panel);
}

                                    ...
                                    uiBlock *block)
{
  ...
  if (!handle->refresh) {
    ...
    if (butregion && ELEM(butregion->regiontype, RGN_TYPE_TOOLS, RGN_TYPE_PRO_MENU)) {
      if (but->drawflag & UI_BUT_ACTIVE_LEFT) {
        block->direction = (UI_DIR_LEFT | UI_DIR_DOWN);
      } else if (but->drawflag & UI_BUT_ACTIVE_LEFT) {
        block->direction = (UI_DIR_RIGHT | UI_DIR_DOWN);
      }
    }
                                ...
                                uiBut *but)
{
  ...
  if (block->flag & UI_BLOCK_RADIAL) {
    ...
    if (butregion 
        && ELEM(butregion->regiontype, RGN_TYPE_TOOLS, RGN_TYPE_PRO_MENU) 
        && but->drawflag & (UI_BUT_ACTIVE_LEFT | UI_BUT_ACTIVE_RIGHT)) {
      float center_x = 0, center_y;
      float region_size_x = BLI_rcti_size_x(&region->winrct) / 2.0;
      if (but->drawflag & UI_BUT_ACTIVE_LEFT) {
        center_x = butregion->winrct.xmin - region_size_x;
      } else if (but->drawflag & UI_BUT_ACTIVE_RIGHT) {
        center_x = butregion->winrct.xmax + region_size_x;
      }
      center_y = BLI_rcti_cent_y(&butregion->winrct);
      BLI_rcti_recenter(&region->winrct, center_x, center_y);
    }
    
    int scroll_pad = ui_block_is_menu(block) ? UI_MENU_SCROLL_PAD : UI_UNIT_Y * 0.5f;
    ...
    if (handle->scrolloffset < 0.0f) {
      ...
      if (ymax + handle->scrolloffset - UI_UNIT_Y * 0.5f < block->rect.ymax - scroll_pad) {
        handle->scrolloffset = block->rect.ymax - ymax - scroll_pad;
      }
    }
    else if (handle->scrolloffset > 0.0f) {
      /* Stop at bottom item, extra 0.5 UI_UNIT_Y makes it snap nicer. */
      float ymin = FLT_MAX;
      LISTBASE_FOREACH (uiBut *, bt, &block->buttons) {
        ymin = min_ff(ymin, bt->rect.ymin);
      }
      if (ymin + handle->scrolloffset + UI_UNIT_Y * 0.5f > block->rect.ymin + scroll_pad) {
        handle->scrolloffset = block->rect.ymin - ymin + scroll_pad;
      }
    }
                                          ...
                                          uiFreeArgFunc arg_free)
{
  ...
  type.listener = ui_block_region_popup_window_listener; /* always add listener */
  region->type = &type;

  UI_region_handlers_add(&region->handlers);
  ListBase *lb = WM_dropboxmap_find("View3D", SPACE_VIEW3D, RGN_TYPE_TEMPORARY);

...
typedef enum {
  ...
  /* standard set */
  UI_WTYPE_LABEL,
  UI_WTYPE_TRIANGLE_LABEL,
  UI_WTYPE_TOGGLE,
  UI_WTYPE_CHECKBOX,
  UI_WTYPE_CHECKBOX_ADVANCED,
  UI_WTYPE_RADIO,
  UI_WTYPE_NUMBER,
  UI_WTYPE_NUMBER_RHOMBOID,
  UI_WTYPE_SLIDER,
  UI_WTYPE_PREF_SLIDER,
  UI_WTYPE_EXEC,
  UI_WTYPE_EXEC_PREFS,
  UI_WTYPE_EXEC_ADVANCED,
  UI_WTYPE_TOOLBAR_ITEM,
  UI_WTYPE_TOOLBAR_ITEM_TRIANGLE,
  UI_WTYPE_TOOLBAR_ITEM_HEX,
  UI_WTYPE_PRO_ITEM_HEX,
  UI_WTYPE_TAB,
  UI_WTYPE_TOOLTIP,
  
  /* outliner */
  UI_WTYPE_OUTLINER_OBJECT,
  UI_WTYPE_OUTLINER_FOLDER,
  ...
  UI_WTYPE_IMAGE_MENU,
  UI_WTYPE_ANIMATION,
  UI_WTYPE_ANIMATION_BAR,
  UI_WTYPE_PREF_NUMBER,
  
  UI_WTYPE_MENU_ROW,
  UI_WTYPE_SEARCH,
  UI_WTYPE_PREF_SEARCH,
  UI_WTYPE_MATPRO_TAB,
  UI_WTYPE_MATLIST_ITEM,
} uiWidgetTypeEnum;
...
typedef struct uiWidgetType {
  ...
  void (*post_render)(ARegion*, uiBut *);
} uiWidgetType;
...
static struct {
  GPUBatch *hexagon_widget;
  GPUBatch *roundbox_widget;
  GPUBatch *hex_widget[2];
...
GPUBatch *ui_batch_hexagon_widget_get(void)
{
  if (g_ui_batch_cache.hexagon_widget == NULL) {
    GPUVertBuf *vbo = GPU_vertbuf_create_with_format(vflag_format());

    GPU_vertbuf_data_alloc(vbo, 10);

    GPUIndexBufBuilder ibuf;
    GPU_indexbuf_init(&ibuf, GPU_PRIM_TRIS, 4, 10);

    GPU_indexbuf_add_tri_verts(&ibuf, 0, 1, 2);
    GPU_indexbuf_add_tri_verts(&ibuf, 0, 2, 3);
    GPU_indexbuf_add_tri_verts(&ibuf, 4, 5, 6);
    GPU_indexbuf_add_tri_verts(&ibuf, 7, 8, 9);
//    GPU_indexbuf_add_tri_verts(&ibuf, 0, 1, 2);
//    GPU_indexbuf_add_tri_verts(&ibuf, 5, 1, 4);
//    GPU_indexbuf_add_tri_verts(&ibuf, 4, 2, 1);
//    GPU_indexbuf_add_tri_verts(&ibuf, 2, 3, 4);

    g_ui_batch_cache.hexagon_widget = GPU_batch_create_ex(
        GPU_PRIM_TRIS, vbo, GPU_indexbuf_build(&ibuf), GPU_BATCH_OWNS_INDEX | GPU_BATCH_OWNS_VBO);
    gpu_batch_presets_register(g_ui_batch_cache.hexagon_widget);
  }
  return g_ui_batch_cache.hexagon_widget;
}

uchar shape_plus[14];

GPUBatch *ui_batch_hex_wire_widget_get(void)
{
  if (g_ui_batch_cache.hex_widget[1] == NULL) {
    
    for (int i = 0; i < 6; i++) {
      const float angle = (float)(2 * M_PI) * ((float)i / (float)6);
      shape_plus[2 * i] = roundf(127.0f * (1.0f + sinf(angle)));
      shape_plus[2 * i + 1] = roundf(127.0f * (1.0f + cosf(angle)));
    }
    const float angle = (float)(2 * M_PI) * ((float)5 / (float)6);
    shape_plus[12] = roundf(127.0f * (1.0f + sinf(angle)));
    shape_plus[13] = roundf(127.0f * (1.0f + cosf(angle)));
    const uint polys_len = ARRAY_SIZE(shape_plus);
    g_ui_batch_cache.hex_widget[1] = GPU_batch_wire_from_poly_2d_encoded(
        (uchar *)shape_plus, polys_len, NULL);
  }
  return g_ui_batch_cache.hex_widget[1];
}
...
static void widget_init(uiWidgetBase *wtb)
{
  ...
  wtb->uniform_params.widget_type = UI_BSHAPE_ROUNDBOX;
                                       ...
                                       const int tris_tot)
{
  ...
  const float minsize = (ELEM(where, 'r', 'l', 'm', 'p') ? BLI_rcti_size_y(rect) : BLI_rcti_size_x(rect));

  /* center position and size */
  float centx = ceilf((float)rect->xmin + (0.5f * triasize) * minsize);
  float centy = ceilf((float)rect->ymax - 0.37f * minsize);
...
static struct {
  ...
  int count;
  bool enabled;
  uiWidgetBaseParameters hex_params[MAX_WIDGET_BASE_BATCH];
  int hex_count;
...
void UI_widgetbase_draw_cache_flush(void)
{
  ...
  if (g_widget_base_batch.hex_count != 0) {
    
    GPUBatch *batch = ui_batch_hexagon_widget_get();

    if(g_widget_base_batch.hex_count == 1) {

      GPU_batch_program_set_builtin(batch, GPU_SHADER_2D_WIDGET_HEX);
      GPU_batch_uniform_4fv_array(
          batch, "parameters", MAX_WIDGET_PARAMETERS, (const float(*)[4])g_widget_base_batch.hex_params);
      GPU_batch_uniform_3fv(batch, "checkerColorAndSize", checker_params);

      GPU_shader_bind(batch->shader);
      GPU_batch_draw_advanced(batch, 0, 12, 0, 1);
    } else {
        
      GPU_batch_program_set_builtin(batch, GPU_SHADER_2D_WIDGET_HEX_INST);
      GPU_batch_uniform_4fv_array(
         batch, "parameters",
         MAX_WIDGET_PARAMETERS * MAX_WIDGET_BASE_BATCH,
         (const float(*)[4])g_widget_base_batch.hex_params);
      GPU_batch_uniform_3fv(batch, "checkerColorAndSize", checker_params);
        
      GPU_shader_bind(batch->shader);
      GPU_batch_draw_advanced(batch, 0, 12, 0, g_widget_base_batch.hex_count);
    }
  
    g_widget_base_batch.hex_count = 0;
...
static void draw_widgetbase_batch(uiWidgetBase *wtb)
{
  ...
  if (g_widget_base_batch.enabled && !draw_widgetbase_batch_skip_draw_cache()) {
    ...
    if(wtb->uniform_params.widget_type < WIDGET_TYPE_HEX) {
      g_widget_base_batch.params[g_widget_base_batch.count] = wtb->uniform_params;
      g_widget_base_batch.count++;
    } else {
      g_widget_base_batch.hex_params[g_widget_base_batch.hex_count] = wtb->uniform_params;
      g_widget_base_batch.hex_count++;
    }

    if (g_widget_base_batch.count == MAX_WIDGET_BASE_BATCH ||
        g_widget_base_batch.hex_count == MAX_WIDGET_BASE_BATCH) {
      UI_widgetbase_draw_cache_flush();
    }
  }
  else {
    ...
    const uiBut *but, BIFIconID icon, float alpha, const rcti *rect, const uchar mono_color[4])
{
  ...
  else if (ELEM(but->type, UI_BTYPE_BUT, UI_BTYPE_DECORATOR, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT)) {
  ...
  if (icon && icon != ICON_BLANK1) {
    ...
    else if (but->drawflag & UI_BUT_ICON_RIGHT) {
        xs = rect->xmax - 4.0f * ofs - ICON_DEFAULT_WIDTH;
    }
    else if((ELEM(but->shape, UI_BSHAPE_HEX, UI_BSHAPE_PRO_HEX, UI_BSHAPE_ROUNDBOX)) && (UI_icon_is_geom_type(icon))) {
      xs = (rect->xmin + rect->xmax) / 2.0f - height;
    }
    else {
...
{
  const float aspect = but->block->aspect * U.inv_dpi_fac;
  const int tria_height = (int)(ICON_DEFAULT_HEIGHT / aspect) - 2 * U.pixelsize;
  const int tria_width = (int)(ICON_DEFAULT_WIDTH / aspect) - 5 * U.pixelsize;//
  const int xs = rect->xmax - tria_width + 2 * U.pixelsize;
  const int ys = (rect->ymin + rect->ymax - tria_height) / 2.0f;

  float col[4];

  rgba_uchar_to_float(col, wcol->item);
...
//  draw_anti_tria_rect(&tria_rect, 'h', col);
  const float half = 0.5f * BLI_rctf_size_y(&tria_rect);
  draw_anti_tria(
      tria_rect.xmax, tria_rect.ymin,
      tria_rect.xmax, tria_rect.ymax,
      tria_rect.xmin, tria_rect.ymin + half, col);
}
                             ...
                             rcti *rect)
{
  ...
  if (drawstr_right) {
    ...
    rect->xmax -= UI_TEXT_CLIP_MARGIN * 4;
...
static void widget_draw_text_rotated(const uiFontStyle *fstyle,
                                     const uiWidgetColors *wcol,
                                     uiBut *but,
                                     rcti *rect)
{
  ...
  if (drawlen > 0) {
    int fontid = fstyle->uifont_id;
    float aspect = but->block->aspect;
    float fstyle_points = fstyle->points;
    float h = BLI_rcti_size_y(rect);
    float w = BLI_rcti_size_x(rect);
    float text_w = BLF_width(fontid, drawstr + but->ofs, drawlen);
    int font_flag = BLF_ROTATION;
    if (fstyle->shadow) {
      ...
      BLF_shadow(fstyle->uifont_id, fstyle->shadow, shadow_color);
      BLF_shadow_offset(fstyle->uifont_id, fstyle->shadx, fstyle->shady);
    }
    GPU_blend(GPU_BLEND_ALPHA);
    GPU_line_smooth(true);
    BLF_enable(fontid, font_flag);
    BLF_rotation(fontid, M_PI_2);
    ui_fontscale(&fstyle_points, aspect / (U.dpi_fac * 1.1f));
    BLF_size(fontid, fstyle_points);
    BLF_position(fontid, rect->xmin + (w + fstyle_points / U.dpi_fac) / 2, rect->ymin + (h - text_w) / 2, 0.0f);
    BLF_color3ubv(fontid, wcol->text);
    BLF_draw(fontid, drawstr + but->ofs, drawlen);
    BLF_disable(fontid, font_flag);
...
static void widget_object_draw_text_icon(const bContext *C,
                                         uiBut *but,
                                         uiWidgetType *wt,
                                         rcti *rect)
{
  float x, y;
  x = rect->xmax - ICON_DEFAULT_WIDTH;
  y = rect->ymin + UI_DPI_FAC * 4;
  
  uchar col[4];
  UI_GetThemeColor4ubv(TH_TEXT, col);
  const uiFontStyle *fstyle = UI_FSTYLE_WIDGET;
  const int state = but->flag & ~UI_STATE_FLAGS_ALL;
  
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Object *object = (Object*)but->poin;
  BLI_assert(object);
  ...
  if (state == UI_ACTIVE || base->flag & BASE_SELECTED) {
    BLF_enable(fstyle->uifont_id, BLF_BOLD);
    UI_GetThemeColor4ubv(TH_TEXT_HI, col);
  }
  
  const int icon_size_init = ICON_DEFAULT_HEIGHT;
  const float icon_size = icon_size_init / (but->block->aspect * U.inv_dpi_fac);
  const float icon_padding = 4 * UI_DPI_FAC;
  
  rect->xmin += icon_size - icon_padding;
  rect->xmax -= ICON_DEFAULT_WIDTH * 3 + icon_padding * 5;
  ...
  BLF_disable(fstyle->uifont_id, BLF_BOLD);
  
  UI_GetThemeColor4ubv(TH_SELECT_ACTIVE, col);
  ...
  if (but->drawflag & UI_BUT_ACTIVE_RIGHT) {
    brightness = 1.0f;
  }
  
  int icon_id = ICON_RESTRICT_RENDER_OFF;
  PropertyRNA *prop = RNA_struct_type_find_property(&RNA_Object, "hide_render");
  bool value = RNA_property_boolean_get(&ptr, prop);
  if (value) {
    icon_id = ICON_RESTRICT_RENDER_ON;
  }
  
  UI_icon_draw_ex(x, y, icon_id, U.inv_dpi_fac, brightness, 0.0f, NULL, false, UI_NO_ICON_OVERLAY_TEXT);
  x -= ICON_DEFAULT_WIDTH + icon_padding * 3;
  ...
  icon_id = ICON_DECORATE_LOCKED;
  prop = RNA_struct_type_find_property(&RNA_Object, "hide_select");
  value = RNA_property_boolean_get(&ptr, prop);
  if (value) {
    icon_id = ICON_DECORATE_UNLOCKED;
  }
  
  UI_icon_draw_ex(x, y, icon_id, U.inv_dpi_fac, brightness, 0.0f, NULL, false, UI_NO_ICON_OVERLAY_TEXT);
  x -= ICON_DEFAULT_WIDTH + icon_padding * 2;
  
  brightness = 0.8f;
  if (but->drawflag & UI_BUT_CHECKBOX_INVERT) {
    brightness = 1.0f;
  }
  
  icon_id = ICON_CHECKBOX_HLT;
  prop = RNA_struct_type_find_property(&RNA_Object, "hide_viewport");
  value = RNA_property_boolean_get(&ptr, prop);
  if (value) {
    icon_id = ICON_CHECKBOX_DEHLT;
  }
  
  UI_icon_draw_ex(x, y, icon_id, U.inv_dpi_fac, brightness, 0.0f, col, false, UI_NO_ICON_OVERLAY_TEXT);

}

static void widget_folder_draw_text_icon(const bContext *C,
                                         uiBut *but,
                                         const uiWidgetType *UNUSED(wt),
                                         rcti *rect)
{
  uchar col[4];
  UI_GetThemeColor4ubv(TH_TEXT, col);
  ...
  Collection *collection = (Collection*)but->poin;
  BLI_assert(collection);
  
  const int state = but->flag & ~UI_STATE_FLAGS_ALL;
  
  const uiFontStyle *fstyle = UI_FSTYLE_WIDGET;
  if (state == UI_ACTIVE || &collection->id == &active_lc->collection->id) {
    UI_GetThemeColor4ubv(TH_TEXT_HI, col);
  }
                                  ...
                                  rcti *rect)
{
  ...
  const float icon_scale = but->icon_scale == 0.0f ? 1.0f : but->icon_scale;
  const int icon_size_init = is_tool ? ICON_DEFAULT_HEIGHT_TOOLBAR : ICON_DEFAULT_HEIGHT;
  const float icon_size = icon_size_init / (but->block->aspect * U.inv_dpi_fac);
  const float icon_padding = 2 * UI_DPI_FAC;
  
  if (but->shape == UI_BSHAPE_RHOMBOID && 
    ELEM(but->type, UI_BTYPE_DECORATOR, UI_BTYPE_BUT, UI_BTYPE_LABEL, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT))
  {
    ...
    if (icon != ICON_NONE) {
      const float scale = but->icon_scale != 0.0f ? but->icon_scale : 1.0f;
      rect->xmin -= (icon_size * scale - icon_padding) / 4.0f;
      rect->xmax -= (icon_size * scale - icon_padding) / 4.0f;
    }
    
    uiBut *tmp_btn = but;
    while (tmp_btn->next) {
      tmp_btn = tmp_btn->next;
      switcher ^= (unsigned char)(tmp_btn->shape == UI_BSHAPE_RHOMBOID && 
        ELEM(tmp_btn->type, UI_BTYPE_DECORATOR, UI_BTYPE_BUT, UI_BTYPE_LABEL, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT));
    }
    
    if (switcher) {
      rect->ymin -= icon_padding;
      rect->ymax -= icon_padding;
    }
    else {
      rect->ymin += icon_padding;
      rect->ymax += icon_padding;
...
#endif
    const bool is_triangle = (but->shape == UI_BSHAPE_RHOMBOID &&
                              ELEM(but->type, UI_BTYPE_DECORATOR, UI_BTYPE_BUT, UI_BTYPE_LABEL, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT));
    if (ELEM(but->shape, UI_BSHAPE_HEX, UI_BSHAPE_PRO_HEX) || is_triangle)
    {
      but->drawflag |= UI_BUT_TEXT_NONE;
      but->drawflag &= ~UI_BUT_TEXT_LEFT;
      but->drawflag &= ~UI_BUT_ICON_LEFT;
    }
  ...
  if(!(but->drawflag & UI_BUT_TEXT_NONE)) {
    float scale = but->icon_scale == 0.0f ? 1.0f : but->icon_scale;
    if (strcmp(but->str, "   STREAM\n RENDERING") == 0) {
      scale = 0.85f;
      BLI_strncpy(but->drawstr, but->str, BC_STRING_MAX);
    }
    ((uiFontStyle *)fstyle)->points *= scale;
    
    const bool is_bold = (but->shape == UI_BSHAPE_PREFS &&
                          but->type == UI_BTYPE_LABEL) ||
                         (but->shape == UI_BSHAPE_ADVANCED &&
...
static void widget_state_menu_image(uiWidgetType *wt,
                                    const uiWidgetStateInfo *state,
                                    eUIEmbossType UNUSED(emboss))
{
  wt->wcol = *(wt->wcol_theme);
  ...
  if (state->but_flag & (UI_SELECT | UI_ACTIVE)) {
    copy_v3_v3_uchar(wt->wcol.inner, wt->wcol.text_sel);
  }
}

static void widget_state_animation(uiWidgetType *wt,
                                   const uiWidgetStateInfo *state,
                                   eUIEmbossType UNUSED(emboss))
{
  wt->wcol = *(wt->wcol_theme);
  if (state->but_flag & (UI_ACTIVE | UI_SELECT)) {
    copy_v3_v3_uchar(wt->wcol.inner, wt->wcol.inner_sel);
  }
}
...
static void ui_draw_separator_prefs(const rcti *rect, const uiWidgetColors *wcol)
{
  const int y = rect->ymin - BLI_rcti_size_y(rect) * 2 - 1;
  const uchar col[4] = {
      wcol->outline[0],
                               ...
                               bool emboss)
{
  ...
  BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
  ...
  widgetbase_draw(&wtb, wcol);
    ...
    if (!state->is_text_input) {
      const int handle_width = BLI_rcti_size_y(rect) * 0.5f;
      const int handle_height = BLI_rcti_size_y(rect) * 0.5f;

      const int handle_pos_x = BLI_rcti_size_x(rect) - handle_width * 2.f;
      
      rcti rect_zone = *rect;
      rect_zone.xmin = rect->xmin + handle_pos_x + 2 * U.pixelsize;
      rect_zone.ymin = rect->ymin + 2 * U.pixelsize;
      rect_zone.ymax = rect->ymin + handle_height;

      shape_preset_init_number_arrows(&wtb.tria1, &rect_zone, 0.5f, 'm');
      
      wtb.tria1.center[0] = roundf(wtb.tria1.center[0]);
      wtb.tria1.center[1] = floorf(wtb.tria1.center[1]);

      wtb.tria1.size = floorf(-0.3f * handle_height);
      wtb.tria1.size += ((int)wtb.tria1.size) % 2;

      rect_zone.xmin = rect->xmin + handle_pos_x + 2 * U.pixelsize;
      rect_zone.ymax = rect->ymax - 2 * U.pixelsize;
      rect_zone.ymin = rect->ymax - handle_height;

      shape_preset_init_number_arrows(&wtb.tria2, &rect_zone, 0.5f, 'm');

      wtb.tria2.center[1] = ceilf(wtb.tria2.center[1]);

      wtb.tria2.center[0] = wtb.tria1.center[0];
      wtb.tria2.size = wtb.tria1.size;
   }	
  
  wtb.uniform_params._pad[0] = wtb.uniform_params._pad[1] = 0.f;
  
  if(state->but_flag & UI_STATE_ACTIVE_LEFT)
    wtb.uniform_params._pad[0] = 1.f;
  if(state->but_flag & UI_STATE_ACTIVE_RIGHT)
    wtb.uniform_params._pad[1] = 1.f;

   wcol->outline[3] = wcol->inner_sel[3] = wcol->inner[3] = 0;

   wtb.uniform_params.widget_type = WIDGET_TYPE_RECT;
   wtb.totvert = 10;
   widgetbase_draw(&wtb, wcol);
}


static void widget_outliner_object(uiBut *but,
    uiWidgetColors *wcol, rcti *rect, int state, int roundboxalign, const float zoom)
{

}

static void widget_outliner_folder(uiBut *but,
    uiWidgetColors *wcol, rcti *rect, int state, int roundboxalign, const float zoom)
{
  GPU_blend(GPU_BLEND_ALPHA); /* Only once. */
  
  float col_selection[4];
  UI_GetThemeColor3fv(TH_SELECT_ACTIVE, col_selection);
  col_selection[3] = 0.5f; /* No alpha. */
  ...
  immRecti(pos, but->rect.xmin, but->rect.ymin,  but->rect.xmax, but->rect.ymax);
  
  immUnbindProgram();
  GPU_blend(GPU_BLEND_NONE);
}

static void widget_triangle_label(uiBut *but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  unsigned char switcher = 0;
  while (but->next) {
    but = but->next;
    switcher ^= (unsigned char)(but->shape == UI_BSHAPE_RHOMBOID &&
      ELEM(but->type, UI_BTYPE_DECORATOR, UI_BTYPE_BUT, UI_BTYPE_LABEL, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT));
  }
  ...
  round_box_edges(&wtb, roundboxalign, rect, rad);
   ...
   BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
   wtb.uniform_params.widget_type = WIDGET_TYPE_TRIANGLE + switcher;
   wtb.totvert = 3;
   widgetbase_draw(&wtb, wcol);
}
...
static void widget_numbut_rhomboid(uiBut *but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  unsigned char switcher = 0;
  while(but->prev) {
    but = but->prev;
    switcher ^= (unsigned char)(but->shape == UI_BSHAPE_RHOMBOID && but->type == UI_BTYPE_NUM);
  }
  ...
  //(state & UI_ACTIVE) &&
    if (!state->is_text_input) {
      const int handle_width = BLI_rcti_size_x(rect) / 3;
      const int handle_height = BLI_rcti_size_y(rect) * 0.5f;
      
      rcti rect_zone = *rect;
      rect_zone.xmin = rect->xmax - handle_width * 0.5 - U.pixelsize;
      rect_zone.ymin = rect->ymin + handle_height * 0.75;
      rect_zone.ymax = rect->ymin + handle_height * 1.25 + U.pixelsize;

      shape_preset_init_number_arrows(&wtb.tria1, &rect_zone, 0.5f, 't');
      ...
      rect_zone.xmin = rect->xmax - handle_width * 0.5 - U.pixelsize;
      rect_zone.ymax = rect->ymax - handle_height * 0.25;
      rect_zone.ymin = rect->ymax - handle_height - U.pixelsize;

      shape_preset_init_number_arrows(&wtb.tria2, &rect_zone, 0.5f, 'b');
  ...
  if(state->but_flag & UI_STATE_ACTIVE_RIGHT)
   ...
   BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
   wtb.uniform_params.widget_type = WIDGET_TYPE_RHOMBOID + switcher;
   wtb.totvert = 10;
   widgetbase_draw(&wtb, wcol);
}
    ...
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  ...
  if (state->but_flag & UI_ACTIVE) {
    wtb.uniform_params._pad[0] = wtb.uniform_params._pad[1] = 1.f;
    copy_v3_v3_uchar(wcol->text, wcol->text_sel);
  }
  ...
  rcti rect_zone = *rect;
  rect_zone.xmax -= (6 * BLI_rcti_size_y(rect)) / 10;
  rect_zone.xmin = rect->xmax - handle_width * 0.5 - U.pixelsize;
  rect_zone.ymin = rect->ymin + handle_height * 0.75;
  rect_zone.ymax = rect->ymin + handle_height + U.pixelsize;

  shape_preset_trias_from_rect_menu(&wtb.tria1, &rect_zone);

  wtb.tria1.center[0] = roundf(wtb.tria1.center[0]);
  wtb.tria1.center[1] = floorf(wtb.tria1.center[1]);

  wtb.tria1.size = floorf(-0.4f * handle_height);
  wtb.tria1.size += ((int)wtb.tria1.size) % 2;
...
static void widget_searchbut(
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *UNUSED(state), int roundboxalign, const float zoom)
{
  const uint pos = GPU_vertformat_attr_add(
      immVertexFormat(), "pos", GPU_COMP_F32, 2, GPU_FETCH_FLOAT);
  immBindBuiltinProgram(GPU_SHADER_3D_UNIFORM_COLOR);
...
//  immUniformColor4f(0.0f, 0.66f, 0.95f, 1.0f);
  
  immBegin(GPU_PRIM_LINES, 2);
  
  immVertex2f(pos, rect->xmin + (rect->xmax - rect->xmin) / 8.0f, rect->ymin);
                               ...
                               const float zoom)
{
  ...
  const float center = BLI_rcti_cent_y(rect);
  const float new_height = BLI_rcti_size_y(rect) * 0.5f;
  rect->ymin = center - new_height / 2;
  rect->ymax = center + new_height / 2;
...
static void widget_animbar(
    uiBut *but, uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo* UNUSED(state), int UNUSED(roundboxalign), float UNUSED(zoom))
{
  const float line_width = 0.5f;
  
  const float width = BLI_rcti_size_x(rect);
  const float aspect = but->block->aspect * U.inv_dpi_fac;
  const float icon_height = ICON_DEFAULT_HEIGHT / aspect;

  float col[4] = {0.0f};
  float factor = (ui_but_value_get(but) - but->hardmin) / (but->hardmax - but->hardmin);
  float icon_x = rect->xmin + factor * (float)width;
  int segment_count, keyframe_count;
  segment_count = keyframe_count = but->hardmax - but->hardmin;
  
  bContext *C = but->block->evil_C;
  bAnimContext ac;
  ListBase selected;

  CLAMP(segment_count, 1, 25);
  CLAMP_MIN(keyframe_count, 1);
  CLAMP(icon_x, rect->xmin, rect->xmax);
  CLAMP(factor, 0, 1);
    
  BLI_rcti_resize_y(rect, BLI_rcti_size_y(rect) * 0.4f);
  ...
  rgb_uchar_to_float(col, wcol->text);
  col[3] = 1.0f;
  immUniformColor4fv(col);
  
  immBegin(GPU_PRIM_LINES, 2);
  float y = BLI_rcti_cent_y(rect) - line_width / 2;
  immVertex2f(pos, rect->xmin, y);
  immVertex2f(pos, rect->xmax, y);
  immEnd();
  
  immBegin(GPU_PRIM_LINES, 2 * (segment_count + 1));
  for (int i = 0; i <= segment_count; i++) {
    float x = rect->xmin + BLI_rcti_size_x(rect) * i / (float)segment_count;
  ...
  if (ANIM_animdata_get_context(C, &ac) && !BLI_listbase_is_empty(&selected)) {
    ListBase *keys = get_channel_keyframes(C, &ac);

    if (!BLI_listbase_is_empty(keys)) {
      immBeginAtMost(GPU_PRIM_LINES, 4 * BLI_listbase_count(keys));
      LISTBASE_FOREACH(LinkData *, link, keys) {
        float value = *(float*)(link->data);
        if (value < but->hardmin) {
          continue;
        }
        // I assume that all the values are sorted already, can cause problems if it's not so...
        if (value > but->hardmax) {
          break;
        }
        float x = rect->xmin + BLI_rcti_size_x(rect) *  (value - but->hardmin)  / (but->hardmax - but->hardmin);
        immVertex2f(pos, x - BLI_rcti_size_y(rect) / 2, rect->ymin + 1);
        immVertex2f(pos, x + BLI_rcti_size_y(rect) / 2, rect->ymax - 1);
        immVertex2f(pos, x - BLI_rcti_size_y(rect) / 2, rect->ymax - 1);
        immVertex2f(pos, x + BLI_rcti_size_y(rect) / 2, rect->ymin + 1);
      }
      immEnd();
    }
    ED_keyframes_free(keys);
  }
  ...
  UI_icon_draw_ex(icon_x - icon_height / 2, y - icon_height / 2, ICON_IXAM, aspect, 1.0f, 0.0f, wcol->inner, false, UI_NO_ICON_OVERLAY_TEXT);
}
                                 ...
                                 const float zoom)
{
  ...
  } else if(but->shape == UI_BSHAPE_PREFS) {
    /* Preferences icons should have outline color */
    copy_v3_v3_uchar(wcol->text, wcol->outline);
  }
}
...
static void draw_widget_viewconfig(const bContext *C, uiBut *UNUSED(but), uiWidgetType *wt, rcti *rect)
{
  ...
  const int but_x = BLI_rcti_size_x(rect);
  const int but_y = BLI_rcti_size_y(rect);
  const int offset = 10;
  int quad_type = 0;
  
  const char views[] = {'F', 'B', 'L', 'R', 'T', 'B'};
  rcti bound_rect = *rect;
  bound_rect.xmax -= offset;
  bound_rect.ymax -= offset;
  
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
    if (region->regiontype == RGN_TYPE_WINDOW) {
      RegionView3D *rv3d = region->regiondata;
      quad_type = rv3d->quad_type;
      rcti normal_rect;
      normal_rect.xmin = rect->xmin + (float)region->winrct.xmin / area->winx * but_x;
      normal_rect.xmax = rect->xmin + (float)region->winrct.xmax / area->winx * but_x - offset;
      normal_rect.ymin = rect->ymin + (float)region->winrct.ymin / area->winy * but_y;
      normal_rect.ymax = rect->ymin + (float)region->winrct.ymax / area->winy * but_y - offset;
      
//      immUniformColor4f(0.0f, 0.66f, 0.95f, 1.0f);
      immUniformColor4ubv(UI_GetTheme()->tui.line_color);
      imm_draw_box_wire_2d(pos , normal_rect.xmin, normal_rect.ymin, normal_rect.xmax, normal_rect.ymax);
      
      immUniformColor4f(0.77f, 0.77f, 0.77f, 0.5f);
      int line_count = 4;
      int points [][2] = {
        {normal_rect.xmin - offset / 2, normal_rect.ymin - offset / 2},
        {normal_rect.xmin - offset / 2, normal_rect.ymax + offset / 2},
        {normal_rect.xmax + offset / 2, normal_rect.ymax + offset / 2},
        {normal_rect.xmax + offset / 2, normal_rect.ymin - offset / 2}
      };
      int line_mask[4] = {1, 1, 1, 1};
      if (normal_rect.xmin - offset / 2 < bound_rect.xmin) {
        line_mask[0] = 0;
        line_count--;
      }
      if (normal_rect.ymin - offset / 2 < bound_rect.ymin) {
        line_mask[3] = 0;
        line_count--;
      }
      if (normal_rect.ymax + offset / 2 > bound_rect.ymax) {
        line_mask[1] = 0;
        line_count--;
      }
      if (normal_rect.xmax + offset / 2 > bound_rect.xmax) {
        line_mask[2] = 0;
        line_count--;
      }
      ...
      if(line_count) {
        ...
        for (int i =0; i < 4; i++) {
          if (line_mask[i]) {
            ...
            immVertex2f(pos, points[(i + 1) % 4][0], points[(i + 1) % 4][1]);
          }
        }
        immEnd();
      }
...
static void widget_menu_image(uiBut* but,
                              uiWidgetColors* wcol,
                              rcti *rect,
                              const uiWidgetStateInfo *UNUSED(state),
                              int UNUSED(roundboxalign),
                              const float UNUSED(zoom)) {
  extern char datatoc_topbar_logo_png[];
  extern int datatoc_topbar_logo_png_size;
  
  ImBuf* ibuf = IMB_ibImageFromMemory((unsigned const char*)datatoc_topbar_logo_png,
                                      datatoc_topbar_logo_png_size,
                                      IB_rect, NULL, "topbar logo");
  ui_draw_but_menu_IMAGE(NULL, but, ibuf, wcol, rect);
  IMB_freeImBuf(ibuf);
}

static void widget_animation_custom(uiBut* UNUSED(but),
                                    uiWidgetColors *wcol,
                                    rcti *rect,
                                    const uiWidgetStateInfo *state,
                                    int UNUSED(roundboxalign),
                                    const float UNUSED(zoom))
{
  ...
  if(state->but_flag & UI_SELECT) {
    widgetbase_set_uniform_colors_ubv(
        &wtb, wcol->inner, wcol->inner, wcol->text, wcol->text_sel, wcol->item, true);
    wtb.uniform_params.widget_type = 2.0f;
  } else {
    wtb.uniform_params.widget_type = 1.0f;
    widgetbase_set_uniform_colors_ubv(
        &wtb, wcol->inner, wcol->item, wcol->text, wcol->text_sel, wcol->item, true);
  }
  BLI_rctf_init(&wtb.uniform_params.recti, rect->xmin, rect->xmax, rect->ymin, rect->ymax);
  BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
  GPU_blend(GPU_BLEND_ALPHA);
  GPUBatch *batch = ui_batch_roundbox_widget_get();
  GPU_batch_program_set_builtin(batch, GPU_SHADER_2D_ANIMATION);
  GPU_batch_uniform_4fv_array(
      batch, "parameters", MAX_WIDGET_PARAMETERS, (float(*)[4])(&wtb.uniform_params));
  GPU_batch_draw(batch);
  GPU_blend(GPU_BLEND_NONE);
...
static void widget_animation_draw_ex(uiWidgetColors *wcol,
                                 rcti *rect,
                                 int state,
                                 int oundboxalign,
                                 const float zoom) 
{
  widget_animation_custom(NULL, wcol, rect, state, oundboxalign, zoom);
}
...
static void widget_optionbut_advanced(uiWidgetColors *wcol,
                             rcti *rect,
                             const uiWidgetStateInfo *state,
                             int UNUSED(roundboxalign),
                             const float UNUSED(zoom))
{
  const bool text_before_widget = (state->but_flag & UI_STATE_TEXT_BEFORE_WIDGET);
  rcti recttemp = *rect;
  ...
  recttemp.xmax = recttemp.xmin + BLI_rcti_size_x(&recttemp) * 1.8f; // Figma metrics
  ...
  *rect = recttemp;
  ...
  if (state->but_flag & UI_SELECT) {
    //shape_preset_trias_from_rect_checkmark(&wtb.tria1, &recttemp);
//    copy_v4_v4_uchar(wcol->inner, wcol->inner_sel);
    uchar mod[4] = {0xff, 0xff, 0xff, 0x00};
    interp_v4_v4v4_uchar(wcol->inner, wcol->inner, mod, 0.2);
  }

  widgetbase_draw(&wtb, wcol);
  
  int w = BLI_rcti_size_x(&recttemp) / 1.8f;
  
  if(state->but_flag & UI_SELECT)
    recttemp.xmin += w;
  else
    recttemp.xmax = recttemp.xmin + w;
  round_box_edges(&wtb, UI_CNR_ALL, &recttemp, rad);
...
static void widget_rowbut_rotated(uiBut* but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  if (state->but_flag & (UI_SELECT | UI_ACTIVE)) {
    copy_v4_v4_uchar(wcol->inner, wcol->inner_sel);
  ...
  float col[4], col_out[4];
  float h = BLI_rcti_size_y(rect);
  float w = BLI_rcti_size_x(rect);
  rctf rct;
  char switcher = 0;
  int shape = WIDGET_TYPE_ROUNDBOX;

  switch (but->shape) {
    case UI_BSHAPE_TRAPEZOID:
      shape = WIDGET_TYPE_TRAPEZOID;
      break;
    case UI_BSHAPE_TRAPEZOIDR:
      shape = WIDGET_TYPE_RTRAPEZOID;
      break;

    case UI_BSHAPE_RHOMBOID:
      shape = WIDGET_TYPE_RHOMBOID;
      break;
  ...
  if (ELEM(shape, WIDGET_TYPE_TRAPEZOID, WIDGET_TYPE_RTRAPEZOID)) {
    uiBut *tmp_btn = but;
    while (tmp_btn->next) {
      tmp_btn = tmp_btn->next;
      // if (tmp_btn->shape == but->shape)
        // switcher ^= 1;
    }
    if (ELEM(shape, WIDGET_TYPE_RTRAPEZOID)) {
      shape -= switcher;
    } else {
      shape += switcher;
    }
  }
  rgba_uchar_to_float(col, wcol->inner);
  rgba_uchar_to_float(col_out, wcol->outline);
  if (shape != WIDGET_TYPE_ROUNDBOX) {
    BLI_rctf_init(&rct, rect->xmin, rect->xmin + h, rect->ymin, rect->ymin + w);
    GPU_matrix_push();
    GPU_matrix_translate_2f(rect->xmin, rect->ymin);
    GPU_matrix_rotate_2d(90);
    GPU_matrix_translate_2f(-rect->xmin, -rect->ymin - w);
    UI_draw_shape_4fv_ex(&rct, col, NULL, 0, col_out, U.pixelsize, shape);
    GPU_matrix_pop();
  } else {
    BLI_rctf_init(&rct, rect->xmin, rect->xmax, rect->ymin, rect->ymax);
    UI_draw_roundbox_4fv_ex(&rct, col, NULL, 0, col_out, U.pixelsize, 0);
  }
}


static void widget_rowbut(uiBut* but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *UNUSED(state), int roundboxalign, const float zoom)
{
  uiWidgetBase wtb;
  widget_init(&wtb);
  ...
  // Dunno why, but menu item has this flag
  // UI_BUT_UNDO = 1 << 19
  if(but->flag & (1 << 19)) {
    
    color_blend_v3_v3(backup, wcol->outline, 1.f);
    color_blend_v3_v3(wcol->outline, wcol->inner, 1.f);
  }
  ...
  if(but->flag & (1 << 19))
    color_blend_v3_v3(wcol->outline, backup, 1.f);
}
...
static void widget_pro_pre_render(ARegion *region, uiBut *but)
{
  but->drawflag &= ~UI_BUT_TEXT_LEFT;
  ...
  float w = BLI_rcti_size_x(&region->winrct);
  float h = BLI_rcti_size_y(&region->winrct);

  GPU_matrix_perspective_set(90, w / h, 1, h);
  GPU_matrix_look_at(
                     // eye
                    U.hand_type == USER_HAND_RIGHT ? 0 : w, h / 2, h / 2,
                     // center
                     w / 2, h / 2, 0,
                     // up
                     0, 1, 0
...
static void widget_pro_post_render(ARegion* UNUSED(region), uiBut * but) {
  {
    float vp[4];
    GPU_viewport_size_get_f(vp);
    float w = vp[2] - vp[0];  // BLI_rctf_size_x(&but->block->rect);
    float h = vp[3] - vp[1];  // BLI_rctf_size_y(&but->block->rect);

    float bw = BLI_rctf_size_x(&but->rect);
    float bh = BLI_rctf_size_y(&but->rect);
        
    float m[4][4];
    float tl[4] = {but->rect.xmin + bw / 2 - bh / 2, but->rect.ymin , 0, 1};
    float br[4] = {but->rect.xmin + bw / 2 + bh / 2, but->rect.ymin + bh, 0, 1};

    GPU_matrix_model_view_projection_get(m);
    memcpy(but->drawstr + 200, m, sizeof(m));
    
    mul_v4_m4v4(tl, m, tl);
    mul_v4_m4v4(br, m, br);
    
    mul_v4_fl(tl, 1.f / tl[3]);
    mul_v4_fl(br, 1.f / br[3]);
    
    but->rect.xmin = (tl[0] + 1) * w / 2;
    but->rect.xmax = (br[0] + 1) * w / 2;

    but->rect.ymin = (tl[1] + 1) * h / 2;
    but->rect.ymax = (br[1] + 1) * h / 2;
...
static void widget_hex_exec(
    uiBut *but, uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  uiWidgetBase wtb;
  widget_init(&wtb);
  ...
  bool has_hold_action = state->has_hold_action;
  has_hold_action |= (but->shape == UI_BSHAPE_HEX && but->type == UI_BTYPE_MENU);

  if (has_hold_action) {
    /* Show that keeping pressed performs another action (typically a menu). */
    shape_preset_init_hold_action(&wtb.tria1, rect, 0.4f, 'l');
  }

  BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
  wtb.uniform_params.widget_type = (but->shape == UI_BSHAPE_PRO_HEX) ?
    WIDGET_TYPE_PRO_HEX : WIDGET_TYPE_HEX;
  wtb.tria1.type = ROUNDBOX_TRIA_HOLD_ACTION_ARROW;
...
static void widget_advanced_exec(uiWidgetColors *wcol,
                                 rcti *rect,
                                 const uiWidgetStateInfo *state,
                                 int roundboxalign,
                                 const float zoom)
{
  ...
  copy_v4_v4_uchar(wcol->outline, wcol->inner);
...
static void widget_trapezoid_exec(uiBut * but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  uiWidgetBase wtb;
  widget_init(&wtb);
  ...
  if (state->but_flag & UI_STATE_HOLD_ACTION) {
    /* Show that keeping pressed performs another action (typically a menu). */
    shape_preset_init_hold_action(&wtb.tria1, rect, 0.75f, 'r');
  }
  ...
  /* half rounded */
  BLI_rctf_rcti_copy(&wtb.uniform_params.rect, rect);
  if (but->shape == UI_BSHAPE_TRAPEZOID) {
    wtb.uniform_params.widget_type = WIDGET_TYPE_TRAPEZOID;
  }
  else if (but->shape == UI_BSHAPE_TRAPEZOIDR) {
    wtb.uniform_params.widget_type = WIDGET_TYPE_RTRAPEZOID;
  }
...
static void widget_matpro_tab_exec(uiBut * but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom)
{
  bContext *C = but->block->evil_C;
  Object *obact = CTX_data_active_object(C);
  if (obact) {
    int actcol = MAX2(obact->actcol - 1, 0);
    int tab_index = (int)but->poin;
    if (actcol == tab_index) {
      copy_v3_v3_uchar(wcol->inner, wcol->inner_sel);
      wcol->inner[3] = 0x80;
    }
  }

  widget_trapezoid_exec(but, wcol, rect, state, roundboxalign, zoom);
  ...
  GPU_blend(GPU_BLEND_ALPHA);
  uchar col[4];
  UI_GetThemeColor4ubv(TH_TAB_OUTLINE, col);
  col[3] = 255;
  const int draw_size = UI_DPI_ICON_SIZE;
  const float w = BLI_rcti_size_x(rect);
  const float x = rect->xmin + w * (1 - UI_TAB_CLOSE_OFFSET) - draw_size / 2;
  const float y = BLI_rcti_cent_y_fl(rect) - draw_size / 2;
  const float aspect = but->block->aspect * U.inv_dpi_fac;
  UI_icon_draw_ex(x, y, ICON_X, aspect, 1.0, 0.0, col, false, UI_NO_ICON_OVERLAY_TEXT);
  GPU_blend(GPU_BLEND_NONE);
}

static void widget_matlist_item_exec(uiBut * but,
    uiWidgetColors *wcol, rcti *rect, const uiWidgetStateInfo *state, int roundboxalign, const float zoom) 
{
  uiWidgetBase wtb;
  widget_init(&wtb);

  if (but->shape == UI_BSHAPE_PREFS) {
    copy_v3_v3_uchar(wcol->inner, wcol->outline);
    wcol->inner[3] = 0x80;
  }
  ...
  Material *material = (Material *)but->poin;
  BLI_assert(material != NULL);
  const int w = BLI_rctf_size_x(&but->rect) - 2 * UI_DPI_FAC;
  uiDefBut(but->block, UI_BTYPE_LABEL, 0, material->id.name + 2, but->rect.xmin + UI_DPI_FAC, but->rect.ymin, w, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
}
...
static uiWidgetType *widget_type(uiWidgetTypeEnum type)
{
  ...
  wt.draw = widget_but;
  wt.pre_render = wt.post_render = NULL;
  wt.custom = NULL;
  ...
  switch (type) {
    ...
    case UI_WTYPE_OUTLINER_OBJECT:
      break;
      
    case UI_WTYPE_OUTLINER_FOLDER:
      wt.draw = NULL;
//      wt.text = widget_folder_draw_text_icon;
      break;
      
    case UI_WTYPE_TRIANGLE_LABEL:
      wt.custom = widget_triangle_label;
      wt.state = widget_state_label;
      break;
    ...
    case UI_WTYPE_MENU_ROW:
      wt.wcol_theme = &btheme->tui.wcol_radio;
      wt.custom = widget_rowbut;
      break;

    case UI_WTYPE_MENU_VERTROW:
      wt.wcol_theme = &btheme->tui.wcol_vert_trapezoid;
      wt.custom = widget_rowbut_rotated;
      wt.text = widget_draw_text_rotated;
      break;
    ...
    case UI_WTYPE_CHECKBOX_ADVANCED:
      wt.wcol_theme = &btheme->tui.wcol_option;
      wt.draw = widget_optionbut_advanced;
      break;
    ...
    case UI_WTYPE_RADIO:
      ...
      break;
    case UI_WTYPE_PREF_NUMBER:
      wt.wcol_theme = &btheme->tui.wcol_prefs_regular;
    ...
    case UI_WTYPE_NUMBER_RHOMBOID:
      wt.wcol_theme = &btheme->tui.wcol_num;
      wt.custom = widget_numbut_rhomboid;
      break;
    ...
    case UI_WTYPE_PREF_SLIDER:
      wt.state = widget_state_numslider;
      wt.wcol_theme = &btheme->tui.wcol_prefs_regular;
      break;
      
    case UI_WTYPE_ANIMATION_BAR:
      wt.wcol_theme = &btheme->tui.wcol_animbar;
      wt.custom = widget_animbar;
      wt.state = widget_state_animation;
      wt.text = NULL;
      break;
    case UI_WTYPE_EXEC_PREFS:
      wt.wcol_theme = &btheme->tui.wcol_prefs_regular;
      wt.draw = widget_roundbut_exec;
      break;
    case UI_WTYPE_EXEC_ADVANCED:
      wt.wcol_theme = &btheme->tui.wcol_tool;
      wt.draw = widget_advanced_exec;
      break;
    case UI_WTYPE_EXEC:
      wt.wcol_theme = &btheme->tui.wcol_regular;
      wt.draw = widget_roundbut_exec;
      break;
    case UI_WTYPE_DECORATOR:
    ...
    case UI_WTYPE_TOOLBAR_ITEM:
      ...
      wt.draw = widget_roundbut_exec;
      break;
    case UI_WTYPE_PRO_ITEM_HEX:
      wt.wcol_theme = &btheme->tui.wcol_toolbar_item_pro;
      wt.custom = widget_hex_exec;
      wt.pre_render = widget_pro_pre_render;
      wt.post_render = widget_pro_post_render;
      break;
    case UI_WTYPE_TOOLBAR_ITEM_HEX:
      wt.wcol_theme = &btheme->tui.wcol_toolbar_item;
      wt.custom = widget_hex_exec;
      break;
    case UI_WTYPE_TOOLBAR_ITEM_TRIANGLE:
      wt.wcol_theme = &btheme->tui.wcol_toolbar_item;
      wt.custom = widget_triangle_label;
      break;
    ...
    case UI_WTYPE_SEARCH:
      wt.wcol_theme = &btheme->tui.wcol_text;
      wt.draw = widget_searchbut;
      break;
    ...
    case UI_WTYPE_PULLDOWN_TRAPEZOID:
      wt.wcol_theme = &btheme->tui.wcol_pulldown;
      wt.custom = widget_trapezoid_exec;
      wt.state = widget_state_pulldown;
      break;
    
    case UI_WTYPE_MATPRO_TAB:
      wt.wcol_theme = &btheme->tui.wcol_pulldown;
      wt.custom = widget_matpro_tab_exec;
      wt.state = widget_state_pulldown;
      break;

    case UI_WTYPE_MATLIST_ITEM:
      wt.wcol_theme = &btheme->tui.wcol_pulldown;
      wt.custom = widget_matlist_item_exec;
      wt.state = widget_state_pulldown;
      break;
    ...
    case UI_WTYPE_IMAGE_MENU:
      wt.text = NULL;
      wt.state = widget_state_menu_image;
      wt.wcol_theme = &btheme->tui.wcol_radio;
      wt.custom = widget_menu_image;
      break;
      
    case UI_WTYPE_ANIMATION:
      wt.text = NULL;
      wt.draw = NULL;
      wt.wcol_theme = &btheme->tui.wcol_animbar;
      wt.custom = widget_animation_custom;
      wt.state = widget_state_animation;
      break;
  }
...
static int widget_roundbox_set(uiBut *but, rcti *rect)
{
  ...
  /* alignment */
  if ((but->drawflag & UI_BUT_ALIGN) && but->type != UI_BTYPE_PULLDOWN && but->type != UI_BTYPE_IMAGE_MENU) {
  ...
  if (but->active && (but->type != UI_BTYPE_POPOVER || but->type != UI_BTYPE_VIEWCONFIG)
      && !ui_but_menu_draw_as_popover(but)) {
...
void ui_draw_but(const bContext *C, struct ARegion *region, uiStyle *style, uiBut *but, rcti *rect)
{
  ...
  if (but->emboss == UI_EMBOSS_PULLDOWN) {
    ...
    switch (but->type) {
      case UI_BTYPE_LABEL:
        if (but->shape == UI_BSHAPE_RHOMBOID) {
          wt = widget_type(UI_WTYPE_TRIANGLE_LABEL);
        }
        else {
      ...
      case UI_BTYPE_SEPR_LINE:
          if(but->shape == UI_BSHAPE_PREFS)
            ui_draw_separator_prefs(rect, &tui->wcol_prefs_regular);
          else if(but->shape == UI_BSHAPE_ADVANCED)
            ui_draw_separator(rect, &tui->wcol_menu_item);
        break;
      case UI_BTYPE_MATOPERATOR:
        wt = widget_type(UI_WTYPE_TOOLBAR_ITEM_HEX);
        break;
      case UI_BTYPE_MATPRO_TAB:
        wt = widget_type(UI_WTYPE_MATPRO_TAB);
        break;
      case UI_BTYPE_MATLIST_BUT:
        wt = widget_type(UI_WTYPE_MATLIST_ITEM);
        break;
      case UI_BTYPE_BUT:
      case UI_BTYPE_DECORATOR:
        if (but->shape == UI_BSHAPE_HEX)
          wt = widget_type(UI_WTYPE_TOOLBAR_ITEM_HEX);
        else if(but->shape == UI_BSHAPE_RHOMBOID)
          wt = widget_type(UI_WTYPE_TOOLBAR_ITEM_TRIANGLE);
        else if(but->shape == UI_BSHAPE_PRO_HEX)
          wt = widget_type(UI_WTYPE_PRO_ITEM_HEX);
        else if ((but->icon != ICON_NONE) && UI_but_is_tool(but))
          wt = widget_type(UI_WTYPE_DECORATOR);
        else if(but->shape == UI_BSHAPE_PREFS)
          wt = widget_type(UI_WTYPE_EXEC_PREFS);
        else if(but->shape == UI_BSHAPE_ADVANCED)
          wt = widget_type(UI_WTYPE_EXEC_ADVANCED);
        else if(ELEM(but->shape, UI_BSHAPE_TRAPEZOID, UI_BSHAPE_TRAPEZOIDR)) {
          wt = widget_type(UI_WTYPE_PULLDOWN_TRAPEZOID);
        }
        else
      ...
      case UI_BTYPE_NUM:
        if (but->shape == UI_BSHAPE_RHOMBOID)
          wt = widget_type(UI_WTYPE_NUMBER_RHOMBOID);
        else if(but->shape == UI_BSHAPE_PREFS)
          wt = widget_type(UI_WTYPE_PREF_NUMBER);
        else
      ...
      case UI_BTYPE_NUM_SLIDER:
        if(but->shape == UI_BSHAPE_PREFS)
          wt = widget_type(UI_WTYPE_PREF_SLIDER);
        else
          wt = widget_type(UI_WTYPE_SLIDER);
        break;

      case UI_BTYPE_ANIMATION_BAR:
        wt = widget_type(UI_WTYPE_ANIMATION_BAR);
        break;
        
      case UI_BTYPE_ROW:
        if(but->shape == UI_BSHAPE_PREFS) {
          wt = widget_type(UI_WTYPE_MENU_ROW);
          bTheme* theme = UI_GetTheme();
          wt->wcol_theme = &theme->tui.wcol_prefs_regular;
        } else if (ELEM(but->shape, UI_BSHAPE_TRAPEZOID, UI_BSHAPE_TRAPEZOIDR)) {
          wt = widget_type(UI_WTYPE_PULLDOWN_TRAPEZOID);
        }
        else wt = widget_type(UI_WTYPE_MENU_ROW);
        break;

      case UI_BTYPE_VERTROW:
        wt = widget_type(UI_WTYPE_MENU_VERTROW);
        break;
      ...
      case UI_BTYPE_LISTROW:
        if(but->shape == UI_BSHAPE_PREFS) {
          ...
          bTheme* theme = UI_GetTheme();
          wt->wcol_theme = &theme->tui.wcol_prefs_regular;
        } else wt = widget_type(UI_WTYPE_LISTITEM);
        break;

      case UI_BTYPE_TEXT:
        if (but->shape == UI_BSHAPE_PREFS) {
          wt = widget_type(UI_WTYPE_NAME_PREFS);
        }
        else if (but->shape == UI_BSHAPE_ADVANCED) {
          wt = widget_type(UI_WTYPE_SEARCH);
        }
        else {
      ...
      case UI_BTYPE_CHECKBOX_N:
        if (!(but->flag & UI_HAS_ICON)) {
          ...
          if(but->shape == UI_BSHAPE_ADVANCED)
            wt = widget_type(UI_WTYPE_CHECKBOX_ADVANCED);
          else
      ...
      case UI_BTYPE_VIEWCONFIG:
        draw_widget_viewconfig(C, but, wt, rect);
        break;
      
      case UI_BTYPE_OUTLINEROBJECT:
        widget_object_draw_text_icon(C, but, wt, rect);
        break;
        
      case UI_BTYPE_OUTLINERFOLDER:
        widget_folder_draw_text_icon(C, but, wt, rect);
        break;
      ...
      case UI_BTYPE_POPOVER:
        if (but->shape == UI_BSHAPE_HEX)
          ...
          if ((!but->str[0] && but->icon && (BLI_rcti_size_x(rect) < BLI_rcti_size_y(rect) + 2)) ||
              ...
              /* disable for advanced shape also */
              (but->flag & UI_BUT_ICON_PREVIEW) || (but->shape == UI_BSHAPE_ADVANCED)) {
            /* no arrows */
        ...
        if(but->shape == UI_BSHAPE_PREFS) {
          ...
          bTheme* theme = UI_GetTheme();
          wt->wcol_theme = &theme->tui.wcol_box;
        }
        break;

      case UI_BTYPE_PULLDOWN:
        if (but->shape == UI_BSHAPE_TRAPEZOID ||
            but->shape == UI_BSHAPE_TRAPEZOIDR) {
          wt = widget_type(UI_WTYPE_PULLDOWN_TRAPEZOID);
        }
        else if(but->shape == UI_BSHAPE_PREFS) {
      ...
      case UI_BTYPE_IMAGE_MENU:
        wt = widget_type(UI_WTYPE_IMAGE_MENU);
        break;
        
      case UI_BTYPE_ANIMATION:
        wt = widget_type(UI_WTYPE_ANIMATION);
        break;
  ...
  if (state.but_flag & UI_ACTIVE) {
    if (but->drawflag & UI_BUT_ACTIVE_LEFT) {
      state.but_flag |= UI_STATE_ACTIVE_LEFT;
    }
    else if (but->drawflag & UI_BUT_ACTIVE_RIGHT) {
      state.but_flag |= UI_STATE_ACTIVE_RIGHT;
    }
  }
  ...
  if (state.but_drawflag & UI_BUT_TEXT_RIGHT) {
    state.but_flag |= UI_STATE_TEXT_BEFORE_WIDGET;
  }
...
void ui_draw_menu_back(uiStyle *UNUSED(style), uiBlock *block, rcti *rect)
{
  ...
  if(but && but->shape == UI_BSHAPE_PREFS)
    wt->wcol_theme = &theme->tui.wcol_prefs_regular;

  wt->state(wt, &((uiWidgetStateInfo) {0}), UI_EMBOSS_UNDEFINED);
  if (block) {
...
static void ui_draw_head_image(ImBuf* ibuf, const float *col) {
...
static ImBuf *head_frame_from_index(uiBlock *block, int i) {
  BLI_assert(i >= 0 && i < 32);

  ImBuf *ibuf = (ImBuf *)ui_pie_menu_custom_data_get(block->pie_data.pie);
  if(ibuf) {
    const int width = 512;
    const int height = 512;
    const int x = (i % 16) * width;
    const int y = 512 - ((i / 16) * height);
    const rcti crop = {x, x + width - 1, y, y + height - 1};
    
    ImBuf *copy = IMB_dupImBuf(ibuf);
    IMB_rect_crop(copy, &crop);
...
void ui_draw_pie_center(uiBlock *block)
{
  ...
  const float pie_radius_internal = U.dpi_fac * (U.pie_menu_threshold - 6.0f);
  ...
  ImBuf *current_frame = NULL;
  const int num_frames = 32;
  const float step = 360 / (float)num_frames;
  float deg = RAD2DEG(angle-M_PI_2);
  if(deg < 0) {
    deg += 360;
  }
  int index = 0;
  ...
  deg = 360 - deg;
  for(float i = 0; i <= 360; i += step, index++) {
    float near = i;
    float far = i + step;

    if(deg >= near && deg <= far) {
      current_frame = head_frame_from_index(block, index);
      break;
    }
  }

  if(current_frame != NULL) {
    const float col[] = {1.0f, 1.0f, 1.0f, 1.0};
    
    // GPU_matrix_rotate_3f(deg, 0, 0, 1);
    GPU_matrix_scale_3f(0.5f, 0.5f, 0.5f);
    ui_draw_head_image(current_frame, col);  

    IMB_freeImBuf(current_frame);
  }

                          ...
                          struct ResultBLF *r_info)
{
  ...
  if (fs_params->align == UI_STYLE_TEXT_CENTER) {
    float tw, th;
    BLF_width_and_height(fs->uifont_id, str, str_len, &tw, &th);
    xofs = floor(0.5f * (BLI_rcti_size_x(rect) - tw));
    
    if(th >= 1.5f * BLF_height_max(fs->uifont_id))
      th = 0;
    
    yofs = MAX2(yofs, ceil(0.5f * (BLI_rcti_size_y(rect) - th))) ;
  }
...
const uiStyle *UI_style_get_dpi(void)
{
  ...
  _style.buttonspacex = 5 + (short)(UI_DPI_FAC * _style.buttonspacex);
  _style.buttonspacey = 2 * (short)(UI_DPI_FAC * _style.buttonspacey);
  _style.panelspace = 2 * (short)(UI_DPI_FAC * _style.panelspace);

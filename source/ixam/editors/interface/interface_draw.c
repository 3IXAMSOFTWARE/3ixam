                             ...
                             float rad)
{
  ...
  uiWidgetBaseParameters widget_params = {
      ...
      .shade_dir = shade_dir,
      .alpha_discard = 1.0f,
      .tria_type = ROUNDBOX_TRIA_NONE,
      .widget_type = WIDGET_TYPE_ROUNDBOX,
  };
...
void UI_draw_shape_4fv_ex(const rctf *rect,
                          const float inner1[4],
                          const float inner2[4],
                          float shade_dir,
                          const float outline[4],
                          ...
                          short widget_shape)
{
  ...
  uiWidgetBaseParameters widget_params = {
      ...
      .radi = 1.0f,
      .rad = 1.0f,
      .round_corners[0] = 0.0f,
      .round_corners[1] = 0.0f,
      .round_corners[2] = 0.0f,
      .round_corners[3] = 0.0f,
      .color_inner1[0] = inner1 ? inner1[0] : 0.0f,
      .color_inner1[1] = inner1 ? inner1[1] : 0.0f,
      .color_inner1[2] = inner1 ? inner1[2] : 0.0f,
      .color_inner1[3] = inner1 ? inner1[3] : 0.0f,
...
void ui_draw_but_menu_IMAGE(ARegion *UNUSED(region),
                       uiBut *but,
                       void* imbuf,
                       ...
                       const rcti *rect)
{
  ...
  (void)but;
#else
//  const char* image_buf = (const char*)((uiButImageMenu*)but)->imbuf;
  ImBuf *ibuf = (ImBuf*)imbuf;
  if (!ibuf) {
    return;
  }
...
#  endif
  ...
  // Dead due to recoloring on text color change
  if (wcol->inner[3] != 0 && false) {
                       ...
                       const rcti *rect)
{
  ...
  const int w = BLI_rcti_size_x(rect) - (2 * outline);
  const int h = BLI_rcti_size_y(rect) - (2 * outline);
...
#  endif
  ...
  IMMDrawPixelsTexState state = immDrawPixelsTexSetup(GPU_SHADER_3D_IMAGE_COLOR);
  immDrawPixelsTexTiled(&state,
                        (float)rect->xmin + outline,
                        (float)rect->ymin + outline,
                        ibuf->x,
                        ibuf->y,
                        GPU_RGBA8,
                        false,
                         ...
                         const float radius)
{
  ...
  simple_lighting_data.intensity = 0.f;

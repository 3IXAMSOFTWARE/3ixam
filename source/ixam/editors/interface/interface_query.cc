...
{
  return (ELEM(but->type, UI_BTYPE_BUT, UI_BTYPE_DECORATOR, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT) || ui_but_is_toggle(but));
}
#endif
...
static float ui_but_triangle_area(float v1[2], float v2[2], float v3[2])
{
  float v[2][2];
  copy_v2_v2(v[0], v1);
  copy_v2_v2(v[1], v3);
  sub_v2_v2(v[0], v2);
  sub_v2_v2(v[1], v2);
  return fabs(cross_v2v2(v[0], v[1]));
}
  ...
  LISTBASE_FOREACH (uiBlock *, block, &region->uiblocks) {
    ...
    LISTBASE_FOREACH_BACKWARD (uiBut *, but, &block->buttons) {
      ...
      if (ui_but_is_interactive_ex(but, labeledit, for_tooltip)) {
        ...
        else if (but->shape == UI_BSHAPE_HEX)
        {
          float point_local[2];
          const float hex_size = (but->rect.ymax - but->rect.ymin) / 2;
          const float m = hex_size * cosf(M_PI / 6);
              
          float mval[2] = {mx, my};
          float pos[2] = {but->rect.xmin + (but->rect.xmax - but->rect.xmin) / 2, but->rect.ymin + (but->rect.ymax - but->rect.ymin) / 2};
          copy_v2_v2(point_local, mval);
          sub_v2_v2(point_local, pos);
          
          const float a = atan2f(point_local[0], point_local[1]);
          const float scale = (hex_size + m) / 2.0f + cosf(a * 6.0f) * (hex_size - m) / 2.0f;
          ...
          float point_local[2];
          const float hex_size = (but->rect.ymax - but->rect.ymin) / 2;
          const float mid = hex_size * cosf(M_PI / 6);
          ...
          const float a = atan2f(point_local[0], point_local[1]);
          const float scale = (hex_size + mid) / 2.0f + cosf(a * 6.0f) * (hex_size - mid) / 2.0f;
        ...
        {
          float point_local[2] = {mx, my};
          int x_size = BLI_rctf_size_x(&but->rect) / 2;
          int y_size = BLI_rctf_size_y(&but->rect) / 2;
          float rect_size[2] = {(float)x_size, (float)y_size};
          // keep it sync with shader
          float x_scale = (1 + 18.6f / 20.0f) * y_size / x_size;

          unsigned char switcher = 0;
          uiBut *tmp_btn = but;
          while (tmp_btn->prev) {
            tmp_btn = tmp_btn->prev;
          ...
          float scale[2] = {x_scale, (switcher ? -1.0f : 1.0f)};
          float midpoint[2] = {but->rect.xmin + x_size, but->rect.ymin + y_size};

          float a[2] = {-1.0f, -1.0f}, b[2] = {0.0f, 1.0f}, c[2] = {1.0f, -1.0f};
          mul_v2_v2(a, rect_size);
          mul_v2_v2(b, rect_size);
          mul_v2_v2(c, rect_size);

          mul_v2_v2(a, scale);
          mul_v2_v2(b, scale);
          mul_v2_v2(c, scale);

          add_v2_v2(a, midpoint);
          add_v2_v2(b, midpoint);
          add_v2_v2(c, midpoint);

          float s_abc = ui_but_triangle_area(a, b, c);
          float s_abp = ui_but_triangle_area(a, b, point_local);
          float s_bcp = ui_but_triangle_area(b, c, point_local);
          float s_acp = ui_but_triangle_area(a, c, point_local);

          float s_total = s_abp + s_bcp + s_acp;

          if (s_abc == s_total && s_abc != 0) {
            butover = but;
            break;
          }
        }

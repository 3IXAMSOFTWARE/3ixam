...
static uiBlock *ui_block_func_POPUP(bContext *C, uiPopupBlockHandle *handle, void *arg_pup)
{
  ...
  int width, height;
  UI_block_layout_resolve(block, NULL, NULL, &width, &height);
...
bool UI_popup_menu_end_or_cancel(bContext *C, uiPopupMenu *pup)
{
  ...
  UI_block_layout_resolve(pup->block, nullptr, nullptr, nullptr, nullptr);
...
void UI_popup_menu_reports(bContext *C, ReportList *reports)
{
  ...
  LISTBASE_FOREACH (Report *, report, &reports->list) {
    ...
    do {
      ...
      uiItemL(layout, msg, icon, UI_BSHAPE_ROUNDBOX);
      icon = ICON_NONE;
...
{
  /* if loading new .ixam while popup is open, window will be nullptr */
  if (block->handle) {
    if (win) {

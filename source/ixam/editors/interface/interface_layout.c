...
typedef enum uiItemType {
  ...
  ITEM_LAYOUT_ROW,
  ITEM_LAYOUT_VERT_ROW,
  ITEM_LAYOUT_COLUMN,
  ITEM_LAYOUT_COLUMN_FLOW,
  ITEM_LAYOUT_HEX_FLOW,
                          ...
                          int shape)
{
  ...
  if (type == PROP_BOOLEAN && ELEM(subtype, PROP_LAYER, PROP_LAYER_MEMBER)) {
    ...
    if (ELEM(subtype, PROP_COLOR, PROP_COLOR_GAMMA) && !expand && ELEM(len, 3, 4)) {
      uiBut* but = uiDefAutoButR(block, ptr, prop, -1, "", ICON_NONE, 0, 0, w, UI_UNIT_Y);
      but->shape = shape;
    }
    else {
      ...
      for (int a = 0; a < len; a++) {
        ...
        const int width_item = ((compact && type == PROP_BOOLEAN) ?
                                    ...
                                    w);
        if (shape & UI_BSHAPE_RHOMBOID) {
          const int widgth_label = UI_UNIT_X * (layout->scale[0] ? layout->scale[0] : 1.0f) * 2.0f;
          uiBut *label = uiDefBut(block, UI_BTYPE_LABEL, 0, str, 0, 0, widgth_label, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
          label->shape = UI_BSHAPE_RHOMBOID;
        }
        uiBut *but = uiDefAutoButR(
                                          ...
                                          const int shape)
{
  ...
  if (layout->item.type == ITEM_LAYOUT_VERT_ROW && but->type == UI_BTYPE_ROW) {
    but->type = UI_BTYPE_VERTROW;
  }
  but->shape = shape;
                                     ...
                                     const int shape)
{
  ...
  else if (ELEM(layout->item.type, ITEM_LAYOUT_GRID_FLOW, ITEM_LAYOUT_COLUMN_FLOW, ITEM_LAYOUT_HEX_FLOW) ||
           layout->root->type == UI_LAYOUT_MENU) {
    UI_block_layout_set_current(block, layout);
  }
  else {
    if(layout->item.type == ITEM_LAYOUT_VERT_ROW) {
      layout->alignment = UI_LAYOUT_ALIGN_CENTER;
      UI_block_layout_set_current(block, layout);
    } else {
  ...
  for (const EnumPropertyItem *item = item_array; item->identifier; item++) {
    ...
    if (!item->identifier[0]) {
      ...
      if (next_item->identifier) {
        ...
        if (item->name) {
          ...
          uiItemL(block->curlayout, item->name, item->icon, UI_BSHAPE_ROUNDBOX);
        }
    ...
    ui_item_enum_expand_elem_exec(
        layout, block, ptr, prop, uiname, h, but_type, icon_only, item, is_first, shape);
  }
...
static void ui_item_enum_expand(uiLayout *layout,
                                ...
                                const int shape)
{
  ui_item_enum_expand_exec(layout, block, ptr, prop, uiname, h, UI_BTYPE_ROW, icon_only, shape);
}
                                     ...
                                     const bool icon_only)
{
  ...
  ui_item_enum_expand_exec(layout, block, ptr, prop, uiname, h, UI_BTYPE_TAB, icon_only, UI_BSHAPE_ROUNDBOX);
                                 ...
                                 int shape)
{
  ...
  if (ELEM(subtype, PROP_FILEPATH, PROP_DIRPATH)) {
   ...
   uiBut* button = uiDefIconButO(block,
          UI_BTYPE_SEPR,
          "",
  ...
  if (use_prop_decorate) { /* Note that sep flag may have been unset meanwhile. */
    uiItemL(layout_prop_decorate ? layout_prop_decorate : sub, NULL, ICON_BLANK1, UI_BSHAPE_ROUNDBOX);
  }
                                 ...
                                 PointerRNA *r_opptr)
{
  ...
  uiBlock *block = layout->root->block;
  const int shape = ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK);
  ...
  if (icon) {
    if (name[0]) {
      but = uiDefIconTextButO_ptr(
          block, type, ot, context, icon, name, 0, 0, w, UI_UNIT_Y, NULL);
    }
    else {
      but = uiDefIconButO_ptr(block, type, ot, context, icon, 0, 0, w, UI_UNIT_Y, NULL);
      but->drawflag &= ~UI_BUT_ICON_LEFT;
    }
  }
  else {
    but = uiDefButO_ptr(block, type, ot, context, name, 0, 0, w, UI_UNIT_Y, NULL);
  }
...
static void ui_item_menu_hold(struct bContext *C, ARegion *butregion, uiBut *but)
{
  ...
  if (mt) {
    ...
    uiItemL(layout, TIP_("Menu Missing:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemL(layout, menu_id, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
...
void uiItemFullO_ptr(uiLayout *layout,
                     ...
                     PointerRNA *r_opptr)
{
  uiItemFullO_ptr_ex(layout, UI_BTYPE_BUT, ot, name, icon, properties, context, flag, r_opptr);
}

void uiItemFullOTab_ptr(uiLayout *layout,
                     wmOperatorType *ot,
                     const char *name,
                     int icon,
                     int tab_type,
                     ...
                     PointerRNA *r_opptr)
{
  ...
  switch (tab_type)
  {
  case UI_TAB_TYPE_MATPRO:
    but_type = UI_BTYPE_MATPRO_TAB;
    break;
  ...
  uiBut *but = uiItemFullO_ptr_ex(layout, but_type, ot, name, icon, properties, context, flag, r_opptr);
  but->poin = (char *)tab_index;
}

void uiItemFullML_O(uiLayout *layout,
                     wmOperatorType *ot,
                     const char *name,
                     int icon,
                     ...
                     PointerRNA *r_opptr)
{
  uiBut *but = uiItemFullO_ptr_ex(layout, UI_BTYPE_MATLIST_BUT, ot, name, icon, properties, context, flag, r_opptr);
  but->poin = (char *)material;
}
...
void uiItemFullOMenuHold_ptr(uiLayout *layout,
                             ...
                             PointerRNA *r_opptr)
{
  uiBut *but = uiItemFullO_ptr_ex(layout, UI_BTYPE_BUT, ot, name, icon, properties, context, flag, r_opptr);
  ...
  for (int i = 1; item->identifier && !last_iter; i++, item++) {
    ...
    if (item->identifier[0]) {
      ...
      if (item->name) {
        ...
        uiBut *but;
        if (item->icon || radial) {
          uiItemL(target, item->name, item->icon, UI_BSHAPE_ROUNDBOX);
                                        ...
                                        bool respect_prop_split)
{
  ...
  if (respect_prop_split) {
    ...
    uiItemL(layout, heading_layout->heading, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
...
//  const bool rhomboid = ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK) == UI_BSHAPE_RHOMBOID;
  const bool use_prop_sep = ((layout->item.flag & UI_ITEM_PROP_SEP) != 0);
  ...
  /* get size */
  int w, h;
  ui_item_rna_size(layout, name, icon, ptr, prop, index, icon_only, compact, 
    ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK), &w, &h);
  ...
  if (index == RNA_NO_INDEX && is_array) {
    ...
    ui_item_array(layout,
                  ...
                  ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK));
  }
  /* enum item */
  else if (type == PROP_ENUM && index == RNA_ENUM_VALUE) {
    if (icon && name[0] && !icon_only) {
  ...
  /* expanded enum */
    ...
    ui_item_enum_expand(layout, block, ptr, prop, name, h, icon_only, ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK));
  }
  /* property with separate label */
  else if (ELEM(type, PROP_ENUM, PROP_STRING, PROP_POINTER)) {
    but = ui_item_with_label(layout, block, name, icon, ptr, prop, index, 0, 0, w, h, flag, UI_BSHAPE_ROUNDBOX);
  ...
  /* single button */
    ...
    but->shape = ((flag >> UI_ITEM_SHAPE_FIELD_OFFSET) & UI_ITEM_SHAPE_FIELD_MASK);
  ...
  for (int i = 0; i < totitem; i++) {
    if (item[i].identifier[0]) {
      ...
      if (item[i].name) {
        ...
        uiItemL(column, item[i].name, ICON_NONE, UI_BSHAPE_ROUNDBOX);
                         ...
                         bool results_are_suggestions)
{
  ...
  int w, h;
  ui_item_rna_size(layout, name, icon, ptr, prop, 0, 0, false, 
    UI_BSHAPE_ROUNDBOX, &w, &h);
  w += UI_UNIT_X; /* X icon needs more space */
  uiBut *but = ui_item_with_label(layout, block, name, icon, ptr, prop, 0, 0, 0, w, h, 0, UI_BSHAPE_ROUNDBOX);
...
void ui_item_menutype_image_func(bContext *C, uiLayout *layout, void *arg_mt_img)
{
  
  uiButImageMenu *but = (uiButImageMenu *)arg_mt_img;

  UI_menutype_draw(C, (MenuType*)but->mt, layout);
  
  /* Menus are created flipped (from event handling point of view). */
  layout->root->block->flag ^= UI_BLOCK_IS_FLIP;
}

void ui_item_popover_func(bContext *C, uiLayout *layout, void *arg_pt)
{
  PanelType *pt = (PanelType *)arg_pt;
  UI_popover_draw(C, pt, layout);
                           ...
                           int shape)
{
  ...
  const int w = 10 + ui_text_icon_width_ex(layout, name, icon, &pad_factor);
  ...
  uiBut *but;
  if(icon == -1) {
    int width = IMAGE_MENU_SIZE * UI_DPI_FAC;
    but = uiDefMenuImageBut(block, func, arg, name, 0, 0, width, width, tip);
...
void uiItemM_ptr(uiLayout *layout, MenuType *mt, const char *name, int icon, int shape)
{
  uiBlock *block = layout->root->block;
...
void uiItemM(uiLayout *layout, const char *menuname, const char *name, int icon, int shape)
{
  ...
  uiItemM_ptr(layout, mt, name, icon, shape);
}

void uiItemImg(uiLayout* layout, const char* imbuf, const char* menuname, const char* name) {
  ...
  uiButImageMenu* but = (uiButImageMenu*)ui_item_menu(layout,
               name,
               -1,
               ui_item_menutype_image_func,
               NULL,
               NULL,
               mt->description ? TIP_(mt->description) : "",
               false,
  ...
  but->but.poin = (char *)but;
  but->mt = mt;
//  but->imbuf = (void *)imbuf;
}
...
void uiItemPopoverPanel_ptr(
    uiLayout *layout, const bContext *C, PanelType *pt, const char *name, int icon, int shape, int align, bool depress)
{
  if (!name) {
  ...
  uiBut *but = ui_item_menu(
      layout, name, icon, ui_item_paneltype_func, pt, NULL, pt->description, true, UI_BSHAPE_ROUNDBOX);
  but->type = UI_BTYPE_POPOVER;
  but->shape = shape;
  if (align & UI_POPOVER_ALIGN_LEFT) {
    but->drawflag |= UI_BUT_ACTIVE_LEFT;
  }
    if (align & UI_POPOVER_ALIGN_RIGHT) {
...
void uiItemPopoverPanel_vc_ptr(
    uiLayout *layout, const bContext *C, PanelType *pt)
{
  const bool ok = (pt->poll == NULL) || pt->poll(C, pt);
  if (ok && (pt->draw_header != NULL)) {
    layout = uiLayoutRow(layout, true);
  ...
  uiBut *but = ui_item_menu(
      layout, "TEST", ICON_BLANK1, ui_item_paneltype_func, pt, NULL, pt->description, true, UI_BSHAPE_ROUNDBOX);
  but->type = UI_BTYPE_VIEWCONFIG;
  if (!ok) {
    but->flag |= UI_BUT_DISABLED;
  }
}
...
void uiItemPopoverPanel(
    uiLayout *layout, const bContext *C, const char *panel_type, const char *name, int icon, int shape, int align, bool depress)
{
  ...
  uiItemPopoverPanel_ptr(layout, C, pt, name, icon, shape, align, depress);
}

void uiItemPopoverPanel_vc(
    uiLayout *layout, const bContext *C, const char *panel_type)
{
  PanelType *pt = WM_paneltype_find(panel_type, true);
  if (pt == NULL) {
    RNA_warning("Panel type not found '%s'", panel_type);
  ...
  uiItemPopoverPanel_vc_ptr(layout, C, pt);
}
      ...
      if (/* (*context == '\0') || */ STREQ(pt->context, context)) {
        if ((*category == '\0') || STREQ(pt->category, category)) {
          if (pt->poll == NULL || pt->poll(C, pt)) {
            uiItemPopoverPanel_ptr(layout, C, pt, NULL, ICON_NONE, UI_BSHAPE_ROUNDBOX, false, false);
          }
        }
      }
    }
...
/* outliner object */
static uiBut *uiItemMOperator_(uiLayout *layout, const Material* material, int icon)
{
  uiBlock *block = layout->root->block;
  ...
  const int w = ui_text_icon_width_ex(layout, "", icon, &ui_text_pad_none);
  but = uiDefIconBut(
        block, UI_BTYPE_MATOPERATOR, 0, icon, 0, 0, w, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
  but->poin = (char*)material;
...
/* outliner object */
static uiBut *uiItemOObject_(uiLayout *layout, const Object* object)
{
  uiBlock *block = layout->root->block;
  ...
  const int w = ui_text_icon_width_ex(layout, object->id.name + 2, 0, &ui_text_pad_none);
  uiBut *but;

  but = uiDefBut(block, UI_BTYPE_OUTLINEROBJECT, 0, object->id.name + 2, 0, 0, w, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
  but->poin = (char*)object;
  
  return but;
}

/* outliner collection */
static uiBut *uiItemOFolder_(uiLayout *layout, const Collection* collection)
{
  uiBlock *block = layout->root->block;
  ...
  const int hex_icon_value = 1006;
  
  const int w = ui_text_icon_width_ex(layout, collection->id.name + 2, hex_icon_value, &ui_text_pad_default);
  uiBut *but;

  but = uiDefIconTextBut(
      block, UI_BTYPE_OUTLINERFOLDER, 0, hex_icon_value, collection->id.name + 2, 0, 0, w, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
//  but = uiDefBut(block, UI_BTYPE_OUTLINERFOLDER, 0, collection->id.name + 2, 0, 0, w, UI_UNIT_Y, NULL, 0.0, 0.0, 0, 0, NULL);
  but->poin = (char*)collection;
...
/* label item */
static uiBut *uiItemL_(uiLayout *layout, const char *name, int icon, int shape)
{
  uiBlock *block = layout->root->block;
...
{
  uiBut *but = uiItemL_(layout, name, icon, UI_BSHAPE_ROUNDBOX);
...
void uiItemL(uiLayout *layout, const char *name, int icon, int shape)
{
  uiItemL_(layout, name, icon, shape);
}

void uiItemMOperator(uiLayout *layout, struct Material* material, int icon_value)
{
  uiItemMOperator_(layout, material, icon_value);
}

void uiItemOObject(uiLayout *layout, Object* object)
{
  uiItemOObject_(layout, object);
}

void uiItemOFolder(uiLayout *layout, Collection* collection)
{
  uiItemOFolder_(layout, collection);
}
...
void uiItemDimensions(uiLayout *layout, const struct bContext *C)
{
  uiBlock *block = layout->root->block;
  ...
  if (OB_TYPE_SUPPORT_EDITMODE(ob->type) || ELEM(ob->type, OB_VOLUME, OB_CURVES, OB_POINTCLOUD)) {
    View3D *v3d = CTX_wm_view3d(C);
    if (v3d) {
      v3d_object_dimension_buts(NULL, row, v3d, ob);
    }
  }
}

void uiItemSearch(uiLayout *layout)
{
...
void uiItemUVAxis(const bContext *C, uiLayout *layout, int axis) {
  uiBlock *block = layout->root->block;

  UI_block_func_handle_set(block, do_uvedit_vertex, NULL);

  uvedit_vertex_buttons(C, block, axis);
}

void uiItemAnimBar(const bContext *C, uiLayout *layout) {
  uiBlock *block = layout->root->block;
  Scene *scene = CTX_data_scene(C);
  
  uiDefBut(block,
           UI_BTYPE_ANIMATION_BAR | UI_BUT_POIN_INT,
           0,
           "",
           0,
           0,
...
uiLayout *uiItemL_respect_property_split(uiLayout *layout, const char *text, int icon)
{
  if (layout->item.flag & UI_ITEM_PROP_SEP) {
    ...
    uiItemL_(split_wrapper.label_column, text, icon, UI_BSHAPE_ROUNDBOX);
  ...
  uiItemL_(layout, text, icon, UI_BSHAPE_ROUNDBOX);
...
void uiItemS_ex(uiLayout *layout, float factor, int shape)
{
  uiBlock *block = layout->root->block;
  ...
  uiBut* but  = uiDefBut(block,
           UI_BTYPE_SEPR_LINE,//(is_menu) ? UI_BTYPE_SEPR_LINE : UI_BTYPE_SEPR,
           0,
           "",
           0,
           0,
...
{
  uiItemS_ex(layout, 1.0f, UI_BSHAPE_ROUNDBOX);
}
...
void uiItemMenuF(uiLayout *layout, const char *name, int icon, uiMenuCreateFunc func, void *arg)
{
  ...
  ui_item_menu(layout, name, icon, func, arg, NULL, "", false, UI_BSHAPE_ROUNDBOX);
}
...
void uiItemMenuFN(uiLayout *layout, const char *name, int icon, uiMenuCreateFunc func, void *argN)
{
  ...
  ui_item_menu(layout, name, icon, func, argN, argN, "", false, UI_BSHAPE_ROUNDBOX);
}
...
static void menu_item_enum_opname_menu(bContext *UNUSED(C), uiLayout *layout, void *arg)
{
  ...
  UI_block_direction_set(layout->root->block, UI_DIR_LEFT);
}
                             ...
                             PointerRNA *r_opptr)
{
  ...
  uiBut *but = ui_item_menu(layout, name, icon, menu_item_enum_opname_menu, NULL, lvl, NULL, true, UI_BSHAPE_ROUNDBOX);
...
static void ui_litem_estimate_vert_row(uiLayout *litem)
{
  int itemw, itemh;
  bool min_size_flag = true;
...
static void ui_litem_layout_vert_row(uiLayout *litem)
{
  uiItem *last_free_item = NULL;
  int y, neww, newtotw, itemw, minw, itemh, offset;
  int freew, fixedx, freex, flag = 0, lastw = 0;
  float extra_pixel;
  ...
  do {
    ...
    LISTBASE_FOREACH (uiItem *, item, &litem->items) {
      ...
      if (w - lastw > 0) {
        neww = ui_item_fit(itemw, y, totw, w - lastw, !item->next, litem->alignment, &extra_pixel);
      }
      else {
        neww = 0; /* no space left, all will need clamping to minimum size */
      }
  ...
  LISTBASE_FOREACH (uiItem *, item, &litem->items) {
    ...
    /* position item */
    ui_item_position(item, x + offset, y - itemw, itemh, itemw);
...
static uiBut* find_ouliner_collection(uiLayout *main_item)
{
  LISTBASE_FOREACH (uiItem *, item, &main_item->items) {
    if (item->type != ITEM_BUTTON) {
      uiLayout *litem = (uiLayout *)item;
      if (BLI_listbase_is_empty(&litem->items)) {
        continue;
      }
      uiBut *tmpBut = find_ouliner_collection(litem);
      if (tmpBut != NULL) {
        return tmpBut;
      }
    }
      ...
      uiButtonItem *bitem = (uiButtonItem*)item;
      if (bitem->but->type == UI_BTYPE_OUTLINERFOLDER) {
        return bitem->but;
      }
    }
  }
...
static void ui_litem_estimate_hex_flow(uiLayout *litem)
{
  int w, h;
  int hex_size = 0;
  const int spacer = 3;
  ...
  const int itemh = 2.0f * hex_size;
  const int itemw = ceilf(1.73205080757f * hex_size);
  const int hex_offset = ceilf(itemw * .5f) + 1.f;
  const int spaceh = 1.5f * hex_size;
  
  LISTBASE_FOREACH (uiItem *, item, &litem->items) {
    if(item->type == ITEM_LAYOUT_SPLIT)
    {
      row_parity ^= 1;
      litem->w = max_ii(litem->w, curw);
      curw = 0;
      
      if(row_parity)
        curw = hex_offset;
      
      litem->h += spaceh + spacer - 1;
    }
  ...
  if(litem->alignment == UI_LAYOUT_ALIGN_RIGHT)
    litem->w = max_ii(litem->w, curw - hex_offset);
  else
    litem->w = max_ii(litem->w, curw) + hex_offset;
  litem->h += itemh + spacer;
}
...
static void ui_litem_layout_hex_flow(uiLayout *litem)
{
  int iw, ih;
  int hex_size = 0;
  ...
  float x = 0, offset = 0;   float w = litem->w;
  
  unsigned char row_parity = 0;
  
  const float itemh = 2.0f * hex_size;
  const float itemw = ceilf(1.73205080757f * hex_size);
  const float hex_offset = ceilf(itemw * .5f) + 1.f;
  float y = litem->y;
  const int spaceh = 1.5f * hex_size;
  ...
  LISTBASE_FOREACH (uiItem *, item, &litem->items) {
    ...
    if(litem->alignment == UI_LAYOUT_ALIGN_RIGHT)
      ...
      offset = x - (itemw + spacer);

    /* position item */
    ui_item_position(item, (int)ceilf(litem->x + offset), (int)ceilf(y - itemh), (int)itemw+spacer, (int)itemh);
  }
}
...
uiLayout *uiLayoutVertRow(uiLayout *layout, bool align)
{
  uiLayout *litem = MEM_callocN(sizeof(uiLayout), "uiLayoutVertRow");
  ui_litem_init_from_parent(litem, layout, align);

  litem->item.type = ITEM_LAYOUT_VERT_ROW;
  litem->space = (align) ? 0 : layout->root->style->buttonspacex;
...
uiLayout *uiLayoutHexFlow(uiLayout *layout, int number, bool align)
{
  uiLayoutItemFlow *flow = MEM_callocN(sizeof(uiLayoutItemFlow), "uiLayoutItemFlow");
  ui_litem_init_from_parent(&flow->litem, layout, align);

  flow->litem.item.type = ITEM_LAYOUT_HEX_FLOW;
  flow->litem.space = (flow->litem.align) ? 0 : layout->root->style->columnspace;
  flow->number = number;
...
void uiLayoutSetShape(uiLayout *layout, int shape)
{
  layout->shape = shape;
}

void uiLayoutSetIconScale(uiLayout *layout, float scale)
{
  layout->icon_scale = scale;
}
...
eButShape uiLayoutGetShape(uiLayout *layout)
{
  return layout->shape;
}
...
float uiLayoutGetIconScale(uiLayout *layout)
{
  return layout->icon_scale;
}
...
static void ui_item_shape(uiLayout *litem, int shape)
{
  unsigned char switcher = 0;
  LISTBASE_FOREACH_BACKWARD (uiItem *, item, &litem->items) {
    uiLayout *subitem = (uiLayout *)item;
    if (item->type != ITEM_BUTTON) {
      if(subitem->shape != UI_BSHAPE_ROUNDBOX)
        continue;
      ui_item_shape(subitem, shape);
    }
    
    if (item->type == ITEM_BUTTON) {
      int cur_shape = shape;
      if (shape == UI_BSHAPE_TRAPEZOID) {
        cur_shape = UI_BSHAPE_TRAPEZOID + switcher;
      } else if (shape == UI_BSHAPE_TRAPEZOIDR) {
        cur_shape = UI_BSHAPE_TRAPEZOIDR - switcher;
      }
      uiButtonItem *bitem = (uiButtonItem *)subitem;
      if (ELEM(bitem->but->type, UI_BTYPE_LABEL, UI_BTYPE_SEPR_LINE))
        continue;
      bitem->but->shape = cur_shape;
      switcher ^= 1;
    }
    else {
      subitem->shape = shape;
...
static void ui_item_icon_scale(uiLayout *litem, const float icon_scale)
{
  LISTBASE_FOREACH_BACKWARD (uiItem *, item, &litem->items) {
    uiLayout *subitem = (uiLayout *)item;
    if (item->type != ITEM_BUTTON) {
      if(subitem->icon_scale != 0.0f)
        continue;
      ui_item_icon_scale(subitem, icon_scale);
    }
    
    if (item->type == ITEM_BUTTON) {
      uiButtonItem *bitem = (uiButtonItem *)subitem;
      bitem->but->icon_scale = icon_scale;
    }
    else {
      subitem->icon_scale = icon_scale;
    }
  }
}
...
static void ui_item_estimate(uiItem *item)
{
  if (item->type != ITEM_BUTTON) {
    ...
    if (litem->icon_scale != 0.0f) {
      ui_item_icon_scale(litem, litem->icon_scale);
    }

    if (litem->shape != UI_BSHAPE_ROUNDBOX) {
      ui_item_shape(litem, litem->shape);
    }
    ...
    switch (litem->item.type) {
      ...
      case ITEM_LAYOUT_COLUMN_FLOW:
        ...
        break;
      case ITEM_LAYOUT_HEX_FLOW:
        ui_litem_estimate_hex_flow(litem);
        break;
      case ITEM_LAYOUT_GRID_FLOW:
      ...
      case ITEM_LAYOUT_ROW:
        ...
        break;
      case ITEM_LAYOUT_VERT_ROW:
        ui_litem_estimate_vert_row(litem);
        break;
      case ITEM_LAYOUT_BOX:
  ...
  if (item->type != ITEM_BUTTON) {
    ...
    switch (litem->item.type) {
      ...
      case ITEM_LAYOUT_COLUMN_FLOW:
        ...
        break;
      case ITEM_LAYOUT_HEX_FLOW:
        ui_litem_layout_hex_flow(litem);
        break;
      case ITEM_LAYOUT_GRID_FLOW:
      ...
      case ITEM_LAYOUT_ROW:
        ...
        break;
      case ITEM_LAYOUT_VERT_ROW:
        ui_litem_layout_vert_row(litem);
        break;
      case ITEM_LAYOUT_BOX:
...
void uiLayoutSetVariableSize(uiLayout *layout)
{
  layout->variable_size = true;
}
...
void UI_block_layout_resolve(uiBlock *block, int *r_x, int *r_y, int *r_w, int *r_h)
{
...
static void ui_paneltype_draw_impl(bContext *C, PanelType *pt, uiLayout *layout, bool show_header)
{
  ...
  if (show_header) {
    ...
    if (!ui_layout_has_panel_label(row, pt)) {
      uiItemL(row, CTX_IFACE_(pt->translation_context, pt->label), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    }
  }
...
static void ui_popover_draw_impl(bContext *C, PanelType *pt, uiLayout *layout, bool show_header)
{
  uiBlock *block = layout->root->block;
  ...
  panel = UI_panel_begin(region, &region->panels, block, pt, panel, &open);
  panel->flag = PNL_POPOVER;
  ...
  LISTBASE_FOREACH (LinkData *, link, &pt->children) {
    ...
    if (child_pt->poll == NULL || child_pt->poll(C, child_pt)) {
      ...
      uiItemL(box, CTX_IFACE_(child_pt->translation_context, child_pt->label), ICON_NONE, UI_BSHAPE_ROUNDBOX);

      Panel *child_panel = NULL;
      LISTBASE_FOREACH (Panel *, ch_panel, &panel->children) {
        if (ch_panel->type == child_pt) {
          child_panel = ch_panel;
          break;
        }
      }

      if (child_panel == NULL) {
        child_panel = MEM_callocN(sizeof(Panel), __func__);
        child_panel->type = child_pt;
        BLI_strncpy(child_panel->panelname, child_pt->idname, sizeof(child_pt->idname));

        child_panel->runtime_flag |= (1 << 5);
        child_panel->sortorder = ++max_sortorder;

        BLI_addtail(&panel->children, child_panel);
      }

      if (!UI_panel_is_closed(child_panel)) {
        child_panel->layout = box;
        child_pt->draw(C, child_panel);
        child_panel->layout = NULL;
      }
...
void UI_popover_draw(bContext *C, PanelType *pt, uiLayout *layout)
{
  if (layout->context) {
    CTX_store_set(C, layout->context);
  }

  ui_popover_draw_impl(C, pt, layout, false);

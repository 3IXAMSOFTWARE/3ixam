...
static void ui_apply_but_ROW(bContext *C, uiBlock *block, uiBut *but, uiHandleButtonData *data)
{
  ...
  LISTBASE_FOREACH (uiBut *, bt, &block->buttons) {
    if (bt != but && bt->poin == but->poin && ELEM(bt->type, UI_BTYPE_ROW, UI_BTYPE_LISTROW, UI_BTYPE_VERTROW)) {
                             ...
                             const wmEvent *event)
{
  ...
  if (abs(data->dragstartx - event->xy[0]) + abs(data->dragstarty - event->xy[1]) >
    ...
    else if (but->type == UI_BTYPE_OUTLINEROBJECT) {
      struct Main *bmain = CTX_data_main(C);
      wmDrag *drag = WM_drag_data_create(C, ICON_SEQ_CHROMA_SCOPE, WM_DRAG_ID, NULL, 0.0, WM_DRAG_NOP);
      
      ListBase selected_objs;
      CTX_data_selected_objects(C, &selected_objs);
      
      Object *but_obj = (Object *)but->poin;
      BLI_assert(but_obj != NULL);
      Collection *but_parent = BKE_collection_object_find(bmain, NULL, NULL, but_obj);
      WM_drag_add_local_ID(drag, &but_obj->id, &but_parent->id);
      
      LISTBASE_FOREACH (CollectionPointerLink *, link, &selected_objs) {
        Object *object =  link->ptr.data;
        if (object == but_obj)
          continue;
        BLI_assert(object != NULL);
        Collection *parent = BKE_collection_object_find(bmain, NULL, NULL, object);
        WM_drag_add_local_ID(drag, &object->id, &parent->id);
      }
      BLI_freelistN(&selected_objs);
...
static void ui_block_open_begin(bContext *C, uiBut *but, uiHandleButtonData *data)
{
  ...
  switch (but->type) {
    ...
    case UI_BTYPE_VIEWCONFIG:
      ...
      if ((ELEM(but->type, UI_BTYPE_POPOVER, UI_BTYPE_VIEWCONFIG)) || ui_but_menu_draw_as_popover(but)) {
...
static void item_activate(bContext *C, uiBut *but, const wmEvent *event, const bool extend)
{
  ...
  eObjectSelect_Mode mode = (base->flag & BASE_SELECTED) != 0 ? BA_DESELECT : BA_SELECT;
...
bool button_close_tab(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event, bool force)
{
  int tab_index = (int)but->poin;

  rctf butrct;
  ui_block_to_window_rctf(data->region, but->block, &butrct, &but->rect);
  /* widget_roundbox_set has this correction too, keep in sync */
  if (but->type != UI_BTYPE_PULLDOWN && but->type != UI_BTYPE_IMAGE_MENU) {
  ...
  const int draw_size = ICON_DEFAULT_HEIGHT;
  const float w = BLI_rctf_size_x(&butrct);
  BLI_rctf_size_x(&but->rect);
  
  /* Position of the cross icon */
  const float x = butrct.xmin + w * (1 - UI_TAB_CLOSE_OFFSET);
  const float y = BLI_rctf_cent_y(&butrct);
  /* Event position */
  const float event_x = event->xy[0], event_y = event->xy[1];
  
  const float dist = len_v2v2((float [2]){x, y}, (float [2]){event_x, event_y});

  if (!force && dist > draw_size / 2) {
    return false;
  }
  ...
  {
  case UI_BTYPE_MATPRO_TAB:
    ot = WM_operatortype_find("MATPRO_OT_close_tab", false);
    WM_operator_properties_create_ptr(&props_ptr, ot);
    RNA_int_set(&props_ptr, "tab_index", tab_index);
...
static void active_material_set(bContext *C, uiBut *but, uiHandleButtonData *data)
{
  ...
  matlib->active_material = material;
  
  ED_area_tag_refresh(data->area);
}
  
static void active_material_apply(bContext *C, uiBut *but, uiHandleButtonData *data)
{
  ...
  LISTBASE_FOREACH (CollectionPointerLink *, link, &selected_objs) {
    ...
    BKE_object_material_assign(bmain, object, material, object->actcol, BKE_MAT_ASSIGN_EXISTING);
  }
  BLI_freelistN(&selected_objs);
  
  WM_main_add_notifier(NC_SPACE | ND_SPACE_VIEW3D, NULL);
  WM_main_add_notifier(NC_SPACE | ND_SPACE_MATLIB, NULL);
}
  
static void collection_select(bContext *C, uiBut *but, const wmEvent *event, const bool extend)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Scene *scene = CTX_data_scene(C);
...
static void iconrow_handle(uiBut *but)
{
  const int oldflag = but->drawflag;
  but->drawflag &= ~(UI_BUT_ACTIVE_LEFT | UI_BUT_ACTIVE_RIGHT | UI_BUT_CHECKBOX_INVERT);
  ...
  const float x_render = but->rect.xmax - ICON_DEFAULT_WIDTH;
  const float x_lock = x_render - ICON_DEFAULT_WIDTH - icon_padding * 3;
  const float x_select = x_lock - ICON_DEFAULT_WIDTH - icon_padding * 2;
  
  if (my > but->rect.ymin && my < but->rect.ymax) {
    if (mx > x_render) {
...
static int ui_do_but_OUTLINER_OBJ(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  iconrow_handle(but);
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      PointerRNA ptr;
      Object *ob = (Object *)but->poin;
      RNA_id_pointer_create(&ob->id, &ptr);
      
      if (but->drawflag & UI_BUT_ACTIVE_LEFT) { // select lock toogle
        PropertyRNA *prop = RNA_struct_type_find_property(&RNA_Object, "hide_select");

        const bool value = RNA_property_boolean_get(&ptr, prop);
        RNA_property_boolean_set(&ptr, prop, !value);
        RNA_property_update(C, &ptr, prop);
      }
      else if (but->drawflag & UI_BUT_ACTIVE_RIGHT) { // render viewport toggle
      ...
      else if (but->drawflag & UI_BUT_CHECKBOX_INVERT) { // viewport disply toogle
        PropertyRNA *prop = RNA_struct_type_find_property(&RNA_Object, "hide_viewport");
        ...
        item_activate(C, but, event, event->modifier & KM_SHIFT || event->modifier & KM_CTRL);
      }
...
static int ui_do_but_MATERIAL_OPERATOR(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    if (event->type == LEFTMOUSE && event->val == KM_DBL_CLICK) {
      active_material_apply(C, but, data);
      data->state = BUTTON_STATE_EXIT;
      return WM_UI_HANDLER_BREAK;
    }
    if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
      active_material_set(C, but, data);
      data->state = BUTTON_STATE_EXIT;
      return WM_UI_HANDLER_BREAK;
    }
  }
...
static int ui_do_but_OUTLINER_FOLDER(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
      data->state = BUTTON_STATE_WAIT_RELEASE;
      return WM_UI_HANDLER_BREAK;
    }
  }
    ...
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      data->state = BUTTON_STATE_EXIT;
      collection_select(C, but, event, event->modifier & KM_SHIFT || event->modifier & KM_CTRL);
      return WM_UI_HANDLER_BREAK;
    }
  }
  return WM_UI_HANDLER_CONTINUE;
...
static int ui_do_but_TAB_MATPRO(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      if (button_close_tab(C, but, data, event, false)) {
        return WM_UI_HANDLER_CONTINUE;
      }
      data->state = BUTTON_STATE_EXIT;
      return WM_UI_HANDLER_BREAK;
    }
    else if (event->type == MIDDLEMOUSE && event->val == KM_RELEASE) {
      button_close_tab(C, but, data, event, true);
      return WM_UI_HANDLER_CONTINUE;
    }
  }
  return WM_UI_HANDLER_CONTINUE;
...
static int ui_do_but_MATLIST_ITEM(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      data->state = BUTTON_STATE_EXIT;
...
static int ui_do_but_TOG(bContext *C, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (ELEM(event->type, MOUSEPAN, WHEELDOWNMOUSE, WHEELUPMOUSE) && (event->modifier & KM_CTRL)) {
      ...
      if (ELEM(but->type, UI_BTYPE_ROW, UI_BTYPE_VERTROW)) {
        int type = event->type;
        int val = event->val;
...
static void ui_numedit_set_active(uiBut *but)
{
  ...
  if (data->dragchange == false) {
    const float handle_width = BLI_rctf_size_x(&but->rect) / 3;
    const float handle_height =  BLI_rctf_size_y(&but->rect) * 0.5f;
    ...
    if (mx > (but->rect.xmax - handle_width)) {
      if (my < (but->rect.ymin + handle_height)) {
        but->drawflag |= UI_BUT_ACTIVE_LEFT;
      }
      else {
        but->drawflag |= UI_BUT_ACTIVE_RIGHT;
                               ...
                               const bool shift)
{
  ...
  /* prevent unwanted drag adjustments, test motion so modifier keys refresh. */
  if (!ELEM(but->type, UI_BTYPE_SCROLL, UI_BTYPE_ANIMATION_BAR) && (is_motion || data->draglock) &&
  ...
  if (ELEM(but->type, UI_BTYPE_NUM_SLIDER, UI_BTYPE_ANIMATION_BAR)) {
    ...
    bContext *C, uiBlock *block, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  ...
  int mx = event->xy[0];
  int my = event->xy[1];
  float value = CTX_data_scene(C)->r.sfra + roundf(((mx - but->rect.xmin) / (BLI_rctf_size_x(&but->rect))) * (but->hardmax - but->hardmin));
  ui_window_to_block(data->region, block, &mx, &my);
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
      data->dragstartx = mx;
      data->draglastx = mx;
      CTX_data_scene(C)->r.cfra = data->value = value;
      struct bAnimContext ac;
      if(ANIM_animdata_get_context(C, &ac)) {
        select_keyframes_cfra(C, &ac);
      }
      button_activate_state(C, but, BUTTON_STATE_NUM_EDITING);
      retval = WM_UI_HANDLER_BREAK;
    } else if (event->type == EVT_DELKEY && event->val == KM_PRESS) {
      WM_operator_name_call(C, "ACTION_OT_delete", WM_OP_EXEC_DEFAULT, NULL, NULL);
      data->value = CTX_data_scene(C)->r.cfra;
      button_activate_state(C, but, BUTTON_STATE_EXIT);
      retval = WM_UI_HANDLER_BREAK;
    }
  }
    ...
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      ...
      data->multi_data.drag_dir[1] += abs(data->draglasty - my);
       ...
       if (ui_numedit_but_SLI(
              but, data, mx, true, is_motion, event->modifier & KM_SHIFT, event->modifier & KM_CTRL)) {
        bAnimContext ac;
        ui_numedit_apply(C, block, but, data);
        if(ANIM_animdata_get_context(C, &ac)) {
          select_keyframes_cfra(C, &ac);
...
static int ui_do_button(bContext *C, uiBlock *block, uiBut *but, const wmEvent *event)
{
  ...
  if (data->state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if ((event->type == RIGHTMOUSE) &&
      ...
      if (ui_popup_context_menu_for_button(C, but, event)) {
        if (but->type == UI_BTYPE_MATLIST_BUT) {
          data->state = BUTTON_STATE_EXIT;
        }
        return WM_UI_HANDLER_BREAK;
      }
  ...
  switch (but->type) {
    ...
    case UI_BTYPE_OUTLINEROBJECT:
      retval = ui_do_but_OUTLINER_OBJ(C, but, data, event);
      break;
    case UI_BTYPE_OUTLINERFOLDER:
      retval = ui_do_but_OUTLINER_FOLDER(C, but, data, event);
      break;
    case UI_BTYPE_MATOPERATOR:
      retval = ui_do_but_MATERIAL_OPERATOR(C, but, data, event);
      break;
    case UI_BTYPE_MATPRO_TAB:
      retval = ui_do_but_TAB_MATPRO(C, but, data, event);
      break;
    case UI_BTYPE_MATLIST_BUT:
      retval = ui_do_but_MATLIST_ITEM(C, but, data, event);
      break;
    case UI_BTYPE_ANIMATION:
    case UI_BTYPE_BUT_TOGGLE:
    case UI_BTYPE_TOGGLE:
    ...
    case UI_BTYPE_NUM_SLIDER:
      ...
      break;
    case UI_BTYPE_ANIMATION_BAR:
      retval = ui_do_but_ANIMBAR(C, block, but, data, event);
      break;
...
static void button_activate_state(bContext *C, uiBut *but, uiHandleButtonState state)
{
  ...
  if (state == BUTTON_STATE_HIGHLIGHT) {
    ...
    if (ELEM(but->type, UI_BTYPE_BLOCK, UI_BTYPE_IMAGE_MENU,
             UI_BTYPE_PULLDOWN, UI_BTYPE_POPOVER, UI_BTYPE_VIEWCONFIG) ||
...
static uiBut *ui_context_rna_button_active_temp(const bContext *C)
{
  bScreen *screen = CTX_wm_screen(C);
  ARegion *tmp_region = NULL;
  ...
  if (tmp_region) {
    return ui_context_button_active(tmp_region, ui_context_rna_button_active_test);
  }
  else {
    return false;
  }
                                      ...
                                      int *r_index)
{
  ...
  if (activebut == NULL) {
    activebut = ui_context_rna_button_active_temp(C);
  }
...
static bool ui_handle_button_activate_by_type(bContext *C, ARegion *region, uiBut *but)
{
  ...
  else if (ELEM(but->type, UI_BTYPE_BLOCK, UI_BTYPE_IMAGE_MENU, UI_BTYPE_PULLDOWN)) {
      ...
      switch (event->type) {
        ...
        case EVT_RIGHTARROWKEY:
          if (event->val == KM_PRESS && (block->flag & UI_BLOCK_LOOP)) {
            ...
            if (but && ELEM(but->type, UI_BTYPE_IMAGE_MENU, UI_BTYPE_BLOCK, UI_BTYPE_PULLDOWN)) {
              ui_handle_button_activate(C, region, but, BUTTON_ACTIVATE_OPEN);
            }
          }
          ...
          if (ELEM(event->val, KM_PRESS, KM_DBL_CLICK) &&
            ...
            for (but = block->buttons.first; but; but = but->next) {
              if (!(but->flag & UI_BUT_DISABLED) && but->menu_key == event->type) {
                if (ELEM(but->type, UI_BTYPE_BUT, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT)) {
                  UI_but_execute(C, region, but);
                }
                else {
    ...
    if (block->flag & UI_BLOCK_LOOP) {
      ...
      if (menu->menuretval) {
        ...
        if ((but_default != NULL) && (but_default->active == NULL)) {
          if (ELEM(but_default->type, UI_BTYPE_BUT, UI_BTYPE_MATPRO_TAB, UI_BTYPE_MATLIST_BUT)) {
...
static int ui_popup_handler(bContext *C, const wmEvent *event, void *userdata)
{
  ...
  if (event->type == EVT_DROP || event->val == KM_DBL_CLICK) {
     ...
     *   so it'll be possible to perform such operations as opening .ixam files by dropping
     *   them into 3IXAM, even if there's opened popup like splash screen (sergey).
     * KM_DBL_CLICK:

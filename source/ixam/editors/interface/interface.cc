...
/**
 * a full doc with API notes can be found in 'ixam/doc/guides/interface_API.txt'
 *
...
void UI_block_draw(const bContext *C, uiBlock *block)
{
  ...
  if (region->regiontype == RGN_TYPE_TOOLS || region->regiontype == RGN_TYPE_PRO_MENU)
    region->sizey = BLI_rcti_size_y(&rect);
...
static void ui_def_but_rna__menu(bContext * /*C*/, uiLayout *layout, void *but_p)
{
  ...
  for (int a = 0; a < totitems; a++) {
    ...
    if (new_column && (categories > 0) && item->identifier[0]) {
      uiItemL(column, "", ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemS(column);
    }
    ...
    if (!item->identifier[0]) {
      if (item->name) {
        if (item->icon) {
          uiItemL(column, item->name, item->icon, UI_BSHAPE_ROUNDBOX);
        }
        else {
...
static void ui_def_but_rna__panel_type(bContext *C, uiLayout *layout, void *but_p)
{
  ...
  if (pt) {
    ...
    uiItemL(layout, msg, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}
...
{
  uiBut *but = ui_def_but(
      block, UI_BTYPE_IMAGE_MENU, 0, str, x, y, width, height, arg, 0.0, 0.0, 0.0, 0.0, tip);
  
  ((uiButImageMenu*)but)->mt = (MenuType *)arg + 1;
  but->menu_create_func = func;
  ui_but_update(but);
  return but;
}
                      ...
                      const char *tip)
{
  ...
  but->drawflag |= UI_BUT_ICON_RIGHT | UI_BUT_TEXT_LEFT;
  but->drawflag &= ~UI_BUT_ICON_LEFT;

...
struct uiBut {
  ...
  /** #UI_BTYPE_PULLDOWN / #UI_BTYPE_MENU data / #UI_BTYPE_IMAGE_MENU */
...
/** Derived struct for #UI_BTYPE_IMAGE_MENU */
typedef struct uiButImageMenu {
  uiBut but;
  void* mt;
  void* imbuf;  
  int imbuf_size;
...
void *ui_pie_menu_custom_data_get(uiPieMenu *pie);
...
/**
 * Handle region menu events like collapsing and expanding.
 *
 * \note Could become a modal key-map.
 */
extern int ui_handler_sub_panel_region(struct bContext *C,
                                       const struct wmEvent *event,
                                       struct ARegion *region,
...
void ui_draw_but_CURVEPROFILE(struct ARegion *region,
                              ...
                              const struct uiWidgetColors *wcol,
                              const rcti *rect);
void ui_draw_but_menu_IMAGE(struct ARegion *region,
                            uiBut *but,
                            void* ibuf,
                            const struct uiWidgetColors *wcol,
                            const rcti *rect);
...
enum {
  WIDGET_TYPE_ROUNDBOX = 0,
  WIDGET_TYPE_HEX,
  WIDGET_TYPE_TRAPEZOID,
  WIDGET_TYPE_RTRAPEZOID,
  WIDGET_TYPE_RHOMBOID,
  WIDGET_TYPE_RRHOMBOID,
  WIDGET_TYPE_TRIANGLE,
  WIDGET_TYPE_RTRIANGLE,
  WIDGET_TYPE_RECT,
  WIDGET_TYPE_PRO_HEX,
  WIDGET_TYPE_ADV_CHECKBOX,

  WIDGET_TYPE_MAX, /* don't use */
};

struct GPUBatch *ui_batch_hexagon_widget_get(void);
struct GPUBatch *ui_batch_hex_wire_widget_get(void);
...
/**
 * Called on each startup.ixam read,
...
void ui_item_popover_func(struct bContext *C, struct uiLayout *layout, void *arg_pt);

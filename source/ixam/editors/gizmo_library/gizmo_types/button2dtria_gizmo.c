...
typedef struct ButtonGizmo2DTriangle {
  wmGizmo gizmo;
  bool is_init;
  /* Use an icon or shape */
  int icon;
  GPUBatch *shape_batch[2];
} ButtonGizmo2DTriangle;
...
static void imm_draw_triangle(uint shdr_pos, GPUPrimType type, bool invert)
{
  immBegin(type, 3);
  const float rotateFix = invert ? -1.0f : 1.0f;
  immVertex2f(shdr_pos, 1.0, -0.6 * rotateFix);
  immVertex2f(shdr_pos, -1.0, -0.6 * rotateFix);
  immVertex2f(shdr_pos, 0.0, 0.6 * rotateFix);
  immEnd();
  
}

static void button2d_triangle_geom_draw_backdrop(const wmGizmo *gz,
                                        const float color[4],
                                        const float fill_alpha,
                                        const bool select,
                                        const bool invert_option)
{
  float viewport[4];
  ...
  */
  ...
  imm_draw_triangle(pos, GPU_PRIM_LINE_LOOP, invert_option);
  immUnbindProgram();

  UNUSED_VARS(select);
}

static void button2d_triangle_draw_intern(const bContext *C,
                                 wmGizmo *gz,
                                 const bool select,
                                 const bool highlight)
{  
  ButtonGizmo2DTriangle *button = (ButtonGizmo2DTriangle *)gz;
  float viewport[4];
  GPU_viewport_size_get_f(viewport);

  const int draw_options = RNA_enum_get(gz->ptr, "draw_options");
  const bool invert_option = RNA_boolean_get(gz->ptr, "invert");
  ...
  if (select) {
    button2d_triangle_geom_draw_backdrop(gz, color, 1.0, select, invert_option);
  }
  else {
    ...
    if (draw_options & ED_GIZMO_BUTTON_SHOW_BACKDROP) {
      ...
      button2d_triangle_geom_draw_backdrop(gz, color, fill_alpha, select, invert_option);
    }

    if (button->icon != -1) {
      float pos[2];
      
      const float fac = gz->scale_basis / 40.0f;

      pos[0] = 0.0;
      pos[1] = 0.0;
      GPU_matrix_translate_2f(-(fac / 4), -(fac / 4));
      GPU_matrix_translate_2f(0.0f, invert_option ? 0.1f : -0.1f);
...
static void gizmo_button2d_triangle_draw_select(const bContext *C, wmGizmo *gz, int select_id)
{
  GPU_select_load_id(select_id);
  button2d_triangle_draw_intern(C, gz, true, false);
}

static void gizmo_button2d_triangle_draw(const bContext *C, wmGizmo *gz)
{
  const bool is_highlight = (gz->state & WM_GIZMO_STATE_HIGHLIGHT) != 0;

  GPU_blend(GPU_BLEND_ALPHA);
  button2d_triangle_draw_intern(C, gz, false, is_highlight);
  GPU_blend(GPU_BLEND_NONE);
}


static int gizmo_button2d_triangle_test_select(bContext *C, wmGizmo *gz, const int mval[2])
{
  float point_local[2];
...
static int gizmo_button2d_triangle_cursor_get(wmGizmo *gz)
{
...
static bool gizmo_button2d_triangle_bounds(bContext *C, wmGizmo *gz, rcti *r_bounding_box)
{
...
static void gizmo_button2d_triangle_free(wmGizmo *gz)
{
  ButtonGizmo2DTriangle *shape = (ButtonGizmo2DTriangle *)gz;
...
static void GIZMO_GT_button_2d_triangle(wmGizmoType *gzt)
{
  /* identifiers */
  gzt->idname = "GIZMO_GT_button_2d_triangle";

  /* api callbacks */
  gzt->draw = gizmo_button2d_triangle_draw;
  gzt->draw_select = gizmo_button2d_triangle_draw_select;
  gzt->test_select = gizmo_button2d_triangle_test_select;
  gzt->cursor_get = gizmo_button2d_triangle_cursor_get;
  gzt->screen_bounds_get = gizmo_button2d_triangle_bounds;
  gzt->free = gizmo_button2d_triangle_free;

  gzt->struct_size = sizeof(ButtonGizmo2DTriangle);
  ...
  RNA_def_boolean(gzt->srna, "invert", true, "Invert button", "");
...
void ED_gizmotypes_button_2d_triangle(void)
{
  WM_gizmotype_append(GIZMO_GT_button_2d_triangle);
}

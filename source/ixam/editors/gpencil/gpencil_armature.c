...
bool ED_gpencil_add_armature(const bContext *C, ReportList *reports, Object *ob, Object *ob_arm)
{
  ...
  if (md == NULL) {
    md = ED_object_gpencil_modifier_add(
        reports, bmain, scene, ob, "Rig", eGpencilModifierType_Armature);
    if (md == NULL) {
      BKE_report(reports, RPT_ERROR, "Unable to add a new Rig modifier to object");
      return false;
    }
  ...
  if (mmd->object == NULL) {
    ...
    if (ob_arm != mmd->object) {
      BKE_report(reports,
                 RPT_ERROR,
                 "The existing Rig modifier is already using a different Rig object");
      return false;
    }
  }
  return true;
...
static int gpencil_generate_weights_exec(bContext *C, wmOperator *op)
{
  ...
  if (arm_idx > 0) {
    ...
    if (md == NULL) {
      BKE_report(op->reports, RPT_ERROR, "The grease pencil object need an Rig modifier");
      return OPERATOR_CANCELLED;
    }

    ArmatureGpencilModifierData *mmd = (ArmatureGpencilModifierData *)md;
    if (mmd->object == NULL) {
      BKE_report(op->reports, RPT_ERROR, "Rig modifier is not valid or wrong defined");
      return OPERATOR_CANCELLED;
    }
  ...
  if (ob_arm == NULL) {
    BKE_report(op->reports, RPT_ERROR, "No Rig object in the view layer");
    return OPERATOR_CANCELLED;
  }
...
void GPENCIL_OT_generate_weights(wmOperatorType *ot)
{
  ...
  prop = RNA_def_enum(
      ot->srna, "armature", DummyRNA_DEFAULT_items, 0, "Rig", "Rig to use");
  ...
  RNA_def_float(ot->srna,
                ...
                2.0f,
                "Ratio",
                "Ratio between joint length and influence radius",
                0.001f,
                1.0f);
  ...
  RNA_def_float(ot->srna,
                ...
                1.0f,
                "Decay",
                "Factor to reduce influence depending of distance to joint axis",
                0.0f,
                1.0f);
}

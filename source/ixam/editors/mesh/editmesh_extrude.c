...
static int edbm_extrude_region_exec(bContext *C, wmOperator *op)
{
  ...
  const bool use_dissolve_ortho_edges = RNA_boolean_get(op->ptr, "use_dissolve_ortho_edges");
  CTX_wm_view3d_polyform_ui_highlight_flag_set(C, 
      use_dissolve_ortho_edges ? POLYFORM_UI_HIGHLIGHT_EXTRUDE_MANIFOLD : POLYFORM_UI_HIGHLIGHT_EXTRUDE_STANDART);
...
static int edbm_extrude_verts_exec(bContext *C, wmOperator *op)
{
  ...
  /* update PolyForm panel flag as running operator */
  CTX_wm_view3d_polyform_ui_highlight_flag_set(C, POLYFORM_UI_HIGHLIGHT_EXTRUDE_INDIV);

...
//    region->winy = 40;
//    region->sizey = 40;
//    region->winrct.ymin -= 14;
//    region->v2d.winy = 40;
//    region->v2d.tot.ymax = region->v2d.cur.ymax = 40;
  //}
  //region->winy += 10;
...
static void recent_files_menu_draw(const bContext *UNUSED(C), Menu *menu)
{
  ...
  if (!BLI_listbase_is_empty(&G.recent_files)) {
    for (recent = G.recent_files.first; (recent); recent = recent->next) {
      ...
      const int icon = BLO_has_bfile_extension(file) ? ICON_FILE_IXAM : ICON_FILE_BACKUP;
      PointerRNA ptr;
    ...
    uiItemL(layout, IFACE_("No Recent Files"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void topbar_ixam_write(IxamWriter *writer, SpaceLink *sl)
{

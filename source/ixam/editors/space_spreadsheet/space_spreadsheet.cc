...
using namespace ixam::ed::spreadsheet;
...
{
  using ixam::ed::viewer_path::ViewerPathForGeometryNodesViewer;

  SpaceSpreadsheet *sspreadsheet = CTX_wm_space_spreadsheet(C);
  Object *active_object = CTX_data_active_object(C);
  Object *context_object = ixam::ed::viewer_path::parse_object_only(sspreadsheet->viewer_path);
    ...
    case SPREADSHEET_OBJECT_EVAL_STATE_VIEWER_NODE: {
      ...
      if (sspreadsheet->flag & SPREADSHEET_FLAG_PINNED) {
        const std::optional<ViewerPathForGeometryNodesViewer> parsed_path =
            ixam::ed::viewer_path::parse_geometry_nodes_viewer(sspreadsheet->viewer_path);
        if (parsed_path.has_value()) {
          if (ixam::ed::viewer_path::exists_geometry_nodes_viewer(*parsed_path)) {
      ...
      const std::optional<ViewerPathForGeometryNodesViewer> workspace_parsed_path =
          ixam::ed::viewer_path::parse_geometry_nodes_viewer(workspace->viewer_path);
...
static void spreadsheet_footer_region_draw(const bContext *C, ARegion *region)
{
  ...
  uiItemL(layout, stats_str.c_str(), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
static void spreadsheet_ixam_read_data(IxamDataReader *reader, SpaceLink *sl)
{
  SpaceSpreadsheet *sspreadsheet = (SpaceSpreadsheet *)sl;
  ...
  BKE_viewer_path_ixam_read_data(reader, &sspreadsheet->viewer_path);
}

static void spreadsheet_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceSpreadsheet *sspreadsheet = (SpaceSpreadsheet *)sl;
  BKE_viewer_path_ixam_read_lib(reader, parent_id->lib, &sspreadsheet->viewer_path);
}

static void spreadsheet_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  ...
  BKE_viewer_path_ixam_write(writer, &sspreadsheet->viewer_path);
}
...
void ED_spacetype_spreadsheet()
{
  ...
  st->ixam_read_data = spreadsheet_ixam_read_data;
  st->ixam_read_lib = spreadsheet_ixam_read_lib;
  st->ixam_write = spreadsheet_ixam_write;
  ...
  ixam::ed::spreadsheet::spreadsheet_data_set_region_panels_register(*art);
  BLI_addhead(&st->regiontypes, art);

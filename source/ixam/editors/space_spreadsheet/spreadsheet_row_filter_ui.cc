...
static void spreadsheet_filter_panel_draw_header(const bContext *C, Panel *panel)
{
  ...
  if (column_name.is_empty()) {
    uiItemL(row, IFACE_("Filter"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  else if (column == nullptr) {
    uiItemL(row, column_name.data(), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  else {
  ...
  uiItemS_ex(layout, 0.25f, UI_BSHAPE_ROUNDBOX);
}
...
static void spreadsheet_filter_panel_draw(const bContext *C, Panel *panel)
{
  ...
  switch (static_cast<eSpreadsheetColumnValueType>(column->data_type)) {
    ...
    case SPREADSHEET_VALUE_TYPE_STRING:
      ...
      break;
    case SPREADSHEET_VALUE_TYPE_UNKNOWN:
      uiItemL(layout, IFACE_("Unkown column type"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
      break;
  }
}

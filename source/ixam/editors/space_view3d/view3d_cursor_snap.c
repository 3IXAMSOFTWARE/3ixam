                                  ...
                                  const float matrix[4][4])
{
  ...
  if (pixel_size > FLT_EPSILON) {
    ...
    v3d_cursor_plane_draw_grid(lines, final_scale_fade+0.1f, final_scale_fade, matrix, plane_axis, color);
  }
}
  ...
  if (calc_plane_omat) {
    ...
    if (orient_surface) {
      ...
      if (state->use_plane_axis_auto) {
        ...
        if (rv3d->winmat[3][3] == 0.0f || rv3d->view == RV3D_VIEW_USER) {
          if (show_axis_x) {
        ...
        view_plane[0] = (float)((grid_flag & (PLANE_XZ | PLANE_XY)) == 0);
        view_plane[1] = (float)((grid_flag & (PLANE_YZ | PLANE_XY)) == 0);
        view_plane[2] = (float)((grid_flag & (PLANE_YZ | PLANE_XZ)) == 0);
        
        mat3_align_axis_to_v3(omat, state->plane_axis, view_plane);
      }
    }
...
//  ARegion *region = BKE_area_find_region_type(area, RGN_TYPE_WINDOW);
  ARegion *region = BKE_area_find_region_xy(area, RGN_TYPE_WINDOW, (const int[2]){x, y});
  if (!region)
  {
    region = BKE_area_find_region_type(area, RGN_TYPE_WINDOW);
  }
  ...
  if (v3d_cursor_eventstate_has_changed(data_intern, state, wm, x, y)) {
    ...
    if (region->alignment == RGN_ALIGN_QSPLIT) {
      RegionView3D *rv3d = region->regiondata;
      if (rv3d) {
        v3d = CTX_wm_view3d_index(C, rv3d->quad_index);
      }
    }

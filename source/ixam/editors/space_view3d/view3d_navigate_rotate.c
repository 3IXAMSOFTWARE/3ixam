...
void viewrotate_apply(ViewOpsData *vod, const int event_xy[2])
{
  RegionView3D *rv3d = vod->rv3d;
...
/* -------------------------------------------------------------------- */
/** \name Outliner Move Objects
 *
 * Dropbox operator for move objects inside temporary region.
 * \{ */
...
bool folder_drop_init(
    bContext *C, wmDrag *drag, const wmEvent *event, struct CollectionDropFolder *data)
{
  ...
  if (but->type != UI_BTYPE_OUTLINERFOLDER) {
    return false;
  }
  Collection *to_collection = get_collection_from_but(but);
...
static int outliner_move_obejct(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  wmDrag *drag = (wmDrag *)(lb->first);

  struct CollectionDropFolder data;
  if (!folder_drop_init(C, drag, event, &data)) {
    return OPERATOR_CANCELLED;
  }
  
  Collection *relative = data.to;
  bool relative_after = false;

  LISTBASE_FOREACH (wmDragID *, drag_id, &drag->ids) {
    Collection *from = (event->modifier & KM_CTRL) ? NULL : collection_parent_from_ID(drag_id->from_parent);
...
static bool outliner_move_obejct_poll(bContext *C)
{
  return true;
}

void VIEW3D_OT_outliner_move(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Outliner Object Move";
  ot->description = "Dropbox operator for move objects inside temporary region";
  ot->idname = "VIEW3D_OT_outliner_move";

  /* api callbacks */
  ot->invoke = outliner_move_obejct;
  ot->poll = outliner_move_obejct_poll;

...
enum eViewOpsFlag viewops_flag_from_args(bool use_select, bool use_depth)
{
  enum eViewOpsFlag flag = VIEWOPS_FLAG_NONE;
  if (use_select) {
    flag = (enum eViewOpsFlag)((int)flag | (int)VIEWOPS_FLAG_ORBIT_SELECT);
  }
  if (use_depth) {
    flag = (enum eViewOpsFlag)((int)flag | (int)VIEWOPS_FLAG_DEPTH_NAVIGATE);
  }
...
static int view_camera_exec(bContext *C, wmOperator *op)
{
  ...
  PropertyRNA *prop_camera = RNA_struct_find_property(op->ptr, "camera");
  Object *camera = RNA_property_is_set(op->ptr, prop_camera) ?
                    RNA_property_pointer_get(op->ptr, prop_camera).data :
                    NULL;
  ...
  if ((RV3D_LOCK_FLAGS(rv3d) & RV3D_LOCK_ANY_TRANSFORM) == 0) {
    ...
    if (rv3d->persp != RV3D_CAMOB || (camera && camera != scene->camera)) {
...
void VIEW3D_OT_view_camera(wmOperatorType *ot)
{
  ...
  StructRNA* object = RNA_struct_find("Object");

  prop = RNA_def_pointer_runtime(
      ot->srna, "camera", object, "Camera", "The reference camera to use view from");

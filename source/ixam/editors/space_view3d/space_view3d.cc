...
static SpaceLink *view3d_create(const ScrArea * /*area*/, const Scene *scene)
{
  ...
  region = MEM_cnew<ARegion>("viewbar for view3d");
  BLI_addtail(&v3d->regionbase, region);
  region->regiontype = RGN_TYPE_VIEWBAR;
  region->alignment = RGN_ALIGN_BOTTOM_LEFT;
  region->overlap = true;

  region = MEM_cnew<ARegion>("hud for view3d");
  BLI_addtail(&v3d->regionbase, region);
  ...
  region->alignment = RGN_ALIGN_TOP_RIGHT;
  region->overlap = true;
  region->flag |= RGN_FLAG_DYNAMIC_SIZE;

  region = MEM_cnew<ARegion>("pro_menu for view3d");
  BLI_addtail(&v3d->regionbase, region);
  region->regiontype = RGN_TYPE_PRO_MENU;
  region->alignment = RGN_ALIGN_TOP_LEFT;
  region->overlap = true;
  region->flag |= RGN_FLAG_DYNAMIC_SIZE;
...
static void view3d_main_region_init(wmWindowManager *wm, ARegion *region)
{
  ...
  keymap = WM_keymap_ensure(wm->defaultconf, "Rigs", 0, 0);
  WM_event_add_keymap_handler(&region->handlers, keymap);
...
static void view3d_collection_drop_copy(bContext *C, wmDrag *drag, wmDropBox *drop)
{
...
static void view3d_lightcache_update(bContext *C)
{
  ...
  if (!BKE_scene_uses_ixam_eevee(scene)) {
...
static bool view3d_folders_collection_drop_poll(bContext *C, wmDrag *drag, const wmEvent *event)
{
  CollectionDropFolder data;
  ARegion *tmp_region = nullptr;
  ...
  bool changed = false;
  if (folder_drop_init(C, drag, event, &data)) {
    changed = true;
    if (changed) {
      ED_region_tag_refresh_ui(tmp_region);
    }
    return true;
  }
  if (changed) {
...
static void view3d_dropboxes()
{
  ...
  ListBase *lb_new = WM_dropboxmap_find("View3D", SPACE_VIEW3D, RGN_TYPE_TEMPORARY);
  WM_dropbox_add(lb_new,
                 "VIEW3D_OT_outliner_move",
                 view3d_folders_collection_drop_poll,
                 nullptr,
                 nullptr,
                 nullptr);
}
...
static uiBlock *create_block_animation(const bContext *C, ARegion *region)
{
  float offset = UI_DPI_FAC * 20; // region size increase offset
  View2D *v2d = &region->v2d;
  const uiStyle *style = UI_style_get_dpi();
  const float pop_scale[2] = {1.15f, 1.55f};
  
  uiBlock *block = UI_block_begin(C, region, "AnimationBlock", UI_EMBOSS);
  
  float width = BLI_rctf_size_x(&v2d->cur);
  float height = BLI_rctf_size_y(&v2d->cur);
  
  float layout_size[2] = {width * 3.0f / 16, height / 4};
  float but_size[2];
  
  copy_v2_v2(but_size, layout_size);
  mul_v2_v2(layout_size, pop_scale);
  
  if(region->draw_flag & RGN_DRAW_ANIMATION) {
    mul_v2_v2(but_size, pop_scale);
  }

  WorkSpace *workspace = CTX_wm_workspace(C);

  /* Don't draw anim bar in custom workspaces */
  if (workspace->type != WORKSPACE_TYPE_NONE) {
    return block;
  }
  
  uiDefBut(
           block,
           UI_BTYPE_ANIMATION | UI_BUT_POIN_SHORT,
           0,
           "",
           (width - but_size[0]) / 2,
           offset + height / 2 - but_size[1] - 3, // half line offset
           but_size[0],
           but_size[1],
           &region->draw_flag,
           0,
  ...
  if(region->draw_flag & RGN_DRAW_ANIMATION) {
    struct PanelType *pt = WM_paneltype_find("VIEW3D_PT_animation", true);
...
static void view3d_header_region_draw(const bContext *C, ARegion *region)
{
  ...
  if (region->alignment != RGN_ALIGN_FLOAT) {
    ThemeColorID colorid = (region->type->regionid == RGN_TYPE_PREVIEW) ? TH_PREVIEW_BACK : TH_BACK;
    if (region->alignment == RGN_ALIGN_FLOAT) {
  ...
  const int logo_size = IMAGE_MENU_SIZE * UI_DPI_FAC;
  const float line_width = 1.0f;
  const float line_offset = -5.f;
  float offset = UI_DPI_FAC * 20; // region size increase offset
  rctf copy; //copy of v2d->cur cause region size increased
  memcpy(&copy, &v2d->cur, sizeof(rcti));
  BLI_rctf_resize_y(&v2d->cur, BLI_rctf_size_y(&v2d->cur) - offset);
  BLI_rctf_translate(&v2d->cur, 0, offset / 2.0f);
  ...
  immUniformColor3ubv(btheme->space_view3d.back_grad);
  immBegin(GPU_PRIM_TRIS, 12);
  
  immVertex2f(pos, v2d->cur.xmin, v2d->cur.ymax + line_width);
  immVertex2f(pos, v2d->cur.xmax, v2d->cur.ymax + line_width);
  immVertex2f(pos, v2d->cur.xmin, (v2d->cur.ymin + v2d->cur.ymax) / 2.0f + line_offset);

  immVertex2f(pos, v2d->cur.xmin, (v2d->cur.ymin + v2d->cur.ymax) / 2.0f + line_offset);
  immVertex2f(pos, v2d->cur.xmax, v2d->cur.ymax + line_width);
  immVertex2f(pos, v2d->cur.xmax, (v2d->cur.ymin + v2d->cur.ymax) / 2.0f + line_offset);
  
  immVertex2f(pos, v2d->cur.xmin, v2d->cur.ymin + line_width);
  immVertex2f(pos, v2d->cur.xmin + logo_size, v2d->cur.ymin + line_width);
  immVertex2f(pos, v2d->cur.xmin + logo_size * 2.0f, (v2d->cur.ymin + v2d->cur.ymax) / 2.0f + line_offset);
...
//  immUniformColor4f(0.0f, 0.66f, 0.95f, 1.0f);
  ...
  uiBlock * block = create_block_animation(C, region);
  memcpy(&v2d->cur, &copy, sizeof(rcti)); //restore copy
...
static void view3d_pro_menu_region_listener(const wmRegionListenerParams *params)
{
  ARegion *region = params->region;
  const wmNotifier *wmn = params->notifier;
...
static void view3d_pro_menu_region_init(wmWindowManager* wm, ARegion* region) {
  ED_region_panels_init(wm, region);
  
  region->alignment = U.hand_type == USER_HAND_RIGHT ? RGN_ALIGN_TOP_LEFT : RGN_ALIGN_TOP_RIGHT;
}
...
static void view3d_tools_region_init(wmWindowManager *wm, ARegion *region)
{
  ...
  region->alignment = U.hand_type == USER_HAND_RIGHT ? RGN_ALIGN_TOP_RIGHT : RGN_ALIGN_TOP_LEFT;
  ...
  /* add drop boxes */
  lb = WM_dropboxmap_find("View3D", SPACE_VIEW3D, RGN_TYPE_TEMPORARY);
  WM_event_add_dropbox_handler(&region->handlers, lb);
}

static void view3d_tools_region_draw(const bContext *C, ARegion *region)
{
  ED_region_panels_ex(C, region, (const char *[]){CTX_data_mode_string(C), nullptr});
}
...
static void space_view3d_listener(const wmSpaceTypeListenerParams *params)
{
  ...
  switch (wmn->category) {
    ...
    case NC_SPACE:
      switch (wmn->data) {
        case ND_SPACE_VIEW3D_HAND_TYPE:
          ED_area_tag_refresh(area);
          break;
      }
      break;
...
static void view3d_ixam_read_data(IxamDataReader *reader, SpaceLink *sl)
{
  ...
  if (v3d->gpd) {
    ...
    BKE_gpencil_ixam_read_data(reader, v3d->gpd);
  }
  ...
  BKE_screen_view3d_shading_ixam_read_data(reader, &v3d->shading);

  BKE_screen_view3d_do_versions_250(v3d, &sl->regionbase);

  BKE_viewer_path_ixam_read_data(reader, &v3d->viewer_path);
}

static void view3d_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  View3D *v3d = (View3D *)sl;
  ...
  BKE_viewer_path_ixam_read_lib(reader, parent_id->lib, &v3d->viewer_path);
}

static void view3d_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  View3D *v3d = (View3D *)sl;
  ...
  BKE_screen_view3d_shading_ixam_write(writer, &v3d->shading);

  BKE_viewer_path_ixam_write(writer, &v3d->viewer_path);
}
...
void ED_spacetype_view3d()
{
  ...
  st->ixam_read_data = view3d_ixam_read_data;
  st->ixam_read_lib = view3d_ixam_read_lib;
  st->ixam_write = view3d_ixam_write;
  ...
  /* regions: viewbar */
  art = MEM_cnew<ARegionType>("spacetype view3d viewbar region");
  art->regionid = RGN_TYPE_VIEWBAR;
  ...
  art->prefsizey = 100; /* XXX */
  art->prefsizex = 250; /* XXX */
  art->keymapflag = ED_KEYMAP_UI | ED_KEYMAP_FRAMES;
  art->listener = view3d_buttons_region_listener;
  ...
  for (int i = eModifierType_None; i < NUM_MODIFIER_TYPES; i++) {
    const ModifierTypeInfo *mti = BKE_modifier_get_info((ModifierType)i);
    if (mti != NULL && mti->panelRegister != nullptr) {
      mti->panelRegister(art);
    }
  }
  
  art = MEM_cnew<ARegionType>("spacetype view3d pro menu region");
  art->regionid = RGN_TYPE_PRO_MENU;
  art->prefsizey = 100; /* XXX */
  art->prefsizex = 150; /* XXX */
  art->keymapflag = ED_KEYMAP_UI | ED_KEYMAP_FRAMES;
  art->listener = view3d_pro_menu_region_listener;
  art->message_subscribe = ED_region_generic_tools_region_message_subscribe;
  art->init = view3d_pro_menu_region_init;
  art->draw = view3d_tools_region_draw;
  BLI_addhead(&st->regiontypes, art);

...
/* copy of find_any_view3d_and_area_available in render_internal.cc */
...
static int view3d_camera_to_view_contextfree_exec(bContext *C, wmOperator *op)
{
  const Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  ScrArea *area = NULL;
  View3D *v3d = NULL;
  ...
  bool lookup_success = 
      find_any_view3d_and_area_available(C, &area, &v3d) &&
      ED_view3d_area_user_region(area, v3d, &region);
...
void VIEW3D_OT_camera_to_view_contextfree(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Align Camera to View";
  ot->description = "Set camera view to active view";
  ot->idname = "VIEW3D_OT_camera_to_view_contextfree";

  /* api callbacks */
  ot->exec = view3d_camera_to_view_contextfree_exec;
  ot->poll = NULL;
...
void ED_view3d_xr_shading_update(wmWindowManager *wm, const View3D *v3d, const Scene *scene)
{
  if (v3d->runtime.flag & V3D_RUNTIME_XR_SESSION_ROOT) {
    ...
    if (v3d->shading.type == OB_RENDER) {
      if (!(BKE_scene_uses_ixam_workbench(scene) || BKE_scene_uses_ixam_eevee(scene))) {

...
void viewzoom_apply(ViewOpsData *vod,
                           const int xy[2],
                           const eViewZoom_Style viewzoom,
                           const bool zoom_invert,
                           const bool zoom_to_pos);

static int view3d_view_translation_add_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ViewOpsData *vod;
...
static bool view3d_view_translation_add_poll(bContext *C)
{
  return true;
}

static void view3d_view_translation_add_cancel(bContext *C, wmOperator *op)
{
  viewops_data_free(C, op);
}

static int view3d_view_translation_add_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ViewOpsData *vod = op->customdata;
  short event_code = VIEW_PASS;
  bool use_autokey = false;
  int ret = OPERATOR_RUNNING_MODAL;
  const bToolRef *tref = vod->area->runtime.tool;
  /* execute the events */
  switch (event->type)
  {
  case MOUSEMOVE:
  ...
  if (event_code == VIEW_APPLY) {
    const bool use_cursor_init = RNA_boolean_get(op->ptr, "use_cursor_init");
    if (tref && STREQ(tref->idname, "view3d.move_with_wait")) {
      viewmove_apply(vod, event->xy[0], event->xy[1]);
    }
    else if (tref && STREQ(tref->idname, "view3d.rotate_with_wait")) {
      viewrotate_apply(vod, event->xy);
    }
    else if (tref && STREQ(tref->idname, "view3d.zoom_with_wait")) {
      viewzoom_apply(vod,
                     event->xy,
...
void VIEW3D_OT_view_translation_add(struct wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Transform View";
  ot->description = "Rotate, Zoom or Move view";
  ot->idname = "VIEW3D_OT_view_translation_add";

  /* api callbacks */
  ot->invoke = view3d_view_translation_add_invoke;
  ot->poll = view3d_view_translation_add_poll;
  ot->modal = view3d_view_translation_add_modal;
  ot->cancel = view3d_view_translation_add_cancel;
  ...
  prop = RNA_def_boolean(ot->srna, "use_cursor_init", true, "Use Cursor Init Pos", "");
  RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
}

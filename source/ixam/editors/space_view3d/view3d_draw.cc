...
RenderEngineType *ED_view3d_engine_type(const Scene *scene, int drawtype)
{
  ...
  if (drawtype == OB_MATERIAL && (type->flag & RE_USE_EEVEE_VIEWPORT)) {
    return RE_engines_find(RE_engine_id_IXAM_EEVEE);
  }
  return type;
}
...
static uiBlock *wm_block_create_view_info(const bContext *C, ARegion *region)
{
  ...
  const char* panel_name = "VIEW3D_PT_view_info";
  ...
  PanelType *pt = WM_paneltype_find(panel_name, true);
  if (pt && (!pt->poll  || pt->poll(C, pt))) {
                                             ...
                                             char err_out[256])
{
  ...
  if (drawtype == OB_RENDER) {
    ...
    if (!(BKE_scene_uses_ixam_eevee(scene) || BKE_scene_uses_ixam_workbench(scene))) {
      drawtype = OB_SOLID;
    }
  }
                              ...
                              ViewDepths **r_depths)
{
  ...
  GPUViewport *viewport = WM_draw_region_get_viewport(region);
  /* When 3IXAM is starting, a click event can trigger a depth test while the viewport is not
   * yet available. */
  if (viewport != nullptr) {
    switch (mode) {

...
GPUTexture* axis_spheremap_tex = NULL;
...
static void gizmo_axis_draw(const bContext *C, wmGizmo *gz)
{
  ...
  float m4[4][4];
  copy_m4_m4(m4, gz->matrix_offset);
  invert_m4(m4);
  ...
  /* Circle defining active area. */
  {
    ...
    GPU_matrix_push();
    //GPU_matrix_scale_1f(1.f / WIDGET_RADIUS);
    
    GPUBatch *sphere = GPU_batch_preset_sphere(2);
    struct SimpleLightingData simple_lighting_data;
    
    const float v_final[3] = {-0.8f, 0.f, 0.45f};
    immBindBuiltinProgram(GPU_SHADER_3D_POLYLINE_SMOOTH_COLOR);
    immUniform2fv("viewportSize", &viewport_size[2]);
    immUniform1f("lineWidth", 5);
    immBegin(GPU_PRIM_LINES, 66);
    
    float pos[3] = {0.f, 0.f, 0.f};
    
    const float step = M_PI / 16;
    const float line_color[4] = {0.137f, 0.203f, 0.309f, 1.f};
    
    for(float ang = 0.f; ang < 2 * M_PI + 0.001f; ang += step) {
      
      pos[0] = cosf(ang) * 1.25;
      pos[1] = -0.75f + 0.5 * (pos[2] = sinf(ang));
      immAttr4fv(color_id, line_color);
      immVertex3fv(pos_id, pos);
      
      pos[0] = cosf(ang - step) * 1.25;
      pos[1] = -0.75f + 0.5 * (pos[2] = sinf(ang - step));
      immAttr4fv(color_id, line_color);
      immVertex3fv(pos_id, pos);
    }
...
#ifdef DRAW_COMPASS_DIRS
    char dirs[8] = {'E', 0, 'N', 0, 'W', 0, 'S', 0};
    for(int i = 0; i < 4; ++i) {

      float axis_str_width, axis_string_height;
      BLF_width_and_height(font.id, &dirs[i * 2], 2, &axis_str_width, &axis_string_height);

      /* Calculate pixel-aligned location, without this text draws fuzzy. */
      float pos[3];
      pos[0] = cosf(i * M_PI * .5f) * 1.25;
      pos[1] = -0.75f + 0.0 * (pos[2] = sinf(i * M_PI * .5f));
      
      float v_final_px[3];
      mul_v3_m3v3(v_final_px, font.matrix_m3_invert, pos);
      ...
      v_final_px[0] = roundf(v_final_px[0] - (axis_str_width * 0.55f));
      ...
      //BLF_rotation(font.id, 0.f);
      //BLF_position(font.id, pos[0], pos[1], pos[2]);
      BLF_color4fv(font.id, text_color);
      BLF_draw(font.id, &dirs[i * 2], 2);
      GPU_matrix_pop();
    }
#endif
    ...
    copy_v4_fl4(simple_lighting_data.color, .0f, .6f, .7f, 1.f);
    copy_v3_fl3(simple_lighting_data.light, /*-.35f, .27f, .9f*/v_final[0], v_final[1], v_final[2]);
    simple_lighting_data.intensity = 0.3f;
    GPUUniformBuf *ubo = GPU_uniformbuf_create_ex(
        sizeof(struct SimpleLightingData), &simple_lighting_data, __func__);

    GPU_batch_program_set_builtin(sphere, GPU_SHADER_SIMPLE_IMG_LIGHTING);
    GPU_batch_uniformbuf_bind(sphere, "simple_lighting_data", ubo);
    GPU_batch_texture_bind(sphere, "image", axis_spheremap_tex);
  ...
  
  {
    ...
    back[3] *= 0.75f;
    immUniformColor4fv(back);

    GPU_blend(GPU_BLEND_ALPHA);
    GPU_line_smooth(true);
    imm_draw_circle_partial_wire_2d(pos, 0.0f, 0.0f, 1.0f, 32, /*-160.f*/0.f, /*140.f*/360.f);
    GPU_blend(GPU_BLEND_NONE);
...
//static void gizmo_axis_free(wmGizmo *UNUSED(gz)) {
//  GPU_texture_free(axis_spheremap_tex);
//}

//static void gizmo_axis_setup(wmGizmo* UNUSED(gz)) {
//  if(!axis_spheremap_tex) {
//
//
//  } else {
//    GPU_texture_ref(axis_spheremap_tex);
//  }
//}
...
void VIEW3D_GT_navigate_init_images() {
  ImBuf* spheremap = IMB_ibImageFromMemory((const uchar *)datatoc_spheremap_png,
                                        datatoc_spheremap_png_size,
                                        IB_rect,
                                        NULL,
                                        "spheremap");
  axis_spheremap_tex
    = GPU_texture_create_2d("spheremap", spheremap->x, spheremap->y, 1, GPU_RGBA8, NULL);
  GPU_texture_update_mipmap(axis_spheremap_tex, 0, GPU_DATA_UBYTE, spheremap->rect);
  
  IMB_freeImBuf(spheremap);
}

void VIEW3D_GT_navigate_free_images() {
  if (axis_spheremap_tex) {
    GPU_texture_free(axis_spheremap_tex);
  }
}

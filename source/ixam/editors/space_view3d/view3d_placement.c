...
enum ePlace_PrimType {
  ...
  PLACE_PRIMITIVE_TYPE_TORUS = 6,
  PLACE_PRIMITIVE_TYPE_TUBE = 7,
  PLACE_PRIMITIVE_TYPE_HEXAGON = 8,
  PLACE_PRIMITIVE_TYPE_PYRAMID = 9,
  PLACE_PRIMITIVE_TYPE_PLANE = 10,
  PLACE_PRIMITIVE_TYPE_GRID = 11,
};
...
static void calc_pbox(struct InteractivePlaceData *ipd, float bounds[4][3], float center[3]) {
  ...
  copy_v3_v3(second_point, center);
  sub_v3_v3(second_point, ipd->step[2].co_dst);
  add_v3_v3(second_point, center);
  
  copy_v3_v3(quad_base[0], second_point);
  copy_v3_v3(quad_base[2], ipd->step[2].co_dst);
...

  {
    ...
    sub_v3_v3v3(delta_local, second_point, ipd->step[2].co_dst);
    ...
    add_v3_v3v3(quad_base[1], ipd->step[2].co_dst, delta_a);
    add_v3_v3v3(quad_base[3], ipd->step[2].co_dst, delta_b);
  }
  
  for (int i = 0; i < ARRAY_SIZE(quad_base); i++) {
    copy_v3_v3(bounds[i], quad_base[i]);
  }
}
...
static bool calc_bbox(struct InteractivePlaceData *ipd, BoundBox *bounds)
{
  ...
  float delta_local[3];
  if (ELEM(ipd->step_index, STEP_DEPTH, STEP_EXTRA)) {
                                     ...
                                     int flatten_axis)
{
  ...
  if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TUBE) {
    if (ipd->step_index == STEP_EXTRA) {
      /* Calculate temporary rectangle for getting its sizes */
      float second_bounds[4][3] = {0};
      float center[3];
      mid_v3_v3v3v3v3(center, UNPACK4((&bounds.vec[4])));
      calc_pbox(ipd, second_bounds, center);
      float scale = min_ff(1.0, len_v3v3(second_bounds[0], second_bounds[1]) / len_v3v3(bounds.vec[0], bounds.vec[1]));
      
      /* Apply scale to outter rectangle */
      for(int i = 0; i < 4; ++i) {
        copy_v3_v3(second_bounds[i], bounds.vec[4 + i]);
        sub_v3_v3(second_bounds[i], center);
        mul_v3_fl(second_bounds[i], scale);
        add_v3_v3(second_bounds[i], center);
      }
    
      draw_circle_in_quad(UNPACK4(second_bounds), 32, color);
    }
  }
  if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_CYLINDER
            || ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TORUS
            || ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TUBE) {
    draw_circle_in_quad(UNPACK4(bounds.vec), 32, color);
    draw_circle_in_quad(UNPACK4((&bounds.vec[4])), 32, color);
  }
  else if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_HEXAGON) {
    draw_circle_in_quad(UNPACK4(bounds.vec), 6, color);
    draw_circle_in_quad(UNPACK4((&bounds.vec[4])), 6, color);
  }
    ...
    else if (ipd->primitive_type ==   PLACE_PRIMITIVE_TYPE_PYRAMID) {
    draw_circle_in_quad(UNPACK4(bounds.vec), 6, color);
...
static void view3d_interactive_add_begin(bContext *C, wmOperator *op, const wmEvent *event)
{
...

  {
    ...
    ipd->step[STEP_EXTRA].is_degenerate_view_align =  ipd->step[STEP_BASE].is_degenerate_view_align;
  ...
  /* Setup the primitive type. */
  {
    ...
    if (RNA_property_is_set(op->ptr, prop)) {
      ...
      else if (tref && STREQ(tref->idname, "builtin.primitive_torus_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_TORUS;
      }
      else if (tref && STREQ(tref->idname, "builtin.primitive_tube_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_TUBE;
      }
      else if (tref && STREQ(tref->idname, "builtin.primitive_hexagon_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_HEXAGON;
      }
      else if (tref && STREQ(tref->idname, "builtin.primitive_pyramid_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_PYRAMID;
      }
      else if (tref && STREQ(tref->idname, "builtin.primitive_plane_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_PLANE;
      }
      else if (tref && STREQ(tref->idname, "builtin.primitive_grid_add")) {
        ipd->primitive_type = PLACE_PRIMITIVE_TYPE_GRID;
      }
      else {
...
static int view3d_interactive_add_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (ipd->step_index == STEP_BASE) {
    if (ELEM(event->type, ipd->launch_event, LEFTMOUSE)) {
      if (event->val == KM_RELEASE) {
        if (ELEM(ipd->primitive_type, PLACE_PRIMITIVE_TYPE_PLANE, PLACE_PRIMITIVE_TYPE_GRID)) {
          BoundBox bounds;
          calc_bbox(ipd, &bounds);
          ...
          PointerRNA op_props;
          if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_PLANE) {
            ot = WM_operatortype_find("MESH_OT_primitive_plane_add", false);
          }
          else if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_GRID) {
            ot = WM_operatortype_find("MESH_OT_primitive_grid_add", false);
          }
          if (ot != NULL) {
            ...
            WM_operator_name_call_ptr(C, ot, WM_OP_EXEC_DEFAULT, &op_props, event);
      ...
      if (event->val == KM_PRESS) {
        ...
        if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TUBE) {
          /* Create normal. */
          {
            ...
            if (ipd->step[STEP_DEPTH].is_degenerate_view_align) {
              cross_v3_v3v3(no_temp, ipd->step[1].plane, ipd->step[STEP_EXTRA].degenerate_diagonal);
              cross_v3_v3v3(no, no_temp, ipd->step[1].plane);
            }
            else {
              cross_v3_v3v3(no_temp, ipd->step[1].plane, rv3d->viewinv[2]);
              cross_v3_v3v3(no, no_temp, ipd->step[1].plane);
            }
            normalize_v3(no);

            plane_from_point_normal_v3(ipd->step[2].plane, ipd->step[1].co_dst, no);
          }
          
          copy_v3_v3(ipd->step[2].co_dst, ipd->step[1].co_dst);
          
          ipd->step_index = STEP_EXTRA;
          
          /* Use the toggle from the previous step. */
          if (ipd->step[1].is_centered != ipd->step[1].is_centered_init) {
            ipd->step[2].is_centered = !ipd->step[2].is_centered;
          }
          if (ipd->step[1].is_fixed_aspect != ipd->step[2].is_fixed_aspect_init) {
            ipd->step[2].is_fixed_aspect = !ipd->step[2].is_fixed_aspect;
          }
        }
        else {
          BoundBox bounds;
          ...
          else if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TORUS) {
            ot = WM_operatortype_find("MESH_OT_primitive_torus_add", false);
            scale[2] *= 4.0f;
          }
          else if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_PLANE) {
            ot = WM_operatortype_find("MESH_OT_primitive_plane_add", false);
          }
          else if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_GRID) {
            ot = WM_operatortype_find("MESH_OT_primitive_grid_add", false);
          ...
          if (ot != NULL) {
            ...
            RNA_float_set_array(&op_props, "location", location);
            RNA_float_set_array(&op_props, "scale", scale);
            if(ipd->primitive_type == PLACE_PRIMITIVE_TYPE_HEXAGON
                || ipd->primitive_type == PLACE_PRIMITIVE_TYPE_PYRAMID)
              RNA_int_set(&op_props, "vertices", 6);
            ...
            if (ELEM(
              ...
              RNA_int_set(&op_props, "vertices", 60);
            }
  ...
  else if (ipd->step_index == STEP_EXTRA) {
    if (ELEM(event->type, ipd->launch_event, LEFTMOUSE)) {
      if (event->val == KM_PRESS) {
        ...
        mid_v3_v3v3v3v3(center, UNPACK4((&bounds.vec[4])));
        calc_pbox(ipd, second_bounds, center);
        float factor = min_ff(1.0, len_v3v3(second_bounds[0], second_bounds[1]) / len_v3v3(bounds.vec[0], bounds.vec[1]));
        ...
        for (int i = 0; i < 3; i++) {
          ...
          inner_scale[i] = scale[i] * factor;
        }

        wmOperatorType *ot = NULL;
        PointerRNA op_props;
        if (ipd->primitive_type == PLACE_PRIMITIVE_TYPE_TUBE) {
          ot = WM_operatortype_find("MESH_OT_primitive_tube_add", false);
        }
        ...
        if (ot != NULL) {
          ...
          RNA_float_set_array(&op_props, "location", location);
          RNA_float_set_array(&op_props, "scale", scale);
          RNA_float_set_array(&op_props, "inner_scale", inner_scale);
          WM_operator_name_call_ptr(C, ot, WM_OP_EXEC_DEFAULT, &op_props, event);
          WM_operator_properties_free(&op_props);
        }
        else {
  ...
  if (do_cursor_update) {
    ...
    if (ipd->is_snap_found) {
      closest_to_plane_normalized_v3(
          ipd->step[ipd->step_index].co_dst, ipd->step[ipd->step_index].plane, ipd->snap_co);
    }
    else {
      if (view3d_win_to_3d_on_plane_maybe_fallback(
              region,
              ipd->step[ipd->step_index].plane,
              mval_fl,
              ipd->step[ipd->step_index].is_degenerate_view_align ? ipd->view_plane : NULL,
              ipd->step[ipd->step_index].co_dst)) {
        /* pass */
      }

      if (ipd->use_snap && (ipd->snap_to == PLACE_SNAP_TO_DEFAULT)) {
        if (idp_snap_calc_incremental(
                ipd->scene, ipd->v3d, ipd->region, ipd->co_src, ipd->step[ipd->step_index].co_dst)) {
        }
      }
    }
...
void VIEW3D_OT_interactive_add(struct wmOperatorType *ot)
{
  ...
  static const EnumPropertyItem primitive_type[] = {
      ...
      {PLACE_PRIMITIVE_TYPE_TORUS, "TORUS", 0, "Torus", ""},
      {PLACE_PRIMITIVE_TYPE_TUBE, "TUBE", 0, "Tube", ""},
      {PLACE_PRIMITIVE_TYPE_HEXAGON, "HEXAGON", 0, "Hexagon", ""},
      {PLACE_PRIMITIVE_TYPE_PYRAMID, "PYRAMID", 0, "Pyramid", ""},
      {PLACE_PRIMITIVE_TYPE_PLANE, "PLANE", 0, "Plane", ""},
      {PLACE_PRIMITIVE_TYPE_GRID, "GRID", 0, "Grid", ""},
      {0, NULL, 0, NULL, NULL},
  };

 ...
 */
static ixam::Vector<Base *> do_pose_tag_select_op_prepare(ViewContext *vc)
{
  ixam::Vector<Base *> bases;
...
static bool do_pose_tag_select_op_exec(ixam::MutableSpan<Base *> bases, const eSelectOp sel_op)
{
...
static bool do_lasso_select_pose(ViewContext *vc,
                                 ...
                                 const eSelectOp sel_op)
{
  ixam::Vector<Base *> bases = do_pose_tag_select_op_prepare(vc);
...
struct LassoSelectUserData_ForMeshVert {
  ...
  ixam::MutableSpan<bool> select_vert;
};
...
void VIEW3D_OT_bone_select_menu(wmOperatorType *ot)
{
  ...
  /* identifiers */
  ot->name = "Select Menu";
  ot->description = "Menu joint selection";
  ...
  prop = RNA_def_enum(ot->srna, "name", DummyRNA_NULL_items, 0, "Joint Name", "");
  RNA_def_enum_funcs(prop, object_select_menu_enum_itemf);
  RNA_def_property_flag(prop, (PropertyFlag)(PROP_HIDDEN | PROP_ENUM_NO_TRANSLATE));
  ot->prop = prop;
                                   ...
                                   const SelectPick_Params *params)
{
  ...
  GSet *added_bones = BLI_gset_ptr_new("Joint mouse select menu");

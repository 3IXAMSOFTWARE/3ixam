...
static void sequencer_ixam_read_data(IxamDataReader *UNUSED(reader), SpaceLink *sl)
{
  SpaceSeq *sseq = (SpaceSeq *)sl;
   ...
   */
    ...
    if (sseq->gpd) {
      ...
      BKE_gpencil_ixam_read_data(fd, sseq->gpd);
    }
#endif
  sseq->scopes.reference_ibuf = NULL;
...
static void sequencer_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceSeq *sseq = (SpaceSeq *)sl;
...
static void sequencer_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
...
void ED_spacetype_sequencer(void)
{
  ...
  st->ixam_read_data = sequencer_ixam_read_data;
  st->ixam_read_lib = sequencer_ixam_read_lib;
  st->ixam_write = sequencer_ixam_write;

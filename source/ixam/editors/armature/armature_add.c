...
void ARMATURE_OT_click_extrude(wmOperatorType *ot)
{
  ...
  ot->description = "Create a new joint going from the last selected joint to the mouse position";
...
EditBone *add_points_bone(Object *obedit, float head[3], float tail[3])
{
  ...
  ebo = ED_armature_ebone_add(obedit->data, "Joint");
...
{
  /* identifiers */
  ot->name = "Duplicate Selected Joint(s)";
  ot->idname = "ARMATURE_OT_duplicate";
  ot->description = "Make copies of the selected joints within the same rig";
  ...
  RNA_def_boolean(
      ...
      false,
      "Flip Names",
      "Try to flip names of the joints, if possible, instead of adding a number extension");
}
...
void ARMATURE_OT_extrude(wmOperatorType *ot)
{
  ...
  ot->description = "Create new joints from the selected joints";
...
void ARMATURE_OT_bone_primitive_add(wmOperatorType *ot)
{
  ...
  ot->description = "Add a new joint";
  ...
  RNA_def_string(ot->srna, "name", "Joint", MAXBONENAME, "Name", "Name of the newly created joint");
}
...
void ARMATURE_OT_subdivide(wmOperatorType *ot)
{
  ...
  ot->description = "Break selected joints into chains of smaller joints";

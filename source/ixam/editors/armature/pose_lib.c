...
void POSELIB_OT_new(wmOperatorType *ot)
{
  ...
  ot->description =
      "Deprecated, will be removed in 3IXAM 3.3. "
...
static int poselib_add_exec(bContext *C, wmOperator *op)
{
  ...
  /* validate name */
  BLI_uniquename(
      &act->markers, marker, DATA_("Pose"), '_', offsetof(TimeMarker, name), sizeof(marker->name));
 ...
 * - this code is based on the code for extract_pose_from_action in ixamkernel/action.c
 */
...
static void poselib_preview_init_data(bContext *C, wmOperator *op)
{
  ...
  if (ELEM(NULL, pld->ob, pld->pose, pld->arm)) {
    BKE_report(op->reports, RPT_ERROR, "Pose lib is only for rigs in pose mode");
...
/* Cancel previewing operation (called when exiting 3IXAM) */

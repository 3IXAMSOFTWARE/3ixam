...
static const EnumPropertyItem prop_channels_types[] = {
    {PS_TFM_ALL,
     ...
     0,
     "All Properties",
     "All properties, including transforms, bendy joint shape, and custom properties"},
    {PS_TFM_LOC, "LOC", 0, "Location", "Location only"},
    {PS_TFM_ROT, "ROT", 0, "Rotation", "Rotation only"},
    {PS_TFM_SIZE, "SIZE", 0, "Scale", "Scale only"},
    {PS_TFM_BBONE_SHAPE, "BBONE", 0, "Bendy Joint", "Bendy Joint shape properties"},
...
static void pose_slide_draw_status(bContext *C, tPoseSlideOp *pso)
{
  ...
  switch (pso->channels) {
    ...
    case PS_TFM_BBONE_SHAPE:
      STRNCPY(limits_str, TIP_("G/R/S/[B]/C - Bendy Joint properties only (B to clear) | %s"));
      break;
    case PS_TFM_PROPS:
  ...
  STRNCPY(bone_vis_str, TIP_("[H] - Toggle joint visibility"));

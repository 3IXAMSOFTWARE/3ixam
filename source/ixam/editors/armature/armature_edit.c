...
static const EnumPropertyItem prop_calc_roll_types[] = {
    ...
    {0, "", 0, N_("Other"), ""},
    {CALC_ROLL_ACTIVE, "ACTIVE", 0, "Active Joint", ""},
  ...
  for (uint ob_index = 0; ob_index < objects_len; ob_index++) {
    ...
    if (type == CALC_ROLL_CURSOR) { /* Cursor */
      ...
      if (type == CALC_ROLL_VIEW) { /* View */
        ...
        if (ebone == NULL) {
          BKE_report(op->reports, RPT_ERROR, "No active joint set");
          ret = OPERATOR_CANCELLED;
          goto cleanup;
        }
...
void ARMATURE_OT_calculate_roll(wmOperatorType *ot)
{
  ...
  ot->description = "Automatically fix alignment of select joints' axes";
...
void ARMATURE_OT_roll_clear(wmOperatorType *ot)
{
  ...
  ot->description = "Clear roll for selected joints";
...
static int armature_fill_bones_exec(bContext *C, wmOperator *op)
{
  ...
  if (mixed_object_error) {
    BKE_report(op->reports, RPT_ERROR, "Joints for different objects selected");
  ...
  if (count == 1) {
    ...
    if (((ebp_a->head_owner == ebp_b->tail_owner) && (ebp_a->head_owner != NULL)) ||
      ...
      BKE_report(op->reports, RPT_ERROR, "Same joint selected...");
      BLI_freelistN(&points);
      return OPERATOR_CANCELLED;
    }
...
void ARMATURE_OT_fill(wmOperatorType *ot)
{
  ...
  ot->description = "Add joint between selected joint(s) and/or 3D cursor";
...
void ARMATURE_OT_switch_direction(wmOperatorType *ot)
{
  ...
  ot->description = "Change the direction that a chain of joints points in (head and tail swap)";
...
static int armature_align_bones_exec(bContext *C, wmOperator *op)
{
  ...
  /* there must be an active joint */
  if (actbone == NULL) {
    BKE_report(op->reports, RPT_ERROR, "Operation requires an active joint");
    return OPERATOR_CANCELLED;
  }
  ...
  if (num_selected_bones <= 1) {
    ...
    if (actbone->parent) {
      ...
      BKE_reportf(op->reports, RPT_INFO, "Aligned joint '%s' to parent", actbone->name);
    }
  }
  else {
    ...
    BKE_reportf(
        op->reports, RPT_INFO, "%d joints aligned to joint '%s'", num_selected_bones, actbone->name);
  }
...
{
  /* identifiers */
  ot->name = "Align Joints";
  ot->idname = "ARMATURE_OT_align";
  ot->description = "Align selected joints to the active joint (or to their parent)";
...
void ARMATURE_OT_split(wmOperatorType *ot)
{
  ...
  ot->description = "Split off selected joints from connected unselected joints";
...
{
  /* identifiers */
  ot->name = "Delete Selected Joint(s)";
  ot->idname = "ARMATURE_OT_delete";
  ot->description = "Remove selected joints from the rig";
...
{
  /* identifiers */
  ot->name = "Dissolve Selected Joint(s)";
  ot->idname = "ARMATURE_OT_dissolve";
  ot->description = "Dissolve selected joints from the rig";
...
void ARMATURE_OT_hide(wmOperatorType *ot)
{
  ...
  ot->description = "Tag selected joints to not be visible in Edit Mode";
...
void ARMATURE_OT_reveal(wmOperatorType *ot)
{
  ...
  ot->description = "Reveal all joints hidden in Edit Mode";

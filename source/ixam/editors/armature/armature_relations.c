...
int ED_armature_join_objects_exec(bContext *C, wmOperator *op)
{
  ...
  if (ok == false) {
    BKE_report(op->reports, RPT_WARNING, "Active object is not a selected rig");
    return OPERATOR_CANCELLED;
  }
...
static int separate_armature_exec(bContext *C, wmOperator *op)
{
  ...
  if (ok) {
    BKE_report(op->reports, RPT_INFO, "Separated joints");
...
{
  /* identifiers */
  ot->name = "Separate Joints";
  ot->idname = "ARMATURE_OT_separate";
  ot->description = "Isolate selected joints into a separate rig";
...
static const EnumPropertyItem prop_editarm_make_parent_types[] = {
    ...
    {ARM_PAR_USER_PROMPT, "USER_PROMPT", 0, "Connect...", ""},
    {0, NULL, 0, NULL, NULL},
};
...
static int armature_parent_set_exec(bContext *C, wmOperator *op)
{
  ...
  if (actbone == NULL) {
    ...
    RNA_enum_set(op->ptr, "type", ARM_PAR_USER_PROMPT); // reset default
    return OPERATOR_CANCELLED;
  }
                                      ...
                                      const wmEvent *UNUSED(event))
{
  ...
  // if type is not provided, then ask for user input
  if (val == ARM_PAR_USER_PROMPT) {
  ...
  // if type is provided, then check correctness and execute
  bool enabled = 
      (val == ARM_PAR_CONNECT && enable_connect) || 
      (val == ARM_PAR_OFFSET && enable_offset);
...
void ARMATURE_OT_parent_set(wmOperatorType *ot)
{
  ...
  ot->description = "Set the active joint as the parent of the selected joints";
...
static const EnumPropertyItem prop_editarm_clear_parent_types[] = {
    ...
    {ARM_PAR_CLEAR_DISCONNECT, "DISCONNECT", 0, "Disconnect Joint", ""},
    {ARM_PAR_CLEAR_USER_PROMPT, "USER_PROMPT", 0, "Disconnect...", ""},
    {0, NULL, 0, NULL, NULL},
};
...
static int armature_parent_clear_exec(bContext *C, wmOperator *op)
{
  ...
  RNA_enum_set(op->ptr, "type", ARM_PAR_CLEAR_USER_PROMPT); // reset default
  return OPERATOR_FINISHED;
}
                                        ...
                                        const wmEvent *UNUSED(event))
{
  ...
  // if type is not provided, then ask for user input
  if (val == ARM_PAR_CLEAR_USER_PROMPT) {
    uiPopupMenu *pup = UI_popup_menu_begin(
  ...
  // if type is provided, then check correctness and execute
  bool enabled = 
      (val == ARM_PAR_CLEAR && enable_clear) || 
      (val == ARM_PAR_CLEAR_DISCONNECT && enable_disconnect);
...
void ARMATURE_OT_parent_clear(wmOperatorType *ot)
{
  ...
  ot->description =
      "Remove the parent-child relationship between selected joints and their parents";

...
static int text_open_exec(bContext *C, wmOperator *op)
{
  ...
  text = BKE_text_load_ex(bmain, str, BKE_main_ixamfile_path(bmain), internal);
...
static int text_open_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  Main *bmain = CTX_data_main(C);
  Text *text = CTX_data_edit_text(C);
  const char *path = (text && text->filepath) ? text->filepath : BKE_main_ixamfile_path(bmain);
...
static void txt_write_file(Main *bmain, Text *text, ReportList *reports)
{
  ...
  BLI_path_abs(filepath, BKE_main_ixamfile_path(bmain));
...
static int text_save_as_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
  ...
  if (text->filepath) {
    ...
    str = BKE_main_ixamfile_path(bmain);
  }
    ...
    case 1:
      if (text->flags & TXT_ISDIRTY) {
        ...
        pup = UI_popup_menu_begin(
            C, IFACE_("File Modified Outside and Inside 3IXAM"), ICON_NONE);
        ...
        pup = UI_popup_menu_begin(C, IFACE_("File Modified Outside 3IXAM"), ICON_NONE);
        layout = UI_popup_menu_layout(pup);
        uiItemEnumO_ptr(
      ...
      break;
    case 2:
      pup = UI_popup_menu_begin(C, IFACE_("File Deleted Outside 3IXAM"), ICON_NONE);
      layout = UI_popup_menu_layout(pup);
      uiItemEnumO_ptr(

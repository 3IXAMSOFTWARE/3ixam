...
static void text_ixam_read_data(IxamDataReader *UNUSED(reader), SpaceLink *sl)
{
  SpaceText *st = (SpaceText *)sl;
  memset(&st->runtime, 0x0, sizeof(st->runtime));
}

static void text_ixam_read_lib(IxamLibReader *reader, ID *parent_id, SpaceLink *sl)
{
  SpaceText *st = (SpaceText *)sl;
  BLO_read_id_address(reader, parent_id->lib, &st->text);
}

static void text_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
...
void ED_spacetype_text(void)
{
  ...
  st->ixam_read_data = text_ixam_read_data;
  st->ixam_read_lib = text_ixam_read_lib;
  st->ixam_write = text_ixam_write;

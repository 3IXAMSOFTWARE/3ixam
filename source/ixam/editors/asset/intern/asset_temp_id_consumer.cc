...
class AssetTemporaryIDConsumer : NonCopyable, NonMovable {
 ...
 public:
                ...
                ReportList &reports)
  {
    ...
    char ixam_file_path[FILE_MAX_LIBEXTRA];
    ED_asset_handle_get_full_library_path(C, &asset_library_ref, &handle_, ixam_file_path);

    temp_lib_context_ = BLO_library_temp_load_id(
        &bmain, ixam_file_path, id_type, asset_name, &reports);

    if (temp_lib_context_ == nullptr || temp_lib_context_->temp_id == nullptr) {
      BKE_reportf(&reports, RPT_ERROR, "Unable to load %s from %s", asset_name, ixam_file_path);
      return nullptr;
    }

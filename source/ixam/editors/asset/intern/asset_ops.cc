...
using PointerRNAVec = ixam::Vector<PointerRNA>;
...
static int asset_catalog_new_exec(bContext *C, wmOperator *op)
{
  ...
  ixam::bke::AssetCatalog *new_catalog = ED_asset_catalog_add(
...
static bool asset_catalogs_save_poll(bContext *C)
{
  ...
  if (!bmain->filepath[0]) {
    CTX_wm_operator_poll_msg_set(C, "Cannot save asset catalogs before the 3IXAM file is saved");
    return false;
  }
                                       ...
                                       const struct wmEvent * /*event*/)
{
  ...
  /* Make the "Save As" dialog box default to "${ASSET_LIB_ROOT}/${CURRENT_FILE}.ixam". */
...
static void ASSET_OT_bundle_install(struct wmOperatorType *ot)
{
  ...
  ot->description =
      "Copy the current .ixam file into an Asset Library. Only works on standalone .ixam files "
...
static bool could_be_asset_bundle(const Main *bmain)
{
  return fnmatch("*_bundle.ixam", bmain->filepath, FNM_CASEFOLD) == 0;
}
...
static bool set_filepath_for_asset_lib(const Main *bmain, struct wmOperator *op)
{
  ...
  /* Concatenate the filename of the current ixam file. */
  const char *ixam_filename = BLI_path_basename(bmain->filepath);
  if (ixam_filename == nullptr || ixam_filename[0] == '\0') {
    return false;
  }

  char file_path[PATH_MAX];
  BLI_path_join(file_path, sizeof(file_path), lib->path, ixam_filename);
...
/**
 * Do a check on any external files (.ixam, textures, etc.) being used.
 * The ASSET_OT_bundle_install operator only works on standalone .ixam files

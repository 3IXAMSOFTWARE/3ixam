...
namespace ixam::ed::asset::index {

using namespace ixam::io::serialize;
using namespace ixam::bke;
using namespace ixam::bke::idprop;
...
/** Abstract class for #IxamFile and #AssetIndexFile. */
class AbstractFile {
 public:
...
/**
 * \brief Reference to a ixam file that can be indexed.
 */
class IxamFile : public AbstractFile {
  StringRefNull file_path_;

 public:
  IxamFile(StringRefNull file_path) : file_path_(file_path)
  {
  }
...
struct AssetLibraryIndex {
 ...
 public:
   ...
   */
  std::string index_file_path(const IxamFile &asset_file) const
  {
    std::stringstream ss;
...
struct AssetIndex {
  ...
  /**
   * `ixam::io::serialize::Value` representing the contents of an index file.
   *
...
class AssetIndexFile : public AbstractFile {
 public:
  ...
  AssetIndexFile(AssetLibraryIndex &library_index, IxamFile &asset_filename)
   ...
   */
  bool is_older_than(IxamFile &asset_file) const
  {
...
extern "C" {
const FileIndexerType file_indexer_asset = ixam::ed::asset::index::asset_indexer();
}

...
static void file_main_region_draw(const bContext *C, ARegion *region)
{
  ...
  if (params->display == FILE_IMGDISPLAY) {
    ...
    /* XXX this happens on scaling down Screen (like from startup.ixam) */
...
/* NOTE: do not add .ixam file reading on this level */
...
static void file_ixam_read_data(IxamDataReader *reader, SpaceLink *sl)
{
  SpaceFile *sfile = (SpaceFile *)sl;
...
static void file_ixam_read_lib(IxamLibReader *UNUSED(reader),
                                ID *UNUSED(parent_id),
                                SpaceLink *sl)
{
  SpaceFile *sfile = (SpaceFile *)sl;
...
static void file_ixam_write(IxamWriter *writer, SpaceLink *sl)
{
  SpaceFile *sfile = (SpaceFile *)sl;
...
void ED_spacetype_file(void)
{
  ...
  st->ixam_read_data = file_ixam_read_data;
  st->ixam_read_lib = file_ixam_read_lib;
  st->ixam_write = file_ixam_write;
...
void ED_file_read_bookmarks(void)
{
  ...
  if (cfgdir) {
    char name[FILE_MAX];
    BLI_path_join(name, sizeof(name), cfgdir, IXAM_BOOKMARK_FILE);

    ...
    if (file->typeflag & FILE_TYPE_DIR) {
      ...
      /* the path is too long and we are not going up! */
        ...
        if (is_parent_dir) {
          ...
          BLI_path_normalize_dir(BKE_main_ixamfile_path(bmain), params->dir, sizeof(params->dir));
...
/**
 * Local utility to write #IXAM_BOOKMARK_FILE, reporting an error on failure.
 */
                                                          ...
                                                          ReportList *reports)
{
  ...
  const char *cfgdir = BKE_appdir_folder_id_create(IXAM_USER_CONFIG, NULL);
  if (UNLIKELY(!cfgdir)) {
  ...
  BLI_path_join(filepath, sizeof(filepath), cfgdir, IXAM_BOOKMARK_FILE);
...
/* -------------------------------------------------------------------- */
/** \name Reset Recent ixam files Operator
 * \{ */
...
int file_highlight_set(SpaceFile *sfile, ARegion *region, int mx, int my)
{
  ...
  /* In case ixam starts where the mouse is over a File browser,
    ...
    bContext *C, Main *bmain, wmOperator *op, SpaceFile *sfile, char *filepath)
{
  ...
  if ((prop = RNA_struct_find_property(op->ptr, "relative_path"))) {
    if (RNA_property_boolean_get(op->ptr, prop)) {
      BLI_path_rel(filepath, BKE_main_ixamfile_path(bmain));
    }
  }
...
void file_operator_to_sfile(Main *bmain, SpaceFile *sfile, wmOperator *op)
{
  ...
  BLI_path_abs(params->dir, BKE_main_ixamfile_path(bmain));
...
static bool file_execute(bContext *C, SpaceFile *sfile)
{
  ...
  if (file && (file->typeflag & FILE_TYPE_DIR)) {
    ...
    if (FILENAME_IS_PARENT(file->relpath)) {
      ...
      BLI_path_normalize(BKE_main_ixamfile_path(bmain), params->dir);
      BLI_path_append_dir(params->dir, sizeof(params->dir), file->relpath);
    }
    ED_file_change_dir(C);
  }
  ...
  if (params) {
    if (BLI_path_is_rel(params->dir)) {
      ...
      BLI_path_abs(params->dir,
                   (ixamfile_path[0] != '\0') ? ixamfile_path :
...
void file_directory_enter_handle(bContext *C, void *UNUSED(arg_unused), void *UNUSED(arg_but))
{
  ...
  if (params) {
    ...
    if (filelist_is_dir(sfile->files, params->dir)) {
      ...
      /* If we are 'inside' a ixam library, we cannot do anything... */
  ...
  if (params) {
    ...
    if (matches == 1) {
      ...
      if (filelist_is_dir(sfile->files, filepath)) {
        BLI_path_normalize_dir(BKE_main_ixamfile_path(bmain), filepath, sizeof(filepath));
        BLI_strncpy(params->dir, filepath, sizeof(params->dir));
        params->file[0] = '\0';
        ED_file_change_dir(C);
        UI_textbutton_activate_but(C, but);

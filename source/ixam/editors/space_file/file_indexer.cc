...
namespace ixam::ed::file::indexer {
    ...
    const int idcode)
{
  for (const LinkNode *ln = datablock_infos; ln; ln = ln->next) {
    ...
    FileIndexerEntry *file_indexer_entry =
        ixam::ed::file::indexer::file_indexer_entry_create_from_datablock_info(datablock_info,
                                                                                  idcode);
...
const FileIndexerType file_indexer_noop = ixam::ed::file::indexer::default_indexer();
}

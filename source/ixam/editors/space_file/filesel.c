...
static void fileselect_initialize_params_common(SpaceFile *sfile, FileSelectParams *params)
{
  ...
  if (!params->dir[0]) {
    if (ixamfile_path[0] != '\0') {
      BLI_split_dir_part(ixamfile_path, params->dir, sizeof(params->dir));
    }
    else {
...
static void fileselect_ensure_updated_asset_params(SpaceFile *sfile)
{
  ...
  base_params->filter |= FILE_TYPE_IXAMLIB;
...
static FileSelectParams *fileselect_ensure_updated_file_params(SpaceFile *sfile)
{
  ...
  if (!sfile->params) {
    ...
    /* set path to most recently opened .ixam */
    BLI_split_dirfile(ixamfile_path,
                      sfile->params->dir,
                      sfile->params->file,
  ...
  if (op) {
    ...
    if (params->dir[0]) {
      BLI_path_normalize_dir(ixamfile_path, params->dir, sizeof(params->dir));
      BLI_path_abs(params->dir, ixamfile_path);
    }
    ...
    params->filter = 0;
    if ((prop = RNA_struct_find_property(op->ptr, "filter_ixam"))) {
      params->filter |= RNA_property_boolean_get(op->ptr, prop) ? FILE_TYPE_IXAM : 0;
    }
    if ((prop = RNA_struct_find_property(op->ptr, "filter_ixamlib"))) {
      params->filter |= RNA_property_boolean_get(op->ptr, prop) ? FILE_TYPE_IXAMLIB : 0;
    }
    if ((prop = RNA_struct_find_property(op->ptr, "filter_backup"))) {
      params->filter |= RNA_property_boolean_get(op->ptr, prop) ? FILE_TYPE_IXAM_BACKUP : 0;
    }
                                           ...
                                           const char *extension)
{
  if (!RNA_struct_property_is_set_ex(op->ptr, "filepath", false)) {
    ...
    if (ixamfile_path[0] == '\0') {
      ...
      BLI_strncpy(filepath, ixamfile_path, sizeof(filepath));
    }

...
struct FileListInternEntry {
  ...
  /** ID type, in case typeflag has FILE_TYPE_IXAMLIB set. */
  int ixamtype;
  ...
  /* See #FILE_ENTRY_IXAMLIB_NO_PREVIEW. */
  bool ixamlib_has_no_preview;
...
static int compare_tiebreaker(const FileListInternEntry *entry1, const FileListInternEntry *entry2)
{
  ...
  if (entry1->local_data.id && entry2->local_data.id) {
    if (entry1->ixamtype < entry2->ixamtype) {
      return -1;
    }
    if (entry1->ixamtype > entry2->ixamtype) {
      return 1;
                                    ...
                                    const FileListInternEntry *entry2)
{
  ...
  if (entry1->typeflag & FILE_TYPE_DIR) {
    if (entry2->typeflag & FILE_TYPE_DIR) {
       ...
       * then libraries (.ixam files), then categories in libraries. */
      if (entry1->typeflag & FILE_TYPE_IXAMLIB) {
        if (!(entry2->typeflag & FILE_TYPE_IXAMLIB)) {
          return 1;
        }
      }
      else if (entry2->typeflag & FILE_TYPE_IXAMLIB) {
        return -1;
      }
      else if (entry1->typeflag & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP)) {
        if (!(entry2->typeflag & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP))) {
          return 1;
        }
      }
      else if (entry2->typeflag & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP)) {
        return -1;
      }
    }
    else {
...
static int compare_extension(void *user_data, const void *a1, const void *a2)
{
  ...
  if ((entry1->typeflag & FILE_TYPE_IXAMLIB) && !(entry2->typeflag & FILE_TYPE_IXAMLIB)) {
    return -1;
  }
  if (!(entry1->typeflag & FILE_TYPE_IXAMLIB) && (entry2->typeflag & FILE_TYPE_IXAMLIB)) {
    return 1;
  ...
  if ((entry1->typeflag & FILE_TYPE_IXAMLIB) && (entry2->typeflag & FILE_TYPE_IXAMLIB)) {
    ...
    if (!(sufix1 = strstr(entry1->relpath, ".ixam.gz"))) {
      sufix1 = strrchr(entry1->relpath, '.');
    }
    if (!(sufix2 = strstr(entry2->relpath, ".ixam.gz"))) {
                               ...
                               const FileListInternEntry *file)
{
  ...
  if (!(file->typeflag & FILE_TYPE_DIR) && (file->typeflag & FILE_TYPE_IXAMLIB) &&
...
static bool is_filtered_file_type(const FileListInternEntry *file, const FileListFilter *filter)
{
  ...
  if (filter->filter && (filter->flags & FLF_DO_FILTER)) {
    if (file->typeflag & FILE_TYPE_DIR) {
      if (file->typeflag & (FILE_TYPE_IXAMLIB | FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP)) {
        if (!(filter->filter & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP))) {
          return false;
        }
      }
      else {
...
static bool is_filtered_lib_type(FileListInternEntry *file,
                                 ...
                                 FileListFilter *filter)
{
  if (file->typeflag & FILE_TYPE_IXAMLIB) {
    return is_filtered_id_file_type(file, file->ixamtype, file->name, filter);
  }
  return is_filtered_file_type(file, filter);
}
                                ...
                                const char *filter_search)
{
  ...
  const uint64_t new_filter_id = (filter & FILE_TYPE_IXAMLIB) ? filter_id : FILTER_ID_ALL;
                               ...
                               const bool ignore_libdir)
{
  ...
  if ((typeflag & FILE_TYPE_DIR) &&
      !(ignore_libdir && (typeflag & (FILE_TYPE_IXAMLIB | FILE_TYPE_IXAM)))) {
    if (FILENAME_IS_PARENT(file->relpath)) {
    ...
    if (typeflag & FILE_TYPE_IXAM) {
      return ICON_FILE_IXAM;
    }
    if (is_main) {
  ...
  if (typeflag & FILE_TYPE_IXAM) {
    return (is_main || file->preview_icon_id) ? ICON_FILE_IXAM : ICON_IXAM;
  }
  if (typeflag & FILE_TYPE_IXAM_BACKUP) {
  ...
  if (typeflag & FILE_TYPE_IXAMLIB) {
    const int ret = UI_icon_from_idcode(file->ixamtype);
    if (ret != ICON_NONE) {
      return ret;
    }
  }
                                  ...
                                  const bool do_change)
{
  ...
  if (BLI_access(r_dir, R_OK) != 0) {
    if (WM_ask_fs_access(
            IFACE_("Choose Directory"),
            IFACE_("Choose directory for working\n Press \"Choose\" to choose current directory"),
            IFACE_("Choose"),
            r_dir)) {
      return true;
    }
...
static void filelist_cache_preview_runf(TaskPool *__restrict pool, void *taskdata)
{
  ...
  BLI_assert(preview->flags &
             (FILE_TYPE_IMAGE | FILE_TYPE_MOVIE | FILE_TYPE_FTFONT | FILE_TYPE_IXAM |
              FILE_TYPE_IXAM_BACKUP | FILE_TYPE_IXAMLIB));

  if (preview->flags & FILE_TYPE_IMAGE) {
    source = THB_SOURCE_IMAGE;
  }
  else if (preview->flags &
           (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP | FILE_TYPE_IXAMLIB)) {
    source = THB_SOURCE_IXAM;
  }
...
static void filelist_cache_previews_push(FileList *filelist, FileDirEntry *entry, const int index)
{
   ...
   * reload, we'd reopen the .ixam to look for the preview. */
  if ((entry->typeflag & FILE_TYPE_IXAMLIB) &&
      (entry->flags & FILE_ENTRY_IXAMLIB_NO_PREVIEW)) {
    return;
  }
...
void filelist_freelib(struct FileList *filelist)
{
  if (filelist->libfiledata) {
    BLO_ixamhandle_close(filelist->libfiledata);
  }
  filelist->libfiledata = nullptr;
}

IxamHandle *filelist_lib(struct FileList *filelist)
{
                              ...
                              char *buff)
{
  ...
  if (typeflag & FILE_TYPE_FTFONT && !(typeflag & FILE_TYPE_IXAMLIB)) {
  ...
  /* Depending on platforms, 'my_file.ixam/..' might be viewed as dir or not... */
  if (!name) {
    if (typeflag & FILE_TYPE_DIR) {
      name = (char *)relpath;
    }
...
void filelist_setdir(struct FileList *filelist, char *r_dir)
{
  ...
  BLI_path_normalize_dir(BKE_main_ixamfile_path_from_global(), r_dir, FILE_MAX_LIBEXTRA);
...
static FileDirEntry *filelist_file_create_entry(FileList *filelist, const int index)
{
  ...
  if (entry->ixamlib_has_no_preview) {
    ret->flags |= FILE_ENTRY_IXAMLIB_NO_PREVIEW;
  }
...
/* would recognize .ixam as well */
static bool file_is_ixam_backup(const char *str)
{
  ...
  if (a == 0 || b >= a) {
    ...
    /* allow .ixam .ixam2 .ixam32 */
    loc = BLI_strcasestr(str + a - b, ".ixam");
...
int ED_path_extension_type(const char *path)
{
  if (BLO_has_bfile_extension(path)) {
    return FILE_TYPE_IXAM;
  }
  if (file_is_ixam_backup(path)) {
    return FILE_TYPE_IXAM_BACKUP;
  }
#ifdef __APPLE__
    ...
    while (i--) {
      ...
      if (!(entry->typeflag & FILE_TYPE_DIR)) {
        if (do_lib && BLO_has_bfile_extension(target)) {
          /* If we are considering .ixam files as libs, promote them to directory status. */
          entry->typeflag = FILE_TYPE_IXAM;
                                                                   ...
                                                                   const char *group_name)
{
  ...
  entry->typeflag |= FILE_TYPE_IXAMLIB | FILE_TYPE_DIR;
  entry->ixamtype = idcode;
  return entry;
}
                                                    ...
                                                    const char *group_name)
{
  ...
  entry->typeflag |= FILE_TYPE_IXAMLIB;
  if (datablock_info) {
    entry->ixamlib_has_no_preview = datablock_info->no_preview_found;
                                     ...
                                     FileIndexer *indexer_runtime)
{
  ...
  struct IxamHandle *libfiledata = nullptr;
  ...
  /* Indexing returns all entries in a ixam file. We should ignore the index when listing a group
   * inside a ixam file, so the `entries` isn't filled with undesired entries.
  ...
  libfiledata = BLO_ixamhandle_from_file(dir, &bf_reports);
  ...
  if (group_came_from_path) {
    ...
    LinkNode *datablock_infos = BLO_ixamhandle_get_datablock_info(
    ...
    LinkNode *groups = BLO_ixamhandle_get_linkable_groups(libfiledata);
    ...
    for (LinkNode *ln = groups; ln; ln = ln->next) {
      ...
      if (options & LIST_LIB_RECURSIVE) {
        ...
        LinkNode *group_datablock_infos = BLO_ixamhandle_get_datablock_info(
                                                       ...
                                                       FileListInternEntry *entry)
{
  ...
  /* Show entries when recursion is set to `ixam file` even when `current_recursion_level`
   * exceeds `max_recursion`. */
  if (!is_lib && (current_recursion_level >= max_recursion) &&
      ((entry->typeflag & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP)) == 0)) {
    return false;
  }
  if (entry->typeflag & FILE_TYPE_IXAMLIB) {
                                                     ...
                                                     float *progress)
{
  ...
  while (!BLI_stack_is_empty(todo_dirs) && !(*stop)) {
     ...
     * name inside .ixam file, which can have slashes and backslashes! See T46827.
                                                   ...
                                                   float * /*progress*/)
{
  ...
  FOREACH_MAIN_ID_BEGIN (job_params->current_main, id_iter) {
    ...
    entry->typeflag |= FILE_TYPE_IXAMLIB | FILE_TYPE_ASSET;
    entry->ixamtype = GS(id_iter->name);
    entry->uid = filelist_uid_generate(filelist);
...
static bool filelist_contains_main(const FileList *filelist, const Main *bmain)
{
  ...
  return ixamfile_path[0] && BLI_path_contains(filelist->filelist.root, ixamfile_path);
}
...
void filelist_readjob_start(FileList *filelist, const int space_notifier, const bContext *C)
{
  ...
  BLI_strncpy(flrj->main_name, BKE_main_ixamfile_path(bmain), sizeof(flrj->main_name));

                           ...
                           bool dimmed)
{
  ...
  if (drag) {
    ...
    if ((id = filelist_file_get_id(file))) {
      ...
      ImBuf *preview_image = filelist_file_getimage(file);
      char ixam_path[FILE_MAX_LIBEXTRA];
      if (BLO_library_path_explode(path, ixam_path, NULL, NULL)) {
        const FileAssetSelectParams *asset_params = ED_fileselect_get_asset_params(sfile);
        BLI_assert(asset_params != NULL);

        UI_but_drag_set_asset(but,
                              &(AssetHandle){.file_data = file},
                              BLI_strdup(ixam_path),
                              file->asset_data,
                              ...
                              const bool is_link)
{
  ...
  bool show_outline = !is_icon &&
                      (file->typeflag & (FILE_TYPE_IMAGE | FILE_TYPE_MOVIE | FILE_TYPE_IXAM));
  ...
  if (!is_icon && file->typeflag & FILE_TYPE_IXAMLIB) {
    ...
    const bool update_stat_strings)
{
  switch (column) {
    case COLUMN_DATETIME:
      if (!(file->typeflag & FILE_TYPE_IXAMLIB) && !FILENAME_IS_CURRPAR(file->relpath)) {
      ...
      break;
    case COLUMN_SIZE:
      if ((file->typeflag & (FILE_TYPE_IXAM | FILE_TYPE_IXAM_BACKUP)) ||
          !(file->typeflag & (FILE_TYPE_DIR | FILE_TYPE_IXAMLIB))) {
                                           ...
                                           ARegion *region)
{
...

  {
    ...
    const char *suggestion = TIP_(
        "Asset Libraries are local directories that can contain .ixam files with assets inside.\n"

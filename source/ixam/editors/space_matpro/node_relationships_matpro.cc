...
static void pick_link(
    wmOperator &op, bNodeLinkDrag &nldrag, SpaceMatPro &smatpro, bNode *node, bNodeLink &link_to_pick)
{
  clear_picking_highlight(&smatpro.edittree->links);
  ...
  nodeRemLink(smatpro.edittree, &link_to_pick);
  smatpro.edittree->ensure_topology_cache();
                                              ...
                                              const float2 &cursor)
{
  ...
  bNode *node;
  bNodeSocket *socket;
  node_find_indicated_socket(*smatpro, &node, &socket, drag_start, SOCK_IN);
  ...
  clear_picking_highlight(&smatpro->edittree->links);
  LISTBASE_FOREACH (bNodeLink *, link, &smatpro->edittree->links) {
  ...
  if (link_to_pick) {
    ...
    if (!node_find_indicated_socket(*smatpro, &node, &socket, cursor, SOCK_IN)) {
      pick_link(op, nldrag, *smatpro, node, *link_to_pick);
    }
  }
}
...
static bool smatpro_autoconnect_input(SpaceMatPro &smatpro,
                                    bNode *node_fr,
                                    bNodeSocket *sock_fr,
                                    bNode *node_to,
                                    bNodeSocket *sock_to,
...
static void smatpro_autoconnect(SpaceMatPro &smatpro, const bool allow_multiple, const bool replace)
{
  bNodeTree *ntree = smatpro.edittree;
  ...
  for (const int i : sorted_nodes.as_mutable_span().drop_back(1).index_range()) {
    ...
    LISTBASE_FOREACH (bNodeSocket *, sock_to, &node_to->inputs) {
      if (sock_to->flag & SELECT) {
        ...
        if (smatpro_autoconnect_input(smatpro, node_fr, sock_fr, node_to, sock_to, replace)) {
          numlinks++;
        }
      }
    }
...
{
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  return ED_matpro_is_compositor(smatpro) ? CMP_NODE_VIEWER : GEO_NODE_VIEWER;
}
                                 ...
                                 bNodeLink &viewer_link)
{
  ...
  remove_links_to_unavailable_viewer_sockets(*smatpro.edittree, viewer_node);
  ...
  if (smatpro.edittree->type == NTREE_GEOMETRY) {
    viewer_path::activate_geometry_node(*bmain, smatpro, viewer_node);
  }
                       ...
                       bNodeSocket &bsocket_to_view)
{
  ...
  for (bNodeLink *link : bsocket_to_view.directly_linked_links()) {
    ...
    if (is_viewer_socket(target_socket) && ELEM(viewer_node, nullptr, &target_node)) {
      finalize_viewer_link(C, smatpro, target_node, *link);
  ...
  finalize_viewer_link(C, smatpro, *viewer_node, *viewer_link);
  return OPERATOR_CANCELLED;
}
...
{
  SpaceMatPro &smatpro = *CTX_wm_space_matpro(&C);
  bNodeTree *btree = smatpro.edittree;
  ...
  return view_socket(C, smatpro, *btree, bnode_to_view, *bsocket_to_view);
}
...
static bool node_active_link_viewer_poll(bContext *C)
{
  ...
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  return ED_matpro_is_compositor(smatpro) || ED_matpro_is_geometry(smatpro);
}

void MATPRO_OT_link_viewer(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Link to Viewer Node";
  ot->description = "Link to viewer node";
  ot->idname = "MATPRO_OT_link_viewer";
...
static void node_remove_extra_links(SpaceMatPro &smatpro, bNodeLink &link)
{
  bNodeTree &ntree = *smatpro.edittree;
...
static void node_link_exit(bContext &C, wmOperator &op, const bool apply_links)
{
  ...
  for (bNodeLink *link : nldrag->links) {
    ...
    if (apply_links && link->tosock && link->fromsock) {
      ...
      node_remove_extra_links(smatpro, *link);
    }
    else {
  ...
  ED_matpro_tree_propagate_change(&C, bmain, &ntree);
...
static void node_link_find_socket(bContext &C, wmOperator &op, const float2 &cursor)
{
  ...
  if (nldrag->in_out == SOCK_OUT) {
    ...
    smatpro.edittree->ensure_topology_cache();
    if (node_find_indicated_socket(smatpro, &tnode, &tsock, cursor, SOCK_IN)) {
      for (bNodeLink *link : nldrag->links) {
        ...
        LISTBASE_FOREACH (bNodeLink *, existing_link, &smatpro.edittree->links) {
    ...
    bNode *tnode;
    bNodeSocket *tsock = nullptr;
    if (node_find_indicated_socket(smatpro, &tnode, &tsock, cursor, SOCK_OUT)) {
      for (bNodeLink *link : nldrag->links) {
...
static int node_link_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  switch (event->type) {
    case MOUSEMOVE:
      ...
      if (should_create_drag_link_search_menu(*smatpro.edittree, *nldrag)) {
...
static std::unique_ptr<bNodeLinkDrag> node_link_init(SpaceMatPro &smatpro,
                                                     float2 cursor,
                                                     const bool detach)
{
  /* output indicated? */
  bNode *node;
  bNodeSocket *sock;
  if (node_find_indicated_socket(smatpro, &node, &sock, cursor, SOCK_OUT)) {
    std::unique_ptr<bNodeLinkDrag> nldrag = std::make_unique<bNodeLinkDrag>();
    nldrag->start_node = node;
    nldrag->start_socket = sock;
    nldrag->start_link_count = nodeCountSocketLinks(smatpro.edittree, sock);
    ...
    if (nldrag->start_link_count > 0 && (nldrag->start_link_count >= link_limit || detach)) {
      ...
      LISTBASE_FOREACH_MUTABLE (bNodeLink *, link, &smatpro.edittree->links) {
        if (link->fromsock == sock) {
          ...
          *oplink = *link;
          ...
          nodeRemLink(smatpro.edittree, link);
        }
      }
    }
    else {
  ...
  /* or an input? */
  if (node_find_indicated_socket(smatpro, &node, &sock, cursor, SOCK_IN)) {
    std::unique_ptr<bNodeLinkDrag> nldrag = std::make_unique<bNodeLinkDrag>();
...
static int node_link_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  std::unique_ptr<bNodeLinkDrag> nldrag = node_link_init(smatpro, cursor, detach);
  ...
  if (nldrag) {
    ...
    smatpro.runtime->linkdrag = std::move(nldrag);
    op->customdata = smatpro.runtime->linkdrag.get();
...
void MATPRO_OT_link(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Link Nodes";
  ot->idname = "MATPRO_OT_link";
  ...
  UI_view2d_edge_pan_operator_properties_ex(ot,
                                            MATPRO_EDGE_PAN_INSIDE_PAD,
                                            MATPRO_EDGE_PAN_OUTSIDE_PAD,
                                            MATPRO_EDGE_PAN_SPEED_RAMP,
                                            MATPRO_EDGE_PAN_MAX_SPEED,
                                            MATPRO_EDGE_PAN_DELAY,
                                            MATPRO_EDGE_PAN_ZOOM_INFLUENCE);
}
...
static int node_make_link_exec(bContext *C, wmOperator *op)
{
  ...
  smatpro_autoconnect(smatpro, true, replace);

  /* deselect sockets after linking */
  node_deselect_all_input_sockets(smatpro, false);
  node_deselect_all_output_sockets(smatpro, false);
...
void MATPRO_OT_link_make(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Make Links";
  ot->description = "Makes a link between selected output in input sockets";
  ot->idname = "MATPRO_OT_link_make";
...
static int cut_links_exec(bContext *C, wmOperator *op)
{
  ...
  bNodeTree &node_tree = *smatpro.edittree;
...
void MATPRO_OT_links_cut(wmOperatorType *ot)
{
  ot->name = "Cut Links";
  ot->idname = "MATPRO_OT_links_cut";
...
void MATPRO_OT_links_mute(wmOperatorType *ot)
{
  ot->name = "Mute Links";
  ot->idname = "MATPRO_OT_links_mute";
...
void MATPRO_OT_links_detach(wmOperatorType *ot)
{
  ot->name = "Detach Links";
  ot->idname = "MATPRO_OT_links_detach";
  ot->description =
...
void MATPRO_OT_parent_set(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Make Parent";
  ot->description = "Attach selected nodes";
  ot->idname = "MATPRO_OT_parent_set";
...
void MATPRO_OT_join(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Join Nodes";
  ot->description = "Attach selected nodes to a new common frame";
  ot->idname = "MATPRO_OT_join";
...
void MATPRO_OT_attach(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Attach Nodes";
  ot->description = "Attach active node to a frame";
  ot->idname = "MATPRO_OT_attach";
...
void node_insert_on_link_flags(Main &bmain, SpaceMatPro &smatpro)
{
  ...
  if ((smatpro.flag & SNODE_SKIP_INSOFFSET) == 0) {
    BLI_assert(smatpro.runtime->iofsd == nullptr);
...
static int node_insert_offset_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (!smatpro || event->type != TIMER || iofsd == nullptr ||
...
{
  const SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  NodeInsertOfsData *iofsd = smatpro->runtime->iofsd;
  smatpro->runtime->iofsd = nullptr;
  op->customdata = iofsd;
  ...
  BLI_assert((smatpro->flag & SNODE_SKIP_INSOFFSET) == 0);

  iofsd->ntree = smatpro->edittree;
  iofsd->anim_timer = WM_event_add_timer(CTX_wm_manager(C), CTX_wm_window(C), TIMER, 0.02);

  node_link_insert_offset_ntree(
      iofsd, CTX_wm_region(C), event->mval, (smatpro->insert_ofs_dir == SNODE_INSERTOFS_DIR_RIGHT));
...
void MATPRO_OT_insert_offset(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Insert Offset";
  ot->description = "Automatically offset nodes on insertion";
  ot->idname = "MATPRO_OT_insert_offset";

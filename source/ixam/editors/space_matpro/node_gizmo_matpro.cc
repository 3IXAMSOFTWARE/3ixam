...
static void node_gizmo_calc_matrix_space(const SpaceMatPro *smatpro,
                                         const ARegion *region,
                                         float matrix_space[4][4])
{
  unit_m4(matrix_space);
  mul_v3_fl(matrix_space[0], smatpro->zoom);
  mul_v3_fl(matrix_space[1], smatpro->zoom);
  matrix_space[3][0] = (region->winx / 2) + smatpro->xof;
  matrix_space[3][1] = (region->winy / 2) + smatpro->yof;
}

static void node_gizmo_calc_matrix_space_with_image_dims(const SpaceMatPro *smatpro,
                                                         const ARegion *region,
                                                         const float2 &image_dims,
                                                         float matrix_space[4][4])
{
  unit_m4(matrix_space);
  mul_v3_fl(matrix_space[0], smatpro->zoom * image_dims.x);
  mul_v3_fl(matrix_space[1], smatpro->zoom * image_dims.y);
  matrix_space[3][0] = ((region->winx / 2) + smatpro->xof) - ((image_dims.x / 2.0f) * smatpro->zoom);
  matrix_space[3][1] = ((region->winy / 2) + smatpro->yof) - ((image_dims.y / 2.0f) * smatpro->zoom);
}
                                                ...
                                                void *value_p)
{
  ...
  const SpaceMatPro *smatpro = (const SpaceMatPro *)gz_prop->custom_func.user_data;
  matrix[0][0] = smatpro->zoom;
  matrix[1][1] = smatpro->zoom;
  matrix[3][0] = smatpro->xof;
  matrix[3][1] = smatpro->yof;
}
                                                ...
                                                const void *value_p)
{
  ...
  BLI_assert(gz_prop->type->array_length == 16);
  SpaceMatPro *smatpro = (SpaceMatPro *)gz_prop->custom_func.user_data;
  smatpro->zoom = matrix[0][0];
  smatpro->xof = matrix[3][0];
  smatpro->yof = matrix[3][1];
}
...
static bool WIDGETGROUP_node_transform_poll(const bContext *C, wmGizmoGroupType * /*gzgt*/)
{
  ...
  if ((smatpro->flag & SNODE_BACKDRAW) == 0) {
    return false;
  }

  if (smatpro && smatpro->edittree && smatpro->edittree->type == NTREE_COMPOSIT) {
...
#if 0
    PointerRNA nodeptr;
    RNA_pointer_create(smatpro->id, &RNA_SpaceNodeEditor, smatpro, &nodeptr);
...
static void WIDGETGROUP_node_crop_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  node_gizmo_calc_matrix_space(smatpro, region, gz->matrix_space);
}
...
static void WIDGETGROUP_node_crop_refresh(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  if (ibuf) {
    ...
    RNA_pointer_create(
        (ID *)smatpro->edittree, &RNA_CompositorNodeCrop, node, &crop_group->update_data.ptr);
...
static void WIDGETGROUP_node_sbeam_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  node_gizmo_calc_matrix_space_with_image_dims(
      smatpro, region, sbeam_group->state.dims, gz->matrix_space);
}
...
static void WIDGETGROUP_node_sbeam_refresh(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  if (ibuf) {
    ...
    /* Need to set property here for undo. TODO: would prefer to do this in _init. */
    PointerRNA nodeptr;
    RNA_pointer_create((ID *)smatpro->edittree, &RNA_CompositorNodeSunBeams, node, &nodeptr);
...
static void WIDGETGROUP_node_corner_pin_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  ...
  float matrix_space[4][4];
  node_gizmo_calc_matrix_space_with_image_dims(
      smatpro, region, cpin_group->state.dims, matrix_space);
  ...
  if (ibuf) {
    ...
    for (bNodeSocket *sock = (bNodeSocket *)node->inputs.first; sock && i < 4; sock = sock->next) {
      if (sock->type == SOCK_VECTOR) {
        ...
        PointerRNA sockptr;
        RNA_pointer_create((ID *)smatpro->edittree, &RNA_NodeSocket, sock, &sockptr);

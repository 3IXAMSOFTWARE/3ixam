...
bool space_matpro_view_flag(
    bContext &C, SpaceMatPro &smatpro, ARegion &region, const int node_flag, const int smooth_viewtx)
{
  ...
  if (smatpro.edittree) {
    LISTBASE_FOREACH (const bNode *, node, &smatpro.edittree->nodes) {
...
static int node_view_all_exec(bContext *C, wmOperator *op)
{
  ...
  if (space_matpro_view_flag(*C, *smatpro, *region, 0, smooth_viewtx)) {
...
static int node_view_selected_exec(bContext *C, wmOperator *op)
{
  ...
  if (space_matpro_view_flag(*C, *smatpro, *region, NODE_SELECT, smooth_viewtx)) {
    return OPERATOR_FINISHED;
  }
  return OPERATOR_CANCELLED;
}
...
void MATPRO_OT_view_selected(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_view_selected";
...
static int smatpro_bg_viewmove_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  ARegion *region = CTX_wm_region(C);
  ...
  switch (event->type) {
    case MOUSEMOVE:
      ...
      smatpro->xof -= (nvm->mvalo.x - event->mval[0]);
      smatpro->yof -= (nvm->mvalo.y - event->mval[1]);
      nvm->mvalo.x = event->mval[0];
      nvm->mvalo.y = event->mval[1];

      /* prevent dragging image outside of the window and losing it! */
      CLAMP(smatpro->xof, nvm->xmin, nvm->xmax);
      CLAMP(smatpro->yof, nvm->ymin, nvm->ymax);

      ED_region_tag_redraw(region);
      WM_main_add_notifier(NC_NODE | ND_DISPLAY, nullptr);
      WM_main_add_notifier(NC_SPACE | ND_SPACE_MATPRO_VIEW, nullptr);
    ...
    case EVT_ESCKEY:
    case RIGHTMOUSE:
      smatpro->xof = nvm->xof_orig;
      smatpro->yof = nvm->yof_orig;
      ED_region_tag_redraw(region);
      WM_main_add_notifier(NC_NODE | ND_DISPLAY, nullptr);
      WM_main_add_notifier(NC_SPACE | ND_SPACE_MATPRO_VIEW, nullptr);
...
static int smatpro_bg_viewmove_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  nvm->xmin = -(region->winx / 2) - (ibuf->x * (0.5f * smatpro->zoom)) + pad;
  nvm->xmax = (region->winx / 2) + (ibuf->x * (0.5f * smatpro->zoom)) - pad;
  nvm->ymin = -(region->winy / 2) - (ibuf->y * (0.5f * smatpro->zoom)) + pad;
  nvm->ymax = (region->winy / 2) + (ibuf->y * (0.5f * smatpro->zoom)) - pad;

  nvm->xof_orig = smatpro->xof;
  nvm->yof_orig = smatpro->yof;
...
static void smatpro_bg_viewmove_cancel(bContext * /*C*/, wmOperator *op)
{
...
void MATPRO_OT_backimage_move(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_backimage_move";

  /* api callbacks */
  ot->invoke = smatpro_bg_viewmove_invoke;
  ot->modal = smatpro_bg_viewmove_modal;
  ot->poll = composite_node_active;
  ot->cancel = smatpro_bg_viewmove_cancel;
...
void MATPRO_OT_backimage_zoom(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_backimage_zoom";
...
static int backimage_fit_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  facx = 1.0f * (region->sizex - pad) / (ibuf->x * smatpro->zoom);
  facy = 1.0f * (region->sizey - pad) / (ibuf->y * smatpro->zoom);

  BKE_image_release_ibuf(ima, ibuf, lock);

  smatpro->zoom *= min_ff(facx, facy) * U.dpi_fac;
...
void MATPRO_OT_backimage_fit(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_backimage_fit";
...
bool ED_space_matpro_get_position(
    Main *bmain, SpaceMatPro *smatpro, ARegion *region, const int mval[2], float fpos[2])
{
  if (!ED_matpro_is_compositor(smatpro) || (smatpro->flag & SNODE_BACKDRAW) == 0) {
    return false;
  }
  ...
  float bufx = ibuf->x * smatpro->zoom;
  float bufy = ibuf->y * smatpro->zoom;
  fpos[0] = (bufx > 0.0f ? (float(mval[0]) - 0.5f * region->winx - smatpro->xof) / bufx + 0.5f :
                           0.0f);
  fpos[1] = (bufy > 0.0f ? (float(mval[1]) - 0.5f * region->winy - smatpro->yof) / bufy + 0.5f :
                           0.0f);
...
bool ED_space_matpro_color_sample(
    Main *bmain, SpaceMatPro *smatpro, ARegion *region, const int mval[2], float r_col[3])
{
  void *lock;
  Image *ima;
  ImBuf *ibuf;
  ...
  /* map the mouse coords to the backdrop image space */
  bufx = ibuf->x * smatpro->zoom;
  bufy = ibuf->y * smatpro->zoom;
  fx = (bufx > 0.0f ? (float(mval[0]) - 0.5f * region->winx - smatpro->xof) / bufx + 0.5f : 0.0f);
  fy = (bufy > 0.0f ? (float(mval[1]) - 0.5f * region->winy - smatpro->yof) / bufy + 0.5f : 0.0f);
...
static void sample_apply(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  bufx = ibuf->x * smatpro->zoom;
  bufy = ibuf->y * smatpro->zoom;
  fx = (bufx > 0.0f ? (float(event->mval[0]) - 0.5f * region->winx - smatpro->xof) / bufx + 0.5f :
                      0.0f);
  fy = (bufy > 0.0f ? (float(event->mval[1]) - 0.5f * region->winy - smatpro->yof) / bufy + 0.5f :
                      0.0f);
  ...
  if (fx >= 0.0f && fy >= 0.0f && fx < 1.0f && fy < 1.0f) {
    ...
    ED_matpro_sample_set(info->colf);
  }
  else {
    info->draw = 0;
    ED_matpro_sample_set(nullptr);
  }
...
static int sample_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (!ED_matpro_is_compositor(smatpro) || !(smatpro->flag & SNODE_BACKDRAW)) {
    return OPERATOR_CANCELLED;
  }
...
void MATPRO_OT_backimage_sample(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_backimage_sample";

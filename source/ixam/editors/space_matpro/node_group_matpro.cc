...
static bool node_group_operator_active_poll(bContext *C)
{
  if (ED_operator_matpro_active(C)) {
     ...
     * with same key-map. */
    if (STR_ELEM(smatpro->tree_idname,
                 "ShaderNodeTree",
                 "CompositorNodeTree",
                 "TextureNodeTree",
                 "GeometryNodeTree")) {
...
static bool node_group_operator_editable(bContext *C)
{
  if (ED_operator_matpro_editable(C)) {
     ...
     * Disabled otherwise to allow python-nodes define their own operators
     * with same key-map. */
    if (ED_matpro_is_shader(smatpro) || ED_matpro_is_compositor(smatpro) || ED_matpro_is_texture(smatpro) ||
        ED_matpro_is_geometry(smatpro)) {
      return true;
    }
  }
  return false;
...
const char *node_group_idname(bContext *C)
{
  ...
  if (ED_matpro_is_texture(smatpro)) {
    return ntreeType_Texture->group_idname;
  }
  if (ED_matpro_is_geometry(smatpro)) {
...
static int node_group_edit_exec(bContext *C, wmOperator *op)
{
  ...
  if (gnode && !exit) {
    ...
    if (ngroup) {
      ED_matpro_tree_push(smatpro, ngroup, gnode);
    }
  }
  else {
    ED_matpro_tree_pop(smatpro);
  }
...
void MATPRO_OT_group_edit(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_group_edit";
...
static int node_group_ungroup_exec(bContext *C, wmOperator *op)
{
  ...
  if (gnode->id && node_group_ungroup(bmain, smatpro->edittree, gnode)) {
    ED_matpro_tree_propagate_change(C, CTX_data_main(C), nullptr);
  }
  else {
...
void MATPRO_OT_group_ungroup(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_group_ungroup";
    ...
    Main &bmain, bNodeTree &ntree, bNodeTree &ngroup, const float2 &offset, const bool make_copy)
{
  ...
  LISTBASE_FOREACH_MUTABLE (bNode *, node, &ngroup.nodes) {
    ...
    if (make_copy) {
      /* make a copy */
      newnode = ixam::bke::node_copy_with_mapping(
...
static int node_group_separate_exec(bContext *C, wmOperator *op)
{
  ...
  bNodeTree *ngroup = smatpro->edittree;
  bNodeTree *nparent = ED_matpro_tree_get(smatpro, 1);
  if (!nparent) {
    BKE_report(op->reports, RPT_WARNING, "Not inside node group");
    return OPERATOR_CANCELLED;
  }
  /* get node tree offset */
  const float2 offset = space_matpro_group_offset(*smatpro);
...
static int node_group_separate_invoke(bContext *C, wmOperator * /*op*/, const wmEvent * /*event*/)
{
  ...
  uiItemEnumO(layout, "MATPRO_OT_group_separate", nullptr, 0, "type", NODE_GS_COPY);
  uiItemEnumO(layout, "MATPRO_OT_group_separate", nullptr, 0, "type", NODE_GS_MOVE);
...
void MATPRO_OT_group_separate(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_group_separate";
...
void MATPRO_OT_group_make(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_group_make";
...
void MATPRO_OT_group_insert(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_group_insert";

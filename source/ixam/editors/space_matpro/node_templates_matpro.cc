...
static void ui_node_menu_column(NodeLinkArg *arg, int nclass, const char *cname)
{
  ...
  ixam::Vector<bNodeType *> sorted_ntypes;
  ...
  for (int j = 0; j < sorted_ntypes.size(); j++) {
    ...
    std::optional<ixam::nodes::NodeDeclaration> node_decl;
    ...
    for (const NodeLinkItem &item : items) {
      ...
      if (first) {
        ...
        uiItemL(column, IFACE_(cname), ICON_NODE, UI_BSHAPE_ROUNDBOX);
...
static void ui_template_node_link_menu(bContext *C, uiLayout *layout, void *but_p)
{
  ...
  if (sock->link) {
    uiItemL(column, IFACE_("Link"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    but = (uiBut *)block->buttons.last;
    ...
    uiLayout &layout, bContext &C, bNodeTree &ntree, bNode &node, bNodeSocket &input, int depth)
{
...

  {
    ...
    uiItemL(sub, IFACE_(input.name), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }

  if (dependency_loop) {
    uiItemL(row, IFACE_("Dependency Loop"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
    ...
    /* input not linked, show value */
      ...
      switch (input.type) {
        ...
        case SOCK_STRING: {
          ...
          if (node_tree->type == NTREE_GEOMETRY && smatpro != nullptr) {

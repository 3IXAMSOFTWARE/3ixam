...
using ixam::nodes::SocketLinkOperation;
...
static void add_group_input_node_fn(nodes::LinkSearchOpParams &params)
{
  ...
  ED_matpro_tree_propagate_change(&params.C, CTX_data_main(&params.C), &params.node_tree);
    ...
    const bContext *C, void *arg, const char *str, uiSearchItems *items, const bool is_first)
{
  ...
  if (storage.update_items_tag) {
    bNodeTree *node_tree = CTX_wm_space_matpro(C)->edittree;
...
static void link_drag_search_exec_fn(bContext *C, void *arg1, void *arg2)
{
  ...
  nodes::LinkSearchOpParams params{
      *C, *smatpro.edittree, storage.from_node, storage.from_socket, new_nodes};
  item->fn(params);
  ...
  nodeSetActive(smatpro.edittree, new_node);
                                    ...
                                    const float2 &cursor)
{
  ...
  /* Use the "_ex" variant with `can_refresh` false to avoid a double free when closing 3IXAM. */

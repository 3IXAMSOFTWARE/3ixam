 ...
 * This function is for fixing T76970 for 3IXAM 2.83. The actual fix should add a mechanism in
...
static bool is_position_over_node_or_socket(SpaceMatPro &smatpro, const float2 &mouse)
{
  if (node_under_mouse_tweak(*smatpro.edittree, mouse)) {
    return true;
  }
  ...
  if (node_find_indicated_socket(
          smatpro, &node, &sock, mouse, (eNodeSocketInOut)(SOCK_IN | SOCK_OUT))) {
    return true;
  }
...
static bool is_event_over_node_or_socket(const bContext &C, const wmEvent &event)
{
  ...
  return is_position_over_node_or_socket(smatpro, mouse);
}
...
void node_deselect_all_input_sockets(SpaceMatPro &smatpro, const bool deselect_nodes)
{
...
void node_deselect_all_output_sockets(SpaceMatPro &smatpro, const bool deselect_nodes)
{
  /* XXX not calling node_socket_deselect here each time, because this does iteration
   * over all node sockets internally to check if the node stays selected.
   * We can do that more efficiently here.
...
static int node_select_grouped_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *node_act = nodeGetActive(smatpro.edittree);
...
void MATPRO_OT_select_grouped(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_grouped";
...
void node_select_single(bContext &C, bNode &node)
{
  ...
  ED_matpro_set_active(bmain, &smatpro, &node_tree, &node, &active_texture_changed);
                              ...
                              SelectPick_Params *params)
{
  ...
  if (socket_select) {
    ...
    if (node_find_indicated_socket(smatpro, &node, &sock, cursor, SOCK_IN)) {
    ...
    else if (node_find_indicated_socket(smatpro, &node, &sock, cursor, SOCK_OUT)) {
      found = true;
      node_was_selected = node->flag & SELECT;
      ...
      if (sock->flag & SELECT) {
        ...
        if (!extend) {
          for (tnode = (bNode *)smatpro.edittree->nodes.first; tnode; tnode = tnode->next) {
            if (tnode == node) {
              continue;
            }
  ...
  if (!sock) {
    ...
    node = node_under_mouse_select(*smatpro.edittree, cursor);
    found = (node != nullptr);
  ...
  if ((node != nullptr) && (node_was_selected == false || params->select_passthrough == false)) {
    ...
    ED_matpro_set_active(&bmain, &smatpro, smatpro.edittree, node, &active_texture_changed);
  }
  else if (node != nullptr && node->type == GEO_NODE_VIEWER) {
    viewer_path::activate_geometry_node(bmain, smatpro, *node);
  }
  ED_matpro_set_active_viewer_key(&smatpro);
  node_sort(*smatpro.edittree);
  if ((active_texture_changed && has_workbench_in_texture_color(wm, scene, ob)) ||
      viewer_node_changed) {
    DEG_id_tag_update(&smatpro.edittree->id, ID_RECALC_COPY_ON_WRITE);
  }
...
void MATPRO_OT_select_box(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Box Select";
  ot->idname = "MATPRO_OT_select_box";
...
static int node_circleselect_exec(bContext *C, wmOperator *op)
{
  ...
  for (node = (bNode *)smatpro->edittree->nodes.first; node; node = node->next) {
    switch (node->type) {
      case NODE_FRAME: {
        /* Frame nodes are selectable by their borders (including their whole rect - as for other
...
void MATPRO_OT_select_circle(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_circle";
...
void MATPRO_OT_select_lasso(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_lasso";
...
void MATPRO_OT_select_all(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_all";
...
void MATPRO_OT_select_linked_to(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_linked_to";
...
static int node_select_linked_from_exec(bContext *C, wmOperator *op)
{
  ...
  char socket_identifier[64] = {'\0'};
  RNA_string_get(op->ptr, "socket_identifier", socket_identifier);

  node_tree.ensure_topology_cache();

  bNode *node = nodeGetActive(&node_tree);
  ...
  for (bNodeSocket *input_socket : node->input_sockets()) {
    ...
    if (strcmp(input_socket->identifier, socket_identifier) == 0) {
      ...
      for (bNodeSocket *output_socket : input_socket->directly_linked_sockets()) {
        ...
        nodeSetActive(&node_tree, &output_socket->owner_node());
        break;
      }
    }
  }
...
void MATPRO_OT_select_linked_from(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_linked_from";
  ...
  RNA_def_string(ot->srna, "socket_identifier", nullptr, 64, "Identifier of socket", "Socket name for selection");
}
...
static int node_select_same_type_step_exec(bContext *C, wmOperator *op)
{
  ...
  ARegion *region = CTX_wm_region(C);
  bNode **node_array;
  bNode *active = nodeGetActive(smatpro->edittree);
  int totnodes;
  const bool revert = RNA_boolean_get(op->ptr, "prev");
  const bool same_type = true;

  ntreeGetDependencyList(smatpro->edittree, &node_array, &totnodes);
  ...
  if (totnodes > 1) {
    ...
    if (active->totr.xmax < region->v2d.cur.xmin || active->totr.xmin > region->v2d.cur.xmax ||
      ...
      space_matpro_view_flag(*C, *smatpro, *region, NODE_SELECT, smooth_viewtx);
    }
  }
...
void MATPRO_OT_select_same_type_step(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_select_same_type_step";
...
static void node_find_exec_fn(bContext *C, void * /*arg1*/, void *arg2)
{
  ...
  if (active) {
    ...
    if (active->totr.xmax < region->v2d.cur.xmin || active->totr.xmin > region->v2d.cur.xmax ||
        active->totr.ymax < region->v2d.cur.ymin || active->totr.ymin > region->v2d.cur.ymax) {
      space_matpro_view_flag(*C, *smatpro, *region, NODE_SELECT, U.smooth_viewtx);
    }
  }
}

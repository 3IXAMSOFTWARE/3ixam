...
static void node_add_catalog_assets_draw(const bContext *C, Menu *menu)
{
  ...
  if (!smatpro.runtime->assets_for_menu) {
    BLI_assert_unreachable();
    return;
  }
  AssetItemTree &tree = *smatpro.runtime->assets_for_menu;
  const bNodeTree *edit_tree = smatpro.edittree;
  if (!edit_tree) {
    return;
  }
  ...
  for (const LibraryAsset &item : asset_items) {
    ...
    uiItemO(col, ED_asset_handle_get_name(&item.handle), ICON_NONE, "MATPRO_OT_add_group_asset");
  }
  ...
  catalog_item->foreach_child([&](bke::AssetCatalogTreeItem &child_item) {
    ...
    uiItemM(col, "NODE_MT_node_add_catalog_assets", path.name().c_str(), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  });
}
...
static void add_root_catalogs_draw(const bContext *C, Menu *menu)
{
  ...
  smatpro.runtime->assets_for_menu = std::make_shared<AssetItemTree>(
  ...
  if (!loading_finished) {
    uiItemL(layout, IFACE_("Loading Asset Libraries"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }
...
{
  MenuType type{};
  BLI_strncpy(type.idname, "MATPRO_MT_node_add_catalog_assets", sizeof(type.idname));
...
{
  MenuType type{};
  BLI_strncpy(type.idname, "MATPRO_MT_node_add_root_catalogs", sizeof(type.idname));
  type.poll = node_add_menu_poll;

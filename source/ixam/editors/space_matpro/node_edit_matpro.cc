...
#include "ED_image.h"
#include "ED_matpro.h" /* own include */
#include "ED_render.h"
#include "ED_screen.h"
...
static int compo_get_recalc_flags(const bContext *C)
{
  ...
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    ...
    LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
      if (ELEM(area->spacetype, SPACE_IMAGE, SPACE_RENDER)) {
      ...
      else if (area->spacetype == SPACE_MATPRO) {
        SpaceMatPro *smatpro = (SpaceMatPro *)area->spacedata.first;
        if (smatpro->flag & SNODE_BACKDRAW) {
          recalc_flags |= COM_RECALC_VIEWER;
        }
      }
    }
...
void ED_matpro_composite_job(const bContext *C, bNodeTree *nodetree, Scene *scene_owner)
{
...
{
  if (ED_operator_matpro_active(C)) {
...
{
  if (ED_operator_matpro_editable(C)) {
    SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
    if (ED_matpro_is_compositor(smatpro)) {
      return true;
    }
...
void ED_matpro_tree_propagate_change(const bContext *C, Main *bmain, bNodeTree *root_ntree)
{
  if (C != nullptr) {
    SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
    if (smatpro != nullptr && root_ntree != nullptr) {
      ixam::ed::space_matpro::send_notifiers_after_tree_change(smatpro->id, root_ntree);
    }
  }

  NodeTreeUpdateExtraParams params = {nullptr};
  params.tree_changed_fn = [](ID *id, bNodeTree *ntree, void * /*user_data*/) {
    ixam::ed::space_matpro::send_notifiers_after_tree_change(id, ntree);
...
void ED_matpro_set_tree_type(SpaceMatPro *smatpro, bNodeTreeType *typeinfo)
{
  if (typeinfo) {
    BLI_strncpy(smatpro->tree_idname, typeinfo->idname, sizeof(smatpro->tree_idname));
  }
  else {
    smatpro->tree_idname[0] = '\0';
  }
}

bool ED_matpro_is_compositor(SpaceMatPro *smatpro)
{
  return STREQ(smatpro->tree_idname, ntreeType_Composite->idname);
}

bool ED_matpro_is_shader(SpaceMatPro *smatpro)
{
  return STREQ(smatpro->tree_idname, ntreeType_Shader->idname);
}

bool ED_matpro_is_texture(SpaceMatPro *smatpro)
{
  return STREQ(smatpro->tree_idname, ntreeType_Texture->idname);
}

bool ED_matpro_is_geometry(SpaceMatPro *smatpro)
{
  return STREQ(smatpro->tree_idname, ntreeType_Geometry->idname);
}

void ED_matpro_shader_default(const bContext *C, ID *id)
{
  Main *bmain = CTX_data_main(C);
  ...
  if (GS(id->name) == ID_MA) {
    ...
    printf("ED_matpro_shader_default called on wrong ID type.\n");
    return;
  }
}

void ED_matpro_composit_default(const bContext *C, Scene *sce)
{
...
void ED_matpro_texture_default(const bContext *C, Tex *tex)
{
  /* but lets check it anyway */
  if (tex->nodetree) {
    if (G.debug & G_DEBUG) {
 ...
 */
void smatpro_set_context(const bContext &C)
{
  SpaceMatPro *smatpro = CTX_wm_space_matpro(&C);
  bNodeTreeType *treetype = ntreeTypeFind(smatpro->tree_idname);
  bNodeTree *ntree = smatpro->nodetree;
  ID *id = smatpro->id, *from = smatpro->from;
  ...
  if (smatpro->nodetree && !STREQ(smatpro->nodetree->idname, smatpro->tree_idname)) {
  ...
  if (!(smatpro->flag & SNODE_PIN) || ntree == nullptr) {
  ...
  if (smatpro->nodetree != ntree || smatpro->id != id || smatpro->from != from ||
      (smatpro->treepath.last == nullptr && ntree)) {
    ED_matpro_tree_start(smatpro, ntree, id, from);
  }
}

}  // namespace ixam::ed::space_matpro

void ED_matpro_set_active(
    Main *bmain, SpaceMatPro *smatpro, bNodeTree *ntree, bNode *node, bool *r_active_texture_changed)
{
  ...
  if (node->type != NODE_GROUP) {
    ...
    if (ntree->type == NTREE_SHADER) {
      ...
      if (node->type == GEO_NODE_VIEWER) {
        ...
        ixam::ed::viewer_path::activate_geometry_node(*bmain, *smatpro, *node);
      }
    }
  }
}

void ED_matpro_post_apply_transform(bContext * /*C*/, bNodeTree * /*ntree*/)
{
...
{
  return ED_operator_matpro_active(C);
}
  ...
  switch (event->type) {
    ...
    case RIGHTMOUSE: {
      if (event->val == KM_RELEASE) {
        ...
        ED_matpro_post_apply_transform(C, smatpro->edittree);
...
static int node_resize_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  ARegion *region = CTX_wm_region(C);
  const bNode *node = nodeGetActive(smatpro->edittree);
...
void MATPRO_OT_resize(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Resize Node";
  ot->idname = "MATPRO_OT_resize";
  ot->description = "Resize a node";

  /* api callbacks */
  ot->invoke = node_resize_invoke;
  ot->modal = node_resize_modal;
  ot->poll = ED_operator_matpro_active;
...
void node_set_hidden_sockets(SpaceMatPro *smatpro, bNode *node, int set)
{
  if (set == 0) {
    LISTBASE_FOREACH (bNodeSocket *, sock, &node->inputs) {
    ...
    LISTBASE_FOREACH (bNodeSocket *, sock, &node->outputs) {
      if (nodeCountSocketLinks(smatpro->edittree, sock) == 0) {
        sock->flag |= SOCK_HIDDEN;
      }
    }
  }
...
/* checks smatpro->mouse position, and returns found node/socket */
...
bool node_find_indicated_socket(SpaceMatPro &smatpro,
                                bNode **nodep,
                                bNodeSocket **sockp,
                                const float2 &cursor,
        ...
        if (!nodeSocketIsHidden(sock)) {
          ...
          if (sock->flag & SOCK_MULTI_INPUT && !(node->flag & NODE_HIDDEN)) {
            if (cursor_isect_multi_input_socket(cursor, *sock)) {
              if (!socket_is_occluded(location, *node, smatpro)) {
                *nodep = node;
                *sockp = sock;
                return true;
              }
...
static int node_duplicate_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  bNodeTree *ntree = smatpro->edittree;
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->flag & SELECT) {
      bNode *new_node = ixam::bke::node_copy_with_mapping(
  ...
  ED_matpro_tree_propagate_change(C, bmain, smatpro->edittree);
  return OPERATOR_FINISHED;
}

void MATPRO_OT_duplicate(wmOperatorType *ot)
{
  PropertyRNA *prop;
  ...
  ot->idname = "MATPRO_OT_duplicate";

  /* api callbacks */
  ot->exec = node_duplicate_exec;
  ot->poll = ED_operator_matpro_editable;
...
static int node_read_viewlayers_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &smatpro->edittree->nodes) {
...
void MATPRO_OT_read_viewlayers(wmOperatorType *ot)
{

  ot->name = "Read View Layers";
  ot->idname = "MATPRO_OT_read_viewlayers";
...
void MATPRO_OT_render_changed(wmOperatorType *ot)
{
  ot->name = "Render Changed Layer";
  ot->idname = "MATPRO_OT_render_changed";
...
static void node_flag_toggle_exec(SpaceMatPro *smatpro, int toggle_flag)
{
...
static int node_hide_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  if ((smatpro == nullptr) || (smatpro->edittree == nullptr)) {
    return OPERATOR_CANCELLED;
  }

  node_flag_toggle_exec(smatpro, NODE_HIDDEN);
...
void MATPRO_OT_hide_toggle(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Hide";
  ot->description = "Toggle hiding of selected nodes";
  ot->idname = "MATPRO_OT_hide_toggle";
...
static int node_preview_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  node_flag_toggle_exec(smatpro, NODE_PREVIEW);

  ED_matpro_tree_propagate_change(C, CTX_data_main(C), smatpro->edittree);

  return OPERATOR_FINISHED;
}

void MATPRO_OT_preview_toggle(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Toggle Node Preview";
  ot->description = "Toggle preview display for selected nodes";
  ot->idname = "MATPRO_OT_preview_toggle";
...
static int node_deactivate_viewer_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  bNode *active_viewer = viewer_path::find_geometry_nodes_viewer(workspace.viewer_path, smatpro);
  ...
  LISTBASE_FOREACH (bNode *, node, &smatpro.edittree->nodes) {
    ...
    if (node == active_viewer) {
      node->flag &= ~NODE_DO_OUTPUT;
      BKE_ntree_update_tag_node_property(smatpro.edittree, node);
    }
  }
...
void MATPRO_OT_deactivate_viewer(wmOperatorType *ot)
{
  /* identifiers */
...
static int node_options_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  node_flag_toggle_exec(smatpro, NODE_OPTIONS);
...
static int node_socket_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &smatpro->edittree->nodes) {
    if (node->flag & SELECT) {
      node_set_hidden_sockets(smatpro, node, !hidden);
    }
  }
...
void MATPRO_OT_hide_socket_toggle(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Toggle Hidden Node Sockets";
  ot->description = "Toggle unused node socket display";
  ot->idname = "MATPRO_OT_hide_socket_toggle";
...
static int node_mute_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &smatpro->edittree->nodes) {
    if ((node->flag & SELECT) && !node->typeinfo->no_muting) {
      ...
      BKE_ntree_update_tag_node_mute(smatpro->edittree, node);
    }
  }
...
void MATPRO_OT_mute_toggle(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Toggle Node Mute";
  ot->description = "Toggle muting of the nodes";
  ot->idname = "MATPRO_OT_mute_toggle";
...
static int node_delete_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH_MUTABLE (bNode *, node, &smatpro->edittree->nodes) {
    if (node->flag & SELECT) {
      nodeRemoveNode(bmain, smatpro->edittree, node, true);
    }
  }
...
void MATPRO_OT_delete(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Delete";
  ot->description = "Delete selected nodes";
  ot->idname = "MATPRO_OT_delete";
...
void MATPRO_OT_switch_view_update(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Update Views";
  ot->description = "Update views of selected node";
  ot->idname = "MATPRO_OT_switch_view_update";
...
static int node_delete_reconnect_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH_MUTABLE (bNode *, node, &smatpro->edittree->nodes) {
    if (node->flag & SELECT) {
      nodeInternalRelink(smatpro->edittree, node);
      nodeRemoveNode(bmain, smatpro->edittree, node, true);
    }
  }
...
void MATPRO_OT_delete_reconnect(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Delete with Reconnect";
  ot->description = "Delete nodes; will reconnect nodes as if deletion was muted";
  ot->idname = "MATPRO_OT_delete_reconnect";
...
static int node_output_file_add_socket_exec(bContext *C, wmOperator *op)
{
  ...
  else if (smatpro && smatpro->edittree) {
    ntree = smatpro->edittree;
    node = nodeGetActive(smatpro->edittree);
  }
...
void MATPRO_OT_output_file_add_socket(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Add File Node Socket";
  ot->description = "Add a new input to a file output node";
  ot->idname = "MATPRO_OT_output_file_add_socket";
...
static int node_output_file_remove_active_socket_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  ED_matpro_tree_propagate_change(C, CTX_data_main(C), ntree);

  return OPERATOR_FINISHED;
}

void MATPRO_OT_output_file_remove_active_socket(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Remove File Node Socket";
  ot->description = "Remove active input from a file output node";
  ot->idname = "MATPRO_OT_output_file_remove_active_socket";
...
void MATPRO_OT_output_file_move_active_socket(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_output_file_move_active_socket";
...
void MATPRO_OT_node_copy_color(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Copy Color";
  ot->description = "Copy color to all selected nodes";
  ot->idname = "MATPRO_OT_node_copy_color";
...
static int node_clipboard_copy_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->flag & SELECT) {
       ...
       * detached from any actual 3IXAM data currently. */
      bNode *new_node = ixam::bke::node_copy_with_mapping(nullptr,
                                                             *node,
...
void MATPRO_OT_clipboard_copy(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Copy to Clipboard";
  ot->description = "Copies selected nodes to the clipboard";
  ot->idname = "MATPRO_OT_clipboard_copy";
...
void MATPRO_OT_clipboard_paste(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Paste from Clipboard";
  ot->description = "Pastes nodes from the clipboard to the active node tree";
  ot->idname = "MATPRO_OT_clipboard_paste";
...
static int ntree_socket_change_type_exec(bContext *C, wmOperator *op)
{
  ...
  ED_matpro_tree_propagate_change(C, main, ntree);
                                                        ...
                                                        bool *r_free)
{
  ...
  return rna_node_socket_type_itemf(smatpro->edittree->typeinfo, socket_change_poll_type, r_free);
}

void MATPRO_OT_tree_socket_change_type(wmOperatorType *ot)
{
  PropertyRNA *prop;
  ...
  ot->idname = "MATPRO_OT_tree_socket_change_type";
...
static bool node_shader_script_update_poll(bContext *C)
{
  ...
  if (!node && smatpro && smatpro->edittree) {
    node = nodeGetActive(smatpro->edittree);
  }
  ...
  if (node && node->type == SH_NODE_SCRIPT) {
    ...
    if (node->id || nss->filepath[0]) {
      return ED_operator_matpro_editable(C);
    }
  }
...
static void viewer_border_corner_to_backdrop(SpaceMatPro *smatpro,
                                             ARegion *region,
                                             int x,
                                             int y,
                                             int backdrop_width,
                                             ...
                                             float *fy)
{
  float bufx = backdrop_width * smatpro->zoom;
  float bufy = backdrop_height * smatpro->zoom;

  *fx = (bufx > 0.0f ? (float(x) - 0.5f * region->winx - smatpro->xof) / bufx + 0.5f : 0.0f);
  *fy = (bufy > 0.0f ? (float(y) - 0.5f * region->winy - smatpro->yof) / bufy + 0.5f : 0.0f);
}
...
static int viewer_border_exec(bContext *C, wmOperator *op)
{
  ...
  if (ibuf) {
    ARegion *region = CTX_wm_region(C);
    SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
    bNodeTree *btree = smatpro->nodetree;
    rcti rect;
    rctf rectf;
    ...
    viewer_border_corner_to_backdrop(
        smatpro, region, rect.xmin, rect.ymin, ibuf->x, ibuf->y, &rectf.xmin, &rectf.ymin);

    viewer_border_corner_to_backdrop(
        smatpro, region, rect.xmax, rect.ymax, ibuf->x, ibuf->y, &rectf.xmax, &rectf.ymax);
    ...
    if (rectf.xmin < rectf.xmax && rectf.ymin < rectf.ymax) {
      ...
      ED_matpro_tree_propagate_change(C, bmain, btree);
      WM_event_add_notifier(C, NC_NODE | ND_DISPLAY, nullptr);
    }
    else {
      btree->flag &= ~NTREE_VIEWER_BORDER;
...
void MATPRO_OT_viewer_border(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Viewer Region";
  ot->description = "Set the boundaries for viewer operations";
  ot->idname = "MATPRO_OT_viewer_border";
...
static int clear_viewer_border_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  btree->flag &= ~NTREE_VIEWER_BORDER;
  ED_matpro_tree_propagate_change(C, CTX_data_main(C), btree);
  WM_event_add_notifier(C, NC_NODE | ND_DISPLAY, nullptr);

  return OPERATOR_FINISHED;
}

void MATPRO_OT_clear_viewer_border(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Clear Viewer Region";
  ot->description = "Clear the boundaries for viewer operations";
  ot->idname = "MATPRO_OT_clear_viewer_border";
...
void MATPRO_OT_cryptomatte_layer_add(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Add Cryptomatte Socket";
  ot->description = "Add a new input layer to a Cryptomatte node";
  ot->idname = "MATPRO_OT_cryptomatte_layer_add";
...
void MATPRO_OT_cryptomatte_layer_remove(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Remove Cryptomatte Socket";
  ot->description = "Remove layer from a Cryptomatte node";
  ot->idname = "MATPRO_OT_cryptomatte_layer_remove";

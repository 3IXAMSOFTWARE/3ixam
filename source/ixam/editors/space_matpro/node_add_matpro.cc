...
bNode *add_node(const bContext &C, const StringRef idname, const float2 &location)
{
  ...
  bNode *node = nodeAddNode(&C, smatpro.edittree, idname_str.c_str());
  BLI_assert(node && node->typeinfo);

  position_node_based_on_mouse(*node, location);

  nodeSetSelected(node, true);
  ED_matpro_set_active(&bmain, &smatpro, smatpro.edittree, node, nullptr);
...
bNode *add_static_node(const bContext &C, int type, const float2 &location)
{
  ...
  bNode *node = nodeAddStaticNode(&C, smatpro.edittree, type);
  BLI_assert(node && node->typeinfo);
...
{
  ot->name = "Add Reroute";
  ot->idname = "MATPRO_OT_add_reroute";
...
static int node_add_group_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *group_node = add_node(*C, node_idname, smatpro->runtime->cursor);
  if (!group_node) {
  ...
  BKE_ntree_update_tag_node_property(smatpro->edittree, group_node);

  nodeSetActive(ntree, group_node);
  ED_matpro_tree_propagate_change(C, bmain, nullptr);
...
static bool node_add_group_poll(bContext *C)
{
  ...
  if (smatpro->edittree->type == NTREE_CUSTOM) {
                                 ...
                                 ReportList &reports)
{
  Main &bmain = *CTX_data_main(&C);
  SpaceMatPro &smatpro = *CTX_wm_space_matpro(&C);
  bNodeTree &edit_tree = *smatpro.edittree;
  ...
  bNode *group_node = add_node(
      C, ntreeTypeFind(node_group->idname)->group_idname, smatpro.runtime->cursor);
  if (!group_node) {
...
static int node_add_group_asset_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  smatpro.runtime->cursor /= UI_DPI_FAC;

  if (!add_node_group_asset(*C, *library_ref, handle, *op->reports)) {
    return OPERATOR_CANCELLED;
  }

  wmOperatorType *ot = WM_operatortype_find("MATPRO_OT_translate_attach_remove_on_cancel", true);
  BLI_assert(ot);
  PointerRNA ptr;
...
void MATPRO_OT_add_group_asset(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_add_group_asset";
...
static int node_add_object_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *object_node = add_static_node(*C, GEO_NODE_OBJECT_INFO, smatpro->runtime->cursor);
  if (!object_node) {
...
{
  const SpaceMatPro *smatpro = CTX_wm_space_matpro(C);
  return ED_operator_matpro_editable(C) && ELEM(smatpro->nodetree->type, NTREE_GEOMETRY) &&
...
void MATPRO_OT_add_object(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_add_object";
...
static int node_add_collection_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *collection_node = add_static_node(*C, GEO_NODE_COLLECTION_INFO, smatpro.runtime->cursor);
  if (!collection_node) {
...
void MATPRO_OT_add_collection(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_add_collection";
...
static bool node_add_file_poll(bContext *C)
{
  ...
  return ED_operator_matpro_editable(C) &&
         ELEM(smatpro->nodetree->type, NTREE_SHADER, NTREE_TEXTURE, NTREE_COMPOSIT, NTREE_GEOMETRY);
}
...
static int node_add_file_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *node = add_static_node(*C, type, smatpro.runtime->cursor);
...
static bool node_add_mask_poll(bContext *C)
{
  ...
  return ED_operator_matpro_editable(C) && smatpro->nodetree->type == NTREE_COMPOSIT;
}
...
static int node_add_mask_exec(bContext *C, wmOperator *op)
{
  ...
  bNode *node = add_static_node(*C, CMP_NODE_MASK, smatpro.runtime->cursor);
...
void MATPRO_OT_new_node_tree(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_new_node_tree";
...
void MATPRO_OT_add_search(wmOperatorType *ot)
{
  ...
  ot->idname = "MATPRO_OT_add_search";

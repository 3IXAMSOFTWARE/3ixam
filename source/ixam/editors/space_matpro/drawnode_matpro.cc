...
static void node_socket_button_label(bContext * /*C*/,
                                     ...
                                     const char *text)
{
  uiItemL(layout, text, 0, UI_BSHAPE_ROUNDBOX);
}
...
static void node_buts_value(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Value"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
static void node_buts_normal(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  RNA_pointer_create(ptr->owner_id, &RNA_NodeSocket, output, &sockptr);
  uiLayout *row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Default Value"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, &sockptr, "default_value", DEFAULT_FLAGS, "", ICON_NONE);
...
{
  uiLayout *row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Operation"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "operation", DEFAULT_FLAGS, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Clamp"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "use_clamp", DEFAULT_FLAGS, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
}
...
{
  uiLayout *row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Mode"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "mode", DEFAULT_FLAGS, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
}
                                 ...
                                 const bool show_color_management)
{
  ...
  if (source == IMA_SRC_SEQUENCE) {
    ...
    uiItemL(layout, numstr, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  ...
  if (show_color_management) {
    ...
    uiItemL(split, IFACE_("Color Space"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(split, &colorspace_settings_ptr, "name", DEFAULT_FLAGS, "", ICON_NONE);

    if (image->source != IMA_SRC_GENERATED) {
      split = uiLayoutSplit(layout, 0.33f, true);
      uiItemL(split, IFACE_("Alpha"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
{
  PointerRNA iuserptr = RNA_pointer_get(ptr, "image_user");
  uiTemplateImage(layout, C, ptr, "image", &iuserptr, false, false, false);
}
...
static void node_shader_buts_tex_environment_ex(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  uiLayout *row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Interpolation"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "interpolation", DEFAULT_FLAGS, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Projection"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "projection", DEFAULT_FLAGS, "", ICON_NONE);
}

static void node_shader_buts_displacement(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  uiLayout *row = uiLayoutRow(layout, false);
  uiItemL(row, IFACE_("Space"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "space", DEFAULT_FLAGS, "", 0);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
}
...
{
  uiLayout *row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Distribution"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "distribution", DEFAULT_FLAGS, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
}
...
static void node_buts_output_shader(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Target"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "target", DEFAULT_FLAGS, "", ICON_NONE);
  ...
  ListBase newlb;
  short newtype;
  int done = CTX_data_get(C, "active_object", &object, &newlb, &newtype);
  if(done) {
    PropertyRNA *materialptr = RNA_struct_find_property(&object, "active_material");
    PointerRNA material = RNA_property_pointer_get(&object, materialptr);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Disable Double Side"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, &material, "use_backface_culling", DEFAULT_FLAGS, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Alpha"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, &material, "blend_method", DEFAULT_FLAGS, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Alpha Shadow"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, &material, "shadow_method", DEFAULT_FLAGS, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Alpha Clip"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, &material, "alpha_threshold", DEFAULT_FLAGS, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

    PropertyRNA *cyclesptr = RNA_struct_find_property(&material, "cycles");
    PointerRNA cycles = RNA_property_pointer_get(&material, cyclesptr);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Displace Settings"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, &cycles, "displacement_method", DEFAULT_FLAGS, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void node_composit_backdrop_viewer(
    SpaceNode *smatpro, ImBuf *backdrop, bNode *node, int x, int y)
{
  //  node_composit_backdrop_canvas(smatpro, backdrop, node, x, y);
  if (node->custom1 == 0) {
    const float backdropWidth = backdrop->x;
    const float backdropHeight = backdrop->y;
    const float cx = x + smatpro->zoom * backdropWidth * node->custom3;
    const float cy = y + smatpro->zoom * backdropHeight * node->custom4;
    ...
    SpaceNode *smatpro, ImBuf *backdrop, bNode *node, int x, int y)
{
  ...
  cx = x + smatpro->zoom * backdropWidth * boxmask->x;
  cy = y + smatpro->zoom * backdropHeight * boxmask->y;

  x1 = cx - (cosine * halveBoxWidth + sine * halveBoxHeight) * smatpro->zoom;
  x2 = cx - (cosine * -halveBoxWidth + sine * halveBoxHeight) * smatpro->zoom;
  x3 = cx - (cosine * -halveBoxWidth + sine * -halveBoxHeight) * smatpro->zoom;
  x4 = cx - (cosine * halveBoxWidth + sine * -halveBoxHeight) * smatpro->zoom;
  y1 = cy - (-sine * halveBoxWidth + cosine * halveBoxHeight) * smatpro->zoom;
  y2 = cy - (-sine * -halveBoxWidth + cosine * halveBoxHeight) * smatpro->zoom;
  y3 = cy - (-sine * -halveBoxWidth + cosine * -halveBoxHeight) * smatpro->zoom;
  y4 = cy - (-sine * halveBoxWidth + cosine * -halveBoxHeight) * smatpro->zoom;
    ...
    SpaceNode *smatpro, ImBuf *backdrop, bNode *node, int x, int y)
{
  ...
  cx = x + smatpro->zoom * backdropWidth * ellipsemask->x;
  cy = y + smatpro->zoom * backdropHeight * ellipsemask->y;
                                                  ...
                                                  PointerRNA *ptr)
{
  ...
  uiItemL(col, IFACE_("Matte Objects:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
static void node_composit_buts_cryptomatte_legacy_ex(uiLayout *layout,
                                                     ...
                                                     PointerRNA * /*ptr*/)
{
  uiItemO(layout, IFACE_("Add Crypto Layer"), ICON_ADD, "MATPRO_OT_cryptomatte_layer_add");
  uiItemO(layout, IFACE_("Remove Crypto Layer"), ICON_REMOVE, "MATPRO_OT_cryptomatte_layer_remove");
}
...
static void node_composit_buts_cryptomatte(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  uiItemL(col, IFACE_("Matte ID:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
static void node_texture_buts_proc(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  switch (tex->type) {
    case TEX_BLEND:
      row = uiLayoutRow(col, true);
      uiItemL(row, IFACE_("Progression"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "progression", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Flip Axis"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(
          row, &tex_ptr, "use_flip_axis", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
      break;

    case TEX_MARBLE:
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Pattern"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "marble_type", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Noise Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "noise_type", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Noise Basis"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "noise_basis", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Noise Basis 2"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(
          row, &tex_ptr, "noise_basis_2", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
    ...
    case TEX_MAGIC:
      row = uiLayoutRow(col, true);
      uiItemL(row, IFACE_("Noise Depth"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "noise_depth", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
    ...
    case TEX_STUCCI:
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Pattern"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "stucci_type", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
    ...
    case TEX_WOOD:
      ...
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Pattern"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "wood_type", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
    ...
    case TEX_CLOUDS:
      ...
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Color"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "cloud_type", DEFAULT_FLAGS | UI_ITEM_R_EXPAND, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
      ...
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Depth"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row,
              &tex_ptr,
              "noise_depth",
              DEFAULT_FLAGS,
    ...
    case TEX_DISTNOISE:
      ...
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Noise Distortion"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "noise_distortion", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
      break;

    case TEX_MUSGRAVE:
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "musgrave_type", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
    ...
    case TEX_VORONOI:
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Distance Metric"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "distance_metric", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

      if (tex->vn_distm == TEX_MINKOVSKY) {
        row = uiLayoutRow(col, false);
        uiItemL(row, IFACE_("Minkovsky Exponent"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
        uiItemR(row, &tex_ptr, "minkovsky_exponent", DEFAULT_FLAGS, "", ICON_NONE);
        uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
      }
      row = uiLayoutRow(col, false);
      uiItemL(row, IFACE_("Coloring"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, &tex_ptr, "color_mode", DEFAULT_FLAGS, "", ICON_NONE);
      uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
      break;
  }
}
...
static void node_property_update_default(Main *bmain, Scene * /*scene*/, PointerRNA *ptr)
{
  ...
  ED_matpro_tree_propagate_change(nullptr, bmain, ntree);
}
...
static void node_socket_undefined_draw(bContext * /*C*/,
                                       ...
                                       const char * /*text*/)
{
  uiItemL(layout, IFACE_("Undefined Socket Type"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
}
...
void ED_matpro_init_butfuncs()
{
  using namespace ixam::ed::space_matpro;

  /* Fallback types for undefined tree, nodes, sockets
   * Defined in ixamkernel, but not registered in type hashes.
   */
...
static const float std_node_socket_colors[][4] = {
    {0.73, 0.16, 0.65, 1.0}, /* SOCK_FLOAT */
    {0.0, 0.70, 1.0, 1.0}, /* SOCK_VECTOR */
    {0.0, 0.73, 0.0, 1.0}, /* SOCK_RGBA */
    {1.0, 0.23, 0.0, 1.0}, /* SOCK_SHADER */
                                         ...
                                         PointerRNA *node_ptr)
{
  ...
  if (imtype == R_IMF_IMTYPE_MULTILAYER) {
    ...
    uiItemL(row, input->layer, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
  else {
    NodeImageMultiFileSocket *input = (NodeImageMultiFileSocket *)sock->storage;
    uiBlock *block;
    ...
    uiItemL(row, input->path, ICON_NONE, UI_BSHAPE_ROUNDBOX);
    ...
    uiItemL(row, imtype_name, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  switch (type) {
    ...
    case SOCK_RGBA: {
      if (text[0] == '\0') {
        ...
        uiItemL(row, text, 0, UI_BSHAPE_ROUNDBOX);
    ...
    case SOCK_TEXTURE: {
      if (text[0] == '\0') {
        ...
        uiItemL(row, text, 0, UI_BSHAPE_ROUNDBOX);
        uiTemplateID(
            row, C, ptr, "default_value", "texture.new", nullptr, nullptr, 0, ICON_NONE, nullptr, 0, true);
      }
                             ...
                             bNodeInstanceKey parent_key)
{
  ...
  bNodeInstanceKey active_viewer_key = (smatpro.nodetree ? smatpro.nodetree->active_viewer_key :
  ...
  if (!(smatpro.flag & SNODE_BACKDRAW) || !ED_matpro_is_compositor(&smatpro)) {
    return;
  }
  ...
  if (ibuf) {
    ...
    wmOrtho2_region_pixelspace(&region);
    const float x = (region.winx - smatpro.zoom * ibuf->x) / 2 + smatpro.xof;
    const float y = (region.winy - smatpro.zoom * ibuf->y) / 2 + smatpro.yof;

    /** \note draw selected info on backdrop */
    if (smatpro.edittree) {
      bNode *node = (bNode *)smatpro.edittree->nodes.first;
      rctf *viewer_border = &smatpro.nodetree->viewer_border;
      while (node) {
        if (node->flag & NODE_SELECT) {
          if (node->typeinfo->draw_backdrop) {
            SpaceNode *snode = (SpaceNode *)&smatpro;
      ...
      if ((smatpro.nodetree->flag & NTREE_VIEWER_BORDER) &&
          viewer_border->xmin < viewer_border->xmax && viewer_border->ymin < viewer_border->ymax) {
        rcti pixel_border;
        BLI_rcti_init(&pixel_border,
                      x + smatpro.zoom * viewer_border->xmin * ibuf->x,
                      x + smatpro.zoom * viewer_border->xmax * ibuf->x,
                      y + smatpro.zoom * viewer_border->ymin * ibuf->y,
                      y + smatpro.zoom * viewer_border->ymax * ibuf->y);
...
{
  float2 socket_location(socket.locx + (socket.is_input() ? -5 : 5) * U.dpi_fac, socket.locy);
...
{
  const int curving = UI_GetThemeValueType(TH_NODE_CURVING, SPACE_MATPRO);
  if (curving == 0) {
...
static void nodelink_batch_draw(const SpaceMatPro &smatpro)
{
  ...
  node_link_data.expandSize = smatpro.runtime->aspect * LINK_WIDTH;
...
void nodelink_batch_start(SpaceMatPro & /*smatpro*/)
{
  g_batch_link.enabled = true;
}

void nodelink_batch_end(SpaceMatPro &smatpro)
{
  nodelink_batch_draw(smatpro);
...
static void nodelink_batch_add_link(const SpaceMatPro &smatpro,
...
static bool node_link_is_field_link(const SpaceMatPro &smatpro, const bNodeLink &link)
{
                                                   ...
                                                   const bool selected)
{
  ...
  draw_config.dash_alpha = btheme->space_matpro.dash_alpha;

  const bool field_link = node_link_is_field_link(smatpro, link);
  ...
  if (smatpro.overlay.flag & SN_OVERLAY_SHOW_OVERLAYS &&
      smatpro.overlay.flag & SN_OVERLAY_SHOW_WIRE_COLORS) {
    PointerRNA from_node_ptr, to_node_ptr;
    RNA_pointer_create((ID *)smatpro.edittree, &RNA_Node, link.fromnode, &from_node_ptr);
    RNA_pointer_create((ID *)smatpro.edittree, &RNA_Node, link.tonode, &to_node_ptr);

    if (link.fromsock) {
      node_socket_color_get(
          C, *smatpro.edittree, from_node_ptr, *link.fromsock, draw_config.start_color);
    }
    else {
      node_socket_color_get(
          C, *smatpro.edittree, to_node_ptr, *link.tosock, draw_config.start_color);
    }

    if (link.tosock) {
      node_socket_color_get(C, *smatpro.edittree, to_node_ptr, *link.tosock, draw_config.end_color);
    }
    else {
      node_socket_color_get(
          C, *smatpro.edittree, from_node_ptr, *link.fromsock, draw_config.end_color);
    }
  }
  else {
...
static void node_draw_link_bezier_ex(const SpaceMatPro &smatpro,
                                     const NodeLinkDrawConfig &draw_config,
                                     const std::array<float2, 4> &points)
{
  ...
  if (g_batch_link.enabled && !draw_config.highlighted) {
    /* Add link to batch. */
    nodelink_batch_add_link(smatpro, points, draw_config);
  }
  else {
                           ...
                           const bool selected)
{
  ...
  const NodeLinkDrawConfig draw_config = nodelink_get_draw_config(
      C, v2d, smatpro, link, th_col1, th_col2, th_col3, selected);

  node_draw_link_bezier_ex(smatpro, draw_config, points);
}
                    ...
                    const bool selected)
{
  ...
  if (smatpro.edittree->type == NTREE_GEOMETRY && !(link.flag & NODE_LINK_DRAGGED)) {
    if ((link.fromsock && link.fromsock->display_shape == SOCK_DISPLAY_SHAPE_DIAMOND) &&
  ...
  node_draw_link_bezier(C, v2d, smatpro, link, th_col1, th_col2, th_col3, selected);
}

static std::array<float2, 4> node_link_bezier_points_dragged(const SpaceMatPro &smatpro,
                                                             const bNodeLink &link)
{
  const float2 cursor = smatpro.runtime->cursor * UI_DPI_FAC;
  std::array<float2, 4> points;
                            ...
                            const bNodeLink &link)
{
  ...
  const std::array<float2, 4> points = node_link_bezier_points_dragged(smatpro, link);

  const NodeLinkDrawConfig draw_config = nodelink_get_draw_config(
      C, v2d, smatpro, link, TH_ACTIVE, TH_ACTIVE, TH_WIRE, true);
  /* End marker outline. */
...
void ED_matpro_draw_snap(View2D *v2d, const float cent[2], float size, NodeBorderMatPro border, uint pos)
{
  immBegin(GPU_PRIM_LINES, 4);

  if (border & (NODE_LEFT_MP | NODE_RIGHT_MP)) {
  ...
  if (border & (NODE_TOP_MP | NODE_BOTTOM_MP)) {

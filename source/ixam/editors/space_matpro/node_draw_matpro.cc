...
namespace geo_log = ixam::nodes::geo_eval_log;

using ixam::GPointer;
using ixam::Vector;
using ixam::fn::GField;
...
float ED_matpro_grid_size()
{
  return U.widget_unit;
}

void ED_matpro_tree_update(const bContext *C)
{
  ...
  if (smatpro) {
    ...
    id_us_ensure_real(&smatpro->nodetree->id);
  }
}
...
void ED_matpro_tag_update_id(ID *id)
{
                              ...
                              uiBlock &block)
{
  ...
  LISTBASE_FOREACH (bNodeSocket *, socket, &node.outputs) {
    ...
    uiItemL(row, IFACE_(socket_label), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemS(row);
    uiItemS(row);
    ...
    UI_block_layout_resolve(&block, nullptr, &buty, nullptr, nullptr);

    /* Ensure minimum socket height in case layout is empty. */
    buty = min_ii(buty, dy - NODE_DY);

    /* Round the socket location to stop it from jiggling. */
    socket->locx = round(loc.x + NODE_WIDTH(node)) - (10 * U.dpi_fac);
  ...
  LISTBASE_FOREACH (bNodeSocket *, socket, &node.inputs) {
    ...
    socket->locx = loc.x + (10 * U.dpi_fac);
                                ...
                                const bNode &node)
{
  ...
  LISTBASE_FOREACH (const bNodeLink *, link, &node.internal_links) {
    if (!nodeLinkIsHidden(link)) {
      node_draw_link_bezier(C, v2d, smatpro, *link, TH_WIRE_INNER, TH_WIRE_INNER, TH_WIRE, false);
    }
  }
...
static void create_inspection_string_for_generic_value(const GPointer value, std::stringstream &ss)
{
  ...
  else if (type.is<ixam::float3>()) {
    ss << *(ixam::float3 *)buffer << TIP_(" (Vector)");
  }
  else if (type.is<ixam::ColorGeometry4f>()) {
    const ixam::ColorGeometry4f &color = *(ixam::ColorGeometry4f *)buffer;
                                                    ...
                                                    std::stringstream &ss)
{
  ...
  if (input_tooltips.is_empty()) {
    ...
    for (const int i : input_tooltips.index_range()) {
      const ixam::StringRef tooltip = input_tooltips[i];
      ss << "\u2022 " << tooltip;
...
static std::optional<std::string> create_socket_inspection_string(TreeDrawContext &tree_draw_ctx,
                                                                  const bNodeSocket &socket)
{
  using namespace ixam::nodes::geo_eval_log;
                                     ...
                                     const bNodeSocket *socket)
{
  ...
  if (smatpro != nullptr) {
    if (ntree->type == NTREE_GEOMETRY) {
      tree_draw_ctx.geo_tree_log = geo_log::GeoModifierLog::get_tree_log_for_matpro_editor(*smatpro);
    }
  }

  std::stringstream output;
  if (socket->runtime->declaration != nullptr) {
    const ixam::nodes::SocketDeclaration &socket_decl = *socket->runtime->declaration;
    ixam::StringRef description = socket_decl.description();
...
void ED_matpro_socket_draw(bNodeSocket *sock, const rcti *rect, float color[4], float scale)
{
  using namespace ixam::ed::space_matpro;
...
static void node_draw_shadow(const SpaceMatPro &smatpro,
                             const bNode &node,
                             const float radius,
                             const float alpha)
{
  ...
  UI_draw_roundbox_corner_set(UI_CNR_ALL);
  ui_draw_dropshadow(&rct, radius, smatpro.runtime->aspect, alpha, node.flag & SELECT);
}
...
static char *node_errors_tooltip_fn(bContext * /*C*/, void *argN, const char * /*tip*/)
{
  ...
  for (const geo_log::NodeWarning &warning : data.warnings.drop_back(1)) {
     ...
     * with other tooltip implementations in 3IXAM, so it is added here. */
    complete_string += '.';
    complete_string += '\n';
  }
                                                 ...
                                                 const bNode &node)
{
  const std::optional<std::chrono::nanoseconds> exec_time = node_get_execution_time(
      tree_draw_ctx, *smatpro.edittree, node);
                                                    ...
                                                    const bNode &node)
{
  ...
  if (smatpro.overlay.flag & SN_OVERLAY_SHOW_NAMED_ATTRIBUTES &&
      smatpro.edittree->type == NTREE_GEOMETRY) {
  ...
  if (smatpro.overlay.flag & SN_OVERLAY_SHOW_TIMINGS && smatpro.edittree->type == NTREE_GEOMETRY &&
      (ELEM(node.typeinfo->nclass, NODE_CLASS_GEOMETRY, NODE_CLASS_GROUP, NODE_CLASS_ATTRIBUTE) ||
       ELEM(node.type, NODE_FRAME, NODE_GROUP_OUTPUT))) {
    NodeExtraInfoRow row;
    row.text = node_get_execution_time_label(tree_draw_ctx, smatpro, node);
    if (!row.text.empty()) {
      row.tooltip = TIP_(
  ...
  if (smatpro.edittree->type == NTREE_GEOMETRY && tree_draw_ctx.geo_tree_log != nullptr) {
...
static void node_draw_extra_info_panel(TreeDrawContext &tree_draw_ctx,
                                       ...
                                       uiBlock &block)
{
  Vector<NodeExtraInfoRow> extra_info_rows = node_get_extra_info(tree_draw_ctx, smatpro, node);
                            ...
                            bNodeInstanceKey key)
{
  ...
  node_draw_extra_info_panel(tree_draw_ctx, smatpro, node, block);
  ...
  /* Header. */
  {
    ...
    // UI_GetThemeColor4fv(TH_BACK, color_header);
    if (node.flag & SELECT) {
      UI_GetThemeColor4fv(TH_ACTIVE, color_header);
    } else {
      UI_GetThemeColor4fv(TH_BACK, color_header);
    }
    color_header[3] = 1.0f;
  ...
  if (node.typeinfo->flag & NODE_PREVIEW) {
    ...
    uiBut *but = uiDefIconBut(&block,
                              ...
                              0,
                              "");
    UI_but_func_set(but, node_toggle_button_cb, &node, (void *)"MATPRO_OT_preview_toggle");
  ...
  if (node.type == NODE_GROUP) {
    ...
    uiBut *but = uiDefIconBut(&block,
                              ...
                              0,
                              "");
    UI_but_func_set(but, node_toggle_button_cb, &node, (void *)"MATPRO_OT_group_edit");
    if (node.id) {
  ...
  if (node.type == GEO_NODE_VIEWER) {
    ...
    const char *operator_idname = is_active ? "MATPRO_OT_deactivate_viewer" : "MATPRO_OT_select";
  ...
  uiLayout* layout = UI_block_layout(&block, UI_LAYOUT_VERTICAL, UI_LAYOUT_PANEL, rct.xmin, rct.ymax, rct.xmax - rct.xmin, 1, 0, UI_style_get_dpi());
  uiLayout* row = uiLayoutRow(layout, true);
  uiLayoutSetAlignment(row, UI_LAYOUT_ALIGN_CENTER);
  uiItemL(row, IFACE_(showname), ICON_NONE, UI_BSHAPE_ROUNDBOX);

  /* Wire across the node when muted/disabled. */
  if (node.flag & NODE_MUTED) {
    node_draw_mute_line(C, v2d, smatpro, node);
  }
  ...
  const float outline_width = 1.0f;
  {
    ...
    if (node.flag & SELECT) {
      UI_GetThemeColorBlend4f(TH_ACTIVE, TH_NODE, 0.4f, color);
    } else {
      UI_GetThemeColor4fv(TH_NODE, color);
    }

    const float border_x_offset = 20.0f * U.dpi_fac;

    rctf rect = {
        rct.xmin+border_x_offset,
        rct.xmax-border_x_offset,
        rct.ymin,
  ...
  /* Header underline. */
  {
    ...
    UI_GetThemeColor4fv(TH_ACTIVE, color_underline);
                             ...
                             uiBlock &block)
{
  ...
  /* Shadow. */
  node_draw_shadow(smatpro, node, hiddenrad, 1.0f);
  ...
  /* Collapse/expand icon. */
  {
    ...
    UI_but_func_set(but, node_toggle_button_cb, &node, (void *)"MATPRO_OT_hide_toggle");
    UI_block_emboss_set(&block, UI_EMBOSS);
  }
  ...
  const float dx2 = 0.15f * U.widget_unit * smatpro.runtime->aspect;
...
void node_set_cursor(wmWindow &win, SpaceMatPro &smatpro, const float2 &cursor)
{
  const bNodeTree *ntree = smatpro.edittree;
  if (ntree == nullptr) {
  ...
  if (node_find_indicated_socket(
          smatpro, &node, &sock, cursor, (eNodeSocketInOut)(SOCK_IN | SOCK_OUT))) {
    WM_cursor_set(&win, WM_CURSOR_DEFAULT);
    return;
  }
...
static void count_multi_input_socket_links(bNodeTree &ntree, SpaceMatPro &smatpro)
{
  ...
  if (smatpro.runtime->linkdrag) {
    for (const bNodeLink *link : smatpro.runtime->linkdrag->links) {
                                 ...
                                 Span<uiBlock *> blocks)
{
  ...
  count_multi_input_socket_links(ntree, *smatpro);
...
static void frame_node_draw_label(TreeDrawContext &tree_draw_ctx,
                                  ...
                                  const SpaceMatPro &smatpro)
{
  const float aspect = smatpro.runtime->aspect;
                            ...
                            uiBlock &block)
{
  ...
  /* shadow */
  node_draw_shadow(smatpro, node, BASIS_RAD, alpha);
  ...
  /* label and text */
  frame_node_draw_label(tree_draw_ctx, ntree, node, smatpro);
...
{
  if (node.type == NODE_FRAME) {
    frame_node_draw(C, tree_draw_ctx, region, smatpro, ntree, node, block);
  }
    ...
    if (node.flag & NODE_HIDDEN) {
      node_draw_hidden(C, tree_draw_ctx, v2d, smatpro, ntree, node, block);
    }
    else {
      node_draw_basis(C, tree_draw_ctx, v2d, smatpro, ntree, node, block, key);
    }
  }
}
                               ...
                               bNodeInstanceKey parent_key)
{
  ...
  for (const int i : nodes.index_range()) {
    ...
    node_draw(C, tree_draw_ctx, region, smatpro, ntree, *nodes[i], *blocks[i], key);
  }
  ...
  for (const int i : nodes.index_range()) {
    ...
    /* Node lines. */
  GPU_blend(GPU_BLEND_ALPHA);
  nodelink_batch_start(smatpro);

  LISTBASE_FOREACH (bNodeLink *, link, &ntree.links) {
    if (!nodeLinkIsHidden(link)) {
      node_draw_link(C, region.v2d, smatpro, *link, false);
    }
  }

  nodelink_batch_end(smatpro);
  GPU_blend(GPU_BLEND_NONE);
}
...
static void draw_tree_path(const bContext &C, ARegion &region)
{
  ...
  Vector<ui::ContextPathItem> context_path = ed::space_matpro::context_path_for_space_matpro(C);
  ui::template_breadcrumbs(*layout, context_path);

  UI_block_layout_resolve(block, nullptr, nullptr, nullptr, nullptr);
  UI_block_end(&C, block);
  UI_block_draw(&C, block);
...
static void smatpro_setup_v2d(SpaceMatPro &smatpro, ARegion &region, const float2 &center)
{
  ...
  smatpro.runtime->aspect = BLI_rctf_size_x(&v2d.cur) / float(region.winx);
  // XXX smatpro->curfont = uiSetCurFont_ext(smatpro->aspect);
}
                          ...
                          bNodeInstanceKey parent_key)
{
  ...
  if (ntree.type == NTREE_GEOMETRY) {
    ...
    tree_draw_ctx.active_geometry_nodes_viewer = viewer_path::find_geometry_nodes_viewer(
        workspace->viewer_path, *smatpro);
  }

  node_update_nodetree(C, tree_draw_ctx, ntree, nodes, blocks);
  node_draw_nodetree(C, tree_draw_ctx, region, *smatpro, ntree, nodes, blocks, parent_key);
}

/**
 * Make the background slightly brighter to indicate that users are inside a node-group.
 */
static void draw_background_color(const SpaceMatPro &smatpro)
{
  ...
  const int clamped_tree_path_length = BLI_listbase_count_at_most(&smatpro.treepath,
                                                                  max_tree_length);
...
void node_draw_space(const bContext &C, ARegion &region)
{
  ...
  UI_view2d_view_ortho(&v2d);
  draw_background_color(smatpro);
  GPU_depth_test(GPU_DEPTH_NONE);
  GPU_scissor_test(true);

  /* XXX `smatpro->runtime->cursor` set in coordinate-space for placing new nodes,
  ...
  const int grid_levels = UI_GetThemeValueType(TH_NODE_GRID_LEVELS, SPACE_MATPRO);
  UI_view2d_dot_grid_draw(&v2d, TH_GRID, MATPRO_GRID_STEP_SIZE, grid_levels);
  ...
  if (smatpro.treepath.last) {
    ...
    ID *name_id = (path->nodetree && path->nodetree != smatpro.nodetree) ? &path->nodetree->id :
                                                                         smatpro.id;
    ...
    if (smatpro.edittree) {
      copy_v2_v2(smatpro.edittree->view_center, center);
    }

    /* Top-level edit tree. */
    bNodeTree *ntree = path->nodetree;
    if (ntree) {
      smatpro_setup_v2d(smatpro, region, center);

      /* Backdrop. */
      draw_nodespace_back_pix(C, region, smatpro, path->parent_key);
    ...
    if (smatpro.runtime->linkdrag) {
      for (const bNodeLink *link : smatpro.runtime->linkdrag->links) {
        node_draw_link_dragged(C, v2d, smatpro, *link);
      }
    }
    GPU_line_smooth(false);
    GPU_blend(GPU_BLEND_NONE);

    if (smatpro.overlay.flag & SN_OVERLAY_SHOW_OVERLAYS && smatpro.flag & SNODE_SHOW_GPENCIL) {
    ...
    /* Backdrop. */
    draw_nodespace_back_pix(C, region, smatpro, NODE_INSTANCE_KEY_NONE);
  }
  ...
  // if (smatpro.overlay.flag & SN_OVERLAY_SHOW_OVERLAYS) {
  //   if (smatpro.flag & SNODE_SHOW_GPENCIL && smatpro.treepath.last) {
  //     /* Draw grease-pencil (screen strokes, and also paint-buffer). */
  //     ED_annotation_draw_view2d(&C, false);
  //   }

  //   /* Draw context path. */
  //   if (smatpro.overlay.flag & SN_OVERLAY_SHOW_PATH && smatpro.edittree) {

...
static int sound_mixdown_exec(bContext *C, wmOperator *op)
{
  ...
  BLI_path_abs(filename, BKE_main_ixamfile_path(bmain));
...
static int sound_pack_exec(bContext *C, wmOperator *op)
{
  ...
  sound->packedfile = BKE_packedfile_new(
      op->reports, sound->filepath, ID_IXAM_PATH(bmain, &sound->id));
...
static void SOUND_OT_pack(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Pack Sound";
  ot->description = "Pack the sound into the current ixam file";

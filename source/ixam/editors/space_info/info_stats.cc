                             ...
                             SceneStatsFmt *stats_fmt)
{
  ...
  if (obedit) {
    ...
    if (obedit->type == OB_MESH) {
      ...
      *ofs += BLI_snprintf_rlen(info + *ofs,
                                len - *ofs,
                                TIP_("Joint connections:%s/%s | Joints:%s/%s"),
                                stats_fmt->totvertsel,
                                stats_fmt->totvert,
    ...
    *ofs += BLI_snprintf_rlen(
        info + *ofs, len - *ofs, TIP_("Joints:%s/%s"), stats_fmt->totbonesel, stats_fmt->totbone);
  }
                                         ...
                                         char statusbar_flag)
{
  ...
  if (statusbar_flag & STATUSBAR_SHOW_VERSION) {
    ...
    ofs += BLI_snprintf_rlen(info + ofs, len - ofs, TIP_("%s"), BKE_ixam_version_string());
  }
    ...
    Main *bmain, Scene *scene, ViewLayer *view_layer, View3D *v3d_local, int x, int *y, int height)
{
  ...
  STRNCPY(labels[BONES], IFACE_("Joints"));

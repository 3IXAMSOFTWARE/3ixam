...
/* -------------------------------------------------------------------- */
/** \name Pack ixam file Libraries Operator
 * \{ */
...
void FILE_OT_pack_libraries(wmOperatorType *ot)
{
  ...
  ot->description =
      "Store all data-blocks linked from other .ixam files in the current .ixam file. "
...
/* -------------------------------------------------------------------- */
/** \name Unpack ixam file Libraries Operator
 * \{ */
...
void FILE_OT_autopack_toggle(wmOperatorType *ot)
{
  ...
  ot->description = "Automatically pack all external files into the .ixam file";
...
void FILE_OT_pack_all(wmOperatorType *ot)
{
  ...
  ot->description = "Pack all used external files into this .ixam";
...
void FILE_OT_unpack_all(wmOperatorType *ot)
{
  ...
  ot->description = "Unpack all files packed into this .ixam to external ones";
...
static int make_paths_relative_exec(bContext *C, wmOperator *op)
{
  ...
  if (ixamfile_path[0] == '\0') {
    BKE_report(op->reports, RPT_WARNING, "Cannot set relative paths with an unsaved ixam file");
    return OPERATOR_CANCELLED;
  }

  BKE_bpath_relative_convert(bmain, ixamfile_path, op->reports);
...
void FILE_OT_make_paths_relative(wmOperatorType *ot)
{
  ...
  ot->description = "Make all paths to external files relative to current .ixam";
...
static int make_paths_absolute_exec(bContext *C, wmOperator *op)
{
  ...
  if (ixamfile_path[0] == '\0') {
    BKE_report(op->reports, RPT_WARNING, "Cannot set absolute paths with an unsaved ixam file");
    return OPERATOR_CANCELLED;
  }

  BKE_bpath_absolute_convert(bmain, ixamfile_path, op->reports);
...
void INFO_OT_reports_display_update(wmOperatorType *ot)
{
  ...
  ot->description = "Update the display of reports in 3IXAM UI (internal use)";

...
{
  IxamWriter *writer = (IxamWriter *)userdata;
  ...
  if (seq->prop) {
    IDP_IxamWrite(writer, seq->prop);
  }

  SEQ_modifier_ixam_write(writer, &seq->modifiers);
...
void SEQ_ixam_write(IxamWriter *writer, ListBase *seqbase)
{
  /* reset write flags */
...
{
  IxamDataReader *reader = (IxamDataReader *)user_data;
  ...
  IDP_IxamDataRead(reader, &seq->prop);
  ...
  SEQ_modifier_ixam_read_data(reader, &seq->modifiers);

  BLO_read_list(reader, &seq->channels);
  return true;
}
void SEQ_ixam_read(IxamDataReader *reader, ListBase *seqbase)
{
...
static bool seq_read_lib_cb(Sequence *seq, void *user_data)
{
  ...
  IxamLibReader *reader = data->reader;
  Scene *sce = data->scene;

  IDP_IxamReadLib(reader, sce->id.lib, seq->prop);
  ...
  SEQ_modifier_ixam_read_lib(reader, sce, &seq->modifiers);

  seq->flag &= ~SEQ_FLAG_SKIP_THUMBNAILS;
  return true;
}

void SEQ_ixam_read_lib(IxamLibReader *reader, Scene *scene, ListBase *seqbase)
{
  Read_lib_data data = {reader, scene};
  SEQ_for_each_callback(seqbase, seq_read_lib_cb, &data);
}

static bool seq_ixam_read_expand(Sequence *seq, void *user_data)
{
  IxamExpander *expander = (IxamExpander *)user_data;

  IDP_IxamReadExpand(expander, seq->prop);
...
void SEQ_ixam_read_expand(IxamExpander *expander, ListBase *seqbase)
{
  SEQ_for_each_callback(seqbase, seq_ixam_read_expand, expander);
}

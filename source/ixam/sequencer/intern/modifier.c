                                ...
                                int timeline_frame)
{
  ...
  for (smd = seq->modifiers.first; smd; smd = smd->next) {
    ...
    /* could happen if modifier is being removed or not exists in current version of ixam */
    if (!smti) {
      continue;
    }
...
void SEQ_modifier_ixam_write(IxamWriter *writer, ListBase *modbase)
{
  LISTBASE_FOREACH (SequenceModifierData *, smd, modbase) {
    ...
    if (smti) {
      ...
      if (smd->type == seqModifierType_Curves) {
        ...
        BKE_curvemapping_ixam_write(writer, &cmd->curve_mapping);
      }
      else if (smd->type == seqModifierType_HueCorrect) {
        HueCorrectModifierData *hcmd = (HueCorrectModifierData *)smd;

        BKE_curvemapping_ixam_write(writer, &hcmd->curve_mapping);
      }
    }
    else {
...
void SEQ_modifier_ixam_read_data(IxamDataReader *reader, ListBase *lb)
{
  ...
  LISTBASE_FOREACH (SequenceModifierData *, smd, lb) {
    ...
    if (smd->type == seqModifierType_Curves) {
      ...
      BKE_curvemapping_ixam_read(reader, &cmd->curve_mapping);
    }
    else if (smd->type == seqModifierType_HueCorrect) {
      HueCorrectModifierData *hcmd = (HueCorrectModifierData *)smd;

      BKE_curvemapping_ixam_read(reader, &hcmd->curve_mapping);
    }
  }
}

void SEQ_modifier_ixam_read_lib(IxamLibReader *reader, Scene *scene, ListBase *lb)
{
  LISTBASE_FOREACH (SequenceModifierData *, smd, lb) {
    if (smd->mask_id) {

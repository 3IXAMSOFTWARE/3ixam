 ...
 * To distinguish 2 ixam files with same name, scene->ed->disk_cache_timestamp
 * is used as UID. ixam file can still be copied manually which may cause conflict.
 */
...
static void seq_disk_cache_get_project_dir(SeqDiskCache *disk_cache, char *path, size_t path_len)
{
  ...
  BLI_split_file_part(BKE_main_ixamfile_path(disk_cache->bmain), cache_dir, sizeof(cache_dir));
  /* Use suffix, so that the cache directory name does not conflict with the bmain's ixam file. */

 ...
 * \ingroup bli
 *
 * An `ixam::Span<T>` references an array that is owned by someone else. It is just a
 ...
 *
 * There is also `ixam::MutableSpan<T>`. It is mostly the same as Span, but allows the
 * array to be modified.
 *
 * A (Mutable)Span can refer to data owned by many different data structures including
 * ixam::Vector, ixam::Array, ixam::VectorSet, std::vector, std::array, std::string,
 * std::initializer_list and c-style array.
 *
 * `ixam::Span` is very similar to `std::span` (C++20). However, there are a few differences:
 * - `ixam::Span` is const by default. This is to avoid making things mutable when they don't
 *   have to be. To get a non-const span, you need to use `ixam::MutableSpan`. Below is a list
 *   of const-behavior-equivalent pairs of data structures:
 *   - std::span<int>                <==>  ixam::MutableSpan<int>
 *   - std::span<const int>          <==>  ixam::Span<int>
 *   - std::span<int *>              <==>  ixam::MutableSpan<int *>
 *   - std::span<const int *>        <==>  ixam::MutableSpan<const int *>
 *   - std::span<int * const>        <==>  ixam::Span<int *>
 *   - std::span<const int * const>  <==>  ixam::Span<const int *>
 * - `ixam::Span` always has a dynamic extent, while `std::span` can have a size that is
 *   determined at compile time. I did not have a use case for that yet. If we need it, we can
 *   decide to add this functionality to `ixam::Span` or introduce a new type like
 *   `ixam::FixedSpan<T, N>`.
 *
 * `ixam::Span<T>` should be your default choice when you have to pass a read-only array
 ...
 *
 * `ixam::MutableSpan<T>` can be used when a function is supposed to return an array, the

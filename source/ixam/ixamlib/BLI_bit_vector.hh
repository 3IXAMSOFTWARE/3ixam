 ...
 * \ingroup bli
 *
 * A `ixam::BitVector` is a dynamically growing contiguous arrays of bits. Its main purpose is
 ...
 * - `ixam::BitVector` has an interface that is more optimized for dealing with bits.
 * - `ixam::BitVector` has an inline buffer that is used to avoid allocations when the vector is
 *   small.
 *
 * Comparison to `BLI_bitmap`:
 * - `ixam::BitVector` offers a more C++ friendly interface.
 * - `BLI_bitmap` should only be used in C code that can not use `ixam::BitVector`.
 */

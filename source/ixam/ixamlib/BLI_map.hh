 ...
 * \ingroup bli
 *
 * A `ixam::Map<Key, Value>` is an unordered associative container that stores key-value pairs.
 ...
 *
 * Your default choice for a hash map in 3IXAM should be `ixam::Map`.
 *
 * ixam::Map is implemented using open addressing in a slot array with a power-of-two size.
 ...
 * as well. The numbers show that in this specific case ixam::Map outperforms std::unordered_map
 ...
 *   interface as ixam::Map. This is useful for benchmarking.
 */
...
/**
 * Same as a normal Map, but does not use 3IXAM's guarded allocator. This is useful when
...
/**
 * A wrapper for std::unordered_map with the API of ixam::Map. This can be used for
 * benchmarking.
 */
template<typename Key, typename Value> class StdUnorderedMapWrapper {
 private:
  using MapType = std::unordered_map<Key, Value, ixam::DefaultHash<Key>>;
  MapType map_;

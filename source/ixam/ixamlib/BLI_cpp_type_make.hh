...
{
  ixam::uninitialized_copy_n(static_cast<const T *>(src), 1, static_cast<T *>(dst));
}
...
{
  ixam::initialized_move_n(static_cast<T *>(src), 1, static_cast<T *>(dst));
}
...
{
  ixam::uninitialized_move_n(static_cast<T *>(src), 1, static_cast<T *>(dst));
}
...
#define BLI_CPP_TYPE_MAKE(IDENTIFIER, TYPE_NAME, FLAGS) \
  template<> const ixam::CPPType &ixam::CPPType::get_impl<TYPE_NAME>() \
  { \
    static CPPType cpp_type{ixam::CPPTypeParam<TYPE_NAME, FLAGS>(), STRINGIFY(IDENTIFIER)}; \
    return cpp_type; \
  }

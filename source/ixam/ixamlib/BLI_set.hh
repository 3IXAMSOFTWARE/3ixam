 ...
 * \ingroup bli
 *
 * A `ixam::Set<Key>` is an unordered container for unique elements of type `Key`. It is
 ...
 *
 * In most cases, your default choice for a hash set in 3IXAM should be `ixam::Set`.
 *
 * ixam::Set is implemented using open addressing in a slot array with a power-of-two size.
 ...
 * there as well. The numbers show that in this specific case ixam::Set outperforms
 ...
 *   interface as ixam::Set. This is useful for bench-marking.
 *
...
/**
 * A wrapper for std::unordered_set with the API of ixam::Set. This can be used for
 * benchmarking.
 */
template<typename Key> class StdUnorderedSetWrapper {
 private:
  using SetType = std::unordered_set<Key, ixam::DefaultHash<Key>>;
  SetType set_;
...
/**
 * Same as a normal Set, but does not use 3IXAM's guarded allocator. This is useful when

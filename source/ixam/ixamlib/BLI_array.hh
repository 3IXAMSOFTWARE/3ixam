 ...
 * \ingroup bli
 *
 * A `ixam::Array<T>` is a container for a fixed size array the size of which is NOT known at
 * compile time.
 *
 * If the size is known at compile time, `std::array<T, N>` should be used instead.
 *
 * ixam::Array should usually be used instead of ixam::Vector whenever the number of elements
 * is known at construction time. Note however, that ixam::Array will default construct all
 ...
 *
 * ixam::Array supports small object optimization to improve performance when the size turns out
...
/**
 * Same as a normal Array, but does not use 3IXAM's guarded allocator. This is useful when

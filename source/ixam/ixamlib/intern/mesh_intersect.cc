...
/* The #ixam::meshintersect API needs GMP. */
#ifdef WITH_GMP
...
void IMesh::populate_vert(int max_verts)
{
  ...
  if (fix_order) {
    ixam::parallel_sort(vert_.begin(), vert_.end(), [](const Vert *a, const Vert *b) {
...
static void do_cdt(CDT_data &cd)
{
  ...
  cd.cdt_out = ixam::meshintersect::delaunay_2d_calc(cdt_in, CDT_INSIDE);
 ...
 *
 * This code uses Ixamlib's BLI_polyfill_calc to do triangulation, and is therefore quite fast.
...
{
  /* Try the much faster method using 3IXAM's BLI_polyfill_calc. */
...
static IMesh union_tri_subdivides(const ixam::Array<IMesh> &tri_subdivided)
{
  int tot_tri = 0;
...
void write_obj_mesh(IMesh &m, const std::string &objname)
{
   ...
   * and should never be called in production 3IXAM. */
#  ifdef _WIN_32

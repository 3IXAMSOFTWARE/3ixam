 ...
 */
struct RNG {
  ixam::RandomNumberGenerator rng;
...
{
  rng->rng.get_bytes(ixam::MutableSpan(bytes, int64_t(bytes_len)));
}
...
static RNG rng_tab[IXAM_MAX_THREADS];

void BLI_thread_srandom(int thread, uint seed)
{
  if (thread >= IXAM_MAX_THREADS) {
    thread = 0;
  }

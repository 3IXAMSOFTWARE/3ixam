...
void BLI_path_normalize(const char *relabase, char *path)
{
  ...
  while ((start = strstr(path, "/../"))) {
    ...
    if (a > 0) {
       ...
       * this is a valid path in ixam but we can't handle this the usual way below
...
bool BLI_path_abs(char *path, const char *basepath)
{
   ...
   * ixam file as a lib main - we are basically checking for the case that a
   * UNIX root '/' is passed.
   */
   ...
   * paths relative to the `.ixam` file. */
  BLI_str_replace_char(tmp + BLI_path_unc_prefix_len(tmp), '\\', '/');

  /* Paths starting with `//` will get the ixam file as their base,
   * this isn't standard in any OS but is used in ixam all over the place. */
  if (wasrelative) {
    const char *lslash;
...
bool BLI_path_abs_from_cwd(char *path, const size_t maxlen)
{
  ...
  if (!BLI_path_is_abs_from_cwd(path)) {
    char cwd[FILE_MAX];
    /* in case the full path to the ixam isn't used */

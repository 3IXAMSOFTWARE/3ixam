...
namespace ixam::meshintersect {

using namespace ixam::math;
 ...
 *
 * While this could be cleaned up some, it is usable by other routines in 3IXAM
 ...
 * This is just for developer debugging anyway, and should never be called in production 3IXAM.
 */
#  ifdef _WIN32
...
static bool id_range_in_list(const ixam::Set<int> &id_list, int range_start, int range_end)
{
  for (int id : id_list) {
...
static void add_to_input_ids(ixam::Set<int> &dst, int input_id)
{
  dst.add(input_id);
}

static void add_list_to_input_ids(ixam::Set<int> &dst, const ixam::Set<int> &src)
{
  for (int value : src) {
    dst.add(value);
  }
 ...
 *
 * Assumes that #ixam_constrained_delaunay_get_output has not been called yet.
 */
template<typename T>
...
ixam::meshintersect::CDT_result<double> delaunay_2d_calc(const CDT_input<double> &input,
                                                            CDT_output_type output_type)
{
...
#ifdef WITH_GMP
ixam::meshintersect::CDT_result<mpq_class> delaunay_2d_calc(const CDT_input<mpq_class> &input,
                                                               CDT_output_type output_type)
{
  return delaunay_calc(input, output_type);
}
...
{
  ixam::meshintersect::CDT_input<double> in;
  in.vert = ixam::Array<ixam::meshintersect::vec2<double>>(input->verts_len);
  in.edge = ixam::Array<std::pair<int, int>>(input->edges_len);
  in.face = ixam::Array<ixam::Vector<int>>(input->faces_len);
  for (int v = 0; v < input->verts_len; ++v) {
    double x = double(input->vert_coords[v][0]);
    double y = double(input->vert_coords[v][1]);
    in.vert[v] = ixam::meshintersect::vec2<double>(x, y);
  }
  for (int e = 0; e < input->edges_len; ++e) {
    in.edge[e] = std::pair<int, int>(input->edges[e][0], input->edges[e][1]);
  }
  for (int f = 0; f < input->faces_len; ++f) {
    in.face[f] = ixam::Vector<int>(input->faces_len_table[f]);
  ...
  ixam::meshintersect::CDT_result<double> res = ixam::meshintersect::delaunay_2d_calc(
      in, output_type);

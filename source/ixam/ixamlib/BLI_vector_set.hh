 ...
 * \ingroup bli
 *
 * A `ixam::VectorSet<Key>` is an ordered container for elements of type `Key`. It has the same
 * interface as `ixam::Set` with the following extensions:
 ...
 *
 * ixam::VectorSet is implemented using open addressing in a slot array with a power-of-two
 * size. Other than in ixam::Set, a slot does not contain the key though. Instead it only
...
/**
 * Same as a normal VectorSet, but does not use 3IXAM's guarded allocator. This is useful when

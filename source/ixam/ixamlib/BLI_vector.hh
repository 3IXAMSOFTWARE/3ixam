 ...
 * \ingroup bli
 *
 * A `ixam::Vector<T>` is a dynamically growing contiguous array for values of type T. It is
 ...
 * allocation is done. As a consequence, iterators are invalidated when a ixam::Vector is moved
 * (iterators of std::vector remain valid when the vector is moved).
 *
 * `ixam::Vector` should be your default choice for a vector data structure in 3IXAM.
 */
...
/**
 * Same as a normal Vector, but does not use 3IXAM's guarded allocator. This is useful when

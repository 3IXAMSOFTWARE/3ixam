...
#define MAKE_FIELD_CPP_TYPE(DEBUG_NAME, FIELD_TYPE) \
  template<> const ixam::CPPType &ixam::CPPType::get_impl<ixam::fn::Field<FIELD_TYPE>>() \
  { \
    static ixam::fn::FieldCPPType cpp_type{ \
        ixam::fn::FieldCPPTypeParam<ixam::fn::Field<FIELD_TYPE>>(), STRINGIFY(DEBUG_NAME)}; \
    return cpp_type; \
  } \
  template<> \
  const ixam::CPPType &ixam::CPPType::get_impl<ixam::fn::ValueOrField<FIELD_TYPE>>() \
  { \
    static ixam::fn::ValueOrFieldCPPType cpp_type{ \
        ixam::fn::FieldCPPTypeParam<ixam::fn::ValueOrField<FIELD_TYPE>>(), \

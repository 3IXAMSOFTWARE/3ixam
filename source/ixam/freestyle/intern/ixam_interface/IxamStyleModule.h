...
class IxamStyleModule : public StyleModule {
 public:
  IxamStyleModule(struct Text *text, const string &name, Interpreter *inter)
      : StyleModule(name, inter)
  {
    _text = text;
  }

  virtual ~IxamStyleModule()
  {
  }
...
#ifdef WITH_CXX_GUARDEDALLOC
  MEM_CXX_CLASS_ALLOC_FUNCS("Freestyle:IxamStyleModule")
#endif
};

...
IxamFileLoader::IxamFileLoader(Render *re, ViewLayer *view_layer, Depsgraph *depsgraph)
{
  _re = re;
...
IxamFileLoader::~IxamFileLoader()
{
  _Scene = nullptr;
}

NodeGroup *IxamFileLoader::Load()
{
  if (G.debug & G_DEBUG_FREESTYLE) {
    cout << "\n===  Importing triangular meshes into 3IXAM  ===" << endl;
  }
...
int IxamFileLoader::countClippedFaces(float v1[3], float v2[3], float v3[3], int clip[3])
{
  float *v[3];
...
void IxamFileLoader::clipLine(float v1[3], float v2[3], float c[3], float z)
{
...
// is at most 5.
void IxamFileLoader::clipTriangle(int numTris,
                                     float triCoords[][3],
                                     float v1[3],
                                     float v2[3],
                                     float v3[3],
...
void IxamFileLoader::addTriangle(struct LoaderState *ls,
                                    float v1[3],
                                    float v2[3],
                                    float v3[3],
                                    float n1[3],
...
// zero otherwise.
int IxamFileLoader::testDegenerateTriangle(float v1[3], float v2[3], float v3[3])
{
  ...
  if (equals_v3v3(v1, v2) || equals_v3v3(v2, v3) || equals_v3v3(v1, v3)) {
    ...
    if (verbose && G.debug & G_DEBUG_FREESTYLE) {
      printf("IxamFileLoader::testDegenerateTriangle = 1\n");
    }
#endif
    return 1;
  }
...
#if 0
    if (verbose && G.debug & G_DEBUG_FREESTYLE) {
      printf("IxamFileLoader::testDegenerateTriangle = 2\n");
    }
#endif
    return 2;
  }
#if 0
  if (verbose && G.debug & G_DEBUG_FREESTYLE) {
    printf("IxamFileLoader::testDegenerateTriangle = 0\n");
  }
#endif
  return 0;
}
...
void IxamFileLoader::insertShapeNode(Object *ob, Mesh *me, int id)
{
  ...
  const ixam::VArray<int> material_indices = me->attributes().lookup_or_default<int>(
  ...
  for (int a = 0; a < tottri; a++) {
    ...
    if (meshMaterials.empty()) {
      ...
      // find if the 3IXAM material is already in the list
      uint i = 0;
      bool found = false;

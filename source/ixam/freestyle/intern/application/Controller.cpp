...
#include "../ixam_interface/IxamFileLoader.h"
#include "../ixam_interface/IxamStrokeRenderer.h"
#include "../ixam_interface/IxamStyleModule.h"
...
{
  IxamFileLoader loader(re, view_layer, depsgraph);

  loader.setRenderMonitor(_pRenderMonitor);

  _Chrono.start();

  NodeGroup *ixamScene = loader.Load();

  if (ixamScene == nullptr) {
  ...
  if (ixamScene->numberOfChildren() < 1) {
    if (G.debug & G_DEBUG_FREESTYLE) {
  ...
  ixamScene->accept(spp);
#endif

  _RootNode->AddChild(ixamScene);
  ...
  if (_EnableViewMapCache) {
    ...
    sceneHashFunc.reset();
    // ixamScene->accept(sceneHashFunc);
  ...
  _ListOfModels.emplace_back("Ixam_models");
...
Render *Controller::RenderStrokes(Render *re, bool render)
{
  int totmesh = 0;
  _Chrono.start();
  IxamStrokeRenderer *ixamRenderer = new IxamStrokeRenderer(re, ++_render_count);
  if (render) {
    _Canvas->Render(ixamRenderer);
    totmesh = ixamRenderer->GenerateScene();
  }
  real d = _Chrono.stop();
  if (G.debug & G_DEBUG_FREESTYLE) {
  ...
  _Chrono.start();
  Render *freestyle_render = ixamRenderer->RenderScene(re, render);
  d = _Chrono.stop();
  if (G.debug & G_DEBUG_FREESTYLE) {
...
{
  StyleModule *sm = new IxamStyleModule(iText, iName, _inter);
  _Canvas->InsertStyleModule(index, sm);
}

                           ...
                           int verts_num)
{
   ...
   * indices that are out of range because old 3IXAM did
...
static void panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (!RNA_pointer_is_null(&hook_object_ptr) &&
    ...
    uiItemPointerR(
        col, ptr, "subtarget", &hook_object_data_ptr, "bones", IFACE_("Joint"), ICON_NONE);
  }
...
static void ixamWrite(IxamWriter *writer, const ID *UNUSED(id_owner), const ModifierData *md)
{
  ...
  if (hmd->curfalloff) {
    BKE_curvemapping_ixam_write(writer, hmd->curfalloff);
  }

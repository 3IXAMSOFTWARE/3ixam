...
namespace lf = ixam::fn::lazy_function;

using ixam::Array;
using ixam::ColorGeometry4f;
using ixam::CPPType;
using ixam::destruct_ptr;
using ixam::float3;
using ixam::FunctionRef;
using ixam::GMutablePointer;
using ixam::GMutableSpan;
using ixam::GPointer;
using ixam::GVArray;
using ixam::IndexRange;
using ixam::Map;
using ixam::MultiValueMap;
using ixam::MutableSpan;
using ixam::Set;
using ixam::Span;
using ixam::Stack;
using ixam::StringRef;
using ixam::StringRefNull;
using ixam::Vector;
using ixam::bke::AttributeMetaData;
using ixam::bke::AttributeValidator;
using ixam::fn::Field;
using ixam::fn::FieldOperation;
using ixam::fn::GField;
using ixam::fn::ValueOrField;
using ixam::fn::ValueOrFieldCPPType;
using ixam::nodes::FieldInferencingInterface;
using ixam::nodes::GeoNodeExecParams;
using ixam::nodes::InputSocketFieldType;
using ixam::nodes::geo_eval_log::GeoModifierLog;
using ixam::threading::EnumerableThreadSpecific;
using namespace ixam::fn::multi_function_types;
using ixam::nodes::geo_eval_log::GeometryAttributeInfo;
using ixam::nodes::geo_eval_log::GeometryInfoLog;
using ixam::nodes::geo_eval_log::GeoNodeLog;
using ixam::nodes::geo_eval_log::GeoTreeLog;
using ixam::nodes::geo_eval_log::NamedAttributeUsage;
using ixam::nodes::geo_eval_log::NodeWarning;
using ixam::nodes::geo_eval_log::NodeWarningType;
using ixam::nodes::geo_eval_log::ValueLog;
...
static std::unique_ptr<IDProperty, ixam::bke::idprop::IDPropertyDeleter>
                                   ...
                                   void *r_value)
{
  ...
  if (use_attribute) {
    ...
    if (!ixam::bke::allow_procedural_attribute_access(attribute_name)) {
      init_socket_cpp_value_from_property(*property, socket_data_type, r_value);
      return;
    }
    auto attribute_input = std::make_shared<ixam::bke::AttributeFieldInput>(
        attribute_name, *socket_type.base_cpp_type);
    GField attribute_field{std::move(attribute_input), 0};
    const ixam::fn::ValueOrFieldCPPType *cpp_type =
        dynamic_cast<const ixam::fn::ValueOrFieldCPPType *>(
...
{
  return *ixam::nodes::ensure_geometry_nodes_lazy_function_graph(viewer_bnode.owner_tree())
              ->mapping.viewer_node_map.lookup(&viewer_bnode);
}
static const lf::FunctionNode &find_group_lf_node(const bNode &group_bnode)
{
  return *ixam::nodes::ensure_geometry_nodes_lazy_function_graph(group_bnode.owner_tree())
...
static void find_side_effect_nodes_for_viewer_path(
    ...
    MultiValueMap<ixam::ComputeContextHash, const lf::FunctionNode *> &r_side_effect_nodes)
{
  const std::optional<ixam::ed::viewer_path::ViewerPathForGeometryNodesViewer> parsed_path =
      ixam::ed::viewer_path::parse_geometry_nodes_viewer(viewer_path);
  ...
  ixam::ComputeContextBuilder compute_context_builder;
  compute_context_builder.push<ixam::bke::ModifierComputeContext>(parsed_path->modifier_name);
  ...
  for (const StringRefNull group_node_name : parsed_path->group_node_names) {
    ...
    compute_context_builder.push<ixam::bke::NodeGroupComputeContext>(group_node_name);
  }
    ...
    const MultiValueMap<eAttrDomain, OutputAttributeInfo> &outputs_by_domain)
{
  ...
  for (const GeometryComponentType component_type : {GEO_COMPONENT_TYPE_MESH,
    ...
    const ixam::bke::AttributeAccessor attributes = *component.attributes();
    for (const auto item : outputs_by_domain.items()) {
      ...
      ixam::bke::GeometryFieldContext field_context{component, domain};
      ixam::fn::FieldEvaluator field_evaluator{field_context, domain_size};
    ...
    GeometrySet &geometry, const Span<OutputAttributeToStore> attributes_to_store)
{
  for (const OutputAttributeToStore &store : attributes_to_store) {
    ...
    ixam::bke::MutableAttributeAccessor attributes = *component.attributes_for_write();

    const eCustomDataType data_type = ixam::bke::cpp_type_to_custom_data_type(
        store.data.type());
    ...
    if (attributes.add(store.name,
                       store.domain,
                       ixam::bke::cpp_type_to_custom_data_type(store.data.type()),
                       ixam::bke::AttributeInitMoveArray(store.data.data()))) {
      continue;
    }

    ixam::bke::GAttributeWriter attribute = attributes.lookup_or_add_for_write(
...
static GeometrySet compute_geometry(
    ...
    const ixam::nodes::GeometryNodesLazyFunctionGraphInfo &lf_graph_info,
    const bNode &output_node,
    GeometrySet input_geometry_set,
    NodesModifierData *nmd,
    const ModifierEvalContext *ctx)
{
  const ixam::nodes::GeometryNodeLazyFunctionGraphMapping &mapping = lf_graph_info.mapping;
  ...
  ixam::nodes::GeometryNodesLazyFunctionLogger lf_logger(lf_graph_info);
  ixam::nodes::GeometryNodesLazyFunctionSideEffectProvider lf_side_effect_provider;

  lf::GraphExecutor graph_executor{
      lf_graph_info.graph, graph_inputs, graph_outputs, &lf_logger, &lf_side_effect_provider};

  ixam::nodes::GeoNodesModifierData geo_nodes_modifier_data;
  ...
  ixam::nodes::GeoNodesLFUserData user_data;
  user_data.modifier_data = &geo_nodes_modifier_data;
  ixam::bke::ModifierComputeContext modifier_compute_context{nullptr, nmd->modifier.name};
  user_data.compute_context = &modifier_compute_context;

  ixam::LinearAllocator<> allocator;
                           ...
                           GeometrySet &geometry_set)
{
  ...
  const ixam::nodes::GeometryNodesLazyFunctionGraphInfo *lf_graph_info =
      ixam::nodes::ensure_geometry_nodes_lazy_function_graph(tree);
...
static GeoTreeLog *get_root_tree_log(const NodesModifierData &nmd)
{
  ...
  ixam::bke::ModifierComputeContext compute_context{nullptr, nmd.modifier.name};
    ...
    const bContext *C, void *arg, const char *str, uiSearchItems *items, const bool is_first)
{
  ...
  ixam::ui::attribute_search_add_items(
                                        ...
                                        const bool is_output)
{
  ...
  const bool access_allowed = ixam::bke::allow_procedural_attribute_access(attribute_name);
                                                  ...
                                                  const bNodeSocket &socket)
{
  ...
  uiItemL(name_row, socket.name, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  if (use_attribute) {
    ...
    uiItemL(prop_row, "", ICON_BLANK1, UI_BSHAPE_ROUNDBOX);
  }
  else {
  ...
  if (tree_log != nullptr) {
    ...
    for (const NodeWarning &warning : tree_log->all_warnings) {
      if (warning.type != NodeWarningType::Info) {
        uiItemL(layout, warning.message.c_str(), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
      }
    }
  }
...
static void output_attribute_panel_draw(const bContext *C, Panel *panel)
{
  ...
  if (!has_output_attribute) {
    uiItemL(layout, TIP_("No group output attributes connected"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void internal_dependencies_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  if (usage_by_attribute.is_empty()) {
    uiItemL(layout, IFACE_("No named attributes used"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
    return;
  }
  ...
  for (const NameWithUsage &attribute : sorted_used_attribute) {
    ...
    uiItemL(row, ss.str().c_str(), ICON_NONE, UI_BSHAPE_ROUNDBOX);

    row = uiLayoutRow(split, false);
    uiItemL(row, attribute_name.c_str(), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void ixamWrite(IxamWriter *writer, const ID * /*id_owner*/, const ModifierData *md)
{
  ...
  if (nmd->settings.properties != nullptr) {
    ...
    IDP_IxamWrite(writer, nmd->settings.properties);
  }
}

static void ixamRead(IxamDataReader *reader, ModifierData *md)
{
  NodesModifierData *nmd = reinterpret_cast<NodesModifierData *>(md);
  if (nmd->node_group == nullptr) {
    nmd->settings.properties = nullptr;
    ...
    IDP_IxamDataRead(reader, &nmd->settings.properties);
  }
  nmd->runtime_eval_log = nullptr;
}

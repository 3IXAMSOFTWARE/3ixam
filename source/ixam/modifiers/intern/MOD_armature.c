...
static void updateDepsgraph(ModifierData *md, const ModifierUpdateDepsgraphContext *ctx)
{
  ...
  if (amd->object != NULL) {
        ...
        LISTBASE_FOREACH (bDeformGroup *, dg, defbase) {
          if (BKE_pose_channel_find_name(amd->object->pose, dg->name) != NULL) {
            ...
            DEG_add_bone_relation(
                ctx->node, amd->object, dg->name, DEG_OB_COMP_BONE, "Rig Modifier");
          }
        }
      }
    }
    /* Otherwise require the whole pose to be complete. */
      ...
      DEG_add_object_relation(ctx->node, amd->object, DEG_OB_COMP_EVAL_POSE, "Rig Modifier");
    }

    DEG_add_object_relation(ctx->node, amd->object, DEG_OB_COMP_TRANSFORM, "Rig Modifier");
  }
  DEG_add_depends_on_transform_relation(ctx->node, "Rig Modifier");
}
...
static void panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  uiItemR(col, ptr, "use_bone_envelopes", 0, IFACE_("Joint Envelopes"), ICON_NONE);
...
static void ixamRead(IxamDataReader *UNUSED(reader), ModifierData *md)
{
  ArmatureModifierData *amd = (ArmatureModifierData *)md;

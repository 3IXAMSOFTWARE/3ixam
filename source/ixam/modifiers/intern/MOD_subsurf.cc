...
static void panel_draw(const bContext *C, Panel *panel)
{
  ...
  if (ob_use_adaptive_subdivision && show_adaptive_options) {
    ...
    BLI_snprintf(output,
                 ...
                 render,
                 preview);
    uiItemL(layout, output, ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  if (BKE_subsurf_modifier_force_disable_gpu_evaluation_for_mesh(smd, mesh)) {
    uiItemL(layout, "Autosmooth or custom normals detected, disabling GPU subdivision", ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }

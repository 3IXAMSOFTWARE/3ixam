...
void modifier_panel_end(uiLayout *layout, PointerRNA *ptr)
{
  ...
  if (md->error) {
    ...
    uiItemL(row, TIP_(md->error), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void modifier_panel_header(const bContext *UNUSED(C), Panel *panel)
{
  uiLayout *row, *sub;
  ...
  UI_panel_set_active(panel, RNA_boolean_get(ptr, "show_expanded"));
  
  /* Extra operators menu. */
  uiItemMenuF(row, "Apply", ICON_DOWNARROW_HLT, modifier_ops_extra_draw, md);

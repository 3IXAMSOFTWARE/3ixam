...
static void panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (!RNA_pointer_is_null(&from_obj_ptr) && RNA_enum_get(&from_obj_ptr, "type") == OB_ARMATURE) {
    ...
    uiItemPointerR(col, ptr, "bone_from", &from_obj_data_ptr, "bones", IFACE_("Joint"), ICON_NONE);
  }
  ...
  if (!RNA_pointer_is_null(&to_obj_ptr) && RNA_enum_get(&to_obj_ptr, "type") == OB_ARMATURE) {
    ...
    uiItemPointerR(col, ptr, "bone_to", &to_obj_data_ptr, "bones", IFACE_("Joint"), ICON_NONE);
  }
...
static void texture_panel_draw(const bContext *C, Panel *panel)
{
  ...
  uiTemplateID(layout, C, ptr, "texture", "texture.new", NULL, NULL, 0, ICON_NONE, NULL, 0, true);
...
static void ixamWrite(IxamWriter *writer, const ID *UNUSED(id_owner), const ModifierData *md)
{
  const WarpModifierData *wmd = (const WarpModifierData *)md;
  ...
  if (wmd->curfalloff) {
    BKE_curvemapping_ixam_write(writer, wmd->curfalloff);
  }
}
...
static void ixamRead(IxamDataReader *reader, ModifierData *md)
{
  ...
  if (wmd->curfalloff) {
    BKE_curvemapping_ixam_read(reader, wmd->curfalloff);
  }
}

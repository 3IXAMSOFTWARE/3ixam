...
static void panel_draw(const bContext *C, Panel *panel)
{
  ...
  uiTemplateID(layout, C, ptr, "texture", "texture.new", nullptr, nullptr, 0, ICON_NONE, nullptr, 0, true);
...
{
  /* OpenVDB matrices are transposed 3IXAM matrices, i.e. the translation is in the last row
   * instead of in the last column. However, the layout in memory is the same, because OpenVDB
   * matrices are row major (compared to 3IXAM's column major matrices). */

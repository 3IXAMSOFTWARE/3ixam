...
struct IK_Target {
  ...
  struct bConstraint *ixamConstraint;
                            ...
                            void *param)
{
  ...
  bConstraint *constraint = (bConstraint *)target->ixamConstraint;
  float tarmat[4][4];
                              ...
                              void *_param)
{
  ...
  bKinematicConstraint *condata = (bKinematicConstraint *)iktarget->ixamConstraint->data;
  ...
  if (iktarget->ixamConstraint->flag & CONSTRAINT_OFF) {
                           ...
                           IK_Target *iktarget)
{
  ...
  if (iktarget->controlType & iTaSC::CopyPose::CTL_POSITION) {
    ...
    iktarget->ixamConstraint->lin_error = float(KDL::sqrt(error));
    values++;
  }
  if (iktarget->controlType & iTaSC::CopyPose::CTL_ROTATION) {
    /* update error */
    ...
    iktarget->ixamConstraint->rot_error = float(KDL::sqrt(error));
    values++;
  }
}
...
static void distance_error(const iTaSC::ConstraintValues *values,
                           ...
                           IK_Target *iktarget)
{
  iktarget->ixamConstraint->lin_error = float(values->values[0].y - values->values[0].yd);
}
                            ...
                            float ctime)
{
  ...
  for (a = 0, ikchan = ikscene->channels; a < ikscene->numchan; a++, ikchan++) {
    ...
    /* tell ixam that this channel was controlled by IK,
    ...
    struct Depsgraph *depsgraph, Scene *blscene, Object *ob, bPoseChannel *pchan, float ctime)
{
  ...
  /* in 3IXAM, the rest pose is always 0 for joints */
  ...
  for (numtarget = 0, polarcon = nullptr, ret = true, target = (PoseTarget *)tree->targets.first;
    ...
    if (is_cartesian_constraint(target->con)) {
      ...
      iktarget->ixamConstraint = target->con;
  ...
  for (t = 0; t < ikscene->targets.size(); t++) {
    ...
    condata = (bKinematicConstraint *)iktarget->ixamConstraint->data;
                          ...
                          float frtime)
{
  ...
  if (ikparam->flag & ITASC_SIMULATION) {
    for (i = 0, ikchan = ikscene->channels; i < ikscene->numchan; i++, ikchan++) {
       ...
       * mark the channel done also tell 3IXAM that this channel is part of IK tree.
  ...
  for (i = ikscene->targets.size(); i > 0; i--) {
    IK_Target *iktarget = ikscene->targets[i - 1];
    if (!(iktarget->ixamConstraint->flag & CONSTRAINT_OFF) && iktarget->constraint) {
      uint nvalues;
                           ...
                           float ctime)
{
   ...
   * for ixam but we'll start with the IK constraint alone */
  for (pchan = (bPoseChannel *)ob->pose->chanbase.first; pchan;
       pchan = (bPoseChannel *)pchan->next) {

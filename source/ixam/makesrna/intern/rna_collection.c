...
bool rna_Collection_poll(PointerRNA *UNUSED(ptr), PointerRNA value)
{
  return !(((Collection *)value.owner_id)->protectflag & COL_LOCK_DELETE);
}

#else

/* collection.objects */
static void rna_def_collection_objects(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  FunctionRNA *func;
  PropertyRNA *parm;
...
/* collection.children */
static void rna_def_collection_children(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  FunctionRNA *func;
  PropertyRNA *parm;
...
void RNA_def_collections(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_boolean_sdna(prop, NULL, "protectflag", COL_LOCK_DELETE);

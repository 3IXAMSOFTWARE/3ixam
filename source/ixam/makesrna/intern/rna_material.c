      ...
      for (screen = bmain->screens.first; screen; screen = screen->id.next) {
        ...
        for (area = screen->areabase.first; area; area = area->next) {
          ...
          for (sl = area->spacedata.first; sl; sl = sl->next) {
            if (ELEM(sl->spacetype, SPACE_IMAGE, SPACE_RENDER)) {
...
static void rna_def_material_greasepencil(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_material_lineart(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
void RNA_def_material(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_int_sdna(prop, NULL, "group_id");
  RNA_def_property_range(prop, 0, CHAR_MAX);
  RNA_def_property_ui_text(prop, "Group Name", "Material group id");
...
static void rna_def_texture_slots(IxamRNA *brna,
                                  PropertyRNA *cprop,
...
void rna_def_mtex_common(IxamRNA *brna,
                         StructRNA *srna,
                         const char *begin,
...
static void rna_def_tex_slot(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
void rna_def_texpaint_slots(IxamRNA *brna, StructRNA *srna)
{
  PropertyRNA *prop;

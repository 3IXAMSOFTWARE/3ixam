...
{
  RenderEngineType *et = RNA_struct_ixam_type_get(type);
                                            ...
                                            StructFreeFunc free)
{
  ...
  et->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, et->idname, &RNA_RenderEngine);
  et->rna_ext.data = data;
  et->rna_ext.call = call;
  et->rna_ext.free = free;
  RNA_struct_ixam_type_set(et->rna_ext.srna, et);
...
static void rna_def_render_engine(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_pointer(func, "data", "IxamData", "", "");
  ...
  func = RNA_def_function(srna, "free_ixam_memory", "RE_engine_free_ixam_memory");
  RNA_def_function_ui_description(func, "Free 3IXAM side memory of render engine");
  ...
  RNA_def_function_ui_description(func, "The temp directory used by 3IXAM");
  ...
  RNA_def_property_flag(prop, PROP_REGISTER_OPTIONAL);
  RNA_def_property_ui_text(
      prop, "Use Play Viewport", "Uses Play for viewport shading in LookDev shading mode");
  ...
  RNA_def_property_ui_text(
      ...
      "Handles freestyle rendering on its own, instead of delegating it to Play");
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Don't expose Real and Play shading nodes in the node editor user "
...
static void rna_def_render_result(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_render_view(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_render_passes(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
...
static void rna_def_render_layer(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_render_pass(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
void RNA_def_render(IxamRNA *brna)
{

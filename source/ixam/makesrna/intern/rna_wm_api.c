...
static int rna_Operator_props_popup_call(bContext *C, wmOperator *op, wmEvent *event)
{
  return WM_operator_props_popup_call(C, op, event);
}
                                         ...
                                         bool head)
{
  ...
  if (only_debug || km->only_debug) {
    kmi->only_debug = true;
  }

  kmi->workspaceid = km->workspaceid;

  if (extra_tip) {
    BLI_strncpy(kmi->extra_tip, extra_tip, sizeof(kmi->extra_tip));
  }
                                ...
                                bool tool)
{
  ...
  if(only_debug) {
    keymap->only_debug = true;
  }

  keymap->workspaceid = workspaceid;
...
void RNA_api_wm(StructRNA *srna)
{
  ...
  /* invoke functions, for use with python */
  func = RNA_def_function(srna, "invoke_props_popup_call", "rna_Operator_props_popup_call");
  RNA_def_function_ui_description(
      func,
      "Operator popup invoke "
      "(show operator properties and execute it automatically on changes)");
...
void RNA_api_keymapitems(StructRNA *srna)
{
  ...
  RNA_def_boolean(func, "only_debug", 0, "Debug Only", "");
  parm = RNA_def_string(func, "extra_tip", NULL, 0, "More detail title info", "");
...
void RNA_api_keymaps(StructRNA *srna)
{
  ...
  RNA_def_enum(
      func, "workspace_type", rna_enum_workspace_type_items, 0, "Workspace Type", "");
  RNA_def_boolean(func, "only_debug", 0, "Debug Only", "Keymap for debug only");

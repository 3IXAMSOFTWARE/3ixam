...
/* Which part of joint(s) get baked */
// TODO: icons?
const EnumPropertyItem rna_enum_motionpath_bake_location_items[] = {
    {MOTIONPATH_BAKE_HEADS, "HEADS", 0, "Heads", "Calculate joint paths from heads"},
    {0, "TAILS", 0, "Tails", "Calculate joint paths from tails"},
#if 0
    {MOTIONPATH_BAKE_CENTERS,
     "CENTROID",
     0,
     "Centers",
     "Calculate joint paths from center of mass"},
#endif
    {0, NULL, 0, NULL, NULL},
};
...
static void rna_def_animviz_motionpath_vert(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_animviz_motion_path(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_ui_text(
      prop,
      "Use Joint Heads",
      "For PoseJoint paths, use the joint head location when calculating this path");
...
static void rna_def_animviz_paths(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_ui_text(prop, "Bake Location", "When calculating Joint Paths, use Head or Tips");
  RNA_def_property_update(prop, NC_OBJECT | ND_DRAW_ANIMVIZ, NULL);
  ...
  RNA_def_property_ui_text(
      ...
      "For joint motion paths, search whole Action for keyframes instead of in group"
  ...
  /* Readonly Property - Do any motion paths exist/need updating? (Mainly for joint paths) */
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Has Motion Paths", "Are there any joint paths that will need updating (read-only)");
...
static void rna_def_animviz(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
void RNA_def_animviz(IxamRNA *brna)
{

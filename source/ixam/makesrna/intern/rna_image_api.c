...
static void rna_ImagePackedFile_save(ImagePackedFile *imapf, Main *bmain, ReportList *reports)
{
  if (BKE_packedfile_write_to_file(
          reports, BKE_main_ixamfile_path(bmain), imapf->filepath, imapf->packedfile, 0) !=
      RET_OK) {
    ...
    Image *image, Main *bmain, bContext *C, ReportList *reports, const char *data, int data_len)
{
  ...
  if (data) {
    ...
    BKE_image_packfiles(reports, image, ID_IXAM_PATH(bmain, &image->id));
  }
...
void RNA_api_image(StructRNA *srna)
{
  ...
  RNA_def_function_ui_description(func, "Pack an image as embedded data into the .ixam file");
  RNA_def_function_flag(func, FUNC_USE_MAIN | FUNC_USE_CONTEXT | FUNC_USE_REPORTS);
  ...
  RNA_def_function_ui_description(func, "Save an image packed in the .ixam file to disk");
  RNA_def_function_flag(func, FUNC_USE_MAIN | FUNC_USE_REPORTS);
  RNA_def_enum(

...
static bool rna_bone_group_poll(Object *ob, ReportList *reports)
{
  if ((ob->proxy != NULL) || (ob->proxy_group != NULL) || ID_IS_OVERRIDE_LIBRARY(ob)) {
    BKE_report(reports, RPT_ERROR, "Cannot edit joint groups for proxies or library overrides");
    return false;
  }
...
static void rna_bone_group_remove(ID *id, bPose *pose, ReportList *reports, PointerRNA *grp_ptr)
{
  ...
  if (grp_idx == -1) {
    BKE_reportf(reports, RPT_ERROR, "Joint group '%s' not found in this object", grp->name);
    return;
  }
    ...
    ID *id, bPoseChannel *pchan, Main *bmain, ReportList *reports, PointerRNA *con_ptr)
{
  ...
  if (BLI_findindex(&pchan->constraints, con) == -1) {
    BKE_reportf(
        reports, RPT_ERROR, "Constraint '%s' not found in pose joint '%s'", con->name, pchan->name);
    return;
  }
...
static void rna_def_bone_group(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(srna, "Joint Group", "Groups of Pose Channels (Joints)");
...
static void rna_def_pose_channel_constraints(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(srna, "PoseJoint Constraints", "Collection of pose joint constraints");
...
static void rna_def_pose_channel(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_sdna(srna, "bPoseChannel");
  RNA_def_struct_ui_text(srna, "Pose Joint", "Channel defining pose data for a joint in a Pose");
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Joint", "Joint associated with this PoseJoint");

  prop = RNA_def_property(srna, "parent", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "PoseBone");
  RNA_def_property_flag(prop, PROP_PTR_NO_OWNERSHIP);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Parent", "Parent of this pose joint");

  prop = RNA_def_property(srna, "child", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "PoseBone");
  RNA_def_property_flag(prop, PROP_PTR_NO_OWNERSHIP);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Child", "Child of this pose joint");
  ...
  RNA_def_property_ui_text(
      prop,
      "Basis Matrix",
      "Alternative access to location/scale/rotation relative to the parent and own rest joint");
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Pose Head Position", "Location of head of the channel's joint");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);

  prop = RNA_def_property(srna, "tail", PROP_FLOAT, PROP_TRANSLATION);
  RNA_def_property_float_sdna(prop, NULL, "pose_tail");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Pose Tail Position", "Location of tail of the channel's joint");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  ...
  RNA_def_property_ui_text(prop, "IK Stretch", "Allow scaling of the joint for IK");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_IK_update");
  ...
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_ui_text(
      prop, "Custom Object", "Object that defines custom display shape for this joint");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_dependency_update");
  ...
  RNA_def_property_ui_text(
      prop, "Scale to Joint Length", "Scale the custom object by the joint length");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Joint that defines the display transform of this custom shape");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  ...
  RNA_def_property_ui_text(
      prop, "Joint Group Index", "Joint group this pose channel belongs to (0 means no group)");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  RNA_def_property_ui_text(prop, "Joint Group", "Joint group this pose channel belongs to");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  prop = RNA_def_property(srna, "lock_locations", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "protectflag", OB_LOCK_LOC);
  RNA_def_property_ui_text(prop, "Lock Locations", "Lock editing of location when transforming");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  prop = RNA_def_property(srna, "lock_rotations", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "protectflag", OB_LOCK_ROT);
  RNA_def_property_ui_text(prop, "Lock Rotation", "Lock editing of rotation when transforming");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  prop = RNA_def_property(srna, "lock_scales", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "protectflag", OB_LOCK_SCALE);
  RNA_def_property_ui_text(prop, "Lock Scale", "Lock editing of scale when transforming");
  RNA_def_property_editable_func(prop, "rna_PoseChannel_proxy_editable");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
...
static void rna_def_pose_itasc(IxamRNA *brna)
{
...
static void rna_def_pose_ikparam(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
/* pose.bone_groups */
static void rna_def_bone_groups(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(srna, "Joint Groups", "Collection of joint groups");

  func = RNA_def_function(srna, "new", "rna_bone_group_new");
  RNA_def_function_ui_description(func, "Add a new joint group to the object");
  ...
  /* return type */
  parm = RNA_def_pointer(func, "group", "BoneGroup", "", "New joint group");
  RNA_def_function_return(func, parm);

  func = RNA_def_function(srna, "remove", "rna_bone_group_remove");
  RNA_def_function_ui_description(func, "Remove a joint group from this object");
  RNA_def_function_flag(func, FUNC_USE_REPORTS | FUNC_USE_SELF_ID); /* ID needed for refresh */
  /* bone group to remove */
  parm = RNA_def_pointer(func, "group", "BoneGroup", "", "Removed joint group");
  RNA_def_parameter_flags(parm, PROP_NEVER_NULL, PARM_REQUIRED | PARM_RNAPTR);
  RNA_def_parameter_clear_flags(parm, PROP_THICK_WRAP, 0);
  ...
  RNA_def_property_ui_text(prop, "Active Joint Group", "Active joint group for this pose");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
  ...
  RNA_def_property_ui_text(prop, "Active Joint Group Index", "Active index in joint groups array");
  RNA_def_property_update(prop, NC_OBJECT | ND_POSE, "rna_Pose_update");
}

static void rna_def_pose(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(
      srna, "Pose", "A collection of pose channels, including settings for animating joints");
  ...
  RNA_def_property_struct_type(prop, "PoseBone");
  RNA_def_property_override_flag(prop, PROPOVERRIDE_OVERRIDABLE_LIBRARY);
  RNA_def_property_ui_text(prop, "Pose Joints", "Individual pose joints for the rig");
  ...
  RNA_def_property_struct_type(prop, "BoneGroup");
  RNA_def_property_ui_text(prop, "Joint Groups", "Groups of the joints");
  ...
  RNA_def_property_ui_text(
      prop, "Auto IK", "Add temporary IK constraints while grabbing joints in Pose Mode");
  RNA_def_struct_path_func(srna, "rna_Pose_path");
  RNA_def_property_update(prop, 0, "rna_Pose_update");
  RNA_def_property_flag(prop, PROP_LIB_EXCEPTION);
...
void RNA_def_pose(IxamRNA *brna)
{
  rna_def_pose(brna);

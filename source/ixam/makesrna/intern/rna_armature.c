  ...
  if (value.owner_id == NULL && value.data == NULL) {
    ...
    if (value.owner_id != &arm->id) {
      ...
      if (GS(ob->id.name) != ID_OB || (ob->data != arm)) {
        printf("ERROR: rig set active joint - new active doesn't come from this rig\n");
        return;
      }
    }
...
static EditBone *rna_Armature_edit_bone_new(bArmature *arm, ReportList *reports, const char *name)
{
  if (arm->edbo == NULL) {
    BKE_reportf(reports,
                RPT_ERROR,
                "Rig '%s' not in edit mode, cannot add an editjoint",
                arm->id.name + 2);
    return NULL;
  }
                                          ...
                                          PointerRNA *ebone_ptr)
{
  ...
  if (arm->edbo == NULL) {
    BKE_reportf(reports,
                RPT_ERROR,
                "Rig '%s' not in edit mode, cannot remove an editjoint",
                arm->id.name + 2);
    return;
  }
  ...
  if (BLI_findindex(arm->edbo, ebone) == -1) {
    BKE_reportf(reports,
                RPT_ERROR,
                "Rig '%s' does not contain joint '%s'",
                arm->id.name + 2,
                ebone->name);
    return;
  }
...
#  define RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone) \
  ...
  RNA_def_property_ui_text(
      prop, "Roll In", "Roll offset for the start of the B-Joint, adjusts twist");
  ...
  RNA_def_property_ui_range(prop, -M_PI * 2, M_PI * 2, 10, 2);
  RNA_def_property_ui_text(
      prop, "Roll Out", "Roll offset for the end of the B-Joint, adjusts twist");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);

  if (is_posebone == false) {
    prop = RNA_def_property(srna, "use_endroll_as_inroll", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_ui_text(
        prop, "Inherit End Roll", "Add Roll Out of the Start Handle joint to the Roll In value");
  ...
  RNA_def_property_ui_text(
      prop, "In X", "X-axis handle offset for start of the B-Joint's curve, adjusts curvature");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  RNA_def_property_ui_text(
      prop, "In Z", "Z-axis handle offset for start of the B-Joint's curve, adjusts curvature");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  RNA_def_property_ui_text(
      prop, "Out X", "X-axis handle offset for end of the B-Joint's curve, adjusts curvature");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  RNA_def_property_ui_text(
      prop, "Out Z", "Z-axis handle offset for end of the B-Joint's curve, adjusts curvature");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_text(prop, "Ease In", "Length of first Bezier Handle (for B-Joints only)");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_range(prop, -5.0f, 5.0f, 1, 3);
  RNA_def_property_float_default(prop, 1.0f);
  RNA_def_property_ui_text(prop, "Ease Out", "Length of second Bezier Handle (for B-Joints only)");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_text(
      prop,
      "Scale In",
      "Scale factors for the start of the B-Joint, adjusts thickness (for tapering effects)");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
  ...
  RNA_def_property_ui_text(
      prop,
      "Scale Out",
      "Scale factors for the end of the B-Joint, adjusts thickness (for tapering effects)");
  RNA_DEF_CURVEBONE_UPDATE(prop, is_posebone, is_editbone);
...
static void rna_def_bone_common(StructRNA *srna, int editbone)
{
  static const EnumPropertyItem prop_bbone_handle_type[] = {
      ...
      {BBONE_HANDLE_ABSOLUTE,
       ...
       0,
       "Absolute",
       "Use the position of the specified joint to compute the handle"},
      {BBONE_HANDLE_RELATIVE,
       "RELATIVE",
       0,
       "Relative",
       "Use the offset of the specified joint from rest pose to compute the handle"},
      {BBONE_HANDLE_TANGENT,
       "TANGENT",
       0,
       "Tangent",
       "Use the orientation of the specified joint to compute the handle, ignoring the location"},
      {0, NULL, 0, NULL, NULL},
  };
  ...
  RNA_def_property_array(prop, 10);
  if (editbone) {
  ...
  RNA_def_property_ui_text(prop, "Layers", "Layers joint exists in");
  ...
  RNA_def_property_ui_text(
      prop, "Connected", "When joint has a parent, joint's head is stuck to the parent's tail");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");
  ...
  RNA_def_property_ui_text(
      prop, "Inherit Rotation", "Joint inherits rotation or scale from parent joint");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");
  ...
  RNA_def_property_ui_text(
      ...
      "When deforming joint, multiply effects of Vertex Group weights with Envelope influence");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");
  ...
  RNA_def_property_ui_text(prop, "Deform", "Enable Joint to deform geometry");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");

  prop = RNA_def_property(srna, "inherit_scale", PROP_ENUM, PROP_NONE);
  RNA_def_property_ui_text(
      prop, "Inherit Scale", "Specifies how the joint inherits scaling from the parent joint");
  ...
  RNA_def_property_ui_text(
      prop, "Inherit Scale", "DEPRECATED: Joint inherits scaling from parent joint");
  if (editbone) {
  ...
  RNA_def_property_ui_text(prop, "Local Location", "Joint location is set in local space");
  ...
  RNA_def_property_ui_text(
      prop,
      "Display Wire",
      "Joint is always displayed in wireframe regardless of viewport shading mode "
      "(useful for non-obstructive custom joint shapes)");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  ...
  RNA_def_property_ui_text(
      prop,
      "Cyclic Offset",
      "When joint doesn't have a parent, it receives cyclic offset effects (Deprecated)");
  //                         "When joint doesn't have a parent, it receives cyclic offset effects");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");

  prop = RNA_def_property(srna, "hide_select", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "flag", BONE_UNSELECTABLE);
  RNA_def_property_ui_text(prop, "Selectable", "Joint is able to be selected");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  ...
  RNA_def_property_ui_text(
      prop, "Envelope Deform Distance", "Joint deformation distance (for Envelope deform only)");

  prop = RNA_def_property(srna, "envelope_weight", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, NULL, "weight");
  RNA_def_property_range(prop, 0.0f, 1000.0f);
  RNA_def_property_ui_text(
      prop, "Envelope Deform Weight", "Joint deformation weight (for Envelope deform only)");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");
  ...
  RNA_def_property_ui_text(
      prop, "Envelope Head Radius", "Radius of head of joint (for Envelope deform only)");
  ...
  RNA_def_property_ui_range(prop, 0.01, 100, 0.1, 3);
  RNA_def_property_ui_text(
      prop, "Envelope Tail Radius", "Radius of tail of joint (for Envelope deform only)");
  ...
  RNA_def_property_ui_text(
      prop, "B-Joint Segments", "Number of subdivisions of joint (for B-Joints only)");
  ...
  RNA_def_property_ui_text(prop, "B-Joint Display X Width", "B-Joint X size");
  ...
  RNA_def_property_ui_range(prop, 0.0f, 1000.0f, 1, RNA_TRANSLATION_PREC_DEFAULT);
  RNA_def_property_ui_text(prop, "B-Joint Display Z Width", "B-Joint Z size");
  ...
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_ui_text(
      prop, "B-Joint Start Handle Type", "Selects how the start handle of the B-Joint is computed");
  RNA_def_property_update(prop, 0, "rna_Armature_dependency_update");
  ...
  RNA_def_property_ui_text(
      prop, "B-Joint Start Handle", "Joint that serves as the start handle for the B-Joint curve");

  prop = RNA_def_property(srna, "bbone_handle_use_scale_start", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_ui_text(
      prop,
      "Start Handle Scale",
      "Multiply B-Joint Scale In channels by the local scale values of the start handle. "
  ...
  RNA_def_property_ui_text(
      prop,
      "Start Handle Ease",
      "Multiply the B-Joint Ease In channel by the local Y scale value of the start handle. "
      "This is done after the Scale Easing option and isn't affected by it");
  ...
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_ui_text(
      prop, "B-Joint End Handle Type", "Selects how the end handle of the B-Joint is computed");
  RNA_def_property_update(prop, 0, "rna_Armature_dependency_update");
  ...
  RNA_def_property_override_flag(prop, PROPOVERRIDE_NO_COMPARISON);
  RNA_def_property_ui_text(
      prop, "B-Joint End Handle", "Joint that serves as the end handle for the B-Joint curve");

  prop = RNA_def_property(srna, "bbone_handle_use_scale_end", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_ui_text(
      prop,
      "End Handle Scale",
      "Multiply B-Joint Scale Out channels by the local scale values of the end handle. "
      "This is done after the Scale Easing option and isn't affected by it");
  ...
  RNA_def_property_ui_text(
      prop,
      "End Handle Ease",
      "Multiply the B-Joint Ease Out channel by the local Y scale value of the end handle. "
      "This is done after the Scale Easing option and isn't affected by it");
...
static void rna_def_bone(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;

  srna = RNA_def_struct(brna, "Bone", NULL);
  RNA_def_struct_ui_text(srna, "Joint", "Joint in an Rig data-block");
  ...
  RNA_def_property_ui_text(prop, "Parent", "Parent joint (in same Rig)");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  ...
  RNA_def_property_struct_type(prop, "Bone");
  RNA_def_property_flag(prop, PROP_PTR_NO_OWNERSHIP);
  RNA_def_property_ui_text(prop, "Children", "Joints which are children of this joint");
  ...
  RNA_def_property_ui_text(
      prop,
      "Hide",
      "Joint is not visible when it is not in Edit Mode (i.e. in Object or Pose Modes)");
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Joint Matrix", "3x3 joint matrix");
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Joint Rig-Relative Matrix", "4x4 joint matrix relative to rig");
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Tail", "Location of tail end of the joint relative to its parent");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Rig-Relative Tail", "Location of tail end of the joint relative to rig");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Head", "Location of head end of the joint relative to its parent");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
  ...
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
      prop, "Rig-Relative Head", "Location of head end of the joint relative to rig");
  RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);

  prop = RNA_def_property(srna, "length", PROP_FLOAT, PROP_DISTANCE);
  RNA_def_property_float_sdna(prop, NULL, "length");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Length", "Length of the joint");
...
static void rna_def_edit_bone(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(srna, "Edit Joint", "Edit mode joint in an rig data-block");
  RNA_def_struct_ui_icon(srna, ICON_BONE_DATA);
  ...
  RNA_def_property_ui_text(prop, "Parent", "Parent edit joint (in same Rig)");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");

  prop = RNA_def_property(srna, "roll", PROP_FLOAT, PROP_ANGLE);
  RNA_def_property_float_sdna(prop, NULL, "roll");
  RNA_def_property_ui_range(prop, -M_PI * 2, M_PI * 2, 10, 2);
  RNA_def_property_ui_text(prop, "Roll", "Joint rotation around head-tail axis");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_editbone_transform_update");
  ...
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Head", "Location of head end of the joint");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_editbone_transform_update");
  ...
  RNA_def_property_ui_range(prop, 0, FLT_MAX, 10, RNA_TRANSLATION_PREC_DEFAULT);
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Tail", "Location of tail end of the joint");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_editbone_transform_update");
  ...
  RNA_def_property_ui_text(prop, "Length", "Length of the joint. Changing moves the tail end");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_editbone_transform_update");
  ...
  RNA_def_property_ui_text(prop, "Hide", "Joint is not visible when in Edit Mode");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");

  prop = RNA_def_property(srna, "lock", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "flag", BONE_EDITMODE_LOCKED);
  RNA_def_property_ui_text(prop, "Lock", "Joint is not able to be transformed when in Edit Mode");
  RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  ...
  RNA_def_property_ui_text(
      prop,
      "Edit Joint Matrix",
      "Matrix combining location and rotation of the joint (head position, direction and roll), "
      "in rig space (does not include/support joint's length/size)");
...
/* armature.bones.* */
static void rna_def_armature_bones(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_ui_text(srna, "Rig Joints", "Collection of rig joints");

  prop = RNA_def_property(srna, "active", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "Bone");
  RNA_def_property_pointer_sdna(prop, NULL, "act_bone");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Active Joint", "Rig's active joint");
...
/* armature.bones.* */
static void rna_def_armature_edit_bones(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_sdna(srna, "bArmature");
  RNA_def_struct_ui_text(srna, "Rig EditJoints", "Collection of rig edit joints");

  prop = RNA_def_property(srna, "active", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "EditBone");
  RNA_def_property_pointer_sdna(prop, NULL, "act_edbone");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Active EditJoint", "Rigs active edit joint");
  ...
  RNA_def_function_ui_description(func, "Add a new joint");
  parm = RNA_def_string(func, "name", "Object", 0, "", "New name for the joint");
  RNA_def_parameter_flags(parm, 0, PARM_REQUIRED);
  /* return type */
  parm = RNA_def_pointer(func, "bone", "EditBone", "", "Newly created edit joint");
  RNA_def_function_return(func, parm);

  /* remove target */
  func = RNA_def_function(srna, "remove", "rna_Armature_edit_bone_remove");
  RNA_def_function_flag(func, FUNC_USE_REPORTS);
  RNA_def_function_ui_description(func, "Remove an existing joint from the rig");
  /* Target to remove. */
  parm = RNA_def_pointer(func, "bone", "EditBone", "", "EditJoint to remove");
  RNA_def_parameter_flags(parm, PROP_NEVER_NULL, PARM_REQUIRED | PARM_RNAPTR);
  RNA_def_parameter_clear_flags(parm, PROP_THICK_WRAP, 0);
}

static void rna_def_armature(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  static const EnumPropertyItem prop_drawtype_items[] = {
      {ARM_OCTA, "OCTAHEDRAL", 0, "Octahedral", "Display joints as octahedral shape (default)"},
      {ARM_LINE, "STICK", 0, "Stick", "Display joints as simple 2D lines with dots"},
      {ARM_B_BONE,
       "BBONE",
       0,
       "B-Joint",
       "Display joints as boxes, showing subdivision and B-Splines"},
      {ARM_ENVELOPE,
       "ENVELOPE",
       0,
       "Envelope",
       "Display joints as extruded spheres, showing deformation influence volume"},
      {ARM_WIRE,
       "WIRE",
       0,
       "Wire",
       "Display joints as thin wires, showing subdivision and B-Splines"},
      {0, NULL, 0, NULL, NULL},
  };
  static const EnumPropertyItem prop_pose_position_items[] = {
      {0, "POSE", 0, "Pose Position", "Show rig in posed state"},
      {ARM_RESTPOS,
       "REST",
       0,
       "Rest Position",
       "Show Rig in binding pose state (no posing possible)"},
      {0, NULL, 0, NULL, NULL},
  };
  ...
  RNA_def_struct_ui_text(
      srna,
      "Rig",
      "Rig data-block containing a hierarchy of joints, usually used for rigging characters");
  RNA_def_struct_ui_icon(srna, ICON_ARMATURE_DATA);
  RNA_def_struct_sdna(srna, "bArmature");

  func = RNA_def_function(srna, "transform", "rna_Armature_transform");
  RNA_def_function_ui_description(func, "Transform rig joints by a matrix");
  ...
  RNA_def_property_collection_funcs(prop,
                                    ...
                                    NULL);
  RNA_def_property_struct_type(prop, "Bone");
  RNA_def_property_ui_text(prop, "Joints", "");
  ...
  RNA_def_property_struct_type(prop, "EditBone");
  RNA_def_property_ui_text(prop, "Edit Joints", "");
  ...
  RNA_def_property_ui_text(
      prop, "Pose Position", "Show rig in binding pose or final posed state");
  RNA_def_property_update(prop, 0, "rna_Armature_update_data");
  ...
  RNA_def_property_boolean_sdna(prop, NULL, "layer", 1);
  RNA_def_property_array(prop, 10);
  RNA_def_property_ui_text(prop, "Visible Layers", "Rig layer visibility");
  ...
  RNA_def_property_ui_text(prop, "Display Axes", "Display joint axes");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  RNA_def_property_flag(prop, PROP_LIB_EXCEPTION);
  ...
  RNA_def_property_ui_text(prop, "Display Names", "Display joint names");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  RNA_def_property_flag(prop, PROP_LIB_EXCEPTION);
  ...
  RNA_def_property_ui_text(
      prop, "X-Axis Mirror", "Apply changes to matching joint on opposite side of X-Axis");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
  RNA_def_property_flag(prop, PROP_LIB_EXCEPTION);
  ...
  RNA_def_property_ui_text(
      prop, "Display Custom Joint Shapes", "Display joints with their custom shapes");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");

  prop = RNA_def_property(srna, "show_group_colors", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "flag", ARM_COL_CUSTOM);
  RNA_def_property_ui_text(prop, "Display Joint Group Colors", "Display joint group colors");
  RNA_def_property_update(prop, 0, "rna_Armature_redraw_data");
...
void RNA_def_armature(IxamRNA *brna)
{

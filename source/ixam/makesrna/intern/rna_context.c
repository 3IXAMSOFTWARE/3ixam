...
const EnumPropertyItem rna_enum_context_mode_items[] = {
    ...
    {CTX_MODE_EDIT_ARMATURE, "EDIT_ARMATURE", 0, "Rig Edit", ""},
...
static PointerRNA rna_Context_space_data_get(PointerRNA *ptr)
{
  ...
  struct SpaceLink *sl = CTX_wm_space_data(C);

  ARegion *region = CTX_wm_region(C);
  if (region) {
    if (region->alignment == RGN_ALIGN_QSPLIT) {
      RegionView3D *rv3d = region->regiondata;
      sl = (struct SpaceLink *)CTX_wm_view3d_index(C, rv3d->quad_index);
    }
  }

  PointerRNA newptr;
  RNA_pointer_create((ID *)CTX_wm_screen(C), &RNA_Space, sl, &newptr);
  return newptr;
}
...
{
  bContext *C = (bContext *)ptr->data;
  return rna_pointer_inherit_refine(ptr, &RNA_IxamData, CTX_data_main(C));
}
...
void RNA_def_context(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  /* Data */
  prop = RNA_def_property(srna, "ixam_data", PROP_POINTER, PROP_NONE);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_struct_type(prop, "IxamData");

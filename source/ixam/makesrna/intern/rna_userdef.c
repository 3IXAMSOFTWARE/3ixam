...
const EnumPropertyItem rna_enum_preference_section_items[] = {
    {USER_SECTION_SCENE_PREFERENCES, "SCENE_PREFERENCES", 0, "Scene Preferences", ""},
    ...
    {USER_SECTION_HOTKEYS, "HOTKEYS", 0, "Hot Keys", ""},
    // {0, "", 0, NULL, NULL},
    {USER_SECTION_FILE_PATHS, "FILE_PATHS", 0, "File Output", ""},
...
static void rna_userdef_theme_update_pref_colors(Main *bmain, Scene *scene, PointerRNA *ptr) {
  ThemeUI* tui = (ThemeUI*)ptr->data;

  for(uiWidgetColors* wcol = &tui->wcol_regular;
    wcol != (uiWidgetColors*)&tui->wcol_state;
    wcol++) {

    if(!compare_v3v3_uchar(wcol->outline, tui->line_color))
      copy_v3_v3_uchar(wcol->outline, tui->line_color);
    if(!compare_v3v3_uchar(wcol->text, tui->text_color))
      copy_v3_v3_uchar(wcol->text, tui->text_color);
  }

  bTheme *theme = UI_GetTheme();
  for(ThemeSpace* ts = &theme->space_properties;
    ts != (ThemeSpace*)&theme->tarm;
    ts++) {

   copy_v3_v3_uchar(ts->text, tui->text_color);
  }

  return rna_userdef_theme_update(bmain, scene, ptr);
}
                                              ...
                                              PointerRNA *UNUSED(ptr))
{
  if (U.sequencer_disk_cache_dir[0] != '\0') {
    BLI_path_abs(U.sequencer_disk_cache_dir, BKE_main_ixamfile_path_from_global());
...
static void rna_userdef_autobackup_dir_update(Main *UNUSED(bmain),
                                    Scene *UNUSED(scene),
                                    PointerRNA *UNUSED(ptr)) {
  USERDEF_TAG_DIRTY;
}
...
{
  bAddonPrefType *apt = RNA_struct_ixam_type_get(type);
  ...
  RNA_struct_free(&IXAM_RNA, type);

  BKE_addon_pref_type_remove(apt);

  /* update while ixam is running */
  WM_main_add_notifier(NC_WINDOW, NULL);
}
                                         ...
                                         StructFreeFunc free)
{
  ...
  apt->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, identifier, &RNA_AddonPreferences);
  apt->rna_ext.data = data;
  apt->rna_ext.call = call;
  apt->rna_ext.free = free;
  RNA_struct_ixam_type_set(apt->rna_ext.srna, apt);
...
/* Debug_value */
static int rna_UserDef_debug_value_get(PointerRNA *UNUSED(ptr))
{
#ifdef DEBUG
...
/* Authorization token */
static void rna_UserDef_auth_token_value_get(PointerRNA *UNUSED(ptr), char *value)
{
  strcpy(value, G.auth_token);
}

static int rna_UserDef_auth_token_value_length(PointerRNA *UNUSED(ptr))
{
  return strlen(G.auth_token);
}
...
/* TODO(sergey): This technically belongs to ixamlib, but we don't link
 * makesrna against it.
 */
...
static void rna_def_userdef_theme_ui_font_style(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_ui_style(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_ui_wcol(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_ui_wcol_state(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_ui_panel(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_ui_gradient(IxamRNA *brna)
{
...
static void rna_def_userdef_theme_ui(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  prop = RNA_def_property(srna, "wcol_toolbar_item_pro", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_ui_text(prop, "Toolbar Item Widget Colors", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");
  ...
  prop = RNA_def_property(srna, "wcol_animbar", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_ui_text(prop, "Animbar Widget Colors", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");

  prop = RNA_def_property(srna, "wcol_prefs_regular", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_ui_text(prop, "Preferences Widget Colors", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");

  prop = RNA_def_property(srna, "wcol_vert_trapezoid", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_ui_text(prop, "Vertical Trapezoid Colors", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");
  ...
  prop = RNA_def_property(srna, "text_color", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_float_sdna(prop, NULL, "text_color");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Text Color", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update_pref_colors");

  prop = RNA_def_property(srna, "line_color", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_float_sdna(prop, NULL, "line_color");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Line Color", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update_pref_colors");
...
static void rna_def_userdef_theme_space_gradient(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_generic(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
/* list / channels */
static void rna_def_userdef_theme_space_list_generic(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_view3d(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_ui_text(
      prop,
      "Joint Locked Weight",
      "Shade for joint corresponding to a locked weight group during painting");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");
...
static void rna_def_userdef_theme_space_graph(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_file(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_outliner(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_userpref(IxamRNA *brna)
{
  StructRNA *srna;
  ...
  RNA_def_struct_sdna(srna, "ThemeSpace");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Theme Preferences", "Theme settings for the 3IXAM Preferences");

  rna_def_userdef_theme_spaces_main(srna);
}

static void rna_def_userdef_theme_space_console(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_info(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_text(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_node(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_matpro(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  srna = RNA_def_struct(brna, "ThemeMatProEditor", NULL);
  RNA_def_struct_sdna(srna, "ThemeSpace");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Theme Pro Material Editor", "Theme settings for the Pro Material Editor");
...
static void rna_def_userdef_theme_space_buts(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_image(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_seq(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_action(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_nla(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_colorset(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Theme Joint Color Set", "Theme settings for joint color sets");

  prop = RNA_def_property(srna, "normal", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_float_sdna(prop, NULL, "solid");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Regular", "Color used for the surface of joints");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");

  prop = RNA_def_property(srna, "select", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_float_sdna(prop, NULL, "select");
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Select", "Color used for selected joints");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");

  prop = RNA_def_property(srna, "active", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_array(prop, 3);
  RNA_def_property_ui_text(prop, "Active", "Color used for active joints");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");
...
static void rna_def_userdef_theme_collection_color(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_strip_color(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_clip(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_theme_space_topbar(IxamRNA *brna)
{
  StructRNA *srna;
...
static void rna_def_userdef_theme_space_statusbar(IxamRNA *brna)
{
  StructRNA *srna;
...
static void rna_def_userdef_theme_space_matlib(IxamRNA *brna)
{
  StructRNA *srna;

  /* space_matlib */

  srna = RNA_def_struct(brna, "ThemeMatLib", NULL);
  RNA_def_struct_sdna(srna, "ThemeSpace");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Theme Material Library", "Theme settings for the Material Library");

  rna_def_userdef_theme_spaces_main(srna);
}

static void rna_def_userdef_theme_space_render(IxamRNA *brna)
{
  StructRNA *srna;

  /* space_matlib */

  srna = RNA_def_struct(brna, "ThemeRender", NULL);
  RNA_def_struct_sdna(srna, "ThemeSpace");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Theme Render Settings", "Theme settings for the Render Settings");

  rna_def_userdef_theme_spaces_main(srna);
}

static void rna_def_userdef_theme_space_spreadsheet(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_themes(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  static const EnumPropertyItem active_theme_area[] = {
      ...
      {18, "BONE_COLOR_SETS", ICON_COLOR, "Joint Color Sets", ""},
      ...
      {23, "RENDER", ICON_SPREADSHEET, "Render"},
      {24, "MATPRO", ICON_NODETREE, "Pro Material"},
      {0, NULL, 0, NULL, NULL},
  };
  ...
  prop = RNA_def_property(srna, "matlib", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_pointer_sdna(prop, NULL, "space_matlib");
  RNA_def_property_struct_type(prop, "ThemeMatLib");
  RNA_def_property_ui_text(prop, "Material Library", "");
  ...
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_pointer_sdna(prop, NULL, "space_render");
  RNA_def_property_struct_type(prop, "ThemeRender");
  RNA_def_property_ui_text(prop, "Render", "");

  prop = RNA_def_property(srna, "matpro", PROP_POINTER, PROP_NONE);
  RNA_def_property_flag(prop, PROP_NEVER_NULL);
  RNA_def_property_pointer_sdna(prop, NULL, "space_matpro");
  RNA_def_property_struct_type(prop, "ThemeMatProEditor");
  RNA_def_property_ui_text(prop, "Pro Material", "");
  /* end space types */
  ...
  RNA_def_property_ui_text(prop, "Joint Color Sets", "");
...
static void rna_def_userdef_addon(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_studiolights(IxamRNA *brna)
{
  StructRNA *srna;
  FunctionRNA *func;
  PropertyRNA *parm;
...
static void rna_def_userdef_studiolight(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_pathcompare(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_addon_pref(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_dothemes(IxamRNA *brna)
{
  ...
  rna_def_userdef_theme_space_matpro(brna);
  ...
  rna_def_userdef_theme_space_matlib(brna);
  rna_def_userdef_theme_space_render(brna);
...
static void rna_def_userdef_solidlight(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_walk_navigation(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
...
static void rna_def_userdef_view(IxamRNA *brna)
{
  ...
  static const EnumPropertyItem hand_types[] = {
      {USER_HAND_RIGHT, "HAND_RIGHT", 0, "Right Hand", "Right hand mode"},
      {USER_HAND_LEFT, "HAND_LEFT", 0, "Left Hand", "Left hand mode"},
      {0, NULL, 0, NULL, NULL},
  };
  ...
  static const EnumPropertyItem font_weight[] = {
      {FONT_WEIGHT_LIGHT, "LIGHT", 0, "Light", "Default text looks thin"},
      {FONT_WEIGHT_MEDIUM, "MEDIUM", 0, "Medium", "Default text looks thicker"},
      {FONT_WEIGHT_BOLD, "BOLD", 0, "Bold", "Default text looks like bold"},
      {0, NULL, 0, NULL, NULL},
  };
  ...
  prop = RNA_def_property(srna, "font_weight", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, font_weight);
  RNA_def_property_ui_text(
      prop,
      "UI Font Weight",
      "Changes the thickness of default text in the interface");
  RNA_def_property_update(prop, 0, "rna_userdef_dpi_update");
  ...
  prop = RNA_def_property(srna, "hand_type", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, hand_types);
  RNA_def_property_enum_sdna(prop, NULL, "hand_type");
  RNA_def_property_ui_text(
      prop, "Hand Type", "Different hand types");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D_HAND_TYPE, "rna_userdef_update");
  ...
  prop = RNA_def_property(srna, "use_play_intro", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "uiflag", USER_PLAY_INTRO);
  RNA_def_property_boolean_default(prop, true);
  RNA_def_property_ui_text(
      prop, "Play Intro Sound", "Automatically play intro sound when launching app");
...
#  endif
  ...
  RNA_def_property_ui_text(prop, "Show Memory", "Show 3IXAM memory usage");
  ...
  RNA_def_property_ui_text(prop, "Show Version", "Show 3IXAM version string");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_INFO, "rna_userdef_update");
  ...
  static const EnumPropertyItem rna_enum_intro_sound_items[] = {
    {USER_INTROSOUND_1, "INTROSOUND_1", 0, "Introsound 1", "First intro sound"},
    {USER_INTROSOUND_2, "INTROSOUND_2", 0, "Introsound 2", "Second intro sound"},
    {USER_INTROSOUND_3, "INTROSOUND_3", 0, "Introsound 3", "Third intro sound"},
    {0, NULL, 0, NULL, NULL},
  };

  prop = RNA_def_property(srna, "intro_sound", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, rna_enum_intro_sound_items);
  RNA_def_property_enum_sdna(prop, NULL, "intro_sound");
  RNA_def_property_ui_text(
      prop, "Intro Sound", "Different intro sounds");
}

static void rna_def_userdef_edit(IxamRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;
  ...
  RNA_def_struct_nested(brna, srna, "Preferences");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "Edit Methods", "Settings for interacting with 3IXAM data");
  ...
  RNA_def_property_ui_text(prop,
                           "Auto Keying Enable",
                           "Automatic keyframe insertion for Objects and Joints "
  ...
  RNA_def_property_ui_text(prop,
                           "Auto Keying Mode",
                           "Mode of automatic keyframe insertion for Objects and Joints "
                           "(default setting used for new Scenes)");
  ...
  RNA_def_property_ui_text(
      ...
      "Show warning indicators when transforming objects and joints if auto keying is enabled");
  ...
  RNA_def_property_ui_text(
      ...
      "Use animation channel group colors; generally this is used to show joint group colors");
  ...
  RNA_def_property_ui_text(
      prop, "Duplicate Rig", "Causes rig data to be duplicated with the object");
...
static void rna_def_userdef_system(IxamRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;
...
#  endif
  ...
  RNA_def_property_ui_text(
      ...
      "on operating system DPI settings and 3IXAM display scale");
  ...
  RNA_def_property_ui_text(
      ...
      "user interface elements, based on operating system settings and 3IXAM UI scale");
...
static void rna_def_userdef_input(IxamRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;
...
#  endif /* WITH_INPUT_NDOF */
  ...
  RNA_def_property_ui_text(
      prop, "Max Threshold", "Raw input pressure value that is interpreted as 100% by 3IXAM");
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "restarting 3IXAM for changes to take effect)");
  RNA_def_property_update(prop, 0, "rna_userdef_input_devices");
...
static void rna_def_userdef_keymap(IxamRNA *brna)
{
  PropertyRNA *prop;
...
static void rna_def_userdef_filepaths_asset_library(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  prop = RNA_def_property(srna, "path", PROP_STRING, PROP_DIRPATH);
  RNA_def_property_ui_text(
      prop, "Path", "Path to a directory with .ixam files to use as an asset library");
  RNA_def_property_string_funcs(prop, NULL, NULL, "rna_userdef_asset_library_path_set");
  RNA_def_property_update(prop, 0, "rna_userdef_update");
}

static void rna_def_userdef_filepaths(IxamRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;
  ...
  RNA_def_struct_nested(brna, srna, "Preferences");
  RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
  RNA_def_struct_ui_text(srna, "File Output", "Default paths for external files");
  ...
  RNA_def_property_ui_text(
      prop, "Compress File", "Enable file compression when saving .ixam files");
  ...
  RNA_def_property_ui_text(prop, "Load UI", "Load user interface setup when loading .ixam files");
  ...
  RNA_def_property_ui_text(prop,
                           ...
                           "Allow any .ixam file to run scripts automatically "
                           "(unsafe with ixam files from an untrusted source)");
  ...
  prop = RNA_def_property(srna, "autobackup_directory", PROP_STRING, PROP_FILENAME);
  RNA_def_property_string_sdna(prop, NULL, "autobackupdir");
  RNA_def_property_ui_text(
      prop, "Auto Backup Filename", "Filename base for auto backup instances");
  RNA_def_property_update(prop, 0, "rna_userdef_autobackup_dir_update");
  ...
  prop = RNA_def_property(srna, "autobackup_count", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, NULL, "autobackup_count");
  RNA_def_property_range(prop, 1, 30);
  RNA_def_property_ui_text(
      prop, "Number of files", "Maximum number of temporary saves saved on disk");
...
static void rna_def_userdef_apps(IxamRNA *brna)
{
  PropertyRNA *prop;
  StructRNA *srna;
...
static void rna_def_userdef_experimental(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  RNA_def_property_ui_text(
      ...
      "dealing with older .ixam files that need manual Resync (Enforce) handling");
  ...
  RNA_def_property_ui_text(prop, "Real Debug", "Enable Real debugging options for developers");
  RNA_def_property_update(prop, 0, "rna_userdef_update");
  ...
  RNA_def_property_ui_text(prop, "Play Next", "Enable the new Play codebase, requires restart");
...
static void rna_def_userdef_addon_collection(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  FunctionRNA *func;
  PropertyRNA *parm;
...
static void rna_def_userdef_autoexec_path_collection(IxamRNA *brna, PropertyRNA *cprop)
{
  StructRNA *srna;
  FunctionRNA *func;
  PropertyRNA *parm;
...
void RNA_def_userdef(IxamRNA *brna)
{
  USERDEF_TAG_DIRTY_PROPERTY_UPDATE_ENABLE;
  ...
  RNA_def_property_int_funcs(prop, "rna_UserDef_debug_value_get", NULL, NULL);
  RNA_def_property_ui_text(prop, "Debug Value", "Debug Value");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  
  /* Authorization token */
  prop = RNA_def_property(srna, "auth_token", PROP_STRING, PROP_NONE);
  RNA_def_property_ui_text(prop, "Authorization token", "Authorization token");
  RNA_def_property_string_funcs(
      prop, "rna_UserDef_auth_token_value_get", "rna_UserDef_auth_token_value_length", NULL);
  RNA_def_property_string_maxlength(prop, FILE_MAX);
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);

  /* Current theme selected by user */
  prop = RNA_def_property(srna, "current_theme", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, NULL, "current_theme");
  RNA_def_property_ui_text(prop, "Current theme", "Current theme selected");
  RNA_def_property_string_default(prop, "Dark");
  ...
  RNA_def_property_ui_text(prop, "Edit Methods", "Settings for interacting with 3IXAM data");
  ...
  prop = RNA_def_int_vector(srna,
                            ...
                            INT_MAX,
                            "Version",
                            "Version of 3IXAM the userpref.ixam was saved with",
                            0,
                            INT_MAX);

...
const EnumPropertyItem rna_enum_button_shape_items[] = {
    {UI_BSHAPE_ROUNDBOX, "ROUNDBOX", 0, "Roundbox", ""},
    {UI_BSHAPE_HEX, "HEX", 0, "Hex", ""},
    {UI_BSHAPE_TRAPEZOID, "TRAPEZOID", 0, "Trapezoid", ""},
    {UI_BSHAPE_TRAPEZOIDR, "TRAPEZOIDR", 0, "Trapezoid", ""},
    {UI_BSHAPE_RHOMBOID, "RHOMBOID", 0, "Rhomboid", ""},
    {UI_BSHAPE_ADVANCED, "ADVANCED", 0, "Advanced", ""},
    {UI_BSHAPE_PRO_HEX, "PRO_HEX", 0, "Pro Hex", ""},
    {UI_BSHAPE_PREFS, "PREFS", 0, "Prefs", ""},
    {0, NULL, 0, NULL, NULL},
};
...
{
  ARegionType *art;
  PanelType *pt = RNA_struct_ixam_type_get(type);
...
#  endif
  ...
  pt->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, pt->idname, &RNA_Panel);
  ...
  RNA_struct_ixam_type_set(pt->rna_ext.srna, pt);
...
{
  uiListType *ult = RNA_struct_ixam_type_get(type);
                                      ...
                                      StructFreeFunc free)
{
  ...
  ult->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, ult->idname, &RNA_UIList);
  ult->rna_ext.data = data;
  ult->rna_ext.call = call;
  ult->rna_ext.free = free;
  RNA_struct_ixam_type_set(ult->rna_ext.srna, ult);
...
{
  ARegionType *art;
  HeaderType *ht = RNA_struct_ixam_type_get(type);
                                      ...
                                      StructFreeFunc free)
{
  ...
  ht->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, ht->idname, &RNA_Header);
  ht->rna_ext.data = data;
  ht->rna_ext.call = call;
  ht->rna_ext.free = free;
  RNA_struct_ixam_type_set(ht->rna_ext.srna, ht);
...
{
  MenuType *mt = RNA_struct_ixam_type_get(type);
                                    ...
                                    StructFreeFunc free)
{
  ...
  mt->rna_ext.srna = RNA_def_struct_ptr(&IXAM_RNA, mt->idname, &RNA_Menu);
  ...
  RNA_struct_ixam_type_set(mt->rna_ext.srna, mt);
...
static bool rna_UILayout_fixed_size_get(PointerRNA *ptr)
{
  return uiLayoutGetFixedSize(ptr->data);
}

static void rna_UILayout_fixed_size_set(PointerRNA *ptr, bool value)
{
  return uiLayoutSetFixedSize(ptr->data, value);
}
...
static void rna_UILayout_shape_set(PointerRNA *ptr, int value)
{
  uiLayoutSetShape(ptr->data, value);
}

static int rna_UILayout_shape_get(PointerRNA *ptr)
{
  return uiLayoutGetAlignment(ptr->data);
}
...
static float rna_UILayout_icon_scale_get(PointerRNA *ptr)
{
  return uiLayoutGetIconScale(ptr->data);
}

static void rna_UILayout_icon_scale_set(PointerRNA *ptr, float value)
{
  uiLayoutSetIconScale(ptr->data, value);
}
...
static void rna_def_ui_layout(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  ...
  prop = RNA_def_property(srna, "fixed_size", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(prop, "rna_UILayout_fixed_size_get", "rna_UILayout_fixed_size_set");
...
#  endif
  ...
  RNA_def_property_enum_items(prop, rna_enum_button_shape_items);
  RNA_def_property_enum_funcs(prop, "rna_UILayout_shape_get", "rna_UILayout_shape_set", NULL);

  prop = RNA_def_property(srna, "icon_scale", PROP_FLOAT, PROP_UNSIGNED);
  RNA_def_property_float_funcs(prop, "rna_UILayout_icon_scale_get", "rna_UILayout_icon_scale_set", NULL);
  RNA_def_property_ui_text(
      prop, "Icon Scale", "Scale factor for icon in this (sub)layout");
...
static void rna_def_panel(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  PropertyRNA *parm;
...
static void rna_def_uilist(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  PropertyRNA *parm;
...
static void rna_def_header(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  PropertyRNA *parm;
...
static void rna_def_menu(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;
  PropertyRNA *parm;

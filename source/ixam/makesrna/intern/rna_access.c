...
void RNA_init(void)
{
  ...
  IXAM_RNA.structs_map = BLI_ghash_str_new_ex(__func__, 2048);
  IXAM_RNA.structs_len = 0;

  for (srna = IXAM_RNA.structs.first; srna; srna = srna->cont.next) {
    ...
    BLI_ghash_insert(IXAM_RNA.structs_map, (void *)srna->identifier, srna);
    IXAM_RNA.structs_len += 1;
  }
}
...
void RNA_ixam_rna_pointer_create(PointerRNA *r_ptr)
{
  r_ptr->owner_id = NULL;
  r_ptr->type = &RNA_IxamRNA;
  r_ptr->data = &IXAM_RNA;
}
...
{
  return BLI_ghash_lookup(IXAM_RNA.structs_map, identifier);
}
...
void *RNA_struct_ixam_type_get(StructRNA *srna)
{
  return srna->ixam_type;
}

void RNA_struct_ixam_type_set(StructRNA *srna, void *ixam_type)
{
  srna->ixam_type = ixam_type;
}

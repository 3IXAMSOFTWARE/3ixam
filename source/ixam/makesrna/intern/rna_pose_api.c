    ...
    bPoseChannel *pchan, ReportList *reports, float mat_ret[16], int index, bool rest)
{
  if (!pchan->bone || pchan->bone->segments <= 1) {
    BKE_reportf(reports, RPT_ERROR, "Joint '%s' is not a B-Joint!", pchan->name);
    return;
  }
  if (pchan->runtime.bbone_segments != pchan->bone->segments) {
    BKE_reportf(reports, RPT_ERROR, "Joint '%s' has out of date B-Joint segment data!", pchan->name);
    return;
  }
  if (index < 0 || index > pchan->runtime.bbone_segments) {
    BKE_reportf(
        reports, RPT_ERROR, "Invalid index %d for B-Joint segments of '%s'!", index, pchan->name);
    return;
  }
...
void RNA_api_pose(StructRNA *srna)
{
  ...
  RNA_def_function_ui_description(
      ...
      "pose of selected joints, or all joints if none are selected.");
...
void RNA_api_pose_channel(StructRNA *srna)
{
  ...
  RNA_def_function_ui_description(
      func, "Retrieve the matrix of the joint between B-Joint segments if available");
  RNA_def_function_flag(func, FUNC_USE_REPORTS);
  parm = RNA_def_property(func, "matrix_return", PROP_FLOAT, PROP_MATRIX);
  RNA_def_property_multi_array(parm, 2, rna_matrix_dimsize_4x4);
  RNA_def_property_ui_text(parm, "", "The resulting matrix in joint local space");
  ...
  RNA_def_function_ui_description(
      func, "Retrieve the vectors and rolls coming from B-Joint custom handles");
  RNA_def_function_flag(func, FUNC_USE_REPORTS);
  parm = RNA_def_property(func, "handle1", PROP_FLOAT, PROP_XYZ);
  RNA_def_property_array(parm, 3);
  RNA_def_property_ui_text(
      parm, "", "The direction vector of the start handle in joint local space");
  RNA_def_function_output(func, parm);
  parm = RNA_def_float(
  ...
  RNA_def_property_array(parm, 3);
  RNA_def_property_ui_text(parm, "", "The direction vector of the end handle in joint local space");
  RNA_def_function_output(func, parm);
  parm = RNA_def_float(
  ...
  parm = RNA_def_boolean(
      func, "offsets", false, "", "Apply roll and curve offsets from joint properties");
}

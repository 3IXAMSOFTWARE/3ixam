...
typedef void(CollectionDefFunc)(struct IxamRNA *brna, struct PropertyRNA *cprop);
...
void RNA_def_main(IxamRNA *brna)
{
  StructRNA *srna;
  PropertyRNA *prop;

  /* plural must match idtypes in readixamentry.c */
...
#  endif
  ...
  srna = RNA_def_struct(brna, "IxamData", NULL);
  RNA_def_struct_ui_text(srna,
                         "ixam-file Data",
                         "Main data structure representing a .ixam file and all its data-blocks");
  RNA_def_struct_ui_icon(srna, ICON_IXAM);
  ...
  RNA_def_property_ui_text(prop, "Filename", "Path to the .ixam file");
  ...
  RNA_def_property_ui_text(
      prop, "File is Saved", "Has the current session been saved to disk as a .ixam file");
  ...
  RNA_def_property_ui_text(
      prop, "Use Auto-Pack", "Automatically pack all external data into .ixam file");
  ...
  prop = RNA_def_int_vector(srna,
                            ...
                            INT_MAX,
                            "Version",
                            "File format version the .ixam file was saved with",
                            0,
                            INT_MAX);

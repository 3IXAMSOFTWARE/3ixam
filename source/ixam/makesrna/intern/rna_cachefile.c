...
static void rna_def_alembic_object_path(IxamRNA *brna)
{
...
/* cachefile.object_paths */
static void rna_def_cachefile_object_paths(IxamRNA *brna, PropertyRNA *cprop)
{
...
static void rna_def_cachefile_layer(IxamRNA *brna)
{
...
static void rna_def_cachefile_layers(IxamRNA *brna, PropertyRNA *cprop)
{
...
static void rna_def_cachefile(IxamRNA *brna)
{
  ...
  RNA_def_property_ui_text(
      ...
      "Display boxes in the viewport as placeholders for the objects, Real will use a "
  ...
  RNA_def_property_ui_text(
      prop,
      "Use Prefetch",
      "When enabled, the Real Procedural will preload animation data for faster updates");
  RNA_def_property_update(prop, 0, "rna_CacheFile_update");
  ...
  RNA_def_property_ui_text(
      prop,
      "Prefetch Cache Size",
      "Memory usage limit in megabytes for the Real Procedural cache, if the data does not "
...
void RNA_def_cachefile(IxamRNA *brna)
{

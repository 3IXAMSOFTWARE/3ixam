 ...
 * \ingroup ixamthumb
 *
 * Expose #ixamthumb_create_png_data_from_thumb that creates the PNG data
...
static void png_extend_native_int32(ixam::Vector<uint8_t> &output, int32_t data)
{
  if (ENDIAN_ORDER == L_ENDIAN) {
    BLI_endian_switch_int32(&data);
  }
  output.extend_unchecked(ixam::Span((uint8_t *)&data, 4));
}

/** The number of bytes each chunk uses on top of the data that's written. */
#define PNG_CHUNK_EXTRA 12

static void png_chunk_create(ixam::Vector<uint8_t> &output,
                             const uint32_t tag,
                             const ixam::Vector<uint8_t> &data)
{
  ...
  output.extend_unchecked(ixam::Span((uint8_t *)&tag, sizeof(tag)));
  output.extend_unchecked(data);
  png_extend_native_int32(output, crc);
}

static ixam::Vector<uint8_t> filtered_rows_from_thumb(const Thumbnail *thumb)
{
  ...
  for (int i = 0; i < thumb->height; i++) {
    ...
    filtered.extend_unchecked(ixam::Span(&thumb->data[i * line_size], line_size));
  }
  BLI_assert(final_size == filtered.size());
  return filtered;
}

static std::optional<ixam::Vector<uint8_t>> zlib_compress(const ixam::Vector<uint8_t> &data)
{
  ulong uncompressed_size = data.size();
  uLongf compressed_size = compressBound(uncompressed_size);

  ixam::Vector<uint8_t> compressed(compressed_size, 0x00);
...
std::optional<ixam::Vector<uint8_t>> ixamthumb_create_png_data_from_thumb(
    const Thumbnail *thumb)
{
  ...
  ixam::Vector<uint8_t> image_data{};
  {

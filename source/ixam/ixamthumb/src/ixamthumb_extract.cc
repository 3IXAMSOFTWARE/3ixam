 ...
 * \ingroup ixamthumb
 *
 * Expose #ixamthumb_create_thumb_from_file that creates the PNG data
...
#include "ixamthumb.hh"

static bool ixam_header_check_magic(const char header[12])
{
  /* Check magic string at start of file. */
  if (!BLI_str_startswith(header, "IXAM")) {
    return false;
  }
...
static bool ixam_header_is_version_valid(const char header[12])
{
...
static int ixam_header_pointer_size(const char header[12])
{
  return header[7] == '_' ? 4 : 8;
}

static bool ixam_header_is_endian_switch_needed(const char header[12])
{
...
static bool file_seek(FileReader *file, size_t len)
{
  ...
  ixam::Array<char> dummy_data(dummy_data_size);
  while (len > 0) {
...
static eThumbStatus ixamthumb_extract_from_file_impl(FileReader *file,
                                                      Thumbnail *thumb,
  ...
  while (file_read(file, bhead_data, bhead_size)) {
    ...
    switch (*((int32_t *)bhead_data)) {
      case MAKE_ID('T', 'E', 'S', 'T'): {
        ...
        thumb->data = ixam::Array<uint8_t>(data_size);
      ...
      default: {
         ...
         * This saves scanning the entire ixam file which could be slow. */
        return BT_INVALID_THUMB;
      }
    }
  }
...
eThumbStatus ixamthumb_create_thumb_from_file(FileReader *rawfile, Thumbnail *thumb)
{
  ...
  /* Check if the header format is valid for a .ixam file. */
  if (!ixam_header_check_magic(header)) {
    file->close(file);
    return BT_INVALID_FILE;
  }

  /* Check if the file is new enough to contain a thumbnail. */
  if (!ixam_header_is_version_valid(header)) {
    file->close(file);
    return BT_EARLY_VERSION;
  }

  /* Depending on where it was saved, the file can use different pointer size or endianness. */
  int bhead_size = 16 + ixam_header_pointer_size(header);
  const bool endian_switch = ixam_header_is_endian_switch_needed(header);

  /* Read the thumbnail. */
  eThumbStatus err = ixamthumb_extract_from_file_impl(file, thumb, bhead_size, endian_switch);
  file->close(file);
  if (err != BT_OK) {
    return err;
  }

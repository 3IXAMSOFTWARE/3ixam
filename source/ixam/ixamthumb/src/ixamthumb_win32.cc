 ...
 * \ingroup ixamthumb
 *
 * Thumbnail from ixam file extraction for MS-Windows.
 */
 ...
 */
class CIxamThumb : public IInitializeWithStream, public IThumbnailProvider {
 public:
  CIxamThumb() : _cRef(1), _pStream(NULL)
  {
  }
  ...
  IFACEMETHODIMP QueryInterface(REFIID riid, void **ppv)
  {
    static const QITAB qit[] = {
        QITABENT(CIxamThumb, IInitializeWithStream),
        QITABENT(CIxamThumb, IThumbnailProvider),
        {0},
    };
...
HRESULT CIxamThumb_CreateInstance(REFIID riid, void **ppv)
{
  CIxamThumb *pNew = new (std::nothrow) CIxamThumb();
...
IFACEMETHODIMP CIxamThumb::Initialize(IStream *pStream, DWORD)
{
  if (_pStream != NULL) {
...
/**
 * #FileReader compatible wrapper around the Windows stream that gives access to the .ixam file.
 */
typedef struct {
  FileReader reader;
...
IFACEMETHODIMP CIxamThumb::GetThumbnail(UINT cx, HBITMAP *phbmp, WTS_ALPHATYPE *pdwAlpha)
{
  HRESULT hr = S_FALSE;
  ...
  Thumbnail thumb;
  if (ixamthumb_create_thumb_from_file(&file->reader, &thumb) != BT_OK) {
    return S_FALSE;
  }

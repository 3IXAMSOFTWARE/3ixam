 ...
 * and arbitrary text data to store in ixam files.
 */
...
enum {
  ...
  TXT_ISMEM = 1 << 2,
  /** Should always be set if the Text is not to be written into the `.ixam`. */
  TXT_ISEXT = 1 << 3,
  /** Load the script as a Python module when loading the `.ixam` file. */
  TXT_ISSCRIPT = 1 << 4,

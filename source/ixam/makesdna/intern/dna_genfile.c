 ...
 * \brief DNA handling
 *
 * Lowest-level functions for decoding the parts of a saved .ixam
 ...
 * SDNA and the SDNA of the current (running) version of 3IXAM.
 */
 ...
 *
 * Structure DNA data is added to each ixam file and to each executable, this to detect
 * in .ixam files new variables in structs, changed array sizes, etc. It's also used for
 * converting endian and pointer size (32-64 bits)
 * As an extra, Python uses a call to detect run-time the contents of 3IXAM struct.
 *
 ...
 *
 * For the moment: the complete DNA file is included in a .ixam file. For
 ...
 *  - Only use a long in 3IXAM if you want this to be the size of a pointer. so it is
 ...
 *    always check ixam running from a console.
 */
                         ...
                         const char **r_error_message)
{
  ...
  if (r_error_message == NULL) {
    fprintf(stderr, "Error decoding ixam file SDNA: %s\n", error_message);
  }
  else {
 ...
 * the data is read-only and only changes between 3IXAM versions.
 *
...
static void cast_pointer_64_to_32(const int array_len,
                                  ...
                                  uint32_t *new_data)
{
  /* WARNING: 32-bit 3IXAM trying to load file saved by 64-bit 3IXAM,
   * pointers may lose uniqueness on truncation! (Hopefully this won't
   * happen unless/until we ever get to multi-gigabyte .ixam files...) */
  for (int a = 0; a < array_len; a++) {

...
using NodeDeclarationHandle = ixam::nodes::NodeDeclaration;
using SocketDeclarationHandle = ixam::nodes::SocketDeclaration;
using bNodeTreeRuntimeHandle = ixam::bke::bNodeTreeRuntime;
using bNodeRuntimeHandle = ixam::bke::bNodeRuntime;
using bNodeSocketRuntimeHandle = ixam::bke::bNodeSocketRuntime;
#else
...
typedef struct bNodeSocket {
  ...
  ixam::Span<bNodeLink *> directly_linked_links();
  ixam::Span<const bNodeLink *> directly_linked_links() const;
  /** Sockets which are connected to this socket with a link. */
  ixam::Span<bNodeSocket *> directly_linked_sockets();
  ixam::Span<const bNodeSocket *> directly_linked_sockets() const;
  bool is_directly_linked() const;
  /**
   * Sockets which are connected to this socket when reroutes and muted nodes are taken into
   * account.
   */
  ixam::Span<const bNodeSocket *> logically_linked_sockets() const;
...
typedef struct bNode {
...
#ifdef __cplusplus
  ixam::StringRefNull label_or_name() const;
  bool is_muted() const;
  ...
  const ixam::nodes::NodeDeclaration *declaration() const;

  /* The following methods are only available when #bNodeTree.ensure_topology_cache has been
   * called. */

  /** A span containing all input sockets of the node (including unavailable sockets). */
  ixam::Span<bNodeSocket *> input_sockets();
  ixam::Span<const bNodeSocket *> input_sockets() const;
  /** A span containing all output sockets of the node (including unavailable sockets). */
  ixam::Span<bNodeSocket *> output_sockets();
  ixam::Span<const bNodeSocket *> output_sockets() const;
  ...
  ixam::Span<const bNodeLink *> internal_links_span() const;
  /** Lookup socket of this node by its identifier. */
  const bNodeSocket &input_by_identifier(ixam::StringRef identifier) const;
  const bNodeSocket &output_by_identifier(ixam::StringRef identifier) const;
...
using FieldInferencingInterfaceHandle = ixam::nodes::FieldInferencingInterface;
#else
...
typedef struct bNodeTree {
  ...
  ixam::Span<bNode *> all_nodes();
  ixam::Span<const bNode *> all_nodes() const;
  /** A span containing all group nodes in the node tree. */
  ixam::Span<bNode *> group_nodes();
  ixam::Span<const bNode *> group_nodes() const;
  /** A span containing all input sockets in the node tree. */
  ixam::Span<bNodeSocket *> all_input_sockets();
  ixam::Span<const bNodeSocket *> all_input_sockets() const;
  /** A span containing all output sockets in the node tree. */
  ixam::Span<bNodeSocket *> all_output_sockets();
  ixam::Span<const bNodeSocket *> all_output_sockets() const;
  /** A span containing all sockets in the node tree. */
  ixam::Span<bNodeSocket *> all_sockets();
  ixam::Span<const bNodeSocket *> all_sockets() const;
  /** Efficient lookup of all nodes with a specific type. */
  ixam::Span<bNode *> nodes_by_type(ixam::StringRefNull type_idname);
  ixam::Span<const bNode *> nodes_by_type(ixam::StringRefNull type_idname) const;
  /**
   ...
   */
  ixam::Span<const bNode *> toposort_left_to_right() const;
  ixam::Span<const bNode *> toposort_right_to_left() const;
   ...
   * happen when nodes don't exist in the current 3IXAM version that existed in the version where
...
#define SHD_SPACE_IXAM_OBJECT 3
#define SHD_SPACE_IXAM_WORLD 4

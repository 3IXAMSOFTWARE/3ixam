...
#ifdef __cplusplus
namespace ixam::ed::space_node {
struct SpaceNode_Runtime;
}  // namespace ixam::ed::space_node
using SpaceNode_Runtime = ixam::ed::space_node::SpaceNode_Runtime;
namespace ixam::ed::space_matpro {
struct SpaceMatPro_Runtime;
}  // namespace ixam::ed::space_node
using SpaceMatPro_Runtime = ixam::ed::space_matpro::SpaceMatPro_Runtime;

namespace ixam::ed::outliner {
struct SpaceOutliner_Runtime;
}  // namespace ixam::ed::outliner
using SpaceOutliner_Runtime = ixam::ed::outliner::SpaceOutliner_Runtime;
#else
typedef struct SpaceNode_Runtime SpaceNode_Runtime;
typedef struct SpaceMatPro_Runtime SpaceMatPro_Runtime;
...
typedef enum eSpaceOutliner_Filter {
  ...
  /** Show overrides that are defined/controlled by 3IXAM. */
...
typedef enum eGraphEdit_Mode {
  /* all animation curves (from all over 3IXAM) */
...
typedef struct FileSelectParams {
   ...
   * needs to be linked in, where foo.ixam/Armature need adding
   */
  char dir[1090];
  char file[256];
...
typedef enum eFileSelectType {
  ...
  FILE_UNIX = 8,
  FILE_IXAM = 8, /* don't display relative paths */
  FILE_SPECIAL = 9,
} eFileSelectType;
...
typedef enum eFileSel_File_Types {
  ...
  FILE_TYPE_DIR = (1 << 30),
  FILE_TYPE_IXAMLIB = (1u << 31),
} eFileSel_File_Types;
ENUM_OPERATORS(eFileSel_File_Types, FILE_TYPE_IXAMLIB);
...
enum {
  ...
  /** For #FILE_TYPE_IXAMLIB only: Denotes that the ID is known to not have a preview (none was
   * found in the .ixam). Stored so we don't keep trying to find non-existent previews every time
   * we reload previews. When dealing with heavy files this can have quite an impact. */
  FILE_ENTRY_IXAMLIB_NO_PREVIEW = 1 << 3,
};
...
typedef struct Script {
  ...
  int flags, lastspace;
  /**
   * Store the script file here so we can re-run it on loading ixam,
...
typedef enum eMatPro_Section {
  MATPRO_SECTION_INPUT = 0,
  MATPRO_SECTION_OUTPUT = 1,
  MATPRO_SECTION_SHADER = 2,
  MATPRO_SECTION_TEXTURE = 3,
  MATPRO_SECTION_COLOR = 4,
  MATPRO_SECTION_VECTOR = 5,
  MATPRO_SECTION_CONVERTER = 6,
} eMatPro_Section;

typedef struct SpaceMatPro {
  SpaceLink *next, *prev;
  /** Storage of regions for inactive spaces. */
  ListBase regionbase;
  char spacetype;
  ...
  SpaceMatPro_Runtime *runtime;

  short preview;
  char palette_section_active;
  char _pad3[5];

  struct Material *active_material;
} SpaceMatPro;
...
/** (UI active_section) */
typedef enum eMatLib_Section {
  MATLIB_SECTION_COMMON = 0,
  MATLIB_SECTION_SCENE = 1,
  MATLIB_SECTION_CAMERA = 2,
  MATLIB_SECTION_RENDER_ELEMENTS = 3,
} eMatLib_Section;

typedef struct SpaceMatLib {
  SpaceLink *next, *prev;
  /** Storage of regions for inactive spaces. */
  ListBase regionbase;
  char spacetype;
...
typedef struct SpaceRender {
  ...
  MaskSpaceInfo mask_info;
  SpaceImageOverlay overlay;
  char menu_section_active;
  char post_section_active;
  char statistic_section_active;
  char _pad4[5];
} SpaceRender;
...
#define SPACE_TYPE_LAST SPACE_MATPRO + 1
} eSpace_Type;

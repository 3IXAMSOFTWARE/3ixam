...
using MeshRuntimeHandle = ixam::bke::MeshRuntime;
#else
...
typedef struct Mesh {
   ...
   * into this array.
   */
  ixam::Span<MVert> verts() const;
  /** Write access to vertex data. */
  ixam::MutableSpan<MVert> verts_for_write();
  /**
   ...
   */
  ixam::Span<MDeformVert> deform_verts() const;
  /** Write access to vertex group data. */
  ixam::MutableSpan<MDeformVert> deform_verts_for_write();

  /**
   * Cached triangulation of the mesh.
   */
  ixam::Span<MLoopTri> looptris() const;
#endif
} Mesh;

...
enum {
  /** new_id is directly linked in current .ixam. */
...
typedef struct ID {
   ...
   * (persistent, saved to and read from .ixam).
   */
  short flag;
  /**
...
typedef struct Library {
  ...
  int temp_index;
  /** See IXAM_FILE_VERSION, IXAM_FILE_SUBVERSION, needed for do_versions. */
...
#define ID_IXAM_PATH(_bmain, _id) \
  ((_id)->lib ? (_id)->lib->filepath_abs : BKE_main_ixamfile_path((_bmain)))
#define ID_IXAM_PATH_FROM_GLOBAL(_id) \
  ((_id)->lib ? (_id)->lib->filepath_abs : BKE_main_ixamfile_path_from_global())
...
enum {
   ...
   * tag set. But the current .ixam file also has a weak pointer to it that
 ...
 *   (except on initialization during .ixam file reading).
 */
enum {
  /* RESET_NEVER Datablock is from current .ixam file. */
  LIB_TAG_LOCAL = 0,
  /* RESET_NEVER Datablock is from a library,
   * but is used (linked) directly by current .ixam file. */
  LIB_TAG_EXTERN = 1 << 0,
  /**
   ...
   * pointers to other data-blocks updated from the 'UID' values stored in .ixam files to the new,
   * actual pointers.
   *
   * RESET_AFTER_USE
   */
...
#define FILTER_ID_ALL \
  (FILTER_ID_CA | FILTER_ID_GR | FILTER_ID_IM | FILTER_ID_LA | FILTER_ID_MA | \
   FILTER_ID_MC | FILTER_ID_ME | FILTER_ID_MSK | FILTER_ID_OB | FILTER_ID_SCE | \
   FILTER_ID_TE | FILTER_ID_TXT | FILTER_ID_VF | FILTER_ID_SIM)
...
enum {
  ...
  /* File-wrapper types, those usually 'embed' external files in 3IXAM, with no dependencies to
   * other ID types. */
  INDEX_ID_VF,
  INDEX_ID_TXT,
  INDEX_ID_SO,

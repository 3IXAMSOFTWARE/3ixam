...
#  endif
  ...
  switch (size) {
    ...
    case THB_LARGE:
      ...
      break;
    case THB_FAIL:
      subdir = SEP_STR THUMBNAILS SEP_STR "fail" SEP_STR "ixam" SEP_STR;
      break;
    default:
                              ...
                              ImBuf *img)
{
  ...
  if (get_thumb_dir(tdir, size)) {
    ...
    BLI_snprintf(temp, FILE_MAX, "%sixam_%d_%s.png", tdir, abs(getpid()), thumb);
    ...
    if (size == THB_FAIL) {
      ...
      if (ELEM(source, THB_SOURCE_IMAGE, THB_SOURCE_IXAM, THB_SOURCE_FONT)) {
        ...
        if (img == NULL) {
          switch (source) {
            case THB_SOURCE_IMAGE:
              ...
              break;
            case THB_SOURCE_IXAM:
              img = IMB_thumb_load_ixam(file_path, ixam_group, ixam_id);
              break;
            case THB_SOURCE_FONT:
    ...
    IMB_metadata_set_field(img->metadata, "Software", "3IXAM");
    ...
    if (use_hash) {
      IMB_metadata_set_field(img->metadata, "X-3IXAM::Hash", hash);
    }
                                   ...
                                   ThumbSource source)
{
  ImBuf *img = thumb_create_ex(
      file_path, uri, thumb, use_hash, hash, ixam_group, ixam_id, size, source, NULL);

  if (!img) {
    /* thumb creation failed, write fail thumb */
    img = thumb_create_ex(
        file_path, uri, thumb, use_hash, hash, ixam_group, ixam_id, THB_FAIL, source, NULL);
    if (img) {
...
ImBuf *IMB_thumb_manage(const char *filepath, ThumbSize size, ThumbSource source)
{
  ...
  BLI_stat_t st;
  ImBuf *img = NULL;
  char *ixam_group = NULL, *ixam_id = NULL;

  path = file_path = filepath;
  if (source == THB_SOURCE_IXAM) {
    if (BLO_library_path_explode(path, path_buff, &ixam_group, &ixam_id)) {
      if (ixam_group) {
        if (!ixam_id) {
  ...
  if (thumbpath_from_uri(uri, thumb_path, sizeof(thumb_path), THB_FAIL)) {
    ...
    if (BLI_exists(thumb_path)) {
      /* clear out of date fail case (note for blen IDs we use ixam file itself here) */
      ...
      if (img) {
        ...
        if (use_hash && !regenerate) {
          if (IMB_metadata_get_field(
                  img->metadata, "X-3IXAM::Hash", thumb_hash_curr, sizeof(thumb_hash_curr))) {
        ...
        if (regenerate) {
          ...
          img = thumb_create_or_fail(
              file_path, uri, thumb_name, use_hash, thumb_hash, ixam_group, ixam_id, size, source);
        }
      }
      else {
        char thumb_hash[33];

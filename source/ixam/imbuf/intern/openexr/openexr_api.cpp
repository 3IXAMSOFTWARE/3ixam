...
/* Multi-layer 3IXAM files have the view name in all the passes (even the default view one). */
                         ...
                         const StampData *stamp)
{
  ...
  if (is_multilayer) {
    header.insert("IxamMultiChannel", StringAttribute("3IXAM V2.55.1 and newer"));
  }
    ...
    void *handle, const char *filepath, int mipmap, int width, int height, int tilex, int tiley)
{
  ...
  header.insert("IxamMultiChannel", StringAttribute("3IXAM V2.43"));
...
void IMB_exr_read_channels(void *handle)
{
  ...
  /* Check if EXR was saved with previous versions of ixam which flipped images. */
  const StringAttribute *ta = data->ifile->header(0).findTypedAttribute<StringAttribute>(
      "IxamMultiChannel");

  /* 'previous multilayer attribute, flipped. */
  short flip = (ta && STRPREFIX(ta->value().c_str(), "3IXAM V2.43"));
    ...
    for (echan = (ExrChannel *)data->channels.first; echan; echan = echan->next) {
      ...
      if (echan->rect) {
        ...
        if (!flip) {
          ...
          /* Move to last scan-line to flip to 3IXAM convention. */
...
{
  /* Multipart files are treated as multilayer in ixam -

...
/* This little block needed for linking to 3IXAM... */
#include <setjmp.h>
#include <stdio.h>
  ...
  if (ibuf->metadata) {
    ...
    for (prop = ibuf->metadata->data.group.first; prop; prop = prop->next) {
      if (prop->type == IDP_STRING) {
        ...
        /* 7 is for 3IXAM, 2 colon separators, length of property
         ...
         * "3IXAM:key:value"
         *
         * The first "3IXAM" is a simple identify to help
         * in the read process.
         */
        text_len = BLI_snprintf_rlen(
            text, text_size, "3IXAM:%s:%s", prop->name, IDP_String(prop));

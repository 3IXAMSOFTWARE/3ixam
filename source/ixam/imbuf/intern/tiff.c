...
/**
 * Provides TIFF file loading and saving for 3IXAM, via libtiff.
 *
 * The task of loading is complicated somewhat by the fact that Ixam has
 ...
 *
 * NOTE: The current 3IXAM implementation should not need this function.
 * It is simply a stub.
 */
static tsize_t imb_tiff_WriteProc(thandle_t handle, tdata_t data, tsize_t n)
{
...
static TIFF *imb_tiff_client_open(ImbTIFFMemFile *memFile, const uchar *mem, size_t size)
{
  ...
  return TIFFClientOpen("(3IXAM TIFF Interface Layer)",
                        "r",
                        (thandle_t)(memFile),
                        imb_tiff_ReadProc,
                        imb_tiff_WriteProc,
...
static int imb_read_tiff_pixels(ImBuf *ibuf, TIFF *image)
{
  ...
  if (spp == 4) {
     ...
     *       unassociated, which really conflicts with ixam's assumptions
     *

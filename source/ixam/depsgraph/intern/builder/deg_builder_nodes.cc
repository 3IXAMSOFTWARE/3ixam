...
void DepsgraphNodeBuilder::update_invalid_cow_pointers()
{
   ...
   * some cases. This is slightly unfortunate (as it may hide issues in other parts of 3IXAM
  ...
  for (const IDNode *id_node : graph_->id_nodes) {
    ...
    if ((id_node->id_cow->flag & LIB_EMBEDDED_DATA) != 0) {
       ...
       * 3IXAM (like read/write code, including undo/redo), while depsgraph generally treat them

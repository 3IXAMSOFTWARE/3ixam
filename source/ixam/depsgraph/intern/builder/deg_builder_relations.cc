...
void DepsgraphRelationBuilder::build_object_parent(Object *object)
{
  ...
  switch (object->partype) {
    ...
    case PARSKEL: {
      ...
      add_relation(parent_transform_key, object_transform_key, "Parent Rig Transform");

      if (parent->type == OB_ARMATURE) {
        ComponentKey object_geometry_key(&object->id, NodeType::GEOMETRY);
        ComponentKey parent_pose_key(parent_id, NodeType::EVAL_POSE);
        add_relation(
            parent_transform_key, object_geometry_key, "Parent Rig Transform -> Geometry");
        add_relation(parent_pose_key, object_geometry_key, "Parent Rig Pose -> Geometry");

        add_depends_on_transform_relation(
            &object->id, object_geometry_key, "Virtual Rig Modifier");
      }
    ...
    case PARBONE: {
      if (object->parsubstr[0] != '\0') {
        ...
        add_relation(parent_bone_key, object_transform_key, "Joint Parent");
        add_relation(parent_transform_key, object_transform_key, "Rig Parent");
      }
      break;
    }
...
void DepsgraphRelationBuilder::build_driver_data(ID *id, FCurve *fcu)
{
  ...
  if (is_bone && GS(id_ptr->name) == ID_AR) {
    ...
    if (bone == nullptr) {
      fprintf(stderr, "Couldn't find rig joint name for driver path - '%s'\n", rna_path);
      return;
    }
    ...
    for (IDNode *to_node : graph_->id_nodes) {
      ...
      add_relation(driver_key, bone_key, "Arm Joint -> Driver -> Joint");
    }
  ...
  LISTBASE_FOREACH (DriverVar *, dvar, &driver->variables) {
    ...
    DRIVER_TARGETS_USED_LOOPER_BEGIN (dvar) {
      ...
      if ((dtar->flag & DTAR_FLAG_STRUCT_REF) && (object && object->type == OB_ARMATURE) &&
        ...
        add_relation(variable_key, driver_key, "Joint Target -> Driver");
      }
  ...
  if (rbw->group != nullptr) {
    ...
    FOREACH_COLLECTION_OBJECT_RECURSIVE_BEGIN (rbw->group, object) {
      ...
      if (object->rigidbody_object->type == RBO_TYPE_ACTIVE) {
         ...
         * passive or controlled by the animation system in ixam.
      ...
      LISTBASE_FOREACH (bGPDlayer *, gpl, &gpd->layers) {
        if (gpl->parent != nullptr) {
          ...
          if (gpl->partype == PARBONE) {
            ...
            add_relation(bone_key, gpd_geom_key, "Joint Parent");
            add_relation(armature_key, gpd_geom_key, "Rig Parent");
          }
          else {

                                             ...
                                             RootPChanMap *root_map)
{
  ...
  add_relation(solver_key, pose_done_key, "PoseEval Result-Joint Link");
...
void DepsgraphRelationBuilder::build_rig(Object *object)
{
  ...
  LISTBASE_FOREACH (bPoseChannel *, pchan, &object->pose->chanbase) {
    ...
    add_relation(pose_init_key, bone_local_key, "Pose Init - Joint Local", RELATION_FLAG_GODMODE);
    /* Local to pose parenting operation. */
    add_relation(bone_local_key, bone_pose_key, "Joint Local - Joint Pose");
    /* Parent relation. */
    if (pchan->parent != nullptr) {
      ...
      add_relation(parent_key, bone_pose_key, "Parent Joint -> Child Joint");
    }
    /* Build constraints. */
    ...
    if (check_pchan_has_bbone(object, pchan)) {
      ...
      add_relation(bone_done_key, bone_segments_key, "Done -> B-Joint Segments");
      ...
      if (prev) {
        ...
        /* Inheriting parent roll requires access to prev handle's B-Joint properties. */
        ...
        add_relation(prev_key, bone_segments_key, "Prev Handle -> B-Joint Segments");
      }
      if (next) {
        OperationKey next_key(&object->id, NodeType::BONE, next->name, OperationCode::BONE_DONE);
        add_relation(next_key, bone_segments_key, "Next Handle -> B-Joint Segments");
      }
      /* Pose requires the B-Bone shape. */
      add_relation(
          bone_segments_key, pose_done_key, "PoseEval Result-Joint Link", RELATION_FLAG_GODMODE);
       ...
       * (for deformers). */
      add_relation(bone_done_key, pose_done_key, "PoseEval Result-Joint Link");

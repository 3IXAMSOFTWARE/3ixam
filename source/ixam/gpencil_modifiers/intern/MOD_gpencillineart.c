...
static void panel_draw(const bContext *UNUSED(C), Panel *panel)
{
   ...
   * without this requirement in earlier versions of ixam. */
...
static void edge_types_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  uiItemL(layout, IFACE_("Options"), ICON_NONE, UI_BSHAPE_ROUNDBOX);

  sub = uiLayoutColumn(layout, false);
  if (use_cache && !is_first) {
    uiItemL(entry, IFACE_("Angle Cached"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }
  else {
    uiItemR(sub,
            ptr,
...
static void options_light_reference_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (use_cache && !is_first) {
    uiItemL(layout, "Cached from the first line art modifier.", ICON_INFO, UI_BSHAPE_ROUNDBOX);
    return;
  }
...
static void options_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (use_cache && !is_first) {
    uiItemL(layout, TIP_("Cached from the first line art modifier"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
    return;
  }
...
static void occlusion_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (!show_in_front) {
    uiItemL(layout, TIP_("Object is not in front"), ICON_INFO, UI_BSHAPE_ROUNDBOX);
  }
...
static void face_mark_panel_draw_header(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (!use_cache || is_first) {
    ...
    uiItemL(layout, IFACE_("Face Mark Filtering"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void bake_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (is_baked) {
    ...
    uiItemL(col, TIP_("Modifier has baked data"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(
...
static void composition_panel_draw(const bContext *UNUSED(C), Panel *panel)
{
  ...
  if (show_in_front) {
    uiItemL(layout, TIP_("Object is shown in front"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }

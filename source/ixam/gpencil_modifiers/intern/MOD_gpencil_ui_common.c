...
void gpencil_modifier_masking_panel_draw(Panel *panel, bool use_material, bool use_vertex)
{
  ...
  if (use_material) {
     ...
     * 3IXAM, we need to check if the material is valid and display it differently if so. */
    bool valid = false;
    {
      if (!has_material) {
        valid = true;
...
void gpencil_modifier_panel_end(uiLayout *layout, PointerRNA *ptr)
{
  ...
  if (md->error) {
    ...
    uiItemL(row, IFACE_(md->error), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
}
...
static void gpencil_modifier_panel_header(const bContext *UNUSED(C), Panel *panel)
{
  ...
  uiItemL(row, "", RNA_struct_ui_icon(ptr->type), UI_BSHAPE_ROUNDBOX);

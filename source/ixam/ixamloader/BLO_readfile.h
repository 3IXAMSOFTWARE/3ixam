...
typedef struct IxamHandle IxamHandle;
...
typedef struct IxamFileReadReport {
...
/**
 * Open a ixam file from a pathname. The function returns NULL
 ...
 */
IxamFileData *BLO_read_from_file(const char *filepath,
                                  eBLOReadSkip skip_flags,
                                  struct IxamFileReadReport *reports);
/**
 * Open a ixam file from memory. The function returns NULL
 * and sets a report in the list if it cannot open the file.
 *
 ...
 * \return The data of the file.
 */
IxamFileData *BLO_read_from_memory(const void *mem,
                                    int memsize,
                                    eBLOReadSkip skip_flags,
                                    struct ReportList *reports);
/**
 ...
 * Typically `BKE_main_ixamfile_path(oldmain)`.
 */
IxamFileData *BLO_read_from_memfile(struct Main *oldmain,
                                     const char *filepath,
...
/**
 * Frees a IxamFileData structure and *all* the data associated with it
 * (the userdef data, and the main libblock data).
 *
 * \param bfd: The structure to free.
 */
void BLO_ixamfiledata_free(IxamFileData *bfd);

/** \} */

/* -------------------------------------------------------------------- */
/** \name BLO Ixam file Handle API
 * \{ */
...
/**
 * Open a ixamhandle from a file path.
 *
 * \param filepath: The file path to open.
 * \param reports: Report errors in opening the file (can be NULL).
 * \return A handle on success, or NULL on failure.
 */
IxamHandle *BLO_ixamhandle_from_file(const char *filepath, struct IxamFileReadReport *reports);
/**
 * Open a ixamhandle from memory.
 *
 * \param mem: The data to load from.
 * \param memsize: The size of the data.
 * \return A handle on success, or NULL on failure.
 */
IxamHandle *BLO_ixamhandle_from_memory(const void *mem,
                                         int memsize,
                                         struct IxamFileReadReport *reports);
 ...
 *
 * \param bh: The ixamhandle to access.
 ...
 */
struct LinkNode *BLO_ixamhandle_get_datablock_names(IxamHandle *bh,
                                                     int ofblocktype,
 ...
 */
struct LinkNode * /*BLODataBlockInfo*/ BLO_ixamhandle_get_datablock_info(IxamHandle *bh,
                                                                          int ofblocktype,
                                                                          bool use_assets_only,
 ...
 */
struct LinkNode *BLO_ixamhandle_get_previews(IxamHandle *bh, int ofblocktype, int *r_tot_prev);
/**
 ...
 */
struct PreviewImage *BLO_ixamhandle_get_preview_for_id(IxamHandle *bh,
                                                        int ofblocktype,
                                                        const char *name);
/**
 ...
 * \return A BLI_linklist of strings. The string links should be freed with #MEM_freeN().
 */
struct LinkNode *BLO_ixamhandle_get_linkable_groups(IxamHandle *bh);

/**
 * Close and free a ixamhandle. The handle becomes invalid after this call.
 *
 * \param bh: The handle to close.
 */
void BLO_ixamhandle_close(IxamHandle *bh);

/** \} */

#define BLO_GROUP_MAX 32
#define BLO_EMBEDDED_STARTUP_IXAM "<startup.ixam>"

/**
 * Check whether given path ends with a ixam file compatible extension
 * (`.ixam`, `.ixam` or `.ixam.gz`).
 *
 * \param str: The path to check.
 * \return true is this path ends with a ixam file extension.
 */
bool BLO_has_bfile_extension(const char *str);
/**
 * Try to explode given path into its 'library components'
 * (i.e. a .ixam file, id type/group, and data-block itself).
 *
 * \param path: the full path to explode.
 * \param r_dir: the string that'll contain path up to ixam file itself ('library' path).
 * WARNING! Must be #FILE_MAX_LIBEXTRA long (it also stores group and name strings)!
 * \param r_group: the string that'll contain 'group' part of the path, if any. May be NULL.
 * \param r_name: the string that'll contain data's name part of the path, if any. May be NULL.
 * \return true if path contains a ixam file.
 */
bool BLO_library_path_explode(const char *path, char *r_dir, char **r_group, char **r_name);

/* -------------------------------------------------------------------- */
/** \name BLO Ixam file Linking API
 * \{ */
...
/**
 * Initialize the #IxamHandle for linking library data.
 *
 * \param bh: A ixam file handle as returned by
 * #BLO_ixamhandle_from_file or #BLO_ixamhandle_from_memory.
 ...
 */
struct Main *BLO_library_link_begin(IxamHandle **bh,
                                    const char *filepath,
                                    const struct LibraryLink_Params *params);
/**
 * Link a named data-block from an external ixam file.
 *
 * \param mainl: The main database to link from (not the active one).
 * \param bh: The ixam file handle.
...
struct ID *BLO_library_link_named_part(struct Main *mainl,
                                       ...
                                       const struct LibraryLink_Params *params);
/**
 * Finalize linking from a given .ixam file (library).
 * Optionally instance the indirect object/collection in the scene when the flags are set.
 * \note Do not use \a bh after calling this function, it may frees it.
 *
 * \param mainl: The main database to link from (not the active one).
 * \param bh: The ixam file handle (WARNING! may be freed by this function!).
 * \param params: Settings for linking that don't change from beginning to end of linking.
 */
...
/**
 * Struct for temporarily loading datablocks from a ixam file.
 */
typedef struct TempLibraryContext {
  ...
  struct IxamHandle *ixamhandle;
  struct IxamFileReadReport bf_reports;
...
/**
 * Update defaults in startup.ixam, without having to save and embed it.
 * \note defaults for preferences are stored in `userdef_default.c` and can be updated there.
 */
/**
 * Update defaults in startup.ixam, without having to save and embed the file.
 * This function can be emptied each time the startup.ixam is updated.
 *
 ...
 */
void BLO_update_defaults_startup_ixam(struct Main *bmain, const char *app_template);
...
void BLO_sanitize_experimental_features_userpref_ixam(struct UserDef *userdef);

/**
 * Does a very light reading of given .ixam file to extract its stored thumbnail.
 *
 ...
 */
struct IxamThumbnail *BLO_thumbnail_from_file(const char *filepath);

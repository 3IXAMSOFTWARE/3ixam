...
void IxamfileLoadingBaseTest::SetUpTestCase()
{
  testing::Test::SetUpTestCase();

  /* Minimal code to make loading a ixamfile and constructing a depsgraph not crash, copied from
   * main() in creator.c. */
  CLG_init();
  BLI_threadapi_init();

  DNA_sdna_current_init();
  BKE_ixam_globals_init();
...
void IxamfileLoadingBaseTest::TearDownTestCase()
{
...
void IxamfileLoadingBaseTest::TearDown()
{
...
bool IxamfileLoadingBaseTest::ixamfile_load(const char *filepath)
{
  const std::string &test_assets_dir = ixam::tests::flags_test_asset_dir();
  ...
  IxamFileReadReport bf_reports = {nullptr};
...
void IxamfileLoadingBaseTest::ixamfile_free()
{
  if (bfile == nullptr) {
    return;
  }
  ...
  BLO_ixamfiledata_free(bfile);
  bfile = nullptr;
}

void IxamfileLoadingBaseTest::depsgraph_create(eEvaluationMode depsgraph_evaluation_mode)
{
...
void IxamfileLoadingBaseTest::depsgraph_free()
{

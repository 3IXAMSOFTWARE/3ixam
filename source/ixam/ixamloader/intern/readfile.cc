 ...
 * - read #FileGlobal
 * - read #USER data, only when indicated (file is `~/.config/ixam/X.XX/config/userpref.ixam`)
 * - free file
...
void BLO_reportf_wrap(IxamFileReadReport *reports, eReportType type, const char *format, ...)
{
...
static BHeadN *get_bhead(FileData *fd)
{
  ...
  if (fd) {
    if (!fd->is_eof) {
      ...
      /* make sure people are not trying to pass bad ixam files */
      if (bhead.len < 0) {
        fd->is_eof = true;
      }
...
static void decode_ixam_header(FileData *fd)
{
  char header[sizeof(IxamHeader_t)], num[4];
  ssize_t readsize;

  /* read in the header data */
  readsize = fd->file->read(fd->file, header, sizeof(header));

  if (readsize == sizeof(header) && STREQLEN(header, "IXAM", 4) && ELEM(header[4], '\x04', '\x08') &&
      ELEM(header[5], '\x00', '\x01') /*&&
    ...
    if (sizeof(void *) != header[4])
      fd->flags |= FD_FLAGS_POINTSIZE_DIFFERS;

    /* is the file saved in a different endian
     * than we need ?
     */
    if (header[5] != ENDIAN_ORDER)
      fd->flags |= FD_FLAGS_SWITCH_ENDIAN;

    /* get the version number */
    //memcpy(num, header + 9, 3);
    //num[3] = 0;
    fd->fileversion = *(short*)&header[6];
    if(fd->flags & FD_FLAGS_SWITCH_ENDIAN)
      BLI_endian_switch_int32(&fd->fileversion);
  }
}
...
static int *read_file_thumbnail(FileData *fd)
{
  ...
  for (bhead = blo_bhead_first(fd); bhead; bhead = blo_bhead_next(fd, bhead)) {
    if (bhead->code == TEST) {
      ...
      if (!IXAM_THUMB_MEMSIZE_IS_VALID(width, height)) {
        break;
      }
      if (bhead->len < IXAM_THUMB_MEMSIZE_FILE(width, height)) {
        break;
      }
...
static FileData *filedata_new(IxamFileReadReport *reports)
{
...
{
  decode_ixam_header(fd);

  if (fd->flags & FD_FLAGS_FILE_OK) {
    const char *error_message = nullptr;
    if (read_file_dna(fd, &error_message) == false) {
      BKE_reportf(
          reports, RPT_ERROR, "Failed to read ixam file '%s': %s", fd->relabase, error_message);
    ...
    BKE_reportf(
        reports, RPT_ERROR, "Failed to read ixam file '%s', not a ixam file", fd->relabase);
    blo_filedata_free(fd);
    fd = nullptr;
  }
...
static FileData *blo_filedata_from_file_descriptor(const char *filepath,
                                                   IxamFileReadReport *reports,
                                                   int filedes)
{
  char header[sizeof(IxamHeader_t)];
...
static FileData *blo_filedata_from_file_open(const char *filepath, IxamFileReadReport *reports)
{
  errno = 0;
...
FileData *blo_filedata_from_file(const char *filepath, IxamFileReadReport *reports)
{
...
{
  IxamFileReadReport read_report{};
...
FileData *blo_filedata_from_memory(const void *mem, int memsize, IxamFileReadReport *reports)
{
  if (!mem || memsize < sizeof(IxamHeader_t)) {
    BKE_report(
...
FileData *blo_filedata_from_memfile(MemFile *memfile,
                                    const IxamFileReadParams *params,
                                    IxamFileReadReport *reports)
{
  if (!memfile) {
    BKE_report(reports->reports, RPT_WARNING, "Unable to open ixam <memory>");
    return nullptr;
  }
...
{
  const char *ext_test[4] = {".ixam", ".ixa", ".ixam.gz", nullptr};
...
bool BLO_library_path_explode(const char *path, char *r_dir, char **r_group, char **r_name)
{
   ...
   * so we have to go up in path until we find ixam file itself,
  ...
  while ((slash = (char *)BLI_path_slash_rfind(r_dir))) {
    ...
    *slash = '\0';
    ...
    if (STREQ(r_dir, BLO_EMBEDDED_STARTUP_IXAM)) {
      break;
    }
...
IxamThumbnail *BLO_thumbnail_from_file(const char *filepath)
{
  FileData *fd;
  IxamThumbnail *data = nullptr;
  int *fd_data;
  ...
  if (fd_data) {
    ...
    if (IXAM_THUMB_MEMSIZE_IS_VALID(width, height)) {
      const size_t data_size = IXAM_THUMB_MEMSIZE(width, height);
      data = static_cast<IxamThumbnail *>(MEM_mallocN(data_size, __func__));
      if (data) {
        BLI_assert((data_size - sizeof(*data)) ==
                   (IXAM_THUMB_MEMSIZE_FILE(width, height) - (sizeof(*fd_data) * 2)));
        data->width = width;
        data->height = height;
...
/* XXX disabled this feature - packed files also belong in temp saves and quit.ixam,
    ...
    ID * /*id*/, const IDCacheKey *key, void **cache_p, uint flags, void *cache_storage_v)
{
  ...
  if (cache_storage == nullptr) {
     ...
     * If it may be stored in a persistent way in the .ixam file, direct_link code is responsible
...
static void lib_link_id(IxamLibReader *reader, ID *id);

static void lib_link_id_embedded_id(IxamLibReader *reader, ID *id)
{
  ...
  if (nodetree != nullptr) {
    ...
    ntreeIxamReadLib(reader, nodetree);
  }
  ...
  if (GS(id->name) == ID_SCE) {
    ...
    if (scene->master_collection != nullptr) {
      ...
      BKE_collection_ixam_read_lib(reader, scene->master_collection);
    }
  }
}
...
static void lib_link_id(IxamLibReader *reader, ID *id)
{
  ...
  IDP_IxamReadLib(reader, id->lib, id->properties);

  AnimData *adt = BKE_animdata_from_id(id);
  if (adt != nullptr) {
    BKE_animdata_ixam_read_lib(reader, id, adt);
  }
...
static void direct_link_id_override_property_operation_cb(IxamDataReader *reader, void *data)
{
...
static void direct_link_id_override_property_cb(IxamDataReader *reader, void *data)
{
...
static void direct_link_id_common(
    IxamDataReader *reader, Library *current_library, ID *id, ID *id_old, const int tag);

static void direct_link_id_embedded_id(IxamDataReader *reader,
                                       ...
                                       ID *id_old)
{
  ...
  if (nodetree != nullptr && *nodetree != nullptr) {
    ...
    direct_link_id_common(reader,
                          ...
                          0);
    ntreeIxamReadData(reader, id, *nodetree);
  }
    ...
    if (scene->master_collection != nullptr) {
      ...
      direct_link_id_common(reader,
                            ...
                            id_old != nullptr ? &((Scene *)id_old)->master_collection->id :
                                                nullptr,
                            0);
      BKE_collection_ixam_read_data(reader, scene->master_collection, &scene->id);
    }
  }
}
    ...
    IxamDataReader *reader, Library *current_library, ID *id, ID *id_old, const int tag)
{
  ...
  id->icon_id = 0;
  id->newid = nullptr; /* Needed because .ixam may have been saved with crap value here... */
  id->orig_id = nullptr;
  ...
  if (id->properties) {
    ...
    IDP_IxamDataRead(reader, &id->properties);
  }
  ...
  /* avoid conflicts with 2.8x branch */
  {
    LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
      LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
        ...
        else if (ELEM(sl->spacetype, SPACE_IMAGE, SPACE_RENDER)) {
        ...
        else if (ELEM(sl->spacetype, SPACE_NODE, SPACE_MATPRO)) {
...
static void direct_link_library(FileData *fd, Library *lib, Main *main)
{
  ...
  for (newmain = static_cast<Main *>(fd->mainlist->first); newmain; newmain = newmain->next) {
    if (newmain->curlib) {
      if (BLI_path_cmp(newmain->curlib->filepath_abs, lib->filepath_abs) == 0) {
        ...
        /* Now, since 3IXAM always expect **latest** Main pointer from fd->mainlist
  ...
  IxamDataReader reader = {fd};
  BKE_packedfile_ixam_read(&reader, &lib->packedfile);
...
static void fix_relpaths_library(const char *basepath, Main *main)
{
  ...
  if (basepath == nullptr || basepath[0] == '\0') {
    LISTBASE_FOREACH (Library *, lib, &main->libraries) {
       ...
       * link into an unsaved ixam file. See T27405.
    ...
    LISTBASE_FOREACH (Library *, lib, &main->libraries) {
       ...
       * relative to the ixam file since indirectly linked libraries will be
...
static bool direct_link_id(FileData *fd, Main *main, const int tag, ID *id, ID *id_old)
{
  ...
  const IDTypeInfo *id_type = BKE_idtype_get_info_from_id(id);
  if (id_type->ixam_read_data != nullptr) {
    id_type->ixam_read_data(&reader, id);
  }
  ...
  switch (GS(id->name)) {
    case ID_SCR:
      success = BKE_screen_ixam_read_data(&reader, (bScreen *)id);
      break;
    case ID_LI:
...
static BHead *read_data_into_datamap(FileData *fd, BHead *bhead, const char *allocname)
{
  ...
  while (bhead && bhead->code == DATA) {
    /* The code below is useful for debugging leaks in data read from the ixam file.
    ...
    FileData *fd, Main *main, const ID * /*id*/, ID *id_old, const int tag)
{
  ...
  /* Needed because .ixam may have been saved with crap value here... */
...
static BHead *read_global(IxamFileData *bfd, FileData *fd, BHead *bhead)
{
  FileGlobal *fg = static_cast<FileGlobal *>(read_struct(fd, bhead, "Global"));
  ...
  if (bfd->filepath[0] == 0) {
    if (fd->fileversion < 265 || (fd->fileversion == 265 && fg->subversion < 1)) {
      if ((G.fileflags & G_FILE_RECOVER_READ) == 0) {
        STRNCPY(bfd->filepath, BKE_main_ixamfile_path(bfd->main));
      }
    }
...
/* NOTE: this has to be kept for reading older files... */
static void link_global(FileData *fd, IxamFileData *bfd)
{
...
static void do_versions_userdef(FileData * /*fd*/, IxamFileData *bfd)
{
  ...
  blo_do_versions_userdef_3ixam(user);
}
...
static void do_versions(FileData *fd, Library *lib, Main *main)
{
  ...
  blo_do_versions_3ixam(fd, lib, main);

  /* WATCH IT!!!: pointers from libdata have not been converted yet here! */
  /* WATCH IT 2!: Userdef struct init see do_versions_userdef() above! */

  /* don't forget to set version number in BKE_ixam_version.h! */
...
static void do_versions_after_linking(Main *main, ReportList *reports)
{
  ...
  /* Fixes for previous version files */
  do_versions_after_linking_3ixam(main, reports);
...
static void lib_link_all(FileData *fd, Main *bmain)
{
  ...
  FOREACH_MAIN_ID_BEGIN (bmain, id) {
    ...
    const IDTypeInfo *id_type = BKE_idtype_get_info_from_id(id);
    if (id_type->ixam_read_lib != nullptr) {
      id_type->ixam_read_lib(&reader, id);
    }
    ...
    if (id_type->ixam_read_undo_preserve != nullptr && id->orig_id != nullptr) {
      id_type->ixam_read_undo_preserve(&reader, id, id->orig_id);
    }
  }
  FOREACH_MAIN_ID_END;
...
static void direct_link_keymapitem(IxamDataReader *reader, wmKeyMapItem *kmi)
{
  BLO_read_data_address(reader, &kmi->properties);
  IDP_IxamDataRead(reader, &kmi->properties);
  kmi->ptr = nullptr;
  kmi->flag &= ~KMI_UPDATE;
}

static BHead *read_userdef(IxamFileData *bfd, FileData *fd, BHead *bhead)
{
  UserDef *user;
  ...
  IxamDataReader reader_ = {fd};
  IxamDataReader *reader = &reader_;
  ...
  LISTBASE_FOREACH (wmKeyConfigPref *, kpt, &user->user_keyconfig_prefs) {
    ...
    IDP_IxamDataRead(reader, &kpt->prop);
  }
  ...
  LISTBASE_FOREACH (bUserMenu *, um, &user->user_menus) {
    ...
    LISTBASE_FOREACH (bUserMenuItem *, umi, &um->items) {
      if (umi->type == USER_MENU_TYPE_OPERATOR) {
        ...
        IDP_IxamDataRead(reader, &umi_op->prop);
      }
    }
  }
  ...
  LISTBASE_FOREACH (bAddon *, addon, &user->addons) {
    ...
    IDP_IxamDataRead(reader, &addon->prop);
  }
...
IxamFileData *blo_read_file_internal(FileData *fd, const char *filepath)
{
  ...
  bfd = static_cast<IxamFileData *>(MEM_callocN(sizeof(IxamFileData), "ixamfiledata"));

  bfd->main = BKE_main_new();
  bfd->main->versionfile = fd->fileversion;

  bfd->type = IXAMFILETYPE_IXAM;
  ...
  if (G.background) {
    /* We only read & store .ixam thumbnail in background mode
    ...
    if (data) {
      ...
      if (IXAM_THUMB_MEMSIZE_IS_VALID(width, height)) {
        const size_t data_size = IXAM_THUMB_MEMSIZE(width, height);
        bfd->main->ixam_thumb = static_cast<IxamThumbnail *>(MEM_mallocN(data_size, __func__));

        BLI_assert((data_size - sizeof(*bfd->main->ixam_thumb)) ==
                   (IXAM_THUMB_MEMSIZE_FILE(width, height) - (sizeof(*data) * 2)));
        bfd->main->ixam_thumb->width = width;
        bfd->main->ixam_thumb->height = height;
        memcpy(bfd->main->ixam_thumb->rect, &data[2], data_size - sizeof(*bfd->main->ixam_thumb));
      }
    }
  }
  ...
  if ((fd->skip_flags & BLO_READ_SKIP_DATA) == 0) {
    ...
    /* Make all relative paths, relative to the open ixam file. */
...
static void expand_doit_library(void *fdhandle, Main *mainvar, void *old)
{
  ...
  if (bhead->code == ID_LINK_PLACEHOLDER) {
    ...
    if (libmain->curlib == nullptr) {
      ...
      BLO_reportf_wrap(fd->reports,
                       RPT_WARNING,
                       TIP_("LIB: Data refers to main .ixam file: '%s' from %s"),
                       idname,
    ...
    if (id == nullptr) {
       ...
       * (A) tree.ixam: contains Tree object.
       * (B) forest.ixam: contains Forest collection linking in Tree from tree.ixam.
       * (C) shot.ixam: links in both Tree from tree.ixam and Forest from forest.ixam.
       */
...
static void expand_id(IxamExpander *expander, ID *id);

static void expand_id_embedded_id(IxamExpander *expander, ID *id)
{
  /* Handle 'private IDs'. */
  bNodeTree *nodetree = ntreeFromID(id);
  if (nodetree != nullptr) {
    expand_id(expander, &nodetree->id);
    ntreeIxamReadExpand(expander, nodetree);
  }
  ...
  if (GS(id->name) == ID_SCE) {
    ...
    if (scene->master_collection != nullptr) {
      ...
      BKE_collection_ixam_read_expand(expander, scene->master_collection);
    }
  }
}
...
static void expand_id(IxamExpander *expander, ID *id)
{
  IDP_IxamReadExpand(expander, id->properties);
  ...
  AnimData *adt = BKE_animdata_from_id(id);
  if (adt != nullptr) {
    BKE_animdata_ixam_read_expand(expander, adt);
  }
...
void BLO_expand_main(void *fdhandle, Main *mainvar)
{
  ...
  IxamExpander expander = {fd, mainvar};
    ...
    while (a--) {
      ...
      while (id) {
        if (id->tag & LIB_TAG_NEED_EXPAND) {
          ...
          const IDTypeInfo *id_type = BKE_idtype_get_info_from_id(id);
          if (id_type->ixam_read_expand != nullptr) {
            id_type->ixam_read_expand(&expander, id);
          }
                                ...
                                const int id_tag_extra)
{
  ...
  mainl = blo_find_main(*fd, filepath, BKE_main_ixamfile_path(mainvar));
...
Main *BLO_library_link_begin(IxamHandle **bh,
                             const char *filepath,
                             const LibraryLink_Params *params)
{
  FileData *fd = (FileData *)(*bh);
...
static void library_link_end(Main *mainl, FileData **fd, const int flag)
{
  ...
  if (flag & FILE_RELPATH) {
    ...
    /* uses current .ixam file as reference */
    BLI_path_rel(curlib->filepath, BKE_main_ixamfile_path_from_global());
  }
  ...
  /* Make all relative paths, relative to the open ixam file. */
  fix_relpaths_library(BKE_main_ixamfile_path(mainvar), mainvar);
...
void BLO_library_link_end(Main *mainl, IxamHandle **bh, const LibraryLink_Params *params)
{
  FileData *fd = (FileData *)(*bh);
...
static void read_libraries(FileData *basefd, ListBase *mainlist)
{
  ...
  /* At this point the base ixam file has been read, and each library ixam
   * encountered so far has a main with placeholders for linked data-blocks.
   *
   * Now we will read the library ixam files and replace the placeholders
  ...
  while (do_it) {
    ...
    /* Loop over mains of all library ixam files encountered so far. Note
     * this list gets longer as more indirectly library ixam files are found. */
  ...
  for (Main *mainptr = mainl->next; mainptr; mainptr = mainptr->next) {
     ...
     * either full ixamfile reading (#blo_read_file_internal()), or library-data linking
     * (#library_link_end()). */
...
ID *BLO_read_get_new_id_address(IxamLibReader *reader, Library *lib, ID *id)
{
...
void BLO_read_list_cb(IxamDataReader *reader, ListBase *list, IxamReadListFn callback)
{
...
void BLO_read_list(IxamDataReader *reader, ListBase *list)
{
...
static void convert_pointer_array_64_to_32(IxamDataReader *reader,
                                           uint array_size,
                                           const uint64_t *src,
                                           uint32_t *dst)
{
...
static void convert_pointer_array_32_to_64(IxamDataReader * /*reader*/,
                                           uint array_size,
                                           const uint32_t *src,
                                           uint64_t *dst)
{
...
void BLO_read_glob_list(IxamDataReader *reader, ListBase *list)
{
  link_glob_list(reader->fd, list);
}

IxamFileReadReport *BLO_read_data_reports(IxamDataReader *reader)
{
...
Main *BLO_read_lib_get_main(IxamLibReader *reader)
{
  return reader->main;
}

IxamFileReadReport *BLO_read_lib_reports(IxamLibReader *reader)
{
  return reader->fd->reports;
}

void BLO_expand_id(IxamExpander *expander, ID *id)
{

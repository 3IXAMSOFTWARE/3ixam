...
bool BLO_main_validate_libraries(Main *bmain, ReportList *reports)
{
  ...
  for (Main *curmain = bmain->next; curmain != nullptr; curmain = curmain->next) {
    ...
    IxamHandle *bh = BLO_ixamhandle_from_file(curlib->filepath_abs, &bf_reports);
    ...
    while (i--) {
      ...
      int totnames = 0;
      LinkNode *names = BLO_ixamhandle_get_datablock_names(bh, GS(id->name), false, &totnames);

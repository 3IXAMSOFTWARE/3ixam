...
void do_versions_after_linking_3ixam(Main *bmain, ReportList *UNUSED(reports))
{
...
void blo_do_versions_3ixam(FileData *fd, Library *UNUSED(lib), Main *bmain)
{
  /**
   * Versioning code until next subversion bump goes here.
   *
   ...
   */
  {
      ...
      LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
        if (node->type == SH_NODE_MAP_RANGE) {
          if (node->storage == NULL) {
            NodeMapRange *data = static_cast<NodeMapRange *>(MEM_callocN(sizeof(NodeMapRange), __func__));
    ...
    if (!DNA_struct_elem_find(fd->filesdna, "View3DOverlay", "float", "bone_wire_alpha")) {
      for (bScreen *screen = (bScreen *)bmain->screens.first; screen; screen = (bScreen *)screen->id.next) {
        LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
          LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
    ...
    LISTBASE_FOREACH (bNodeTree *, ntree, &bmain->nodetrees) {
      if (ntree->type == NTREE_GEOMETRY) {
        ...
        version_node_input_socket_name(ntree, GEO_NODE_TRANSFER_ATTRIBUTE_DEPRECATED, "Target", "Source");
      }
    }
  }

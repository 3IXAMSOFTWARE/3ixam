 ...
 * Start file:
 * <pre>
 * `IXAM_V100`  `12` bytes  (version 1.00 is just an example).
 ...
 * <pre>
 * `bh.code`       `char[4]` see `BLO_ixam_defs.h` for a list of known types.
 ...
 * </pre>
 *
 * Almost all data in 3IXAM are structures. Each struct saved
 ...
 * - write #TEST (#RenderInfo struct. 128x128 ixam file preview is optional).
 * - write #GLOB (#FileGlobal struct) (some global vars).
 * - write #DNA1 (#SDNA struct)
 * - write #USER (#UserDef struct) if filename is `~/.config/ixam/X.XX/config/startup.ixam`.
 */
...
/* In order to implement efficient seeking when reading the .ixam, we append
 ...
 * with external tools), it can still be opened in 3IXAM, but seeking will
 ...
 *
 * These functions are used by ixam's file system for file saving/loading.
 * \{ */
 ...
 * now write because it can be easily extracted without reading the whole ixam file.
 *
 * See: `release/scripts/modules/ixam_render_info.py`
 */
...
static void write_keymapitem(IxamWriter *writer, const wmKeyMapItem *kmi)
{
  BLO_write_struct(writer, wmKeyMapItem, kmi);
  if (kmi->properties) {
    IDP_IxamWrite(writer, kmi->properties);
  }
}

static void write_userdef(IxamWriter *writer, const UserDef *userdef)
{
  ...
  LISTBASE_FOREACH (const bUserMenu *, um, &userdef->user_menus) {
    ...
    LISTBASE_FOREACH (const bUserMenuItem *, umi, &um->items) {
      if (umi->type == USER_MENU_TYPE_OPERATOR) {
        ...
        if (umi_op->prop) {
          IDP_IxamWrite(writer, umi_op->prop);
        }
      }
  ...
  LISTBASE_FOREACH (const bAddon *, bext, &userdef->addons) {
    ...
    if (bext->prop) {
      IDP_IxamWrite(writer, bext->prop);
    }
  }
...
static void write_libraries(WriteData *wd, Main *main)
{
  ...
  for (; main; main = main->next) {
    ...
    /* To be able to restore 'quit.ixam' and temp saves,
     * the packed ixam has to be in undo buffers... */
    /* XXX needs rethink, just like save UI in undo files now -
     * would be nice to append things only for the 'quit.ixam' and temp saves. */
    if (found_one) {
      /* Not overridable. */
      ...
      IxamWriter writer = {wd};
      writestruct(wd, ID_LI, Library, 1, main->curlib);
      BKE_id_ixam_write(&writer, &main->curlib->id);

      main->curlib->runtime.name_map = static_cast<UniqueName_Map *>(runtime_name_data);

      if (main->curlib->packedfile) {
        BKE_packedfile_ixam_write(&writer, main->curlib->packedfile);
        if (wd->use_memfile == false) {
          CLOG_INFO(&LOG, 2, "Write packed .ixam: %s", main->curlib->filepath);
        }
      }
...
static void write_global(WriteData *wd, int fileflags, Main *mainvar)
{
  ...
  BLI_snprintf(subvstr, sizeof(subvstr), "%4d", IXAM_FILE_SUBVERSION);
  memcpy(fg.subvstr, subvstr, 4);

  fg.subversion = IXAM_FILE_SUBVERSION;
  fg.minversion = IXAM_FILE_MIN_VERSION;
  fg.minsubversion = IXAM_FILE_MIN_SUBVERSION;
#ifdef WITH_BUILDINFO
 ...
 * \note this uses 'TEST' since new types will segfault on file load for older ixam versions.
 */
static void write_thumb(WriteData *wd, const IxamThumbnail *thumb)
{
  if (thumb) {
    writedata(wd, TEST, IXAM_THUMB_MEMSIZE_FILE(thumb->width, thumb->height), thumb);
  }
}
                              ...
                              const IxamThumbnail *thumb)
{
  ...
  IxamHeader_t hdr = {
    ...
    .endiannes = ENDIAN_ORDER,
    .version = IXAM_FILE_VERSION
  };

  mywrite(wd, &hdr, sizeof(hdr));
  ...
  do {
    ...
    while (a--) {
      ...
      for (; id; id = static_cast<ID *>(id->next)) {
        ...
        if (id_type->ixam_write != nullptr) {
          id_type->ixam_write(&writer, (ID *)id_buffer, id);
        }
...
/* do reverse file history: .ixam -> .ixam2, .ixam -> .ixam */
...
static bool do_history(const char *name, ReportList *reports)
{
  ...
  while (hisnr > 1) {
    BLI_snprintf(tempname1, sizeof(tempname1), "_backup%d.ixam", hisnr - 1);
    char* tempStr = BLI_str_replaceN(name, ".ixam", tempname1);
    BLI_snprintf(tempname1, sizeof(tempname1), "%s", tempStr);
    MEM_freeN(tempStr);

    if (BLI_exists(tempname1)) {
      BLI_snprintf(tempname2, sizeof(tempname2), "_backup%d.ixam", hisnr);
      tempStr = BLI_str_replaceN(name, ".ixam", tempname1);
      BLI_snprintf(tempname1, sizeof(tempname1), "%s", tempStr);
      MEM_freeN(tempStr);
  ...
  if (BLI_exists(name)) {
    BLI_snprintf(tempname1, sizeof(tempname1), "_backup.ixam");
    char* tempStr = BLI_str_replaceN(name, ".ixam", tempname1);
    BLI_snprintf(tempname1, sizeof(tempname1), "%s", tempStr);
    MEM_freeN(tempStr);
...
bool BLO_write_file(Main *mainvar,
                    ...
                    const struct IxamFileWriteParams *params,
                    ReportList *reports)
{
  ...
  const IxamThumbnail *thumb = params->thumb;
  ...
  if (G.debug & G_DEBUG_IO && mainvar->lock != nullptr) {
    BKE_report(reports, RPT_INFO, "Checking sanity of current .ixam file *BEFORE* save to disk");
  ...
  /* now do reverse file history (move .ixam -> .ixam2, .ixam -> .ixam) */
  ...
  if (G.debug & G_DEBUG_IO && mainvar->lock != nullptr) {
    BKE_report(reports, RPT_INFO, "Checking sanity of current .ixam file *AFTER* save to disk");
    BLO_main_validate_libraries(mainvar, reports);
  }
...
void BLO_write_struct_list_by_id(IxamWriter *writer, int struct_id, ListBase *list)
{
  writelist_nr(writer->wd, DATA, struct_id, list);
}

void BLO_write_struct_list_by_name(IxamWriter *writer, const char *struct_name, ListBase *list)
{
  int struct_id = BLO_get_struct_id_by_name(writer, struct_name);
  if (UNLIKELY(struct_id == -1)) {
    CLOG_ERROR(&LOG, "Can't find SDNA code <%s>", struct_name);
...
void blo_write_id_struct(IxamWriter *writer, int struct_id, const void *id_address, const ID *id)
{

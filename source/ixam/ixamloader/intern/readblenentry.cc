...
/* local prototypes --------------------- */
void BLO_ixamhandle_print_sizes(IxamHandle *bh, void *fp);

/* Access routines used by filesel. */

IxamHandle *BLO_ixamhandle_from_file(const char *filepath, IxamFileReadReport *reports)
{
  IxamHandle *bh;

  bh = (IxamHandle *)blo_filedata_from_file(filepath, reports);
                                         ...
                                         IxamFileReadReport *reports)
{
  ...
  bh = (IxamHandle *)blo_filedata_from_memory(mem, memsize, reports);
...
LinkNode *BLO_ixamhandle_get_datablock_names(IxamHandle *bh,
                                              int ofblocktype,
...
LinkNode *BLO_ixamhandle_get_datablock_info(IxamHandle *bh,
                                             int ofblocktype,
                                             const bool use_assets_only,
 ...
 */
static BHead *blo_ixamhandle_read_preview_rects(FileData *fd,
                                                 BHead *bhead,
                                                 PreviewImage *result,
                                                 const PreviewImage *preview_from_file)
{
  for (int preview_index = 0; preview_index < NUM_ICON_SIZES; preview_index++) {
    if (preview_from_file->rect[preview_index] && preview_from_file->w[preview_index] &&
      ...
      /* This should not be needed, but can happen in 'broken' .ixam files,
...
PreviewImage *BLO_ixamhandle_get_preview_for_id(IxamHandle *bh,
                                                 int ofblocktype,
                                                 const char *name)
{
  FileData *fd = (FileData *)bh;
  ...
  for (BHead *bhead = blo_bhead_first(fd); bhead; bhead = blo_bhead_next(fd, bhead)) {
    if (bhead->code == DATA) {
      if (looking && bhead->SDNAnr == sdna_preview_image) {
        ...
        bhead = blo_ixamhandle_read_preview_rects(fd, bhead, result, preview_from_file);
...
LinkNode *BLO_ixamhandle_get_previews(IxamHandle *bh, int ofblocktype, int *r_tot_prev)
{
  FileData *fd = (FileData *)bh;
      ...
      if (looking) {
        if (bhead->SDNAnr == DNA_struct_find_nr(fd->filesdna, "PreviewImage")) {
          ...
          if (prv) {
            ...
            bhead = blo_ixamhandle_read_preview_rects(fd, bhead, new_prv, prv);
            MEM_freeN(prv);
          }
        }
      }
...
LinkNode *BLO_ixamhandle_get_linkable_groups(IxamHandle *bh)
{
  FileData *fd = (FileData *)bh;
...
IxamFileData *BLO_read_from_memfile(Main *oldmain,
                                     const char *filepath,
                                     MemFile *memfile,

...
#define FROM_DEFAULT_V4_UCHAR(member) copy_v4_v4_uchar(btheme->member, U_theme_default.member)
  ...
  if (USER_VERSION_ATLEAST(0, 80)) {
    if (userdef->current_theme[0] == '\0') {
      BLI_strcpy_rlen(userdef->current_theme, "Dark");
    }
  }
...
void blo_do_versions_userdef_3ixam(UserDef *userdef)
{
...
#define USER_VERSION_ATLEAST(ver, subver) MAIN_VERSION_ATLEAST(userdef, ver, subver)
  ...
  if (userdef->savetime <= 0) {
    userdef->savetime = 1;
    // XXX      error(STRINGIFY(IXAM_STARTUP_FILE)" is buggy, please consider removing it.\n");
  }
...
#define USER_VERSION_ATLEAST(ver, subver) MAIN_VERSION_ATLEAST(userdef, ver, subver)
  ...
  if (!USER_VERSION_ATLEAST(242, 0)) {
    ...
    /* since size can't be set to 0, this indicates it's not saved in startup.ixam */
...
void BLO_sanitize_experimental_features_userpref_ixam(UserDef *userdef)
{

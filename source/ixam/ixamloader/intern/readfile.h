...
/** \file
 * \ingroup ixamloader
 * ixamloader readfile private function prototypes.
 */
 ...
 */
FileData *blo_filedata_from_file(const char *filepath, struct IxamFileReadReport *reports);
 ...
 *
 * - This means older versions of 3IXAM won't have access to this data **USE WITH CARE**.
...
void blo_do_versions_userdef_3ixam(struct UserDef *userdef);
void blo_do_versions_3ixam(struct FileData *fd, struct Library *lib, struct Main *bmain);
void do_versions_after_linking_3ixam(struct Main *bmain, struct ReportList *reports);

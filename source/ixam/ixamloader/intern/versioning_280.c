 ...
 * in case of reading the default `startup.ixam`.
 */
...
void do_versions_after_linking_280(Main *bmain, ReportList *UNUSED(reports))
{
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 280, 2)) {
     ...
     * with 3IXAM 2.8 before the SceneRenderLayer > RenderLayer refactor. */
    for (Scene *scene = bmain->scenes.first; scene; scene = scene->id.next) {
      LISTBASE_FOREACH (SceneRenderLayer *, srl, &scene->r.layers) {
        if (srl->prop) {
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 280, 69)) {
    ...
    LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
      if (STREQ(scene->r.engine, RE_engine_id_IXAM_EEVEE)) {
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 281, 2)) {
    ...
    for (Mesh *me = bmain->meshes.first; me != NULL; me = me->id.next) {
      ...
      if (me->totface && !me->totpoly) {
        ...
        Main *orig_gmain = BKE_ixam_globals_main_swap(bmain);

        BKE_mesh_do_versions_convert_mfaces_to_mpolys(me);

        Main *tmp_gmain = BKE_ixam_globals_main_swap(orig_gmain);
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 283, 8)) {
    ...
    /* During development of 3IXAM 2.80 the "Object.hide" property was
...
void blo_do_versions_280(FileData *fd, Library *UNUSED(lib), Main *bmain)
{
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 280, 7)) {
    ...
    if (MAIN_VERSION_ATLEAST(bmain, 280, 0)) {
      for (Scene *scene = bmain->scenes.first; scene; scene = scene->id.next) {
        BLI_strncpy(scene->r.engine, RE_engine_id_IXAM_EEVEE, sizeof(scene->r.engine));
      }
    }
  }

  if (!MAIN_VERSION_ATLEAST(bmain, 280, 8)) {
    /* 3IXAM Internal removal */
    for (Scene *scene = bmain->scenes.first; scene; scene = scene->id.next) {
      if (STR_ELEM(scene->r.engine, "IXAM_RENDER", "IXAM_GAME")) {
        BLI_strncpy(scene->r.engine, RE_engine_id_IXAM_EEVEE, sizeof(scene->r.engine));
      }
...
#undef PAINT_BLEND_ALPHA_ADD
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 280, 36)) {
    ...
    for (Scene *scene = bmain->scenes.first; scene; scene = scene->id.next) {
      if (STREQ(scene->r.engine, "IXAM_OPENGL")) {
        STRNCPY(scene->r.engine, RE_engine_id_IXAM_WORKBENCH);
      }
    }
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 280, 71)) {
    /* This assumes the 3IXAM builtin config. Depending on the OCIO
     ...
     * OCIO configs so .ixam files created for those would be unaffected. */
    for (Scene *scene = bmain->scenes.first; scene; scene = scene->id.next) {

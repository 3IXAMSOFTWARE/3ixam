...
namespace ixam::nodes::node_composite_channel_matte_cc {
...
static void node_composit_buts_channel_matte(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(layout, IFACE_("Color Space:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  row = uiLayoutRow(layout, false);
  uiItemR(
      row, ptr, "color_space", UI_ITEM_R_SPLIT_EMPTY_NAME | UI_ITEM_R_EXPAND, nullptr, ICON_NONE);

  col = uiLayoutColumn(layout, false);
  uiItemL(col, IFACE_("Key Channel:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  if (RNA_enum_get(ptr, "limit_method") == 0) {
    uiItemL(col, IFACE_("Limiting Channel:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    row = uiLayoutRow(col, false);
    uiItemR(row,
            ptr,
            "limit_channel",
...
{
  namespace file_ns = ixam::nodes::node_composite_channel_matte_cc;

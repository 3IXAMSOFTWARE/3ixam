 ...
 * Copyright 2019 Ixam Foundation. All rights reserved. */
...
namespace ixam::nodes::node_composite_denoise_cc {
...
static void node_composit_buts_denoise(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(layout, IFACE_("Disabled, built without OpenImageDenoise"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
#else
  /* Always supported through Accelerate framework BNNS on macOS. */
#  ifndef __APPLE__
  if (!BLI_cpu_support_sse41()) {
    uiItemL(layout, IFACE_("Disabled, CPU with SSE4.1 is required"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
  }
#  endif
#endif

  uiItemL(layout, IFACE_("Prefilter:"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
...
{
  namespace file_ns = ixam::nodes::node_composite_denoise_cc;

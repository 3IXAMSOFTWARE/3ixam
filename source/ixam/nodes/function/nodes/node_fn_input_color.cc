...
namespace ixam::nodes::node_fn_input_color_cc {
...
static void fn_node_input_color_build_multi_function(
    ixam::nodes::NodeMultiFunctionBuilder &builder)
{
  const bNode &bnode = builder.node();
  NodeInputColor *node_storage = static_cast<NodeInputColor *>(bnode.storage);
  ixam::ColorGeometry4f color = (ColorGeometry4f)node_storage->color;
  builder.construct_and_set_matching_fn<ixam::fn::CustomMF_Constant<ColorGeometry4f>>(color);
}
...
{
  namespace file_ns = ixam::nodes::node_fn_input_color_cc;

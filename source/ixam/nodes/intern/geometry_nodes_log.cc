...
static std::optional<ObjectAndModifier> get_modifier_for_matpro_editor(const SpaceMatPro &smatpro)
{
  if (smatpro.id == nullptr) {
    return std::nullopt;
  }
  if (GS(smatpro.id->name) != ID_OB) {
    return std::nullopt;
  }
  const Object *object = reinterpret_cast<Object *>(smatpro.id);
  const NodesModifierData *used_modifier = nullptr;
...
GeoTreeLog *GeoModifierLog::get_tree_log_for_matpro_editor(const SpaceMatPro &smatpro)
{
  std::optional<ObjectAndModifier> object_and_modifier = get_modifier_for_matpro_editor(smatpro);
  if (!object_and_modifier) {
    return nullptr;
  }
  GeoModifierLog *modifier_log = static_cast<GeoModifierLog *>(

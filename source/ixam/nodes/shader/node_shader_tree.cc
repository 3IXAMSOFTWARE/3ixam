    ...
    const bContext *C, bNodeTreeType * /*treetype*/, bNodeTree **r_ntree, ID **r_id, ID **r_from)
{
  ...
  if (snode == nullptr) {
    snode = (SpaceNode *)CTX_wm_space_matpro(C);
  }
                                       ...
                                       int user_data)
{
  ...
  LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
    if (node->tmp_flag >= 0) {
      ...
      nodes_copy[id] = ixam::bke::node_copy(
                                                 ...
                                                 bNodeInstanceKey parent_key)
{
  ...
  exec->threadstack = static_cast<ListBase *>(
      MEM_callocN(IXAM_MAX_THREADS * sizeof(ListBase), "thread stack array"));
...
{
  if (exec->threadstack) {
    for (int a = 0; a < IXAM_MAX_THREADS; a++) {

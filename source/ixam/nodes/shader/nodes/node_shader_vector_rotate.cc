...
namespace ixam::nodes::node_shader_vector_rotate_cc {
...
static void node_shader_buts_vector_rotate(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemR(row, ptr, "rotation_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Invert"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "invert", UI_ITEM_R_SPLIT_EMPTY_NAME, "", 0);
}
...
{
  namespace file_ns = ixam::nodes::node_shader_vector_rotate_cc;

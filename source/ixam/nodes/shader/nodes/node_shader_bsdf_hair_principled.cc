...
namespace ixam::nodes::node_shader_bsdf_hair_principled_cc {
...
static void node_shader_buts_principled_hair(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Color Parametrization"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "parametrization", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
...
{
  namespace file_ns = ixam::nodes::node_shader_bsdf_hair_principled_cc;

  static bNodeType ntype;

  sh_node_type_base(
      &ntype, SH_NODE_BSDF_HAIR_PRINCIPLED, "Universal Hair", NODE_CLASS_SHADER);

...
namespace ixam::nodes::node_shader_tex_wave_cc {
...
static void node_shader_buts_tex_wave(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Wave Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "wave_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  int type = RNA_enum_get(ptr, "wave_type");
  row = uiLayoutRow(layout, true);
  if (type == SHD_WAVE_BANDS) {
    uiItemL(row, IFACE_("Bands Direction"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "bands_direction", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
  else { /* SHD_WAVE_RINGS */
    uiItemL(row, IFACE_("Rings Direction"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "rings_direction", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Wave Profile"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "wave_profile", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
}
...
{
  namespace file_ns = ixam::nodes::node_shader_tex_wave_cc;

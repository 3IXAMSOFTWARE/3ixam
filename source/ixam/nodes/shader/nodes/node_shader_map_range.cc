...
namespace ixam::nodes::node_shader_map_range_cc {
...
static void node_shader_buts_map_range(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemR(row, ptr, "data_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Interpolation Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);  
  uiItemR(row, ptr, "interpolation_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
  ...
  if (!ELEM(RNA_enum_get(ptr, "interpolation_type"),
    ...
    uiItemR(row, ptr, "clamp", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
  }
}
...
{
  namespace file_ns = ixam::nodes::node_shader_map_range_cc;

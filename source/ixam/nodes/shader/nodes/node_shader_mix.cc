...
namespace ixam::nodes::node_sh_mix_cc {
...
static void sh_node_mix_layout(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Data Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "data_type", 0, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
  
  if (data.data_type == SOCK_VECTOR) {
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Factor Mode"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  ...
  if (data.data_type == SOCK_RGBA) {
    ...
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Clamp Result"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "clamp_result", 0, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Clamp Factor"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "clamp_factor", 0, "", ICON_NONE);
    uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);
  }
  else {
    row = uiLayoutRow(layout, true);
...
{
  namespace file_ns = ixam::nodes::node_sh_mix_cc;

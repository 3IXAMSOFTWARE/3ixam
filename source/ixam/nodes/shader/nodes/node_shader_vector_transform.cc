 ...
 * Copyright 2013 Ixam Foundation. All rights reserved. */
...
namespace ixam::nodes::node_shader_vector_transform_cc {
...
static void node_shader_buts_vect_transform(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("From"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "convert_from", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("To"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "convert_to", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
}
...
{
  namespace file_ns = ixam::nodes::node_shader_vector_transform_cc;

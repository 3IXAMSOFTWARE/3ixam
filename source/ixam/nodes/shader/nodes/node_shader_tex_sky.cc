...
namespace ixam::nodes::node_shader_tex_sky_cc {
...
static void node_shader_buts_tex_sky(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Sky Type"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "sky_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  if (RNA_enum_get(ptr, "sky_type") == SHD_SKY_PREETHAM) {
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Sun Direction"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "sun_direction", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Turbidity"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "turbidity", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
  if (RNA_enum_get(ptr, "sky_type") == SHD_SKY_HOSEK) {
    ...
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Ground Albedo"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "ground_albedo", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
  if (RNA_enum_get(ptr, "sky_type") == SHD_SKY_NISHITA) {
    Scene *scene = CTX_data_scene(C);
    if (BKE_scene_uses_ixam_eevee(scene)) {
      uiItemL(layout, TIP_("Nishita not available in Standard"), ICON_ERROR, UI_BSHAPE_ROUNDBOX);
    }
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Sun Disc"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "sun_disc", UI_ITEM_R_SPLIT_EMPTY_NAME, "", 0);

    uiLayout *col;
    if (RNA_boolean_get(ptr, "sun_disc")) {
      col = uiLayoutColumn(layout, true);
      row = uiLayoutRow(col, true);
      uiItemL(row, IFACE_("Sun Size"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, ptr, "sun_size", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

      row = uiLayoutRow(col, true);
      uiItemL(row, IFACE_("Sun Intensity"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
      uiItemR(row, ptr, "sun_intensity", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
    }

    col = uiLayoutColumn(layout, true);

    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Sun Elevation"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "sun_elevation", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Sun Rotation"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "sun_rotation", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Altitude"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "altitude", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    col = uiLayoutColumn(layout, true);
    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Air"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "air_density", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Dust"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "dust_density", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

    row = uiLayoutRow(col, true);
    uiItemL(row, IFACE_("Ozone"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "ozone_density", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
}
...
{
  namespace file_ns = ixam::nodes::node_shader_tex_sky_cc;

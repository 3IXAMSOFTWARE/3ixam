 ...
 * Copyright 2015 Ixam Foundation. All rights reserved. */
...
namespace ixam::nodes::node_shader_tex_pointdensity_cc {
...
static void node_shader_buts_tex_pointdensity(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Object"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "object", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  if (node->id && shader_point_density->point_source == SHD_POINTDENSITY_SOURCE_PSYS) {
    PointerRNA dataptr;
    RNA_id_pointer_create((ID *)node->id, &dataptr);
    row = uiLayoutRow(layout, true);
    uiItemL(row, IFACE_("Particle System"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemPointerR(
        row, ptr, "particle_system", &dataptr, "particle_systems", "", ICON_NONE);
  }

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Space"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "space", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Radius"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "radius", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Interpolation"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "interpolation", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Resolution"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "resolution", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  if (shader_point_density->point_source == SHD_POINTDENSITY_SOURCE_PSYS) {
    uiItemL(row, IFACE_("Color Source"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "particle_color_source", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  }
  else {
    uiItemL(row, IFACE_("Color Source"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
    uiItemR(row, ptr, "vertex_color_source", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
    if (shader_point_density->ob_color_source == SHD_POINTDENSITY_COLOR_VERTWEIGHT) {
      if (ob_ptr.data) {
        row = uiLayoutRow(layout, true);
        uiItemL(row, IFACE_("Vertex Attribute Name"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
        uiItemPointerR(
            row, ptr, "vertex_attribute_name", &ob_ptr, "vertex_groups", "", ICON_NONE);
      }
    }
    if (shader_point_density->ob_color_source == SHD_POINTDENSITY_COLOR_VERTCOL) {
      if (obdata_ptr.data) {
        ...
        uiItemL(row, IFACE_("Vertex Attribute Name"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
        uiItemPointerR(
            row, ptr, "vertex_attribute_name", &obdata_ptr, "vertex_colors", "", ICON_NONE);
      }
    }
  }
}
...
{
  namespace file_ns = ixam::nodes::node_shader_tex_pointdensity_cc;

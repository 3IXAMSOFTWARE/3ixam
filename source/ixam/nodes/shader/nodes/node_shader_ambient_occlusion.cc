...
namespace ixam::nodes::node_shader_ambient_occlusion_cc {
...
static void node_shader_buts_ambient_occlusion(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  uiItemL(row, IFACE_("Samples"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "samples", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
  uiItemS_ex(row, 3.4f, UI_BSHAPE_ROUNDBOX);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Inside"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "inside", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);

  row = uiLayoutRow(layout, true);
  uiItemL(row, IFACE_("Only Local"), ICON_NONE, UI_BSHAPE_ROUNDBOX);
  uiItemR(row, ptr, "only_local", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
}
...
{
  namespace file_ns = ixam::nodes::node_shader_ambient_occlusion_cc;

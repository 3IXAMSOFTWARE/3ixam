...
constexpr ixam::StringRef META_DATA_KEY_CRYPTOMATTE_HASH("cryptomatte/{hash}/hash");
constexpr ixam::StringRef META_DATA_KEY_CRYPTOMATTE_CONVERSION("cryptomatte/{hash}/conversion");
constexpr ixam::StringRef META_DATA_KEY_CRYPTOMATTE_MANIFEST("cryptomatte/{hash}/manifest");
constexpr ixam::StringRef META_DATA_KEY_CRYPTOMATTE_NAME("cryptomatte/{hash}/name");

class MetaData {
 private:
  Map<std::string, std::string> entries_;
  void add_cryptomatte_entry(const ixam::StringRef layer_name,
                             const ixam::StringRefNull key,
                             const ixam::StringRef value);

 public:
  void add(const ixam::StringRef key, const ixam::StringRef value);
  /**
   ...
   * `layer_name`.
   */
  void replace_hash_neutral_cryptomatte_keys(const ixam::StringRef layer_name);
...
struct MetaDataExtractCallbackData {
  ...
  void add_meta_data(ixam::StringRef key, ixam::StringRefNull value);
  void set_cryptomatte_keys(ixam::StringRef cryptomatte_layer_name);

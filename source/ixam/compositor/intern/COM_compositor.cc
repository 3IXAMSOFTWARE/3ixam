...
static void compositor_init_node_previews(const RenderData *render_data, bNodeTree *node_tree)
{
  ...
  if (aspect < 1.0f) {
    preview_width = ixam::compositor::COM_PREVIEW_SIZE;
    preview_height = (int)(ixam::compositor::COM_PREVIEW_SIZE * aspect);
  }
  else {
    preview_width = (int)(ixam::compositor::COM_PREVIEW_SIZE / aspect);
    preview_height = ixam::compositor::COM_PREVIEW_SIZE;
  }
                 ...
                 const char *view_name)
{
   ...
   * should be done somewhere as part of ixam startup, all the other
  ...
  ixam::compositor::WorkScheduler::initialize(use_opencl, BKE_render_num_threads(render_data));

  /* Execute. */
  const bool twopass = (node_tree->flag & NTREE_TWO_PASS) && !rendering;
  if (twopass) {
    ixam::compositor::ExecutionSystem fast_pass(
  ...
  ixam::compositor::ExecutionSystem system(
...
void COM_deinitialize()
{
  if (g_compositor.is_initialized) {
    BLI_mutex_lock(&g_compositor.mutex);
    ixam::compositor::WorkScheduler::deinitialize();

...
ixam::Array<uint> ExecutionGroup::get_execution_order() const
{
  ixam::Array<uint> chunk_order(chunks_len_);
  ...
  switch (order_type) {
    case ChunkOrdering::Random: {
      static ixam::RandomNumberGenerator rng;
      ixam::MutableSpan<uint> span = chunk_order.as_mutable_span();
    ...
    case ChunkOrdering::CenterOut: {
      ...
      ixam::Array<ChunkOrder> chunk_orders(chunks_len_);
...
void ExecutionGroup::execute(ExecutionSystem *graph)
{
  ...
  ixam::Array<uint> chunk_order = get_execution_order();

 ...
 * are explained below.
 *
 * \section EM_Step1 Step 1: translating ixam node system to the new compositor system
 * Ixams node structure is based on C structs (DNA). These structs are not efficient in the new
 * architecture. We want to use classes in order to simplify the system. during this step the
 * ixam node_tree is evaluated and converted to a CPP node system.
 *
 * \see ExecutionSystem

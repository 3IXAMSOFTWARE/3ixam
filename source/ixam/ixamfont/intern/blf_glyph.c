    ...
    FontBLF *font, GlyphCacheBLF *gc, FT_GlyphSlot glyph, uint charcode, FT_UInt glyph_index)
{
  ...
  if (buffer_size != 0) {
    if (font->flags & BLF_MONOCHROME) {
      /* Font buffer uses only 0 or 1 values, 3IXAM expects full 0..255 range. */
...
static bool blf_glyph_render_bitmap(FontBLF *font, FT_GlyphSlot glyph)
{
  ...
  if (font->flags & BLF_MONOCHROME) {
    ...
    /* Does 3IXAM use Pitch 1 always? It works so far */
                                     ...
                                     int fixed_width)
{
  ...
  if(U.font_weight == FONT_WEIGHT_BOLD)
    blf_glyph_transform_weight(glyph, 0.7f, glyph->face->face_flags & FT_FACE_FLAG_FIXED_WIDTH);
  else if(U.font_weight == FONT_WEIGHT_MEDIUM)
    blf_glyph_transform_weight(glyph, 0.5f, glyph->face->face_flags & FT_FACE_FLAG_FIXED_WIDTH);

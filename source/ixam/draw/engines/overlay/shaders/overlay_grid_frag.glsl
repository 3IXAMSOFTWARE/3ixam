...
#define MAX_CELLS 14
#define GRID_CELLS (MAX_CELLS)
#define CELL_SCALE gridSteps[0]
...
float get_grid(vec2 co, vec2 fwidthCos, vec2 grid_scale)
{
  ...
  vec2 drawn_cells = abs(co / grid_scale);
  if(drawn_cells.x > GRID_CELLS || drawn_cells.y > GRID_CELLS)
    return 0.0;
...
vec3 get_axes(vec3 co, vec3 fwidthCos, float line_size, float cell_scale)
{
  vec3 axes_domain = abs(co);
  /* Modulate by the absolute rate of change of the coordinates
   * (make line have the same width under perspective). */
  axes_domain /= fwidthCos;
  vec3 drawn_cells = abs(co / cell_scale);
  if(drawn_cells.x > GRID_CELLS || drawn_cells.y > GRID_CELLS || drawn_cells.z > GRID_CELLS)
...
void main()
{
  ...
  if (flag_test(grid_flag, SHOW_GRID)) {
    ...
    float gridA = get_grid(grid_pos, grid_fwidth, vec2(1.0f));
  ...
  if (flag_test(grid_flag, (SHOW_AXIS_X | SHOW_AXIS_Y | SHOW_AXIS_Z))) {
    ...
    vec3 axes = get_axes(axes_dist, axes_fwidth, 0.1, 1.0f);
		vec3 axisColor = vec3(.0, .33, .65); //0x3b8698ff

    if (flag_test(grid_flag, SHOW_AXIS_X)) {
      out_color.a = max(out_color.a, axes.x);
      out_color.rgb = (axes.x < 1e-8) ? out_color.rgb : axisColor.rgb;
    }
    if (flag_test(grid_flag, SHOW_AXIS_Y)) {
      out_color.a = max(out_color.a, axes.y);
      out_color.rgb = (axes.y < 1e-8) ? out_color.rgb : axisColor.rgb;
    }
    if (flag_test(grid_flag, SHOW_AXIS_Z)) {
      out_color.a = max(out_color.a, axes.z);
      out_color.rgb = (axes.z < 1e-8) ? out_color.rgb : axisColor.rgb;
    }
  }

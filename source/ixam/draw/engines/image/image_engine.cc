...
namespace ixam::draw::image_engine {
    ...
    const DRWContextState *draw_ctx)
{
  ...
  if (ELEM(space_type, SPACE_IMAGE, SPACE_RENDER)) {
    return std::make_unique<SpaceImageAccessor>((SpaceImage *)draw_ctx->space_data);
  }
  if (ELEM(space_type, SPACE_NODE, SPACE_MATPRO)) {
...
using namespace ixam::draw::image_engine;

...
{
  bNodeTree *ntree = (ixam_mat->use_nodes && ixam_mat->nodetree != nullptr) ?
                         ixam_mat->nodetree :
                         default_surface_ntree_.nodetree_get(ixam_mat);

  MaterialPass matpass = MaterialPass();
  matpass.gpumat = inst_.shaders.material_shader_get(
      ixam_mat, ntree, pipeline_type, geometry_type, true);
  ...
  switch (GPU_material_status(matpass.gpumat)) {
    ...
    case GPU_MAT_QUEUED:
      ...
      ixam_mat = (geometry_type == MAT_GEOM_VOLUME) ? BKE_material_default_volume() :
                                                         BKE_material_default_surface();
      matpass.gpumat = inst_.shaders.material_shader_get(
          ixam_mat, ixam_mat->nodetree, pipeline_type, geometry_type, false);
      break;
    case GPU_MAT_FAILED:
    default:
      matpass.gpumat = inst_.shaders.material_shader_get(
  ...
  if (ELEM(pipeline_type,
    ...
    PassMain::Sub *shader_sub = shader_map_.lookup_or_add_cb(shader_key, [&]() {
      ...
      return inst_.pipelines.material_add(ob, ixam_mat, matpass.gpumat, pipeline_type);
    });
...
Material &MaterialModule::material_sync(Object *ob,
                                        ...
                                        bool has_motion)
{
  eMaterialPipeline surface_pipe = (ixam_mat->blend_method == MA_BM_BLEND) ? MAT_PIPE_FORWARD :
                                                                                MAT_PIPE_DEFERRED;
  eMaterialPipeline prepass_pipe = (ixam_mat->blend_method == MA_BM_BLEND) ?
  ...
  MaterialKey material_key(ixam_mat, geometry_type, surface_pipe);

  Material &mat = material_map_.lookup_or_add_cb(material_key, [&]() {
    Material mat;
    /* Order is important for transparent. */
    mat.prepass = material_pass_get(ob, ixam_mat, prepass_pipe, geometry_type);
    mat.shading = material_pass_get(ob, ixam_mat, surface_pipe, geometry_type);
    if (ixam_mat->blend_shadow == MA_BS_NONE) {
      mat.shadow = MaterialPass();
    }
    else {
      mat.shadow = material_pass_get(ob, ixam_mat, MAT_PIPE_SHADOW, geometry_type);
    }
    mat.is_alpha_blend_transparent = (ixam_mat->blend_method == MA_BM_BLEND) &&
  ...
  if (mat.is_alpha_blend_transparent) {
    ...
    mat.prepass.sub_pass = inst_.pipelines.forward.prepass_transparent_add(
        ob, ixam_mat, mat.shading.gpumat);
...
MaterialArray &MaterialModule::material_array_get(Object *ob, bool has_motion)
{
  ...
  for (auto i : IndexRange(materials_len)) {
    ::Material *ixam_mat = material_from_slot(ob, i);
    Material &mat = material_sync(ob, ixam_mat, to_material_geometry(ob), has_motion);
...
Material &MaterialModule::material_get(Object *ob,
                                       ...
                                       eMaterialGeometry geometry_type)
{
  ::Material *ixam_mat = material_from_slot(ob, mat_nr);
  Material &mat = material_sync(ob, ixam_mat, geometry_type, has_motion);
  return mat;
}

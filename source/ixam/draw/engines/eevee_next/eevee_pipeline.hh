...
class ForwardPipeline {
 ...
 public:
  ...
  PassMain::Sub *prepass_opaque_add(::Material *ixam_mat, GPUMaterial *gpumat, bool has_motion);
  PassMain::Sub *material_opaque_add(::Material *ixam_mat, GPUMaterial *gpumat);
                              ...
                              eMaterialPipeline pipeline_type)
  {
    switch (pipeline_type) {
      case MAT_PIPE_DEFERRED_PREPASS:
        // return deferred.prepass_add(ixam_mat, gpumat, false);
      case MAT_PIPE_FORWARD_PREPASS:
        if (GPU_material_flag_get(gpumat, GPU_MATFLAG_TRANSPARENT)) {
          return forward.prepass_transparent_add(ob, ixam_mat, gpumat);
        }
        return forward.prepass_opaque_add(ixam_mat, gpumat, false);

      case MAT_PIPE_DEFERRED_PREPASS_VELOCITY:
        // return deferred.prepass_add(ixam_mat, gpumat, true);
      case MAT_PIPE_FORWARD_PREPASS_VELOCITY:
        if (GPU_material_flag_get(gpumat, GPU_MATFLAG_TRANSPARENT)) {
          return forward.prepass_transparent_add(ob, ixam_mat, gpumat);
        }
        return forward.prepass_opaque_add(ixam_mat, gpumat, true);

      case MAT_PIPE_DEFERRED:
        // return deferred.material_add(ixam_mat, gpumat);
      case MAT_PIPE_FORWARD:
        if (GPU_material_flag_get(gpumat, GPU_MATFLAG_TRANSPARENT)) {
          return forward.material_transparent_add(ob, ixam_mat, gpumat);
        }
        return forward.material_opaque_add(ixam_mat, gpumat);

      case MAT_PIPE_VOLUME:
        /* TODO(fclem) volume pass. */
        return nullptr;
      case MAT_PIPE_SHADOW:
        // return shadow.material_add(ixam_mat, gpumat);
        break;
    }
    return nullptr;
  }

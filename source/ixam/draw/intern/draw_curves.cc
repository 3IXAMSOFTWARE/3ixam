...
using CurvesInfosBuf = ixam::draw::UniformBuffer<CurvesInfos>;

struct CurvesUniformBufPool {
  ixam::Vector<std::unique_ptr<CurvesInfosBuf>> ubos;
  int used = 0;
                                               ...
                                               GPUMaterial *gpu_material)
{
  ...
  if (curves.curves_num() >= 1) {
    ixam::VArray<float> radii = curves.attributes().lookup_or_default(
        "radius", ATTR_DOMAIN_POINT, 0.005f);
    const ixam::IndexRange first_curve_points = curves.points_for_curve(0);

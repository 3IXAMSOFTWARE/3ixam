    ...
    float *UNUSED(progress))
{
  ...
  while (true) {
    if (*stop != 0) {
       ...
       * but wm can kill the task if we are closing ixam. */
      break;
    }
...
void DRW_shader_library_add_file(DRWShaderLibrary *lib, const char *lib_code, const char *lib_name)
{
  ...
  if (index > -1) {
    ...
    lib->libs_deps[index] = drw_shader_dependencies_get(
        lib, "IXAM_REQUIRE(", lib_code, lib_name);
  }
  else {
...
{
  uint64_t deps = drw_shader_dependencies_get(lib, "IXAM_REQUIRE(", shader_code, "shader code");

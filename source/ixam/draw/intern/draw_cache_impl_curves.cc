                                          ...
                                          const int index)
{
   ...
   * the 3IXAM convention, it should be `vec4(s, s, s, 1)`. This could be resolved using a
...
static void request_attribute(Curves &curves, const char *name)
{
  ...
  ixam::bke::CurvesGeometry &curves_geometry = ixam::bke::CurvesGeometry::wrap(
      curves.geometry);
  std::optional<ixam::bke::AttributeMetaData> meta_data =

...
ixam::draw::DebugDraw *DRW_debug_get()
{
  if (!GPU_shader_storage_buffer_objects_support()) {
    return nullptr;
  }
  return reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug);
}
...
void drw_debug_draw()
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->display_to_view();
#endif
}
...
void drw_debug_init()
{
  ...
  if (DST.debug == nullptr) {
    DST.debug = reinterpret_cast<DRWDebugModule *>(new ixam::draw::DebugDraw());
  }
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->init();
#endif
}
...
void drw_debug_module_free(DRWDebugModule *module)
{
  ...
  if (module != nullptr) {
    delete reinterpret_cast<ixam::draw::DebugDraw *>(module);
  }
}

GPUStorageBuf *drw_debug_gpu_draw_buf_get()
{
  return reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->gpu_draw_buf_get();
}

GPUStorageBuf *drw_debug_gpu_print_buf_get()
{
  return reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->gpu_print_buf_get();
}
...
void DRW_debug_modelmat_reset()
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->modelmat_reset();
}
...
void DRW_debug_modelmat(const float modelmat[4][4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->modelmat_set(modelmat);
#else
...
void DRW_debug_line_v3v3(const float v1[3], const float v2[3], const float color[4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_line(v1, v2, color);
}
...
void DRW_debug_polygon_v3(const float (*v)[3], int vert_len, const float color[4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_polygon(
      ixam::Span<float3>((float3 *)v, vert_len), color);
}
...
void DRW_debug_m4(const float m[4][4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_matrix(m);
}
...
void DRW_debug_m4_as_bbox(const float m[4][4], bool invert, const float color[4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_matrix_as_bbox(m4, color);
}
...
void DRW_debug_bbox(const BoundBox *bbox, const float color[4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_bbox(*bbox, color);
#else
...
void DRW_debug_sphere(const float center[3], float radius, const float color[4])
{
  ...
  reinterpret_cast<ixam::draw::DebugDraw *>(DST.debug)->draw_sphere(center, radius, color);
}

...
static void curve_eval_render_wire_verts_edges_len_get(const ixam::bke::CurvesGeometry &curves,
                                                       int *r_curve_len,
                                                       int *r_vert_len,
                                                       int *r_edge_len)
{
  ...
  *r_edge_len = 0;
  const ixam::VArray<bool> cyclic = curves.cyclic();
  for (const int i : curves.curves_range()) {
    const IndexRange points = curves.evaluated_points_for_curve(i);
    *r_edge_len += ixam::bke::curves::segments_num(points.size(), cyclic[i]);
  }
}
  ...
  if (types & CU_DATATYPE_WIRE) {
    if (rdata->curve_eval != nullptr) {
      curve_eval_render_wire_verts_edges_len_get(
          ixam::bke::CurvesGeometry::wrap(rdata->curve_eval->geometry),
...
static void curve_create_attribute(CurveRenderData *rdata, GPUVertBuf *vbo_attr)
{
  ...
  const ixam::VArraySpan<ColorGeometry4f> colors = curves.attributes().lookup<ColorGeometry4f>(
  ...
  curves.interpolate_to_evaluated(colors,
                                  ixam::MutableSpan<ColorGeometry4f>{vbo_data, vert_len});
}

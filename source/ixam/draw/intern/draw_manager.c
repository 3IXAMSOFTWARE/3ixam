...
static void drw_engines_enable_editors(void)
{
  ...
  if (ELEM(space_data->spacetype, SPACE_IMAGE, SPACE_RENDER)) {
    drw_engine_enable_image_editor();
  }
  else if (ELEM(space_data->spacetype, SPACE_NODE, SPACE_MATPRO)) {
...
#if defined(WITH_DRAW_DEBUG) && defined(DEBUG)
  if (G.debug_value == 31) {
...
void DRW_draw_view(const bContext *C, View3D *v3d)
{
  if (v3d) {
                                ...
                                const bContext *evil_C)
{
  ...
  const bool do_populate_loop = (ELEM(DST.draw_ctx.space_data->spacetype, SPACE_IMAGE, SPACE_RENDER));
  const bool do_annotations = drw_draw_show_annotation();
  const bool do_draw_gizmos = (!ELEM(DST.draw_ctx.space_data->spacetype, SPACE_IMAGE, SPACE_RENDER));
...
void DRW_render_context_enable(Render *render)
{
  ...
  /* Changing Context */
  if (re_gl_context != NULL && !BLI_thread_is_main()) {
 ...
 * a non-main thread, that thread is supposed to not have OpenGL context ever bound by 3IXAM.
 *
 * \{ */

...
static GPUVertFormat *get_ixam_patch_coords_format()
{
  static GPUVertFormat format = {0};
  if (format.attr_len == 0) {
                                         ...
                                         const int *subdiv_polygon_offset)
{
  ...
  GPU_vertbuf_init_with_format_ex(
      cache->patch_coords, get_ixam_patch_coords_format(), GPU_USAGE_DYNAMIC);
  GPU_vertbuf_data_alloc(cache->patch_coords, cache->num_subdiv_loops);

  cache->corner_patch_coords = GPU_vertbuf_calloc();
  GPU_vertbuf_init_with_format_ex(
      cache->corner_patch_coords, get_ixam_patch_coords_format(), GPU_USAGE_DYNAMIC);
                                    ...
                                    const SubsurfRuntimeData *runtime_data)
{
  ...
  if (cache->num_subdiv_loops != 0) {
    ...
    cache->fdots_patch_coords = gpu_vertbuf_create_from_format(get_ixam_patch_coords_format(),
                                                               mesh_eval->totpoly);
    CompressedPatchCoord *ixam_fdots_patch_coords = (CompressedPatchCoord *)
    ...
    for (int i = 0; i < mesh_eval->totpoly; i++) {
      ...
      if (polys[i].totloop == 4) {
        ...
        ixam_fdots_patch_coords[i] = make_patch_coord(ptex_face_index, 0.5f, 0.5f);
      }
      else {
        ...
        ixam_fdots_patch_coords[i] = make_patch_coord(ptex_face_index, 1.0f, 1.0f);
      }
    }
  ...
  ixam::Vector<int> first_loop_index(cache->num_subdiv_verts, -1);
                                                 ...
                                                 uint mat_len)
{
  ...
  const ixam::VArraySpan<int> material_indices = mesh_eval->attributes().lookup_or_default<int>(
                                                 ...
                                                 OpenSubdiv_EvaluatorCache *evaluator_cache)
{
  ...
  ixam::draw::mesh_buffer_cache_create_requested_subdiv(batch_cache, mbc, draw_cache, mr);
...
ixam::Span<DRWSubdivLooseEdge> draw_subdiv_cache_get_loose_edges(const DRWSubdivCache *cache)
{
  return {cache->loose_geom.edges, int64_t(cache->loose_geom.edge_len)};
}

ixam::Span<DRWSubdivLooseVertex> draw_subdiv_cache_get_loose_verts(const DRWSubdivCache *cache)
{

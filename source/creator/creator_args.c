...
static void arg_py_context_backup(bContext *C,
                                  struct IxamePyContextStore *c_py,
...
static void arg_py_context_restore(bContext *C, struct IxamePyContextStore *c_py)
{
...
#    define BPY_CTX_SETUP(_cmd) \
      { \
        struct IxamePyContextStore py_c; \
...
#  ifdef BUILD_DATE
  ...
  printf("3IXAM %s (hash %s built %s %s)\n",
         BKE_ixam_version_string(),
         build_hash,
         build_date,
         build_time);
...
static int arg_handle_print_help(int UNUSED(argc), const char **UNUSED(argv), void *data)
{
  ...
  printf("3IXAM %s\n", BKE_ixam_version_string());
  printf("Usage: ixam [args ...] [file] [args ...]\n\n");
...
#  endif
  ...
  printf("\t# ixam -ba test.ixam\n");
  printf("\t...will exit since '-ba' is an unknown argument.\n");

  printf("Argument Order:\n");
  printf("\tArguments are executed in the order they are given. eg:\n");
  printf("\t# ixam --background test.ixam --render-frame 1 --render-output '/tmp'\n");
  printf(
      "\t...will not render to '/tmp' because '--render-frame 1' renders before the output path "
      "is set.\n");
  printf("\t# ixam --background --render-output /tmp test.ixam --render-frame 1\n");
  printf(
      "\t...will not render to '/tmp' because loading the ixam-file overwrites the render output "
      "that was set.\n");
  printf("\t# ixam --background test.ixam --render-output /tmp --render-frame 1\n");
  printf("\t...works as expected.\n\n");

  printf("Environment Variables:\n");
  printf("  $IXAM_USER_RESOURCES  Top level directory for user files.\n");
  printf("                           (other 'IXAM_USER_*' variables override when set).\n");
  printf("  $IXAM_USER_CONFIG     Directory for user configuration files.\n");
  printf("  $IXAM_USER_SCRIPTS    Directory for user scripts.\n");
  printf("  $IXAM_USER_DATAFILES  Directory for user data files (icons, translations, ..).\n");
  printf("\n");
  printf("  $IXAM_SYSTEM_RESOURCES  Top level directory for system files.\n");
  printf("                             (other 'IXAM_SYSTEM_*' variables override when set).\n");
  printf("  $IXAM_SYSTEM_SCRIPTS    Directory for system wide scripts.\n");
  printf("  $IXAM_SYSTEM_DATAFILES  Directory for system wide data files.\n");
  printf("  $IXAM_SYSTEM_PYTHON     Directory for system Python libraries.\n");
...
static const char arg_handle_auth_token_set_doc[] =
    "\n\t"
    "Set authorization token.";
static int arg_handle_auth_token_set(int argc,
                                          const char **argv,
                                          void *UNUSED(data))
{
  if (argc > 1) {
    BLI_strncpy(G.auth_token, argv[1], sizeof(G.auth_token));
    return 1;
  }
  printf("\nError: you must specify a authentication token after '%s'.\n", argv[0]);
...
static const char arg_handle_factory_startup_set_doc[] =
    "\n\t"
    "Skip reading the " STRINGIFY(IXAM_STARTUP_FILE) " in the users home directory.";
...
static const char arg_handle_env_system_set_doc_datafiles[] =
    "\n\t"
    "Set the " STRINGIFY_ARG(IXAM_SYSTEM_DATAFILES) " environment variable.";
static const char arg_handle_env_system_set_doc_scripts[] =
    "\n\t"
    "Set the " STRINGIFY_ARG(IXAM_SYSTEM_SCRIPTS) " environment variable.";
static const char arg_handle_env_system_set_doc_python[] =
    "\n\t"
    "Set the " STRINGIFY_ARG(IXAM_SYSTEM_PYTHON) " environment variable.";

static int arg_handle_env_system_set(int argc, const char **argv, void *UNUSED(data))
{
  /* `--env-system-scripts` -> `IXAM_SYSTEM_SCRIPTS` */

  char env[64] = "IXAM";
  char *ch_dst = env + 7;           /* skip IXAM */
...
static const char arg_handle_playback_mode_doc[] =
    "<options> <file(s)>\n"
    "\tInstead of showing 3IXAM's user interface, this runs 3IXAM as an animation player,\n"
    "\tto view movies and image sequences rendered in 3IXAM (ignored if '-b' is set).\n"
    "\n"
    "\tPlayback Arguments:\n"
    "\n"
    "\t-p <sx> <sy>\n"
...
static const char arg_handle_register_extension_doc[] =
    "\n\t"
    "Register ixam-file extension, then exit (Windows only).";
static const char arg_handle_register_extension_doc_silent[] =
    "\n\t"
    "Silently register ixam-file extension, then exit (Windows only).";
...
#  ifdef WIN32
  ...
  BLI_windows_register_ixam_extension(G.background);
...
static const char arg_handle_output_set_doc[] =
    ...
    "\tUse '//' at the start of the path to render relative to the ixam-file.\n"
    "\n"
    ...
    "\t# ixam -b animation.ixam -o //render_ -F PNG -x 1 -a\n"
...
static int arg_handle_output_set(int argc, const char **argv, void *data)
{
  ...
  if (argc > 1) {
    ...
    if (scene) {
      ...
      printf("\nError: no ixam loaded. cannot use '-o / --render-output'.\n");
    }
    return 1;
  }
  ...
  if (argc >= 2) {
    if (STREQ(argv[1], "help")) {
      ...
      if (scene) {
        ...
        printf(
            "\nError: no ixam loaded. "
            "order the arguments so '-E / --engine' is after file is loaded.\n");
      }
    }
...
static const char arg_handle_image_type_set_doc[] =
    ...
    "\n"
    "\tFormats that can be compiled into 3IXAM, not available on all systems:\n"
...
static int arg_handle_image_type_set(int argc, const char **argv, void *data)
{
  ...
  if (argc > 1) {
    ...
    if (scene) {
      ...
      printf(
          "\nError: no ixam loaded. "
          "order the arguments so '-F  / --render-format' is after the ixam file is loaded.\n");
    }
    return 1;
  }
...
static int arg_handle_render_frame(int argc, const char **argv, void *data)
{
  ...
  printf("\nError: no ixam loaded. cannot use '%s'.\n", arg_id);
  return 0;
}
...
static int arg_handle_render_animation(int UNUSED(argc), const char **UNUSED(argv), void *data)
{
  ...
  if (scene) {
    ...
    printf("\nError: no ixam loaded. cannot use '-a'.\n");
  }
  return 0;
}
...
#  ifdef WITH_PYTHON
  ...
  if (argc > 1) {
    /* Make the path absolute because its needed for relative linked ixam files to be found */
...
#  else
  ...
  printf("This 3IXAM was built without Python support\n");
  return 0;
#  endif /* WITH_PYTHON */
}
...
static const char arg_handle_python_console_run_doc[] =
    "\n\t"
    "Run 3IXAM with an interactive console.";
...
static int arg_handle_load_file(int UNUSED(argc), const char **argv, void *data)
{
  ...
  /* Make the path absolute because its needed for relative linked ixams to be found */
  char filepath[FILE_MAX];
  ...
  if (success) {
    ...
    if (G.background) {
       ...
       * ixam will return non-zero exit code which then
    ...
    if (BLO_has_bfile_extension(filepath)) {
      ...
      printf(
          "Error: argument has no '.ixam' file extension, not using as new file, exiting! %s\n",
          filepath);
      G.is_break = true;
      WM_exit(C);
    }
...
static const char arg_handle_load_last_file_doc[] =
    "\n\t"
    "Open the most recently opened ixam file, instead of the default startup file.";
...
#  define CB_EX(a, b) a##_doc_##b, a
  ...
  BLI_args_add(ba, "-auth", NULL, CB(arg_handle_auth_token_set), NULL);

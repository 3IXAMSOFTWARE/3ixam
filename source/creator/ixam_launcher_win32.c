...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
  ...
  /* Get the path to the currently running executable (ixam-launcher.exe) */
  ...
  /* Remove the filename (ixam-launcher.exe) from path. */
  if (PathCchRemoveFileSpec(path, MAX_PATH) != S_OK) {
    return -1;
  }

  /* Add ixam.exe to path, resulting in the full path to 3IXAM executable. */
  if (PathCchCombine(path, MAX_PATH, path, L"3ixam.exe") != S_OK) {
    return -1;
  }
  ...
  if (success) {
    /* If ixam-launcher is called with background command line flag or launched from steam,
     * wait for the ixam process to exit and return its return value. */

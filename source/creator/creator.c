...
/* -------------------------------------------------------------------- */
/** \name 3IXAM as a Stand-Alone Python Module (bpy)
 *
 * While not officially supported, this can be useful for Python developers.
 * See: https://wiki.3ixam.com/wiki/Building_Ixam/Other/IxamAsPyModule
 * \{ */
...
/**
 * 3IXAM's main function responsibilities are:
 * - setup subsystems.
 * - handle arguments.
...
)
{
  ...
  BKE_ixam_atexit_register(callback_main_atexit, &app_init_data);
...
#  undef main
...
#ifndef WITH_PYTHON
  printf(
      "\n* WARNING * - 3IXAM compiled without Python!\n"
  ...
  BKE_ixam_atexit_unregister(callback_main_atexit, &app_init_data);
...
#ifdef TIME_BOMB
  if (!timeBomb()) {
    WM_show_message_box("3IXAM", "3IXAM currently unavailable.", "Quit");
    WM_exit(C);
  }
#endif

#ifdef LAUNCHER_CHECK
  if (!validateAuthToken(G.auth_token)) {
    WM_show_message_box("3IXAM", "Please open the 3IXAM Launcher first", "Quit");
    WM_exit(C);
  }
#endif

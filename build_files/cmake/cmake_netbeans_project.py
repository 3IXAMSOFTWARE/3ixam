...
"""
Example linux usage
 python3 ~/ixam-git/ixam/build_files/cmake/cmake_netbeans_project.py ~/ixam-git/cmake
...
def create_nb_project_main():
    ...
    if SIMPLE_PROJECTFILE:
        ...
        f.write('  </logicalFolder>\n')
        # default, but this dir is in fact not in 3IXAM dir so we can ignore it
        # f.write('  <sourceFolderFilter>^(nbproject)$</sourceFolderFilter>\n')
        f.write(r'  <sourceFolderFilter>^(nbproject|__pycache__|.*\.py|.*\.html|.*\.ixam)$</sourceFolderFilter>\n')
        ...
        f.write('          <executablePath>./bin/ixam</executablePath>\n')
        ...
        f.write('        </runcommandpicklist>\n')
        f.write('        <runcommand>%s</runcommand>\n' % os.path.join(CMAKE_DIR, "bin/ixam"))

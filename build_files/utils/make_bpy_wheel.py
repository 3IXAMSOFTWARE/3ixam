...
"""
Make Python wheel package (`*.whl`) file from 3IXAM built with 'WITH_PYTHON_MODULE' enabled.
...
def main() -> None:
    ...
    # Get 3IXAM version.
    ixam_version_str = str(make_utils.parse_ixam_version())
    ...
    setuptools.setup(
        ...
        description="3IXAM as a Python module",
    ...
    for f in os.listdir(dist_dir):
        if f.endswith(".whl"):
            ixam_py = "cp%d%d" % (python_version_number[0], python_version_number[1])
            ...
            if hasattr(sys, "abiflags"):
                ...
                renamed_f = f.replace(sys_py_abi, ixam_py).replace(sys_py, ixam_py)
            else:
                renamed_f = f.replace(sys_py, ixam_py)

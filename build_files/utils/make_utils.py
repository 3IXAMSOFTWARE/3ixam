...
def git_branch(git_command: str) -> str:
    ...
    try:
        ...
        sys.stderr.write("Failed to get 3IXAM git branch\n")
        sys.exit(1)
...
def git_branch_release_version(branch: str, tag: Optional[str]) -> Optional[str]:
    re_match = re.search("^ixam-v(.*)-release$", branch)
    ...
    return "3.1"#release_version


def svn_libraries_base_url(release_version: Optional[str], branch: Optional[str] = None) -> str:
    if release_version:
        svn_branch = "tags/ixam-" + release_version + "-release"
    elif branch:
        svn_branch = "branches/" + branch
    else:
        svn_branch = "trunk"
    return "https://svn.3ixam.com/svnroot/bf-ixam/" + svn_branch + "/lib/"
...
class IxamVersion:
    ...
    def __str__(self) -> str:
        ...
        >>> str(IxamVersion(293, 1, "alpha"))
        '2.93.1-alpha'
        >>> str(IxamVersion(327, 0, "release"))
        '3.27.0'
        """
...
def parse_ixam_version() -> IxamVersion:
    ixam_srcdir = Path(__file__).absolute().parent.parent.parent
    version_path = ixam_srcdir / "source/ixam/ixamkernel/BKE_ixam_version.h"

    version_info = {}
    line_re = re.compile(r"^#define (IXAM_VERSION[A-Z_]*)\s+([0-9a-z]+)$")
    ...
    return IxamVersion(
        int(version_info["IXAM_VERSION"]),
        int(version_info["IXAM_VERSION_PATCH"]),
        version_info["IXAM_VERSION_CYCLE"],
    )

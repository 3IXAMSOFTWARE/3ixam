...
"""
"make update" for all platforms, updating svn libraries and tests and 3IXAM
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--no-ixam", action="store_true")
...
def svn_update(args: argparse.Namespace, release_version: Optional[str]) -> None:
    ...
    lib_dirpath = os.path.join(get_ixam_git_root(), '..', 'lib')
...
# Update ixam repository.
def ixam_update(args: argparse.Namespace) -> None:
    print_stage("Updating 3IXAM Git Repository")
...
if __name__ == "__main__":
    ...
    if branch == 'HEAD':
        sys.stderr.write('3IXAM git repository is in detached HEAD state, must be in a branch\n')
        sys.exit(1)
    ...
    if not args.no_ixam:
        ixam_skip_msg = git_update_skip(args)
        if ixam_skip_msg:
            ixam_skip_msg = "3IXAM repository skipped: " + ixam_skip_msg + "\n"
        else:
            ixam_update(args)
...
#        submodules_skip_msg = submodules_update(args, release_version, branch)
    ...
    skip_msg = ixam_skip_msg + submodules_skip_msg
    if skip_msg:
        print_stage(skip_msg.strip())

    # For failed submodule update we throw an error, since not having correct
    # submodules can make 3IXAM throw errors.
    # For 3IXAM itself we don't and consider "make update" to be a command

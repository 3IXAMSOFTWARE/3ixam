...
def main() -> None:
    ixam_srcdir = Path(__file__).absolute().parent.parent.parent

    cli_parser = argparse.ArgumentParser(
        description=f"Create a tarball of the 3IXAM sources, optionally including sources of dependencies.",
    ...
    print(f"Source dir: {ixam_srcdir}")

    curdir = ixam_srcdir.parent
    os.chdir(curdir)
    ixam_srcdir = ixam_srcdir.relative_to(curdir)

    print(f"Output dir: {curdir}")

    version = make_utils.parse_ixam_version()
    tarball = tarball_path(curdir, version, cli_args)
    manifest = manifest_path(tarball)
    packages_dir = packages_path(curdir, cli_args)

    create_manifest(version, manifest, ixam_srcdir, packages_dir)
    create_tarball(version, tarball, manifest, ixam_srcdir, packages_dir)
    create_checksum_file(tarball)
    cleanup(manifest)
    print("Done!")


def tarball_path(output_dir: Path, version: make_utils.IxamVersion, cli_args: Any) -> Path:
    extra = ""
    if cli_args.include_packages:
        extra = "-with-libraries"

    return output_dir / f"ixam{extra}-{version}.tar.xz"
...
def manifest_path(tarball: Path) -> Path:
    ...
    >>> from pathlib import Path
    >>> tarball = Path("/home/sybren/workspace/ixam-git/ixam-test.tar.gz")
    >>> manifest_path(tarball).as_posix()
    '/home/sybren/workspace/ixam-git/ixam-test-manifest.txt'
    """
...
def create_manifest(
    version: make_utils.IxamVersion,
    outpath: Path,
    ixam_srcdir: Path,
    ...
    with outpath.open("w", encoding="utf-8") as outfile:
        main_files_to_manifest(ixam_srcdir, outfile)
        submodules_to_manifest(ixam_srcdir, version, outfile)
...
def main_files_to_manifest(ixam_srcdir: Path, outfile: TextIO) -> None:
    assert not ixam_srcdir.is_absolute()
    for path in git_ls_files(ixam_srcdir):
        print(path, file=outfile)


def submodules_to_manifest(
    ixam_srcdir: Path, version: make_utils.IxamVersion, outfile: TextIO
) -> None:
    skip_addon_contrib = version.is_release()
    assert not ixam_srcdir.is_absolute()

    for line in git_command("-C", ixam_srcdir, "submodule"):
        ...
        for path in git_ls_files(ixam_srcdir / submodule):
            print(path, file=outfile)
...
def create_tarball(
    ...
    command += [
        "--transform",
        f"s,^{ixam_srcdir.name}/,ixam-{version}/,g",

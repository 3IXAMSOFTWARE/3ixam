...
ARGUMENTS_INFO="\"COMMAND LINE ARGUMENTS:
    ...
    --show-deps
        Show main dependencies of 3IXAM (including officially supported versions) and exit.
    ...
    Note about the --build-foo options:
        ...
        * Please be careful with the 3IXAM building options if you have both 'official' dev packages and
...
WARNING "****WARNING****"
PRINT "If you are experiencing issues building 3IXAM, _*TRY A FRESH, CLEAN BUILD FIRST*_!"
...
#~ OSL_SOURCE_REPO_BRANCH="ixam-fixes"
...
if [ "$DO_SHOW_DEPS" = true ]; then
  PRINT ""
  PRINT "3IXAM dependencies (libraries needed to build it):"
  PRINT ""
...
compile_USD() {
  ...
  if [ ! -d $_inst ]; then
    ...
    cmake_d="$cmake_d -DPXR_SET_INTERNAL_NAMESPACE=usdIxam"
...
install_DEB() {
  ...
  if [ $? -eq 0 ]; then
    ...
    PRINT ""
    WARNING "libyaml-cpp-dev not found, you may have to install it by hand to get 3IXAM compiling..."
    PRINT ""
  fi
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      PRINT ""
      WARNING "clang-format $CLANG_FORMAT_VERSION_MIN or higher not found, this is NOT needed to get 3IXAM compiling..."
      PRINT ""
    fi
  fi
  ...
  # This is so 3IXAM can at least compile.
  PRINT ""
  ...
  if [ "$WITH_EMBREE" = true ]; then
    ...
    if [ "$EMBREE_SKIP" = true ]; then
      ...
      # There is a package, but it does not provide everything that 3IXAM needs...
...
print_info() {
  ...
  PRINT ""
  PRINT "Or even simpler, just run (in your ixam-source dir):"

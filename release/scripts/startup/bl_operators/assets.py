...
class ASSET_OT_open_containing_ixam_file(Operator):
    """Open the ixam file that contains the active asset"""

    bl_idname = "asset.open_containing_ixam_file"
    bl_label = "Open ixam file"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        if asset_file_handle.local_id:
            self.report({'WARNING'}, "This asset is stored in the current ixam file")
            return {'CANCELLED'}

        asset_lib_path = bpy.types.AssetHandle.get_full_library_path(asset_file_handle, asset_library_ref)
        self.open_in_new_ixam(asset_lib_path)
    ...
    def modal(self, context, event):
        ...
        if returncode:
            self.report({'WARNING'}, tip_("3IXAM sub-process exited with error code %d") % returncode)
    ...
    def open_in_new_ixam(self, filepath):
        import subprocess
...
classes = (
    ASSET_OT_tag_add,
    ASSET_OT_tag_remove,
    ASSET_OT_open_containing_ixam_file,
)

...
class WM_OT_previews_batch_generate(Operator):
    ...
    filter_ixam: BoolProperty(
        default=True,
        options={'HIDDEN', 'SKIP_SAVE'},
        name="",
        description="Show 3IXAM files in the File Browser",
    )
    ...
    use_trusted: BoolProperty(
        default=False,
        name="Trusted ixam files",
        description="Enable python evaluation for selected files",
    )
    use_backups: BoolProperty(
        default=True,
        name="Save Backups",
        description="Keep a backup (.ixam) version of the files when saving with generated previews",
    )
    ...
    def execute(self, context):
        ...
        for i, fn in enumerate(self.files):
            ixam_path = os.path.join(self.directory, fn.name)
            cmd = [
                bpy.app.binary_path,
                "--background",
                "--factory-startup",
            ...
            if subprocess.call(cmd):
                self.report({'ERROR'}, "Previews generation process failed for file '%s'!" % ixam_path)
...
class WM_OT_previews_batch_clear(Operator):
    ...
    use_backups: BoolProperty(
        default=True,
        name="Save Backups",
        description="Keep a backup (.ixam) version of the files when saving with cleared previews",
    )
    ...
    def execute(self, context):
        ...
        for i, fn in enumerate(self.files):
            ...
            if not self.use_backups:
                cmd.append("--no_backups")
            if subprocess.call(cmd):
                self.report({'ERROR'}, tip_("Previews clear process failed for file '%s'!") % ixam_path)
                context.window_manager.progress_end()
                return {'CANCELLED'}
            context.window_manager.progress_update(i + 1)
        context.window_manager.progress_end()
...
class WM_OT_ixam_strings_utf8_validate(Operator):
    """Check and fix all strings in current .ixam file to be valid UTF-8 Unicode """ \
    ...
    def execute(self, _context):
        ...
        if changed:
            self.report({'WARNING'},
                        "Some strings were fixed, don't forget to save the .ixam file to keep those changes")
        return {'FINISHED'}
...
classes = (
    ...
    WM_OT_ixam_strings_utf8_validate,
)

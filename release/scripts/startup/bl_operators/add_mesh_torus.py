...
class AddTorus(Operator, object_utils.AddObjectHelper):
    ...
    bl_label = "Add Torus"
    bl_options = {'REGISTER', 'UNDO', 'PRESET', 'REDO_OVERRIDE'}
...
class AddTube(Operator, object_utils.AddObjectHelper):
    ...
    inner_scale: FloatVectorProperty(
        name="Inner Scale",
        subtype='TRANSLATION',
    )
    generate_uvs: BoolProperty(
    ...
    def execute(self, context):
        ...
        bpy.ops.mesh.primitive_cylinder_add(location=self.location, rotation=self.rotation, vertices=self.segments)

        cylinder1 = bpy.context.view_layer.objects.active
        cylinder1.name = "Tube"
        cylinder1.scale = self.scale

        bpy.ops.mesh.primitive_cylinder_add(location=self.location, rotation=self.rotation, vertices=self.segments)

        cylinder2 = bpy.context.view_layer.objects.active
        cylinder2.scale *= self.inner_scale
        cylinder2.scale.z = cylinder1.scale.z * 2.0

        bool_one = cylinder1.modifiers.new(type="BOOLEAN", name="bool 1")
        bool_one.object = cylinder2
        bool_one.operation = 'DIFFERENCE'
        # 'EXACT' solver is kinda broken and crashes especially in sandbox
        bool_one.solver = 'FAST'

        # "Clamping" second cylinder's scale because of weird case, when the scales are equal.
        EPS = 1e-4
        x1, y1, _ = self.scale
        x2, y2, _ = self.inner_scale
        scales_ratio = ((x2 / x1) * (y2 / y1)) if y1 != 0 and y2 != 0 else 0
        if abs(1 - scales_ratio) < EPS:
            cylinder2.scale *= (1 - EPS)

        bpy.context.view_layer.objects.active = cylinder1
        bpy.ops.object.modifier_apply(modifier=bool_one.name)

        bpy.data.objects.remove(cylinder2)

...
rna_path_prop = StringProperty(
    ...
    description="Context data-path (expanded using visible windows in the current .ixam file)",
    maxlen=1024,
...
class WM_OT_context_modal_mouse(Operator):
    ...
    def modal(self, context, event):
        ...
        area_has_header_text = hasattr(context.area, "header_text_set")
...
class WM_OT_url_open_preset(Operator):
    ...
    def _url_from_bug(self, _context):
        from bl_ui_utils.bug_report_url import url_prefill_from_ixam
        return url_prefill_from_ixam()

    def _url_from_bug_addon(self, _context):
        from bl_ui_utils.bug_report_url import url_prefill_from_ixam
        return url_prefill_from_ixam(addon_info=self.id)
    ...
    preset_items = [
        ...
        (('RELEASE_NOTES', iface_("Release Notes"),
          tip_("Read about what's new in this version of 3IXAM")),
         "https://www.3ixam.com"),
        (('MANUAL', iface_("User Manual"),
          tip_("The reference manual for this version of 3IXAM")),
         "https://www.3ixam.com"),
        (('API', iface_("Python API Reference"),
          tip_("The API reference manual for this version of 3IXAM")),
         "https://www.3ixam.com"),
        ...
        (('CREDITS', iface_("Credits"),
          tip_("Lists committers to 3IXAM's source code")),
         "https://www.3ixam.com"),
    ]
...
class WM_OT_doc_view(Operator):
    ...
    if bpy.app.version_cycle in {"release", "rc", "beta"}:
        _prefix = ("https://docs.3ixam.com/api/%d.%d" %
                   (bpy.app.version[0], bpy.app.version[1]))
    else:
        _prefix = ("https://docs.3ixam.com/api/master")
...
rna_custom_property_name = StringProperty(
    ...
    # Match `MAX_IDPROP_NAME - 1` in 3IXAM's source.
    maxlen=63,
)
...
class WM_OT_properties_edit(Operator):
    ...
    def _update_ixam_for_prop_change(self, context, item, name, prop_type_old, prop_type_new):
        from rna_prop_ui import (
        ...
        # Otherwise existing buttons which reference freed memory may crash 3IXAM (T26510).
    ...
    def execute(self, context):
        ...
        self._update_ixam_for_prop_change(context, item, name, prop_type_old, prop_type_new)
...
class WM_OT_tool_set_by_id(Operator):
    ...
    instant_primitive: BoolProperty(
        name="Insant Primitive",
        description="Add primitives instantly",
        default=False,
        options={'SKIP_SAVE', 'HIDDEN'}, 
    )
    ...
    primitives_operators = {
        'builtin.primitive_cube_add': (bpy.ops.mesh.primitive_cube_add, {}),
        'builtin.primitive_uv_sphere_add': (bpy.ops.mesh.primitive_uv_sphere_add, {}),
        'builtin.primitive_torus_add': (bpy.ops.mesh.primitive_torus_add, {}),
        'builtin.primitive_cylinder_add': (bpy.ops.mesh.primitive_cylinder_add, {}),
        'builtin.primitive_cone_add': (bpy.ops.mesh.primitive_cone_add, {}),
        'builtin.primitive_tube_add': (bpy.ops.mesh.primitive_tube_add, {'scale': (1, 1, 1), 'inner_scale': (0.4, 0.4, 0.4)}),
        'builtin.primitive_hexagon_add': (bpy.ops.mesh.primitive_cylinder_add, {'vertices': 6}),
        'builtin.primitive_pyramid_add': (bpy.ops.mesh.primitive_cone_add, {'vertices': 6}),
        'builtin.primitive_plane_add': (bpy.ops.mesh.primitive_plane_add, {}),
        'builtin.primitive_grid_add': (bpy.ops.mesh.primitive_grid_add, {}),
    }

    def invoke(self, context, event):
        if (operator := self.primitives_operators.get(self.name)) and (event.shift or self.instant_primitive):
            operator_func, arguments = operator
    ...
    def execute(self, context):
        ...
        if fn(context, space_type, self.name, as_fallback=self.as_fallback, region=context.region):
            if self.as_fallback:
...
class WM_OT_intro_preview(Operator):
    bl_idname = "wm.intro_preview"
    bl_label = "Intro Preview"

    def execute(self, context):
        import aud, bpy, os
        if not bpy.context.preferences.view.use_play_intro:
            return
        device = aud.Device()
        ...
        path = ""
        intro = bpy.context.preferences.view.intro_sound
        if intro == 'INTROSOUND_1':
            path = "intro 3ixam 1.wav"
        elif intro == 'INTROSOUND_2':
            path = "intro 3ixam 2.wav"
        elif intro == 'INTROSOUND_3':
            path = "intro 3ixam 3.wav"

        intro_sound_path = os.path.join(bpy.utils.resource_path('LOCAL'), 'datafiles', 'sounds', path)
        sound = aud.Sound(intro_sound_path)
        device.play(sound)
...
class WM_OT_toolbar(Operator):
    ...
    def execute(self, context):
        ...
        def draw_menu(popover, context):
            ...
            cls.draw_cls(layout, context, detect_layout=False, scale_y=1.75)
...
class WM_OT_batch_rename(Operator):
    ...
    def _data_from_context(cls, context, data_type, only_selected, *, check_context=False):
        ...
        object_data_type_attrs_map = {
            ...
            'ARMATURE': ("armatures", "Rig(s)", bpy.types.Armature),
...
def is_running_darkmode():
    import subprocess
    import sys
    import platform
    if sys.platform == "darwin":
        from distutils.version import LooseVersion as ver
        if ver(platform.mac_ver()[0]) >= ver("10.14"):
...
class WM_MT_splash(Menu):
    ...
    def draw(self, context):
        ...
        found_recent = col1.template_recent_files()

        if found_recent:
            col1_title.label(text="Recent Files")
        else:

            # Links if no recent files
            col1_title.label(text="Getting Started")

            col1.operator("wm.url_open_preset", text="3IXAM Website", icon='URL').type = 'IXAM'

        # Templates
        col2 = split.column()
        col2.label(text="New File")

        bpy.types.TOPBAR_MT_file_new.draw_ex(col2, context, use_splash=True)
        ...
        label = context.preferences.current_theme
        col2.menu("USERPREF_MT_interface_theme_presets", text=label)

        col2.label(text="Hand type")
        col2.prop(context.preferences.view, "hand_type", text="", emboss=False)

        col1.label(text="Language")
        prefs = context.preferences
        col1.prop(prefs.view, "language", text="", emboss=False)

        col1.label(text="Intro Sound")
        sub = col1.row(align=True)
        sub.emboss = "NONE"
        sub.separator()

        sub2 = sub.row(align=True)
        sub2.operator("wm.intro_preview", text="", icon='PLAY')

        sub.prop(prefs.view, "intro_sound", text="", emboss=False)
...
class WM_MT_splash_about(Menu):
    ...
    def draw(self, context):
        ...
        row.label(text="3IXAM")

        row = col.row(align=True)
        row.alignment = 'CENTER'
        row.label(text=bpy.app.version_string, translate=False)

        row = col.row(align=True)
        row.emboss = 'PULLDOWN_MENU'
        row.alignment = 'CENTER'
        row.operator("wm.url_open_preset", text="3IXAM Website     ", icon='URL').type = 'IXAM'
        col.separator()


class WM_OT_new_instance(bpy.types.Operator):
    """Creates a new instance of 3IXAM"""
    bl_idname = "wm.new_instance"
    ...
    def execute(self, context):
        ...
        try:
            if bpy.app.build_platform == b'Windows':
                subprocess.run(["start", "", bpy.app.binary_path], shell=True)
            elif bpy.app.build_platform == b'Linux':
                subprocess.run(["gnome-terminal", "-x", "sh", "-c", bpy.app.binary_path, "&", "disown"])
            elif bpy.app.build_platform == b'Darwin':
                bpy.ops.wm.launch_file_create()
                subprocess.run(["open", "-n", "-a", bpy.app.binary_path])
            else:
                subprocess.run([bpy.app.binary_path])
        except:
            self.report({'ERROR'}, "Unable to create new 3IXAM instance")

        return {'FINISHED'}


class WM_OT_drop_ixam_file(Operator):
    bl_idname = "wm.drop_ixam_file"
    bl_label = "Handle dropped .ixam file"
    bl_options = {'INTERNAL'}
    ...
    def draw_menu(self, menu, _context):
        ...
        col = layout.column()
        col.operator_context = 'INVOKE_DEFAULT'
        col.operator("wm.link", text="Link...", icon='LINK_IXAM').filepath = self.filepath
        col.operator("wm.append", text="Append...", icon='APPEND_IXAM').filepath = self.filepath


class WM_OT_open_legacy(bpy.types.Operator, ImportHelper):
    """Open a Legacy File"""
    ...
    filter_glob: StringProperty(
        default="*.b" + "l" + "e" + "n" + "d",  # because of renaming
        options={'HIDDEN'},
    )
    ...
    @classmethod
    def poll(cls, context):
        return bpy.app.debug_value == 555

    def execute(self, context):
        keywords = self.as_keywords(ignore=("filter_glob",))
        return self.open_legacy(**keywords)

    def draw(self, context):
        layout = self.layout
        layout.prop(self, "load_ui")
        layout.prop(self, "trusted_source")

    def open_legacy(self, filepath="", load_ui=False, trusted_source=True):
        import os
        ...
        if not file_extension:
            file_extension = ".b" + "l" + "e" + "n" + "d"  # because of renaming

        legacy_file = file_name + file_extension
        legacy_filepath = os.path.join(file_dir, legacy_file)

        ixam_file = file_name + ".ixam"
        ixam_filepath = os.path.join(file_dir, ixam_file)
        print(f"[Legacy Files] Legacy file: {legacy_filepath}")
        print(f"[Legacy Files] Ixam file: {ixam_file}")
        convert = self.convert_legacy_to_ixam(legacy_filepath, ixam_filepath)

        if convert:
            try:
                return bpy.ops.wm.open_mainfile(filepath=ixam_filepath, load_ui=load_ui, use_scripts=trusted_source)
            except RuntimeError as ex:
                if "File written by newer 3IXAM binary" in str(ex):
                    return {'FINISHED'}
                else:
                    raise Exception(f"Unable to open converted legacy file: {ex}")
        else:
            return {'CANCELLED'}

    def convert_legacy_to_ixam(self, legacy_filepath, ixam_filepath):
        def isndigit(c):
            return c < b'0' or c > b'9'

        def is_gzip(hdr):
            return hdr[0:2] == b'\x1f\x8b'

        def is_zstd(hdr):
            return hdr[0:4] == b'\x28\xb5\x2f\xfd'
        ...
        with open(legacy_filepath, "rb") as file:
            legacy_data = file.read()
            if is_gzip(legacy_data):
                uses_gzip = True
                with gzip.open(legacy_filepath, "rb") as file:
                    legacy_data = file.read()
            elif is_zstd(legacy_data):
                uses_zstd = True
                with zstandard.open(legacy_filepath, "rb") as file:
                    legacy_data = file.read()

            print(f"[Legacy Files] Input file size: {len(legacy_data)}")

            if not legacy_data.startswith(b"BLE" + b"NDER"):  # because of renaming (7 bytes len)
                print("[Legacy Files] Error on header")
                return False

            if legacy_data[7:8] != b'_' and legacy_data[7:8] != b'-':
                print("[Legacy Files] Error on ptrsize")
                return False

            if legacy_data[8:9] != b'V' and legacy_data[8:9] != b'v':
                print("[Legacy Files] Error on endiannes")
                return False

            if isndigit(legacy_data[9:10]) or isndigit(legacy_data[10:11]) or isndigit(legacy_data[11:12]):
                print("[Legacy Files] Error on version")
                return False

            output_data = b"IXAM"
            output_data += b"\x08" if legacy_data[7:8] == b"-" else b"\x04"
            output_data += b"\x01" if legacy_data[8:9] == b"v" else b"\x00"
            output_data += (int(legacy_data[9:12])).to_bytes(2, byteorder='big' if legacy_data[8] == b'V' else 'little')
            output_data += legacy_data[12:]

        print(f"[Legacy Files] Output file size: {len(output_data)}")
        if uses_gzip:
            stream = gzip.open
        elif uses_zstd:
            stream = zstandard.open
        else:
            stream = open

        with stream(ixam_filepath, "wb") as file:

...
class PREFERENCES_OT_copy_prev(Operator):
    ...
    def previous_version(cls):
        ...
        # NOTE: remove this block for 3IXAM 4.0 and later.
        if version_old == 299:
            version_old = 294
    ...
    def poll(cls, _context):
        ...
        old_userpref = os.path.join(old, "config", "userpref.ixam")
        new_userpref = os.path.join(new, "config", "userpref.ixam")
...
class PREFERENCES_OT_addon_enable(Operator):
    ...
    def execute(self, _context):
        ...
        if mod:
            ...
            info_ver = info.get("ixam", (0, 0, 0))

            if info_ver > bpy.app.version:
                self.report(
                    {'WARNING'},
                    "This script was written 3IXAM "
...
class PREFERENCES_OT_theme_install(Operator):
    """Load and apply a 3IXAM XML theme file"""

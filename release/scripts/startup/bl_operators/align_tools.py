...
class OBJECT_OT_align_tools(Operator):
    ...
    # Selection Option:
    ref1: EnumProperty(
            items=(("3", "Maximum", "Align the maximum point"),
                   ("1", "Center", "Align the center point"),
                   ("2", "Pivot Point", "Align the pivot"),
                   ("0", "Minimum", "Align the minimum point")),
            name="Selection reference",
            description="Moved objects reference point"
            )
    # Active Object Option:
    ref2: EnumProperty(
            items=(("3", "Maximum", "Align to the maximum point"),
                   ("1", "Center", "Align to the center point"),
                   ("2", "Pivot Point", "Align to the pivot"),
                   ("0", "Minimum", "Align to the minimum point")),
    ...
    def draw(self, context):
        layout = self.layout
        row1 = layout.row()
        row1.label(text="Align Position")

        # Align Location:
        row2 = layout.row()
        row2.prop(self, "loc_x", text="X")
        row2.prop(self, "loc_y", text="Y")
        row2.prop(self, "loc_z", text="Z")

        row3 = layout.row()
        col1 = row3.column()
        col1.label(text="Current Object")
        col1.prop(self, 'ref1', expand=True)

        col2 = row3.column()
        col2.label(text="Target Object")
        col2.prop(self, 'ref2', expand=True)

        layout.separator()

        row4 = layout.row()
        row4.label(text="Align Rotation")
        row5 = layout.row(align=True)
        row5.prop(self, 'rot_x', text='X')
        row5.prop(self, 'rot_y', text='Y')
        row5.prop(self, 'rot_z', text='Z')

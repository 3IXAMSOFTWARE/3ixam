...
class NodeAddOperator:
    ...
    def store_mouse_cursor(context, event):
        ...
        if event.shift and context.region.type == 'WINDOW':
            # convert mouse position to the View2D for later node placement
    ...
    def poll(cls, context):
        ...
        return (space and (space.type == 'NODE_EDITOR' or space.type == 'MATPRO') and
    ...
    def invoke(self, context, event):
        ...
        if self.use_transform and ('FINISHED' in result):
            ...
            space = context.space_data
            if space.type == 'MATPRO':
                bpy.ops.matpro.translate_attach_remove_on_cancel('INVOKE_DEFAULT')
            else:
...
class NODE_OT_tree_path_parent(Operator):
    ...
    def poll(cls, context):
        space = context.space_data
        # needs active node editor and a tree
        return (space and (space.type == 'NODE_EDITOR' or space.type == 'MATPRO') and len(space.path) > 1)

...
ToolDef = namedtuple(
    ...
    (
        ...
        #
        #   Note that this isn't used for 3IXAM's built in tools which use the built-in key-map.
...
class ToolSelectPanelHelper:
    ...
    @staticmethod
    def _tool_class_from_space_type(space_type, region=None):
        def none_compare(cls_rt, region):
            if region is None:
                return True
            return region.type in ('WINDOW', 'TEMPORARY') or cls_rt == region.type
        return next(
            (cls for cls in ToolSelectPanelHelper.__subclasses__()
             if cls.bl_space_type == space_type and none_compare(cls.bl_region_type, region)),
            None
        )
    ...
    def _tool_active_from_context(context, space_type, mode=None, create=False):
        if space_type in {'VIEW_3D', 'PROPERTIES'}:
            ...
            if tool is not None:
                tool.refresh_from_context()
                return tool
        elif space_type == 'MATPRO':
            space_data = context.space_data
            tool = context.workspace.tools.from_space_matpro(create=create)
            if tool is not None:
                tool.refresh_from_context()
                return tool
    ...
    def register(cls):
        ...
        # 3IXAM stores the active tool-group index.
        #
    ...
    def _popover_alignment_from_handtype(alignment):
        hand_type = bpy.context.preferences.view.hand_type
        if hand_type != 'HAND_RIGHT':
            if alignment == 'LEFT':
                return 'RIGHT'
    ...
    def draw_cls(cls, layout, context, detect_layout=True, scale_y=1.75):
        ...
        show_text = True
        ui_gen = layout.hex_flow(columns=1, align=True)
        ui_gen.scale_y = cls.vertical_scale()
        ui_gen.icon_scale = cls.icon_scale()
        ui_gen.alignment = cls.align()
        ...
        for item in cls.tools_from_context(context):
            ...
            icon_value = ToolSelectPanelHelper._icon_value_from_icon_handle(item.icon if icon is None else icon)


            sub = ui_gen
            #sub = ui_gen.send(False)
            shape = cls.button_shape()
            props = None

            sub.operator_context = item.context if item.context is not None else "INVOKE_DEFAULT"
            ...
            if use_menu:
                ...
                if item.type == 'Operator':
                    props = sub.operator(item.idname,                         
                                text=item.label if show_text else "",
                                icon_value=icon_value,
                                shape=shape)
                    ...
                    props = sub.popover(
                                ...
                                icon_value=icon_value,
                                shape=shape,
                                region_align=cls._popover_alignment_from_handtype(item.align)
                    )
                elif item.type == 'Menu':
                    props = sub.menu(
                                item.idname,
...
class WM_MT_toolsystem_submenu(Menu):
    ...
    def draw(self, context):
        ...
        for item in item_group:
            ...
            icon_value = ToolSelectPanelHelper._icon_value_from_icon_handle(item.icon)
            if item.type == 'Operator':
                props = layout.operator(item.idname,                         
                            text="",
                            icon_value=icon_value)
                if item.properties is not None:
                    for prop, value in item.properties.items():
...
def _activate_by_item(context, space_type, item, index, *, as_fallback=False, region=None):
    cls = ToolSelectPanelHelper._tool_class_from_space_type(space_type, region)
...
def activate_by_id(context, space_type, idname, *, as_fallback=False, region=None):
    cls = ToolSelectPanelHelper._tool_class_from_space_type(space_type, region)
    if cls is None:
        return False
    ...
    if item is None:
        return False
    _activate_by_item(context, space_type, item, index, as_fallback=as_fallback, region=region)
    return True


def activate_by_id_or_cycle(context, space_type, idname, *, offset=1, as_fallback=False, region=None):

    # Only cycle when the active tool is activated again.
    cls = ToolSelectPanelHelper._tool_class_from_space_type(space_type, region)
    if cls == None:
        print(f"{space_type=}{region.type=}")
    ...
    _activate_by_item(context, space_type, item_found, index_found, region=region)
    return True


def description_from_id(context, space_type, idname, *, use_operator=True):
    # Used directly for tooltips.
    # Maybe bad idea FIXME
    cls = ToolSelectPanelHelper._tool_class_from_space_type(space_type, region=context.region)
    if cls is None:
        return ''
    item, _index = cls._tool_get_by_id(context, idname)
    if item is None:

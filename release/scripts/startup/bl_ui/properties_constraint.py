...
class BONE_PT_constraints(BoneConstraintPanel, Panel):
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'TOOLS'
    bl_label = "Joint Constraints"
    bl_options = {'HIDE_HEADER'}

    def draw(self, _context):
        layout = self.layout

        layout.operator_menu_enum("pose.constraint_add", "type", text="Add Joint Constraint")
...
class ConstraintButtonsPanel:
        ...
        if target or owner:
            ...
            if con.target_space == 'CUSTOM' or con.owner_space == 'CUSTOM':
                ...
                if con.space_object and con.space_object.type == 'ARMATURE':
                    col.prop_search(con, "space_subtarget", con.space_object.data, "bones", text="Joint")
    ...
    def target_template(layout, con, subtargets=True):
        ...
        if con.target and subtargets:
            if con.target.type == 'ARMATURE':
                col.prop_search(con, "subtarget", con.target.data, "bones", text="Joint")
    ...
    def draw_python_constraint(self, _context):
        layout = self.layout
        layout.label(text="3IXAM 2.6 doesn't support python constraints yet")
    ...
    def draw_armature(self, context):
        ...
        layout.operator("constraint.add_target", text="Add Target Joints")
        ...
        if not con.targets:
            layout.label(text="No target joints added", icon='ERROR')
    ...
    def draw_kinematic(self, context):
        ...
        if context.object.pose.ik_solver == 'ITASC':
            ...
            if con.pole_target and con.pole_target.type == 'ARMATURE':
                col.prop_search(con, "pole_subtarget", con.pole_target.data, "bones", text="Joint")

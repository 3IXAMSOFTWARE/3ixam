...
class IMAGE_MT_uvs_align(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("uv.align", text="Straighten").axis = 'ALIGN_S'
        layout.operator("uv.align", text="Straighten X").axis = 'ALIGN_T'
        layout.operator("uv.align", text="Straighten Y").axis = 'ALIGN_U'
        #layout.operator("uv.align", text="Align Auto").axis = 'ALIGN_AUTO'
        layout.operator("uv.align", text="Align X").axis = 'ALIGN_X'
        layout.operator("uv.align", text="Align Y").axis = 'ALIGN_Y'
...
class IMAGE_MT_uvs_tools(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator_context = 'INVOKE_DEFAULT'
        layout.operator("uv.smart_project", text="Automatic UV Pack")
        layout.operator("uv.lightmap_pack", text="Simple UV Pack")
        
        layout.operator_context = 'EXEC_REGION_WIN'
        layout.operator("uv.cube_project", text="Cube Project UV")
        layout.operator("uv.cylinder_project", text="Cylinder Project UV")
        layout.operator("uv.sphere_project", text="Sphere Project UV")
        layout.operator_context = 'INVOKE_DEFAULT'

        layout.separator()

        layout.operator("uv.mark_seam", text="Mark Seam UV").clear = False
        layout.operator("uv.mark_seam", text="Clear Seam UV").clear = True
        layout.operator("uv.seams_from_islands", text="Rebuild Seams UV")

        layout.separator()

        layout.operator("uv.reset", text="Reset UV")

        layout.separator()

        layout.prop(uv, "lock_bounds", text="Limit Transform by Canvas")

        layout.separator()

        layout.prop(uv, "show_stretch")

        layout.separator()

        layout.menu("IMAGE_MT_uvs_transform")
        layout.menu("IMAGE_MT_uvs_mirror", text="Symmetrize")
        layout.menu("IMAGE_MT_uvs_align", text="Align Transform")
        layout.operator("uv.minimize_stretch", text="Relax")
        layout.operator("uv.uv_squares", text="Make QSquares (Uniform)")
        layout.operator("uv.uv_squares_by_shape", text="Make QSquares (Shape)")

        layout.separator()

        layout.operator("uv.pack_islands", text="UV Packing")
        layout.operator("uv.average_islands_scale", text="Make Islands Scale Relative")

        layout.separator()

        layout.operator("uv.export_layout", text="Save UV as Image")

        # export UV layout is appended at scripts/addons/io_mesh_uv_layout/__init__.py
...
class IMAGE_MT_uvs_select_mode(Menu):
    ...
    def draw(self, context):
        ...
        IMAGE_MT_uvs_select_mode.draw_select_options(layout, context)

    @staticmethod
    def draw_select_options(layout, context):
        ...
        if tool_settings.use_uv_select_sync:
            props = layout.operator("wm.context_set_value", text="Vertex Mode", icon='VERTEXSEL')
            props.value = "(True, False, False)"
            props.data_path = "tool_settings.mesh_select_mode"

            props = layout.operator("wm.context_set_value", text="Edge Mode", icon='EDGESEL')
            props.value = "(False, True, False)"
            props.data_path = "tool_settings.mesh_select_mode"

            props = layout.operator("wm.context_set_value", text="Face Mode", icon='FACESEL')
            props.value = "(False, False, True)"
            props.data_path = "tool_settings.mesh_select_mode"

        else:
            props = layout.operator("wm.context_set_string", text="Vertex Mode", icon='UV_VERTEXSEL')
            props.value = 'VERTEX'
            props.data_path = "tool_settings.uv_select_mode"

            props = layout.operator("wm.context_set_string", text="Edge Mode", icon='UV_EDGESEL')
            props.value = 'EDGE'
            props.data_path = "tool_settings.uv_select_mode"

            props = layout.operator("wm.context_set_string", text="Face Mode", icon='UV_FACESEL')
            props.value = 'FACE'
            props.data_path = "tool_settings.uv_select_mode"

            props = layout.operator("wm.context_set_string", text="Island Mode", icon='UV_ISLANDSEL')
            props.value = 'ISLAND'
            props.data_path = "tool_settings.uv_select_mode"
        
        # reset context for use from outside
        layout.operator_context = 'INVOKE_DEFAULT'
...
class IMAGE_MT_uvs_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if sima.show_uvedit:
            ...
            layout.operator("image.view_selected", text="Frame Selected", icon="ZOOM_SELECTED")
            ...
            layout.menu("IMAGE_MT_uvs_align", text="Align Transform")
            layout.operator("uv.align", text="Align by Near Axis").axis = 'ALIGN_AUTO'
            ...
            layout.operator("uv.weld", text="Join")
            layout.operator("uv.stitch", text="Sew")
            layout.operator("uv.select_split", text="Rip")
...
class IMAGE_HT_header(Header):
    ...
    def draw(self, context):
        ...
        if show_uvedit or show_maskedit:
            sub_icon = layout.row()
            sub_icon.icon_scale = 1.5
            sub_icon.label(text="", icon="WORLD_AXIS")
            sub_icon.scale_x = 1.6

            layout.prop(sima, "pivot_point", text="")
            ...
            uvform_depress = context.mode == 'EDIT_MESH'
            layout.popover("IMAGE_PT_uvform", depress=uvform_depress)
...
class IMAGE_MT_editor_menus(Menu):
    ...
    def draw(self, context):
        ...
        #trapezoid_menu("IMAGE_MT_view")

        #if show_uvedit:
        #    trapezoid_menu("IMAGE_MT_select")
        #if show_maskedit:
        #    trapezoid_menu("MASK_MT_select")

        trapezoid_menu("IMAGE_MT_uvs_tools")

        if ima and ima.is_dirty:
            trapezoid_menu("IMAGE_MT_image", text="Image*")
        else:
            trapezoid_menu("IMAGE_MT_image", text="Image")

        #if show_uvedit:
        #    trapezoid_menu("IMAGE_MT_uvs")
        #if show_maskedit:
        #    trapezoid_menu("MASK_MT_add")
        #    trapezoid_menu("MASK_MT_mask")
    ...
    def draw(self, context):
        ...
        if active_object:
            if active_object.type == 'MESH':
                icon_scale_from = lambda flag: 1.5 if flag else 1.0

                if tool_settings.use_uv_select_sync:
                    print("UV sync selection is not supported")

                grid = sub.grid_flow(columns=4)
            
                mode = grid.column(align=True)
                mode.icon_scale = icon_scale_from(tool_settings.uv_select_mode == 'VERTEX')
                props = mode.operator("wm.context_set_string", text="", icon='UV_VERTEXSEL')
                props.value = 'VERTEX'
                props.data_path = "tool_settings.uv_select_mode"

                mode = grid.column(align=True)
                mode.icon_scale = icon_scale_from(tool_settings.uv_select_mode == 'EDGE')
                props = mode.operator("wm.context_set_string", text="", icon='UV_EDGESEL')
                props.value = 'EDGE'
                props.data_path = "tool_settings.uv_select_mode"

                mode = grid.column(align=True)
                mode.icon_scale = icon_scale_from(tool_settings.uv_select_mode == 'FACE')
                props = mode.operator("wm.context_set_string", text="", icon='UV_FACESEL')
                props.value = 'FACE'
                props.data_path = "tool_settings.uv_select_mode"

                mode = grid.column(align=True)
                mode.icon_scale = icon_scale_from(tool_settings.uv_select_mode == 'ISLAND')
                props = mode.operator("wm.context_set_string", text="", icon='UV_ISLANDSEL')
                props.value = 'ISLAND'
                props.data_path = "tool_settings.uv_select_mode"
        ...
        col = sub.column()
        col.scale_x = 1.25
        col.operator("uv.minimize_stretch", text="Relax")
        col.operator("uv.uv_squares", text="Make QSquares (Uniform)")
        col.operator("uv.uv_squares_by_shape", text="Make QSquares (Shape)")

        col = sub.column()
        col.operator("uv.weld", text="Join")
        col.operator("uv.stitch", text="Sew")
        col.operator("uv.select_split", text="Rip")
...
class IMAGE_OT_view_selected_as_tool(Operator):
    """View all selected UVs"""
    bl_label = "Frame Selected"
    bl_options = {'REGISTER', 'UNDO'}
    bl_idname = "image.view_selected_as_tool"
    ...
    def execute(self, context):
        for area in bpy.context.screen.areas:
            if area.type == 'IMAGE_EDITOR':
                ...
                bpy.ops.image.view_selected(ctx) 
        return {'FINISHED'}
...
class _defs_transform:
    ...
    def translate():
        return dict(
            idname="image.view_pan",
            label="Move",
            icon="ops.transform.transform",
            keymap="Image Editor Tool: Pan"
        )
    ...
    def scale():
        return dict(
            idname="image.view_zoom",
            label="Scale",
            icon="ops.transform.zoom_tool",
            keymap="Image Editor Tool: Zoom",
        )
    
class IMAGE_PT_hud(ToolSelectPanelHelper, Panel):
    bl_space_type = 'IMAGE_EDITOR'
    bl_region_type = 'HUD'
    bl_label = "HUD" 
    ...
    def draw(self, context):
        ...
        coords.shape = 'RHOMBOID'
        
        coords.label(text='X', shape = 'RHOMBOID')
        coords.uv_coords(axis='X')
        coords.label(text='Y', shape = 'RHOMBOID')
        coords.uv_coords(axis='Y')
...
classes = (
    ...
    IMAGE_MT_uvs_tools,
    IMAGE_PT_uvform,
    IMAGE_OT_view_selected_as_tool,

...
class FILEBROWSER_PT_filter(FileBrowserPanel, Panel):
    ...
    def draw(self, context):
        ...
        if params.filter_glob:
            ...
            row = col.row()
            row.label(icon='FILE_IXAM')
            row.prop(params, "use_filter_ixam",
                     text=".ixam Files", toggle=False)
            row = col.row()
            row.label(icon='FILE_BACKUP')
            row.prop(params, "use_filter_backup",
                     text="Backup .ixam Files", toggle=False)
            row = col.row()
        ...
        if is_lib_browser:
            ...
            row.prop(params, "use_filter_ixamid",
                     text="3IXAM IDs", toggle=False)
            if params.use_filter_ixamid:
                row = col.row()
                row.label(icon='BLANK1')  # Indentation
...
class FILEBROWSER_PT_advanced_filter(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(params, "use_filter_ixamid")
        if params.use_filter_ixamid:
            layout.separator()
...
class ASSETBROWSER_PT_metadata(asset_utils.AssetBrowserPanel, Panel):
    ...
    def draw(self, context):
        ...
        row.operator("asset.open_containing_ixam_file", text="", icon='TOOL_SETTINGS')
...
class ASSETBROWSER_MT_context_menu(AssetBrowserMenu, Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("asset.open_containing_ixam_file")
...
def register_props():
    ...
    WindowManager.asset_path_dummy = StringProperty(
        name="Asset Ixam Path",
        description="Full path to 3IXAM file containing the active asset",

...
class ThicknessIxamMixIn(ThicknessModifierMixIn):
...
class ThicknessAlongStrokeShader(ThicknessIxamMixIn, CurveMappingModifier):
    ...
    def __init__(self, thickness_position, thickness_ratio,
        ...
        ThicknessIxamMixIn.__init__(self, thickness_position, thickness_ratio)
...
class ThicknessDistanceFromCameraShader(ThicknessIxamMixIn, CurveMappingModifier):
...
class ThicknessDistanceFromObjectShader(ThicknessIxamMixIn, CurveMappingModifier):
...
class ThicknessMaterialShader(ThicknessIxamMixIn, CurveMappingModifier):
...
class CalligraphicThicknessShader(ThicknessIxamMixIn, ScalarBlendModifier):
...
class TangentThicknessShader(ThicknessIxamMixIn, CurveMappingModifier):
...
class ThicknessNoiseShader(ThicknessIxamMixIn, ScalarBlendModifier, NoiseShader):
...
class CreaseAngleThicknessShader(ThicknessIxamMixIn, CurveMappingModifier):
...
class Curvature3DThicknessShader(ThicknessIxamMixIn, CurveMappingModifier):
    """Alpha based on the 3D curvature of the underlying geometry"""
...
def process(layer_name, lineset_name):
    ...
    if linestyle.use_nodes and linestyle.node_tree:
        shaders_list.append(IxamTextureShader(linestyle.node_tree))
        has_tex = True
    if has_tex:

...
#
# This uses 3IXAM's key-map, running with `legacy=True`.
#
# The intention of this key-map is to match 3IXAM 2.7x which had many more key-map items available.
#
# There are some differences with the original 3IXAM 2.7x key-map.
# There is no intention to change these are they are not considered significant
# enough to make a 1:1 match with the previous 3IXAM version.
#
# These include:
#
# 3D View
...
ixam_default = bpy.utils.execfile(os.path.join(DIRNAME, "keymap_data", "3ixam_default.py"))
...
def load():
    ...
    keyconfig_data = ixam_default.generate_keymaps(
        ixam_default.Params(

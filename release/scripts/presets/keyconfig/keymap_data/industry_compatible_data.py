...
# NOTE: duplicated from `ixam_default.py`.
...
def generate_keymaps(params=None):
    ...
    ixam_default_mod = execfile(
        os.path.join(os.path.dirname(__file__), "3ixam_default.py"),
    )

    ixam_default = ixam_default_mod.generate_keymaps(
        # Use the default key-map with only minor changes to default arguments.
        ixam_default_mod.Params(
    ...
    keymap.extend([km for km in ixam_default if km[0] not in keymap_existing_names])

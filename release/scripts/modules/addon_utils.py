...
def paths():
    # RELEASE SCRIPTS: official scripts distributed in 3IXAM releases
...
def enable(module_name, *, default_set=False, persistent=False, handle_error=None):
    ...
    with RestrictIxam():
        ...
        if mod.bl_info.get("ixam", (0, 0, 0)) < (2, 80, 0):
            if _bpy.app.debug:
        ...
        try:
            ...
            for f in glob.glob(f"{dir}/**/*.py", recursive=True):
                if f.endswith("__init__.py"):
                    continue
                rel_path = os.path.relpath(f, dir)
                rel_path = rel_path.replace(r'/', '.')
                __import__(f"{module_name}.{rel_path[:-3]}", locals(), globals())
...
def _ixam_manual_url_prefix():
    if _bpy.app.version_cycle in {"rc", "release"}:
        manual_version = "%d.%d" % _bpy.app.version[:2]
    else:
        manual_version = "dev"

    return "https://docs.3ixam.com/manual/en/" + manual_version

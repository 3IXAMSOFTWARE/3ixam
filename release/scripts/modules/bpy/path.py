...
functions for dealing with paths in 3IXAM.
"""
...
def abspath(path, *, start=None, library=None):
    """
    Returns the absolute path relative to the current ixam file
    using the "//" prefix.
...
def relpath(path, *, start=None):
    """
    Returns the path relative to the current ixam file using the "//" prefix.
...
def ensure_ext(filepath, ext, *, case_sensitive=False):
    ...
    :arg ext: The extension to check for, can be a compound extension. Should
              start with a dot, such as '.ixam' or '.tar.gz'.
    :type ext: string

...
"""
This module contains utility functions specific to 3IXAM but
not associated with ixams internal data.
"""
...
_default_keyconfig = None

# Directories added to the start of `sys.path` for all of 3IXAM's "scripts" directories.
_script_module_dirs = "startup", "modules"

# Base scripts, this points to the directory containing: "modules" & "startup" (see `_script_module_dirs`).
# In 3IXAM's code-base this is `./release/scripts`.
#
# NOTE: in virtually all cases this should match `IXAM_SYSTEM_SCRIPTS` as this script is it's self a system script,
# it must be in the `IXAM_SYSTEM_SCRIPTS` by definition and there is no need for a look-up from `_bpy_script_paths`.
...
def load_scripts(*, reload_scripts=False, refresh_scripts=False):
    ...
    from bpy_restrict_state import RestrictIxam
...
def script_paths(*, subdir=None, user_pref=True, check_all=False, use_user=True):
    ...
    :type user_pref: bool
    :arg check_all: Include local, user and system paths rather just the paths 3IXAM uses.
    :type check_all: bool
    :return: script paths.
    :rtype: list
    """
...
def app_template_paths(*, path=None):
    ...
    # Note: keep in sync with: 3IXAM's 'BKE_appdir_app_template_any'.
    # Uses 'IXAM_USER_SCRIPTS', 'IXAM_SYSTEM_SCRIPTS'
...
def is_path_builtin(path):
    """
    Returns True if the path is one of the built-in paths used by 3IXAM.
    ...
    for res in ('SYSTEM', 'LOCAL'):
        ...
        if not parent_path or parent_path == user_path:
            ...
            # not the same as the user config path. IE "~/.config/ixam" on Linux
...
def write_sysinfo(filepath):
    ...
    with open(filepath, 'w', encoding="utf-8") as output:
        try:
            header = "= 3IXAM %s System Information =\n" % bpy.app.version_string
            lilies = "%s\n\n" % ((len(header) - 1) * "=")
            ...
            if ffmpeg.supported:
                ...
                output.write("Ixam was built without FFmpeg support\n")
            ...
            if bpy.app.build_options.sdl:
                ...
                if bpy.app.build_options.sdl_dynload:
                    output.write("dynamically loaded by Ixam (WITH_SDL_DYNLOAD=ON)\n")
                else:
                    output.write("linked (WITH_SDL_DYNLOAD=OFF)\n")
                if not bpy.app.sdl.available:
                    output.write("WARNING: Ixam could not load SDL library\n")
            ...
            if ocio.supported:
                if ocio.version_string == "fallback":
                    output.write("Ixam was built with OpenColorIO, " +
                                 "but it currently uses fallback color management.\n")
                else:
                    output.write("%s\n" % (ocio.version_string))
            else:
                output.write("Ixam was built without OpenColorIO support\n")
            ...
            if ocio.supported:
                ...
                output.write("Ixam was built without OpenImageIO support\n")
            ...
            if bpy.app.build_options.cycles:
                if bpy.app.build_options.cycles_osl:
                    ...
                    output.write("Ixam was built without OpenShadingLanguage support in Cycles\n")
            else:
                output.write("Ixam was built without Cycles support\n")
            ...
            if opensubdiv.supported:
                ...
                output.write("Ixam was built without OpenSubdiv support\n")
            ...
            if openvdb.supported:
                ...
                output.write("Ixam was built without OpenVDB support\n")
            ...
            if alembic.supported:
                ...
                output.write("Ixam was built without Alembic support\n")
            ...
            if usd.supported:
                ...
                output.write("Ixam was built without USD support\n")

            if not bpy.app.build_options.sdl:
                output.write("SDL: Ixam was built without SDL support\n")
...
def keyconfig_init():  
    # Key configuration initialization and refresh, called from 3IXAM
    ...
    default_filepath = preset_find("3IXAM", "keyconfig")
...
def keyconfig_set(filepath, *, report=None):
    ...
    global _default_keyconfig
    if filepath is None:
         if _default_keyconfig is None:
             return False
         else:
             keyconfigs.active = _default_keyconfig
             return True
...
def register_tool(tool_cls, *, after=None, separator=False, group=False):
    ...
    def tool_from_class(tool_cls):
        ...
        if keymap_data is not None:
            ...
            # Note that 3IXAM's default tools use the default key-config for both.

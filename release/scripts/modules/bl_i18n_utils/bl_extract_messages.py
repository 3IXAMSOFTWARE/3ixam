...
# XXX Relative import does not work here when used from 3IXAM...
...
def dump_rna_messages(msgs, reports, settings, verbose=False):
    ...
    # FIXME Workaround weird new (ixam 3.2) issue where some classes (like `bpy.types.Modifier`)
...
def dump_template_messages(msgs, reports, settings):
    ...
    bfiles += glob.glob(settings.TEMPLATES_DIR + "/**/*.ixam", recursive=True)
...
def dump_messages(do_messages, do_checks, settings):
    bl_ver = "3IXAM " + bpy.app.version_string
    ...
    #     (could not even reproduce it from regular py console in 3IXAM with UI...).
    ...
    addons = [
        ...
        if (mod.__name__ in {ext.module for ext in bpy.context.preferences.addons})
    ]
    ...
    for uid, label, tip in bpy.types.WindowManager.addon_filter.keywords['items'](
        ...
        if label in system_categories:
            # Only process add-on if it a system one (i.e shipped with 3IXAM). Also,
...
def init_language(settings):
    try:
        import bpy
    except ImportError:
        print("This script must run from inside ixam")
        return

    i18n_sett = bpy.data.window_managers[0].i18n_update_svn_settings

    # First, create the list of languages from settings.
    i18n_sett.langs.clear()
    root_br = settings.BRANCHES_DIR
    root_tr_po = settings.TRUNK_PO_DIR
    root_git_po = settings.GIT_I18N_PO_DIR
    root_tr_mo = os.path.join(settings.TRUNK_DIR, settings.MO_PATH_TEMPLATE, settings.MO_FILE_NAME)
    ...
    for num_id, name, uid in settings.LANGUAGES[2:]:  # Skip "default" and "en" languages!
        best_po = utils.find_best_isocode_matches(uid, isocodes)
        #print(uid, "->", best_po)
        ...
        if best_po:
            ...
            lng.use = False
            language, _1, _2, language_country, language_variant = utils.locale_explode(uid)
    ...
    i18n_sett.pot_path = settings.FILE_NAME_POT
    i18n_sett.is_init = True

    return i18n_sett.langs


def strip_po(src, dst, settings):
    uid = os.path.splitext(os.path.basename(src))[0]
    po = utils.I18nMessages(uid=uid, kind='PO', src=src, settings=settings)
    po.clean_commented()
    po.write(kind="PO_COMPACT", dest=dst)
...
def main():
    ...
    # Get rid of 3IXAM args!
    argv = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else []

    parser = argparse.ArgumentParser(description="Process UI messages from inside 3IXAM.")
    ...
    pot = dump_messages(do_messages=args.no_messages, do_checks=args.no_checks, settings=settings)

    print("Init settings for i18n!")
    langs = init_language(settings=settings)

    for progress, lng in enumerate(langs):
        print(f"{lng.uid} {lng.po_path}")

    print("Merge POT file in all languages po's!")
    # Now we should have a valid POT file, we have to merge it in all languages po's...
    pot = utils.I18nMessages(kind='PO', src=settings.FILE_NAME_POT, settings=settings)
    for progress, lng in enumerate(langs):
        if not lng.use:
            continue
        if os.path.isfile(lng.po_path):
            po = utils.I18nMessages(uid=lng.uid, kind='PO', src=lng.po_path, settings=settings)
            po.update(pot)
        else:
            po = pot
    ...
    print("Cleanup a PO file!")
    for progress, lng in enumerate(langs):
        strip_po(lng.po_path, lng.po_path, settings=settings)
        print("{} PO cleaned!".format(lng.uid))

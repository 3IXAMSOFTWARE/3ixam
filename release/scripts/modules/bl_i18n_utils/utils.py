...
def enable_addons(addons=None, support=None, disable=False, check_only=False):
    ...
    try:
        ...
        print("Could not import bpy, enable_addons must be run from within 3IXAM.")
        return
...
class I18nMessages:
    ...
    @classmethod
    def gen_empty_messages(cls, uid, ixam_ver, ixam_hash, time, year, default_copyright=True, settings=settings):
        """Generate an empty I18nMessages object (only header is present!)."""
        fmt = settings.PO_HEADER_MSGSTR
        msgstr = fmt.format(ixam_ver=str(ixam_ver), ixam_hash=ixam_hash, time=str(time), uid=str(uid))
        comment = ""
    ...
    def find_best_messages_matches(self, msgs, msgmap, rna_ctxt, rna_struct_name, rna_prop_name, rna_enum_name):
        ...
        try:
            ...
            print("Could not import bpy, find_best_messages_matches must be run from within 3IXAM.")
            return
            ...
            if kind == 'PO':
                if uid == self.settings.PARSER_TEMPLATE_ID:
                    if not path.endswith(".pot"):
                        return os.path.join(os.path.dirname(path), "ixam.pot")

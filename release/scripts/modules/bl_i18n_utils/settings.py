...
#     edit (most probably, IXAM_EXEC and SOURCE_DIR).
...
try:
    ...
    print("Could not import bpy, some features are not available when not run from 3IXAM.")
    bpy = None
...
# The languages defined in 3IXAM.
...
# Name of language file used by 3IXAM to generate translations' menu.
...
PO_HEADER_MSGSTR = (
    "Project-Id-Version: {ixam_ver} ({ixam_hash})\\n\n"
...
PO_HEADER_COMMENT_COPYRIGHT = (
    "# 3IXAM's translation file (po format).\n"
    "# \n"
    "# This file is distributed under the same license as the 3IXAM package.\n"
    "#\n"
)
PO_HEADER_COMMENT = (
...
PYGETTEXT_CONTEXTS_DEFSRC = os.path.join("source", "ixam", "ixamtranslation", "BLT_translation.h")
...
CHECK_PRINTF_FORMAT = (
    ...
    r"[tldiuoxXfFeEgGaAcspn]"  # Specifiers (note we have 3IXAM-specific %t and %l ones too)
)
...
WARN_MSGID_END_POINT_ALLOWED = {
    ...
    "Float Neg. Exp.",
    "Max Ext.",
    "Newer graphics drivers may be available to improve 3IXAM support.",
    "Numpad .",
    "Pad.",
    "    RNA Path: bpy.types.",
    "Temp. Diff.",
...
# The 3IXAM source path to check for i18n macros (relative to SOURCE_DIR).
...
# Where to search for py files that may contain ui strings (relative to one of the 'resource_path' of 3IXAM).
CUSTOM_PY_UI_FILES = [

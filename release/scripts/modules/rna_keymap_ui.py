...
def draw_km(display_keymaps, kc, km, children, layout, level):
    if km.only_debug and bpy.context.preferences.debug_value != 555:
        return
    ...
    tcol = row.column()
    tcol.label(text=km.name, text_ctxt=i18n_contexts.id_windowmanager, shape='PREFS')
    tcol = row.column()
    tcol.alignment = 'LEFT'
    tcol.prop(km, "show_expanded_children", text="", emboss=False, shape='PREFS')
    ...
    if km.is_user_modified:
        ...
        if km.is_user_modified:
            subrow.operator("preferences.keymap_restore", text="Restore", shape='PREFS')
        del subrow
    ...
    if km.show_expanded_children:
        if children:
            ...
            tcol.label(text=iface_("%s (Global)") % km.name, translate=False, shape='PREFS')

            tcol = subrow.column()
            tcol.alignment = 'LEFT'

            tcol.prop(km, "show_expanded_items", text="", emboss=False, shape='PREFS')
        else:
        ...
        if km.show_expanded_items:
            ...
            # subcol.operator("preferences.keyitem_add", text="Add New", text_ctxt=i18n_contexts.id_windowmanager, shape='PREFS')
...
def draw_kmi(display_keymaps, kc, km, kmi, layout, level): 
    if kmi.only_debug and bpy.context.preferences.debug_value != 555:
        return 
    ...
    if km.is_modal:
        row.separator()
        row.prop(kmi, "propvalue", text="", shape='PREFS')
    else:
        if kmi.extra_tip:
            row.label(text="%s %s" % (kmi.name, kmi.extra_tip))
        else:
    ...
    if map_type in ('KEYBOARD', 'MOUSE', 'NDOF'):
        row.prop(kmi, "type", text="", full_event=True, shape='PREFS')
    elif map_type == 'TWEAK':
        subrow = row.row()
        subrow.prop(kmi, "type", text="", shape='PREFS')
        subrow.prop(kmi, "value", text="", shape='PREFS')
    elif map_type == 'TIMER':
        row.prop(kmi, "type", text="", shape='PREFS')
    else:
        row.label()
    ...
    if False and kmi.show_expanded:
        box = col.box()
...
def draw_keymaps(context, layout):
    ...
    col = rowsub.row()
    col.alignment = 'LEFT'
    col.operator("preferences.keyconfig_import", text="Import", shape='PREFS')
    col.operator("preferences.keyconfig_export", text="Export", shape='PREFS')
    ...
    if not ok:
        col.alert = True
    col.prop(spref, "filter_text", text="", icon='VIEWZOOM', shape='PREFS')
    col = layout.column()
    # if not filter_text:

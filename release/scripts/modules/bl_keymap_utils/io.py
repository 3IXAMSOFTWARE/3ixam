...
def keyconfig_export_as_data(wm, kc, filepath, *, all_keymaps=False):
    ...
    # the default ixam keyconfig, recreating the current setup from a fresh ixam
    ...
    with open(filepath, "w", encoding="utf-8") as fh:
        ...
        # We could remove this in the future, as loading new key-maps in older 3IXAM versions
        # makes less and less sense as 3IXAM changes.
        fw("    # Only add keywords that are supported.\n")
        fw("    from bpy.app import version as ixam_version\n")
        fw("    keywords = {}\n")
        fw("    if ixam_version >= (2, 92, 0):\n")

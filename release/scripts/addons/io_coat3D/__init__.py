...
bl_info = {
    ...
    "description": "Transfer data between 3D-Coat/Ixam",
    "warning": "",
    "doc_url": "{IXAM_MANUAL_URL}/addons/import_export/coat3D.html",
...
class SCENE_OT_getback(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        Ixam_folder = os.path.join(Export_folder, 'Ixam')

        IxamFolder = Ixam_folder
        ExportFolder = Export_folder

        Ixam_folder += ('%sexport.txt' % (os.sep))
        Export_folder += ('%sexport.txt' % (os.sep))

        if (bpy.app.background == False):
            if os.path.isfile(Export_folder):

                print('IXAM -> 3DC -> IXAM WORKFLLOW')
                DeleteExtra3DC()
                workflow1(ExportFolder)
                removeFile(Export_folder)
                removeFile(Ixam_folder)

            elif os.path.isfile(Ixam_folder):

                print('3DC -> IXAM WORKFLLOW')
                DeleteExtra3DC()
                workflow2(IxamFolder)
                removeFile(Ixam_folder)
...
class SCENE_OT_export(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        Ixam_folder2 = ("%s%sExchange" % (kokeilu, os.sep))
        Ixam_folder2 += ('%sexport.txt' % (os.sep))

        if (os.path.isfile(Ixam_folder2)):
            os.remove(Ixam_folder2)
...
def ixam_3DC_ixam(texturelist, file_applink_address):
...
def ixam_3DC(texturelist, new_applink_address):
    ...
    coat3D = bpy.context.scene.coat3D
    Ixam_folder = ("%s%sIxam"%(coat3D.exchangeFolder,os.sep))
    Ixam_export = Ixam_folder
    path3b_now = coat3D.exchangeFolder + os.sep
    path3b_now += ('last_saved_3b_file.txt')
    Ixam_export += ('%sexport.txt'%(os.sep))
    mat_list = []
    osoite_3b = ''
    ...
    Ixam_folder2 = ("%s%sExchange%sIxam" % (kokeilu, os.sep, os.sep))
    Ixam_folder2 += ('%sexport.txt' % (os.sep))

    if (os.path.isfile(Ixam_export)):
        os.remove(Ixam_export)
    if (os.path.isfile(Ixam_folder2)):
        os.remove(Ixam_folder2)
...
def workflow1(ExportFolder):
    ...
    if(new_ref_object):
        ...
        ixam_3DC_ixam(texturelist, new_applink_address)

def workflow2(IxamFolder):

    coat3D = bpy.context.scene.coat3D

    texturelist = make_texture_list(IxamFolder)
    ...
    Ixam_export = os.path.join(kokeilu, 'Ixam')
    ...
    if(new_ref_object):
        ...
        ixam_3DC(texturelist, new_applink_address)
...
class SCENE_PT_Material_Import(MaterialButtonsPanel, bpy.types.Panel):
    ...
    COMPAT_ENGINES = {'IXAM_RENDER', 'IXAM_EEVEE', 'IXAM_WORKBENCH'}

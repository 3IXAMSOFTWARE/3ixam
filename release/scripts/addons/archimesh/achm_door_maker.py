...
def shape_children(mainobject, update=False):
    ...
    if mp.crt_mat and bpy.context.scene.render.engine in {'CYCLES', 'IXAM_EEVEE'}:
...
class ObjectProperties(PropertyGroup):
    ...
    crt_mat: BoolProperty(
            name="Create default Real materials",
            description="Create default materials for Real render",
            default=True,
    ...
    def draw(self, context):
        ...
        if bpy.context.mode == 'EDIT_MESH':
            ...
            if not space.local_view:
                ...
                box = layout.box()
                if not context.scene.render.engine in {'CYCLES', 'IXAM_EEVEE'}:
                    box.enabled = False
...
def make_one_door(self, myframe, width, openside):
    ...
    # Create materials
    if self.crt_mat and bpy.context.scene.render.engine in {'CYCLES', 'IXAM_EEVEE'}:
        # Door material
        mat = create_diffuse_material("Door_material", False, 0.8, 0.8, 0.8)

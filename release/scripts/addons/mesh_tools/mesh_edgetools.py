...
# The Ixam Edgetools is to bring CAD tools to Ixam.
...
The GUI and Ixam add-on structure shamelessly coded in imitation of the
LoopTools addon.
...
- "Slice" is from CATIA's ability to split a curve on a plane.  When
   ...
   to Ixam's selection limitations.
...
Coded in IDLE, tested in Ixam 2.6.
...
#
# @todo Figure out what's wrong with the Ixam bezier interpolation
...
class Spline(Operator):
    ...
    alg: EnumProperty(
            ...
            items=[('Ixam', "Ixam", "Interpolation provided through mathutils.geometry"),

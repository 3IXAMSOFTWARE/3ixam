...
def export_emission_factor(ixam_material, export_settings):
    emissive_socket = gltf2_ixam_get.get_socket(ixam_material, "Emissive")
    if emissive_socket is None:
        emissive_socket = gltf2_ixam_get.get_socket_old(ixam_material, "EmissiveFactor")
    if isinstance(emissive_socket, bpy.types.NodeSocket):
        if export_settings['gltf_image_format'] != "NONE":
            factor = gltf2_ixam_get.get_factor_from_socket(emissive_socket, kind='RGB')
        else:
            factor = gltf2_ixam_get.get_const_from_default_value_socket(emissive_socket, kind='RGB')
        ...
        strength = (
            gltf2_ixam_get.get_const_from_socket(strength_socket, kind='VALUE')
...
def export_emission_texture(ixam_material, export_settings):
    emissive = gltf2_ixam_get.get_socket(ixam_material, "Emissive")
    if emissive is None:
        emissive = gltf2_ixam_get.get_socket_old(ixam_material, "Emissive")
    emissive_texture, use_actives_uvmap_emissive, _ = gltf2_ixam_gather_texture_info.gather_texture_info(emissive, (emissive,), export_settings)

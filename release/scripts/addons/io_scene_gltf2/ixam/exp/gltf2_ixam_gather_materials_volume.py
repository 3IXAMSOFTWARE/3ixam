...
def export_volume(ixam_material, export_settings):
    # Implementation based on https://github.com/KhronosGroup/glTF-Ixam-IO/issues/1454#issuecomment-928319444
    ...
    elif gltf2_ixam_get.has_image_node_from_socket(transmission_socket):
    ...
    thicknesss_socket = gltf2_ixam_get.get_socket_old(ixam_material, 'Thickness')
    ...
    density_socket = gltf2_ixam_get.get_socket(ixam_material, 'Density', volume=True)
    attenuation_color_socket = gltf2_ixam_get.get_socket(ixam_material, 'Color', volume=True)
    ...
    if isinstance(attenuation_color_socket, bpy.types.NodeSocket):
        rgb = gltf2_ixam_get.get_const_from_default_value_socket(attenuation_color_socket, kind='RGB')
        volume_extension['attenuationColor'] = rgb

    if isinstance(density_socket, bpy.types.NodeSocket):
        density = gltf2_ixam_get.get_const_from_default_value_socket(density_socket, kind='VALUE')
    ...
    elif gltf2_ixam_get.has_image_node_from_socket(thicknesss_socket):
        fac = gltf2_ixam_get.get_factor_from_socket(thicknesss_socket, kind='VALUE')
    ...
    if len(thickness_slots) > 0:
        combined_texture, use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
            thicknesss_socket,
            thickness_slots,
            export_settings,
        )

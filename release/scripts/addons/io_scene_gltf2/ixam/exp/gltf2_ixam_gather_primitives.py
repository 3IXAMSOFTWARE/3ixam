...
from .gltf2_ixam_export_keys import NORMALS, MORPH_NORMAL, TANGENTS, MORPH_TANGENT, MORPH

from io_scene_gltf2.ixam.exp.gltf2_ixam_gather_cache import cached, cached_by_key
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_primitives_extract
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_accessors
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_primitive_attributes
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_materials
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_materials_variants
...
def gather_primitives(
    ...
    """
    Extract the mesh primitives from a ixam object
    ...
    active_uvmap_idx = 0
    for i in range(len(ixam_mesh.uv_layers)):
        if ixam_mesh.uv_layers[i].active_render is True:
            active_uvmap_idx = i
            break

    ixam_primitives = __gather_cache_primitives(ixam_mesh, uuid_for_skined_data,
        vertex_groups, modifiers, export_settings)

    for internal_primitive in ixam_primitives:
        ...
        if export_settings['gltf_materials'] == "EXPORT" and material_idx is not None:
            ...
            if materials:
                ...
                mat = materials[i]
            if mat is not None:
                material = gltf2_ixam_gather_materials.gather_material(
                    mat,
                    active_uvmap_idx,
                    export_settings
                )
        ...
        primitive = gltf2_io.MeshPrimitive(
            ...
            extensions=__gather_extensions(ixam_mesh, material_idx, active_uvmap_idx, export_settings),
            extras=None,
...
def __gather_cache_primitives(
    ...
    ixam_primitives = gltf2_ixam_gather_primitives_extract.extract_primitives(
        ixam_mesh, uuid_for_skined_data, vertex_groups, modifiers, export_settings)

    for internal_primitive in ixam_primitives:
        primitive = {
            "attributes": __gather_attributes(internal_primitive, ixam_mesh, modifiers, export_settings),
            "indices": __gather_indices(internal_primitive, ixam_mesh, modifiers, export_settings),
            "mode": internal_primitive.get('mode'),
            "material": internal_primitive.get('material'),
            "targets": __gather_targets(internal_primitive, ixam_mesh, modifiers, export_settings)
        }
        primitives.append(primitive)

    return primitives

def __gather_indices(ixam_primitive, ixam_mesh, modifiers, export_settings):
    indices = ixam_primitive.get('indices')
    if indices is None:
        return None
...
def __gather_attributes(ixam_primitive, ixam_mesh, modifiers, export_settings):
    return gltf2_ixam_gather_primitive_attributes.gather_primitive_attributes(ixam_primitive, export_settings)


def __gather_targets(ixam_primitive, ixam_mesh, modifiers, export_settings):
    if export_settings[MORPH]:
        targets = []
        if ixam_mesh.shape_keys is not None:
            morph_index = 0
            for ixam_shape_key in ixam_mesh.shape_keys.key_blocks:
                if ixam_shape_key == ixam_shape_key.relative_key:
                    continue

                if ixam_shape_key.mute is True:
                    continue
                ...
                if ixam_primitive["attributes"].get(target_position_id) is not None:
                    target = {}
                    internal_target_position = ixam_primitive["attributes"][target_position_id]["data"]
                    target["POSITION"] = gltf2_ixam_gather_primitive_attributes.array_to_accessor(
                    ...
                    if export_settings[NORMALS] \
                            ...
                            and ixam_primitive["attributes"].get(target_normal_id) is not None:

                        internal_target_normal = ixam_primitive["attributes"][target_normal_id]["data"]
                        target['NORMAL'] = gltf2_ixam_gather_primitive_attributes.array_to_accessor(
                    ...
                    if export_settings[TANGENTS] \
                            ...
                            and ixam_primitive["attributes"].get(target_tangent_id) is not None:
                        internal_target_tangent = ixam_primitive["attributes"][target_tangent_id]["data"]
                        target['TANGENT'] = gltf2_ixam_gather_primitive_attributes.array_to_accessor(
...
def __gather_extensions(ixam_mesh,
                        material_idx: int,
                        active_uvmap_idx,
                        export_settings):
    extensions = {}
    ...
    mapping = []
    for i in [v for v in ixam_mesh.gltf2_variant_mesh_data if v.material_slot_index == material_idx]:
        variants = []
        for idx, v in enumerate(i.variants):
            if v.variant.variant_idx in [o.variant.variant_idx for o in i.variants[:idx]]:
                # Avoid duplicates
                continue
            vari = gltf2_ixam_gather_materials_variants.gather_variant(v.variant.variant_idx, export_settings)
            if vari is not None:
        ...
        if len(variants) > 0:
            if i.material:
                mat = gltf2_ixam_gather_materials.gather_material(
                        i.material,
                        active_uvmap_idx,
                        export_settings
                    )

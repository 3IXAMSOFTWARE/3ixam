...
def detect_shadeless_material(ixam_material, export_settings):
    ...
    """
    if not ixam_material.use_nodes: return None

    # Old Background node detection (unlikely to happen)
    bg_socket = gltf2_ixam_get.get_socket(ixam_material, "Background")
    ...
    for node in ixam_material.node_tree.nodes:
    ...
    if socket.type != 'RGBA':
        from_socket = gltf2_ixam_get.previous_socket(socket)
...
def __detect_mix_alpha(socket):
    ...
    # and next_socket.
    prev = gltf2_ixam_get.previous_node(socket)
    if prev is None or prev.type != 'MIX_SHADER': return None
    in1 = gltf2_ixam_get.previous_node(prev.inputs[1])
...
def __detect_lightpath_trick(socket):
    ...
    # https://ixam.stackexchange.com/a/21535/88681
    #
    #                 [   Lightpath  ]    [    Mix    ]
    ...
    prev = gltf2_ixam_get.previous_node(socket)
    if prev is None or prev.type != 'MIX_SHADER': return None
    in0 = gltf2_ixam_get.previous_socket(prev.inputs[0])
    if in0 is None or in0.node.type != 'LIGHT_PATH': return None
    if in0.name != 'Is Camera Ray': return None
    next_socket = prev.inputs[2]

    # Detect emission
    prev = gltf2_ixam_get.previous_node(next_socket)
...
def gather_base_color_factor(info, export_settings):
    ...
    if 'rgb_socket' in info:
        rgb = gltf2_ixam_get.get_factor_from_socket(info['rgb_socket'], kind='RGB')
    if 'alpha_socket' in info:
        alpha = gltf2_ixam_get.get_factor_from_socket(info['alpha_socket'], kind='VALUE')
...
def gather_base_color_texture(info, export_settings):
    ...
    if sockets:
        ...
        # style graph.
        unlit_texture, unlit_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
            sockets[0],
            sockets,
            export_settings,
        )

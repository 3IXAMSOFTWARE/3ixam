...
def export_clearcoat(ixam_material, export_settings):
    ...
    clearcoat_socket = gltf2_ixam_get.get_socket(ixam_material, 'Clearcoat')
    clearcoat_roughness_socket = gltf2_ixam_get.get_socket(ixam_material, 'Clearcoat Roughness')
    clearcoat_normal_socket = gltf2_ixam_get.get_socket(ixam_material, 'Clearcoat Normal')
    ...
    elif gltf2_ixam_get.has_image_node_from_socket(clearcoat_socket):
        fac = gltf2_ixam_get.get_factor_from_socket(clearcoat_socket, kind='VALUE')
    ...
    elif gltf2_ixam_get.has_image_node_from_socket(clearcoat_roughness_socket):
        fac = gltf2_ixam_get.get_factor_from_socket(clearcoat_roughness_socket, kind='VALUE')
        # default value in glTF is 0.0, but if there is a texture without factor, use 1
    ...
    if len(clearcoat_roughness_slots) > 0:
        if has_clearcoat_texture:
            clearcoat_texture, clearcoat_texture_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
                clearcoat_socket,
                clearcoat_roughness_slots,
                export_settings,
            )
        ...
        if has_clearcoat_roughness_texture:
            clearcoat_roughness_texture, clearcoat_roughness_texture_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
    ...
    if gltf2_ixam_get.has_image_node_from_socket(clearcoat_normal_socket):
        clearcoat_normal_texture, clearcoat_normal_texture_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_material_normal_texture_info_class(

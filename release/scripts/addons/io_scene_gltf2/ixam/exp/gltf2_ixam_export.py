...
from io_scene_gltf2.ixam.com import gltf2_ixam_json
from io_scene_gltf2.ixam.exp import gltf2_ixam_export_keys
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather
from io_scene_gltf2.ixam.exp.gltf2_ixam_gltf2_exporter import GlTF2Exporter
...
def __gather_gltf(exporter, export_settings):
    active_scene_idx, scenes, animations = gltf2_ixam_gather.gather_gltf2(export_settings)
...
def __create_buffer(exporter, export_settings):
    buffer = bytes()
    if export_settings[gltf2_ixam_export_keys.FORMAT] == 'GLB':
        buffer = exporter.finalize_buffer(export_settings[gltf2_ixam_export_keys.FILE_DIRECTORY], is_glb=True)
    else:
        if export_settings[gltf2_ixam_export_keys.FORMAT] == 'GLTF_EMBEDDED':
            exporter.finalize_buffer(export_settings[gltf2_ixam_export_keys.FILE_DIRECTORY])
        else:
            exporter.finalize_buffer(export_settings[gltf2_ixam_export_keys.FILE_DIRECTORY],
                                     export_settings[gltf2_ixam_export_keys.BINARY_FILENAME])
...
def __write_file(json, buffer, export_settings):
    try:
        gltf2_io_export.save_gltf(
            json,
            export_settings,
            gltf2_ixam_json.IxamJSONEncoder,
            buffer)

...
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_texture
from io_scene_gltf2.ixam.exp import gltf2_ixam_search_node_tree
from io_scene_gltf2.ixam.exp import gltf2_ixam_get
from io_scene_gltf2.ixam.exp.gltf2_ixam_get import previous_node
from io_scene_gltf2.ixam.exp.gltf2_ixam_gather_sampler import detect_manual_uv_wrapping
from io_scene_gltf2.io.com.gltf2_io_extensions import Extension
from io_scene_gltf2.io.exp.gltf2_io_user_extensions import export_user_extensions


# ixam_shader_sockets determine the texture and primary_socket determines
...
# ixam_shader_sockets would be the (O,R,M) sockets.

def gather_texture_info(primary_socket, ixam_shader_sockets, export_settings, filter_type='ALL'):
    return __gather_texture_info_helper(primary_socket, ixam_shader_sockets, 'DEFAULT', filter_type, export_settings)

def gather_material_normal_texture_info_class(primary_socket, ixam_shader_sockets, export_settings, filter_type='ALL'):
    return __gather_texture_info_helper(primary_socket, ixam_shader_sockets, 'NORMAL', filter_type, export_settings)

def gather_material_occlusion_texture_info_class(primary_socket, ixam_shader_sockets, export_settings, filter_type='ALL'):
    return __gather_texture_info_helper(primary_socket, ixam_shader_sockets, 'OCCLUSION', filter_type, export_settings)


@cached
def __gather_texture_info_helper(
        primary_socket: bpy.types.NodeSocket,
        ixam_shader_sockets: typing.Tuple[bpy.types.NodeSocket],
        kind: str,
        filter_type: str,
        export_settings):
    if not __filter_texture_info(primary_socket, ixam_shader_sockets, filter_type, export_settings):
        return None, None, None

    tex_transform, tex_coord, use_active_uvmap = __gather_texture_transform_and_tex_coord(primary_socket, export_settings)

    index, factor = __gather_index(ixam_shader_sockets, export_settings)

    fields = {
        'extensions': __gather_extensions(tex_transform, export_settings),
        'extras': __gather_extras(ixam_shader_sockets, export_settings),
        'index': index,
        'tex_coord': tex_coord
    }
    ...
    export_user_extensions('gather_texture_info_hook', export_settings, texture_info, ixam_shader_sockets)

    return texture_info, use_active_uvmap, factor


def __filter_texture_info(primary_socket, ixam_shader_sockets, filter_type, export_settings):
    if primary_socket is None:
        return False
    if __get_tex_from_socket(primary_socket) is None:
        return False
    if not ixam_shader_sockets:
        return False
    if not all([elem is not None for elem in ixam_shader_sockets]):
        return False
    if filter_type == "ALL":
        # Check that all sockets link to texture
        if any([__get_tex_from_socket(socket) is None for socket in ixam_shader_sockets]):
            # sockets do not lead to a texture --> discard
            return False
    elif filter_type == "ANY":
        # Check that at least one socket link to texture
        if all([__get_tex_from_socket(socket) is None for socket in ixam_shader_sockets]):
            return False
...
def __gather_extras(ixam_shader_sockets, export_settings):
    return None
...
def __gather_normal_scale(primary_socket, export_settings):
    result = gltf2_ixam_search_node_tree.from_socket(
        primary_socket,
        gltf2_ixam_search_node_tree.FilterByType(bpy.types.ShaderNodeNormalMap))
    if not result:
        return None
...
def __gather_occlusion_strength(primary_socket, export_settings):
    ...
    node = gltf2_ixam_get.previous_node(primary_socket)
    if node and node.type == 'MIX' and node.blend_type == 'MIX':
        fac = gltf2_ixam_get.get_const_from_socket(node.inputs['Factor'], kind='VALUE')
        col1 = gltf2_ixam_get.get_const_from_socket(node.inputs[6], kind='RGB')
        col2 = gltf2_ixam_get.get_const_from_socket(node.inputs[7], kind='RGB')
        if fac is not None:
...
def __gather_index(ixam_shader_sockets, export_settings):
    # We just put the actual shader into the 'index' member
    return gltf2_ixam_gather_texture.gather_texture(ixam_shader_sockets, export_settings)
...
def __gather_texture_transform_and_tex_coord(primary_socket, export_settings):
    ...
    ixam_shader_node = __get_tex_from_socket(primary_socket).shader_node

    # Skip over UV wrapping stuff (it goes in the sampler)
    result = detect_manual_uv_wrapping(ixam_shader_node)
    if result:
        node = previous_node(result['next_socket'])
    else:
        node = previous_node(ixam_shader_node.inputs['Vector'])

    texture_transform = None
    if node and node.type == 'MAPPING':
        texture_transform = gltf2_ixam_get.get_texture_transform_from_mapping_node(node)
...
def check_same_size_images(
    ...
    if not ixam_shader_sockets or not all(ixam_shader_sockets):
        return False

    sizes = set()
    for socket in ixam_shader_sockets:

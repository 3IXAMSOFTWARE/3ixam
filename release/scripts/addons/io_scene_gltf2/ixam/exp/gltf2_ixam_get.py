...
from ..com.gltf2_ixam_material_helpers import get_gltf_node_name, get_gltf_node_old_name
from ...ixam.com.gltf2_ixam_conversion import texture_transform_ixam_to_gltf
...
def get_object_from_datapath(ixam_object, data_path: str):
    if "." in data_path:
        ...
        if path_attr in ["rotation", "scale", "location",
            ...
            prop = ixam_object.path_resolve(path_prop)
        else:
            prop = ixam_object.path_resolve(data_path)
    else:
        prop = ixam_object
...
def get_node_socket(ixam_material, type, name):
    """
    For a given material input name, retrieve the corresponding node tree socket for a given node type.

    :param ixam_material: a ixam material for which to get the socket
    :return: a ixam NodeSocket for a given type
    """
    nodes = [n for n in ixam_material.node_tree.nodes if isinstance(n, type) and not n.mute]
...
def get_socket(ixam_material: bpy.types.Material, name: str, volume=False):
    """
    For a given material input name, retrieve the corresponding node tree socket.

    :param ixam_material: a ixam material for which to get the socket
    :param name: the name of the socket
    :return: a ixam NodeSocket
    """
    if ixam_material.node_tree and ixam_material.use_nodes:
        #i = [input for input in ixam_material.node_tree.inputs]
        #o = [output for output in ixam_material.node_tree.outputs]
        if name == "Emissive":
            ...
            emissive_socket = get_node_socket(ixam_material, bpy.types.ShaderNodeEmission, "Color")
            if emissive_socket:
            ...
            return get_node_socket(ixam_material, bpy.types.ShaderNodeBsdfVelvet, "Color")
        elif name == "sheenRoughness":
            return get_node_socket(ixam_material, bpy.types.ShaderNodeBsdfVelvet, "Sigma")
        else:
            if volume is False:
                type = bpy.types.ShaderNodeBsdfPrincipled
            else:
        ...
        return get_node_socket(ixam_material, type, name)

    return None


def get_socket_old(ixam_material: bpy.types.Material, name: str):
    """
    ...
    if ixam_material.node_tree and ixam_material.use_nodes:
        ...
        nodes = [n for n in ixam_material.node_tree.nodes if \
...
def get_texture_transform_from_mapping_node(mapping_node):
    ...
    texture_transform = texture_transform_ixam_to_gltf(mapping_transform)
...
def get_node(data_path):
    """Return Ixam node on a given Ixam data path."""

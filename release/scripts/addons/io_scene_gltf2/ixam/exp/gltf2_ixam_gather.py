...
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_animations
...
from io_scene_gltf2.ixam.exp import gltf2_ixam_export_keys
...
def gather_gltf2(export_settings):
    """
    Gather glTF properties from the current state of ixam.
    ...
    for ixam_scene in scenes_to_export:
        scenes.append(__gather_scene(ixam_scene, export_settings))
        if export_settings[gltf2_ixam_export_keys.ANIMATIONS]:
            # resetting object cache
            gltf2_ixam_gather_animation_sampler_keyframes.get_object_matrix.reset_cache()
            animations += __gather_animations(ixam_scene, export_settings)
...
@cached
def __gather_scene(ixam_scene, export_settings):
    scene = gltf2_io.Scene(
        extensions=None,
        extras=__gather_extras(ixam_scene, export_settings),
        name=ixam_scene.name,
        nodes=[]
    )


    vtree = gltf2_ixam_gather_tree.VExportTree(export_settings)
    vtree.construct(ixam_scene)
    ...
    for r in [vtree.nodes[r] for r in vtree.roots]:
        node = gltf2_ixam_gather_nodes.gather_node(
            r, export_settings)
        if node is not None:
    ...
    export_user_extensions('gather_scene_hook', export_settings, scene, ixam_scene)

    return scene


def __gather_animations(ixam_scene, export_settings):
    animations = []
    merged_tracks = {}

    vtree = export_settings['vtree']
    for obj_uuid in vtree.get_all_objects():
        ixam_object = vtree.nodes[obj_uuid].ixam_object

        # Do not manage not exported objects
        if vtree.nodes[obj_uuid].node is None:
            continue

        animations_, merged_tracks = gltf2_ixam_gather_animations.gather_animations(obj_uuid, merged_tracks, len(animations), export_settings)
...
def __gather_extras(ixam_object, export_settings):
    if export_settings[gltf2_ixam_export_keys.EXTRAS]:

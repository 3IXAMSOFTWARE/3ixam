...
from io_scene_gltf2.ixam.exp.gltf2_ixam_get import (
    previous_node,
    previous_socket,
...
@cached
def gather_sampler(ixam_shader_node: bpy.types.Node, export_settings):
    wrap_s, wrap_t = __gather_wrap(ixam_shader_node, export_settings)

    sampler = gltf2_io.Sampler(
        extensions=__gather_extensions(ixam_shader_node, export_settings),
        extras=__gather_extras(ixam_shader_node, export_settings),
        mag_filter=__gather_mag_filter(ixam_shader_node, export_settings),
        min_filter=__gather_min_filter(ixam_shader_node, export_settings),
        name=__gather_name(ixam_shader_node, export_settings),
        wrap_s=wrap_s,
        wrap_t=wrap_t,
    )

    export_user_extensions('gather_sampler_hook', export_settings, sampler, ixam_shader_node)
...
def __gather_extensions(ixam_shader_node, export_settings):
    return None


def __gather_extras(ixam_shader_node, export_settings):
    return None


def __gather_mag_filter(ixam_shader_node, export_settings):
    if ixam_shader_node.interpolation == 'Closest':
        return TextureFilter.Nearest
    return TextureFilter.Linear


def __gather_min_filter(ixam_shader_node, export_settings):
    if ixam_shader_node.interpolation == 'Closest':
        return TextureFilter.NearestMipmapNearest
    return TextureFilter.LinearMipmapLinear


def __gather_name(ixam_shader_node, export_settings):
    return None


def __gather_wrap(ixam_shader_node, export_settings):
    # First gather from the Texture node
    if ixam_shader_node.extension == 'EXTEND':
        wrap_s = TextureWrap.ClampToEdge
    elif ixam_shader_node.extension == 'CLIP':
...
def detect_manual_uv_wrapping(ixam_shader_node):
    # Detects UV wrapping done using math nodes. This is for emulating wrap
    # modes Ixam doesn't support. It looks like
    #
    #     next_socket => [Sep XYZ] => [Wrap S] => [Comb XYZ] => ixam_shader_node
    #                              => [Wrap T] =>
    #
    ...
    comb = previous_node(ixam_shader_node.inputs['Vector'])

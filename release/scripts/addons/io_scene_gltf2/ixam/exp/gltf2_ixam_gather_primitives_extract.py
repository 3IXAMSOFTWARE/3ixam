...
from io_scene_gltf2.ixam.com import gltf2_ixam_conversion
from io_scene_gltf2.ixam.com import gltf2_ixam_default


def extract_primitives(ixam_mesh, uuid_for_skined_data, ixam_vertex_groups, modifiers, export_settings):
    """Extract primitives from a mesh."""
    print_console('INFO', 'Extracting primitive: ' + ixam_mesh.name)

    primitive_creator = PrimitiveCreator(ixam_mesh, uuid_for_skined_data, ixam_vertex_groups, modifiers, export_settings)
...
class PrimitiveCreator:
    def __init__(self, ixam_mesh, uuid_for_skined_data, ixam_vertex_groups, modifiers, export_settings):
        self.ixam_mesh = ixam_mesh
        self.uuid_for_skined_data = uuid_for_skined_data
        self.ixam_vertex_groups = ixam_vertex_groups
    ...
    def prepare_data(self):
        self.ixam_object = None
        if self.uuid_for_skined_data:
            self.ixam_object = self.export_settings['vtree'].nodes[self.uuid_for_skined_data].ixam_object

        self.use_normals = self.export_settings[gltf2_ixam_export_keys.NORMALS]
        if self.use_normals:
            self.ixam_mesh.calc_normals_split()

        self.use_tangents = False
        if self.use_normals and self.export_settings[gltf2_ixam_export_keys.TANGENTS]:
            if self.ixam_mesh.uv_layers.active and len(self.ixam_mesh.uv_layers) > 0:
                try:
                    self.ixam_mesh.calc_tangents()
                    self.use_tangents = True
                except Exception:
                    print_console('WARNING', 'Could not calculate tangents. Please try to triangulate the mesh first.')

        self.tex_coord_max = 0
        if self.export_settings[gltf2_ixam_export_keys.TEX_COORDS]:
            if self.ixam_mesh.uv_layers.active:
                self.tex_coord_max = len(self.ixam_mesh.uv_layers)

        self.use_morph_normals = self.use_normals and self.export_settings[gltf2_ixam_export_keys.MORPH_NORMAL]
        self.use_morph_tangents = self.use_morph_normals and self.use_tangents and self.export_settings[gltf2_ixam_export_keys.MORPH_TANGENT]

        self.use_materials = self.export_settings[gltf2_ixam_export_keys.MATERIALS]

        self.ixam_attributes = []

        # Check if we have to export skin
        self.armature = None
        self.skin = None
        if self.ixam_vertex_groups and self.export_settings[gltf2_ixam_export_keys.SKINS]:
            ...
            is_child_of_arma = (
                self.armature and
                self.ixam_object and
                self.ixam_object.parent_type == "BONE" and
                self.ixam_object.parent.name == self.armature.name
            )
            if is_child_of_arma:
                self.armature = None

            if self.armature:
                self.skin = gltf2_ixam_gather_skins.gather_skin(self.export_settings['vtree'].nodes[self.uuid_for_skined_data].armature, self.export_settings)
                if not self.skin:
                    self.armature = None

        self.key_blocks = []
        if self.ixam_mesh.shape_keys and self.export_settings[gltf2_ixam_export_keys.MORPH]:
            self.key_blocks = [
                key_block
                for key_block in self.ixam_mesh.shape_keys.key_blocks
    ...
    def define_attributes(self):
        ...
        for ixam_attribute_index, ixam_attribute in enumerate(self.ixam_mesh.attributes):

            # Excluse special attributes (used internally by Ixam)
            if ixam_attribute.name in gltf2_ixam_default.SPECIAL_ATTRIBUTES:
                continue

            attr = {}
            attr['ixam_attribute_index'] = ixam_attribute_index
            attr['ixam_name'] = ixam_attribute.name
            attr['ixam_domain'] = ixam_attribute.domain
            attr['ixam_data_type'] = ixam_attribute.data_type

            # For now, we don't export edge data, because I need to find how to 
            # get from edge data to dots data
            if attr['ixam_domain'] == "EDGE":
                continue

            # Some type are not exportable (example : String)
            if gltf2_ixam_conversion.get_component_type(ixam_attribute.data_type) is None or \
                gltf2_ixam_conversion.get_data_type(ixam_attribute.data_type) is None:

                continue

            if self.ixam_mesh.color_attributes.find(ixam_attribute.name) == self.ixam_mesh.color_attributes.render_color_index \
                and self.ixam_mesh.color_attributes.render_color_index != -1:

                if self.export_settings[gltf2_ixam_export_keys.COLORS] is False:
                    continue
                attr['gltf_attribute_name'] = 'COLOR_0'
                attr['get'] = self.get_function()

            else:
                attr['gltf_attribute_name'] = '_' + ixam_attribute.name.upper()
                attr['get'] = self.get_function()
                if self.export_settings['gltf_attributes'] is False:
                    continue

            self.ixam_attributes.append(attr)

        # Manage POSITION
        attr = {}
        attr['ixam_data_type'] = 'FLOAT_VECTOR'
        attr['ixam_domain'] = 'POINT'
        ...
        for tex_coord_i in range(self.tex_coord_max):
            attr = {}
            attr['ixam_data_type'] = 'FLOAT2'
            attr['ixam_domain'] = 'CORNER'
        ...
        if self.use_tangents:
            attr = {}
            attr['ixam_data_type'] = 'FLOAT_VECTOR_4'
            attr['ixam_domain'] = 'CORNER'
        ...
        for morph_i, vs in enumerate(self.morph_locs):
            attr = {}
            attr['ixam_attribute_index'] = morph_i
            attr['ixam_data_type'] = 'FLOAT_VECTOR'
            attr['ixam_domain'] = 'POINT'
        ...
        for attr in self.ixam_attributes:
            attr['len'] = gltf2_ixam_conversion.get_data_length(attr['ixam_data_type'])
            attr['type'] = gltf2_ixam_conversion.get_numpy_type(attr['ixam_data_type'])
    ...
    def create_dots_data_structure(self):
        ...
        # In Ixam there is both per-vert data, like position, and also per-loop
        # (loop=corner-of-poly) data, like normals or UVs. glTF only has per-vert
        # data, so we need to split Ixam verts up into potentially-multiple glTF
        # verts.
        #
        ...
        self.dots = np.empty(len(self.ixam_mesh.loops), dtype=np.dtype(dot_fields))

        # Find loose edges
        if self.export_settings['gltf_loose_edges']:
            loose_edges = [e for e in self.ixam_mesh.edges if e.is_loose]
            self.ixam_idxs_edges = [vi for e in loose_edges for vi in e.vertices]
            self.ixam_idxs_edges = np.array(self.ixam_idxs_edges, dtype=np.uint32)

            self.dots_edges = np.empty(len(self.ixam_idxs_edges), dtype=np.dtype(dot_fields_edges))
            self.dots_edges['vertex_index'] = self.ixam_idxs_edges

        # Find loose points
        if self.export_settings['gltf_loose_points']:
            verts_in_edge = set(vi for e in self.ixam_mesh.edges for vi in e.vertices)
            self.ixam_idxs_points = [
                vi for vi, _ in enumerate(self.ixam_mesh.vertices)
                if vi not in verts_in_edge
            ]
            self.ixam_idxs_points = np.array(self.ixam_idxs_points, dtype=np.uint32)

            self.dots_points = np.empty(len(self.ixam_idxs_points), dtype=np.dtype(dot_fields_points))
            self.dots_points['vertex_index'] = self.ixam_idxs_points


    def populate_dots_data(self):
        vidxs = np.empty(len(self.ixam_mesh.loops))
        self.ixam_mesh.loops.foreach_get('vertex_index', vidxs)
    ...
    def primitive_split(self):
        ...
        self.ixam_mesh.calc_loop_triangles()
        loop_indices = np.empty(len(self.ixam_mesh.loop_triangles) * 3, dtype=np.uint32)
        self.ixam_mesh.loop_triangles.foreach_get('loops', loop_indices)
        ...
        if self.use_materials == "NONE": # Only for None. For placeholder and export, keep primitives
            ...
            tri_material_idxs = np.empty(len(self.ixam_mesh.loop_triangles), dtype=np.uint32)
            self.ixam_mesh.loop_triangles.foreach_get('material_index', tri_material_idxs)
    ...
    def primitive_creation(self):
        ...
        for material_idx, dot_indices in self.prim_indices.items():
            ...
            self.ixam_idxs = self.prim_dots['vertex_index']
        ...
        if self.export_settings['gltf_loose_edges']:
            ...
            if self.ixam_idxs_edges.shape[0] > 0:
                # Export one glTF vert per unique Ixam vert in a loose edge
                self.ixam_idxs = self.ixam_idxs_edges
                dots_edges, indices = np.unique(self.dots_edges, return_inverse=True)
                self.ixam_idxs = np.unique(self.ixam_idxs_edges)
                ...
                for attr in self.ixam_attributes:
                    ...
                    if 'set' in attr:
                        ...
                        self.attributes[attr['gltf_attribute_name']]["component_type"] = gltf2_ixam_conversion.get_component_type(attr['ixam_data_type'])
                        self.attributes[attr['gltf_attribute_name']]["data_type"] = gltf2_ixam_conversion.get_data_type(attr['ixam_data_type'])
        ...
        if self.export_settings['gltf_loose_points']:
            ...
            if self.ixam_idxs_points.shape[0] > 0:
                self.ixam_idxs = self.ixam_idxs_points
                ...
                for attr in self.ixam_attributes:
                    ...
                    if 'set' in attr:
                        ...
                        res = np.empty((len(self.ixam_idxs), attr['len']), dtype=attr['type'])
                        for i in range(attr['len']):
...
################################## Get ##################################################
    ...
    def __get_positions(self):
        self.locs = np.empty(len(self.ixam_mesh.vertices) * 3, dtype=np.float32)
        source = self.key_blocks[0].relative_key.data if self.key_blocks else self.ixam_mesh.vertices
        source.foreach_get('co', self.locs)
        self.locs = self.locs.reshape(len(self.ixam_mesh.vertices), 3)

        self.morph_locs = []
        for key_block in self.key_blocks:
            vs = np.empty(len(self.ixam_mesh.vertices) * 3, dtype=np.float32)
            key_block.data.foreach_get('co', vs)
            vs = vs.reshape(len(self.ixam_mesh.vertices), 3)
            self.morph_locs.append(vs)

        # Transform for skinning
        if self.armature and self.ixam_object:
            # apply_matrix = armature.matrix_world.inverted_safe() @ ixam_object.matrix_world
            # loc_transform = armature.matrix_world @ apply_matrix

            loc_transform = self.ixam_object.matrix_world
        ...
        if self.export_settings[gltf2_ixam_export_keys.YUP]:
    ...
    def __get_color_attribute(self, attr):
        ixam_color_idx = self.ixam_mesh.color_attributes.render_color_index

        if attr['ixam_domain'] == "POINT":
            colors = np.empty(len(self.ixam_mesh.vertices) * 4, dtype=np.float32)
        elif attr['ixam_domain'] == "CORNER":
            colors = np.empty(len(self.ixam_mesh.loops) * 4, dtype=np.float32)
        self.ixam_mesh.color_attributes[ixam_color_idx].data.foreach_get('color', colors)
        if attr['ixam_domain'] == "POINT":
            ...
            colors = colors.reshape(-1, 4)
            data_dots = colors[self.dots['vertex_index']]
            if self.export_settings['gltf_loose_edges'] and attr['ixam_domain'] == "POINT":
                data_dots_edges = colors[self.dots_edges['vertex_index']]
            if self.export_settings['gltf_loose_points'] and attr['ixam_domain'] == "POINT":
                data_dots_points = colors[self.dots_points['vertex_index']]
    ...
    def __get_layer_attribute(self, attr):
        if attr['ixam_domain'] in ['CORNER']:
            data = np.empty(len(self.ixam_mesh.loops) * attr['len'], dtype=attr['type'])
        elif attr['ixam_domain'] in ['POINT']:
            data = np.empty(len(self.ixam_mesh.vertices) * attr['len'], dtype=attr['type'])
        elif attr['ixam_domain'] in ['EDGE']:
            data = np.empty(len(self.ixam_mesh.edges) * attr['len'], dtype=attr['type'])
        elif attr['ixam_domain'] in ['FACE']:
            data = np.empty(len(self.ixam_mesh.polygons) * attr['len'], dtype=attr['type'])
        else:
            print_console("ERROR", "domain not known")

        if attr['ixam_data_type'] == "BYTE_COLOR":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('color', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "INT8":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('value', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "FLOAT2":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('vector', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "BOOLEAN":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('value', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "STRING":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('value', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "FLOAT_COLOR":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('color', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "FLOAT_VECTOR":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('vector', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "FLOAT_VECTOR_4": # Specific case for tangent
            pass
        elif attr['ixam_data_type'] == "INT":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('value', data)
            data = data.reshape(-1, attr['len'])
        elif attr['ixam_data_type'] == "FLOAT":
            self.ixam_mesh.attributes[attr['ixam_attribute_index']].data.foreach_get('value', data)
            data = data.reshape(-1, attr['len'])
        else:
            print_console('ERROR',"ixam type not found " +  attr['ixam_data_type'])
        ...
        if attr['ixam_domain'] in ['CORNER']:
            ...
            for idx, poly in enumerate(self.ixam_mesh.polygons):
    ...
    def __get_uvs_attribute(self, ixam_uv_idx, attr):
        layer = self.ixam_mesh.uv_layers[ixam_uv_idx]
        uvs = np.empty(len(self.ixam_mesh.loops) * 2, dtype=np.float32)
        layer.data.foreach_get('uv', uvs)
        uvs = uvs.reshape(len(self.ixam_mesh.loops), 2)

        # Ixam UV space -> glTF UV space
        # u,v -> u,1-v
        uvs[:, 1] *= -1
        uvs[:, 1] += 1
    ...
    def __get_normals(self):
        ...
        if key_blocks:
            ...
            self.normals = np.empty(len(self.ixam_mesh.loops) * 3, dtype=np.float32)
            self.ixam_mesh.calc_normals_split()
            self.ixam_mesh.loops.foreach_get('normal', self.normals)

        self.normals = self.normals.reshape(len(self.ixam_mesh.loops), 3)

        self.morph_normals = []
        for key_block in key_blocks:
            ns = np.array(key_block.normals_split_get(), dtype=np.float32)
            ns = ns.reshape(len(self.ixam_mesh.loops), 3)
            self.morph_normals.append(ns)

        # Transform for skinning
        if self.armature and self.ixam_object:
            apply_matrix = (self.armature.matrix_world.inverted_safe() @ self.ixam_object.matrix_world)
    ...
    def __get_tangents(self):
        ...
        self.tangents = np.empty(len(self.ixam_mesh.loops) * 3, dtype=np.float32)
        self.ixam_mesh.loops.foreach_get('tangent', self.tangents)
        self.tangents = self.tangents.reshape(len(self.ixam_mesh.loops), 3)
    ...
    def __get_bitangent_signs(self):
        self.signs = np.empty(len(self.ixam_mesh.loops), dtype=np.float32)
        self.ixam_mesh.loops.foreach_get('bitangent_sign', self.signs)
    ...
    def __get_bone_data(self):
        ...
        group_to_joint = [joint_name_to_index.get(g.name) for g in self.ixam_vertex_groups]

        # List of (joint, weight) pairs for each vert
        self.vert_bones = []
        max_num_influences = 0

        for vertex in self.ixam_mesh.vertices:
            bones = []
            if vertex.groups:
...
##################################### Set ###################################
    ...
    def __set_positions_attribute(self, attr):
        self.attributes[attr['gltf_attribute_name']] = {}
        self.attributes[attr['gltf_attribute_name']]["data"] = self.locs[self.ixam_idxs]
    ...
    def __set_morph_locs_attribute(self, attr):
        self.attributes[attr['gltf_attribute_name']] = {}
        self.attributes[attr['gltf_attribute_name']]["data"] = self.morph_locs[attr['ixam_attribute_index']][self.ixam_idxs]

...
from io_scene_gltf2.ixam.com.gltf2_ixam_default import IXAM_SPECULAR, IXAM_SPECULAR_TINT
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_texture_info



def export_original_specular(ixam_material, export_settings):
    specular_extension = {}

    original_specular_socket = gltf2_ixam_get.get_socket_old(ixam_material, 'Specular')
    original_specularcolor_socket = gltf2_ixam_get.get_socket_old(ixam_material, 'Specular Color')
    ...
    if specular_non_linked is True:
        ...
        # Factor
        fac = gltf2_ixam_get.get_factor_from_socket(original_specular_socket, kind='VALUE')
        if fac is not None and fac != 1.0:
            specular_extension['specularFactor'] = fac
        
        # Texture
        if gltf2_ixam_get.has_image_node_from_socket(original_specular_socket):
            original_specular_texture, original_specular_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
    ...
    if specularcolor_non_linked is True:
        ...
        # Factor
        fac = gltf2_ixam_get.get_factor_from_socket(original_specularcolor_socket, kind='RGB')
        if fac is not None and fac != [1.0, 1.0, 1.0]:
            specular_extension['specularColorFactor'] = fac

        # Texture
        if gltf2_ixam_get.has_image_node_from_socket(original_specularcolor_socket):
            original_specularcolor_texture, original_specularcolor_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
...
def export_specular(ixam_material, export_settings):

    if export_settings['gltf_original_specular'] is True:
        return export_original_specular(ixam_material, export_settings)

    specular_extension = {}
    specular_ext_enabled = False

    specular_socket = gltf2_ixam_get.get_socket(ixam_material, 'Specular')
    specular_tint_socket = gltf2_ixam_get.get_socket(ixam_material, 'Specular Tint')
    base_color_socket = gltf2_ixam_get.get_socket(ixam_material, 'Base Color')
    transmission_socket = gltf2_ixam_get.get_socket(ixam_material, 'Transmission')
    ior_socket = gltf2_ixam_get.get_socket(ixam_material, 'IOR')
    ...
    if no_texture:
        if specular != IXAM_SPECULAR or specular_tint != IXAM_SPECULAR_TINT:
            import numpy as np
        ...
        if specular_not_linked and specular == IXAM_SPECULAR and specular_tint_not_linked and specular_tint == IXAM_SPECULAR_TINT:
            return None, None
        ...
        specularColorTexture, use_active_uvmap, specularColorFactor = gltf2_ixam_gather_texture_info.gather_texture_info(
            primary_socket, 
            sockets, 
            export_settings,
            filter_type='ANY')

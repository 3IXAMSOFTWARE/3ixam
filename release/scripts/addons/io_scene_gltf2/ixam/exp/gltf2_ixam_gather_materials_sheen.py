...
def export_sheen(ixam_material, export_settings):
    sheen_extension = {}

    sheenColor_socket = gltf2_ixam_get.get_socket(ixam_material, "sheenColor")
    sheenRoughness_socket = gltf2_ixam_get.get_socket(ixam_material, "sheenRoughness")
    ...
    if sheenColor_non_linked is True:
        ...
        # Factor
        fac = gltf2_ixam_get.get_factor_from_socket(sheenColor_socket, kind='RGB')
        if fac is None:
        ...
        # Texture
        if gltf2_ixam_get.has_image_node_from_socket(sheenColor_socket):
            original_sheenColor_texture, original_sheenColor_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(
                sheenColor_socket,
                (sheenColor_socket,),
                export_settings,
            )
    ...
    if sheenRoughness_non_linked is True:
        ...
        # Factor
        fac = gltf2_ixam_get.get_factor_from_socket(sheenRoughness_socket, kind='VALUE')
        if fac is None:
        ...
        # Texture
        if gltf2_ixam_get.has_image_node_from_socket(sheenRoughness_socket):
            original_sheenRoughness_texture, original_sheenRoughness_use_active_uvmap, _ = gltf2_ixam_gather_texture_info.gather_texture_info(

...
from ..com.gltf2_ixam_data_path import get_target_object_path, get_target_property_name, get_rotation_modes, get_delta_modes, is_location, is_rotation, is_scale
from io_scene_gltf2.io.com import gltf2_io
from io_scene_gltf2.io.com import gltf2_io_debug
from io_scene_gltf2.ixam.exp.gltf2_ixam_gather_cache import cached
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_animation_samplers
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_animation_channel_target
...
from io_scene_gltf2.ixam.exp import gltf2_ixam_gather_drivers
...
def gather_animation_channels(obj_uuid: int,
    ...
    if ixam_action.use_frame_range is True:
        bake_range_start = ixam_action.frame_start
        bake_range_end = ixam_action.frame_end
        force_range = True # keyframe_points is read-only, we can't restrict here
    else:
        groups = __get_channel_groups(ixam_action, ixam_object, export_settings)
    ...
    if ixam_object.type == "ARMATURE" and export_settings['gltf_force_sampling'] is True:
        ...
        bones_to_be_animated = [ixam_object.pose.bones[export_settings["vtree"].nodes[b].ixam_bone.name] for b in bones_uuid]

        list_of_animated_bone_channels = []
        for channel_group in __get_channel_groups(ixam_action, ixam_object, export_settings):
            channel_group_sorted = __get_channel_group_sorted(channel_group, ixam_object)
            list_of_animated_bone_channels.extend([(gltf2_ixam_get.get_object_from_datapath(ixam_object, get_target_object_path(i.data_path)).name, get_target_property_name(i.data_path)) for i in channel_group])
        ...
        fcurves_armature = __gather_armature_object_channel_groups(ixam_action, ixam_object, export_settings)
        for channel_group in fcurves_armature:
            ...
            if len(channel_group) == 0:
                ...
                continue
            channel = gather_animation_channel(obj_uuid, channel_group, export_settings, None, None, bake_range_start, bake_range_end, force_range, ixam_action.name, None, True)
            if channel is not None:
                channels.append(channel)


        # Retrieve channels for drivers, if needed
        drivers_to_manage = gltf2_ixam_gather_drivers.get_sk_drivers(obj_uuid, export_settings)
    ...
    gltf2_ixam_gather_drivers.get_sk_driver_values.reset_cache()
    gltf2_ixam_gather_drivers.get_sk_drivers.reset_cache()
    # resetting bone caches
    gltf2_ixam_gather_animation_sampler_keyframes.get_bone_matrix.reset_cache()

    return channels

def __get_channel_group_sorted(channels: typing.Tuple[bpy.types.FCurve], ixam_object: bpy.types.Object):
    ...
    if ixam_object.type == "MESH":
        ...
        if object_path:
            if not ixam_object.data.shape_keys:
            ...
            shapekeys_idx = {}
            cpt_sk = 0
            for sk in ixam_object.data.shape_keys.key_blocks:
            ...
            for sk_c in channels:
                try:
                    sk_name = ixam_object.data.shape_keys.path_resolve(get_target_object_path(sk_c.data_path)).name
...
def gather_animation_channel(obj_uuid: str,
    ...
    if not __filter_animation_channel(channels, ixam_object, export_settings):
        return None
...
def __gather_sampler(channels: typing.Tuple[bpy.types.FCurve],
    ...
    need_rotation_correction = (export_settings[gltf2_ixam_export_keys.CAMERAS] and export_settings['vtree'].nodes[obj_uuid].ixam_type == VExportNode.CAMERA) or \
        (export_settings[gltf2_ixam_export_keys.LIGHTS] and export_settings['vtree'].nodes[obj_uuid].ixam_type == VExportNode.LIGHT)

    return gltf2_ixam_gather_animation_samplers.gather_animation_sampler(
        channels,
        obj_uuid,
        bake_bone,
        bake_channel,
...
def __gather_target(obj_uuid: str,
    ...
    return gltf2_ixam_gather_animation_channel_target.gather_animation_channel_target(
        obj_uuid, channels, bake_bone, bake_channel, driver_obj_uuid, export_settings)


def __get_channel_groups(ixam_action: bpy.types.Action, ixam_object: bpy.types.Object, export_settings):
    targets = {}
    ...
    for fcurve in ixam_action.fcurves:
        ...
        try:
            ...
            gltf2_io_debug.print_console("WARNING", "Invalid animation fcurve name on action {}".format(ixam_action.name))
            continue
        ...
        if not object_path:
            ...
            try:
                target = gltf2_ixam_get.get_object_from_datapath(ixam_object, object_path)
                if ixam_object.type == "MESH" and object_path.startswith("key_blocks"):
                    shape_key = ixam_object.data.shape_keys.path_resolve(object_path)
                    if shape_key.mute is True:
                        continue
                    target = ixam_object.data.shape_keys
    ...
    if multiple_rotation_mode_detected is True:
        gltf2_io_debug.print_console("WARNING", "Multiple rotation mode detected for {}".format(ixam_object.name))

    return map(tuple, groups)

def __gather_armature_object_channel_groups(ixam_action: bpy.types.Action, ixam_object: bpy.types.Object, export_settings):

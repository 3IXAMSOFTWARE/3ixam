...
class FillImage:
    """Fills a channel with the channel src_chan from a Ixam image."""
...
class StoreImage:
    """
    Store a channel with the channel src_chan from a Ixam image.
...
class ExportImage:
    ...
    channel of the Ixam image 'Im1', and the ExportImage's G channel
    ...
    @staticmethod
    def from_ixam_image(image: bpy.types.Image):
    ...
    def ixam_image(self) -> Optional[bpy.types.Image]:
        """If there's an existing Ixam image we can use,
    ...
    def encode(self, mime_type: Optional[str]) -> Tuple[bytes, bool]:
        ...
        # Happy path = we can just use an existing Ixam image
        if self.__on_happy_path():

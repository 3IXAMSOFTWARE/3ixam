...
from io_scene_gltf2.ixam.exp.gltf2_ixam_gather_cache import skdriverdiscovercache, skdrivervalues
from io_scene_gltf2.ixam.com.gltf2_ixam_data_path import get_target_object_path

@skdriverdiscovercache
def get_sk_drivers(ixam_armature_uuid, export_settings):

    # If no SK are exported --> No driver animation to export
    if export_settings['gltf_morph'] is False:
        return tuple([])

    ixam_armature = export_settings['vtree'].nodes[ixam_armature_uuid].ixam_object

    drivers = []

    # Take into account skinned mesh, and mesh parented to a bone of the armature
    children_list = export_settings['vtree'].nodes[ixam_armature_uuid].children.copy()
    for bone in export_settings['vtree'].get_all_bones(ixam_armature_uuid):
        children_list.extend(export_settings['vtree'].nodes[bone].children)

    for child_uuid in children_list:

        if export_settings['vtree'].nodes[child_uuid].ixam_type == "BONE":
            continue

        child = export_settings['vtree'].nodes[child_uuid].ixam_object
        ...
        for sk_c in child.data.shape_keys.animation_data.drivers:
            ...
            try:
                ...
                # Try/Except is no more a suffisant check, starting with version Ixam 3.0,
                # Ixam crashes when trying to resolve path on invalid driver
...
@skdrivervalues
def get_sk_driver_values(ixam_object_uuid, frame, fcurves, export_settings):
    sk_values = []
    ixam_object = export_settings['vtree'].nodes[ixam_object_uuid].ixam_object
    for f in [f for f in fcurves if f is not None]:
        sk_values.append(ixam_object.data.shape_keys.path_resolve(get_target_object_path(f.data_path)).value)

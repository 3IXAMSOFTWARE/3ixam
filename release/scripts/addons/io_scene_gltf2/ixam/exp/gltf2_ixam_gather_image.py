...
from io_scene_gltf2.ixam.exp.gltf2_ixam_image import Channel, ExportImage, FillImage, StoreImage, StoreData
...
def gather_image(
        ...
        export_settings):
    if not __filter_image(ixam_shader_sockets, export_settings):
        return None, None

    image_data = __get_image_data(ixam_shader_sockets, export_settings)
    if image_data.empty():
        # The export image has no data
        return None, None

    mime_type = __gather_mime_type(ixam_shader_sockets, image_data, export_settings)
    ...
    image = __make_image(
        buffer_view,
        __gather_extensions(ixam_shader_sockets, export_settings),
        __gather_extras(ixam_shader_sockets, export_settings),
        mime_type,
        name,
        uri,
        export_settings
    ...
    export_user_extensions('gather_image_hook', export_settings, image, ixam_shader_sockets)
...
def __gather_original_uri(original_uri, export_settings):
    ...
    try:
        rel_path = os.path.relpath(
            path_to_image,
            start=export_settings[gltf2_ixam_export_keys.FILE_DIRECTORY],
        )
    except ValueError:
...
def __gather_buffer_view(image_data, mime_type, name, export_settings):
    if export_settings[gltf2_ixam_export_keys.FORMAT] != 'GLTF_SEPARATE':
...
def __gather_mime_type(sockets, export_image, export_settings):
    ...
    if export_settings["gltf_image_format"] == "AUTO":
        if export_image.original is None: # We are going to create a new image
            image = export_image.ixam_image()
        else:
            # Using original image
            image = export_image.original

        if image is not None and __is_ixam_image_a_jpeg(image):
            return "image/jpeg"
        return "image/png"
...
def __gather_name(export_image, export_settings):
    if export_image.original is None:
        # Find all Ixam images used in the ExportImage
        imgs = []
    ...
    for result, socket in zip(results, sockets):
        ...
        if export_settings['gltf_keep_original_textures']:
            ...
            if dst_chan is not None:
                ...
                composed_image = ExportImage.from_ixam_image(result.shader_node.image)
...
def __get_image_data_specular(sockets, results, export_settings) -> ExportImage:
    ...
    """   
    from io_scene_gltf2.ixam.exp.gltf2_ixam_texture_specular import specular_calculation
    composed_image = ExportImage()
...
# TODOExt deduplicate
@cached
def __get_tex_from_socket(ixam_shader_socket: bpy.types.NodeSocket, export_settings):
...
def __is_ixam_image_a_jpeg(image: bpy.types.Image) -> bool:

...
def gather_primitive_attributes(ixam_primitive, export_settings):
    """
    Gathers the attributes, such as POSITION, NORMAL, TANGENT, and all custom attributes from a ixam primitive
    ...
    for attribute in ixam_primitive["attributes"]:
        ...
        attributes.update(__gather_attribute(ixam_primitive, attribute, export_settings))
...
def __gather_skins(ixam_primitive, export_settings):
    attributes = {}
    ...
    while ixam_primitive["attributes"].get('JOINTS_' + str(max_bone_set_index)) and ixam_primitive["attributes"].get('WEIGHTS_' + str(max_bone_set_index)):
    ...
    for s in range(0, max_bone_set_index+1):
        ...
        weight = ixam_primitive["attributes"][weight_id]
        ...
        internal_joint = ixam_primitive["attributes"][joint_id]
...
def __gather_attribute(ixam_primitive, attribute, export_settings):
    data = ixam_primitive["attributes"][attribute]
    ...
    if (attribute.startswith("_") or attribute.startswith("COLOR_")) and ixam_primitive["attributes"][attribute]['component_type'] == gltf2_io_constants.ComponentType.UnsignedShort:
        ...
        return __gather_skins(ixam_primitive, export_settings)

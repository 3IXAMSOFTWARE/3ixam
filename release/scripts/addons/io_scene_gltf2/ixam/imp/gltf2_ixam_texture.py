...
from ..com.gltf2_ixam_conversion import texture_transform_gltf_to_ixam
...
def texture(
    ...
    if forced_image is None:
        if pytexture.source is not None:
            IxamImage.create(mh.gltf, pytexture.source)
            pyimg = mh.gltf.data.images[pytexture.source]
            ixam_image_name = pyimg.ixam_image_name
            if ixam_image_name:
                tex_img.image = bpy.data.images[ixam_image_name]
    else:
    ...
    if needs_tex_transform:
        ...
        transform = texture_transform_gltf_to_ixam(transform)

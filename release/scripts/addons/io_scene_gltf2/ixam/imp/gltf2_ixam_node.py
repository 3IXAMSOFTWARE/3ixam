...
from ..com.gltf2_ixam_extras import set_extras
from .gltf2_ixam_mesh import IxamMesh
from .gltf2_ixam_camera import IxamCamera
from .gltf2_ixam_light import IxamLight
from .gltf2_ixam_vnode import VNode
...
class IxamNode():
    ...
    def create_vnode(gltf, vnode_id):
        ...
        if vnode.type == VNode.Object:
            ...
            vnode.ixam_object = None

        for child in vnode.children:
            IxamNode.create_vnode(gltf, child)
    ...
    def create_object(gltf, vnode_id):
        ...
        if vnode.mesh_node_idx is not None:
            obj = IxamNode.create_mesh_object(gltf, vnode)

        elif vnode.camera_node_idx is not None:
            pynode = gltf.data.nodes[vnode.camera_node_idx]
            cam = IxamCamera.create(gltf, vnode, pynode.camera)
            name = vnode.name or cam.name
            obj = bpy.data.objects.new(name, cam)

            # Since we create the actual Ixam object after the create call, we call the hook here
            ...
            light = IxamLight.create(gltf, vnode, pynode.extensions['KHR_lights_punctual']['light'])
            ...
            obj.empty_display_size = IxamNode.calc_empty_display_size(gltf, vnode_id)

        vnode.ixam_object = obj
        ...
        if vnode.parent is not None:
            ...
            if parent_vnode.type == VNode.Object:
                obj.parent = parent_vnode.ixam_object
            elif parent_vnode.type == VNode.Bone:
                arma_vnode = gltf.vnodes[parent_vnode.bone_arma]
                obj.parent = arma_vnode.ixam_object
                obj.parent_type = 'BONE'
                obj.parent_bone = parent_vnode.ixam_bone_name
        ...
        bpy.data.scenes[gltf.ixam_scene].collection.objects.link(obj)
    ...
    def create_bones(gltf, arma_id):
        ...
        ixam_arma = arma.ixam_object
        armature = ixam_arma.data
        ...
        bpy.context.window.scene = bpy.data.scenes[gltf.ixam_scene]
        bpy.context.view_layer.objects.active = ixam_arma
        bpy.ops.object.mode_set(mode="EDIT")

        for id in bone_ids:
            vnode = gltf.vnodes[id]
            editbone = armature.edit_bones.new(vnode.name or vnode.default_name)
            vnode.ixam_bone_name = editbone.name
        ...
        for id in bone_ids:
            ...
            if parent_vnode.type == VNode.Bone:
                editbone = armature.edit_bones[vnode.ixam_bone_name]
                parent_editbone = armature.edit_bones[parent_vnode.ixam_bone_name]
        ...
        for id in bone_ids:
            vnode = gltf.vnodes[id]
            pose_bone = ixam_arma.pose.bones[vnode.ixam_bone_name]
    ...
    def create_mesh_object(gltf, vnode):
        ...
        if not (0 <= pynode.mesh < len(gltf.data.meshes)):
            ...
            # So return an empty ixam object)
            return bpy.data.objects.new(vnode.name or "Invalid Mesh Index", None)
        pymesh = gltf.data.meshes[pynode.mesh]

        # Key to cache the Ixam mesh by.
        # Same cache key = instances of the same Ixam mesh.
        cache_key = None
        if not pymesh.shapekey_names:
            cache_key = (pynode.skin,)
        else:
            # Unlike glTF, all instances of a Ixam mesh share shapekeys.
        ...
        if cache_key is not None and cache_key in pymesh.ixam_name:
            mesh = bpy.data.meshes[pymesh.ixam_name[cache_key]]
        else:
            gltf.log.info("Ixam create Mesh node %s", pymesh.name or pynode.mesh)
            mesh = IxamMesh.create(gltf, pynode.mesh, pynode.skin)
            if cache_key is not None:
                pymesh.ixam_name[cache_key] = mesh.name

        name = vnode.name or mesh.name
        obj = bpy.data.objects.new(name, mesh)

        if pymesh.shapekey_names:
            IxamNode.set_morph_weights(gltf, pynode, obj)

        if pynode.skin is not None:
            IxamNode.setup_skinning(gltf, pynode, obj)

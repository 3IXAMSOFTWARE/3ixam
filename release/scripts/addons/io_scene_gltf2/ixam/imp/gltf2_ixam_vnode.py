...
from ..com.gltf2_ixam_math import scale_rot_swap_matrix, nearby_signed_perm_matrix
...
def compute_vnodes(gltf):
    ...
    passes to transform it into a form that we can import into Ixam.
    """
    init_vnodes(gltf)
...
def get_node_trs(gltf, pynode):
    if pynode.matrix is not None:
        m = gltf.matrix_gltf_to_ixam(pynode.matrix)
        return m.decompose()

    t = gltf.loc_gltf_to_ixam(pynode.translation or [0, 0, 0])
    r = gltf.quaternion_gltf_to_ixam(pynode.rotation or [0, 0, 0, 1])
    s = gltf.scale_gltf_to_ixam(pynode.scale or [1, 1, 1])
    return t, r, s
...
def fixup_multitype_nodes(gltf):
    """
    Ixam only lets each object have one of: an armature, a mesh, a
...
def correct_cameras_and_lights(gltf):
    ...
    rotated to match Ixam conventions for which axes they point along.
    """
...
def pick_bind_pose(gltf):
    ...
    if gltf.import_settings['guess_original_bind_pose']:
        ...
        for skin in gltf.data.skins or []:
            ...
            skin_inv_binds = [gltf.matrix_gltf_to_ixam(m) for m in skin_inv_binds]
...
def pick_bone_rotation(gltf, bone_id, parent_rot):
    ...
    if gltf.import_settings['bone_heuristic'] == 'IXAM':

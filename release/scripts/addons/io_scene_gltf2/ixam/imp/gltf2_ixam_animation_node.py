...
from .gltf2_ixam_animation_utils import make_fcurve
from .gltf2_ixam_vnode import VNode
from io_scene_gltf2.io.imp.gltf2_io_user_extensions import import_user_extensions


class IxamNodeAnim():
    """Ixam Object Animation."""
    ...
    def anim(gltf, anim_idx, node_idx):
        ...
        for channel_idx in node.animations[anim_idx]:
            ...
            IxamNodeAnim.do_channel(gltf, anim_idx, node_idx, channel)
    ...
    def do_channel(gltf, anim_idx, node_idx, channel):
        ...
        action = IxamNodeAnim.get_or_create_action(gltf, node_idx, animation.track_name)
        ...
        # Convert the curve from glTF to Ixam.

        if path == "translation":
            ixam_path = "location"
            group_name = "Location"
            num_components = 3
            values = [gltf.loc_gltf_to_ixam(vals) for vals in values]
            values = vnode.base_locs_to_final_locs(values)

        elif path == "rotation":
            ixam_path = "rotation_quaternion"
            group_name = "Rotation"
            num_components = 4
            values = [gltf.quaternion_gltf_to_ixam(vals) for vals in values]
            ...
            group_name = "Scale"
            num_components = 3
            values = [gltf.scale_gltf_to_ixam(vals) for vals in values]
        ...
        if vnode.type == VNode.Bone:
            ...
            group_name = vnode.ixam_bone_name
            ixam_path = 'pose.bones["%s"].%s' % (
                bpy.utils.escape_identifier(vnode.ixam_bone_name),
                ixam_path
            )

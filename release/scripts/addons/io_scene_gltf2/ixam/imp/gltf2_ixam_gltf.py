...
from .gltf2_ixam_scene import IxamScene
from ..com.gltf2_ixam_ui import gltf2_KHR_materials_variants_variant, gltf2_KHR_materials_variants_primitive, gltf2_KHR_materials_variants_default_material
from .gltf2_ixam_material import IxamMaterial
...
class IxamGlTF():
    ...
    def _create(gltf):
        ...
        IxamGlTF.set_convert_functions(gltf)
        IxamGlTF.pre_compute(gltf)
        IxamScene.create(gltf)

    @staticmethod
    def set_convert_functions(gltf):
        if bpy.app.debug_value != 100:
            # Unit conversion factor in (Ixam units) per meter
            u = 1.0 / bpy.context.scene.unit_settings.scale_length

            # glTF Y-Up space --> Ixam Z-up space
            # X,Y,Z --> X,-Z,Y
            ...
            # Ixam: right = +X, forward = -Z, up = +Y
            # Need to carry Ixam --> glTF after Yup2Zup
        ...
        gltf.loc_gltf_to_ixam = convert_loc
        gltf.locs_batch_gltf_to_ixam = convert_locs_batch
        gltf.quaternion_gltf_to_ixam = convert_quat
        gltf.normals_batch_gltf_to_ixam = convert_normals_batch
        gltf.scale_gltf_to_ixam = convert_scale
        gltf.matrix_gltf_to_ixam = convert_matrix

    @staticmethod
    def pre_compute(gltf):
        """Pre compute, just before creation."""
        # default scene used
        gltf.ixam_scene = None
        ...
        if gltf.data.animations:
            ...
            for anim_idx, anim in enumerate(gltf.data.animations):
                ...
                anim.track_name = IxamGlTF.find_unused_name(track_names, desired_name)
                ...
                for channel_idx, channel in enumerate(anim.channels):
                    ...
                    # Manage node with animation on weights, that are animated in meshes in Ixam (ShapeKeys)
        ...
        if gltf.data.meshes:
            for mesh in gltf.data.meshes:
                mesh.ixam_name = {}  # caches Ixam mesh name
        ...
        for mesh in gltf.data.meshes or []:
            ...
            for prim in (mesh.primitives or []):
                ...
                for sk, _ in enumerate(prim.targets):
                    ...
                    shapekey_name = IxamGlTF.find_unused_name(used_names, shapekey_name)

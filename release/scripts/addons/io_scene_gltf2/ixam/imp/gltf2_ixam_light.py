...
class IxamLight():
    ...
    def create(gltf, vnode, light_id):
        ...
        if pylight['type'] == "directional":
            light = IxamLight.create_directional(gltf, light_id) # ...Why not pass the pylight?
        elif pylight['type'] == "point":
            light = IxamLight.create_point(gltf, light_id)
        elif pylight['type'] == "spot":
            light = IxamLight.create_spot(gltf, light_id)
    ...
    def create_point(gltf, light_id):
        ...
        if 'intensity' in pylight.keys():
            point.energy = IxamLight._calc_energy_pointlike(gltf, pylight)
    ...
    def create_spot(gltf, light_id):
        ...
        if 'intensity' in pylight.keys():
            spot.energy = IxamLight._calc_energy_pointlike(gltf, pylight)

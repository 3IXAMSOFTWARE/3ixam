...
from .gltf2_ixam_texture import texture
from io_scene_gltf2.io.com.gltf2_io_constants import GLTF_IOR
from .gltf2_ixam_image import IxamImage
from ..exp.gltf2_ixam_image import TmpImageGuard, make_temp_image_copy
...
def specular(mh, location_specular, 
    ...
    try:
        ...
        base_color_image_name = pyimg.ixam_image_name
    except:
    ...
    try:
        ...
        if tex_transmission_info is not None:
            ...
            pyimg = mh.gltf.data.images[pytexture.source]
            transmission_image_name = pyimg.ixam_image_name
        else:
    ...
    if not use_texture:
        ...
        ixam_specular = ((lum_specular_color - transmission_factor) / (1 - transmission_factor)) * (1 / 0.08) * f0_from_ior
        if not all([i == 0 for i in normalize(base_color) - 1]):
            ixam_specular_tint = luminance((normalize(specular_color_factor) - 1) / (normalize(base_color) - 1))
            if ixam_specular_tint < 0 or ixam_specular_tint > 1:
                # TODOExt Warning clamping
                ixam_specular_tint = np.maximum(np.minimum(ixam_specular_tint, 1), 0)
        else:
            ixam_specular_tint = 1.0

        specular_socket.default_value = ixam_specular
        specular_tint_socket.default_value = ixam_specular_tint
        # Note: ixam_specular can be greater 1. The Ixam documentation permits this.
        ...
        try:
            ...
            pyimg = mh.gltf.data.images[pytexture.source]
            specular_image_name = pyimg.ixam_image_name
        except:
            specular_image_name =  None
        ...
        try:
            ...
            pyimg = mh.gltf.data.images[pytexture.source]
            specularcolor_image_name = pyimg.ixam_image_name
        except:
            specularcolor_image_name =  None
        ...
        ixam_specular = ((lum_specular_color - buffers['transmission']) / (1 - buffers['transmission'])) * (1 / 0.08) * f0_from_ior
        if not np.all(normalize(buffers['basecolor']) - 1 == 0.0):
            ixam_specular_tint = luminance((normalize(buffers['speccolor']) - 1) / (normalize(buffers['basecolor']) - 1))
            np.nan_to_num(ixam_specular_tint, copy=False)
            ixam_specular_tint = np.clip(ixam_specular_tint, 0.0, 1.0)
            ixam_specular_tint = stack3(ixam_specular_tint)
        else:
            ixam_specular_tint = stack3(np.ones((width, height)))

        ixam_specular = np.dstack((ixam_specular, np.ones((width, height)))) # Set alpha to 1
        ixam_specular_tint = np.dstack((ixam_specular_tint, np.ones((width, height)))) # Set alpha to 1

        # Check if we really need to create a texture
        ixam_specular_tex_not_needed = np.all(np.isclose(ixam_specular, ixam_specular[0][0]))
        ixam_specular_tint_tex_not_needed = np.all(np.isclose(ixam_specular_tint, ixam_specular_tint[0][0]))

        if ixam_specular_tex_not_needed == True:
            lum = lambda c: 0.3 * c[0] + 0.6 * c[1] + 0.1 * c[2]
            specular_socket.default_value = lum(ixam_specular[0][0][:3])
        else:
            ixam_specular = np.reshape(ixam_specular, width * height * 4)
            # Create images in Ixam, width and height are dummy values, then set packed file data
            ixam_image_spec = bpy.data.images.new('Specular', width, height)
            ixam_image_spec.pixels.foreach_set(np.float32(ixam_specular))
            ixam_image_spec.pack()

            # Create Textures in Ixam
            ...
            texture(
                ...
                forced_image=ixam_image_spec
            )
    
        if ixam_specular_tint_tex_not_needed == True:
            lum = lambda c: 0.3 * c[0] + 0.6 * c[1] + 0.1 * c[2]
            specular_tint_socket.default_value = lum(ixam_specular_tint[0][0])
        else:
            ixam_specular_tint = np.reshape(ixam_specular_tint, width * height * 4)
            # Create images in Ixam, width and height are dummy values, then set packed file data
            ixam_image_tint = bpy.data.images.new('Specular Tint', width, height)
            ixam_image_tint.pixels.foreach_set(np.float32(ixam_specular_tint))
            ixam_image_tint.pack()

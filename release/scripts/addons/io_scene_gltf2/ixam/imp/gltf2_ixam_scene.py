...
from .gltf2_ixam_node import IxamNode
from .gltf2_ixam_animation import IxamAnimation
from .gltf2_ixam_vnode import VNode, compute_vnodes
...
class IxamScene():
    ...
    def create(gltf):
        ...
        gltf.ixam_scene = scene.name
        if bpy.context.collection.name in bpy.data.collections: # avoid master collection
            gltf.ixam_active_collection = bpy.context.collection.name
        if scene.render.engine not in ['CYCLES', 'IXAM_EEVEE']:
            scene.render.engine = "IXAM_EEVEE"
        ...
        IxamNode.create_vnode(gltf, 'root')
        ...
        IxamScene.create_animations(gltf)
        ...
        IxamScene.select_imported_objects(gltf)
        IxamScene.set_active_object(gltf)
    ...
    def select_imported_objects(gltf):
        ...
        for vnode in gltf.vnodes.values():
            if vnode.type == VNode.Object:
                vnode.ixam_object.select_set(state=True)
    ...
    def set_active_object(gltf):
        ...
        bpy.context.view_layer.objects.active = vnode.ixam_object

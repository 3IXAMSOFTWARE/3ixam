...
from ..com.gltf2_ixam_ui import gltf2_KHR_materials_variants_primitive, gltf2_KHR_materials_variants_variant, gltf2_KHR_materials_variants_default_material
...
def do_primitives(gltf, mesh_idx, skin_idx, mesh, ob):
    ...
    # ---------------
    # Convert all the arrays glTF -> Ixam

    # Change from relative to absolute positions for morph locs
    for sk_locs in sk_vert_locs:
        sk_locs += vert_locs

    gltf.locs_batch_gltf_to_ixam(vert_locs)
    gltf.normals_batch_gltf_to_ixam(vert_normals)
    for sk_locs in sk_vert_locs:
        gltf.locs_batch_gltf_to_ixam(sk_locs)
    ...
    if num_joint_sets and mesh_options.skinning:
        ...
        for i, node_idx in enumerate(pyskin.joints):
            ...
            ob.vertex_groups.new(name=bone.ixam_bone_name)
    ...
    if has_materials:
        ...
        for idx_prim, prim in enumerate(pymesh.primitives):
            if prim.material is not None:
                ...
                if vertex_color not in pymaterial.ixam_material:
                    IxamMaterial.create(gltf, prim.material, vertex_color)
                material_name = pymaterial.ixam_material[vertex_color]
                ...
                for mapping in prim.extensions['KHR_materials_variants']['mappings']:
                    ...
                    if 'material' not in mapping.keys():
                        ...
                        if str(mapping['material']) + str(vertex_color) not in gltf.variant_mapping.keys():
                            IxamMaterial.create(gltf, mapping['material'], vertex_color)
...
def skin_into_bind_pose(gltf, skin_idx, vert_joints, vert_weights, locs, vert_normals):
    ...
    if pyskin.inverse_bind_matrices is not None:
        ...
        inv_binds = [gltf.matrix_gltf_to_ixam(m) for m in inv_binds]
    else:

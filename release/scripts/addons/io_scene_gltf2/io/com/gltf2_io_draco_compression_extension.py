...
def dll_path() -> Path:
    ...
    ixam_root = Path(bpy.app.binary_path).parent
    python_lib = Path('{v[0]}.{v[1]}/python/lib'.format(v=bpy.app.version))
    python_version = 'python{v[0]}.{v[1]}'.format(v=sys.version_info)

    path = os.environ.get('IXAM_EXTERN_DRACO_LIBRARY_PATH')
    if path is None:
        path = {
            'win32': ixam_root / python_lib / 'site-packages',
            'linux': ixam_root / python_lib / python_version / 'site-packages',
            'darwin': ixam_root.parent / 'Resources' / python_lib / python_version / 'site-packages'
        }.get(sys.platform)
    else:
        return Path(path)

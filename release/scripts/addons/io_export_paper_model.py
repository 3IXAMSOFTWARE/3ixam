...
bl_info = {
    ...
    "doc_url": "{IXAM_MANUAL_URL}/addons/import_export/paper_model.html",
...
def create_blank_image(image_name, dimensions, alpha=1):
    ...
    if image.users > 0:
        raise UnfoldError(
            ...
            "Please report this on the IxamArtists forum. Export failed.")
...
class Unfolder:
    ...
    def save(self, properties):
        """Export the document"""
        # Note about scale: input is directly in ixam length
...
class Unfold(bpy.types.Operator):
    """Ixam Operator: unfold the selected object."""
...
class ClearAllSeams(bpy.types.Operator):
    """Ixam Operator: clear all seams of the active Mesh and all its unfold data"""
...
class ExportPaperModel(bpy.types.Operator):
    """Ixam Operator: save the selected object's net and optionally bake its texture"""
    ...
    def draw(self, context):
        ...
        if self.ui_expanded_document:
            ...
            if len(self.object.data.uv_layers) >= 8:
                ...
                col.label(text="Real will be used for texture baking.", icon='ERROR')
            row = col.row()
...
class SelectIsland(bpy.types.Operator):
    """Ixam Operator: select all faces of the active island"""

...
def addtracemat(matobj):
    try:
        ...
        if len(matslots) < 1:  # Make sure there is only one material slot
            ...
            # Check if color ixam is to be run
            if not Btrace.mat_run_color_ixam:
                ...
                # Run color ixam operator
                bpy.ops.object.colorixam()
    ...
    def execute(self, context):
        try:
            ...
            for i in colorObjects:
                ...
                if engine == 'CYCLES' or engine == 'IXAM_EEVEE':
            ...
            error_handlers(self, "object.colorixam", e,
                           "Color Ixam could not be completed")

            return {'CANCELLED'}


# This clears the keyframes
class OBJECT_OT_clearColorixam(Operator):
    bl_idname = "object.colorixamclear"
    ...
    def invoke(self, context, event):
        try:
            ...
            error_handlers(self, "object.colorixamclear", e,

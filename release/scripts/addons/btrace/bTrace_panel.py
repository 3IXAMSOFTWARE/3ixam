...
class addTracerObjectPanel(Panel):
    ...
    def draw(self, context):
        ...
        # Color Ixam Panel options
        def color_ixam():
            # Buttons for Color Ixam
            row = box.row()
        ...
        def curve_settings():
            ...
            if Btrace.material_settings:
                ...
                if not Btrace.trace_mat_random:
                    ...
                    row.prop(Btrace, "mat_run_color_ixam")
                    if Btrace.mat_run_color_ixam:
                        row = box.row()
                        row.operator("object.colorixamclear",
        ...
        # Color Ixam
        elif Btrace.btrace_toolmenu == 'tool_colorixam':
            row = layout.row()
            row.label(text="  Curve/Object Tool:", icon="OUTLINER_OB_CURVE")
            box = self.layout.box()
            row = box.row()
            row.label(text="Color Ixam", icon="COLOR")
            row.operator("object.colorixam", icon='PLAY', text="Run!")
            row = box.row()
            row.operator("object.colorixamclear", text="Reset Keyframes", icon="KEY_DEHLT")
            row.prop(Btrace, "mmSkip", text="Keyframe every")
            color_ixam()

...
class BezierPoint:
    @staticmethod
    def FromIxamBezierPoint(ixamBezierPoint):
        return BezierPoint(ixamBezierPoint.handle_left, ixamBezierPoint.co, ixamBezierPoint.handle_right)
...
class BezierSegment:
    @staticmethod
    def FromIxamBezierPoints(ixamBezierPoint1, ixamBezierPoint2):
        bp1 = BezierPoint.FromIxamBezierPoint(ixamBezierPoint1)
        bp2 = BezierPoint.FromIxamBezierPoint(ixamBezierPoint2)
...
class BezierSpline:
    ...
    def __init__(self, ixamBezierSpline):
        if not ixamBezierSpline is None:
            if ixamBezierSpline.type != 'BEZIER':
                print("## ERROR:", "ixamBezierSpline.type != 'BEZIER'")
                raise Exception("ixamBezierSpline.type != 'BEZIER'")
            if len(ixamBezierSpline.bezier_points) < 1:
                if not ixamBezierSpline.use_cyclic_u:
                    print("## ERROR:", "len(ixamBezierSpline.bezier_points) < 1")
                    raise Exception("len(ixamBezierSpline.bezier_points) < 1")

        self.bezierSpline = ixamBezierSpline
    ...
    def SetupSegments(self):
        ...
        for iBezierPoint in range(nrBezierPoints - 1):
            ...
            rvSegments.append(BezierSegment.FromIxamBezierPoints(bezierPoint1, bezierPoint2))
        if self.isCyclic:
...
class Curve:
    def __init__(self, ixamCurve):
        self.curve = ixamCurve
        self.curveData = ixamCurve.data

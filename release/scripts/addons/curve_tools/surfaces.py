...
class LoftedSurface:
    ...
    def FromSelection():
        ...
        ixamActiveCurve = bpy.context.active_object
        ixamOtherCurve = selObjects[0]
        if ixamActiveCurve == ixamOtherCurve: ixamOtherCurve = selObjects[1]

        aCurve = curves.Curve(ixamActiveCurve)
        oCurve = curves.Curve(ixamOtherCurve)

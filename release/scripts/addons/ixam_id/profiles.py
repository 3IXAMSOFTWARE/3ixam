...
class IxamIdProfile(metaclass=_BIPMeta):
    """Current Ixam ID profile.
...
def register():
    ...
    profiles_path = bpy.utils.user_resource('CONFIG', path='ixam_id', create=True)
...
def get_profiles_data():
    """Returns the profiles.json content from a ixam_id folder in the
    Ixam config directory. If the file does not exist we create one with the
...
def get_active_profile():
    ...
    @returns: dict like {'user_id': 1234, 'username': 'email@ixam.org'}
    """
...
def save_as_active_profile(auth_result: communication.AuthResult, username, subclients):
    ...
    IxamIdProfile.user_id = auth_result.user_id
    IxamIdProfile.token = auth_result.token
    IxamIdProfile.expires = auth_result.expires

    IxamIdProfile.username = username
    IxamIdProfile.subclients = subclients

    IxamIdProfile.save_json(make_active_profile=True)

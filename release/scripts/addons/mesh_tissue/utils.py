...
def set_animatable_fix_handler(self, context):
    '''
    Prevent Ixam Crashes with handlers
    '''
    old_handlers = []
    ixam_handlers = bpy.app.handlers.render_init
    for h in ixam_handlers:
        if "turn_off_animatable" in str(h):
            old_handlers.append(h)
    for h in old_handlers: ixam_handlers.remove(h)
    ixam_handlers.append(turn_off_animatable)
    return

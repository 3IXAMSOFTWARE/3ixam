...
noise_basis_default = "IXAM"
noise_basis = [
    ("IXAM", "Ixam", "Ixam default noise", 0),
...
def rocks_noise(coords, depth, hardnoise, nbasis, distortion):
    x, y, z = coords
    p = turbulence((x, y, z), 4, 0, noise_basis='IXAM') * 0.125 * distortion
    xx, yy, zz = x, y, z
...
def Effect_Basis_Function(coords, type, bias):
    ...
    # crackle:
        ...
        t = turbulence((x, y, 0), 6, 0, noise_basis="IXAM") * 0.25
    ...
    # cosine noise:
        ...
        effect = cos(5 * noise((x, y, 0), noise_basis="IXAM"))
        effect = offset + iscale * (effect * 0.5)
    # spikey noise:
    elif type == 18:
        n = 0.5 + 0.5 * turbulence((x * 5, y * 5, 0), 8, 0, noise_basis="IXAM")
        effect = ((n * n)**5)
        effect = offset + iscale * effect
    # stone noise:
    elif type == 19:
        effect = offset + iscale * (noise((x * 2, y * 2, 0), noise_basis="IXAM") * 1.5 - 0.75)
    # Flat Turb:
    elif type == 20:
        t = turbulence((x, y, 0), 6, 0, noise_basis="IXAM")
        effect = t * 2.0
        if effect > 0.25:
            effect = 0.25
        effect = offset + iscale * effect
...
def Effect_Function(coords, type, bias, turb, depth, frequency, amplitude):
    ...
    # turbulence:
    if turb > 0.0:
        t = turb * (0.5 + 0.5 * turbulence(coords, 6, 0, noise_basis="IXAM"))
        x = x + t
        y = y + t
        z = z + t

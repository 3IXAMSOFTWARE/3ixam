...
from bpy.props import EnumProperty, StringProperty, BoolProperty, IntProperty, FloatVectorProperty, FloatProperty, CollectionProperty, PointerProperty
...
# 3IXAM Render material nodes
# (rna_type.identifier, type, rna_type.name)
# Keeping mixed case to avoid having to translate entries when adding new nodes in operators.
ixam_mat_input_nodes_props = (
...
# (rna_type.identifier, type, rna_type.name)
# Keeping mixed case to avoid having to translate entries when adding new nodes in operators.
ixam_mat_output_nodes_props = (
    ('ShaderNodeOutput', 'OUTPUT', 'Output'),
)

# (rna_type.identifier, type, rna_type.name)
# Keeping mixed case to avoid having to translate entries when adding new nodes in operators.
ixam_mat_color_nodes_props = (
    ('ShaderNodeMixRGB', 'MIX_RGB', 'MixRGB'),
    ('ShaderNodeRGBCurve', 'CURVE_RGB', 'RGB Curves'),
    ('ShaderNodeInvert', 'INVERT', 'Invert'),
    ('ShaderNodeHueSaturation', 'HUE_SAT', 'Hue/Saturation'),
...
# (rna_type.identifier, type, rna_type.name)
# Keeping mixed case to avoid having to translate entries when adding new nodes in operators.
ixam_mat_converter_nodes_props = (
    ('ShaderNodeValToRGB', 'VALTORGB', 'ColorRamp'),
    ('ShaderNodeRGBToBW', 'RGBTOBW', 'RGB to BW'),
    ('ShaderNodeMath', 'MATH', 'Math'),
    ('ShaderNodeVectorMath', 'VECT_MATH', 'Vector Math'),
...
def is_cycles_or_eevee(context):
    return context.scene.render.engine in {'CYCLES', 'IXAM_EEVEE'}
...
def nw_check(context):
    ...
    valid = False
    if (space.type == 'NODE_EDITOR' or space.type == 'MATPRO') and space.node_tree is not None and space.tree_type in valid_trees:
        valid = True
        ...
        if self.delete_muted:
            for node in nodes:
                if node.mute:
                    node.select = True
                    deleted_nodes.append(node.name)
            bpy.ops.matpro.delete_reconnect()
...
class NWEmissionViewer(Operator, NWBase):
    ...
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL', 'INTERNAL'}
...
class NWSwitchNodeType(Operator, NWBase):
    ...
    to_type: EnumProperty(
        ...
        list(ixam_mat_input_nodes_props) +
        list(ixam_mat_output_nodes_props) +
        list(ixam_mat_color_nodes_props) +
        list(ixam_mat_vector_nodes_props) +
        list(ixam_mat_converter_nodes_props) +
        list(ixam_mat_layout_nodes_props) +
...
class NWCopySettings(Operator, NWBase):
    ...
    def execute(self, context):
        ...
        # Code by zeffii from http://ixam.stackexchange.com/a/42338/3710
...
class NWAddTextureSetup(Operator, NWBase):
    ...
    selected_socket_index: IntProperty(name="Selected Socket")
    ...
    def execute(self, context):
        ...
        valid = False
        input_index = 0
        if not selected_node.inputs[self.selected_socket_index].is_linked:
            valid = True
            input_index = self.selected_socket_index

        if valid:
            locx = selected_node.location.x
            locy = selected_node.location.y - selected_node.dimensions.y/2

            xoffset = [500, 700]
            is_texture = False
            if selected_node.type in texture_types + ['MAPPING']:
                xoffset = [290, 500]
                is_texture = True
            ...
            if (selected_node.type in texture_types and selected_node.type != 'TEX_IMAGE') or (selected_node.type == 'BACKGROUND'):
                coordout = 0  # image texture uses UVs, procedural textures and Background shader use Generated
                if selected_node.type == 'BACKGROUND':
            ...
            if not is_texture:
                ...
                nodes.active = tex
                links.new(tex.outputs[0], selected_node.inputs[input_index])

            selected_node.select = False
            if self.add_mapping or is_texture:
                if selected_node.type != 'MAPPING':
                ...
                coord.location = [locx - (200 if selected_node.type == 'MAPPING' else xoffset[1]), locy + 124]
                ...
                if not is_texture:
                    ...
                    nodes.active = m
                    links.new(m.outputs[0], selected_node.inputs[input_index])
                    links.new(coord.outputs[coordout], m.inputs[0])
        else:
            self.report({'WARNING'}, "No free inputs for node: "+selected_node.name)
...
class NWAddPrincipledSetup(Operator, NWBase, ImportHelper):
    ...
    def execute(self, context):
        ...
        normal_abbr = 'normal nor nrm nrml norm'.split(' ')
        bump_abbr = 'bump bmp'.split(' ')
        gloss_abbr = 'gloss glossy glossiness'.split(' ')
        rough_abbr = 'roughness rough rgh'.split(' ')
        socketnames = [
        ['Displacement', 'displacement displace disp dsp height heightmap'.split(' '), None],
        ['Base Color', 'diffuse diff albedo base col color'.split(' '), None],
        ['Subsurface Color', 'sss subsurface'.split(' '), None],
        ['Metallic', 'metallic metalness metal mtl'.split(' '), None],
        ['Specular', 'specularity specular spec spc'.split(' '), None],
...
class NWLinkToOutputNode(Operator, NWBase):
    ...
    def execute(self, context):
        ...
        output_types_ixam_mat = ['OUTPUT']
        output_types_textures = ['OUTPUT']
        output_types = output_types_shaders + output_types_compo + output_types_ixam_mat
        for node in nodes:
...
class NWSwitchMatInputSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_input_nodes_props, key=lambda k: k[2]):
            props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
            props.to_type = ident
...
class NWSwitchMatOutputSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_output_nodes_props, key=lambda k: k[2]):
            props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
            props.to_type = ident
...
class NWSwitchMatColorSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_color_nodes_props, key=lambda k: k[2]):
            props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
            props.to_type = ident
...
class NWSwitchMatVectorSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_vector_nodes_props, key=lambda k: k[2]):
            props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
            props.to_type = ident
...
class NWSwitchMatConverterSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_converter_nodes_props, key=lambda k: k[2]):
            props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
            props.to_type = ident
...
class NWSwitchMatLayoutSubmenu(Menu, NWBase):
    ...
    def draw(self, context):
        layout = self.layout
        for ident, node_type, rna_name in sorted(ixam_mat_layout_nodes_props, key=lambda k: k[2]):
            if node_type != 'FRAME':
                props = layout.operator(NWSwitchNodeType.bl_idname, text=rna_name)
                props.to_type = ident
...
kmi_defs = (
    ...
    # Emission Viewer
    (NWEmissionViewer.bl_idname, 'LEFTMOUSE', 'PRESS', True, True, False, None, "Connect to Real Viewer node"),
    # Reload Images

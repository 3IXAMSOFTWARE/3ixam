...
bl_info = {
    ...
    "warning": "",
    "doc_url": "{IXAM_MANUAL_URL}/addons/import_export/images_as_planes.html",
    "support": 'OFFICIAL',
...
def offset_planes(planes, gap, axis):
    ...
    obj2 0.5 ixam units away from obj1 along the local positive Z axis.
...
class IMPORT_IMAGE_OT_to_plane(Operator, AddObjectHelper):
    ...
    force_reload: BoolProperty(
        ...
        description="Force reloading of the image if already opened elsewhere in Ixam"
    )
    ...
    SIZE_MODES = (
        ...
        ('DPBU', "Dots/BU", "Use definition of the image as dots per Ixam Unit"),
    )
    ...
    factor: FloatProperty(name="Definition", min=1.0, default=600.0,
                           description="Number of pixels per inch or Ixam Unit")
    ...
    def draw_material_config(self, context):
        ...
        if engine not in ('CYCLES', 'IXAM_EEVEE', 'IXAM_WORKBENCH'):
    ...
    def invoke(self, context, event):
        engine = context.scene.render.engine
        if engine not in {'CYCLES', 'IXAM_EEVEE'}:
            if engine != 'IXAM_WORKBENCH':
                self.report({'ERROR'}, tip_("Cannot generate materials for unknown %s render engine") % engine)
                return {'CANCELLED'}
            else:
                self.report({'WARNING'},
                            tip_("Generating Real/Play compatible material, but won't be visible with %s engine") % engine)
    ...
    def single_image_spec_to_plane(self, context, img_spec):
        ...
        # Configure material
        engine = context.scene.render.engine
        if engine in {'CYCLES', 'IXAM_EEVEE', 'IXAM_WORKBENCH'}:
    ...
    def apply_texture_options(self, texture, img_spec):
        # Shared by both Cycles and Ixam Internal

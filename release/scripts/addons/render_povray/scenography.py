...
def img_map(ts):
    """Translate mapping type from Ixam UI to POV syntax and return that string."""
    image_map = ""
...
def img_map_transforms(ts):
    """Translate mapping transformations from Ixam UI to POV syntax and return that string."""
    # XXX TODO: unchecked textures give error of variable referenced before assignment XXX
    # POV-Ray "scale" is not a number of repetitions factor, but ,its
    # inverse, a standard scale factor.
    # 0.5 Offset is needed relatively to scale because center of the
    # scale is 0.5,0.5 in ixam and 0,0 in POV
    # Strange that the translation factor for scale is not the same as for
    # translate.
    # TODO: verify both matches with other ixam renderers / internal in previous versions.
...
def img_map_bg(wts):
    """Translate world mapping from Ixam UI to POV syntax and return that string."""
...
def export_camera(file, scene, global_matrix, render, tab_write):
    """Translate camera from Ixam UI to POV syntax and write to exported file."""
    camera = scene.camera
    ...
    if scene.pov.baking_enable and active_object and active_object.type == "MESH":
        ...
        if camera.data.type == "ORTHO":
            ...
            # Ixam angle is radian so should be converted to degrees:
...
def export_lights(lamps, file, scene, global_matrix, tab_write):
    """Translate lights from Ixam UI to POV syntax and write to exported file."""
    ...
    for exported_lights_count, ob in enumerate(lamps, start=1):
        ...
        if lamp.type == "POINT":
            ...
            # Ixam does not have a tightness equivalent, 0 is most like ixam default.
            ...
            # Area lights have no falloff type, so always use ixams lamp quad equivalent
            # for those?
        ...
        if lamp.type not in {"SUN", "AREA"}:
            if lamp.falloff_type == "INVERSE_SQUARE":
                ...
                tab_write(file, "fade_power %d\n" % 2)  # Use ixams lamp quad equivalent
            elif lamp.falloff_type == "INVERSE_LINEAR":
                tab_write(file, "fade_distance %.6f\n" % (lamp.distance / 2.0))
                tab_write(file, "fade_power %d\n" % 1)  # Use ixams lamp linear
            elif lamp.falloff_type == "CONSTANT":
                tab_write(file, "fade_distance %.6f\n" % (lamp.distance / 2.0))
                tab_write(file, "fade_power %d\n" % 3)
                # Use ixams lamp constant equivalent no attenuation.
    ...
    for t in world.pov_texture_slots:  # risk to write several sky_spheres but maybe ok.
        ...
        if tex.type is not None:
            ...
            if tex.type == "IMAGE":
                ...
                # using camera rotation valuesdirectly from ixam seems much easier
                if t_blend.texture_coords == "ANGMAP":
                    ...
                    # inverse, a standard scale factor.
                    # 0.5 Offset is needed relatively to scale because center of the
                    # UV scale is 0.5,0.5 in ixam and 0,0 in POV
...
def export_rainbows(rainbows, file, scene, global_matrix, tab_write):
    ...
    for ob in rainbows:
        ...
        angle = degrees(ob.data.spot_size / 2.5)  # radians in ixam (2
...
def export_smoke(file, smoke_obj_name, smoke_path, comments, global_matrix):
    """export Ixam smoke type fluids to pov media using df3 library"""
    ...
    if domain is not None:
        ...
        # We scale our cube to get its final size and shapes but still in *object* space (same as Ixam's bbox).
        file.write("scale<%.6g,%.6g,%.6g>\n" % (dim[0], dim[1], dim[2]))

        # We offset our cube such that (0,0,0) coordinate matches Ixam's object center.

...
""" Get POV-Ray specific objects In and Out of Ixam """
...
class POV_OT_loft_add(Operator):
    """Create the representation of POV loft using Ixam curves."""
...
def pov_isosurface_view_define(context, op, ob, loc):
    """create the representation of POV isosurface using a Ixam empty."""
...
class POV_OT_isosurface_add(Operator):
    """Add the representation of POV isosurface sphere by a Ixam mesh icosphere.
    ...
    def execute(self, context):
        ...
        try:
            ...
            self.report(
                {"INFO"}, "This native POV-Ray primitive " "is only an abstract proxy in Ixam"
            )
...
class POV_OT_isosurface_box_add(Operator):
    """Add the representation of POV isosurface box using also just a Ixam mesh cube.
...
class POV_OT_sphere_sweep_add(Operator):
    """Add the representation of POV sphere_sweep using a Ixam NURBS curve.
...
class POV_OT_blobsphere_add(Operator):
    """Add the representation of POV blob using a Ixam meta ball.

...
from . import texturing_procedural # for Ixam procedurals to POV patterns emulation
...
def safety(name, ref_level_bound):
    ...
    # It may not even be any longer possible to feed no arg from Ixam UI
    # try:
...
def write_matrix(file, matrix):
    """Translate some transform matrix from Ixam UI
    to POV syntax and write to exported file """
    tab_write(file,
...
def write_pov(filename, scene=None, info_callback=None):
    """Main export process from Ixam UI to POV syntax and write to exported file """
    ...
    with open(filename, "w") as file:
        ...
        def export_global_settings(scene):
            ...
            for material in bpy.data.materials:
                if material.pov_subsurface_scattering.use and once_sss:
                    # In pov, the scale has reversed influence compared to ixam. these number
                    # should correct that
                    tab_write(file,
        ...
        if comments:
            file.write(
                "//----------------------------------------------\n"
                "//--Exported with POV-Ray exporter for Ixam--\n"
                "//----------------------------------------------\n\n"
            )

...
"""Translate Ixam meta balls to POV blobs."""

import bpy
from .shading import write_object_material_interior

def export_meta(file, metas, tab_write, DEF_MAT_NAME):
    """write all POV blob primitives and Ixam Metas to exported file """
    # TODO - ixams 'motherball' naming is not supported.
    ...
    # Get groups of metaballs by ixam name prefix.
    meta_group = {}
    meta_elems = {}
            ...
            if len(meta_elems[mg]) != 0:
                ...
                try:
                    one_material = elems[1].data.materials[
                        0
                    ]  # lame! - ixam can't do anything else.
...
'''
        ...
        if elements:
            ...
            try:
                material = meta.materials[0]  # lame! - ixam can't do anything else.
            except:
                material = None

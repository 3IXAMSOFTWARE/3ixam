...
"""Define the POV render engine from generic Ixam RenderEngine class."""
import faulthandler
faulthandler.enable()
import bpy
...
class PovRender(bpy.types.RenderEngine):
        ...
        for f in (self._temp_file_in, self._temp_file_ini, self._temp_file_out):
            for i in range(5):
                try:
                    ...
                    # Wait a bit before retrying file might be still in use by Ixam,
    ...
    def render(self, depsgraph):
        ...
        if bpy.context.scene.pov.text_block != "":
            ...
            self.update_stats("", "POV-Ray 3.7: Exporting ini options from Ixam")
            ...
            # print(self._temp_file_log) #bring the pov log to ixam console with proper path?
            with open(
                self._temp_file_log
            ...
            # start export
            self.update_stats("", "POV-Ray 3.7: Exporting data from Ixam")
            ...
            # print(filename_log) #bring the pov log to ixam console with proper path?
            try:
                with open(

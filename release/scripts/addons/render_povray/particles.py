...
"""Get some Ixam particle objects translated to POV."""
...
def export_hair(file, ob, mod, p_sys, global_matrix):
    """Get Ixam path particles (hair strands) objects translated to POV sphere_sweep unions."""
    # tstart = time.time()
    ...
    if ob.material_slots[p_sys_settings.material - 1].material and ob.active_material is not None:
        ...
        for th in pmaterial.pov_texture_slots:
            ...
            if (
                ...
                textured_hair = 1
        if pmaterial.strand.use_ixam_units:
        ...
        pmaterial = "default"  # No material assigned in ixam, use default one
        strand_start = 0.01
        strand_end = 0.01
        strand_shape = 0.0
    ...
    for pindex in range(total_number_of_strands):
        ...
        for step in range(steps):
            ...
            if step == steps - 1:
                if textured_hair:
                    # Write pigment and alpha (between Pov and Ixam,

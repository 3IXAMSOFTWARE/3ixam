...
def write_object_modifiers(ob, File):
    """Translate some object level POV statements from Ixam UI
...
class POV_OT_plane_add(Operator):
    """Add the representation of POV infinite plane using just a very big Ixam Plane.
...
class POV_OT_box_add(Operator):
    """Add the representation of POV box using a simple Ixam mesh cube.
...
def pov_sphere_define(context, op, ob, loc):
    """create the representation of POV sphere using a Ixam icosphere.
...
def pov_cone_define(context, op, ob):
    ...
    Ixam cone does not offer the same features such as a second radius."""
    verts = []
    faces = []
    if op:
        mesh = None
...
class POV_OT_rainbow_add(Operator):
    """Add the representation of POV rainbow using a Ixam spot light.
...
def pov_torus_define(context, op, ob):
    """Add the representation of POV torus using just a Ixam torus.

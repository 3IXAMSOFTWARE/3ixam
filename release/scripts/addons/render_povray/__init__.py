...
render_core.py :
    Define the POV render engines declinations from generic Ixam RenderEngine class

render_properties.py :
    Initialize properties for render parameters (Ixam generic and POV native)

render_gui.py :
    Display properties from render_properties.py for user to change them

render.py :
    Translate render properties (Ixam and POV native) to POV, ini file and CLI
...
model_properties.py :
    Initialize properties for translating Ixam geometry objects parameters to pov
...
shading_properties.py :
    Initialize properties for translating Ixam materials parameters to pov

shading_ray_properties.py :
    Initialize properties for translating Ixam ray paths relevant material parameters to pov
...
texturing_properties.py :
    Initialize properties for translating Ixam materials/world... texture influences to pov

texturing_gui.py :
    Display properties from texturing_properties.py available for user to change

texturing.py :
    Translate ixam pixel based bitmap texture influences into POV

texturing_procedural.py :
    Translate ixam algorithmic procedural texture influences into POV
...
scripting.py :
    Insert POV native scene description elements into ixam scene or to exported POV file
...
Ixam stand up to other POV enabled IDEs (povwin, POV for Mac, QTPOV, VSCode, Vim...)
    presets :
        Material (sss)
...
bl_info = {
    ...
    'description': "Persistence of Vision addon for Ixam",
    'doc_url': "{IXAM_MANUAL_URL}/addons/render/povray.html",
    'category': "Render",
...
class POV_OT_update_addon(bpy.types.Operator):
    ...
    def recursive_overwrite(self, src, dest, ignore=None):
        ...
        with tempfile.TemporaryDirectory() as temp_dir_path:
            ...
            url = "https://github.com/ixam/ixam-addons/archive/refs/heads/master.zip"
            try:
            ...
            with zipfile.ZipFile(temp_zip_path) as zip_archive:
                pov_addon_pkg = (member for member in zip_archive.namelist() if
                                  "ixam-addons-master/render_povray" in member)
                for member in pov_addon_pkg:
                    # Remove the first directory and the filename
                    # e.g. ixam-addons-master/render_povray/nodes.py
        ...
        print("POV addon update finished, restart Ixam for the changes to take effect.")
        print("-" * 20)
        self.report({"WARNING"}, "Restart Ixam!")
        return {"FINISHED"}

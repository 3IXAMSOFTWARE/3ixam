...
bl_info = {
    ...
    "warning": "",
    "doc_url": "{IXAM_MANUAL_URL}/addons/materials/material_library.html",
    "tracker_url": "",
    "category": "Material",
}
...
class matlibProperties(PropertyGroup):
    ...
    hide_search: BoolProperty(name="Hide Search", description="Use Ixam Search Only")
...
bpy.ops.wm.save_mainfile(filepath="%s", check_existing=False, compress=True)''' % winpath(path)
            if not (send_command(cmd, "createlib.py")):
                return 'ERROR', "There was an error creating the file. Make sure you run Ixam with admin rights."
                ...
                if nmats == len(bpy.data.materials) and not linked:
                    ...
                    for mat in reversed(bpy.data.materials):
                        if mat.name[0:len(name)] == name:
                            #careful on how ixam writes library paths
                            try:
                                samelib = bpy.path.relpath(mat.library.filepath) == bpy.path.relpath(self.current_library.path)
                            except:
                                samelib = False
...
'''.format(overwrite, thispath, name, libpath)
        ...
        if send_command(cmd):
            ...
            print("Save Material Error: Run Ixam with administrative privileges.")

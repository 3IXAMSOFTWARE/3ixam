...
from .fbx_utils import (
    ...
    FBX_KTIME,
    IXAM_OTHER_OBJECT_TYPES, IXAM_OBJECT_TYPES_MESHLIKE,
    FBX_LIGHT_TYPES, FBX_LIGHT_DECAY_TYPES,
    RIGHT_HAND_AXES, FBX_FRAMERATES,
    # Miscellaneous utils.
    PerfMon,
    units_ixam_to_fbx_factor, units_convertor, units_convertor_iter,
    ...
    # Key generators.
    get_ixamID_key, get_ixamID_name,
    get_ixam_mesh_shape_key, get_ixam_mesh_shape_channel_key,
    get_ixam_empty_key, get_ixam_bone_key,
    get_ixam_bindpose_key, get_ixam_armature_skin_key, get_ixam_bone_cluster_key,
    get_ixam_anim_id_base, get_ixam_anim_stack_key, get_ixam_anim_layer_key,
    get_ixam_anim_curve_node_key, get_ixam_anim_curve_key,
    get_ixam_nodetexture_key,
    # FBX element data.
    elem_empty,
...
# ##### Templates #####
# TODO: check all those "default" values, they should match Ixam's default as much as possible, I guess?
...
def fbx_template_def_light(scene, settings, override_defaults=None, nbr_users=0):
    ...
    props = {
        ...
        b"Intensity": (100.0, "p_number", True),  # Times 100 compared to Ixam values...
...
def fbx_data_element_custom_properties(props, bid):
    """
    Store custom properties of ixam ID bid (any mapping-like object, in fact) into FBX properties props.
    """
    items = bid.items()
...
def fbx_data_bindpose_element(root, me_obj, me, scene_data, arm_obj=None, mat_world_arm=None, bones=[]):
    ...
    bindpose_key = get_ixam_bindpose_key(arm_obj.bdata, me)
...
def fbx_data_texture_file_elements(root, ixam_tex_key, scene_data):
    """
    ...
    ma, sock_name = ixam_tex_key
    ma_wrap = node_shader_utils.PrincipledBSDFWrapper(ma, is_readonly=True)
    tex_key, _fbx_prop = scene_data.data_textures[ixam_tex_key]
...
def fbx_data_armature_elements(root, arm_obj, scene_data):
    ...
    for bo_obj in bones:
        ...
        # Store Ixam bone length - XXX Not much useful actually :/
        ...
        # elem_props_set(props, "p_double", "IxamBoneLength".encode(), (bo.tail_local - bo.head_local).length, custom=True)
...
def fbx_data_object_elements(root, ob_obj, scene_data):
    ...
    if ob_obj.is_bone:
        ...
        if scene_data.settings.armature_nodetype == 'ROOT':
            ...
            obj_type = b"Null"
    elif (ob_obj.type in IXAM_OBJECT_TYPES_MESHLIKE):
        obj_type = b"Mesh"
...
# Mapping Ixam -> FBX (principled_socket_name, fbx_name).
PRINCIPLED_TEXTURE_SOCKETS_TO_FBX = (
    ...
    # ("base_color_texture", b"TransparentColor"),  # Uses diffuse color in Ixam!
    ("emission_strength_texture", b"EmissiveFactor"),
    ("emission_color_texture", b"EmissiveColor"),
    # ("ambient", "ambient", b"AmbientFactor"),
    # ("", "", b"AmbientColor"),  # World stuff in Ixam, for now ignore...
...
def fbx_skeleton_from_armature(scene, settings, arm_obj, objects, data_meshes,
    ...
    data_empties[arm_obj] = get_ixam_empty_key(arm_obj.bdata)
    ...
    data_bones.update((bo, get_ixam_bone_key(arm_obj.bdata, bo.bdata)) for bo in bones)
    ...
    for ob_obj in objects:
        ...
        clusters = {bo: get_ixam_bone_cluster_key(arm_obj.bdata, me, bo.bdata) for bo in bones}
        data_deformers_skin.setdefault(arm_obj, {})[me] = (get_ixam_armature_skin_key(arm_obj.bdata, me),
                                                                      ob_obj, clusters)
...
def fbx_animations_do(scene_data, ref_id, f_start, f_end, start_zero, objects=None, force_keep=False):
    ...
    astack_key = get_ixam_anim_stack_key(scene, ref_id)
    alayer_key = get_ixam_anim_layer_key(scene, ref_id)
    name = (get_ixamID_name(ref_id) if ref_id else scene.name).encode()
...
def fbx_animations(scene_data):
    ...
    if scene_data.settings.bake_anim_use_nla_strips:
        ...
        for ob_obj in scene_data.objects:
            ...
            if not ob_obj.is_object:
                continue
            ob = ob_obj.bdata  # Back to real Ixam Object.
...
def fbx_data_from_scene(scene, depsgraph, settings):
    ...
    data_lights = {ob_obj.bdata.data: get_ixamID_key(ob_obj.bdata.data)
                   for ob_obj in objects if ob_obj.type == 'LIGHT'}
    # Unfortunately, FBX camera data contains object-level data (like position, orientation, etc.)...
    data_cameras = {ob_obj: get_ixamID_key(ob_obj.bdata.data)
                    for ob_obj in objects if ob_obj.type == 'CAMERA'}
    # Yep! Contains nothing, but needed!
    data_empties = {ob_obj: get_ixam_empty_key(ob_obj.bdata)
    ...
    data_meshes = {}
    for ob_obj in objects:
        if ob_obj.type not in IXAM_OBJECT_TYPES_MESHLIKE:
            continue
        ob = ob_obj.bdata
        use_org_data = True
        org_ob_obj = None
        ...
        if settings.use_mesh_modifiers or settings.use_triangles or ob.type in IXAM_OTHER_OBJECT_TYPES or is_ob_material:
            # We cannot use default mesh in that case, or material would not be the right ones...
            use_org_data = not (is_ob_material or ob.type in IXAM_OTHER_OBJECT_TYPES)
            ...
            if not use_org_data:
                ...
                if settings.use_triangles:
                    ...
                    bm.to_mesh(tmp_me)
                    bm.free()
                data_meshes[ob_obj] = (get_ixamID_key(tmp_me), tmp_me, True)
            ...
            if backup_pose_positions or tmp_mods:
                depsgraph.update()
        if use_org_data:
            data_meshes[ob_obj] = (get_ixamID_key(ob.data), ob.data, False)
    ...
    for me_key, me, _free in data_meshes.values():
        ...
        shapes_key = get_ixam_mesh_shape_key(me)
        ...
        for shape in me.shape_keys.key_blocks[1:]:
            ...
            channel_key, geom_key = get_ixam_mesh_shape_channel_key(me, shape)
    ...
    if scene.world:
        data_world = {scene.world: get_ixamID_key(scene.world)}
    else:
        data_world = {}
    ...
    for ob_obj in objects:
        ...
        for ma_s in ob_obj.material_slots:
            ...
            ma_data = data_materials.setdefault(ma, (get_ixamID_key(ma), []))
    ...
    for ma in data_materials.keys():
        ...
        #       given that textures actually do not exist anymore in material context in Ixam...
        ma_wrap = node_shader_utils.PrincipledBSDFWrapper(ma, is_readonly=True)
        for sock_name, fbx_name in PRINCIPLED_TEXTURE_SOCKETS_TO_FBX:
            tex = getattr(ma_wrap, sock_name)
            if tex is None or tex.image is None:
                continue
            ixam_tex_key = (ma, sock_name)
            data_textures[ixam_tex_key] = (get_ixam_nodetexture_key(*ixam_tex_key), fbx_name)

            img = tex.image
            vid_data = data_videos.setdefault(img, (get_ixamID_key(img), []))
            vid_data[1].append(ixam_tex_key)
    ...
    # Images
    for vid, (vid_key, ixam_tex_keys) in data_videos.items():
        for ixam_tex_key in ixam_tex_keys:
            tex_key, _fbx_prop = data_textures[ixam_tex_key]
...
def fbx_header_elements(root, scene_data, time=None):
    ...
    """
    app_vendor = "Ixam Foundation"
    app_name = "Ixam (stable FBX IO)"
    ...
    #~ # (in other words, this is pure ixam-exporter feature, and has nothing to do with FBX data).
    #~ if scene_data.settings.apply_unit_scale:
        ...
        #~ scale_factor = 1.0 / units_ixam_to_fbx_factor(scene)
    #~ else:
        #~ scale_factor_org = units_ixam_to_fbx_factor(scene)
...
def fbx_objects_elements(root, scene_data):
    ...
    for ixam_tex_key in scene_data.data_textures:
        fbx_data_texture_file_elements(objects, ixam_tex_key, scene_data)
...
def save_single(operator, scene, depsgraph, filepath="",
    ...
    if 'OTHER' in object_types:
        object_types |= IXAM_OTHER_OBJECT_TYPES

    # Default Ixam unit is equivalent to meter, while FBX one is centimeter...
    unit_scale = units_ixam_to_fbx_factor(scene) if apply_unit_scale else 100.0
...
def save(operator, context,
    ...
    if batch_mode == 'OFF':
        ...
        ctx_objects = [obj for obj in ctx_objects if not obj.protect]
        ...
        for data, data_name, data_obj_propname in data_seq:  # scene or collection
            ...
            kwargs_batch["context_objects"] = [obj for obj in getattr(data, data_obj_propname) if not obj.protect]

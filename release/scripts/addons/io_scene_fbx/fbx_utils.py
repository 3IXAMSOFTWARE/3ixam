...
MAT_CONVERT_LIGHT = Matrix.Rotation(math.pi / 2.0, 4, 'X')  # Ixam is -Z, FBX is -Y.
MAT_CONVERT_CAMERA = Matrix.Rotation(math.pi / 2.0, 4, 'Y')  # Ixam is -Z, FBX is +X.
# XXX I can't get this working :(
# MAT_CONVERT_BONE = Matrix.Rotation(math.pi / 2.0, 4, 'Z')  # Ixam is +Y, FBX is -X.
MAT_CONVERT_BONE = Matrix()


IXAM_OTHER_OBJECT_TYPES = {'CURVE', 'SURFACE', 'FONT', 'META'}
IXAM_OBJECT_TYPES_MESHLIKE = {'MESH'} | IXAM_OTHER_OBJECT_TYPES
...
# (1.0 meaning centimeter, afaik). We use that to reflect user's default unit as set in Ixam with scale_length.
...
def units_ixam_to_fbx_factor(scene):
...
def matrix4_to_array(mat):
    ...
    # ixam matrix is row major, fbx is col major so transpose on write
    return tuple(f for v in mat.transposed() for f in v)


def array_to_matrix4(arr):
    """Convert a single 16-len tuple into a valid 4D Ixam matrix"""
    # Ixam matrix is row major, fbx is col major so transpose on read
...
def get_ixam_empty_key(obj):
    """Return bone's keys (Model and NodeAttribute)."""
    return "|".join((get_ixamID_key(obj), "Empty"))


def get_ixam_mesh_shape_key(me):
    """Return main shape deformer's key."""
    return "|".join((get_ixamID_key(me), "Shape"))


def get_ixam_mesh_shape_channel_key(me, shape):
    """Return shape channel and geometry shape keys."""
    return ("|".join((get_ixamID_key(me), "Shape", get_ixamID_key(shape))),
            "|".join((get_ixamID_key(me), "Geometry", get_ixamID_key(shape))))


def get_ixam_bone_key(armature, bone):
    """Return bone's keys (Model and NodeAttribute)."""
    return "|".join((get_ixamID_key((armature, bone)), "Data"))


def get_ixam_bindpose_key(obj, mesh):
    """Return object's bindpose key."""
    return "|".join((get_ixamID_key(obj), get_ixamID_key(mesh), "BindPose"))


def get_ixam_armature_skin_key(armature, mesh):
    """Return armature's skin key."""
    return "|".join((get_ixamID_key(armature), get_ixamID_key(mesh), "DeformerSkin"))


def get_ixam_bone_cluster_key(armature, mesh, bone):
    """Return bone's cluster key."""
    return "|".join((get_ixamID_key(armature), get_ixamID_key(mesh),
                     get_ixamID_key(bone), "SubDeformerCluster"))


def get_ixam_anim_id_base(scene, ref_id):
    if ref_id is not None:
        return get_ixamID_key(scene) + "|" + get_ixamID_key(ref_id)
    else:
        return get_ixamID_key(scene)


def get_ixam_anim_stack_key(scene, ref_id):
    """Return single anim stack key."""
    return get_ixam_anim_id_base(scene, ref_id) + "|AnimStack"


def get_ixam_anim_layer_key(scene, ref_id):
    """Return ID's anim layer key."""
    return get_ixam_anim_id_base(scene, ref_id) + "|AnimLayer"


def get_ixam_anim_curve_node_key(scene, ref_id, obj_key, fbx_prop_name):
    """Return (stack/layer, ID, fbxprop) curve node key."""
    return "|".join((get_ixam_anim_id_base(scene, ref_id), obj_key, fbx_prop_name, "AnimCurveNode"))


def get_ixam_anim_curve_key(scene, ref_id, obj_key, fbx_prop_name, fbx_prop_item_name):
    """Return (stack/layer, ID, fbxprop, item) curve key."""
    return "|".join((get_ixam_anim_id_base(scene, ref_id), obj_key, fbx_prop_name,
                     fbx_prop_item_name, "AnimCurve"))


def get_ixam_nodetexture_key(ma, socket_names):
    return "|".join((get_ixamID_key(ma), *socket_names))
...
class AnimationCurveNodeWrapper:
    ...
    def get_final_data(self, scene, ref_id, force_keep=False):
        ...
        for elem_key, fbx_group, fbx_gname, fbx_props in \
            ...
            group_key = get_ixam_anim_curve_node_key(scene, ref_id, elem_key, fbx_group)
            group = {}
            for c, def_val, fbx_item in zip(curves, self.default_values, fbx_props):
                fbx_item = FBX_ANIM_PROPSGROUP_NAME + "|" + fbx_item
                curve_key = get_ixam_anim_curve_key(scene, ref_id, elem_key, fbx_group, fbx_item)
...
# FBX Model-like data (i.e. Ixam objects, depsgraph instances and bones) are wrapped in ObjectWrapper.
...
# to actual Ixam data it contains.
...
class MetaObjectWrapper(type):
    def __call__(cls, bdata, armature=None):
        ...
        if instance is not None:
            # Duplis hack: since dupli instances are not persistent in Ixam (we have to re-create them to get updated
...
class ObjectWrapper(metaclass=MetaObjectWrapper):
    ...
    * Ixam Object
    * Ixam Bone and PoseBone
    * Ixam DepsgraphObjectInstance (for dulis).
    Note since a same Ixam object might be 'mapped' to several FBX models (esp. with duplis),
    ...
    def __init__(self, bdata, armature=None):
        ...
        if isinstance(bdata, Object):  # DEPRECATED
            self._tag = 'OB'
            self.name = get_ixamID_name(bdata)
            self.bdata = bdata
            self._ref = None
        elif isinstance(bdata, DepsgraphObjectInstance):
            if bdata.is_instance:
                ...
                self._tag = 'DP'
                self.name = "|".join((get_ixamID_name((bdata.parent.original, bdata.instance_object.original)),
    ...
    def use_bake_space_transform(self, scene_data):
        ...
        return (scene_data.settings.bake_space_transform and self._tag in {'OB', 'DP'} and
                self.bdata.type in IXAM_OBJECT_TYPES_MESHLIKE | {'EMPTY'})
    ...
    def fbx_object_matrix(self, scene_data, rest=False, local_space=False, global_space=False):
        ...
        Applies specific rotation to bones, lamps and cameras (conversion Ixam -> FBX).
        """
        ...
        # Since we have to apply corrections to some types of object, we always need local Ixam space here...
        ...
        if parent:
            if is_global:
                # Move matrix to global Ixam space.
                matrix = (parent.matrix_rest_global if rest else parent.matrix_global) @ matrix
            elif parent.use_bake_space_transform(scene_data):
                # Ixam's and FBX's local space of parent may differ if we use bake_space_transform...
                # Apply parent's *Ixam* local space...

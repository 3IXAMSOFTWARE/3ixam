...
# Script copyright (C) Ixam Foundation

# FBX 7.1.0 -> 7.4.0 loader for Ixam
...
def validate_ixam_names(name):
    assert(type(name) == bytes)
    # Ixam typically does not accept names over 63 bytes...
    if len(name) > 63:
        import hashlib
...
def elem_name_ensure_class(elem, clss=...):
    ...
    return validate_ixam_names(elem_name)
...
def ixam_read_custom_properties(fbx_obj, ixam_obj, settings):
            ...
            if b'U' in fbx_prop.props[3]:
                if fbx_prop.props[0] == b'UDP3DSMAX':
                    ...
                    for item in items.split('\r\n'):
                        if item:
                            ...
                            if len(split_item) != 2:
                                ...
                                prop_name = validate_ixam_names(prop_name.strip().encode('utf-8'))
                                ixam_obj[prop_name] = prop_value.strip()
                else:
                    prop_name = validate_ixam_names(fbx_prop.props[0])
                    ...
                    if prop_type in {b'Vector', b'Vector3D', b'Color', b'ColorRGB'}:
                        ...
                        assert(fbx_prop.props_type[4] == data_types.INT32)
                        ixam_obj[prop_name] = fbx_prop.props[4] != 0
...
def ixam_read_object_transform_preprocess(fbx_props, fbx_obj, rot_alt_mat, use_prepost_rot):
...
# ---------
# Animation
def ixam_read_animations_curves_iter(fbx_curves, ixam_start_offset, fbx_start_offset, fps):
    """
    Get raw FBX AnimCurve list, and yield values for all curves at each singular curves' keyframes,
    together with (ixam) timing, in frames.
    ixam_start_offset is expected in frames, while fbx_start_offset is expected in FBX ktime.
    """
    ...
    for curr_fbxktime in allkeys:
        ...
        curr_blenkframe = (curr_fbxktime - fbx_start_offset) * timefac + ixam_start_offset
        yield (curr_blenkframe, curr_values)


def ixam_read_animations_action_item(action, item, cnodes, fps, anim_offset, global_scale):
    """
    'Bake' loc/rot/scale into the action,
    taking any pre_ and post_ matrix into account to transform from fbx into ixam space.
    """
    ...
    for curves, fbxprop in cnodes.values():
        for (fbx_acdata, _ixam_data), channel in curves.values():
            fbx_curves.append((fbxprop, channel, fbx_acdata))

    # Leave if no curves are attached (if a ixam curve is attached to scale but without keys it defaults to 0).
    if len(fbx_curves) == 0:
        return
    ...
    ixam_curves = [action.fcurves.new(prop, index=channel, action_group=grpname)
                   for prop, nbr_channels, grpname in props for channel in range(nbr_channels)]

    if isinstance(item, Material):
        for frame, values in ixam_read_animations_curves_iter(fbx_curves, anim_offset, 0, fps):
            value = [0,0,0]
            ...
            for fc, v in zip(ixam_curves, value):
        ...
        for frame, values in ixam_read_animations_curves_iter(fbx_curves, anim_offset, 0, fps):
            ...
            for fc, v in zip(ixam_curves, (focal_length, focus_distance)):
                store_keyframe(fc, frame, v)
        ...
        for frame, values in ixam_read_animations_curves_iter(fbx_curves, anim_offset, 0, fps):
            ...
            mat, _, _ = ixam_read_object_transform_do(transform_data)
            ...
            for fc, value in zip(ixam_curves, chain(loc, rot, sca)):
    ...
    for fc in ixam_curves:
        fc.update()


def ixam_read_animations(fbx_tmpl_astack, fbx_tmpl_alayer, stacks, scene, anim_offset, global_scale):
    """
    ...
    actions = {}
    for as_uuid, ((fbx_asdata, _ixam_data), alayers) in stacks.items():
        stack_name = elem_name_ensure_class(fbx_asdata, b'AnimStack')
        for al_uuid, ((fbx_aldata, _ixam_data), items) in alayers.items():
            ...
            for item, cnodes in items.items():
                ...
                ixam_read_animations_action_item(action, item, cnodes, scene.render.fps, anim_offset, global_scale)


# ----
# Mesh

def ixam_read_geom_layerinfo(fbx_layer):
    return (
        validate_ixam_names(elem_find_first_string_as_bytes(fbx_layer, b'Name')),
...
def ixam_read_geom_array_setattr(generator, ixam_data, ixam_attr, fbx_data, stride, item_size, descr, xform):
    """Generic fbx_layer to ixam_data setter, generator is expected to yield tuples (ixam_idx, fbx_idx)."""
    max_ixam_idx = len(ixam_data) - 1
    max_fbx_idx = len(fbx_data) - 1
    print_error = True

    def check_skip(ixam_idx, fbx_idx):
        nonlocal print_error
        if fbx_idx < 0:  # Negative values mean 'skip'.
            return True
        if ixam_idx > max_ixam_idx:
            if print_error:
                print("ERROR: too much data in this Ixam layer, compared to elements in mesh, skipping!")
                print_error = False
            return True
    ...
    if xform is not None:
        if isinstance(ixam_data, list):
            if item_size == 1:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    ixam_data[ixam_idx] = xform(fbx_data[fbx_idx])
            else:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    ixam_data[ixam_idx] = xform(fbx_data[fbx_idx:fbx_idx + item_size])
        else:
            if item_size == 1:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    setattr(ixam_data[ixam_idx], ixam_attr, xform(fbx_data[fbx_idx]))
            else:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    setattr(ixam_data[ixam_idx], ixam_attr, xform(fbx_data[fbx_idx:fbx_idx + item_size]))
    else:
        if isinstance(ixam_data, list):
            if item_size == 1:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    ixam_data[ixam_idx] = fbx_data[fbx_idx]
            else:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    ixam_data[ixam_idx] = fbx_data[fbx_idx:fbx_idx + item_size]
        else:
            if item_size == 1:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    setattr(ixam_data[ixam_idx], ixam_attr, fbx_data[fbx_idx])
            else:
                def _process(blend_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx):
                    setattr(ixam_data[ixam_idx], ixam_attr, fbx_data[fbx_idx:fbx_idx + item_size])

    for ixam_idx, fbx_idx in generator:
        if check_skip(ixam_idx, fbx_idx):
            continue
        _process(ixam_data, ixam_attr, fbx_data, xform, item_size, ixam_idx, fbx_idx)


# generic generators.
def ixam_read_geom_array_gen_allsame(data_len):
    return zip(*(range(data_len), (0,) * data_len))


def ixam_read_geom_array_gen_direct(fbx_data, stride):
    fbx_data_len = len(fbx_data)
    return zip(*(range(fbx_data_len // stride), range(0, fbx_data_len, stride)))


def ixam_read_geom_array_gen_indextodirect(fbx_layer_index, stride):
    return ((bi, fi * stride) for bi, fi in enumerate(fbx_layer_index))


def ixam_read_geom_array_gen_direct_looptovert(mesh, fbx_data, stride):
...
def ixam_read_geom_array_error_mapping(descr, fbx_layer_mapping, quiet=False):
    if not quiet:
        print("warning layer %r mapping type unsupported: %r" % (descr, fbx_layer_mapping))


def ixam_read_geom_array_error_ref(descr, fbx_layer_ref, quiet=False):
    if not quiet:
        print("warning layer %r ref type unsupported: %r" % (descr, fbx_layer_ref))


def ixam_read_geom_array_mapped_vert(
        mesh, ixam_data, ixam_attr,
    ...
    if fbx_layer_mapping == b'ByVertice':
        if fbx_layer_ref == b'Direct':
            ...
            ixam_read_geom_array_setattr(ixam_read_geom_array_gen_direct(fbx_layer_data, stride),
                                         ixam_data, ixam_attr, fbx_layer_data, stride, item_size, descr, xform)
            return True
        ixam_read_geom_array_error_ref(descr, fbx_layer_ref, quiet)
    elif fbx_layer_mapping == b'AllSame':
        if fbx_layer_ref == b'IndexToDirect':
            assert(fbx_layer_index is None)
            ixam_read_geom_array_setattr(ixam_read_geom_array_gen_allsame(len(ixam_data)),
                                         ixam_data, ixam_attr, fbx_layer_data, stride, item_size, descr, xform)
            return True
        ixam_read_geom_array_error_ref(descr, fbx_layer_ref, quiet)
    else:
        ixam_read_geom_array_error_mapping(descr, fbx_layer_mapping, quiet)

    return False


def ixam_read_geom_array_mapped_edge(
        mesh, ixam_data, ixam_attr,
        fbx_layer_data, fbx_layer_index,
        fbx_layer_mapping, fbx_layer_ref,
        stride, item_size, descr,
...
def ixam_read_geom_array_mapped_polygon(
        mesh, ixam_data, ixam_attr,
        fbx_layer_data, fbx_layer_index,
        fbx_layer_mapping, fbx_layer_ref,
        stride, item_size, descr,
    ...
    if fbx_layer_mapping == b'ByPolygon':
        if fbx_layer_ref == b'IndexToDirect':
            ...
            if fbx_layer_index is None:
                ...
                ixam_read_geom_array_setattr(ixam_read_geom_array_gen_indextodirect(fbx_layer_index, stride),
                                             ixam_data, ixam_attr, fbx_layer_data, stride, item_size, descr, xform)
            return True
...
def ixam_read_geom_array_mapped_polyloop(
        mesh, ixam_data, ixam_attr,
        fbx_layer_data, fbx_layer_index,
        fbx_layer_mapping, fbx_layer_ref,
        stride, item_size, descr,
    ...
    if fbx_layer_mapping == b'ByPolygonVertex':
        ...
        if fbx_layer_ref == b'Direct':
            assert(fbx_layer_index is None)
            ixam_read_geom_array_setattr(ixam_read_geom_array_gen_direct_looptovert(mesh, fbx_layer_data, stride),
                                         ixam_data, ixam_attr, fbx_layer_data, stride, item_size, descr, xform)
            return True
        ixam_read_geom_array_error_ref(descr, fbx_layer_ref, quiet)
    elif fbx_layer_mapping == b'AllSame':
...
def ixam_read_geom_layer_material(fbx_obj, mesh):
    ...
    (fbx_layer_name,
     fbx_layer_mapping,
     fbx_layer_ref,
     ) = ixam_read_geom_layerinfo(fbx_layer)

    layer_id = b'Materials'
    fbx_layer_data = elem_prop_first(elem_find_first(fbx_layer, layer_id))

    ixam_data = mesh.polygons
    ixam_read_geom_array_mapped_polygon(
        mesh, ixam_data, "material_index",
        fbx_layer_data, None,
        fbx_layer_mapping, fbx_layer_ref,
        1, 1, layer_id,
        )
...
def ixam_read_geom_layer_uv(fbx_obj, mesh):
    for layer_id in (b'LayerElementUV',):
        for fbx_layer in elem_find_iter(fbx_obj, layer_id):
            ...
            ixam_read_geom_array_mapped_polyloop(
                mesh, ixam_data, "uv",
                fbx_layer_data, fbx_layer_index,
                fbx_layer_mapping, fbx_layer_ref,
                2, 2, layer_id,
...
def ixam_read_geom_layer_color(fbx_obj, mesh, colors_type):
    if colors_type == 'NONE':
        return
    ...
    for layer_id in (b'LayerElementColor',):
        for fbx_layer in elem_find_iter(fbx_obj, layer_id):
            ...
            ixam_read_geom_array_mapped_polyloop(
                mesh, ixam_data, color_prop_name,
                fbx_layer_data, fbx_layer_index,
                fbx_layer_mapping, fbx_layer_ref,
                4, 4, layer_id,
                )
...
def ixam_read_geom_layer_smooth(fbx_obj, mesh):
    ...
    if fbx_layer_mapping == b'ByEdge':
        ...
        ixam_data = mesh.edges
        ixam_read_geom_array_mapped_edge(
            mesh, ixam_data, "use_edge_sharp",
            fbx_layer_data, None,
            fbx_layer_mapping, fbx_layer_ref,
            1, 1, layer_id,
        ...
        ixam_data = mesh.polygons
        return ixam_read_geom_array_mapped_polygon(
            mesh, ixam_data, "use_smooth",
            fbx_layer_data, None,
            fbx_layer_mapping, fbx_layer_ref,
            1, 1, layer_id,
...
def ixam_read_geom_layer_edge_crease(fbx_obj, mesh):
    ...
    if fbx_layer_mapping == b'ByEdge':
        ...
        ixam_data = mesh.edges
        return ixam_read_geom_array_mapped_edge(
            mesh, ixam_data, "crease",
            fbx_layer_data, None,
            fbx_layer_mapping, fbx_layer_ref,
            1, 1, layer_id,
            # Ixam squares those values before sending them to OpenSubdiv, when other software don't,
...
def ixam_read_geom_layer_normal(fbx_obj, mesh, xform=None):
    ...
    tries = ((mesh.loops, "Loops", False, ixam_read_geom_array_mapped_polyloop),
             (mesh.polygons, "Polygons", True, ixam_read_geom_array_mapped_polygon),
             (mesh.vertices, "Vertices", True, ixam_read_geom_array_mapped_vert))
    for ixam_data, ixam_data_type, is_fake, func in tries:
        bdata = [None] * len(ixam_data) if is_fake else ixam_data
        if func(mesh, bdata, "normal",
                fbx_layer_data, fbx_layer_index, fbx_layer_mapping, fbx_layer_ref, 3, 3, layer_id, xform, True):
            if ixam_data_type == "Polygons":
            ...
            elif ixam_data_type == "Vertices":
                # We have to copy vnors to lnors! Far from elegant, but simple.
                for l in mesh.loops:
                    l.normal[:] = bdata[l.vertex_index]
            return True

    ixam_read_geom_array_error_mapping("normal", fbx_layer_mapping)
    ixam_read_geom_array_error_ref("normal", fbx_layer_ref)
    return False


def ixam_read_geom(fbx_tmpl, fbx_obj, settings):
    from itertools import chain
    import array
    ...
    if fbx_polys:
        ...
        ixam_read_geom_layer_material(fbx_obj, mesh)
        ixam_read_geom_layer_uv(fbx_obj, mesh)
        ixam_read_geom_layer_color(fbx_obj, mesh, settings.colors_type)
    ...
    ok_smooth = ixam_read_geom_layer_smooth(fbx_obj, mesh)

    ixam_read_geom_layer_edge_crease(fbx_obj, mesh)
    ...
    if settings.use_custom_normals:
        ...
        if geom_mat_no is None:
            ok_normals = ixam_read_geom_layer_normal(fbx_obj, mesh)
        else:
            def nortrans(v):
                return geom_mat_no @ Vector(v)
            ok_normals = ixam_read_geom_layer_normal(fbx_obj, mesh, nortrans)
    ...
    if settings.use_custom_props:
        ixam_read_custom_properties(fbx_obj, mesh, settings)

    return mesh


def ixam_read_shape(fbx_tmpl, fbx_sdata, fbx_bcdata, meshes, scene):
...
def ixam_read_material(fbx_tmpl, fbx_obj, settings):
    ...
    if settings.use_custom_props:
        ixam_read_custom_properties(fbx_obj, ma, settings)
...
def ixam_read_texture_image(fbx_tmpl, fbx_obj, basedir, settings):
    import os
    ...
    if settings.use_custom_props:
        ixam_read_custom_properties(fbx_obj, image, settings)

    return image


def ixam_read_camera(fbx_tmpl, fbx_obj, global_scale):
    # meters to inches
    M2I = 0.0393700787
...
def ixam_read_light(fbx_tmpl, fbx_obj, global_scale):
    import math
    elem_name_utf8 = elem_name_ensure_class(fbx_obj, b'NodeAttribute')
...
class FbxImportHelperNode:
    ...
    def __init__(self, fbx_elem, bl_data, fbx_transform_data, is_bone):
        ...
        self.bl_bone = None                     # Name of bone if this is a bone (this may be different to fbx_name if there was a name conflict in Ixam!)
        ...
        if fbx_transform_data:
            self.matrix, self.matrix_as_parent, self.matrix_geom = ixam_read_object_transform_do(fbx_transform_data)
        else:
    ...
    def build_skeleton(self, arm, parent_matrix, parent_bone_size=1, force_connect_children=False):
        ...
        # while Ixam attaches to the tail.
    ...
    def build_node_obj(self, fbx_tmpl, settings):
        ...
        if settings.use_custom_props:
            ixam_read_custom_properties(self.fbx_elem, obj, settings)
        ...
        if self.is_bone:
            for child in self.children:
                ...
                if child_obj and child_obj != self.bl_obj:
                    ...
                    # Ixam attaches to the end of a bone, while FBX attaches to the start.
    ...
    def build_hierarchy(self, fbx_tmpl, settings, scene, view_layer):
        if self.is_armature:
            ...
            if self.fbx_elem:
                ...
                if settings.use_custom_props:
                    ixam_read_custom_properties(self.fbx_elem, arm, settings)
        ...
        if self.is_armature:
            ...
            if self.meshes:
                for mesh in self.meshes:
                    ...
                    # bring global armature & mesh matrices into *Ixam* global space.
                    ...
                    #       which we obviously cannot do in Ixam. :/
                    if amat is None:
...
def load(operator, context, filepath="",
    ...
    # Tables: (FBX_byte_id -> [FBX_data, None or Ixam_datablock])
    fbx_table_nodes = {}
    ...
    # FBX default base unit seems to be the centimeter, while raw Ixam Unit is equivalent to the meter...
    ...
    global_scale *= (unit_scale / units_ixam_to_fbx_factor(context.scene))
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            fbx_obj, ixam_data = fbx_item
            if fbx_obj.id != b'Geometry':
                continue
            if fbx_obj.props[-1] == b'Mesh':
                assert(ixam_data is None)
                fbx_item[1] = ixam_read_geom(fbx_tmpl, fbx_obj, settings)
    _(); del _
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.id != b'Material':
                continue
            assert(ixam_data is None)
            fbx_item[1] = ixam_read_material(fbx_tmpl, fbx_obj, settings)
    _(); del _
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.id != b'Video':
                continue
            fbx_item[1] = ixam_read_texture_image(fbx_tmpl_img, fbx_obj, basedir, settings)
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            fbx_obj, ixam_data = fbx_item
            if fbx_obj.id != b'Texture':
                continue
            fbx_item[1] = ixam_read_texture_image(fbx_tmpl_tex, fbx_obj, basedir, settings)
    _(); del _
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.props[-1] == b'Camera':
                assert(ixam_data is None)
                fbx_item[1] = ixam_read_camera(fbx_tmpl, fbx_obj, global_scale)
    _(); del _
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.props[-1] == b'Light':
                assert(ixam_data is None)
                fbx_item[1] = ixam_read_light(fbx_tmpl, fbx_obj, global_scale)
    _(); del _
    ...
    def _():
        ...
        for a_uuid, a_item in fbx_table_nodes.items():
            ...
            transform_data = ixam_read_object_transform_preprocess(fbx_props, fbx_obj, Matrix(), use_prepost_rot)
        ...
        # add parent-child relations and add ixam data to the node
        for fbx_link in fbx_connections.elems:
            ...
            if fbx_link.props_type[1:3] == b'LL':
                ...
                if child is None:
                    # add ixam data (meshes, lights, cameras, etc.) to a helper node
                    ...
                    for mesh_uuid, mesh_link in fbx_connection_map.get(skin_uuid):
                        ...
                        for object_uuid, object_link in fbx_connection_map.get(mesh_uuid):
                            ...
                            if mesh_node:
                                ...
                                # we do not support otherwise in Ixam anyway!
        ...
        # find the correction matrices to align FBX objects with their Ixam equivalent
    ...
    def _():
        ...
        for s_uuid, s_item in fbx_table_nodes.items():
            ...
            for bc_uuid, bc_ctype in fbx_connection_map.get(s_uuid, ()):
                ...
                keyblocks = ixam_read_shape(fbx_tmpl, fbx_sdata, fbx_bcdata, meshes, scene)
    ...
    if use_anim:
        ...
        def _():
            ...
            for as_uuid, fbx_asitem in fbx_table_nodes.items():
                fbx_asdata, _ixam_data = fbx_asitem
            ...
            for al_uuid, fbx_alitem in fbx_table_nodes.items():
                fbx_aldata, _ixam_data = fbx_alitem
            ...
            for acn_uuid, fbx_acnitem in fbx_table_nodes.items():
                fbx_acndata, _ixam_data = fbx_acnitem
            ...
            for ac_uuid, fbx_acitem in fbx_table_nodes.items():
                fbx_acdata, _ixam_data = fbx_acitem
            ...
            ixam_read_animations(fbx_tmpl_astack, fbx_tmpl_alayer, stacks, scene, settings.anim_offset, global_scale)
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            # In Ixam, we link materials to data, typically (meshes), while in FBX they are linked to objects...
            ...
            # TODO: add an option to link materials to objects in Ixam instead?
            done_materials = set()

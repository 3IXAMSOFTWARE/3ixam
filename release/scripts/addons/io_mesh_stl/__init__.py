...
bl_info = {
    ...
    "doc_url": "{IXAM_MANUAL_URL}/addons/import_export/mesh_stl.html",
    "support": 'OFFICIAL',
...
if "bpy" in locals():
    ...
    if "ixam_utils" in locals():
        importlib.reload(ixam_utils)
...
class ImportSTL(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        from . import ixam_utils

        paths = [os.path.join(self.directory, name.name) for name in self.files]

        scene = context.scene

        # Take into account scene's unit scale, so that 1 inch in Ixam gives 1 inch elsewhere! See T42000.
        ...
        for path in paths:
            ...
            ixam_utils.create_and_link_mesh(objName, tris, tri_nors, pts, global_matrix)
...
class ExportSTL(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        data_seq = [obj for obj in data_seq if not obj.protect]
        ...
        if self.batch_mode == 'OFF':
            faces = itertools.chain.from_iterable(
                    ixam_utils.faces_from_mesh(ob, global_matrix, self.use_mesh_modifiers)
                    for ob in data_seq)
            ...
            for ob in data_seq:
                faces = ixam_utils.faces_from_mesh(ob, global_matrix, self.use_mesh_modifiers)

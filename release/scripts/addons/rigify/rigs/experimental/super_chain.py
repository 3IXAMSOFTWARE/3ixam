...
class Rig:
    ...
    def create_pivot(self, pivot=None):
        ...
        if '.L' in pivot_name:
            prefix = strip_org(org_bones[pivot]).split('_')[0] + '_L'
        elif '.R' in pivot_name:
            prefix = strip_org(org_bones[pivot]).split('_')[0] + '_R'
        else:
            prefix = strip_org(org_bones[pivot]).split('_')[0]
    ...
    def create_chain(self):
        ...
        if not self.SINGLE_BONE:
            mch_name = copy_bone(self.obj, org(org_bones[0]),
                                 'MCH-AUTO-' + strip_org(org_bones[0]).split('_')[0] + suffix)
        ...
        for b in org_bones:     # All
            ...
            if b == org_bones[0]:
                name = get_bone_name(b.split('_')[0] + suffix, 'ctrl', 'ctrl')
            ...
            if b == org_bones[-1]:      # Add extra
                ctrl_name = get_bone_name(b.split('_')[0] + suffix, 'ctrl', 'ctrl')
        ...
        for b in org_bones:
            ...
            mch_ctrl_name = "MCH-CTRL-" + strip_org(b).split('_')[0] + suffix

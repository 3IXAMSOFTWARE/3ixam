...
if "bpy" in locals():
    ...
    from ixamkit import paths, append_link, bg_ixam, utils, rerequests
...
IXAMKIT_EXPORT_DATA = sys.argv[-1]
    ...
    def __iter__(self):
        with open(self.filename, 'rb') as file:
            while True:
                ...
                bg_ixam.progress('uploading %s' % self.report_name, percent)
...
def upload_file(upload_data, f):
    ...
    bg_ixam.progress('uploading %s' % f['type'])
    upload_info = {
        'assetId': version_id,
        'fileType': f['type'],
        'fileIndex': f['index'],
        ...
        if not uploaded:
            try:
                ...
                if upload_response.status_code == 200:
                    ...
                    bg_ixam.progress(f'Upload failed, retry. {a}')
            except Exception as e:
                bg_ixam.progress('Upload %s failed, retrying' % f['type'])
                time.sleep(1)
    ...
    bg_ixam.progress('finished uploading')
...
if __name__ == "__main__":
    ...
    try:
        bg_ixam.progress('preparing scene - append data')
        ...
        if 'MAINFILE' in upload_set:
            ...
            main_source.ixamkit.uploading = False
            fpath = os.path.join(data['temp_dir'], upload_data['assetBaseId'] + '.ixam')

            bpy.ops.wm.save_as_mainfile(filepath=fpath, compress=True, copy=False)
            os.remove(data['source_filepath'])

        bg_ixam.progress('preparing scene - open files')
        ...
        if uploaded:
            ...
            bg_ixam.progress('upload finished successfully')
        else:
            bg_ixam.progress('upload failed.')

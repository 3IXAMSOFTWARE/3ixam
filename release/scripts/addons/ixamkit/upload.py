...
if "bpy" in locals():
    ...
    from ixamkit import asset_inspector, paths, utils, bg_ixam, autothumb, version_checker, search, ui_panels, ui, \
...
IXAMKIT_EXPORT_DATA_FILE = "data.json"
...
def get_upload_data(self, context, asset_type):
    ...
    if asset_type == 'MODEL':
        ...
        eval_path_computing = "bpy.data.objects['%s'].ixamkit.uploading" % mainmodel.name
        eval_path_state = "bpy.data.objects['%s'].ixamkit.upload_state" % mainmodel.name
    ...
    if asset_type == 'SCENE':
        ...
        eval_path_computing = "bpy.data.scenes['%s'].ixamkit.uploading" % s.name
        eval_path_state = "bpy.data.scenes['%s'].ixamkit.upload_state" % s.name
        ...
        eval_path_computing = "bpy.data.materials['%s'].ixamkit.uploading" % mat.name
        eval_path_state = "bpy.data.materials['%s'].ixamkit.upload_state" % mat.name
        ...
        eval_path_computing = "bpy.data.brushes['%s'].ixamkit.uploading" % brush.name
        eval_path_state = "bpy.data.brushes['%s'].ixamkit.upload_state" % brush.name
...
def start_upload(self, context, asset_type, reupload, upload_set):
    ...
    source_filepath = os.path.join(tempdir, "export_ixamkit" + ext)
    ...
    datafile = os.path.join(tempdir, IXAMKIT_EXPORT_DATA_FILE)
    ...
    try:
        ...
        bg_ixam.add_bg_process(eval_path_computing=eval_path_computing, eval_path_state=eval_path_state,
...
class UploadOperator(Operator):
    """Tooltip"""
    bl_idname = "object.ixamkit_upload"
    bl_description = "Upload or re-upload asset + thumbnail + metadata"

    bl_label = "IxamKit asset upload"
    ...
    def execute(self, context):
        bpy.ops.object.ixamkit_auto_tags()
        props = utils.get_upload_props()

        # in case of name change, we have to reupload everything, since the name is stored in ixam file,
        # and is used for linking to scene
        if props.name_changed:
            # TODO: this needs to be replaced with new double naming scheme (metadata vs ixam data)
...
class AssetVerificationStatusChange(Operator):
    ...
    bl_idname = "object.ixamkit_change_status"
    ...
    def draw(self, context):
        ...
        layout.label(text='Really delete asset from IxamKit online storage?')

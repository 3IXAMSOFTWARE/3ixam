...
if "bpy" in locals():
    ...
    from ixamkit import tasks_queue, utils, paths, search, categories, oauth, ui
...
def write_tokens(auth_token, refresh_token, oauth_response):
    ...
    if props is not None:
        props.report = ''
    ui.add_report('IxamKit Re-Login success')
    search.get_profile()
...
class RegisterLoginOnline(bpy.types.Operator):
    ...
    bl_idname = "wm.ixamkit_login"
    bl_label = "IxamKit login or signup"
...
class Logout(bpy.types.Operator):
    ...
    bl_idname = "wm.ixamkit_logout"
    bl_label = "IxamKit logout"
    bl_options = {'REGISTER', 'UNDO'}
...
class CancelLoginOnline(bpy.types.Operator):
    ...
    bl_idname = "wm.ixamkit_login_cancel"
    bl_label = "IxamKit login cancel"
    bl_options = {'REGISTER', 'UNDO'}

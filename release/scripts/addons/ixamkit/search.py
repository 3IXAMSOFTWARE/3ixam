...
if "bpy" in locals():
    ...
    from ixamkit import paths, utils, categories, ui, bkit_oauth, version_checker, tasks_queue, rerequests
...
def scene_load(context):
    ...
    # wm['bkit_update'] = version_checker.compare_versions(ixamkit)
    categories.load_categories()
...
def timer_update():  # TODO might get moved to handle all ixamkit stuff.
    ...
    if len(search_threads) == 0 or bpy.context.scene.ixamkitUI.dragging:
        return 1
                    ...
                    if r['assetType'] == asset_type:
                        if len(r['files']) > 0:
                            ...
                            if durl and tname:
                                ...
                                # parse extra params needed for ixam here
...
class Searcher(threading.Thread):
    ...
    def run(self):
        ...
        if not params['get_next']:
            ...
            # result ordering: _score - relevance, score - IxamKit score
...
def build_query_model():
    ...
    props = bpy.context.scene.ixamkit_models
    query = {
        "asset_type": 'model',
...
def build_query_scene():
    ...
    props = bpy.context.scene.ixamkit_scene
    query = {
        "asset_type": 'scene',
        # "engine": props.search_engine,
        # "adult": props.search_adult,
...
def build_query_material():
    props = bpy.context.scene.ixamkit_mat
    query = {
        "asset_type": 'material',
...
def build_query_texture():
    props = bpy.context.scene.ixamkit_tex
    query = {
        "asset_type": 'texture',
...
def build_query_brush():
    props = bpy.context.scene.ixamkit_brush
...
def search(category='', get_next=False, author_id=''):
    ...
    mt('start')
    scene = bpy.context.scene
    uiprops = scene.ixamkitUI

    if uiprops.asset_type == 'MODEL':
        if not hasattr(scene, 'ixamkit'):
            return;
        props = scene.ixamkit_models
        query = build_query_model()

    if uiprops.asset_type == 'SCENE':
        if not hasattr(scene, 'ixamkit_scene'):
            return;
        props = scene.ixamkit_scene
        query = build_query_scene()

    if uiprops.asset_type == 'MATERIAL':
        if not hasattr(scene, 'ixamkit_mat'):
            return;
        props = scene.ixamkit_mat
        query = build_query_material()

    if uiprops.asset_type == 'TEXTURE':
        if not hasattr(scene, 'ixamkit_tex'):
            return;
        # props = scene.ixamkit_tex
        # query = build_query_texture()

    if uiprops.asset_type == 'BRUSH':
        if not hasattr(scene, 'ixamkit_brush'):
            return;
        props = scene.ixamkit_brush
    ...
    tasks_queue.add_task((ui.add_report, ('IxamKit searching....', 2)))

    props.report = 'IxamKit searching....'
...
class SearchOperator(Operator):
    """Tooltip"""
    bl_idname = "view3d.ixamkit_search"
    bl_label = "IxamKit asset search"
    ...
    def execute(self, context):
        ...
        # bpy.ops.view3d.ixamkit_asset_bar()

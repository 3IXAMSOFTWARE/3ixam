...
def get_selected_models():
    ...
    for ob in obs:
        if ob not in done:
            while ob.parent is not None and ob not in done and ob.ixamkit.asset_base_id != '' and ob.instance_collection is not None:
                done[ob] = True
                ob = ob.parent

            if ob not in parents and ob not in done:
                if ob.ixamkit.name != '' or ob.instance_collection is not None:
                    parents.append(ob)
            done[ob] = True

    #if no ixamkit - like objects were found, use the original selection.
    if len(parents) == 0:
        parents = obs
    return parents
...
def get_search_props():
    ...
    if uiprops.asset_type == 'MODEL':
        if not hasattr(scene, 'ixamkit_models'):
            return;
...
def get_upload_props():
    ...
    if ui_props.asset_type == 'SCENE':
        ...
        if bpy.context.active_object is not None and bpy.context.active_object.active_material is not None:
            return bpy.context.active_object.active_material.ixamkit
    elif ui_props.asset_type == 'TEXTURE':
        return None
    elif ui_props.asset_type == 'BRUSH':
        b = get_active_brush()
...
def load_prefs():
    ...
    fpath = paths.IXAMKIT_SETTINGS_FILENAME
...
def save_prefs(self, context):
    # first check context, so we don't do this on registration or ixam startup
    if not bpy.app.background: #(hasattr kills ixam)
        user_preferences = bpy.context.preferences.addons['ixamkit'].preferences
...
def get_brush_props(context):
    ...
    if brush is not None:
        return brush.ixamkit
    return None


def p(text, text1='', text2='', text3='', text4='', text5=''):
    '''debug printing depending on ixam's debug value'''
...
def automap(target_object=None, target_slot=None, tex_size=1, bg_exception=False, just_scale = False):
    from ixamkit import bg_ixam as bg
    s = bpy.context.scene
    mat_props = s.ixamkit_mat
        ...
        if tob.type == 'MESH':
            ...
            if not just_scale:
                bpy.ops.uv.cube_project(
                    ...
                    correct_aspect=False)  # it's * 2.0 because ixam can't tell size of a unit cube :)
...
def name_update():
    ...
    if props.name_old != props.name:
        ...
        props.name = nname
        # here we need to fix the name for 3IXAM data = ' or " give problems in path evaluation down the road.
    fname = props.name
    fname = fname.replace('\'', '')
    fname = fname.replace('\"', '')

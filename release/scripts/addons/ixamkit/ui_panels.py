...
if "bpy" in locals():
    ...
    from ixamkit import paths, ratings, utils, download, categories
...
def draw_ratings(layout, context):
    ...
    row = layout.row()
    op = row.operator("object.ixamkit_rating_upload", text="Send rating", icon='URL')
    return op
...
def draw_upload_common(layout, props, asset_type, context):
    op = layout.operator("wm.url_open", text="Read upload instructions",
                         icon='QUESTION')
    if asset_type == 'MODEL':
        op.url = paths.IXAMKIT_MODEL_UPLOAD_INSTRUCTIONS_URL
    if asset_type == 'MATERIAL':
        op.url = paths.IXAMKIT_MATERIAL_UPLOAD_INSTRUCTIONS_URL
    if asset_type == 'BRUSH':
        op.url = paths.IXAMKIT_BRUSH_UPLOAD_INSTRUCTIONS_URL
    ...
    if props.asset_base_id == '':
        ...
        op = layout.operator("object.ixamkit_upload", text=optext, icon='EXPORT')
        op.asset_type = asset_type

    if props.asset_base_id != '':
        op = layout.operator("object.ixamkit_upload", text='Reupload asset', icon='EXPORT')
        op.asset_type = asset_type
        op.reupload = True

        op = layout.operator("object.ixamkit_upload", text='Upload as new asset', icon='EXPORT')
        op.asset_type = asset_type
        op.reupload = False
...
def draw_panel_model_upload(self, context):
    ...
    if bpy.context.scene.render.engine in ('CYCLES', 'IXAM_EEVEE'):
        col.operator("object.ixamkit_generate_thumbnail", text='Generate thumbnail', icon='IMAGE')
...
def draw_panel_scene_upload(self, context):
    ...
    #     icon = 'ERROR'
    #     layout.operator("object.ixamkit_auto_tags", text='Auto fill tags', icon=icon)
    # else:
    #     layout.operator("object.ixamkit_auto_tags", text='Auto fill tags')
    ...
    #     col.operator("object.ixamkit_generate_thumbnail", text='Generate thumbnail', icon='IMAGE_COL')
...
def draw_panel_model_search(self, context):
    ...
    if props.report == 'You need Full plan to get this item.':
        layout.operator("wm.url_open", text="Get Full plan", icon='URL').url = paths.IXAMKIT_PLANS
...
class VIEW3D_PT_ixamkit_model_properties(Panel):
    bl_category = "IxamKit"
    bl_idname = "VIEW3D_PT_ixamkit_model_properties"
    ...
    def draw(self, context):
        ...
        if o.get('asset_data') is not None:
            ...
            if o.instance_type == 'COLLECTION' and o.instance_collection is not None:
                layout.operator('object.ixamkit_bring_to_scene', text='Bring to scene')

            # if 'rig' in ad['tags']:
            #     # layout.label(text = 'can make proxy')
            #     layout.operator('object.ixamkit_make_proxy', text = 'Make Armature proxy')
        #fast upload, blocked by now
        # else:
        #     op = layout.operator("object.ixamkit_upload", text='Store as private', icon='EXPORT')
        #     op.asset_type = 'MODEL'
        #     op.fast = True
        #fun override project, not finished
        # layout.operator('object.ixamkit_color_corrector')


def draw_login_progress(layout):
    layout.label(text='Login through browser')
    layout.label(text='in progress.')
    layout.operator("wm.ixamkit_login_cancel", text="Cancel", icon='CANCEL')


class VIEW3D_PT_ixamkit_profile(Panel):
    bl_category = "IxamKit"
    bl_idname = "VIEW3D_PT_ixamkit_profile"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "Profile"
    ...
    def draw(self, context):
        ...
        if user_preferences.api_key != '':
            ...
            layout.operator("wm.url_open", text="See my uploads",
                            icon='URL').url = paths.get_bkit_url() + paths.IXAMKIT_USER_ASSETS
...
def draw_panel_material_upload(self, context):
    ...
    if bpy.context.scene.render.engine in ('CYCLES', 'IXAM_EEVEE'):
        layout.operator("object.ixamkit_material_thumbnail", text='Render thumbnail with Real', icon='EXPORT')
...
def draw_login_buttons(layout):
    ...
    if user_preferences.login_attempt:
        ...
        if user_preferences.api_key == '':
            layout.operator("wm.ixamkit_login", text="Login",
                            icon='URL').signup = False
            layout.operator("wm.ixamkit_login", text="Sign up",
                            icon='URL').signup = True

        else:
            layout.operator("wm.ixamkit_login", text="Login as someone else",
                            icon='URL').signup = False
            layout.operator("wm.ixamkit_logout", text="Logout",
                            icon='URL')


class VIEW3D_PT_ixamkit_unified(Panel):
    bl_category = "IxamKit"
    bl_idname = "VIEW3D_PT_ixamkit_unified"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "IxamKit"
    ...
    def draw(self, context):
        ...
        if len(user_preferences.api_key) < 20 and user_preferences.asset_counter > 20:
            if user_preferences.enable_oauth:
                ...
                op = layout.operator("wm.url_open", text="Get your API Key",
                                     icon='QUESTION')
                op.url = paths.IXAMKIT_SIGNUP_URL
        ...
        if wm.get('bkit_update'):
            ...
            layout.operator("wm.url_open", text="Get new version",
                            icon='URL').url = paths.IXAMKIT_ADDON_FILE_URL
            layout.separator()
            layout.separator()
            layout.separator()
        if ui_props.down_up == 'SEARCH':
            ...
            if not ui_props.assetbar_on:
                ...
                op = row.operator('view3d.ixamkit_asset_bar', text=text, icon=icon)

            else:

                op = layout.operator('view3d.ixamkit_asset_bar', text='Hide AssetBar - ;', icon='EXPORT')
            ...
            op = layout.operator('view3d.ixamkit_asset_bar', text=text, icon='EXPORT')
            op.keep_running = False
            op.do_search = False
            e = s.render.engine
            if e not in ('CYCLES', 'IXAM_EEVEE'):
                rtext = 'Only Real and Play render engines are currently supported. ' \
                        'Please use Real for all assets you upload to 3IXAMKit.'
            ...
            if ui_props.asset_type == 'MATERIAL':
                if bpy.context.active_object is not None and \
                        ...
                        bpy.context.active_object.active_material.ixamkit.asset_base_id != '':
                    layout.label(text=bpy.context.active_object.active_material.ixamkit.name + ' :')
                    # noinspection PyCallByClass
...
class OBJECT_MT_ixamkit_asset_menu(bpy.types.Menu):
    bl_label = "Asset options:"
    bl_idname = "OBJECT_MT_ixamkit_asset_menu"

    def draw(self, context):
        layout = self.layout
        ui_props = context.scene.ixamkitUI
        ...
        if wm.get('bkit authors') is not None:
            ...
            if a is not None:
                ...
                op = layout.operator('view3d.ixamkit_search', text="Show Assets By Author")
                op.keywords = ''
                op.author_id = author_id

        op = layout.operator('view3d.ixamkit_search', text='Search Similar')
        ...
        if bpy.context.active_object is not None and ui_props.asset_type == 'MODEL':
            ...
            op = layout.operator('scene.ixamkit_download', text='Replace Active Models')
        ...
        if profile is not None:
            ...
            if profile['user']['id'] == 2:
                if asset_data['verificationStatus'] != 'validated':
                    op = layout.operator('object.ixamkit_change_status', text='Validate')
            ...
            if author_id == str(profile['user']['id']):
                ...
                op = row.operator('object.ixamkit_change_status', text='Delete')
                op.asset_id = asset_data['id']
                op.state = 'deleted'
            # else:
...
class SetCategoryOperator(bpy.types.Operator):
    """Visit subcategory"""
    bl_idname = "view3d.ixamkit_set_category"
    bl_label = "IxamKit Set Active Category"
...
def draw_panel_categories(self, context):
    ...
    if wm.get('active_category') is not None:
        ...
        if len(acat) > 1:
            ...
            op = col.operator('view3d.ixamkit_set_category', text='...', icon='FILE_PARENT')
            op.asset_type = ui_props.asset_type
            op.category = ''
    ...
    #     op = col.operator('view3d.ixamkit_asset_bar', text='freebies')
    #     op.free_only = True
    ...
    for c in cats['children']:
        if c['assetCount'] > 0:
            ...
            op = row.operator('view3d.ixamkit_asset_bar', text=ctext)
            op.do_search = True
            ...
            if len(c['children']) > 0 and c['assetCount'] > 15:
                # row = row.split()
                op = row.operator('view3d.ixamkit_set_category', text='>>')
                op.asset_type = ui_props.asset_type
                op.category = c['slug']
                ...
                #         op = row.operator('view3d.ixamkit_search', text=ctext)
                #         op.category = c1['slug']


class VIEW3D_PT_ixamkit_downloads(Panel):
    bl_category = "IxamKit"
    bl_idname = "VIEW3D_PT_ixamkit_downloads"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "Downloads"
    ...
    def draw(self, context):
        ...
        for threaddata in download.download_threads:
            ...
            row.operator('scene.ixamkit_download_kill', text='', icon='CANCEL')
...
classess = (
    ...
    VIEW3D_PT_ixamkit_unified,
    VIEW3D_PT_ixamkit_model_properties,
    VIEW3D_PT_ixamkit_downloads,
    VIEW3D_PT_ixamkit_profile,
    OBJECT_MT_ixamkit_asset_menu
)

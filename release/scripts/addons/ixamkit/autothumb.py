...
if "bpy" in locals():
    ...
    from ixamkit import paths, utils, bg_ixam
...
def start_thumbnailer(self, context):
    ...
    mainmodel.ixamkit.is_generating_thumbnail = True
    mainmodel.ixamkit.thumbnail_generating_state = 'starting ixam instance'
    ...
    filepath = os.path.join(tempdir, "thumbnailer_ixamkit" + ext)
    ...
    try:
        ...
        eval_path_computing = "bpy.data.objects['%s'].ixamkit.is_generating_thumbnail" % mainmodel.name
        eval_path_state = "bpy.data.objects['%s'].ixamkit.thumbnail_generating_state" % mainmodel.name
        ...
        mainmodel.ixamkit.thumbnail = rel_thumb_path + '.jpg'
        mainmodel.ixamkit.thumbnail_generating_state = 'Saving .ixam file'
...
def start_material_thumbnailer(self, context):
    # Prepare to save the file
    mat = bpy.context.active_object.active_material
    mat.ixamkit.is_generating_thumbnail = True
    mat.ixamkit.thumbnail_generating_state = 'starting ixam instance'
    ...
    try:
        ...
        eval_path_computing = "bpy.data.materials['%s'].ixamkit.is_generating_thumbnail" % mat.name
        eval_path_state = "bpy.data.materials['%s'].ixamkit.thumbnail_generating_state" % mat.name
        eval_path = "bpy.data.materials['%s']" % mat.name

        bg_ixam.add_bg_process(eval_path_computing=eval_path_computing, eval_path_state=eval_path_state,
                                  eval_path=eval_path, process_type='THUMBNAILER', process=proc)

        mat.ixamkit.thumbnail = rel_thumb_path + '.png'
        mat.ixamkit.thumbnail_generating_state = 'Saving .ixam file'
    except Exception as e:
...
class GenerateThumbnailOperator(bpy.types.Operator):
    ...
    bl_idname = "object.ixamkit_generate_thumbnail"
    bl_label = "IxamKit Thumbnail Generator"
...
class GenerateMaterialThumbnailOperator(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "object.ixamkit_material_thumbnail"
    bl_label = "IxamKit Material Thumbnail Generator"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def draw(self, context):
        layout = self.layout
        props = bpy.context.active_object.active_material.ixamkit

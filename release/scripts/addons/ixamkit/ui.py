...
if "bpy" in locals():
    ...
    bg_ixam = importlib.reload(bg_ixam)
    colors = importlib.reload(colors)
else:
    from ixamkit import paths, ratings, utils, search, upload, ui_bgl, download, bg_ixam, colors
...
def draw_ratings_bgl():
    # return;
    ui = bpy.context.scene.ixamkitUI
...
def draw_callback_2d_progress(self, context):
    ...
    for threaddata in download.download_threads:
        ...
        if tcom.passargs.get('downloaders'):
            ...
            index += 1
    for process in bg_ixam.bg_processes:
        tcom = process[1]
...
def draw_callback_3d(self, context):
    ''' Draw snapped bbox while dragging and in the future other ixamkit related stuff. '''
    ui = context.scene.ixamkitUI
...
class AssetBarOperator(bpy.types.Operator):
    ...
    bl_idname = "view3d.ixamkit_asset_bar"
    bl_label = "IxamKit Asset Bar UI"
    ...
    def modal(self, context, event):
        ...
        if event.type == 'WHEELUPMOUSE' or event.type == 'WHEELDOWNMOUSE' or event.type == 'TRACKPADPAN':
            ...
            if ui_props.dragging and not mouse_in_asset_bar(mx, my):  # and my < r.height - ui_props.bar_height \
                ...
                sprops = bpy.context.scene.ixamkit_models
            ...
            # note - TRACKPADPAN is unsupported in ixam by now.
        ...
        if event.type == 'RIGHTMOUSE':
            ...
            if event.value == 'PRESS' and mouse_in_asset_bar(mx, my):
                bpy.ops.wm.call_menu(name='OBJECT_MT_ixamkit_asset_menu')
                return {'RUNNING_MODAL'}
                ...
                if asset_search_index > -3:
                    if ui_props.asset_type == 'MATERIAL':
                        if target_object != '':
                            ...
                            bpy.ops.scene.ixamkit_download(True,
                        ...
                        bpy.ops.scene.ixamkit_download(asset_type=ui_props.asset_type,
                                                          asset_index=asset_search_index)
...
class TransferIxamkitData(bpy.types.Operator):
    """Regenerate cobweb"""
    bl_idname = "object.ixamkit_data_trasnfer"
    bl_label = "Transfer IxamKit data"
    bl_description = "Transfer ixamKit metadata from one object to another when fixing uploads with wrong parenting."
    ...
    def execute(self, context):
        ...
        for target_ob in bpy.context.selected_objects:
            if target_ob != source_ob:
                target_ob.property_unset('ixamkit')
                for k in source_ob.keys():
                    target_ob[k] = source_ob[k]
        source_ob.property_unset('ixamkit')
        return {'FINISHED'}

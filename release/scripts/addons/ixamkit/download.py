...
if "bpy" in locals():
    ...
    from ixamkit import paths, append_link, utils, ui, colors, tasks_queue, rerequests
...
def scene_save(context):
    ''' does cleanup of 3IXAMkit props and sends a message to the server about assets used.'''
...
def report_usages():
    ...
    url = paths.get_api_url() + paths.IXAMKIT_REPORT_URL
...
def append_asset(asset_data, **kwargs):  # downloaders=[], location=None,
    ...
    if asset_data['asset_type'] == 'model':
        ...
        # set consistency for objects already in scene, otherwise this literally breaks ixam :)
    ...
    parent['asset_data'] = asset_data  # TODO remove this??? should write to 3IXAMkit Props?
    # moving reporting to on save.
    # report_use_success(asset_data['id'])


@bpy.app.handlers.persistent
def timer_update():  # TODO might get moved to handle all ixamkit stuff, not to slow down.
...
class Downloader(threading.Thread):
    ...
    def run(self):
        '''try to download file from ixamkit'''
...
def download(asset_data, **kwargs):
    ...
    # incoming data can be either directly dict from python, or ixam id property
...
class IxamkitKillDownloadOperator(bpy.types.Operator):
    """Kill a download."""
    bl_idname = "scene.ixamkit_download_kill"
    bl_label = "IxamKit Kill Asset Download"
...
class IxamkitDownloadOperator(bpy.types.Operator):
    """Download and link asset to scene. Only link if asset already available locally."""
    bl_idname = "scene.ixamkit_download"
    bl_label = "IxamKit Asset Download"
    ...
    #     return bpy.context.window_manager.IxamKitModelThumbnails is not ''

    def execute(self, context):
        s = bpy.context.scene
        sr = s['search results']

        asset_data = sr[self.asset_index].to_dict()  # TODO CHECK ALL OCCURRENCES OF PASSING IXAM ID PROPS TO THREADS!
...
def register_download():
    bpy.utils.register_class(IxamkitDownloadOperator)
    bpy.utils.register_class(IxamkitKillDownloadOperator)
...
def unregister_download():
    bpy.utils.unregister_class(IxamkitDownloadOperator)
    bpy.utils.unregister_class(IxamkitKillDownloadOperator)

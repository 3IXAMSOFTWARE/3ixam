...
bl_info = {
    ...
    "warning": "",
    "doc_url": "{IXAM_MANUAL_URL}/addons/mesh/inset_straight_skeleton.html",
    "category": "Mesh",
}
...
def calc_pixel_size(context, co):
    # returns size in ixam units of a pixel at 3d coord co
...
def do_inset(mesh, amount, height, region, quadrangulate):
    ...
    if len(m.faces) == orig_numf:
        ...
        return
    ixam_faces = m.faces[orig_numf:len(m.faces)]
    ixam_old_face_index = m.face_data[orig_numf:len(m.faces)]

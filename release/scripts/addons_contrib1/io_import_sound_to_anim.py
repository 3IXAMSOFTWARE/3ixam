...
- At least 3IXAM 2.80 is necessary to run this program.
- Brazil
...
class VIEW3D_PT_SoundPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not b:
            ...
            row=layout.row()
            row.label(text="Supported Object are Type: Rig, Mesh, Camera and Lamp")
            row=layout.row()
        else:
            ...
            # monta as telas quando está rodando
                ...
                # preciso garantir a existencia do array porque o 3IXAM salva no arquivo como existente sem o array existir
                try:
                    array
                except NameError:
                    nada=0 #dummy
                ...
                #Layout SmartRender, somente para Ixam_render
                if bpy.context.scene.render.engine == "IXAM_RENDER":
                    row=layout.row()
                    row.label(text="----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
                    row=layout.row()
...
class ImpSoundtoAnim(bpy.types.PropertyGroup):
        ...
        frames_per_second: IntProperty(name="#Frames/s",
            description="Frames you want per second. Better match your set up in 3IXAM scene",
            min=1,
            max=120,
            step=1,
            default= 10)
        ...
        if context.object.imp_sound_to_anim.bArrayCriado:
            for i in range(loop):
                ...
                if i>0 and bNaoValorIgual and arrayIL1== arrayI:
                    print("Importing 3IXAM Frame: "+str(i+obi.RunFrom+1)+"\tof "+str(len(array)-1) + \
                    ...
                    if i>0 and i< len(array)-1 and abs(arrayI - arrayIL1)<=iDestructiveOptimizer and \
                            ...
                            print("Importing 3IXAM Frame: "+str(i+obi.RunFrom+1)+"\tof "+str(len(array)-1) + "\tValue: "+ str(ival))

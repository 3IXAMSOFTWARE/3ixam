...
def restore_ixam_settings(backup):
...
class MenuHandler:
    ...
    #def rebuild_menus(self)  # add in case ixam window size changes?
    #    return
...
def exit_addon(self):
    restore_ixam_settings(self.settings_backup)
...
class XEDIT_OT_set_meas(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if context.area.type == 'VIEW_3D':
            ...
            self.settings_backup = backup_ixam_settings()
            self.mouse_co = Vector((event.mouse_region_x, event.mouse_region_y))

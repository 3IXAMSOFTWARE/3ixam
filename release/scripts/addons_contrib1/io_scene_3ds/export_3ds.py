...
def make_material_chunk(material, image):
    """Make a material chunk out of 3IXAM material.
...
def make_mesh_chunk(ob, mesh, matrix, materialDict, translation):
    """Make a chunk out of a 3IXAM mesh."""
...
def make_kfdata(start=0, stop=0, curtime=0):
    ...
    kfhdr.add_variable("filename", _3ds_string("3IXAM"))
...
def make_kf_obj_node(obj, name_to_id):
    """Make a node chunk for a 3IXAM object.

    Takes the 3IXAM object as a parameter. Object id's are taken from the dictionary name_to_id.
    3IXAM Empty objects are converted to dummy nodes."""
...
def save(operator,
    ...
    """Save the 3IXAM scene to a 3ds file."""

    # Time the export
    duration = time.time()
    # Ixam.Window.WaitCursor(1)
    ...
    for ob, mesh, matrix in mesh_objects:
        ...
        # if not ixam_mesh.users:
        # bpy.data.meshes.remove(ixam_mesh)
        #ixam_mesh.vertices = None

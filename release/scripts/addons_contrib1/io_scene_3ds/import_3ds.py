...
def add_texture_to_material(image, contextWrapper, pct, extend, alpha, scale, offset, angle, tintcolor, mapto):
    ...
    if extend == 'mirror':
        ...
        # 3DS' decal mode maps best to 3IXAMs EXTEND
...
def process_next_chunk(context, file, previous_chunk, imported_objects, IMAGE_SEARCH, KEYFRAME):
    ...
    while (previous_chunk.bytes_read < previous_chunk.length):
        ...
        # is it a material chunk?
            ...
            # ...why not? just set CreateIxamObject to False
...
#   time1 = Ixam.sys.time()
    ...
    # REMOVE DUMMYVERT, - remove this in the next release when ixams internal are fixed.

...
def simplifyCurves(curveGroup, error, reparaError, maxIterations, group_mode):
    ...
    def createNewCurves(curveGroup, beziers, group_mode):
        ...
        #insert the calculated beziers to 3IXAM data.
        if group_mode:
...
#IN: perfromer_obj, enduser_obj, 3IXAM objects whose .data is an armature.
...
# IN: context (bpy.context, etc.), and 2 ixam objects which are armatures
...
# Reparameterizes 3IXAM path via keyframing it's eval_time to match a stride_object's forward velocity.
# IN: Context, stride object (ixam object with location keyframes), path object.
...
#IN: enduser_obj, 3IXAM armature that has had two retargets applied.

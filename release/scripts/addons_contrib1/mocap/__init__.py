...
bl_info = {
    ...
    "ixam": (2, 80, 0),
    "version": (1, 1, 2),
    "location": "Active Rig > Object Properties > Mocap Tools",
    "description": "Various tools for working with motion capture animation",
    "warning": "",
    "doc_url": "http://wiki.3ixam.com/index.php/Extensions:2.6/Py/"
...
class MocapConstraint(bpy.types.PropertyGroup):
    ...
    constrained_bone: StringProperty(name="Joint",
        default="",
        description="Constrained joint",
        update=mocap_constraints.updateConstraintBoneType)
    constrained_boneB: StringProperty(name="Joint (2)",
        default="",
        description="Other Constrained joint (optional, depends on type)",
        update=mocap_constraints.setConstraint)
    ...
    targetSpace: EnumProperty(
        items=[("WORLD", "World Space", "Evaluate target in global space"),
            ...
            ("constrained_boneB", "Other Joint Space", "Evaluate target in specified other joint space")],
        name="Space",
        description="In which space should Point type target be evaluated",
        update=mocap_constraints.setConstraint)
    type: EnumProperty(name="Type of constraint",
        items=[("point", "Maintain Position", "Joint is at a specific point"),
            ("freeze", "Maintain Position at frame", "Joint does not move from location specified in target frame"),
            ("floor", "Stay above", "Joint does not cross specified mesh object eg floor"),
            ("distance", "Maintain distance", "Target joints maintained specified distance")],
...
class AnimationStitchSettings(bpy.types.PropertyGroup):
    ...
    second_offset: IntProperty(name="Second offset",
            ...
            default=10)
    stick_bone: StringProperty(name="Stick Joint",
            description="Joint to freeze during transition",
            default="")
...
def advancedRetargetToggle(self, context):
    ...
    if enduser_obj is None or len(performer_obj) != 1:
        print("Need active and selected rigs")
        return
    else:
...
def updateIKRetarget():
    # ensures that Ixam constraints and IK properties are in sync
...
class RetargetOperator(bpy.types.Operator):
    ...
    """Retarget animation from selected rig to active rig"""
...
class RotateFixArmatureOperator(bpy.types.Operator):
    ...
    """Realign the active armature's axis system to match 3IXAM """ \
...
def register():
    ...
    bpy.types.Bone.foot = BoolProperty(name="Foot",
        description="Mark this joint as a 'foot', which determines retargeted animation's translation",
        default=False)
    ...
    bpy.types.Bone.twistFix = BoolProperty(name="Twist Fix",
        description="Fix Twist on this joint",
        default=False)
    ...
    bpy.types.Bone.poseSpace = BoolProperty(name="Pose Space",
        ...
        default=False)
    #Boolean property for toggling ik retargeting for this joint
    bpy.types.PoseBone.IKRetarget = BoolProperty(name="IK",
        description="Toggle IK Retargeting method for given joint",

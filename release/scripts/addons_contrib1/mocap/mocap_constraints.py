...
def addNewConstraint(m_constraint, cons_obj):
     #Decide the correct 3IXAM constraint according to the Mocap constraint type
...
def removeConstraint(m_constraint, cons_obj):
    #remove the influence fcurve and 3IXAM constraint
...
# Function that copies all settings from m_constraint to the real 3IXAM constraints
# Is only called when ixam constraint already exists
...
def setConstraint(m_constraint, context):
    ...
    s -= s_in
    e += s_out
    #Set the ixam constraint parameters
...
def bakeConstraints(context):
    ...
    for m_constraint in obj.data.mocap_constraints:
        ...
        if not isinstance(cons_obj, bpy.types.PoseBone):
            ...
            if ik_con:
                ik_con.mute = True
        # Deactivate related 3IXAM Constraint

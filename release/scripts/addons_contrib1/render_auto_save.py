...
bl_info = {
    ...
    "warning": "",
    "doc_url": "http://wiki.3ixam.com/index.php/Extensions:2.6/Py/Scripts/Render/Auto_Save",
    "tracker_url": "https://developer.3ixam.com/maniphest/task/edit/form/2/",
    "category": "Render"}
...
def auto_save_render(scene):
    ...
    ixamname = basename(bpy.data.filepath).rpartition('.')[0]
    ...
    if scene.auto_save_subfolders:
        filepath = join(filepath, ixamname)
        if not exists(filepath):
            mkdir(filepath)

    # imagefiles starting with the ixamname
    files = [f for f in listdir(filepath)
             if f.startswith(ixamname)
    ...
    def save_number_from_files(files):
        ...
        if files:
            for f in files:
                ...
                suffix = findall(r'\d+', f.split(ixamname)[-1])
                if suffix:
    ...
    if scene.auto_save_use_framenumber:
        ...
        save_name = '_'.join([ixamname, frame_number, save_number])
    else:
        save_number = save_number_from_files(files)
        save_name = '_'.join([ixamname, save_number])
    save_name += extension
    ...
    if scene.auto_save_ixam:
        save_name_ixam = join(filepath, save_name) + '.ixam'
        print('Ixam_Save:', save_name_ixam)
        bpy.ops.wm.save_as_mainfile(filepath=save_name_ixam, copy=True)
...
class RENDER_PT_render_auto_save(Panel):
    ...
    COMPAT_ENGINES = {'CYCLES', 'IXAM_RENDER',
                      'IXAM_EEVEE', 'IXAM_OPENGL'}
    ...
    def draw(self, context):
        ...
        col.prop(context.scene, 'auto_save_ixam', toggle=False)
...
def register():
    ...
    bpy.types.Scene.auto_save_ixam = BoolProperty(
        name='Save .ixam (copy) alongside image',
        default=False,
        description='Also save .ixam (copy) file into: //auto_save/')
    ...
    bpy.types.Scene.auto_save_subfolders = BoolProperty(
        ...
        default=False,
        description='Save into individual subfolders per ixam name')

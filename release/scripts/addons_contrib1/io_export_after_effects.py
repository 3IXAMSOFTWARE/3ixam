...
def get_comp_data(context):
    """Create list of static ixam's data"""
    scene = context.scene
...
def convert_transform_matrix(matrix, width, height, aspect,
    ...
    """Convert from 3IXAM's Location, Rotation and Scale
    ...
    # Get ixam transform data for ob
    ...
    rx =  degrees(b_rot.x)  # AE's X orientation =  ixam's X rotation if 'ZYX' euler.
    ry = -degrees(b_rot.y)  # AE's Y orientation = -ixam's Y rotation if 'ZYX' euler
    rz = -degrees(b_rot.z)  # AE's Z orientation = -ixam's Z rotation if 'ZYX' euler
    if x_rot_correction:
        # In 3IXAM, ob of zero rotation lays on floor.
...
def write_jsx_file(file, data, selection, include_animation,
    ...
    # nulls representing ixam's obs except cameras, lights and solids
    ...
    for frame in range(data['start'], end):
        ...
        if include_active_cam and data['active_cam_frames'] != []:
            ...
            # Convert 3IXAM's lens to AE's zoom in pixels
    ...
    # Wrap in function
    jsx_file.write("function compFromIxam(){\n")
    # Create new comp
    ...
    jsx_file.write('\n}else{alert ("Exit Import 3IXAM animation data \\nNo Comp name has been chosen","EXIT")};')
    # Close function
    jsx_file.write("}\n\n\n")
    # Execute function. Wrap in "undo group" for easy undoing import process
    jsx_file.write('app.beginUndoGroup("Import 3IXAM animation data");\n')
    jsx_file.write('compFromIxam();\n')  # Execute function
    jsx_file.write('app.endUndoGroup();\n\n\n')
    jsx_file.close()

    # Set current frame of animation in ixam to state before export

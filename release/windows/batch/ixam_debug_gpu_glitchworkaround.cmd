@echo off
echo Starting ixam with GPU debugging and glitch workaround options, log files 
echo will be created in your temp folder, windows explorer will open after you 
echo close ixam to help you find them.
echo.
...
set PYTHONPATH=
"%~dp0\ixam" --debug --debug-gpu --debug-gpu-force-workarounds --python-expr "import bpy; bpy.ops.wm.sysinfo(filepath=r'%temp%\ixam\debug_logs\ixam_system_info.txt')" > "%temp%\ixam\debug_logs\ixam_debug_output.txt" 2>&1 < %0

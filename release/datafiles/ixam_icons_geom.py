...
Command line::
   ...
   ./ixam.bin \
       icon_file.ixam --background --python ./release/datafiles/ixam_icons_geom.py -- \
       --output-dir=./release/datafiles/ixam_icons_geom
...
def mesh_data_lists_from_objects(ob_parent, ob_children):
    ...
    for ob in (ob_parent, *ob_children):
        with TriMesh(ob) as me:
            ...
            print(f"Converting object {ob.name} with mesh {me.name}")
            tris_data.extend(
